...
async def test_22_admin_user_exists(db_session, lei_14_24_data):
    ...
    assert user is not None, f"Usuário admin não encontrado"
...
async def test_29_circular_references_check(db_session):
    ...
    count = await db_session.fetchval(
        "SELECT COUNT(*) FROM locations WHERE id = parent_id"
    )
...
async def test_30_all_provinces_have_null_parent(db_session):
    ...
    assert len(provinces_with_parent) == 0, f"Encontradas províncias com parent: {provinces_with_parent}"
