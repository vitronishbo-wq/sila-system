...
class RoutesHealthMonitor:
        ...
        for py_file in py_files:
            try:
                ...
                if 'response_model=' in content:
                    self.stats['with_response_model'] += len(re.findall(r'response_model\s*=', content))
    ...
    def check_openapi(self) -> Dict:
        ...
        try:
            ...
            documented = sum(1 for p, methods in paths.items() 
    ...
    def generate_report(self):
        ...
        for method, count in sorted(self.stats['by_method'].items(), key=lambda x: -x[1]):
            pct = (count * 100) // self.stats['total_endpoints'] if self.stats['total_endpoints'] > 0 else 0
            bar = "█" * (count // 5)
        ...
        if openapi['status'] == 'found':
            ...
            print(f"  Gap: {self.stats['total_endpoints'] - openapi['total_paths']} endpoints not in OpenAPI")
        else:
        ...
        print(f"  Response Models:  {self.stats['with_response_model']} ({self.stats['with_response_model']*100//total}%)")
        print(f"  Error Handling:   {self.stats['with_error_handling']} ({self.stats['with_error_handling']*100//total}%)")
        print(f"  Type Hints:       {self.stats['with_type_hints']} ({self.stats['with_type_hints']*100//total}%)")
        print(f"  Docstrings:       {self.stats['with_docstring']} ({self.stats['with_docstring']*100//total}%)")
        ...
        if score < 70:
            ...
            print("     1. Complete OpenAPI documentation (current: {:.1f}%)".format(openapi['documented']*100/total if total else 0))
