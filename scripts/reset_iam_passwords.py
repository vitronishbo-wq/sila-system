    ...
    try:
        ...
        async with SessionLocal() as session:
            ...
            update_result = await session.execute(
                ...
                RETURNING id, email
            """),
                {"password_hash": password_hash, "now": datetime.utcnow()},
            )
            ...
            for _user_id, email in updated_users:
                print(f"  ✅ {email}")
...
async def test_password(username: str, universal_password: str):
    ...
    try:
        async with SessionLocal() as session:
            ...
            # Test password
            if bcrypt.checkpw(universal_password.encode("utf-8"), stored_hash.encode("utf-8")):
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Phase 5 - Password Reset Script")
    parser.add_argument(
        "--password", default="Sila_1983", help="Universal password to set (default: Sila_1983)"
    )
    parser.add_argument("--test", action="store_true", help="Test password after reset")
    parser.add_argument("--status", action="store_true", help="Show password status")
    parser.add_argument("--test-user", help="Test password for specific user")
