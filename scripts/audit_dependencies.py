...
class DependencyAuditor:
    ...
    def audit(self):
        ...
        self.modules = sorted([d.name for d in self.modules_path.iterdir() 
    ...
    def _extract_module_name(self, import_path: str) -> str:
        ...
        # Handle 'app.modules.identity.domain' -> 'identity'
        ...
        for module_name in self.modules:
            ...
            for py_file in module_path.rglob("*.py"):
                ...
                try:
                    ...
                    if "from app.modules" in content or "from ..application" in content or "from ..infrastructure" in content:
                        self.violations.append(f"ISOLATION: {module_name}/domain imports from application/infrastructure")
                ...
                try:
                    ...
                    for other_module in self.modules:
                        if other_module != module_name:
                            pattern = f"from app.modules.{other_module}.infrastructure"
                            if pattern in content:
                                self.violations.append(f"COUPLING: {module_name}/infrastructure imports from {other_module}/infrastructure")
                                violations_found = True
