...
def find_alembic_ini():
    ...
    candidates = [
        ...
        workspace / "apps" / "backend" / "alembic.ini"
    ]
...
def main():
    ...
    if alembic_ini:
        ...
        versions_dir = Path(__file__).parent.parent / "apps" / "backend" / "app" / "db" / "migrations" / "versions"
    ...
    print(f"  1. cd /home/dev03wsl/sila-system")
    print(f"  2. alembic upgrade head")
    print(f"")
    print(f"  This will apply the merge and consolidate your migration history")
    print(f"  into a single linear chain.")
    
    print(f"\n⚠️  IMPORTANT:")
    print(f"  - Backup your database BEFORE running upgrade")
    print(f"  - The merge migration makes NO schema changes")
    print(f"  - It only consolidates migration history")
    print(f"  - After upgrade, all future migrations will be linear")
