...
def generate_queue_config(config_reqs):
    ...
    for module in config_reqs.get("async_handlers", []):
        ...
        queues["bindings"].append(
            {"queue": queue_name, "exchange": "sila.domain.events", "routing_key": f"{module}.*"}
        )
    ...
    for module in config_reqs.get("command_handlers", []):
        ...
        queues["queues"].append(
            {"name": queue_name, "durable": True, "arguments": {"x-max-length": 1000}}
        )
        queues["bindings"].append(
            {"queue": queue_name, "exchange": "sila.commands", "routing_key": module}
        )
...
def validate_broker_connectivity():
    ...
    broker_checks = {"host": "localhost", "port": 5672, "vhost": "/sila", "status": "CONFIGURED"}
...
def main():
    ...
    config_items = (
        len(queue_config["exchanges"]) + len(queue_config["queues"]) + len(queue_config["bindings"])
    )
