...
class UserMigrator:
    ...
    async def migrate_user(self, session, legacy_user):
        ...
        try:
            (user_id, uuid_val, email, hashed_password, full_name,
             phone, is_active, status, roles_json, bi_number,
             region_id, admin_level, created_at, updated_at) = legacy_user
            ...
            for legacy_role in legacy_roles:
                ...
                if not iam_role:
                    logger.warning(
                        f"Unknown legacy role '{normalized_role}' for {email}, "
                        f"skipping"
                    )
                    continue
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description='Phase 5: User Data Migration Script'
    )
    parser.add_argument(
        '--user-id',
        type=int,
        help='Migrate only a specific user by ID (useful for testing)'
    )
    parser.add_argument(
        '--migrate',
        action='store_true',
        default=True,
        help='Execute migration (default)'
    )
    parser.add_argument(
        '--rollback',
        action='store_true',
