...
for router_file in sorted(router_files):
    ...
    if "from apps.backend.app.core.observability" not in content:
        ...
        # Inserir novo import
        lines.insert(import_end, "from apps.backend.app.core.observability import observability_middleware")
        content = "\n".join(lines)
...
for service_file in sorted(service_files):
    ...
    # Adicionar import se necessário
    if "@trace" in content and "from apps.backend.app.core.observability" not in content:
        lines = content.split("\n")
        import_end = 0
        ...
        # Inserir novo import
        lines.insert(import_end, "from apps.backend.app.core.observability import trace")
        content = "\n".join(lines)
    ...
    for i, line in enumerate(lines):
        ...
        if (
            line.strip().startswith(("async def", "def"))
            and not line.strip().startswith(("async def __", "def __"))  # Skip dunder methods
            and (i == 0 or not lines[i - 1].strip().startswith("@"))
            and "async def" in line
        ):  # Apenas métodos async
