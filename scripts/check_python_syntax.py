...
def auto_fix_common_syntax_errors(source, file_path):
    ...
    if re.search(fstring_pattern, source):
        ...
        fixed = True
        print("   🔧 Corrigido f-string quebrado")
    ...
    if re.search(unmatched_brackets, source):
        ...
        fixed = True
        print("   🔧 Corrigido colchetes em excesso")
    ...
    if re.search(func_no_colon, source, re.MULTILINE):
        ...
        fixed = True
        print("   🔧 Adicionado dois-pontos em definição de função")
    ...
    if re.search(missing_comma, source):
        ...
        fixed = True
        print("   🔧 Adicionada vírgula ausente")
...
def check_python_syntax(file_path, auto_fix=False):
    ...
    try:
        ...
        try:
            ...
            if auto_fix:
                print(f"🔧 {file_path.relative_to(Path.cwd())} - Tentando correção automática...")
                fixed_source, was_fixed = auto_fix_common_syntax_errors(source, file_path)
...
def main():
    ...
    parser.add_argument("--fix", action="store_true", help="Corrigir automaticamente erros comuns")
