...
class SQLiteRemovalEngine:
        ...
        for file_path in python_files:
            ...
            try:
                ...
                if "sqlite" in content.lower():
                    lines = content.split("\n")
                    sqlite_lines = [
                        (i + 1, line) for i, line in enumerate(lines) if "sqlite" in line.lower()
                    ]
                    if sqlite_lines:
    ...
    def analyze(self) -> RemovalReport:
        ...
        print("[ANALISE] Referencias SQLite:")
        print(f"{'=' * 60}")
    ...
    def create_backup(self) -> str:
        ...
        backup_dir = (
            Path(tempfile.gettempdir()) / f"sila_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _process_env_files(self):
        ...
        for env_file in env_files:
            ...
            patterns = [
                (
                    r"DATABASE_URL=postgresql+asyncpg://localhost/sila_test",
                    "DATABASE_URL=postgresql+asyncpg://localhost/sila_test",
                ),
                (
                    r"ASYNC_DATABASE_URL=sqlite\+aiopostgresql+asyncpg://localhost/sila_test",
                    "ASYNC_DATABASE_URL=postgresql+asyncpg://localhost/sila_test",
                ),
            ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ferramenta automatica de remocao de SQLite")

    parser.add_argument("--analyze", action="store_true", help="Apenas analisa referencias SQLite")
    parser.add_argument("--execute", action="store_true", help="Executa remocao completa de SQLite")
    parser.add_argument(
        "--validate", action="store_true", help="Valida se SQLite foi completamente removido"
    )
    parser.add_argument(
        "--workspace",
        type=str,
