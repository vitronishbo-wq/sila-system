...
def main() -> None:
    ...
    root_example_keys = load_env_keys(root_example)
    ...
    missing_critical = {
        env_name: sorted(list(critical_keys - keys))
        for env_name, keys in backend_envs.items()
    }

    for env_name, missing in missing_critical.items():
        if missing:
            problems[f"apps/backend/config/{env_name}"] = (
                "Chaves críticas ausentes: " + ", ".join(missing)
            )
