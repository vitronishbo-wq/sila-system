...
async def approve_document(
    ...
    decision = policy_engine.authorize(
        user_roles={user_role}, resource="documents", action="approve"
    )

    if not decision:
        raise HTTPException(status_code=403, detail=f"Cannot approve documents: {decision.reason}")
...
async def delete_document(
    ...
    if user_role not in allowed_roles:
        raise HTTPException(
            status_code=403,
            detail=f"Role '{user_role}' cannot delete documents. Required: {allowed_roles}",
        )
...
async def get_document(
    ...
    decision = policy_engine.authorize(user_roles={user_role}, resource="documents", action="read")
...
class DocumentService:
    ...
    def create_document(self, title: str, user_role: str) -> dict:
        ...
        decision = self.policy_engine.authorize(
            user_roles={user_role}, resource="documents", action="create"
        )
...
def generate_auth_tokens(user_id: str, role: str) -> dict:
    ...
    access_token = jwt_handler.create_access_token(
        subject=user_id, additional_claims={"role": role}
    )

    # Or with custom expiration
    jwt_handler.create_access_token(
        subject=user_id, additional_claims={"role": role}, expires_delta=timedelta(hours=1)
    )
...
def setup_role_hierarchy():
    ...
    role_manager.register_role(
        ...
        parent_roles={"admin"},
        permissions={"documents:read", "documents:write", "documents:delete", "documents:approve"},
    )
...
def register_module_policies():
    ...
    policy_engine.register_policy(
        resource="documents", action="read", required_roles={"user", "admin", "moderator"}
    )
    ...
    policy_engine.register_policy(
        resource="documents", action="delete", required_roles={"admin", "document_admin"}
    )
    ...
    policies = [
        ...
        {"resource": "documents", "action": "execute_workflow", "required_roles": ["admin"]},
        {
            "resource": "documents",
            "action": "export",
...
async def update_document(
    ...
    return {"id": doc_id, "title": title, "updated": True}
