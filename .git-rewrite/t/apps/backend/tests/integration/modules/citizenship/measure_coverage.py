...
if not MODULE_DIR.exists():
    ...
    print(
        f"❌ Conteúdo de {BASE_DIR / 'app' / 'modules'}: {list((BASE_DIR / 'app' / 'modules').glob('*'))}"
    )
    sys.exit(1)
...
class CoverageStats:
    ...
    def add_file(self, file_path: str, lines: int, covered: int):
        ...
        self.coverage_percent = (
            (self.covered_lines / self.total_lines * 100)
...
def print_coverage_report(stats: CoverageStats):
    ...
    for file in sorted(stats.files, key=lambda x: x["coverage"]):
        print(
            f"{file['file']:<60} {file['lines']:>8} {file['covered']:>10} {file['coverage']:>9.1f}%"
        )

    # Rodapé
    print("-" * 90)
    print(
        f"{'TOTAL':<60} {stats.total_lines:>8} {stats.covered_lines:>10} {stats.coverage_percent:>9.1f}%"
    )
    print("=" * 80)
    ...
    if stats.coverage_percent >= MIN_COVERAGE:
        print(
            f"✅ Parabéns! A cobertura de código está em {stats.coverage_percent:.1f}% (mínimo: {MIN_COVERAGE}%)"
        )
    else:
        print(
            f"⚠️  Atenção: A cobertura de código está em {stats.coverage_percent:.1f}%, abaixo do mínimo de {MIN_COVERAGE}%"
        )
