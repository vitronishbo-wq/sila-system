...
CREDENTIAL_PATTERNS = [
    ...
    # Passwords
    (
        r'(?i)(pass(word)?\s*[=:]\s*["\']?)(?!Truman1*Marcelo1*)([^\s"\'<,;]+)',
        "password",
    ),
    # Hosts
        ...
        for pattern, cred_type in CREDENTIAL_PATTERNS:
            ...
            for match in matches:
                if cred_type == "db_url":
                    url = match.group(0)
                    if (
                        "postgres:Truman1*Marcelo1*@" not in url
                        and 'os.getenv("DATABASE_URL")' not in url
                    ):
                    ...
                    if value and value.lower() not in (
                        v.lower() for v in ALLOWED_CREDENTIALS.values()
                    ):
                        violations.append((line_num, cred_type, line.strip()))
