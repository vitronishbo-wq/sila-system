...
from app.infrastructure.event_sourcing.postgres_models import StoredEvent
from app.infrastructure.event_sourcing.registry import EventRegistry
...
class PostgreSQLEventStore(EventStore):
    ...
    async def append(self, event: DomainEvent) -> None:
        ...
        try:
            async with self.session_factory() as session:
                ...
                stored_event.event_metadata = json.dumps(event.metadata, cls=EventJSONEncoder) if event.metadata else None
    ...
    async def get_events_for_aggregate(self, aggregate_id: UUID, from_version: int=0) -> List[DomainEvent]:
        """
        ...
        try:
            async with self.session_factory() as session:
                stmt = select(StoredEvent).where(and_(StoredEvent.aggregate_id == aggregate_id, StoredEvent.version >= from_version)).order_by(StoredEvent.timestamp.asc())
    ...
    async def get_events_by_type(self, event_type: str, from_date: Optional[datetime]=None, to_date: Optional[datetime]=None) -> List[DomainEvent]:
        """
        ...
        try:
            ...
            async with self.session_factory() as session:
                stmt = select(StoredEvent).where(and_(*conditions) if conditions else True).order_by(StoredEvent.timestamp.asc())
                result = await session.execute(stmt)
                stored_events = result.scalars().all()
                return [self._stored_event_to_domain_event(se) for se in stored_events]
        except SQLAlchemyError as e:
    ...
    async def get_all_events(self, from_date: Optional[datetime]=None, to_date: Optional[datetime]=None, limit: int=1000) -> List[DomainEvent]:
        """
    ...
    async def get_event_by_id(self, event_id: UUID) -> Optional[DomainEvent]:
        """
