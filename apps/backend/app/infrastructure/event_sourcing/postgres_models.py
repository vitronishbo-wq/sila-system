...
"""
from sqlalchemy import Column, String, DateTime, Integer, Text, Index, create_engine
...
class StoredEvent(Base):
    ...
    __table_args__ = (Index('ix_aggregate_id_timestamp', 'aggregate_id', 'timestamp'), Index('ix_event_type_timestamp', 'event_type', 'timestamp'), Index('ix_aggregate_type_timestamp', 'aggregate_type', 'timestamp'), Index('ix_timestamp', 'timestamp'))
    ...
    def to_dict(self):
        ...
        return {'event_id': str(self.event_id), 'aggregate_id': str(self.aggregate_id), 'aggregate_type': self.aggregate_type, 'event_type': self.event_type, 'version': self.version, 'timestamp': self.timestamp.isoformat(), 'event_data': json.loads(self.event_data), 'event_metadata': json.loads(self.event_metadata) if self.event_metadata else {}, 'created_at': self.created_at.isoformat()}