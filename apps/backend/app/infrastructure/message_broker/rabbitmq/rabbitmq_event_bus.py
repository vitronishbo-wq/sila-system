...
try:
    import aio_pika
    from aio_pika import connect_robust, Channel, Exchange
except ImportError:
...
class RabbitMQEventBus(EventBus):
    ...
    def __init__(self, rabbitmq_url: str='amqp://guest:guest@localhost/', exchange_name: str='domain_events', queue_prefix: str='sila_'):
    ...
    async def publish(self, event: DomainEvent) -> None:
        ...
        try:
            message = aio_pika.Message(body=json.dumps(event.to_dict()).encode(), content_type='application/json', headers={'event_type': event.event_type, 'aggregate_type': event.aggregate_type, 'aggregate_id': str(event.aggregate_id)})
    ...
    async def subscribe(self, event_type: str, handler: Callable[[DomainEvent], Awaitable[None]]) -> None:
        """
    ...
    async def unsubscribe(self, event_type: str, handler: Callable[[DomainEvent], Awaitable[None]]) -> None:
