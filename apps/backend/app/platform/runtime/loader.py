...
def _to_import_path(base_path: Path, router_file: Path) -> str:
    ...
    return f'app.{rel.with_suffix('').as_posix().replace('/', '.')}'
...
def discover_and_register_routers(app: FastAPI) -> dict[str, list[dict[str, Any]]]:
    ...
    for path in sorted(base_path.rglob('api/router.py')):
        ...
        try:
            ...
            report['failed'].append({'file': rel.as_posix(), 'module': module_path, 'prefix': prefix, 'error': f'{e.__class__.__name__}: {e}'})
            continue
        router = getattr(module, 'router', None)
        if not isinstance(router, APIRouter):
            report['skipped'].append({'file': rel.as_posix(), 'module': module_path, 'reason': 'router_missing_or_invalid'})
            continue
        try:
            app.include_router(router, prefix=prefix, tags=[tag])
            report['loaded'].append({'file': rel.as_posix(), 'module': module_path, 'prefix': prefix, 'tag': tag})
        except Exception as e:
            report['failed'].append({'file': rel.as_posix(), 'module': module_path, 'prefix': prefix, 'error': f'{e.__class__.__name__}: {e}'})
    return report