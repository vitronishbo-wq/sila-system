...
def upgrade():
    ...
    op.create_index(
        "ix_operational_orders_citizen_id", "operational_orders", ["citizen_id"], unique=False
    )
    op.create_index(
        "ix_operational_orders_service_id", "operational_orders", ["service_id"], unique=False
    )
    op.create_index("ix_operational_orders_status", "operational_orders", ["status"], unique=False)
    op.create_index(
        "ix_operational_orders_workflow_instance_id",
        "operational_orders",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["order_id"], ["operational_orders.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_operational_order_documents_order_id",
        "operational_order_documents",
        ["order_id"],
        unique=False,
    )
    ...
    op.create_index(
        "ix_operational_payments_order_id", "operational_payments", ["order_id"], unique=False
    )
    op.create_index(
        "ix_operational_payments_reference", "operational_payments", ["reference"], unique=True
    )
    op.create_index(
        "ix_operational_payments_status", "operational_payments", ["status"], unique=False
    )
...
def downgrade():
    ...
    op.drop_index(
        "ix_operational_order_documents_order_id", table_name="operational_order_documents"
    )
