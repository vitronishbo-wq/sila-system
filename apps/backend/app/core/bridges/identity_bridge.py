...
from apps.backend.app.modules.identity.subdomains.citizen_identity_graph.application.identity_graph_service import (
...
from apps.backend.app.modules.justice._deprecated.bounded_contexts.infrastructure.models.bi_record import (
    BIRecord,
)
...
class CitizenFUC(Base):
    ...
    citizen_id = Column(
        PGUUID(as_uuid=True), unique=True, nullable=False, index=True, default=uuid.uuid4
    )
...
class IdentityBridge:
    ...
    def __init__(
        self, identity_graph: IdentityGraphService | None = None, user_service: Any | None = None
    ):
    ...
    async def create_identity_from_birth(self, citizen_id: str, name: str, birth_date: str):
        ...
        await self.identity_graph.create_citizen_node(
            identity_id=identity_id, citizen_id=citizen_id, name=name, birth_date=birth_date
        )
