...
def make_educacao_matricula_repository(db: AsyncSession):
    from apps.backend.app.modules.society.educacao.infrastructure.repositories.sqlalchemy_matricula_repository import SQLAlchemyMatriculaRepository
    return SQLAlchemyMatriculaRepository(db)

def make_educacao_propina_repository(db: AsyncSession):
    from apps.backend.app.modules.society.educacao.infrastructure.repositories.sqlalchemy_propina_repository import SQLAlchemyPropinaRepository
    return SQLAlchemyPropinaRepository(db)

def make_educacao_turma_repository(db: AsyncSession):
    from apps.backend.app.modules.society.educacao.infrastructure.repositories.sqlalchemy_turma_repository import SQLAlchemyTurmaRepository
    return SQLAlchemyTurmaRepository(db)

def make_educacao_escola_repository(db: AsyncSession):
    from apps.backend.app.modules.society.educacao.infrastructure.repositories.sqlalchemy_escola_repository import SQLAlchemyEscolaRepository
...
def make_emprego_contrato_repository(db: AsyncSession):
    from apps.backend.app.modules.society.emprego.infrastructure.repositories.sqlalchemy_contrato_repository import SQLAlchemyContratoRepository
