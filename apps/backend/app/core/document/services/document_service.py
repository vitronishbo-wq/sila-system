...
class DocumentService:
    ...
    async def upload_document(self, owner_id: uuid.UUID, owner_type: str, category: str, file_name: str, content: bytes, content_type: str=None, meta_data: dict=None) -> DocumentFile:
        """
        ...
        doc_file = DocumentFile(id=uuid.uuid4(), owner_id=owner_id, owner_type=owner_type, category=category, file_name=file_name, file_path=file_path, content_type=content_type, file_size=len(content), meta_data=meta_data)
        self.db.add(doc_file)
        await self.db.flush()
        await audit_log(action='FILE_UPLOADED', actor_id='SYSTEM', resource_id=str(doc_file.id), resource_type='DocumentFile', new_value={'category': category, 'owner_id': str(owner_id), 'file_name': file_name}, db=self.db)
        return doc_file
    ...
    async def get_owner_documents(self, owner_id: uuid.UUID) -> List[DocumentFile]:
        ...
        result = await self.db.execute(select(DocumentFile).where(DocumentFile.owner_id == owner_id))
    ...
    async def has_document_category(self, owner_id: uuid.UUID, category: str) -> bool:
        ...
        result = await self.db.execute(select(DocumentFile).where(DocumentFile.owner_id == owner_id, DocumentFile.category == category))
    ...
    async def delete_document(self, document_id: uuid.UUID):
        ...
        if doc:
            ...
            await self.db.flush()
            await audit_log(action='FILE_DELETED', actor_id='SYSTEM', resource_id=str(document_id), resource_type='DocumentFile', db=self.db)