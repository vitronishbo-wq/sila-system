...
class DocumentService:
    ...
    def __init__(self, db: AsyncSession, storage: LocalStorage | None = None, **kwargs):
    ...
    async def upload_document(
        ...
        await audit_log(
            ...
            new_value={"category": category, "owner_id": str(owner_id), "file_name": file_name},
            db=self.db,
        )
        return doc_file
    ...
    async def has_document_category(self, owner_id: uuid.UUID, category: str) -> bool:
        ...
        result = await self.db.execute(
            select(DocumentFile).where(
                DocumentFile.owner_id == owner_id, DocumentFile.category == category
            )
        )
