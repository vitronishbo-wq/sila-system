...
DomainName = Literal['economy', 'educacao', 'governance', 'infrastructure_sector', 'intelligence', 'justice', 'resources', 'society']
...
def discover_modules_dynamic(modules_root: Path=MODULES_ROOT_DEFAULT) -> tuple[ModuleSpec, ...]:
    ...
    for macro_domain_path in sorted(modules_root.iterdir()):
        ...
        if macro_domain not in domain_names:
            continue
        is_standalone_module = all(((macro_domain_path / component).is_dir() for component in ['api', 'application', 'domain', 'infrastructure']))
        if is_standalone_module:
            router_path = macro_domain_path / 'api' / 'router.py'
            specs.append(ModuleSpec(name=macro_domain, domain=macro_domain, router_import=f'app.modules.{macro_domain}.api.router:router' if router_path.exists() else None, bootstrap_scope='api', enabled=True))
            continue
        for module_path in sorted(macro_domain_path.iterdir()):
            if not module_path.is_dir() or module_path.name.startswith('__'):
                continue
            module_name = module_path.name
            has_module_structure = any(((module_path / component).is_dir() for component in ['api', 'application', 'domain', 'infrastructure']))
            if not has_module_structure:
                continue
            router_path = module_path / 'api' / 'router.py'
            if router_path.exists():
                router_import = f'app.modules.{macro_domain}.{module_name}.api.router:router'
            else:
                router_import = None
            spec = ModuleSpec(name=module_name, domain=macro_domain, router_import=router_import, bootstrap_scope='api', enabled=True)
            specs.append(spec)
...
def find_bootstrap_misalignment(modules_root: Path=MODULES_ROOT_DEFAULT) -> tuple[str, ...]:
    ...
    discovered_names = set(discover_module_names(modules_root))
    bootstrap_names = set((spec.name for spec in iter_modules(enabled_only=False) if spec.bootstrap_scope != 'none'))
