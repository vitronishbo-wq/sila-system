...
class TerritorialAccessDenied(HTTPException):
    ...
    def __init__(
        ...
        detail: str = "Acesso negado por boundary territorial",
    ):
        super().__init__(status_code=status.HTTP_403_FORBIDDEN, detail=detail)
        self.user_email = user_email
        self.territory_id = territory_id
        logger.warning(
            f"🚫 RBAC Territorial Denied: {user_email} tried to access territory {territory_id}"
        )
...
async def verify_territorial_access(
    ...
    result = await db.execute(
        select(1)
        .select_from(__import__("sqlalchemy").literal_column("territory_closure tc"))
        .where(
            and_(
                __import__("sqlalchemy").literal_column("tc.ancestor_id") == str(user.territory_id),
                __import__("sqlalchemy").literal_column("tc.descendant_id")
                == str(resource_territory_id),
            )
        )
        .limit(1)
    )
    ...
    result = await db.execute(
        ...
        .where(
            and_(
                ...
                __import__("sqlalchemy").literal_column("tc.descendant_id")
                == str(resource_territory_id),
                __import__("sqlalchemy").literal_column("tc.depth") > 0,
            )
        )
        .limit(1)
    )
...
def require_territorial_access(resource_territory_id: str | None = None):
    """
    ...
    async def _verify(
        user: User = current_user_dep, db: AsyncSession = db_dep
    ) -> User:
        if resource_territory_id is None:
            return user
        if user.territory_id is None:
...
async def require_admin(user: User = current_user_dep) -> User:
...
async def require_not_citizen(user: User = current_user_dep) -> User:
    """Bloqueia acesso a cidadãos simples"""
    if user.roles == "CITIZEN":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Cidadãos não podem executar esta ação"
        )
    return user
