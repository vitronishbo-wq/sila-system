...
_REDIS = redis_async.from_url(_REDIS_URL, decode_responses=True) if redis_async and _REDIS_URL else None
...
async def calculate_sla(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro no cálculo: {str(exc)}")


@router.post("/calculate/batch", response_model=List[Optional[SLAResponse]])
...
async def get_service_sla(
    service_id: str,
    province: Province = Query(Province.LUANDA),
    citizen_type: CitizenType = Query(CitizenType.NORMAL),
    db: AsyncSession = Depends(get_db),
) -> SLAResponse:
...
async def create_override(
    ...
    governance = SLAGovernance(db)
    record = await governance.create_override(override=override, created_by=current_user.get("email"))
...
async def get_violations(
    ...
    stmt = select(SLAViolationDB, SLABaseDB.service_name).outerjoin(
        SLABaseDB, SLABaseDB.service_id == SLAViolationDB.service_id
...
@router.get("/health")
async def sla_health_check(db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    try:
...
@router.get("/base")
async def list_base_sla(db: AsyncSession = Depends(get_db)) -> list[dict[str, Any]]:
    result = await db.execute(select(SLABaseDB).order_by(SLABaseDB.module.asc(), SLABaseDB.service_name.asc()))
