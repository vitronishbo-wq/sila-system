...
class FUCClient:
    ...
    async def validate_citizen(self, citizen_id: str) -> bool:
        ...
        if random.random() < 0.02:
            logger.error('FUC_CLIENT: transient communication failure for citizen_id=%s', citizen_id)
    ...
    async def get_citizen_data(self, citizen_id: str) -> Optional[Dict]:
        try:
            ...
            if not is_valid:
                return None
            return {'id': citizen_id, 'full_name': 'Antonio Manuel dos Santos (Identidade Validada)', 'tax_id': citizen_id, 'fiscal_status': 'REGULAR', 'residence': 'Luanda, Angola', 'last_updated': '2024-03-15T09:45:00Z', 'metadata': {'source': 'FUC_SILA_GATEWAY_V2', 'authority': 'DNIC_ANGOLA'}}
        except FUCError:
            raise
        except Exception as exc:
            logger.exception('FUC_CLIENT: unexpected error querying citizen_id=%s', citizen_id)
            raise FUCError(f'Erro interno no processamento da consulta ao Ficheiro Unico: {str(exc)}')