...
class IAMClient:
    ...
    async def get_current_user(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        if user:
            return {'id': user.id, 'username': user.username, 'email': user.email, 'permissions': user.permissions, 'roles': user.roles}
        return None
...
__all__ = ['ALGORITHM', 'SECRET_KEY', 'ACCESS_TOKEN_EXPIRE_MINUTES', 'create_access_token', 'IAMClient', 'get_iam_client', 'get_password_hash', 'verify_password']