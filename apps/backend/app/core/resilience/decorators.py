...
def circuit_breaker(
    ...
    def wrapper(func):
        ...
        def inner_sync(*args, **kwargs):
            ...
            raise RuntimeError(
                "Sync function decorated with circuit_breaker called inside a running event loop"
            )
    ...
    def wrapper(func):
        ...
        def inner_sync(*args, **kwargs):
            try:
                ...
                return asyncio.run(
                    asyncio.wait_for(asyncio.to_thread(func, *args, **kwargs), timeout=seconds)
                )
            raise RuntimeError(
                "Sync function decorated with with_timeout called inside a running event loop"
            )
