...
class CircuitBreaker:
    ...
    def record_failure(self):
        ...
        if self.failure_count >= self.failure_threshold:
            self.state = 'open'
            logger.warning('circuit_breaker.opened', extra={'failure_count': self.failure_count})
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning('request.retry', extra={'attempt': attempt + 1, 'max_retries': max_retries, 'wait_time': wait_time, 'error': str(e)})
                        await asyncio.sleep(wait_time)
                    else:
                        logger.error('request.failed_after_retries', extra={'attempts': max_retries, 'error': str(e)}, exc_info=True)
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            try:
                ...
                logger.error('request.timeout', extra={'timeout': timeout})
                raise
        return async_wrapper
    return decorator
...
class ResilientClient:
    ...
    def __init__(self, base_url: str, timeout: float=30.0, max_retries: int=3, circuit_breaker: Optional[CircuitBreaker]=None):
