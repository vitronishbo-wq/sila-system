...
"""
from .base import SilaException, NotFoundException, ValidationException, BusinessLogicException, UnauthorizedException, ForbiddenException, ConflictException, ExternalServiceException, InvalidRequestError
__all__ = ['SilaException', 'NotFoundException', 'ValidationException', 'BusinessLogicException', 'UnauthorizedException', 'ForbiddenException', 'ConflictException', 'ExternalServiceException', 'InvalidRequestError']
...
class WorkflowError(BaseRequestException):
    ...
    def __init__(self, workflow_id: str='', reason: str='', **kwargs):
        message = f'Erro no fluxo {workflow_id}: {reason}' if workflow_id else f'Erro no fluxo: {reason}'
        super().__init__(message, **kwargs)
...
class NotificationError(BaseRequestException):
    ...
    def __init__(self, reason: str='', **kwargs):
        message = f'Erro ao enviar notificação: {reason}' if reason else 'Erro ao enviar notificação'
        super().__init__(message, **kwargs)
...
def exception_to_http(exc: Exception) -> HTTPException:
    ...
    return HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Erro interno do servidor')