...
class DatabaseConfig:
    ...
    url: str = os.getenv('DATABASE_URL', 'postgresql+asyncpg://sila_user:Trumanmarcelo_1983@127.0.0.1:5432/sila_db')
...
class Database:
    ...
    def engine(self) -> AsyncEngine:
        if self._engine is None:
            engine_kwargs = {'echo': self.config.echo, 'pool_pre_ping': self.config.pool_pre_ping, 'poolclass': NullPool}
    ...
    def session_factory(self) -> async_sessionmaker:
        if self._session_factory is None:
            self._session_factory = async_sessionmaker(self.engine, class_=AsyncSession, expire_on_commit=False, autocommit=False, autoflush=False)
...
def register_models() -> None:
    ...
    import app.db.base
__all__ = ['Base', 'db', 'engine', 'get_db', 'get_session', 'get_async_db', 'transaction', 'AsyncSessionLocal', 'async_session_factory', 'importAsyncSessionLocal', 'get_engine', 'register_models', 'Database', 'DatabaseConfig', 'CoreBase']