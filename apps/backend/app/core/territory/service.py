...
class TerritoryService:
    ...
    async def get_all_provinces(db: AsyncSession):
        ...
        from apps.backend.app.core.models.location import Location

        result = await db.execute(
            select(Location)
            .where(and_(Location.type == "province", Location.parent_id is None))
    ...
    async def get_province_by_id(db: AsyncSession, province_id: int):
        ...
        result = await db.execute(
            select(Location).where(and_(Location.id == province_id, Location.type == "province"))
        )
    ...
    async def get_municipalities_in_province(db: AsyncSession, province_id: int):
        ...
        result = await db.execute(
            select(Location)
            .where(and_(Location.type == "municipality", Location.parent_id == province_id))
            .order_by(Location.name)
        )
        return result.scalars().all()
    ...
    async def get_communes_in_municipality(db: AsyncSession, municipality_id: int):
        ...
        result = await db.execute(
            select(Location)
            .where(and_(Location.type == "commune", Location.parent_id == municipality_id))
            .order_by(Location.name)
        )
        return result.scalars().all()
    ...
    async def get_full_hierarchy(db: AsyncSession, province_id: int):
        ...
        hierarchy = {
            "province": {"id": province.id, "name": province.name, "type": province.type},
            "municipalities": [],
        }
        for mun in municipalities:
            comm_result = await db.execute(
                select(Location)
                .where(and_(Location.type == "commune", Location.parent_id == mun.id))
                .order_by(Location.name)
            )
            communes = comm_result.scalars().all()
            hierarchy["municipalities"].append(
                {
                    "municipality": {"id": mun.id, "name": mun.name, "type": mun.type},
                    "communes": [{"id": c.id, "name": c.name, "type": c.type} for c in communes],
                }
            )
        return hierarchy
