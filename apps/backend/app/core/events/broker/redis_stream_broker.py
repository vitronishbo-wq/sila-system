...
class RedisStreamBroker:
    ...
    async def read(self, stream: str, group: str, consumer: str, block_ms: int=5000, count: int=10) -> AsyncIterator[Tuple[str, Dict[str, Any]]]:
        ...
        while True:
            try:
                messages = await self.client.xreadgroup(group, consumer, {stream: '>'}, count=count, block=block_ms)
