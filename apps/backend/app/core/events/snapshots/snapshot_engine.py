...
from apps.backend.app.core.events.store.repositories import SnapshotRepository
...
class SnapshotEngine:
    ...
    async def restore_from_snapshot(self, aggregate_id: UUID) -> tuple | None:
    ...
    def _serialize_aggregate(self, aggregate: BaseAggregate) -> dict:
        ...
        return {
            "id": str(aggregate.id),
            "version": aggregate.version,
            **self._get_aggregate_state(aggregate),
        }
