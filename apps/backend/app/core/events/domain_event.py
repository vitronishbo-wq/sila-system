...
class DomainEvent(ABC):
    ...
    def from_dict(cls, data: dict[str, Any]) -> "DomainEvent":
        ...
        data_copy = data.copy()
        data_copy["event_id"] = (
            UUID(data_copy["event_id"])
            if isinstance(data_copy.get("event_id"), str)
            else data_copy.get("event_id")
        )
        data_copy["aggregate_id"] = (
            UUID(data_copy["aggregate_id"])
            if isinstance(data_copy.get("aggregate_id"), str)
            else data_copy.get("aggregate_id")
        )
        if data_copy.get("correlation_id"):
            data_copy["correlation_id"] = (
                UUID(data_copy["correlation_id"])
                if isinstance(data_copy["correlation_id"], str)
                else data_copy["correlation_id"]
            )
        if data_copy.get("causation_id"):
            data_copy["causation_id"] = (
                UUID(data_copy["causation_id"])
                if isinstance(data_copy["causation_id"], str)
                else data_copy["causation_id"]
            )
