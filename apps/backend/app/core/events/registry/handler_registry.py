...
class HandlerRegistry:
    ...
    def register(cls, event_name: str, handler) -> None:
        ...
        logger.info('handler_registered', extra={'event': event_name, 'handler': handler.__class__.__name__, 'total_handlers': len(cls._handlers[event_name])})
    ...
    def get_stats(cls) -> dict:
        ...
        """
        return {'event_types': len(cls._handlers), 'total_handlers': sum((len(h) for h in cls._handlers.values())), 'by_event': {k: len(v) for k, v in cls._handlers.items()}}