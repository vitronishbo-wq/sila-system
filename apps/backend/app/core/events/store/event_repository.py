...
class EventRepository:
    ...
    async def save(self, event: DomainEvent | dict[str, Any]) -> None:
        if isinstance(event, dict):
            ...
            event_name = getattr(event, 'name', None) or getattr(event, 'event_name', None) or 'UNKNOWN_EVENT'
    ...
    async def get_stream(self, aggregate_id):
        query = select(OutboxEvent).where(OutboxEvent.payload['aggregate_id'].astext == str(aggregate_id))
