...
from apps.backend.app.core.events.bus_enhanced import EventBus as _EnhancedBus
...
from apps.backend.app.core.events.store.repositories import EventStoreRepository
        ...
        try:
            if self.outbox_repo:
                await self.outbox_repo.save(
                    event_name=event_name, event_id=event_id, payload=event_payload
                )
            if self.event_store_repo and aggregate_id is not None:
                await self.event_store_repo.append(
                    aggregate_id=aggregate_id,
                    aggregate_type=aggregate_type or "Unknown",
                    event_type=event_name or "UNKNOWN_EVENT",
            ...
            logger.error(
                "event_bus_bridge_publish_failed", extra={"error": str(exc), "event": event_name}
            )
            raise
