...
class EventBus:
    ...
    async def publish(cls, event: DomainEvent) -> None:
        ...
        if not cls._initialized:
            cls.initialize()
        logger.info('event_published', extra={'event_name': event.name, 'event_id': event.id, 'request_id': event.metadata.get('request_id', 'N/A')})
        try:
            message_dict = {'name': event.name, 'payload': event.payload, 'id': event.id, 'occurred_at': event.occurred_at.isoformat(), 'version': event.version, 'metadata': event.metadata}
    ...
    async def dispatch(cls, event: DomainEvent) -> None:
        ...
        for handler in handlers:
            try:
                ...
                logger.debug('handler_executed_successfully', extra={'event': event.name, 'handler': handler.__class__.__name__})
            except Exception as exc:
                logger.error('handler_execution_failed', extra={'event': event.name, 'handler': handler.__class__.__name__, 'error': str(exc)})
    ...
    async def health_check(cls) -> dict[str, Any]:
        ...
        return {'status': 'healthy' if broker_healthy else 'degraded', 'broker': {'name': 'Redis', 'healthy': broker_healthy}, 'registry': registry_stats}