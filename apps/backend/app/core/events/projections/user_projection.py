...
from apps.backend.app.core.events.projections.registry import BaseProjection
    ...
    async def handle_event(self, event: DomainEvent) -> dict:
        ...
        if event.event_type == "USER_CREATED":
            return {
                ...
                "tenant_id": str(event.metadata.get("tenant_id")),
                "status": "CREATED",
                "permissions": [],
            ...
            return {
                "id": str(event.aggregate_id),
                "status": "SUSPENDED",
                "suspension_reason": event.metadata.get("reason"),
                "updated_at": event.timestamp.isoformat(),
            }
            ...
            return {
                "id": str(event.aggregate_id),
                "permission_added": f"{permission}:{resource}",
                "updated_at": event.timestamp.isoformat(),
            }
            ...
            return {
                "id": str(event.aggregate_id),
                "permission_removed": f"{permission}:{resource}",
                "updated_at": event.timestamp.isoformat(),
            }
        return None
    ...
    async def handle_event(self, event: DomainEvent) -> dict:
        ...
        if event.event_type == "PERMISSION_GRANTED":
            ...
            return {
                ...
                "permission": event.metadata.get("permission"),
                "resource": event.metadata.get("resource"),
                "revoked_at": event.timestamp.isoformat(),
                "status": "REVOKED",
            }
        return None
