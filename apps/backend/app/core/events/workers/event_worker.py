...
from app.core.events.audit_chain.audit_ledger import AuditLedger
from app.core.events.broker.redis_stream_broker import RedisStreamBroker
from app.core.events.event_versioning.event_upcaster import EventUpcaster
from app.core.events.multi_region_replication.replication_manager import ReplicationManager
from app.core.events.orchestrator.saga_orchestrator import SagaOrchestrator
...
class EventWorker:
    ...
    def __init__(self, stream_name: str, group_name: str, consumer_name: str, broker: Optional[RedisStreamBroker]=None):
    ...
    async def start(self):
        ...
        logger.info('event_worker_started', extra={'stream': self.stream_name, 'group': self.group_name, 'consumer': self.consumer_name})
        try:
            async for event_id, payload in self.broker.read(self.stream_name, self.group_name, self.consumer_name):
                if not self.running:
                    break
    ...
    async def _process_event(self, event_id: str, payload: dict):
        ...
        try:
            ...
            for handler in handlers:
                try:
                    logger.info('invoking_handler', extra={'event': event_name, 'handler': handler.__class__.__name__})
                    await handler.handle(event_payload)
                except Exception as e:
                    logger.error('handler_error', extra={'event': event_name, 'handler': handler.__class__.__name__, 'error': str(e)})
