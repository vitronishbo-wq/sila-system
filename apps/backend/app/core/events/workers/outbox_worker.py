...
from app.core.events.outbox.outbox_publisher import OutboxPublisher
...
class OutboxWorker:
    ...
    def __init__(self, db: AsyncSession, broker: Optional[RedisStreamBroker]=None, poll_interval: int=1):
        self.db = db
        ...
        try:
            ...
            for event in events:
                try:
                    ...
                    logger.info('outbox_event_processed', extra={'event': event.event_name, 'event_id': event.event_id})
                except Exception as e:
                    logger.error('outbox_event_error', extra={'event': event.event_name, 'event_id': event.event_id, 'error': str(e)})
        except Exception as e:
