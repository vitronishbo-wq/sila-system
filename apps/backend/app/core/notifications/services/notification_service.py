...
class NotificationService:
    ...
    async def notify_user(self, user_id: uuid.UUID, title: str, message: str, n_type: NotificationType=NotificationType.INFO, channels: list=['db', 'email']) -> Notification:
        """Cria e persiste uma notificação no banco de dados e envia para canais externos."""
        notification = None
        if 'db' in channels:
            notification = Notification(id=uuid.uuid4(), user_id=user_id, title=title, message=message, notification_type=n_type.value)
    ...
    async def notify_request_created(self, user_id: uuid.UUID, request_id: uuid.UUID, request_data: Dict[str, Any]):
        ...
        await self.notify_user(user_id=user_id, title=title, message=message, n_type=NotificationType.SUCCESS, channels=['db', 'email', 'push'])

    async def notify_request_status_changed(self, user_id: uuid.UUID, request_id: uuid.UUID, old_status: str, new_status: str):
        ...
        await self.notify_user(user_id=user_id, title=title, message=message, n_type=NotificationType.INFO, channels=['db', 'email', 'sms'])

    async def notify_operator(self, user_id: uuid.UUID, title: str, message: str, data: Dict[str, Any]=None):
        """Notifica um operador/funcionário sobre uma nova atribuição."""
        await self.notify_user(user_id=user_id, title=title, message=message, n_type=NotificationType.INFO, channels=['db', 'push'])

    async def notify_request_update(self, user_id: uuid.UUID, request_id: uuid.UUID, new_status: str):
