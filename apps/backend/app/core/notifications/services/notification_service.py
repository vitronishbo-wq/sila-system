...
from apps.backend.app.core.notifications.models.notification import Notification, NotificationType
...
class NotificationService:
    ...
    async def notify_request_created(
        self, user_id: uuid.UUID, request_id: uuid.UUID, request_data: dict[str, Any]
    ):
        """Notifica o cidadão que o seu pedido foi recebido com sucesso."""
        title = "Pedido Recebido"
        message = (
            f"O seu pedido {str(request_id)[:8]} foi registado com sucesso e está em processamento."
        )
    ...
    async def notify_request_status_changed(
        self, user_id: uuid.UUID, request_id: uuid.UUID, old_status: str, new_status: str
    ):
        """Notifica o cidadão sobre a alteração de estado do seu pedido."""
        title = "Atualização de Estado"
        message = (
            f"O seu pedido {str(request_id)[:8]} mudou de {old_status} para {new_status.upper()}."
        )
        await self.notify_user(
            user_id=user_id,
            title=title,
    ...
    async def notify_operator(
        self, user_id: uuid.UUID, title: str, message: str, data: dict[str, Any] = None
    ):
    ...
    async def notify_request_update(
        self, user_id: uuid.UUID, request_id: uuid.UUID, new_status: str
    ):
