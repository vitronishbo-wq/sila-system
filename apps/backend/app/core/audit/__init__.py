...
from app.core.audit.sla_definitions import ANOMALY_THRESHOLDS, SLA_DEFINITIONS, SLO_TARGETS, evaluate_sla_status, get_sla_for_service
...
async def audit_log(action: str | None=None, **payload: Any):
    ...
    entry = {'action': event_action, 'resource_id': payload.get('resource_id'), 'new_value': payload.get('new_value'), 'request_id': payload.get('request_id'), 'created_at': datetime.utcnow()}
        ...
        try:
            ...
            if columns:
                ...
                if 'metadata_json' in columns:
                    insert_map['metadata_json'] = json.dumps({'new_value': entry['new_value'] or {}, 'request_id': entry.get('request_id')})
                if 'user_id' in columns:
                    user_id = payload.get('user_id')
                    if user_id is None:
                        user_res = await db.execute(text('SELECT id FROM users ORDER BY id LIMIT 1'))
                ...
                if insert_map:
                    ...
                    await db.execute(text(f'INSERT INTO audit_logs ({cols}) VALUES ({binds})'), insert_map)
...
class ImmutableAuditLog:
    ...
    def clear(cls):
        cls._entries.clear()
__all__ = ['audit_log', 'ImmutableAuditLog', 'AuditAnalytics', 'SLA_DEFINITIONS', 'SLO_TARGETS', 'ANOMALY_THRESHOLDS', 'get_sla_for_service', 'evaluate_sla_status']