...
class AuditAnalytics:
    ...
    async def get_sla_metrics(self, start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        if audit_tbl is None:
            res_created = await self.db.execute(
                text(
            ...
            res_issued = await self.db.execute(
                text(
        ...
        if payload_col is None:
            return {"avg_issuance_time_seconds": 0, "total_processed": 0}
        q_created = select(audit_tbl.c.resource_id, audit_tbl.c.created_at).where(
        ...
        if payload_col in audit_tbl.c:
            q_issued = select(audit_tbl.c.created_at, getattr(audit_tbl.c, payload_col)).where(
                audit_tbl.c.action == "DOCUMENT_ISSUED", audit_tbl.c.created_at >= start_date
            )
            ...
            res_issued = await self.db.execute(
                q_issued, {"action": "DOCUMENT_ISSUED", "start_date": start_date}
            )
        issued_rows = res_issued.fetchall()
        durations = []
        for row in issued_rows:
    ...
    async def detect_anomalies(
        self, lookback_minutes: int = 60, threshold: int = 10
    ) -> list[dict[str, Any]]:
        """
        ...
        cutoff = datetime.utcnow() - timedelta(minutes=lookback_minutes)
        query = (
            select(audit_tbl.c.action, func.count(audit_tbl.c.id).label("count"))
            .where(audit_tbl.c.created_at >= cutoff)
            .group_by(audit_tbl.c.action)
            .having(func.count(audit_tbl.c.id) > threshold)
        )
        ...
        return [
            {
                ...
                "details": row_dict.get(details_col)
                if details_col and details_col in row_dict
                else row_dict.get("details"),
            }
            for row in logs
