...
class CoreAudit:
    ...
    def _audit_structure(self):
        ...
        expected_dirs = {
            ...
            "territory": "Territorial hierarchy",
            ...
            if module_path.is_dir():
                ...
                self.findings.append(
                    AuditFinding(
                        severity="high",
                        category="structure",
                        message=f"Módulo {module} não encontrado",
                        location=f"core/{module}",
                        suggestion=f"Criar diretório: mkdir -p {module_path}",
                    )
                )
        ...
        if exceptions_path.is_file():
            self.findings.append(
                AuditFinding(
                    severity="low",
                    category="imports",
                    message="✅ Exceções centralizadas detectadas",
                    location="core/exceptions/base.py",
                    suggestion="Manter este padrão",
                )
            )
        ...
        if doc_file.is_file():
            self.findings.append(
                AuditFinding(
                    ...
                    message="✅ Documentação arquitetural encontrada",
                    location="core/ARCHITECTURE.md",
                    suggestion="Manter atualizado",
                )
            )
        else:
            self.findings.append(
                AuditFinding(
                    severity="high",
                    category="documentation",
                    message="❌ Falta documentação arquitetural",
                    location="core/",
                    suggestion="Criar: core/ARCHITECTURE.md",
                )
            )
            ...
            if not missing:
                self.findings.append(
                    AuditFinding(
                        severity="low",
                        category="structure",
                        message=f"✅ Todas {len(exc_types)} exceções centralizadas estão presentes",
                ...
                for exc_type in missing:
                    self.findings.append(
                        AuditFinding(
                            severity="medium",
                            category="structure",
                            message=f"Exceção {exc_type} não exportada",
                            location="core/exceptions/__init__.py",
                            suggestion=f"Adicionar ao __all__: '{exc_type}'",
                        )
                    )
                ...
                if layer_path.is_dir():
                    self.findings.append(
                        AuditFinding(
                            severity="low",
                            category="structure",
                            message=f"✅ {module}/{layer} encontrado",
                            location=f"core/{module}/{layer}",
                            suggestion="",
                        )
                    )
                else:
                    self.findings.append(
                        AuditFinding(
                            severity="high",
                            category="structure",
                            message=f"Falta camada DDD: {module}/{layer}",
                            location=f"core/{module}/",
                            suggestion=f"Criar: mkdir -p {module}/{layer}",
                        )
                    )
    ...
    def _generate_report(self) -> dict:
        ...
        report = {
            ...
            "conformance_score": round(score, 1),
