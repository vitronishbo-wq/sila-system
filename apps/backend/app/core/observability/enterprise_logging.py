...
Uso:
    from app.core.observability.enterprise_logging import setup_enterprise_logging
...
from .enterprise_formatter import setup_json_logging, SilaStructuredLogFilter
...
def setup_enterprise_logging(service_name: Optional[str]=None, environment: Optional[str]=None, log_level: str='INFO', include_context: bool=True, include_traceback: bool=True) -> logging.Logger:
    """
    ...
    root_logger = setup_json_logging(logger=logging.getLogger(), level=log_level_int, include_context=include_context, include_traceback=include_traceback)
...
def get_logger(name: str) -> logging.Logger:
    ...
    if not any((isinstance(h.formatter, type(None)) or (hasattr(h.formatter, '__class__') and 'SilaEnterpriseJSONFormatter' in str(h.formatter.__class__)) for h in logging.root.handlers)):
        setup_enterprise_logging()
...
__all__ = ['setup_enterprise_logging', 'setup_global_logging', 'get_logger', 'silence_verbose_loggers']