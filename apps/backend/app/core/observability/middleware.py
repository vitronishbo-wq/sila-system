...
def track_operation(op_name: str):
    ...
    try:
        ...
        logger.info('operation.complete', extra={'operation': op_name, 'duration': f'{duration:.3f}s'})
        Metrics.timing(f'operation.{op_name}', duration)
    except Exception as e:
        duration = time.time() - start_time
        logger.error('operation.error', extra={'operation': op_name, 'error': str(e), 'duration': f'{duration:.3f}s'}, exc_info=True)
...
class ObservabilityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        logger.info('request.start', extra={'method': method, 'path': path, 'client': request.client.host if request.client else 'unknown'})
        try:
            response = await call_next(request)
            status_code = response.status_code
        except Exception as e:
            duration = time.time() - start_time
            logger.error('request.error', extra={'method': method, 'path': path, 'error': str(e), 'duration': f'{duration:.3f}s'}, exc_info=True)
            ...
            if 'status_code' in locals():
                Metrics.increment('request.status', tags={'path': path, 'method': method, 'status': status_code})
                log_level = 'info' if status_code < 400 else 'warning' if status_code < 500 else 'error'
                logger.log(getattr(logging, log_level.upper()), 'request.complete', extra={'method': method, 'path': path, 'status': status_code, 'duration': f'{duration:.3f}s'})
        return response
