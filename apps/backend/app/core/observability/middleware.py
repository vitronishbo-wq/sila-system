...
class Metrics:
    ...
    def timing(cls, metric_name: str, duration: float, tags: dict = None):
        ...
        logger.info(
            "metric.timing",
            extra={"metric": metric_name, "duration": f"{duration:.3f}s", "tags": tags},
        )
...
def track_operation(op_name: str):
    ...
    try:
        ...
        logger.info(
            "operation.complete", extra={"operation": op_name, "duration": f"{duration:.3f}s"}
        )
        ...
        logger.error(
            "operation.error",
            extra={"operation": op_name, "error": str(e), "duration": f"{duration:.3f}s"},
            exc_info=True,
        )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if "status_code" in locals():
                Metrics.increment(
                    "request.status", tags={"path": path, "method": method, "status": status_code}
                )
                log_level = (
