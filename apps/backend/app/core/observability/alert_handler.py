...
app = FastAPI(title='SILA Alert Handler', description='Alert processing and notification orchestration', version='1.0.0')
...
class NotificationManager:
    ...
    async def _send_slack(self, alert: AlertPayload, channel: str='general'):
        ...
        payload = {'attachments': [{'color': color_map.get(alert.severity.value, 'warning'), 'title': f'🚨 Alert: {alert.type}', 'text': alert.description, 'fields': [{'title': 'User', 'value': alert.user_id, 'short': True}, {'title': 'Tenant', 'value': alert.tenant_id, 'short': True}, {'title': 'Severity', 'value': alert.severity.value.upper(), 'short': True}, {'title': 'Score', 'value': f'{alert.score:.1f}/100', 'short': True}, {'title': 'Recommended Action', 'value': alert.recommended_action, 'short': False}], 'footer': 'SILA Alert Handler', 'ts': int(alert.timestamp.timestamp())}]}
        try:
...
async def security_alert(alert: AlertPayload, background_tasks: BackgroundTasks):
    ...
    alert_history.append({**alert.dict(), 'received_at': datetime.utcnow().isoformat(), 'status': AlertStatus.NEW.value})
...
async def get_alert_history(limit: int=100, severity: Optional[str]=None):
    ...
    return {'total': len(alert_history), 'recent': sorted(result, key=lambda x: x['received_at'], reverse=True)}
...
async def get_alert_stats():
    ...
    return {'total_alerts': len(alert_history), 'by_severity': dict(by_severity), 'by_type': dict(by_type), 'last_24h': len([a for a in alert_history if (datetime.utcnow() - datetime.fromisoformat(a['received_at'])).days < 1])}

@app.get('/health')
async def health():
    """Health check"""
    return {'status': 'healthy', 'timestamp': datetime.utcnow().isoformat(), 'alerts_in_queue': len(alert_history)}
