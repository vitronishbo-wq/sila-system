...
def test_context_vars():
    ...
    try:
        from apps.backend.app.core.observability.context import (
            get_context_dict,
            get_request_id,
            get_user_id,
            set_request_context,
        ...
        traceback.print_exc()
        raise AssertionError(f"Erro: {e}") from e
...
def test_json_formatter():
    ...
    try:
        from apps.backend.app.core.observability.enterprise_formatter import SilaEnterpriseJSONFormatter
        ...
        print("✅ JSON Formatter produz output válido")
...
def test_circuit_breaker():
    ...
    try:
        from apps.backend.app.core.resilience.circuit_breaker_patterns import CircuitBreaker, CircuitBreakerOpen
        ...
        for _i in range(3):
            try:
                ...
                pass
        from apps.backend.app.core.resilience.circuit_breaker_patterns import CircuitState

        assert cb.state == CircuitState.OPEN, "Circuit não abriu após falhas"
        try:
            cb.call(failing_func)
            raise AssertionError("Deveria ter levantado CircuitBreakerOpen")
