...
class UserProfile:
    ...
    def _compute_baselines(self):
        ...
        if not self.ip_addresses:
            return
        sorted_ips = sorted(
            self.ip_addresses, key=lambda ip: -sum(1 for t in self.login_times if True)
        )
...
class AnomalyDetector:
    ...
    def __init__(self):
        ...
        self.thresholds = {
            "login_spike": 3.0,
            "failed_login_cluster": 5,
            "unusual_time_stddev": 2.5,
            "geo_anomaly_probability": 0.05,
            "privilege_escalation_risk": 75.0,
            "data_exfiltration_size": 1024 * 1024,
        }
    ...
    def detect(self, event: dict) -> Anomaly | None:
        """
        ...
        if profile.learning_phase:
            return None
        anomalies = [
            self._detect_login_spike(profile, event),
            self._detect_failed_login_cluster(profile, event),
            self._detect_unusual_time(profile, event),
            self._detect_geo_anomaly(profile, event),
            self._detect_privilege_escalation(profile, event),
            self._detect_data_exfiltration(profile, event),
            self._detect_behavioral_change(profile, event),
        ]
    ...
    def _detect_login_spike(self, profile: UserProfile, event: dict) -> Anomaly | None:
        ...
        recent_logins = sum(
            1
            for t in profile.login_times
            if isinstance(t, datetime) and (now - t).total_seconds() < 300
        )
        ...
        if z_score > self.thresholds["login_spike"]:
            return Anomaly(
                type=AnomalyType.LOGIN_SPIKE,
                user_id=profile.user_id,
                tenant_id=profile.tenant_id,
                severity="medium",
                score=min(100.0, 50.0 + z_score * 10),
                description=f"Login spike: {recent_logins} logins in 5 min (expected ~{expected:.1f})",
                timestamp=now,
                evidence={"recent_logins": recent_logins, "expected": expected, "z_score": z_score},
                recommended_action="Review login patterns. Possible account compromise.",
            )
        return None

    def _detect_failed_login_cluster(self, profile: UserProfile, event: dict) -> Anomaly | None:
        ...
        if recent_failures >= self.thresholds["failed_login_cluster"]:
            return Anomaly(
                type=AnomalyType.FAILED_LOGIN_cluster,
                user_id=profile.user_id,
                tenant_id=profile.tenant_id,
                severity="high",
                score=min(100.0, 70.0 + recent_failures * 5),
                description=f"{recent_failures} failed login attempts in 10 minutes",
                timestamp=now,
                evidence={
                    "failure_count": recent_failures,
                    "threshold": self.thresholds["failed_login_cluster"],
                },
                recommended_action="Block account temporarily. Investigate password attack.",
            )
        return None

    def _detect_unusual_time(self, profile: UserProfile, event: dict) -> Anomaly | None:
        ...
        if z_score > self.thresholds["unusual_time_stddev"]:
            return Anomaly(
                ...
                tenant_id=profile.tenant_id,
                severity="medium",
                score=min(100.0, 60.0 + z_score * 8),
                description=f"Admin accessing system at {hour:02d}:00 (outside business hours)",
                timestamp=now,
                evidence={
                    "access_hour": hour,
                    "expected_hours": "08:00-18:00",
                    "deviation": deviation,
                },
                recommended_action="Verify legitimate admin access outside business hours.",
            )
        return None

    def _detect_geo_anomaly(self, profile: UserProfile, event: dict) -> Anomaly | None:
        """Detecta IP novo/anômalo"""
        source_ip = event.get("source_ip")
        if not source_ip or profile.is_new_ip(source_ip):
            return None
        return Anomaly(
            type=AnomalyType.GEO_ANOMALY,
            user_id=profile.user_id,
            tenant_id=profile.tenant_id,
            severity="low",
            score=30.0,
            ...
            evidence={"source_ip": source_ip, "known_ips": list(profile.typical_ips)[:5]},
            recommended_action="Low priority: New IP detected. Log for reference.",
        )

    def _detect_privilege_escalation(self, profile: UserProfile, event: dict) -> Anomaly | None:
        ...
        if "ADMIN" in (event.get("roles_attempted") or []):
            return Anomaly(
                ...
                severity="critical",
                score=95.0,
                description="Privilege escalation attempt: Non-admin trying to gain admin role",
                timestamp=event.get("@timestamp", datetime.utcnow()),
                evidence={
                    "current_roles": event.get("roles"),
                    "attempted_roles": event.get("roles_attempted"),
                    "subject_id": event.get("subject_id"),
                },
                recommended_action="CRITICAL: Block operation immediately. Investigate user and requestor.",
            )
        return None

    def _detect_data_exfiltration(self, profile: UserProfile, event: dict) -> Anomaly | None:
        ...
        if data_size > self.thresholds["data_exfiltration_size"]:
            return Anomaly(
                ...
                severity="high",
                score=85.0,
                description=f"Large data export: {data_size / (1024 * 1024):.1f}MB",
                timestamp=event.get("@timestamp", datetime.utcnow()),
                evidence={
                ...
                recommended_action="High: Review exported data. Verify legitimate business need.",
            )
        return None

    def _detect_behavioral_change(self, profile: UserProfile, event: dict) -> Anomaly | None:
        ...
        if tenant and profile.is_unusual_tenant(tenant):
            return Anomaly(
                ...
                severity="medium",
                score=65.0,
                description=f"Access to unusual tenant: {tenant}",
                timestamp=event.get("@timestamp", datetime.utcnow()),
                evidence={
                    "unusual_tenant": tenant,
                    "typical_tenants": list(profile.typical_tenants)[:5],
                },
                recommended_action="Verify cross-tenant access is authorized.",
            )
        return None
