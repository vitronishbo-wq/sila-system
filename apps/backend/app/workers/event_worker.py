...
class EventWorker:
    ...
    async def _handle_message(self, channel: str, message: str) -> None:
        ...
        try:
            ...
            logger.debug('event_received', extra={'event': payload.get('name', 'UNKNOWN'), 'channel': channel})
            event = DomainEvent(name=payload.get('name', ''), payload=payload.get('payload', {}), id=payload.get('id', ''), version=payload.get('version', '1.0'), metadata=payload.get('metadata', {}))
            ...
            for handler in handlers:
                try:
                    ...
                    logger.debug('handler_executed', extra={'event': event.name, 'handler': handler.__class__.__name__})
                except Exception as exc:
                    logger.error('handler_failed', extra={'event': event.name, 'handler': handler.__class__.__name__, 'error': str(exc)})
    ...
    async def start(self) -> None:
        ...
        if not event_types:
            ...
            return
        logger.info('subscribing_to_events', extra={'event_count': len(event_types), 'events': event_types})
