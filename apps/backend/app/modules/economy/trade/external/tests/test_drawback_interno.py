...
async def test_service_fluxo_principal_drawback_interno():
    ...
    item = await service.solicitar(
        ...
        razao_social="Drawback Interno SA",
        cnpj_cpf="50020030000163",
        numero_processo="PROC-DRAW-INTN-2026-001",
    ...
    item = await service.aprovar(
        item.id,
        numero_radar="RADAR-DRAW-2026-5006",
...
async def test_service_detecta_processo_duplicado_drawback_interno():
    ...
    payload = dict(
        tipo_pessoa=TipoPessoa.JURIDICA,
        razao_social="Duplicada Drawback Interno SA",
        cnpj_cpf="50020030000164",
        numero_processo="PROC-DRAW-INTN-2026-002",
        data_solicitacao=date(2026, 3, 15),
    )
...
def test_endpoint_solicitar_drawback_interno_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/drawback_interno/",
        json={
            "tipo_pessoa": "juridica",
            "razao_social": "HTTP Drawback Interno SA",
            "cnpj_cpf": "50020030000165",
            "numero_processo": "PROC-DRAW-INTN-2026-003",
...
def test_endpoint_obter_drawback_interno_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=DrawbackInternoNotFoundError("Drawback Interno nao encontrado")
        )
    )
    app = FastAPI()
    app.include_router(drawback_interno_router, prefix="/comercio_externo")
