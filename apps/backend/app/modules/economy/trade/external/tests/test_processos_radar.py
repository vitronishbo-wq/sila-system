...
async def test_service_fluxo_principal_processo_radar(case):
    ...
    item = await service.solicitar(tipo_pessoa=TipoPessoa.JURIDICA, razao_social=f'Empresa {case['id']}', cnpj_cpf=case['cnpj'], numero_processo=case['processo'], data_solicitacao=date(2026, 3, 15))
    assert item.status == StatusHabilitacao.PENDENTE
    item = await service.rejeitar(item.id, data_analise=date(2026, 3, 16), motivo='Pendencia de documentacao')
    assert item.status == StatusHabilitacao.CANCELADO
    item = await service.reabrir(item.id)
    assert item.status == StatusHabilitacao.PENDENTE
    item = await service.aprovar(item.id, numero_radar='RADAR-2026-7000', data_analise=date(2026, 3, 17), data_validade=date(2027, 3, 17))
...
async def test_service_detecta_processo_duplicado_processo_radar(case):
    service = case['service_cls'](repository=case['repository_cls']())
    payload = dict(tipo_pessoa=TipoPessoa.JURIDICA, razao_social=f'Duplicada {case['id']}', cnpj_cpf=case['cnpj'], numero_processo=case['processo'], data_solicitacao=date(2026, 3, 15))
...
def test_endpoint_solicitar_retorna_201_processo_radar(case):
    ...
    response = client.post(f'{case['base_url']}/', json={'tipo_pessoa': 'juridica', 'razao_social': f'HTTP {case['id']}', 'cnpj_cpf': case['cnpj'], 'numero_processo': case['processo'], 'data_solicitacao': '2026-03-15'})
    assert response.status_code == 201
    assert response.json()['status'] == 'pendente'

@pytest.mark.parametrize('case', CASES, ids=[case['id'] for case in CASES])
def test_endpoint_obter_retorna_404_processo_radar(case):
    service = SimpleNamespace(obter_por_id=AsyncMock(side_effect=case['not_found_error_cls']('Processo de radar nao encontrado')))
    app = FastAPI()
    app.include_router(case['router'], prefix='/comercio_externo')
    app.dependency_overrides[case['dependency']] = lambda: service
    client = TestClient(app)
