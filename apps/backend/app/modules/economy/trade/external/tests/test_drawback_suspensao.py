...
async def test_service_fluxo_principal_drawback_suspensao():
    ...
    item = await service.solicitar(
        ...
        razao_social="Drawback Suspensao SA",
        cnpj_cpf="50020030000145",
        numero_processo="PROC-DRAW-SUS-2026-001",
    ...
    item = await service.aprovar(
        item.id,
        numero_radar="RADAR-DRAW-2026-4001",
...
async def test_service_detecta_processo_duplicado_drawback_suspensao():
    ...
    payload = dict(
        tipo_pessoa=TipoPessoa.JURIDICA,
        razao_social="Duplicada Drawback Suspensao SA",
        cnpj_cpf="50020030000146",
        numero_processo="PROC-DRAW-SUS-2026-002",
        data_solicitacao=date(2026, 3, 15),
    )
...
def test_endpoint_solicitar_drawback_suspensao_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/drawback_suspensao/",
        json={
            "tipo_pessoa": "juridica",
            "razao_social": "HTTP Drawback Suspensao SA",
            "cnpj_cpf": "50020030000147",
            "numero_processo": "PROC-DRAW-SUS-2026-003",
...
def test_endpoint_obter_drawback_suspensao_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=DrawbackSuspensaoNotFoundError("Drawback suspensao nao encontrado")
        )
    )
    app = FastAPI()
    app.include_router(drawback_suspensao_router, prefix="/comercio_externo")
