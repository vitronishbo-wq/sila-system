...
from apps.backend.app.modules.economy.trade.external.api.deps import (
    get_drawback_verde_amarelo_service,
)
...
async def test_service_fluxo_principal_drawback_verde_amarelo():
    ...
    item = await service.solicitar(
        ...
        razao_social="Drawback Verde Amarelo SA",
        cnpj_cpf="50020030000166",
        numero_processo="PROC-DRAW-VA-2026-001",
    ...
    item = await service.aprovar(
        item.id,
        numero_radar="RADAR-DRAW-2026-5007",
...
async def test_service_detecta_processo_duplicado_drawback_verde_amarelo():
    ...
    payload = dict(
        tipo_pessoa=TipoPessoa.JURIDICA,
        razao_social="Duplicada Drawback Verde Amarelo SA",
        cnpj_cpf="50020030000167",
        numero_processo="PROC-DRAW-VA-2026-002",
        data_solicitacao=date(2026, 3, 15),
    )
...
def test_endpoint_solicitar_drawback_verde_amarelo_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/drawback_verde_amarelo/",
        json={
            "tipo_pessoa": "juridica",
            "razao_social": "HTTP Drawback Verde Amarelo SA",
            "cnpj_cpf": "50020030000168",
            "numero_processo": "PROC-DRAW-VA-2026-003",
...
def test_endpoint_obter_drawback_verde_amarelo_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=DrawbackVerdeAmareloNotFoundError("Drawback Verde Amarelo nao encontrado")
        )
    )
    app = FastAPI()
    app.include_router(drawback_verde_amarelo_router, prefix="/comercio_externo")
