...
async def test_service_fluxo_principal_importador():
    ...
    item = await service.cadastrar(
        razao_social="Import Angola SA",
        cnpj_cpf="50020030000159",
        tipo_pessoa=TipoPessoa.JURIDICA,
        endereco="Av. Alfandega",
        numero="120",
        bairro="Centro",
        municipio="Luanda",
        provincia="Luanda",
        cep="1000-010",
        regimes_autorizados=[RegimeImportacao.DEFINITIVA],
    )
    assert item.status == StatusHabilitacao.PENDENTE
    item = await service.habilitar(
        item.id,
        numero_radar="RADAR-2026-2001",
        data_habilitacao=date(2026, 3, 2),
        data_validade=date(2027, 3, 2),
    )
    ...
    item = await service.suspender(
        item.id, data_suspensao=date(2026, 7, 1), motivo="Auditoria de compliance"
    )
    assert item.status == StatusHabilitacao.SUSPENSO
    item = await service.reabilitar(item.id)
    assert item.status == StatusHabilitacao.HABILITADO
    item = await service.cancelar(
        item.id, data_cancelamento=date(2026, 12, 5), motivo="Encerramento da atividade"
    )
...
async def test_service_detecta_cnpj_cpf_duplicado_importador():
    service = ImportadorService(repository=InMemoryImportadorRepository())
    payload = dict(
        razao_social="Duplicado Import SA",
        cnpj_cpf="50020030000158",
        tipo_pessoa=TipoPessoa.JURIDICA,
        endereco="Rua B",
        numero="2",
        bairro="Centro",
...
def test_endpoint_cadastrar_importador_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/importadores/",
        json={
            "razao_social": "Import Teste SA",
            "cnpj_cpf": "50020030000157",
...
def test_endpoint_obter_importador_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(side_effect=ImportadorNotFoundError("Importador nao encontrado"))
    )
    app = FastAPI()
    app.include_router(importadores_router, prefix="/comercio_externo")
    app.dependency_overrides[get_importador_service] = lambda: service
