...
from apps.backend.app.modules.economy.trade.external.api.deps import (
    get_habilitacao_exportador_service,
    get_habilitacao_importador_service,
    get_habilitacao_radar_service,
)
...
CASES = (
    {
        "id": "habilitacao_exportador",
        "service_cls": HabilitacaoExportadorService,
        "repository_cls": InMemoryHabilitacaoExportadorRepository,
        "dependency": get_habilitacao_exportador_service,
        "router": habilitacoes_exportador_router,
        "already_exists_error_cls": HabilitacaoExportadorAlreadyExistsError,
        "not_found_error_cls": HabilitacaoExportadorNotFoundError,
        "base_url": "/comercio_externo/habilitacoes-exportador",
        "processo": "PROC-EXP-2026-001",
        "cnpj": "50020030000140",
    },
    {
        "id": "habilitacao_importador",
        "service_cls": HabilitacaoImportadorService,
        "repository_cls": InMemoryHabilitacaoImportadorRepository,
        "dependency": get_habilitacao_importador_service,
        "router": habilitacoes_importador_router,
        "already_exists_error_cls": HabilitacaoImportadorAlreadyExistsError,
        "not_found_error_cls": HabilitacaoImportadorNotFoundError,
        "base_url": "/comercio_externo/habilitacoes-importador",
        "processo": "PROC-IMP-2026-001",
        "cnpj": "50020030000141",
    },
    {
        "id": "habilitacao_radar",
        "service_cls": HabilitacaoRadarService,
        "repository_cls": InMemoryHabilitacaoRadarRepository,
        "dependency": get_habilitacao_radar_service,
        "router": habilitacao_radar_router,
        "already_exists_error_cls": HabilitacaoRadarAlreadyExistsError,
        "not_found_error_cls": HabilitacaoRadarNotFoundError,
        "base_url": "/comercio_externo/habilitacao_radar",
        "processo": "PROC-RADAR-2026-001",
        "cnpj": "50020030000142",
    },
)
...
async def test_service_fluxo_principal_habilitacao(case):
    ...
    item = await service.solicitar(
        ...
        data_solicitacao=date(2026, 3, 10),
    )
    assert item.status == StatusHabilitacao.PENDENTE
    item = await service.rejeitar(
        item.id, data_analise=date(2026, 3, 11), motivo="Pendencia documental"
    )
    ...
    item = await service.aprovar(
        item.id,
        numero_radar="RADAR-2026-5000",
        data_analise=date(2026, 3, 12),
        data_validade=date(2027, 3, 12),
    )
...
def test_endpoint_obter_retorna_404(case):
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=case["not_found_error_cls"]("Habilitacao nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(case["router"], prefix="/comercio_externo")
