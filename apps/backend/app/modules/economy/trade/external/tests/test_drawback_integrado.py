...
async def test_service_fluxo_principal_drawback_integrado():
    ...
    item = await service.solicitar(
        ...
        razao_social="Drawback Integrado SA",
        cnpj_cpf="50020030000157",
        numero_processo="PROC-DRAW-INT-2026-001",
    ...
    item = await service.aprovar(
        item.id,
        numero_radar="RADAR-DRAW-2026-5004",
...
async def test_service_detecta_processo_duplicado_drawback_integrado():
    ...
    payload = dict(
        tipo_pessoa=TipoPessoa.JURIDICA,
        razao_social="Duplicada Drawback Integrado SA",
        cnpj_cpf="50020030000158",
        numero_processo="PROC-DRAW-INT-2026-002",
        data_solicitacao=date(2026, 3, 15),
    )
...
def test_endpoint_solicitar_drawback_integrado_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/drawback_integrado/",
        json={
            "tipo_pessoa": "juridica",
            "razao_social": "HTTP Drawback Integrado SA",
            "cnpj_cpf": "50020030000159",
            "numero_processo": "PROC-DRAW-INT-2026-003",
...
def test_endpoint_obter_drawback_integrado_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=DrawbackIntegradoNotFoundError("Drawback Integrado nao encontrado")
        )
    )
    app = FastAPI()
    app.include_router(drawback_integrado_router, prefix="/comercio_externo")
