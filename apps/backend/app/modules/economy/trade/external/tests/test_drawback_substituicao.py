...
from apps.backend.app.modules.economy.trade.external.api.deps import (
    get_drawback_substituicao_service,
)
...
async def test_service_fluxo_principal_drawback_substituicao():
    ...
    item = await service.solicitar(
        ...
        razao_social="Drawback Substituicao SA",
        cnpj_cpf="50020030000154",
        numero_processo="PROC-DRAW-SUB-2026-001",
    ...
    item = await service.aprovar(
        item.id,
        numero_radar="RADAR-DRAW-2026-5003",
...
async def test_service_detecta_processo_duplicado_drawback_substituicao():
    ...
    payload = dict(
        tipo_pessoa=TipoPessoa.JURIDICA,
        razao_social="Duplicada Drawback Substituicao SA",
        cnpj_cpf="50020030000155",
        numero_processo="PROC-DRAW-SUB-2026-002",
        data_solicitacao=date(2026, 3, 15),
    )
...
def test_endpoint_solicitar_drawback_substituicao_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/drawback_substituicao/",
        json={
            "tipo_pessoa": "juridica",
            "razao_social": "HTTP Drawback Substituicao SA",
            "cnpj_cpf": "50020030000156",
            "numero_processo": "PROC-DRAW-SUB-2026-003",
...
def test_endpoint_obter_drawback_substituicao_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=DrawbackSubstituicaoNotFoundError("Drawback Substituicao nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(drawback_substituicao_router, prefix="/comercio_externo")
