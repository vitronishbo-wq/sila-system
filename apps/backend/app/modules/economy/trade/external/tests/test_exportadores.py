...
async def test_service_fluxo_principal_exportador():
    ...
    item = await service.cadastrar(
        razao_social="Export Angola SA",
        cnpj_cpf="50020030000199",
        tipo_pessoa=TipoPessoa.JURIDICA,
        endereco="Av. Comercio Exterior",
        numero="100",
        bairro="Centro",
        municipio="Luanda",
        provincia="Luanda",
        cep="1000-001",
        regimes_autorizados=[RegimeExportacao.DEFINITIVA],
    )
    assert item.status == StatusHabilitacao.PENDENTE
    item = await service.habilitar(
        item.id,
        numero_radar="RADAR-2026-0001",
        data_habilitacao=date(2026, 3, 1),
    ...
    item = await service.suspender(
        item.id, data_suspensao=date(2026, 6, 1), motivo="Auditoria documental"
    )
    assert item.status == StatusHabilitacao.SUSPENSO
    item = await service.reabilitar(item.id)
    assert item.status == StatusHabilitacao.HABILITADO
    item = await service.cancelar(
        item.id, data_cancelamento=date(2026, 12, 1), motivo="Encerramento operacional"
    )
...
async def test_service_detecta_cnpj_cpf_duplicado():
    service = ExportadorService(repository=InMemoryExportadorRepository())
    payload = dict(
        razao_social="Duplicado Export SA",
        cnpj_cpf="50020030000188",
        tipo_pessoa=TipoPessoa.JURIDICA,
        endereco="Rua A",
        numero="1",
        bairro="Centro",
...
def test_endpoint_cadastrar_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/exportadores/",
        json={
            "razao_social": "Export Teste SA",
            "cnpj_cpf": "50020030000177",
            ...
            "provincia": "Luanda",
            "cep": "1000-200",
            "regimes_autorizados": ["definitiva", "temporaria"],
        },
    )
...
def test_endpoint_obter_por_id_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(side_effect=ExportadorNotFoundError("Exportador nao encontrado"))
    )
    app = FastAPI()
    app.include_router(exportadores_router, prefix="/comercio_externo")
    app.dependency_overrides[get_exportador_service] = lambda: service
...
def test_http_full_cycle_exportador():
    ...
    create_resp = client.post(
        "/comercio_externo/exportadores/",
        json={
            "razao_social": "Export Ciclo SA",
            "cnpj_cpf": "50020030000166",
            "tipo_pessoa": "juridica",
            "endereco": "Rua Porto",
            "numero": "10",
            "bairro": "Portuario",
            ...
            "provincia": "Benguela",
            "cep": "2000-300",
            "regimes_autorizados": ["definitiva"],
        },
    )
    assert create_resp.status_code == 201
    item_id = create_resp.json()["id"]
    habilitar_resp = client.patch(
        f"/comercio_externo/exportadores/{item_id}/habilitar",
        json={
            "numero_radar": "RADAR-2026-1000",
            "data_habilitacao": "2026-03-10",
            "data_validade": "2027-03-10",
        },
    )
    assert habilitar_resp.status_code == 200
    assert habilitar_resp.json()["status"] == "habilitado"
    produto_resp = client.patch(
        f"/comercio_externo/exportadores/{item_id}/produtos", json={"produto": "Diamante"}
    )
    assert produto_resp.status_code == 200
    assert produto_resp.json()["produtos_principais"] == ["Diamante"]
    pais_resp = client.patch(
        f"/comercio_externo/exportadores/{item_id}/paises-destino", json={"pais": "za"}
    )
