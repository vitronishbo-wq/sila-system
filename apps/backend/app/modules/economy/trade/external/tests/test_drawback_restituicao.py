...
from apps.backend.app.modules.economy.trade.external.api.deps import (
    get_drawback_restituicao_service,
)
...
async def test_service_fluxo_principal_drawback_restituicao():
    ...
    item = await service.solicitar(
        ...
        razao_social="Drawback Restituicao SA",
        cnpj_cpf="50020030000151",
        numero_processo="PROC-DRAW-RES-2026-001",
    ...
    item = await service.aprovar(
        item.id,
        numero_radar="RADAR-DRAW-2026-5002",
...
async def test_service_detecta_processo_duplicado_drawback_restituicao():
    ...
    payload = dict(
        tipo_pessoa=TipoPessoa.JURIDICA,
        razao_social="Duplicada Drawback Restituicao SA",
        cnpj_cpf="50020030000152",
        numero_processo="PROC-DRAW-RES-2026-002",
        data_solicitacao=date(2026, 3, 15),
    )
...
def test_endpoint_solicitar_drawback_restituicao_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/drawback_restituicao/",
        json={
            "tipo_pessoa": "juridica",
            "razao_social": "HTTP Drawback Restituicao SA",
            "cnpj_cpf": "50020030000153",
            "numero_processo": "PROC-DRAW-RES-2026-003",
...
def test_endpoint_obter_drawback_restituicao_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=DrawbackRestituicaoNotFoundError("Drawback Restituicao nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(drawback_restituicao_router, prefix="/comercio_externo")
