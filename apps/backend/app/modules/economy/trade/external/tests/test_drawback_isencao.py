...
async def test_service_fluxo_principal_drawback_isencao():
    ...
    item = await service.solicitar(
        ...
        razao_social="Drawback Isencao SA",
        cnpj_cpf="50020030000148",
        numero_processo="PROC-DRAW-ISN-2026-001",
    ...
    item = await service.aprovar(
        item.id,
        numero_radar="RADAR-DRAW-2026-5001",
...
async def test_service_detecta_processo_duplicado_drawback_isencao():
    ...
    payload = dict(
        tipo_pessoa=TipoPessoa.JURIDICA,
        razao_social="Duplicada Drawback Isencao SA",
        cnpj_cpf="50020030000149",
        numero_processo="PROC-DRAW-ISN-2026-002",
        data_solicitacao=date(2026, 3, 15),
    )
...
def test_endpoint_solicitar_drawback_isencao_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/drawback_isencao/",
        json={
            "tipo_pessoa": "juridica",
            "razao_social": "HTTP Drawback Isencao SA",
            "cnpj_cpf": "50020030000150",
            "numero_processo": "PROC-DRAW-ISN-2026-003",
...
def test_endpoint_obter_drawback_isencao_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=DrawbackIsencaoNotFoundError("Drawback Isencao nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(drawback_isencao_router, prefix="/comercio_externo")
