...
async def test_service_fluxo_principal_drawback_externo():
    ...
    item = await service.solicitar(
        ...
        razao_social="Drawback Externo SA",
        cnpj_cpf="50020030000160",
        numero_processo="PROC-DRAW-EXT-2026-001",
    ...
    item = await service.aprovar(
        item.id,
        numero_radar="RADAR-DRAW-2026-5005",
...
async def test_service_detecta_processo_duplicado_drawback_externo():
    ...
    payload = dict(
        tipo_pessoa=TipoPessoa.JURIDICA,
        razao_social="Duplicada Drawback Externo SA",
        cnpj_cpf="50020030000161",
        numero_processo="PROC-DRAW-EXT-2026-002",
        data_solicitacao=date(2026, 3, 15),
    )
...
def test_endpoint_solicitar_drawback_externo_retorna_201():
    ...
    response = client.post(
        "/comercio_externo/drawback_externo/",
        json={
            "tipo_pessoa": "juridica",
            "razao_social": "HTTP Drawback Externo SA",
            "cnpj_cpf": "50020030000162",
            "numero_processo": "PROC-DRAW-EXT-2026-003",
...
def test_endpoint_obter_drawback_externo_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=DrawbackExternoNotFoundError("Drawback Externo nao encontrado")
        )
    )
    app = FastAPI()
    app.include_router(drawback_externo_router, prefix="/comercio_externo")
