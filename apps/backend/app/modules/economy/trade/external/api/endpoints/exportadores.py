...
from apps.backend.app.modules.economy.trade.external.api.deps import (
    ...
    get_exportador_service_protected,
)
...
exportador_service_dep = Depends(get_exportador_service)
exportador_service_protected_dep = Depends(get_exportador_service_protected)


@router.post("/", response_model=ExportadorResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_exportador(
    data: ExportadorCreate, service: ExportadorService = exportador_service_protected_dep
):
    try:
...
async def habilitar_exportador(
    item_id: UUID,
    data: HabilitacaoInput,
    service: ExportadorService = exportador_service_protected_dep,
):
    try:
...
async def suspender_exportador(
    item_id: UUID,
    data: SuspensaoInput,
    service: ExportadorService = exportador_service_protected_dep,
):
...
async def cancelar_exportador(
    item_id: UUID,
    data: CancelamentoInput,
    service: ExportadorService = exportador_service_protected_dep,
):
    try:
...
async def reabilitar_exportador(
    item_id: UUID, service: ExportadorService = exportador_service_protected_dep
):
    try:
...
async def adicionar_pais_destino(
    item_id: UUID,
    data: PaisDestinoInput,
    service: ExportadorService = exportador_service_protected_dep,
):
    try:
...
async def obter_exportador(
    item_id: UUID, service: ExportadorService = exportador_service_dep
):
    try:
...
async def listar_exportadores(
    ...
    service: ExportadorService = exportador_service_dep,
):
