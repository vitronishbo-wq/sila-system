...
def build_operador_logistico_router(
    *,
    ...
    habilitacao_schema: type[Any],
    suspensao_schema: type[Any],
    cancelamento_schema: type[Any],
    ...
    async def habilitar(
        item_id: UUID, data: dict[str, Any] = Body(...), service: Any = Depends(get_service)
    ):
        try:
            payload = _validate_payload(habilitacao_schema, data)
            return await service.habilitar(
                item_id,
                numero_radar=payload.numero_radar,
                data_habilitacao=payload.data_habilitacao,
    ...
    async def suspender(
        ...
        try:
            ...
            return await service.suspender(
                item_id, data_suspensao=payload.data_suspensao, motivo=payload.motivo
            )
        except not_found_error_cls as exc:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
        except invalid_state_error_cls as exc:
    ...
    async def cancelar(
        ...
        try:
            ...
            return await service.cancelar(
                item_id, data_cancelamento=payload.data_cancelamento, motivo=payload.motivo
            )
        except not_found_error_cls as exc:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
        except invalid_state_error_cls as exc:
