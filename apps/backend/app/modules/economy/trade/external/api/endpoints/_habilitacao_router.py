...
def build_habilitacao_router(
    *,
    prefix: str,
    tag: str,
    get_service: Callable[..., Any],
    create_schema: type[Any],
    aprovacao_schema: type[Any],
    rejeicao_schema: type[Any],
    ...
    async def rejeitar(
        ...
        try:
            ...
            return await service.rejeitar(
                item_id, data_analise=payload.data_analise, motivo=payload.motivo
            )
        except not_found_error_cls as exc:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
        except invalid_state_error_cls as exc:
    ...
    async def listar(
        status_habilitacao: StatusHabilitacao | None = None, service: Any = Depends(get_service)
    ):
