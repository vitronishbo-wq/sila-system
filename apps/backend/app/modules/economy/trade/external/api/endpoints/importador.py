...
from apps.backend.app.modules.economy.trade.external.api.deps import get_importador_service, get_importador_service_protected
...
async def cadastrar_importador(data: ImportadorCreate, service: ImportadorService=Depends(get_importador_service_protected)):
    try:
...
async def habilitar_importador(item_id: UUID, data: HabilitacaoImportadorInput, service: ImportadorService=Depends(get_importador_service_protected)):
    try:
...
async def suspender_importador(item_id: UUID, data: SuspensaoImportadorInput, service: ImportadorService=Depends(get_importador_service_protected)):
    try:
...
async def cancelar_importador(item_id: UUID, data: CancelamentoImportadorInput, service: ImportadorService=Depends(get_importador_service_protected)):
    try:
...
async def reabilitar_importador(item_id: UUID, service: ImportadorService=Depends(get_importador_service_protected)):
    try:
...
async def adicionar_produto_importador(item_id: UUID, data: ProdutoImportadorInput, service: ImportadorService=Depends(get_importador_service_protected)):
    try:
...
async def adicionar_pais_origem(item_id: UUID, data: PaisOrigemInput, service: ImportadorService=Depends(get_importador_service_protected)):
    try:
