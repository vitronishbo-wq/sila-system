...
from apps.backend.app.modules.economy.trade.external.api.deps import (
    get_importador_service,
    get_importador_service_protected,
)
...
from apps.backend.app.modules.economy.trade.external.exceptions import (
    ImportadorAlreadyExistsError,
    ImportadorNotFoundError,
    InvalidImportadorStateError,
)

router = APIRouter(prefix="/importadores", tags=["Comercio Externo - Importadores"])

importador_service_dep = Depends(get_importador_service)
importador_service_protected_dep = Depends(get_importador_service_protected)


@router.post("/", response_model=ImportadorResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_importador(
    data: ImportadorCreate, service: ImportadorService = importador_service_protected_dep
):
    try:
        return await service.cadastrar(
            ...
            cep=data.cep,
            regimes_autorizados=data.regimes_autorizados,
        )
...
async def habilitar_importador(
    item_id: UUID,
    data: HabilitacaoImportadorInput,
    service: ImportadorService = importador_service_protected_dep,
):
    try:
        return await service.habilitar(
            item_id,
            numero_radar=data.numero_radar,
            data_habilitacao=data.data_habilitacao,
...
async def suspender_importador(
    item_id: UUID,
    data: SuspensaoImportadorInput,
    service: ImportadorService = importador_service_protected_dep,
):
    try:
        return await service.suspender(
            item_id, data_suspensao=data.data_suspensao, motivo=data.motivo
        )
    except ImportadorNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except InvalidImportadorStateError as exc:
...
async def cancelar_importador(
    item_id: UUID,
    data: CancelamentoImportadorInput,
    service: ImportadorService = importador_service_protected_dep,
):
    try:
        return await service.cancelar(
            item_id, data_cancelamento=data.data_cancelamento, motivo=data.motivo
        )
    except ImportadorNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except InvalidImportadorStateError as exc:
...
async def reabilitar_importador(
    item_id: UUID, service: ImportadorService = importador_service_protected_dep
):
    try:
...
async def adicionar_produto_importador(
    item_id: UUID,
    data: ProdutoImportadorInput,
    service: ImportadorService = importador_service_protected_dep,
):
    try:
...
async def adicionar_pais_origem(
    item_id: UUID,
    data: PaisOrigemInput,
    service: ImportadorService = importador_service_protected_dep,
):
    try:
...
async def obter_importador(
    item_id: UUID, service: ImportadorService = importador_service_dep
):
    try:
...
async def listar_importadores(
    status_habilitacao: StatusHabilitacao | None = None,
    municipio: str | None = None,
    service: ImportadorService = importador_service_dep,
):
