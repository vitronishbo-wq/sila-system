...
class Importador:
    ...
    @classmethod
    def cadastrar(cls, *, razao_social: str, cnpj_cpf: str, tipo_pessoa: TipoPessoa, endereco: str, numero: str, bairro: str, municipio: str, provincia: str, cep: str, regimes_autorizados: list[RegimeImportacao]) -> 'Importador':
        ...
        return cls(id=uuid4(), cadastro_radar='', tipo_operador=TipoOperador.IMPORTADOR, tipo_pessoa=tipo_pessoa, status=StatusHabilitacao.PENDENTE, razao_social=razao_social.strip(), cnpj_cpf=cnpj_cpf.strip(), endereco=endereco.strip(), numero=numero.strip(), bairro=bairro.strip(), municipio=municipio.strip(), provincia=provincia.strip(), cep=cep.strip(), regimes_autorizados=list(regimes_autorizados))
