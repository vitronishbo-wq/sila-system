...
from apps.backend.app.modules.economy.trade.services.api.deps import (
    ...
    get_estabelecimento_comercial_service_protected,
)
from apps.backend.app.modules.economy.trade.services.api.schemas.estabelecimento_comercial_schema import (
    DataInput,
    EncerramentoInput,
    EstabelecimentoComercialCreate,
    EstabelecimentoComercialResponse,
    MotivoInput,
    PorteInput,
    RamoInput,
)
...
from apps.backend.app.modules.economy.trade.services.exceptions import (
    ...
    InvalidEstabelecimentoComercialStateError,
)

router = APIRouter(
    prefix="/estabelecimentos_comerciais", tags=["Comercio Servicos - Estabelecimentos Comerciais"]
)
protected_service_dep = Depends(get_estabelecimento_comercial_service_protected)
service_dep = Depends(get_estabelecimento_comercial_service)


@router.post(
    "/", response_model=EstabelecimentoComercialResponse, status_code=status.HTTP_201_CREATED
)
async def cadastrar_estabelecimento_comercial(
    data: EstabelecimentoComercialCreate,
    service: EstabelecimentoComercialService = protected_service_dep,
):
    try:
...
async def encerrar(
    item_id: UUID,
    data: EncerramentoInput,
    service: EstabelecimentoComercialService = protected_service_dep,
):
    try:
        return await service.encerrar(
            item_id, data_encerramento=data.data_encerramento, motivo=data.motivo
        )
    except EstabelecimentoComercialNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except InvalidEstabelecimentoComercialStateError as exc:
...
async def obter_por_id(
    item_id: UUID,
    service: EstabelecimentoComercialService = service_dep,
):
    try:
...
async def listar(
    status_comercial: StatusComercial | None = None,
