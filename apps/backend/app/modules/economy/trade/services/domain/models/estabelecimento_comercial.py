...
class EstabelecimentoComercial:
    ...
    def iniciar_atividades(self, data_inicio: date) -> None:
        ...
        self._registrar_evento(
            f"Transicao de status: licenciamento -> ativo (data_inicio={data_inicio.isoformat()})"
        )
    ...
    def encerrar(self, *, data_encerramento: date, motivo: str) -> None:
        ...
        self.observacoes = motivo.strip()
        self._registrar_evento(
            f"Transicao de status: -> inativo (data_encerramento={data_encerramento.isoformat()})"
        )
    ...
    def associar_ramo(self, ramo: RamoComercio) -> None:
        ...
        self._registrar_evento(
            f"Ramo associado: {anterior.value} -> {ramo.codigo.value} ({ramo.descricao})"
        )
    ...
    def definir_porte(self, porte: PorteComercio) -> None:
        ...
        self._registrar_evento(
            f"Porte definido: {anterior.value} -> {porte.codigo.value} ({porte.descricao})"
        )
