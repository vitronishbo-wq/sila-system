...
from apps.backend.app.modules.economy.trade.services.api.deps import (
    get_estabelecimento_comercial_service,
)
from apps.backend.app.modules.economy.trade.services.api.endpoints.estabelecimentos_comerciais import (
    router as estabelecimentos_router,
)
...
from apps.backend.app.modules.economy.trade.services.exceptions import (
    EstabelecimentoComercialAlreadyExistsError,
    EstabelecimentoComercialNotFoundError,
)
...
async def test_service_fluxo_principal_estabelecimento_comercial():
    service = EstabelecimentoComercialService(
        repository=SQLAlchemyEstabelecimentoComercialRepository()
    )
    item = await service.cadastrar(
        cnpj="50020030000199",
        razao_social="Comercial Luanda Centro SA",
        tipo=TipoEstabelecimentoComercial.LOJA,
        ramo=RamoComercial.VAREJISTA,
        porte=PorteComercial.MEDIA,
        regime_tributario=TipoRegimeTributario.SIMPLES_NACIONAL,
        cnae_principal="4711-3/02",
        data_abertura=date(2026, 1, 10),
        endereco="Av. Comercio",
        numero="100",
        bairro="Centro",
        municipio="Luanda",
        provincia="Luanda",
    ...
    item = await service.encerrar(
        item.id, data_encerramento=date(2026, 12, 1), motivo="Encerramento de operacao"
    )
...
async def test_service_detecta_cnpj_duplicado():
    ...
    payload = dict(
        cnpj="50020030000188",
        razao_social="Atacado Benguela SA",
        tipo=TipoEstabelecimentoComercial.MATRIZ,
        ramo=RamoComercial.ATACADISTA,
        porte=PorteComercial.GRANDE,
        regime_tributario=TipoRegimeTributario.LUCRO_REAL,
        cnae_principal="4639-7/01",
        data_abertura=date(2026, 2, 1),
        endereco="Zona Portuaria",
        numero="20",
        bairro="Industrial",
        municipio="Lobito",
        provincia="Benguela",
...
def test_endpoint_cadastrar_retorna_201():
    item = EstabelecimentoComercial.cadastrar(
        cnpj="50020030000177",
        razao_social="Loja Huambo SA",
        tipo=TipoEstabelecimentoComercial.LOJA,
        ramo=RamoComercial.VESTUARIO,
        porte=PorteComercial.PEQUENA,
        regime_tributario=TipoRegimeTributario.SIMPLES_NACIONAL,
        cnae_principal="4781-4/00",
        data_abertura=date(2026, 2, 15),
    ...
    response = client.post(
        "/comercio_servicos/estabelecimentos_comerciais/",
        json={
            "cnpj": item.cnpj,
...
def test_endpoint_obter_por_id_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=EstabelecimentoComercialNotFoundError(
...
def test_http_full_cycle_estabelecimento_comercial():
    ...
    create_resp = client.post(
        ...
        json={
            ...
            "endereco": "Estrada Nacional",
            "numero": "150",
            "bairro": "Comercial Sul",
            "municipio": "Lubango",
            "provincia": "Huila",
            "cep": "4000-300",
        },
    ...
    iniciar_resp = client.patch(
        f"/comercio_servicos/estabelecimentos_comerciais/{item_id}/iniciar_atividades",
        json={"data": "2026-03-01"},
    )
    assert iniciar_resp.status_code == 200
    assert iniciar_resp.json()["status"] == "ativo"
    ramo_resp = client.patch(
        f"/comercio_servicos/estabelecimentos_comerciais/{item_id}/associar_ramo",
        json={"ramo": "eletronicos"},
    )
    assert ramo_resp.status_code == 200
    assert ramo_resp.json()["ramo"] == "eletronicos"
    porte_resp = client.patch(
        f"/comercio_servicos/estabelecimentos_comerciais/{item_id}/definir_porte",
        json={"porte": "emporio"},
    )
    ...
    assert any(
        ...
        and (item["ramo"] == "eletronicos")
        and (item["porte"] == "emporio")
        for item in itens
    )
...
def test_http_auditoria_mantem_sequencia_eventos():
    ...
    create_resp = client.post(
        ...
        json={
            "cnpj": "50020030000155",
            "razao_social": "Loja Eletronica Centro SA",
            "tipo": "loja",
            "ramo": "varejista",
            "porte": "pequena",
            "regime_tributario": "simples_nacional",
    ...
    assert (
        client.patch(
            f"/comercio_servicos/estabelecimentos_comerciais/{item_id}/suspender_atividades",
            json={"motivo": "Parada tecnica"},
        ).status_code
        == 200
    )
    assert (
        client.patch(
            f"/comercio_servicos/estabelecimentos_comerciais/{item_id}/encerrar",
            json={"data_encerramento": "2026-12-20", "motivo": "Encerramento definitivo"},
        ).status_code
        == 200
    )
    ...
    expected_markers = [
        "Cadastro inicial do estabelecimento comercial",
        "Transicao de status: licenciamento -> ativo",
        "Ramo associado:",
        "Porte definido:",
        "Transicao de status: ativo -> suspenso",
        "Transicao de status: -> inativo",
    ]
    current_index = 0
