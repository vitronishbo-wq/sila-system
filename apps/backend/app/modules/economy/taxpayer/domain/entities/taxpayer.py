...
from ..aggregate_entities import TaxDeclaration, TaxDebt, TaxPayment, TaxCertificate
...
class Taxpayer:
    ...
    def validate(self) -> None:
        ...
        try:
            ...
            raise ValueError(f'NIF inválido para contribuinte: {e}')
    ...
    def update_personal_data(self, name: Optional[str]=None, email: Optional[str]=None, phone: Optional[str]=None, address: Optional[str]=None, updated_by: Optional[UUID]=None) -> None:
    ...
    def set_tax_regime(self, regime: str, updated_by: Optional[UUID]=None) -> None:
    ...
    def get_declaration_by_number(self, number: str) -> Optional[TaxDeclaration]:
    ...
    def update_declaration_status(self, declaration_number: str, new_status: str, updated_by: Optional[UUID]=None) -> TaxDeclaration:
    ...
    def get_debt_by_number(self, number: str) -> Optional[TaxDebt]:
    ...
    def resolve_debt(self, debt_number: str, updated_by: Optional[UUID]=None) -> TaxDebt:
    ...
    def get_payment_by_number(self, number: str) -> Optional[TaxPayment]:
    ...
    def update_payment_status(self, payment_number: str, new_status: str, updated_by: Optional[UUID]=None) -> TaxPayment:
    ...
    def get_certificate_by_number(self, number: str) -> Optional[TaxCertificate]:
    ...
    def _mark_modified(self, by: Optional[UUID]=None) -> None:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'id': str(self.id), 'nif': self.nif, 'name': self.name, 'email': self.email, 'phone': self.phone, 'address': self.address, 'tax_regime': self.tax_regime, 'tenant_id': str(self.tenant_id), 'citizen_id': str(self.citizen_id) if self.citizen_id else None, 'status': self.status.value, 'addresses': self.addresses, 'phones': self.phones, 'emails': self.emails, 'declarations': [d.to_dict() for d in self.declarations], 'debts': [d.to_dict() for d in self.debts], 'payments': [p.to_dict() for p in self.payments], 'certificates': [c.to_dict() for c in self.certificates], 'created_at': self.created_at.isoformat(), 'created_by': str(self.created_by) if self.created_by else None, 'updated_at': self.updated_at.isoformat() if self.updated_at else None, 'updated_by': str(self.updated_by) if self.updated_by else None, 'version': self.version, 'metadata': self.metadata}