...
class TaxPaid:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'event_version': self.event_version, 'event_type': self.event_type, 'payment_id': str(self.payment_id), 'taxpayer_id': str(self.taxpayer_id), 'tax_type': self.tax_type, 'amount': str(self.amount), 'currency': self.currency, 'payment_method': self.payment_method, 'payment_reference': self.payment_reference, 'tax_period': self.tax_period, 'declaration_id': str(self.declaration_id) if self.declaration_id else None, 'debt_id': str(self.debt_id) if self.debt_id else None, 'paid_by': str(self.paid_by) if self.paid_by else None, 'payment_date': self.payment_date.isoformat(), 'occurred_at': self.occurred_at.isoformat(), 'metadata': self.metadata}
