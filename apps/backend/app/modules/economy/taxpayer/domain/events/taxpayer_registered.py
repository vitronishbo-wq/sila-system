...
class TaxpayerRegistered:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'event_version': self.event_version, 'event_type': self.event_type, 'taxpayer_id': str(self.taxpayer_id), 'nif': self.nif, 'name': self.name, 'email': self.email, 'phone': self.phone, 'address': self.address, 'tax_regime': self.tax_regime, 'registered_by': str(self.registered_by) if self.registered_by else None, 'occurred_at': self.occurred_at.isoformat(), 'metadata': self.metadata}
