...
class TaxPeriod:
    ...
    def __post_init__(self) -> None:
        ...
        if not self._is_valid(self.value, self.period_type):
            raise ValueError(f"Período fiscal inválido: {self.value} (tipo: {self.period_type})")
    ...
    def _is_valid(value: str, period_type: str) -> bool:
        ...
        validators = {
            "YEAR": lambda v: re.match("^\\d{4}$", v),
            "QUARTER": lambda v: re.match("^\\d{4}-Q[1-4]$", v),
            "MONTH": lambda v: re.match("^\\d{4}-M(0[1-9]|1[0-2])$", v),
            "SEMESTER": lambda v: re.match("^\\d{4}-S[1-2]$", v),
        }
    ...
    def get_semester(self) -> int | None:
