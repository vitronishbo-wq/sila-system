...
class TaxAmount:
    ...
    def __add__(self, other: Union["TaxAmount", Decimal, float, int]) -> "TaxAmount":
        ...
        if isinstance(other, TaxAmount):
            if self.currency != other.currency:
                raise ValueError(
                    f"Não é possível somar moedas diferentes: {self.currency} e {other.currency}"
                )
    ...
    def __sub__(self, other: Union["TaxAmount", Decimal, float, int]) -> "TaxAmount":
        ...
        if isinstance(other, TaxAmount):
            if self.currency != other.currency:
                raise ValueError(
                    f"Não é possível subtrair moedas diferentes: {self.currency} e {other.currency}"
                )
    ...
    def __mul__(self, factor: Decimal | float | int) -> "TaxAmount":
    ...
    def __truediv__(self, divisor: Decimal | float | int) -> "TaxAmount":
