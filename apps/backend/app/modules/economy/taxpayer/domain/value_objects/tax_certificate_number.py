...
class TaxCertificateNumber:
    ...
    def __post_init__(self) -> None:
        ...
        if parts[1] != self.certificate_type:
            raise ValueError(f'Tipo de certidão não corresponde: {parts[1]} vs {self.certificate_type}')
        if int(parts[2]) != self.year:
            raise ValueError(f'Ano não corresponde: {parts[2]} vs {self.year}')
    ...
    def _is_valid(value: str) -> bool:
        ...
        if not value:
            return False
        pattern = '^CERT-(REGULAR|NEGATIVA|ESPECIAL)-\\d{4}-\\d{6,10}$'
        return bool(re.match(pattern, value.strip()))

    @classmethod
    def generate(cls, certificate_type: str, year: int, sequence: int) -> 'TaxCertificateNumber':
        """
        ...
        if certificate_type not in valid_types:
            raise ValueError(f'Tipo de certidão inválido: {certificate_type}. Use um dos: {', '.join(valid_types)}')
    ...
    def format_display(self) -> str:
        ...
        parts = self.value.split('-')
        type_name = {'REGULAR': 'Regularidade', 'NEGATIVA': 'Dívida Não Existente', 'ESPECIAL': 'Especial'}
        return f'{type_name.get(self.certificate_type, self.certificate_type)} - {parts[2]} #{parts[3].lstrip('0') or '0'}'

    def description_pt(self) -> str:
        """Retorna descrição em português."""
        descriptions = {'REGULAR': 'Certidão de Regularidade Fiscal', 'NEGATIVA': 'Certidão de Dívida Não Existente', 'ESPECIAL': 'Certidão Especial'}
        return descriptions.get(self.certificate_type, f'Certidão {self.certificate_type}')