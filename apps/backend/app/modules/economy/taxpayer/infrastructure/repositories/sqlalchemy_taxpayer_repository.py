...
from apps.backend.app.modules.economy.taxpayer.application.ports.taxpayer_repository import (
    TaxpayerRepositoryPort,
)
from apps.backend.app.modules.economy.taxpayer.domain.entities.taxpayer import Taxpayer
from apps.backend.app.modules.economy.taxpayer.infrastructure.models.taxpayer_model import (
    TaxpayerModel,
)
...
class SQLAlchemyTaxpayerRepository(TaxpayerRepositoryPort):
    ...
    async def update(self, entity: Taxpayer) -> Taxpayer:
        await self.session.execute(
            sa_update(TaxpayerModel)
            .where(TaxpayerModel.id == entity.id)
            .values(
                name=entity.name,
                nif=entity.nif,
                status=entity.status.value,
...
def _to_entity(model: TaxpayerModel) -> Taxpayer:
    return Taxpayer(
        ...
        name=model.name,
        status=__import__(
            "apps.backend.app.modules.economy.taxpayer.domain.enums.taxpayer_status", fromlist=["TaxpayerStatus"]
        ).TaxpayerStatus(model.status),
