...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable):
        public_paths = ['/docs', '/redoc', '/openapi.json', '/health', '/api/v1/taxpayer/webhook']
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        if self.limiter and (not await self.limiter.check(f'ratelimit:{client_ip}:{request.url.path}')):
            raise HTTPException(status_code=429, detail={'message': 'Muitas requisições. Tente novamente mais tarde.', 'code': 'RATE_LIMIT_EXCEEDED'})
        return await call_next(request)
    ...
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        try:
            ...
            if response.status_code < 400 and self.audit:
                await self.audit.log(action=f'{request.method}_{request.url.path}', user_id=getattr(request.state, 'user_id', None), entity_type='taxpayer', details={'path': request.url.path, 'method': request.method, 'status_code': response.status_code, 'duration': time.time() - start_time}, ip_address=getattr(request.client, 'host', None))
            return response
        except Exception as e:
            if self.audit:
                await self.audit.log_error(error_type='API_ERROR', message=str(e), details={'path': request.url.path, 'method': request.method}, user_id=getattr(request.state, 'user_id', None))
            raise
