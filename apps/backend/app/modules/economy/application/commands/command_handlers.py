...
from apps.backend.app.modules.economy.application.commands.economy_commands import (
...
from apps.backend.app.modules.economy.domain.ports.invoice_repository_port import (
    InvoiceRepositoryPort,
)
from apps.backend.app.modules.economy.domain.ports.payment_repository_port import (
...
class CreateInvoiceHandler:
    ...
    async def handle(self, command: CreateInvoiceCommand) -> Invoice:
        ...
        invoice = Invoice(
            ...
            revenue_code=command.metadata.get("revenue_code", "0000")
            if command.metadata
            else "0000",
            cost_center=command.metadata.get("cost_center", "CC0001")
            if command.metadata
            else "CC0001",
            service_code=command.metadata.get("service_code", "SRV0001")
            if command.metadata
            else "SRV0001",
            service_name=command.metadata.get("service_name", "Serviço")
            if command.metadata
            else "Serviço",
            amount=float(command.amount),
            due_date=command.metadata.get("due_date", datetime.utcnow())
            if command.metadata
            else datetime.utcnow(),
            currency=command.metadata.get("currency", "AOA") if command.metadata else "AOA",
...
class ProcessInvoiceHandler:
    ...
    async def handle(self, command: ProcessInvoiceCommand) -> Invoice:
        ...
        invoice.change_status(
            new_status, command.metadata.get("reason", "") if command.metadata else ""
        )
        if command.metadata:
...
class CreatePaymentHandler:
    ...
    async def handle(self, command: CreatePaymentCommand) -> Payment:
        ...
        if command.amount > invoice.amount:
            raise ValueError(
                f"Payment amount {command.amount} exceeds invoice amount {invoice.amount}"
            )
        payment = Payment(
            id=str(uuid.uuid4()),
            ...
            amount=float(command.amount),
            gateway_reference=command.reference or str(uuid.uuid4()),
