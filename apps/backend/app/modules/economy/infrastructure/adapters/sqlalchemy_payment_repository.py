...
class SQLAlchemyPaymentRepository(PaymentRepositoryPort):
    ...
    async def get_by_gateway_ref(self, gateway_reference: str) -> Optional[Payment]:
        ...
        result = await self.session.execute(
            select(EconomyPaymentModel).where(EconomyPaymentModel.gateway_reference == gateway_reference)
        )
        model = result.scalars().first()
        return self._to_domain(model) if model else None
    ...
    async def exists_by_gateway_ref(self, gateway_reference: str) -> bool:
        ...
        result = await self.session.execute(
            select(EconomyPaymentModel.id).where(EconomyPaymentModel.gateway_reference == gateway_reference)
        )
