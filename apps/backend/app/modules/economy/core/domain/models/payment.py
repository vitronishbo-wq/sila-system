...
class Payment:
    ...
    def complete(self) -> Dict[str, Any]:
        ...
        return {'entity_type': 'PAYMENT', 'entity_id': self.id, 'action': 'PAYMENT_COMPLETED', 'previous_state': {'status': old_status.value}, 'new_state': {'status': self.status.value}, 'timestamp': self.confirmed_at}
    ...
    def fail(self, reason: str) -> Dict[str, Any]:
        ...
        return {'entity_type': 'PAYMENT', 'entity_id': self.id, 'action': 'PAYMENT_FAILED', 'previous_state': {'status': old_status.value}, 'new_state': {'status': self.status.value, 'reason': reason}, 'timestamp': datetime.utcnow()}
