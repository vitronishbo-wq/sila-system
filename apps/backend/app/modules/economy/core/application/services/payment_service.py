...
from ....domain.models.enums import InvoiceStatus, PaymentStatus
...
class PaymentService:
    ...
    async def register_payment(self, data: CreatePaymentSchema) -> Payment:
        ...
        """
        logger.info(
            f"Registando pagamento para fatura: {data.invoice_id} (ref: {data.gateway_reference})"
        )
        ...
        if invoice.status == InvoiceStatus.PAID:
            ...
            raise InvalidInvoiceStateError(
                current_status=invoice.status.value, action="pagar novamente"
            )
        if invoice.status == InvoiceStatus.CANCELLED:
            logger.error(f"Tentativa de pagar fatura cancelada: {invoice.id}")
            raise InvalidInvoiceStateError(
                current_status=invoice.status.value, action="processar pagamento"
            )
        payment = Payment(
            id=str(uuid.uuid4()),
        ...
        try:
            ...
            logger.info(
                f"Pagamento {new_payment.id} processado. Fatura {invoice.reference} marcada como PAGA"
            )
            return new_payment
    ...
    async def _notify_origin_module(self, invoice, payment: Payment) -> None:
        ...
        if reference_id is None:
            logger.info(
                ...
                extra={"invoice_id": invoice.id, "service_code": invoice.service_code},
            )
            return
        callback_name = {
        ...
        try:
            ...
            logger.warning(
                ...
                extra={"invoice_id": invoice.id, "module": module_name, "error": str(exc)},
            )
