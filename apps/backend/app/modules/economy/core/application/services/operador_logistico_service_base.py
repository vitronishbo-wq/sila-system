...
class OperadorLogisticoServiceBase(Generic[TOperadorLogistico]):
    ...
    def __init__(self, *, repository: OperadorLogisticoRepositoryPort[TOperadorLogistico], domain_cls: type[TOperadorLogistico], not_found_error_cls: type[Exception], already_exists_error_cls: type[Exception], invalid_state_error_cls: type[Exception], entity_label: str) -> None:
    ...
    async def cadastrar(self, *, razao_social: str, cnpj_cpf: str, tipo_pessoa: TipoPessoa, endereco: str, numero: str, bairro: str, municipio: str, provincia: str, cep: str) -> TOperadorLogistico:
        existente = await self._repository.get_by_cnpj_cpf(cnpj_cpf)
        if existente:
            raise self._already_exists_error_cls(f'{self._entity_label} ja cadastrado para o CNPJ/CPF informado')
        item = self._domain_cls.cadastrar(razao_social=razao_social, cnpj_cpf=cnpj_cpf, tipo_pessoa=tipo_pessoa, endereco=endereco, numero=numero, bairro=bairro, municipio=municipio, provincia=provincia, cep=cep)
        return await self._repository.save(item)

    async def habilitar(self, id: UUID, *, numero_radar: str, data_habilitacao: date, data_validade: date) -> TOperadorLogistico:
    ...
    async def cancelar(self, id: UUID, *, data_cancelamento: date, motivo: str) -> TOperadorLogistico:
        item = await self._obter_ou_erro(id)
        try:
    ...
    async def listar(self, *, status: StatusHabilitacao | None=None, municipio: str | None=None) -> list[TOperadorLogistico]:
