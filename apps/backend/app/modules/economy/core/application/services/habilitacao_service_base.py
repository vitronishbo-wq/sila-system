...
class HabilitacaoServiceBase(Generic[THabilitacao]):
    def __init__(
        self,
        *,
        repository: HabilitacaoRepositoryPortBase[THabilitacao],
        ...
        *,
        ...
        if existente:
            raise self._already_exists_error_cls(
                f"{self._entity_label} ja cadastrada para o numero de processo informado"
            )
        item = self._domain_cls.solicitar(
    ...
    async def aprovar(
        self, id: UUID, *, numero_radar: str, data_analise: date, data_validade: date
    ) -> THabilitacao:
        item = await self._obter_ou_erro(id)
        try:
            item.aprovar(
                numero_radar=numero_radar, data_analise=data_analise, data_validade=data_validade
            )
