...
from ....trade.services.application.ports import EstabelecimentoComercialRepositoryPort
from ....trade.services.domain.enums import (
    PorteComercial,
    RamoComercial,
    StatusComercial,
...
from ....trade.services.domain.models import EstabelecimentoComercial
from ....trade.services.domain.shared import get_porte, get_ramo
from ....trade.services.exceptions import (
...
class EstabelecimentoComercialService:
    ...
    async def cadastrar(
        ...
        *,
        ...
        if existente:
            raise EstabelecimentoComercialAlreadyExistsError(
                "Estabelecimento comercial ja cadastrado para o CNPJ informado"
            )
        ...
        logger.info(
            "comercio_servicos.estabelecimento.cadastrado id=%s cnpj=%s ramo=%s",
            str(salvo.id),
            salvo.cnpj,
            salvo.ramo.value,
        )
    ...
    async def encerrar(
        self, id: UUID, *, data_encerramento: date, motivo: str
    ) -> EstabelecimentoComercial:
        item = await self._obter_ou_erro(id)
        try:
    ...
    async def associar_ramo(self, id: UUID, *, ramo: RamoComercial) -> EstabelecimentoComercial:
        ...
        salvo = await self._repository.save(item)
        logger.info(
            "comercio_servicos.estabelecimento.ramo id=%s ramo=%s", str(salvo.id), salvo.ramo.value
        )
        return salvo
    ...
    async def definir_porte(self, id: UUID, *, porte: PorteComercial) -> EstabelecimentoComercial:
        ...
        salvo = await self._repository.save(item)
        logger.info(
            "comercio_servicos.estabelecimento.porte id=%s porte=%s",
            str(salvo.id),
            salvo.porte.value,
        )
        return salvo
