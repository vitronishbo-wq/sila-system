...
from ....trade.external.application.ports import ExportadorRepositoryPort
from ....trade.external.domain.enums import RegimeExportacao, StatusHabilitacao, TipoPessoa
from ....trade.external.domain.models import Exportador
from ....trade.external.exceptions import ExportadorAlreadyExistsError, ExportadorNotFoundError, InvalidExportadorStateError
...
class ExportadorService:
    ...
    async def cadastrar(self, *, razao_social: str, cnpj_cpf: str, tipo_pessoa: TipoPessoa, endereco: str, numero: str, bairro: str, municipio: str, provincia: str, cep: str, regimes_autorizados: list[RegimeExportacao]) -> Exportador:
        ...
        item = Exportador.cadastrar(razao_social=razao_social, cnpj_cpf=cnpj_cpf, tipo_pessoa=tipo_pessoa, endereco=endereco, numero=numero, bairro=bairro, municipio=municipio, provincia=provincia, cep=cep, regimes_autorizados=regimes_autorizados)
        return await self._repository.save(item)

    async def habilitar(self, id: UUID, *, numero_radar: str, data_habilitacao: date, data_validade: date) -> Exportador:
    ...
    async def listar(self, *, status: StatusHabilitacao | None=None, municipio: str | None=None) -> list[Exportador]:
