...
from ....domain.exceptions import DomainValidationError, FUCError, InvoiceNotFoundError
...
class InvoiceService:
    ...
    def __init__(
        ...
        assistencia_social_service: AssistenciaSocialServicePort | None = None,
        service_requests_service: ServiceRequestsServicePort | None = None,
        identidade_service: IdentidadeServicePort | None = None,
    ):
    ...
    async def create_invoice(self, data: CreateInvoiceSchema) -> Invoice:
        ...
        try:
            ...
            if not is_valid:
                logger.warning(
                    f"Emissão negada: Cidadão {data.citizen_id} inválido ou inativo no FUC."
                )
                raise DomainValidationError(
                    f"Cidadão {data.citizen_id} não habilitado para emissão de faturas."
                )
            ...
            raise FUCError(
                "Serviço de validação de identidade indisponível. Tente novamente mais tarde."
            ) from e
    ...
    @trace()
    async def create_educacao_tuition_invoice(
        self,
        *,
        matricula_id: UUID,
        citizen_id: str,
        ...
        service_name: str = "Propina Escolar",
    ) -> Invoice:
        ...
        return await self._create_module_invoice(
            citizen_id=citizen_id,
            service_code="EDU_PROPINA",
            service_name=service_name,
            amount=amount,
            due_date=due_date or datetime.now(UTC) + timedelta(days=30),
            revenue_code=revenue_code,
            cost_center=cost_center,
            request_id=str(matricula_id),
        )

    @trace()
    async def create_juventude_scholarship_invoice(
        self,
        *,
        referencia_id: UUID,
        citizen_id: str,
        ...
        cost_center: str,
        due_date: datetime | None = None,
        service_name: str = "Bolsa/Auxílio Juventude",
    ) -> Invoice:
        ...
        return await self._create_module_invoice(
            citizen_id=citizen_id,
            service_code="JUV_BOLSA",
            service_name=service_name,
            amount=amount,
            due_date=due_date or datetime.now(UTC),
            revenue_code=revenue_code,
            cost_center=cost_center,
            request_id=str(referencia_id),
        )

    @trace()
    async def create_emprego_salary_invoice(
        self,
        *,
        contrato_id: UUID,
        citizen_id: str,
        ...
        cost_center: str,
        due_date: datetime | None = None,
        service_name: str = "Salário Emprego",
    ) -> Invoice:
        ...
        return await self._create_module_invoice(
            citizen_id=citizen_id,
            service_code="EMP_SALARIO",
            service_name=service_name,
            amount=amount,
            due_date=due_date or datetime.now(UTC),
            revenue_code=revenue_code,
            cost_center=cost_center,
            request_id=str(contrato_id),
        )

    @trace()
    async def create_saude_service_invoice(
        self,
        *,
        atendimento_id: UUID,
        citizen_id: str,
        ...
        cost_center: str,
        due_date: datetime | None = None,
        service_name: str = "Taxa de Serviço de Saúde",
    ) -> Invoice:
        ...
        return await self._create_module_invoice(
            ...
            revenue_code=revenue_code,
            cost_center=cost_center,
            request_id=str(atendimento_id),
        )

    @trace()
    async def create_assistencia_benefit_invoice(
        self,
        *,
        beneficio_id: UUID,
        citizen_id: str,
        ...
        cost_center: str,
        due_date: datetime | None = None,
        service_name: str = "Benefício Assistência Social",
    ) -> Invoice:
        ...
        return await self._create_module_invoice(
            citizen_id=citizen_id,
            service_code="ASS_BENEFICIO",
            service_name=service_name,
            amount=amount,
            due_date=due_date or datetime.now(UTC),
            revenue_code=revenue_code,
            cost_center=cost_center,
            request_id=str(beneficio_id),
        )

    @trace()
    async def create_service_request_fee_invoice(
        self,
        *,
        request_id: UUID,
        citizen_id: str,
        ...
        cost_center: str,
        due_date: datetime | None = None,
        service_name: str = "Taxa de Pedido de Serviço",
    ) -> Invoice:
        ...
        return await self._create_module_invoice(
            ...
            service_name=service_name,
            amount=amount,
            due_date=due_date or datetime.now(UTC) + timedelta(days=15),
            revenue_code=revenue_code,
            cost_center=cost_center,
    ...
    async def cancel_invoice(
        self, invoice_id: str, reason: str = "Cancelamento administrativo"
    ) -> Invoice:
        """
    ...
    async def mark_overdue(self, invoice_id: str) -> Invoice:
        ...
        if invoice.is_overdue() and invoice.status == InvoiceStatus.PENDING:
            invoice.change_status(
                InvoiceStatus.OVERDUE, reason="Data limite de pagamento ultrapassada"
            )
            updated_invoice = await self.repository.save(invoice)
    ...
    async def _create_module_invoice(
        self,
        *,
        citizen_id: str,
        service_code: str,
