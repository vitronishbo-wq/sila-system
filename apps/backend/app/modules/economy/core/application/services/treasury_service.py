...
class _FallbackPaymentService:
    ...
    def create_payment(self, db, data: dict):
        return {'id': None, 'citizen_id': data.get('citizen_id'), 'amount': data.get('amount'), 'type': data.get('type'), 'status': 'created'}

def _resolve_payment_service():
    try:
        module = import_module('app.modules.payment.services.payment_service')
...
class TreasuryService:
    ...
    def receive_tax_payment(self, db: Session, citizen_id: str, tax_id: str, amount: float):
        with UnitOfWork(db):
            payment = self.payment.create_payment(db, {'citizen_id': citizen_id, 'amount': amount, 'type': 'tax'})
