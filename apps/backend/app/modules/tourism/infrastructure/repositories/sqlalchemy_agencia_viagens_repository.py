...
from apps.backend.app.modules.tourism.infrastructure.models.agencia_viagens_model import (
    AgenciaViagensModel,
)
...
class SQLAlchemyAgenciaViagensRepository(AgenciaViagensRepositoryPort):
    ...
    async def save(self, agencia: AgenciaViagens) -> AgenciaViagens:
        model = AgenciaViagensModel(
            id=agencia.id,
            registro=agencia.registro,
            ...
            cep=agencia.cep,
            proprietario_id=agencia.proprietario_id,
            data_registro=agencia.data_registro,
            ativa=agencia.ativa,
            especialidades=list(agencia.especialidades or []),
            site=agencia.site,
            observacoes=agencia.observacoes,
        )
    ...
    def _to_domain(model: AgenciaViagensModel) -> AgenciaViagens:
        return AgenciaViagens(
            ...
            ativa=model.ativa,
            especialidades=list(model.especialidades or []),
            site=model.site,
