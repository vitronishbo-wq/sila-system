...
class SQLAlchemyPousadaRepository(PousadaRepositoryPort):
    ...
    async def save(self, pousada: Pousada) -> Pousada:
        model = PousadaModel(
            id=pousada.id,
            cadastur=pousada.cadastur,
            nome=pousada.nome,
            tipo=pousada.tipo,
            classificacao=pousada.classificacao,
            cnpj=pousada.cnpj,
            endereco=pousada.endereco,
            numero=pousada.numero,
            bairro=pousada.bairro,
            municipio=pousada.municipio,
            provincia=pousada.provincia,
            cep=pousada.cep,
            telefone=pousada.telefone,
            email=pousada.email,
            quartos=pousada.quartos,
            capacidade_maxima=pousada.capacidade_maxima,
            proprietario_id=pousada.proprietario_id,
            data_abertura=pousada.data_abertura,
            ativa=pousada.ativa,
            site=pousada.site,
            observacoes=pousada.observacoes,
        )
