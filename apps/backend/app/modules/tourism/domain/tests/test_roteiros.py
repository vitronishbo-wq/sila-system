...
from apps.backend.app.modules.tourism.infrastructure.adapters.comercio_servicos_service_adapter import (
    ComercioServicosServiceAdapter,
)
from apps.backend.app.modules.tourism.infrastructure.adapters.transportes_logistica_service_adapter import (
...
class _FakeTransportesService:
    ...
    async def estimate_tempo_viagem_horas(
        self, *, origem: str, destino: str, modal: str | None = None
    ) -> float | None:
        return 4.0
...
def test_cadastrar_roteiro_com_integracao_cruzada() -> None:
    ...
    service = RoteiroService(
        repository=repo, transportes_service=transportes, comercio_service=comercio
    )
    result = asyncio.run(
        service.cadastrar(
            titulo="Roteiro Serra e Praia",
            descricao="Combina trilha de serra e descanso na praia.",
            municipio_origem="Benguela",
            provincia_origem="Benguela",
            duracao_horas=10,
            pontos_parada=["Lobito", "Baia Azul"],
            acessivel=False,
            valor_estimado=Decimal("15000"),
        )
    )
...
def test_listar_roteiros_filtra_ativo() -> None:
    ...
    item = asyncio.run(
        service.cadastrar(
            titulo="Roteiro Historico",
            descricao="Circuito por sitios historicos.",
            municipio_origem="Luanda",
            provincia_origem="Luanda",
            duracao_horas=6,
            acessivel=True,
        )
    )
