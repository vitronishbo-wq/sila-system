...
def test_cadastrar_hotel_sucesso() -> None:
    ...
    service = MeioHospedagemService(
        hotel_repo=hotel_repo, citizen_service=citizen_service, request_service=request_service
    )
    result = asyncio.run(
        service.cadastrar_hotel(
            nome="Hotel Luanda",
            tipo=TipoMeioHospedagem.HOTEL,
            classificacao=ClassificacaoHoteleira.SUPERIOR,
