...
from apps.backend.app.modules.tourism.api.schemas.agencia_viagens_schema import (
    AgenciaViagensCreate,
    AgenciaViagensResponse,
    AgenciaViagensUpdate,
)
...
agencia_service_dep = Depends(get_agencia_service)


@router.post("/", response_model=AgenciaViagensResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_agencia(
    data: AgenciaViagensCreate, service: AgenciaViagensService = agencia_service_dep
) -> AgenciaViagensResponse:
    try:
...
async def obter_agencia(
    agencia_id: UUID, service: AgenciaViagensService = agencia_service_dep
) -> AgenciaViagensResponse:
    try:
...
async def listar_agencias(
    ...
    service: AgenciaViagensService = agencia_service_dep,
) -> list[AgenciaViagensResponse]:
