...
from apps.backend.app.modules.tourism.api.schemas.roteiro_schema import (
    RoteiroCreate,
    RoteiroResponse,
    RoteiroUpdate,
)
...
roteiro_service_dep = Depends(get_roteiro_service)


@router.post("/", response_model=RoteiroResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_roteiro(
    data: RoteiroCreate, service: RoteiroService = roteiro_service_dep
) -> RoteiroResponse:
    try:
        return await service.cadastrar(
            ...
            provincia_origem=data.provincia_origem,
            duracao_horas=data.duracao_horas,
            pontos_parada=data.pontos_parada,
...
async def obter_roteiro(
    roteiro_id: UUID, service: RoteiroService = roteiro_service_dep
) -> RoteiroResponse:
    try:
...
async def listar_roteiros(
    ...
    service: RoteiroService = roteiro_service_dep,
) -> list[RoteiroResponse]:
...
async def atualizar_roteiro(
    roteiro_id: UUID, data: RoteiroUpdate, service: RoteiroService = roteiro_service_dep
) -> RoteiroResponse:
    try:
        ...
        status_code = (
            ...
            if "nao encontrado" in str(exc).lower()
