...
from apps.backend.app.modules.tourism.api.schemas.pousada_schema import (
    PousadaCreate,
    PousadaResponse,
    PousadaUpdate,
)
...
pousada_service_dep = Depends(get_pousada_service)


@router.post("/", response_model=PousadaResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_pousada(
    data: PousadaCreate, service: MeioHospedagemService = pousada_service_dep
) -> PousadaResponse:
    try:
...
async def obter_pousada(
    pousada_id: UUID, service: MeioHospedagemService = pousada_service_dep
) -> PousadaResponse:
    try:
...
async def listar_pousadas(
    ...
    service: MeioHospedagemService = pousada_service_dep,
) -> list[PousadaResponse]:
    return await service.listar_pousadas(
        municipio=municipio, classificacao=classificacao, ativa=ativa
    )
