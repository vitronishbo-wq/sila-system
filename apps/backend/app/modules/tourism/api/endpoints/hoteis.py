...
hotel_service_dep = Depends(get_hotel_service)


@router.post("/", response_model=HotelResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_hotel(
    data: HotelCreate, service: MeioHospedagemService = hotel_service_dep
) -> HotelResponse:
    try:
        return await service.cadastrar_hotel(
            nome=data.nome,
            tipo=data.tipo,
...
async def obter_hotel(
    hotel_id: UUID, service: MeioHospedagemService = hotel_service_dep
) -> HotelResponse:
    try:
...
async def listar_hoteis(
    ...
    service: MeioHospedagemService = hotel_service_dep,
) -> list[HotelResponse]:
