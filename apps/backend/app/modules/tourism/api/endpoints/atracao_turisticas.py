...
from apps.backend.app.modules.tourism.api.schemas.atracao_turistica_schema import (
    AtracaoTuristicaCreate,
    AtracaoTuristicaResponse,
    AtracaoTuristicaUpdate,
)
...
atracao_service_dep = Depends(get_atracao_service)


@router.post("/", response_model=AtracaoTuristicaResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_atracao(
    data: AtracaoTuristicaCreate, service: AtracaoService = atracao_service_dep
) -> AtracaoTuristicaResponse:
    try:
        return await service.cadastrar(
            ...
            capacidade_visitantes_dia=data.capacidade_visitantes_dia,
...
async def obter_atracao(
    atracao_id: UUID, service: AtracaoService = atracao_service_dep
) -> AtracaoTuristicaResponse:
    try:
...
async def listar_atracoes(
    ...
    service: AtracaoService = atracao_service_dep,
) -> list[AtracaoTuristicaResponse]:
...
async def atualizar_atracao(
    atracao_id: UUID,
    data: AtracaoTuristicaUpdate,
    service: AtracaoService = atracao_service_dep,
) -> AtracaoTuristicaResponse:
    try:
