...
from app.modules.wallet.schemas import DocumentOut
from app.modules.wallet.service import get_documents
...
async def get_wallet(user: dict=Depends(get_current_user), db: AsyncSession=Depends(get_db)):
    ...
    if not citizen_id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='citizen_id missing in token')
