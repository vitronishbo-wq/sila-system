...
class SQLAlchemyMandadoRepository(MandadoRepositoryPort):
    ...
    async def list_by_ocorrencia(self, ocorrencia_id: UUID) -> list[Mandado]:
        stmt = (
            select(MandadoModel)
            .where(MandadoModel.ocorrencia_id == ocorrencia_id)
            .order_by(MandadoModel.data_expedicao.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_tipo(self, tipo: TipoMandado) -> list[Mandado]:
        stmt = (
            select(MandadoModel)
            .where(MandadoModel.tipo == tipo.value)
            .order_by(MandadoModel.data_expedicao.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_status(self, status: StatusMandado) -> list[Mandado]:
        stmt = (
            select(MandadoModel)
            .where(MandadoModel.status == status.value)
            .order_by(MandadoModel.data_expedicao.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_numero(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(MandadoModel)
            .where(MandadoModel.numero_mandado.like(f"{prefix}%"))
        )
    ...
    def _to_domain(model: MandadoModel) -> Mandado:
        return Mandado(
            id=model.id,
            numero_mandado=model.numero_mandado,
            ocorrencia_id=model.ocorrencia_id,
            tipo=TipoMandado(model.tipo),
            autoridade_judicial=model.autoridade_judicial,
            data_expedicao=model.data_expedicao,
            data_validade=model.data_validade,
            status=StatusMandado(model.status),
