...
class SQLAlchemyEvidenciaRepository(EvidenciaRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_evidencia: str) -> Evidencia | None:
        stmt = select(EvidenciaModel).where(EvidenciaModel.codigo_evidencia == codigo_evidencia.strip())
    ...
    async def list_by_vestigio(self, vestigio_id: UUID) -> list[Evidencia]:
        stmt = select(EvidenciaModel).where(EvidenciaModel.vestigio_id == vestigio_id).order_by(EvidenciaModel.data_registro.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]

    async def list_by_status(self, status: StatusEvidencia) -> list[Evidencia]:
        stmt = select(EvidenciaModel).where(EvidenciaModel.status == status.value).order_by(EvidenciaModel.data_registro.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        prefix = f'EVD/{year}/'
        stmt = select(func.count()).select_from(EvidenciaModel).where(EvidenciaModel.codigo_evidencia.like(f'{prefix}%'))
        count = (await self.session.execute(stmt)).scalar() or 0
        return f'{prefix}{count + 1:06d}'

    @staticmethod
    def _to_domain(model: EvidenciaModel) -> Evidencia:
        return Evidencia(id=model.id, codigo_evidencia=model.codigo_evidencia, vestigio_id=model.vestigio_id, cadeia_custodia_id=model.cadeia_custodia_id, tipo=TipoEvidencia(model.tipo), descricao=model.descricao, fonte=model.fonte, confiabilidade=model.confiabilidade, status=StatusEvidencia(model.status), data_registro=model.data_registro, analisado_por_id=model.analisado_por_id, observacoes=model.observacoes, ativo=model.ativo)