...
class SQLAlchemyOcorrenciaRepository(OcorrenciaRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_ocorrencia: str) -> Ocorrencia | None:
        stmt = select(OcorrenciaModel).where(
            OcorrenciaModel.codigo_ocorrencia == codigo_ocorrencia.strip()
        )
    ...
    async def list_by_unidade(self, unidade_id: UUID) -> list[Ocorrencia]:
        stmt = (
            ...
            .where(OcorrenciaModel.unidade_id == unidade_id)
            .order_by(OcorrenciaModel.data_ocorrencia.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_tipo(self, tipo: TipoOcorrencia) -> list[Ocorrencia]:
        stmt = (
            select(OcorrenciaModel)
            .where(OcorrenciaModel.tipo == tipo.value)
            .order_by(OcorrenciaModel.data_ocorrencia.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_status(self, status: StatusOcorrencia) -> list[Ocorrencia]:
        stmt = (
            select(OcorrenciaModel)
            .where(OcorrenciaModel.status == status.value)
            .order_by(OcorrenciaModel.data_ocorrencia.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_periodo(self, inicio: datetime, fim: datetime) -> list[Ocorrencia]:
        stmt = (
            select(OcorrenciaModel)
            .where(
                OcorrenciaModel.data_ocorrencia >= inicio, OcorrenciaModel.data_ocorrencia <= fim
            )
            .order_by(OcorrenciaModel.data_ocorrencia.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            ...
            .where(OcorrenciaModel.codigo_ocorrencia.like(f"{prefix}%"))
        )
    ...
    def _to_domain(model: OcorrenciaModel) -> Ocorrencia:
        return Ocorrencia(
            ...
            policial_responsavel_id=model.policial_responsavel_id,
            tipo=TipoOcorrencia(model.tipo),
            ...
            preso_em_flagrante=model.preso_em_flagrante,
