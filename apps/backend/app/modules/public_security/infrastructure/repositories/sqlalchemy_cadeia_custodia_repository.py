...
class SQLAlchemyCadeiaCustodiaRepository(CadeiaCustodiaRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_cadeia: str) -> CadeiaCustodia | None:
        stmt = select(CadeiaCustodiaModel).where(
            CadeiaCustodiaModel.codigo_cadeia == codigo_cadeia.strip()
        )
    ...
    async def list_by_status(self, status: StatusCadeiaCustodia) -> list[CadeiaCustodia]:
        stmt = (
            select(CadeiaCustodiaModel)
            .where(CadeiaCustodiaModel.status == status.value)
            .order_by(CadeiaCustodiaModel.data_inicio.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(CadeiaCustodiaModel)
            .where(CadeiaCustodiaModel.codigo_cadeia.like(f"{prefix}%"))
        )
    ...
    def _to_domain(model: CadeiaCustodiaModel) -> CadeiaCustodia:
        return CadeiaCustodia(
            id=model.id,
            codigo_cadeia=model.codigo_cadeia,
            prova_id=model.prova_id,
            ocorrencia_id=model.ocorrencia_id,
            status=StatusCadeiaCustodia(model.status),
            local_atual=model.local_atual,
            responsavel_id=model.responsavel_id,
            data_inicio=model.data_inicio,
            data_ultima_movimentacao=model.data_ultima_movimentacao,
            historico_movimentacoes=model.historico_movimentacoes,
            integridade_verificada=model.integridade_verificada,
