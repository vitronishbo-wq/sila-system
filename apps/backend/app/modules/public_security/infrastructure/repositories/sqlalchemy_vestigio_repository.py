...
class SQLAlchemyVestigioRepository(VestigioRepositoryPort):
    ...
    async def list_by_cadeia(self, cadeia_custodia_id: UUID) -> list[Vestigio]:
        stmt = select(VestigioModel).where(VestigioModel.cadeia_custodia_id == cadeia_custodia_id).order_by(VestigioModel.data_coleta.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]

    async def list_by_status(self, status: StatusVestigio) -> list[Vestigio]:
        stmt = select(VestigioModel).where(VestigioModel.status == status.value).order_by(VestigioModel.data_coleta.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        prefix = f'VST/{year}/'
        stmt = select(func.count()).select_from(VestigioModel).where(VestigioModel.codigo_vestigio.like(f'{prefix}%'))
        count = (await self.session.execute(stmt)).scalar() or 0
        return f'{prefix}{count + 1:06d}'

    @staticmethod
    def _to_domain(model: VestigioModel) -> Vestigio:
        return Vestigio(id=model.id, codigo_vestigio=model.codigo_vestigio, cadeia_custodia_id=model.cadeia_custodia_id, ocorrencia_id=model.ocorrencia_id, tipo=TipoVestigio(model.tipo), descricao=model.descricao, localizacao=model.localizacao, data_coleta=model.data_coleta, status=StatusVestigio(model.status), coletado_por_id=model.coletado_por_id, observacoes=model.observacoes, ativo=model.ativo)