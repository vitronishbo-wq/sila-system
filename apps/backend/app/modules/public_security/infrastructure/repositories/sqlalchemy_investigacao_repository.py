...
class SQLAlchemyInvestigacaoRepository(InvestigacaoRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_investigacao: str) -> Investigacao | None:
        stmt = select(InvestigacaoModel).where(
            InvestigacaoModel.codigo_investigacao == codigo_investigacao.strip()
        )
    ...
    async def list_by_ocorrencia(self, ocorrencia_id: UUID) -> list[Investigacao]:
        stmt = (
            select(InvestigacaoModel)
            .where(InvestigacaoModel.ocorrencia_id == ocorrencia_id)
            .order_by(InvestigacaoModel.data_abertura.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_status(self, status: StatusInvestigacao) -> list[Investigacao]:
        stmt = (
            select(InvestigacaoModel)
            .where(InvestigacaoModel.status == status.value)
            .order_by(InvestigacaoModel.data_abertura.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(InvestigacaoModel)
            .where(InvestigacaoModel.codigo_investigacao.like(f"{prefix}%"))
        )
    ...
    def _to_domain(model: InvestigacaoModel) -> Investigacao:
        return Investigacao(
            id=model.id,
            codigo_investigacao=model.codigo_investigacao,
            ocorrencia_id=model.ocorrencia_id,
            unidade_id=model.unidade_id,
            data_abertura=model.data_abertura,
            status=StatusInvestigacao(model.status),
            delegado_responsavel_id=model.delegado_responsavel_id,
