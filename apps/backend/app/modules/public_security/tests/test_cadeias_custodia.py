...
def test_iniciar_cadeia_custodia_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        prova = await prova_service.coletar_prova(
            ...
            descricao="Material embalado para custodia",
            local_coleta="Armazem principal",
            coletado_por_id=perito.id,
        )
        cadeia = await cadeia_service.iniciar_cadeia(
            prova_id=prova.id, local_atual="Cofre central", responsavel_id=perito.id
        )
...
def test_iniciar_cadeia_custodia_falha_duplicidade_prova() -> None:
    ...
    async def scenario() -> None:
        ...
        unidade = await unidade_service.cadastrar_unidade(
            nome="Base Custodia Norte",
            tipo=TipoUnidadePolicial.BASE_OPERACIONAL,
            municipio="Bengo",
            provincia="Bengo",
            endereco="Zona Norte",
            comandante="Comandante Norte",
        )
        responsavel = await policial_service.cadastrar_policial(
            unidade_id=unidade.id,
            nome="Agente Custodia",
        ...
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            ...
            descricao="Material apreendido para analise",
            municipio="Bengo",
            provincia="Bengo",
        )
        prova = await prova_service.coletar_prova(
            ocorrencia_id=ocorrencia.id,
            tipo=TipoProva.DOCUMENTAL,
            descricao="Documentacao sensivel",
            local_coleta="Arquivo local",
        )
        await cadeia_service.iniciar_cadeia(
            prova_id=prova.id, local_atual="Deposito A", responsavel_id=responsavel.id
        )
        with pytest.raises(ValueError, match="Ja existe cadeia de custodia"):
            await cadeia_service.iniciar_cadeia(
                prova_id=prova.id, local_atual="Deposito B", responsavel_id=responsavel.id
            )
...
def test_registrar_movimentacao_cadeia_custodia() -> None:
    ...
    async def scenario() -> None:
        ...
        unidade = await unidade_service.cadastrar_unidade(
            nome="Base Custodia Sul",
            tipo=TipoUnidadePolicial.BASE_OPERACIONAL,
            municipio="Luanda",
            provincia="Luanda",
            endereco="Zona Sul",
            comandante="Comandante Sul",
        )
        responsavel = await policial_service.cadastrar_policial(
            unidade_id=unidade.id,
            nome="Operador Custodia",
        ...
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            unidade_id=unidade.id,
            policial_responsavel_id=responsavel.id,
            tipo=TipoOcorrencia.DANO,
            prioridade=PrioridadeOcorrencia.BAIXA,
            data_ocorrencia=datetime.now() - timedelta(hours=8),
            descricao="Dano em equipamento publico",
            municipio="Luanda",
            provincia="Luanda",
        )
        prova = await prova_service.coletar_prova(
            ...
            descricao="Imagem digital do equipamento danificado",
            local_coleta="Centro de monitoramento",
            coletado_por_id=responsavel.id,
        )
        cadeia = await cadeia_service.iniciar_cadeia(
            prova_id=prova.id, local_atual="Sala 01", responsavel_id=responsavel.id
        )
        atualizada = await cadeia_service.registrar_movimentacao(
            cadeia_id=cadeia.id,
            status=StatusCadeiaCustodia.EM_TRANSITO,
            local_atual="Laboratorio forense",
            responsavel_id=responsavel.id,
            observacao="Transferencia para analise",
        )
