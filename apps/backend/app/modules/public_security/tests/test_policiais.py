...
def test_ativar_porte_policial() -> None:
    ...
    async def scenario() -> None:
        ...
        unidade = await unidade_service.cadastrar_unidade(
            nome="Companhia Sul",
            tipo=TipoUnidadePolicial.COMPANHIA,
            municipio="Benguela",
            provincia="Benguela",
            endereco="Av Sul",
            comandante="Major Sul",
        )
        policial = await policial_service.cadastrar_policial(
            unidade_id=unidade.id,
            nome="Carlos Costa",
            ...
            cpf="321.654.987-10",
            rg="RG445566",
            tipo=TipoAgente.MILITAR,
            vinculo=TipoVinculo.EFETIVO,
        )
        atualizado = await policial_service.ativar_porte(
            policial_id=policial.id,
            numero_porte="PA-2026-0001",
