...
def test_expedir_mandado_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        mandado_service = MandadoService(
            mandado_repo=mandado_repo, ocorrencia_repo=ocorrencia_repo, policial_repo=policial_repo
        )
        ...
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            ...
            descricao="Roubo qualificado em estabelecimento",
            municipio="Luanda",
            provincia="Luanda",
        )
        mandado = await mandado_service.expedir_mandado(
            ...
            autoridade_judicial="Juizo Criminal Central",
...
def test_expedir_mandado_falha_ocorrencia_inexistente() -> None:
    ...
    async def scenario() -> None:
        service = MandadoService(
            mandado_repo=InMemoryMandadoRepository(),
            ocorrencia_repo=InMemoryOcorrenciaRepository(),
            policial_repo=InMemoryPolicialRepository(),
        )
        with pytest.raises(ValueError, match="Ocorrencia nao encontrada"):
            await service.expedir_mandado(
                ocorrencia_id=uuid4(),
                tipo=TipoMandado.PRISAO,
                autoridade_judicial="Juizo A",
                data_expedicao=date.today(),
...
def test_atualizar_status_mandado() -> None:
    ...
    async def scenario() -> None:
        ...
        mandado_repo = InMemoryMandadoRepository()
        unidade_service = UnidadePolicialService(unidade_repo=unidade_repo)
        PolicialService(policial_repo=policial_repo, unidade_repo=unidade_repo)
        ocorrencia_service = OcorrenciaService(
            ocorrencia_repo=ocorrencia_repo, unidade_repo=unidade_repo, policial_repo=policial_repo
        )
        mandado_service = MandadoService(
        ...
        unidade = await unidade_service.cadastrar_unidade(
            ...
            provincia="Luanda",
            endereco="Rua Norte",
            comandante="Delegado Norte",
        )
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            unidade_id=unidade.id,
            tipo=TipoOcorrencia.FURTO,
            prioridade=PrioridadeOcorrencia.MEDIA,
            data_ocorrencia=datetime.now() - timedelta(hours=5),
            descricao="Furto de carga em armazem",
            municipio="Luanda",
            provincia="Luanda",
        )
        mandado = await mandado_service.expedir_mandado(
            ocorrencia_id=ocorrencia.id,
            tipo=TipoMandado.PRISAO,
            autoridade_judicial="Juizo Provincial",
            data_expedicao=date.today(),
            data_validade=date.today() + timedelta(days=20),
        )
        atualizado = await mandado_service.atualizar_status(
            mandado_id=mandado.id,
            status=StatusMandado.CUMPRIDO,
            observacoes="Cumprido sem resistencia",
        )
