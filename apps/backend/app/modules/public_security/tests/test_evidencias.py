...
def test_registrar_evidencia_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        unidade = await unidade_service.cadastrar_unidade(
            nome="Nucleo Evidencias",
            tipo=TipoUnidadePolicial.BASE_OPERACIONAL,
            municipio="Luanda",
            provincia="Luanda",
            endereco="Rua E1",
            comandante="Comandante E1",
        )
        agente = await policial_service.cadastrar_policial(
            unidade_id=unidade.id,
            nome="Analista Evidencias",
        ...
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            unidade_id=unidade.id,
            policial_responsavel_id=agente.id,
            tipo=TipoOcorrencia.FALSIDADE,
            prioridade=PrioridadeOcorrencia.ALTA,
            data_ocorrencia=datetime.now() - timedelta(hours=3),
            descricao="Ocorrencia para registro de evidencia",
            municipio="Luanda",
            provincia="Luanda",
        )
        prova = await prova_service.coletar_prova(
            ...
            descricao="Documentos para análise",
            local_coleta="Arquivo A",
            coletado_por_id=agente.id,
        )
        cadeia = await cadeia_service.iniciar_cadeia(
            prova_id=prova.id, local_atual="Cofre E1", responsavel_id=agente.id
        )
        vestigio = await vestigio_service.registrar_vestigio(
            cadeia_custodia_id=cadeia.id,
            tipo=TipoVestigio.DOCUMENTO,
            descricao="Documento original coletado",
        ...
        evidencia = await evidencia_service.registrar_evidencia(
            ...
            descricao="Evidencia documental autenticada",
            fonte="Arquivo central",
            confiabilidade=5,
            analisado_por_id=agente.id,
        )
    ...
    async def scenario() -> None:
        ...
        with pytest.raises(ValueError, match="Vestigio nao encontrado"):
            await service.registrar_evidencia(
                vestigio_id=uuid4(),
                tipo=TipoEvidencia.FISICA,
                descricao="Evidencia sem vestigio base",
                fonte="Fonte X",
            )
...
def test_atualizar_status_evidencia() -> None:
    ...
    async def scenario() -> None:
        ...
        unidade = await unidade_service.cadastrar_unidade(
            nome="Nucleo Evidencias B",
            tipo=TipoUnidadePolicial.BASE_OPERACIONAL,
            municipio="Bengo",
            provincia="Bengo",
            endereco="Rua E2",
            comandante="Comandante E2",
        )
        agente = await policial_service.cadastrar_policial(
            unidade_id=unidade.id,
            nome="Analista B",
        ...
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            ...
            descricao="Ocorrencia para update de evidencia",
            municipio="Bengo",
            provincia="Bengo",
        )
        prova = await prova_service.coletar_prova(
            ...
            descricao="Arquivo digital para evidência",
            local_coleta="Sala B",
        )
        cadeia = await cadeia_service.iniciar_cadeia(
            prova_id=prova.id, local_atual="Cofre E2", responsavel_id=agente.id
        )
        vestigio = await vestigio_service.registrar_vestigio(
            cadeia_custodia_id=cadeia.id,
            tipo=TipoVestigio.MIDIA_DIGITAL,
            descricao="HD externo apreendido",
            localizacao="Armario 5",
            coletado_por_id=agente.id,
        )
        evidencia = await evidencia_service.registrar_evidencia(
            vestigio_id=vestigio.id,
            tipo=TipoEvidencia.DIGITAL,
            descricao="Evidencia digital indexada",
            fonte="Laboratorio digital",
            confiabilidade=4,
        )
        atualizada = await evidencia_service.atualizar_status(
            evidencia_id=evidencia.id,
            status=StatusEvidencia.VALIDADA,
            observacoes="Validada em conferência técnica",
        )
