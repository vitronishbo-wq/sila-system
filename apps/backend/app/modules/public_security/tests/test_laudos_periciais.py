...
from apps.backend.app.modules.public_security.tests._fakes import (
    InMemoryLaudoPericialRepository,
...
def test_emitir_laudo_pericial_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        unidade = await unidade_service.cadastrar_unidade(
            nome="Instituto Pericial",
            tipo=TipoUnidadePolicial.DELEGACIA,
            municipio="Luanda",
            provincia="Luanda",
            endereco="Rua Forense",
            comandante="Diretor Forense",
        )
        perito = await policial_service.cadastrar_policial(
            unidade_id=unidade.id,
            nome="Perito Almeida",
            data_nascimento=date.today() - timedelta(days=365 * 38),
            cpf="222.777.999-00",
            rg="RG227799",
        ...
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            ...
            tipo=TipoOcorrencia.CORRUPCAO,
            prioridade=PrioridadeOcorrencia.ALTA,
            data_ocorrencia=datetime.now() - timedelta(days=1),
            descricao="Apreensao de dispositivo para analise digital",
            municipio="Luanda",
            provincia="Luanda",
        )
        prova = await prova_service.coletar_prova(
            ...
            descricao="Notebook apreendido no local",
            local_coleta="Laboratorio digital",
        ...
        laudo = await laudo_service.emitir_laudo(
            ...
            perito_id=perito.id,
            conclusao="Foram encontrados artefatos digitais compatíveis com a investigacao.",
        )
...
def test_emitir_laudo_pericial_falha_prova_inexistente() -> None:
    ...
    async def scenario() -> None:
        service = LaudoPericialService(
            laudo_repo=InMemoryLaudoPericialRepository(),
            prova_repo=InMemoryProvaPericialRepository(),
            policial_repo=InMemoryPolicialRepository(),
        )
        with pytest.raises(ValueError, match="Prova pericial nao encontrada"):
            await service.emitir_laudo(
                prova_id=uuid4(),
                tipo_laudo=TipoLaudo.BALISTICO,
                perito_id=uuid4(),
                conclusao="Laudo sem prova deve falhar por regra de dominio.",
            )
...
def test_atualizar_status_laudo_pericial() -> None:
    ...
    async def scenario() -> None:
        ...
        unidade = await unidade_service.cadastrar_unidade(
            ...
            municipio="Bengo",
            provincia="Bengo",
            endereco="Rua Balistica",
            comandante="Chefe Balistico",
        )
        perito = await policial_service.cadastrar_policial(
            unidade_id=unidade.id,
            nome="Perito Balistico",
            data_nascimento=date.today() - timedelta(days=365 * 39),
            cpf="444.555.666-77",
            rg="RG445566",
            tipo=TipoAgente.PERITO,
            vinculo=TipoVinculo.EFETIVO,
        ...
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            unidade_id=unidade.id,
            policial_responsavel_id=perito.id,
            tipo=TipoOcorrencia.POSSE_ARMA,
            prioridade=PrioridadeOcorrencia.MEDIA,
            data_ocorrencia=datetime.now() - timedelta(hours=12),
            descricao="Arma apreendida para exame balistico",
            municipio="Bengo",
            provincia="Bengo",
        )
        prova = await prova_service.coletar_prova(
            ocorrencia_id=ocorrencia.id,
            tipo=TipoProva.BALISTICA,
            descricao="Projetil e arma para confronto",
            local_coleta="Sala de custodia",
            coletado_por_id=perito.id,
        )
        laudo = await laudo_service.emitir_laudo(
            prova_id=prova.id,
            tipo_laudo=TipoLaudo.BALISTICO,
            perito_id=perito.id,
            conclusao="Correspondencia entre arma apreendida e vestigios analisados.",
        )
        atualizado = await laudo_service.atualizar_status(
            laudo_id=laudo.id,
            status=StatusLaudo.RETIFICADO,
            observacoes="Ajuste de metadados do exame",
        )
