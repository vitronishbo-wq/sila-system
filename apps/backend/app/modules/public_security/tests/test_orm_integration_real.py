...
def _tables():
    from apps.backend.app.modules.public_security.infrastructure.models.cadeia_custodia_model import (
        CadeiaCustodiaModel,
    )
    from apps.backend.app.modules.public_security.infrastructure.models.evidencia_model import (
        EvidenciaModel,
    )
    from apps.backend.app.modules.public_security.infrastructure.models.investigacao_model import (
        InvestigacaoModel,
    )
    from apps.backend.app.modules.public_security.infrastructure.models.laudo_pericial_model import (
        LaudoPericialModel,
    )
    from apps.backend.app.modules.public_security.infrastructure.models.mandado_model import (
        MandadoModel,
    )
    from apps.backend.app.modules.public_security.infrastructure.models.ocorrencia_model import (
        OcorrenciaModel,
    )
    ...
    return [
        UnidadePolicialModel.__table__,
        PolicialModel.__table__,
        OcorrenciaModel.__table__,
        MandadoModel.__table__,
        InvestigacaoModel.__table__,
        ProvaPericialModel.__table__,
        CadeiaCustodiaModel.__table__,
        LaudoPericialModel.__table__,
        VestigioModel.__table__,
        EvidenciaModel.__table__,
    ]
...
def test_fluxo_real_orm_seguranca_publica_slices_foundation_investigativo_forense() -> None:
    ...
    async def scenario() -> None:
        async with _session_scope() as session:
            ...
            policial_service = PolicialService(
                policial_repo=policial_repo, unidade_repo=unidade_repo
            )
            ...
            mandado_service = MandadoService(
                mandado_repo=mandado_repo,
                ocorrencia_repo=ocorrencia_repo,
                policial_repo=policial_repo,
            )
            ...
            laudo_service = LaudoPericialService(
                laudo_repo=laudo_repo, prova_repo=prova_repo, policial_repo=policial_repo
            )
            ...
            evidencia_service = EvidenciaService(
                evidencia_repo=evidencia_repo,
                vestigio_repo=vestigio_repo,
                cadeia_repo=cadeia_repo,
                policial_repo=policial_repo,
            )
            unidade = await unidade_service.cadastrar_unidade(
                nome="Delegacia ORM Seguranca",
                tipo=TipoUnidadePolicial.DELEGACIA,
                municipio="Luanda",
                provincia="Luanda",
                endereco="Rua ORM 100",
                comandante="Delegado ORM",
            )
            policial = await policial_service.cadastrar_policial(
                unidade_id=unidade.id,
                nome="Inspector ORM",
                data_nascimento=date.today() - timedelta(days=365 * 35),
                cpf="999.888.777-66",
                rg="RG999888",
            ...
            ocorrencia = await ocorrencia_service.registrar_ocorrencia(
                ...
                data_ocorrencia=datetime.now() - timedelta(hours=4),
                descricao="Teste integracao ORM seguranca publica",
                municipio="Luanda",
                provincia="Luanda",
                vitimas=2,
                suspeitos=1,
            )
            mandado = await mandado_service.expedir_mandado(
                ocorrencia_id=ocorrencia.id,
                tipo=TipoMandado.BUSCA_APREENSAO,
                autoridade_judicial="Juizo Criminal ORM",
            ...
            investigacao = await investigacao_service.abrir_investigacao(
                ocorrencia_id=ocorrencia.id,
                delegado_responsavel_id=policial.id,
                resumo="Fluxo ORM de investigacao",
            )
            prova = await prova_service.coletar_prova(
                ocorrencia_id=ocorrencia.id,
                tipo=TipoProva.DIGITAL,
                descricao="Midia digital apreendida para pericia",
                local_coleta="Laboratorio forense",
                coletado_por_id=policial.id,
            )
            cadeia = await cadeia_service.iniciar_cadeia(
                prova_id=prova.id, local_atual="Cofre forense", responsavel_id=policial.id
            )
            await cadeia_service.registrar_movimentacao(
                cadeia_id=cadeia.id,
                status=StatusCadeiaCustodia.EM_TRANSITO,
                local_atual="Laboratorio de analise",
                responsavel_id=policial.id,
                observacao="Transferida para analise",
            )
            laudo = await laudo_service.emitir_laudo(
                prova_id=prova.id,
                tipo_laudo=TipoLaudo.INFORMATICA,
                perito_id=policial.id,
                conclusao="Evidencias digitais compativeis com a narrativa da ocorrencia.",
            )
            vestigio = await vestigio_service.registrar_vestigio(
                cadeia_custodia_id=cadeia.id,
                tipo=TipoVestigio.MIDIA_DIGITAL,
                descricao="Vestigio digital associado ao material periciado",
                localizacao="Laboratorio de analise",
                coletado_por_id=policial.id,
            )
            evidencia = await evidencia_service.registrar_evidencia(
                vestigio_id=vestigio.id,
                tipo=TipoEvidencia.DIGITAL,
                descricao="Evidencia digital consolidada para instruir a investigacao",
                fonte="Analise forense",
                confiabilidade=5,
                analisado_por_id=policial.id,
            )
