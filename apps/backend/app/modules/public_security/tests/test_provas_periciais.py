...
def test_coletar_prova_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        unidade = await unidade_service.cadastrar_unidade(
            nome="Delegacia Cientifica",
            tipo=TipoUnidadePolicial.DELEGACIA,
            municipio="Luanda",
            provincia="Luanda",
            endereco="Rua Cientifica",
            comandante="Delegado Pericial",
        )
        perito = await policial_service.cadastrar_policial(
            unidade_id=unidade.id,
            nome="Perito Ferreira",
            data_nascimento=date.today() - timedelta(days=365 * 36),
            cpf="666.777.888-99",
            rg="RG667788",
        ...
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            ...
            descricao="Apreensao de documentos com suspeita de falsificacao",
            municipio="Luanda",
            provincia="Luanda",
        )
        prova = await prova_service.coletar_prova(
            ...
            descricao="Documentos apreendidos para exame",
            local_coleta="Arquivo central",
...
def test_coletar_prova_falha_ocorrencia_inexistente() -> None:
    ...
    async def scenario() -> None:
        service = ProvaPericialService(
            prova_repo=InMemoryProvaPericialRepository(),
            ocorrencia_repo=InMemoryOcorrenciaRepository(),
            policial_repo=InMemoryPolicialRepository(),
        )
        with pytest.raises(ValueError, match="Ocorrencia nao encontrada"):
            await service.coletar_prova(
                ocorrencia_id=uuid4(),
                tipo=TipoProva.BIOLOGICA,
                descricao="Vestigio biologico coletado no local",
                local_coleta="Laboratorio",
            )
...
def test_vincular_cadeia_custodia_prova() -> None:
    ...
    async def scenario() -> None:
        ...
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(
            ...
            descricao="Apreensao de substancia entorpecente",
            municipio="Bengo",
            provincia="Bengo",
        )
        prova = await prova_service.coletar_prova(
            ...
            descricao="Amostra de substancia apreendida",
            local_coleta="Viatura operacional",
        )
        cadeia_id = uuid4()
        atualizada = await prova_service.vincular_cadeia_custodia(
            prova_id=prova.id, cadeia_custodia_id=cadeia_id
        )
