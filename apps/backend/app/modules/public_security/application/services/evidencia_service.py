...
class EvidenciaService:
    ...
    def __init__(self, *, evidencia_repo: EvidenciaRepositoryPort, vestigio_repo: VestigioRepositoryPort, cadeia_repo: CadeiaCustodiaRepositoryPort, policial_repo: PolicialRepositoryPort, request_service: RequestServicePort | None=None) -> None:
    ...
    async def registrar_evidencia(self, *, vestigio_id: UUID, tipo: TipoEvidencia, descricao: str, fonte: str, confiabilidade: int=3, analisado_por_id: UUID | None=None, observacoes: str | None=None, citizen_id: UUID | None=None) -> Evidencia:
        ...
        evidencia = Evidencia.registrar(codigo_evidencia=codigo, vestigio_id=vestigio_id, cadeia_custodia_id=vestigio.cadeia_custodia_id, tipo=tipo, descricao=descricao, fonte=fonte, confiabilidade=confiabilidade, analisado_por_id=analisado_por_id, observacoes=observacoes)
        saved = await self.evidencia_repo.save(evidencia)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='REGISTRO_EVIDENCIA', entity_id=saved.id, numero_processo=saved.codigo_evidencia, citizen_id=citizen_id, metadata={'codigo_evidencia': saved.codigo_evidencia, 'vestigio_id': str(saved.vestigio_id), 'cadeia_custodia_id': str(saved.cadeia_custodia_id), 'tipo': saved.tipo.value, 'confiabilidade': saved.confiabilidade})
        return saved
    ...
    async def listar_evidencias(self, *, vestigio_id: UUID | None=None, status: StatusEvidencia | None=None) -> list[Evidencia]:
    ...
    async def atualizar_status(self, *, evidencia_id: UUID, status: StatusEvidencia, observacoes: str | None=None) -> Evidencia:
