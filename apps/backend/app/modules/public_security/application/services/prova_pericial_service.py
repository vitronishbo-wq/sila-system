...
class ProvaPericialService:
    ...
    def __init__(self, *, prova_repo: ProvaPericialRepositoryPort, ocorrencia_repo: OcorrenciaRepositoryPort, policial_repo: PolicialRepositoryPort, request_service: RequestServicePort | None=None) -> None:
        self.prova_repo = prova_repo
        self.ocorrencia_repo = ocorrencia_repo
        self.policial_repo = policial_repo
        self.request_service = request_service

    async def coletar_prova(self, *, ocorrencia_id: UUID, tipo: TipoProva, descricao: str, local_coleta: str, data_coleta: date | None=None, coletado_por_id: UUID | None=None, observacoes: str | None=None, citizen_id: UUID | None=None) -> ProvaPericial:
        ...
        prova = ProvaPericial.coletar(codigo_prova=codigo, ocorrencia_id=ocorrencia_id, tipo=tipo, descricao=descricao, local_coleta=local_coleta, data_coleta=data_coleta, coletado_por_id=coletado_por_id, observacoes=observacoes)
        saved = await self.prova_repo.save(prova)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='COLETA_PROVA_PERICIAL', entity_id=saved.id, numero_processo=saved.codigo_prova, citizen_id=citizen_id, metadata={'codigo_prova': saved.codigo_prova, 'ocorrencia_id': str(saved.ocorrencia_id), 'tipo': saved.tipo.value})
        return saved
    ...
    async def listar_provas(self, *, ocorrencia_id: UUID | None=None, tipo: TipoProva | None=None, status: StatusProva | None=None) -> list[ProvaPericial]:
    ...
    async def atualizar_status(self, *, prova_id: UUID, status: StatusProva, observacoes: str | None=None) -> ProvaPericial:
        prova = await self.buscar_prova(prova_id)
        prova.atualizar_status(status, observacoes)
        return await self.prova_repo.save(prova)

    async def vincular_cadeia_custodia(self, *, prova_id: UUID, cadeia_custodia_id: UUID) -> ProvaPericial:
        prova = await self.buscar_prova(prova_id)
