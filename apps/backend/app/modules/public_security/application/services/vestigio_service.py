...
class VestigioService:
    ...
    async def registrar_vestigio(
        ...
        *,
        ...
        vestigio = Vestigio.registrar(
            codigo_vestigio=codigo,
            cadeia_custodia_id=cadeia_custodia_id,
            ocorrencia_id=cadeia.ocorrencia_id,
            tipo=tipo,
            descricao=descricao,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="REGISTRO_VESTIGIO",
                entity_id=saved.id,
                numero_processo=saved.codigo_vestigio,
                citizen_id=citizen_id,
                metadata={
                    "codigo_vestigio": saved.codigo_vestigio,
    ...
    async def listar_vestigios(
        self, *, cadeia_custodia_id: UUID | None = None, status: StatusVestigio | None = None
    ) -> list[Vestigio]:
    ...
    async def atualizar_status(
        self, *, vestigio_id: UUID, status: StatusVestigio, observacoes: str | None = None
    ) -> Vestigio:
