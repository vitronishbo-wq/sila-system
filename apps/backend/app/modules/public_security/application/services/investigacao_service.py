...
class InvestigacaoService:
    ...
    def __init__(self, *, investigacao_repo: InvestigacaoRepositoryPort, ocorrencia_repo: OcorrenciaRepositoryPort, policial_repo: PolicialRepositoryPort, request_service: RequestServicePort | None=None) -> None:
        self.investigacao_repo = investigacao_repo
        self.ocorrencia_repo = ocorrencia_repo
        self.policial_repo = policial_repo
        self.request_service = request_service

    async def abrir_investigacao(self, *, ocorrencia_id: UUID, delegado_responsavel_id: UUID | None=None, resumo: str | None=None, observacoes: str | None=None, citizen_id: UUID | None=None) -> Investigacao:
        ...
        investigacao = Investigacao.abrir(codigo_investigacao=codigo, ocorrencia_id=ocorrencia_id, unidade_id=ocorrencia.unidade_id, delegado_responsavel_id=delegado_responsavel_id, resumo=resumo, observacoes=observacoes)
        saved = await self.investigacao_repo.save(investigacao)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='ABERTURA_INVESTIGACAO', entity_id=saved.id, numero_processo=saved.codigo_investigacao, citizen_id=citizen_id, metadata={'codigo_investigacao': saved.codigo_investigacao, 'ocorrencia_id': str(saved.ocorrencia_id), 'unidade_id': str(saved.unidade_id)})
        return saved
    ...
    async def listar_investigacoes(self, *, ocorrencia_id: UUID | None=None, status: StatusInvestigacao | None=None) -> list[Investigacao]:
    ...
    async def atualizar_status(self, *, investigacao_id: UUID, status: StatusInvestigacao, observacoes: str | None=None) -> Investigacao:
