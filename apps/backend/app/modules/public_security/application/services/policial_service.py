...
class PolicialService:
    ...
    def __init__(self, *, policial_repo: PolicialRepositoryPort, unidade_repo: UnidadePolicialRepositoryPort, request_service: RequestServicePort | None=None) -> None:
        self.policial_repo = policial_repo
        self.unidade_repo = unidade_repo
        self.request_service = request_service

    async def cadastrar_policial(self, *, unidade_id: UUID, nome: str, data_nascimento: date, cpf: str, rg: str, tipo: TipoAgente, vinculo: TipoVinculo, cargo: CargoPolicial | None=None, patente: Patente | None=None, telefone: str | None=None, email: str | None=None, endereco: str | None=None, observacoes: str | None=None, citizen_id: UUID | None=None) -> Policial:
        ...
        policial = Policial.cadastrar(matricula=matricula, unidade_id=unidade_id, nome=nome, data_nascimento=data_nascimento, cpf=cpf, rg=rg, tipo=tipo, vinculo=vinculo, cargo=cargo, patente=patente, telefone=telefone, email=email, endereco=endereco, observacoes=observacoes)
        saved = await self.policial_repo.save(policial)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='CADASTRO_POLICIAL', entity_id=saved.id, citizen_id=citizen_id, numero_processo=saved.matricula, metadata={'matricula': saved.matricula, 'nome': saved.nome, 'tipo': saved.tipo.value, 'unidade_codigo': unidade.codigo_unidade})
        return saved
    ...
    async def listar_policiais(self, *, unidade_id: UUID | None=None, tipo: TipoAgente | None=None, status: StatusAgente | None=None) -> list[Policial]:
    ...
    async def atualizar_status(self, *, policial_id: UUID, status: StatusAgente, motivo: str | None=None) -> Policial:
    ...
    async def ativar_porte(self, *, policial_id: UUID, numero_porte: str, data_validade: date) -> Policial:
        policial = await self.buscar_policial(policial_id)
