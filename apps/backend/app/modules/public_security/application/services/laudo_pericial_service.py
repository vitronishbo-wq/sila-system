...
class LaudoPericialService:
    def __init__(
        self,
        *,
        laudo_repo: LaudoPericialRepositoryPort,
        ...
        *,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="EMISSAO_LAUDO_PERICIAL",
                entity_id=saved.id,
                numero_processo=saved.numero_laudo,
                citizen_id=citizen_id,
                metadata={
                    "numero_laudo": saved.numero_laudo,
                    "prova_id": str(saved.prova_id),
                    "tipo_laudo": saved.tipo_laudo.value,
                },
            )
        return saved
    ...
    async def atualizar_status(
        self, *, laudo_id: UUID, status: StatusLaudo, observacoes: str | None = None
    ) -> LaudoPericial:
