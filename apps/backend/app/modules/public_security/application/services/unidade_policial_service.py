...
class UnidadePolicialService:
    ...
    async def cadastrar_unidade(
        ...
        *,
        ...
        unidade = UnidadePolicial.cadastrar(
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CADASTRO_UNIDADE_POLICIAL",
                entity_id=saved.id,
                numero_processo=saved.codigo_unidade,
                metadata={
                    "codigo_unidade": saved.codigo_unidade,
                    "nome": saved.nome,
    ...
    async def listar_unidades(
        self, *, municipio: str | None = None, status: StatusUnidadePolicial | None = None
    ) -> list[UnidadePolicial]:
        if municipio:
    ...
    async def atualizar_status(
        self, *, unidade_id: UUID, status: StatusUnidadePolicial, motivo: str | None = None
    ) -> UnidadePolicial:
