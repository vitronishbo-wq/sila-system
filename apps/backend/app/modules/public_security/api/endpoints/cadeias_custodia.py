...
cadeia_custodia_service_dep = Depends(get_cadeia_custodia_service)


@router.post("/", response_model=CadeiaCustodiaResponse, status_code=status.HTTP_201_CREATED)
async def iniciar_cadeia_custodia(
    data: CadeiaCustodiaCreate,
    service: CadeiaCustodiaService = cadeia_custodia_service_dep,
) -> CadeiaCustodiaResponse:
    try:
        return await service.iniciar_cadeia(
...
async def obter_cadeia_custodia(
    cadeia_id: UUID, service: CadeiaCustodiaService = cadeia_custodia_service_dep
) -> CadeiaCustodiaResponse:
    try:
...
async def listar_cadeias_custodia(
    status_cadeia: StatusCadeiaCustodia | None = None,
    service: CadeiaCustodiaService = cadeia_custodia_service_dep,
) -> list[CadeiaCustodiaResponse]:
...
async def registrar_movimentacao_cadeia(
    cadeia_id: UUID,
    data: CadeiaCustodiaMovimentacao,
    service: CadeiaCustodiaService = cadeia_custodia_service_dep,
) -> CadeiaCustodiaResponse:
    try:
        return await service.registrar_movimentacao(
            cadeia_id=cadeia_id,
            status=data.status,
            local_atual=data.local_atual,
            responsavel_id=data.responsavel_id,
...
async def remover_cadeia_custodia(
    cadeia_id: UUID, service: CadeiaCustodiaService = cadeia_custodia_service_dep
) -> None:
    try:
