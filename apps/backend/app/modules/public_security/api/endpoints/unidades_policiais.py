...
from apps.backend.app.modules.public_security.api.deps import get_unidade_policial_service
...
unidade_policial_service_dep = Depends(get_unidade_policial_service)


@router.post("/", response_model=UnidadePolicialResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_unidade(
    data: UnidadePolicialCreate,
    service: UnidadePolicialService = unidade_policial_service_dep,
) -> UnidadePolicialResponse:
    try:
...
async def obter_unidade(
    unidade_id: UUID, service: UnidadePolicialService = unidade_policial_service_dep
) -> UnidadePolicialResponse:
    try:
...
async def listar_unidades(
    ...
    status_unidade: StatusUnidadePolicial | None = None,
    service: UnidadePolicialService = unidade_policial_service_dep,
) -> list[UnidadePolicialResponse]:
...
async def atualizar_status_unidade(
    unidade_id: UUID,
    data: UnidadePolicialStatusUpdate,
    service: UnidadePolicialService = unidade_policial_service_dep,
) -> UnidadePolicialResponse:
    try:
        return await service.atualizar_status(
            unidade_id=unidade_id, status=data.status, motivo=data.motivo
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
