...
ocorrencia_service_dep = Depends(get_ocorrencia_service)


@router.post("/", response_model=OcorrenciaResponse, status_code=status.HTTP_201_CREATED)
async def registrar_ocorrencia(
    data: OcorrenciaCreate, service: OcorrenciaService = ocorrencia_service_dep
) -> OcorrenciaResponse:
    try:
        return await service.registrar_ocorrencia(
            ...
            preso_em_flagrante=data.preso_em_flagrante,
...
async def obter_ocorrencia(
    ocorrencia_id: UUID, service: OcorrenciaService = ocorrencia_service_dep
) -> OcorrenciaResponse:
    try:
...
async def listar_ocorrencias(
    ...
    service: OcorrenciaService = ocorrencia_service_dep,
) -> list[OcorrenciaResponse]:
    return await service.listar_ocorrencias(
        unidade_id=unidade_id, tipo=tipo, status=status_ocorrencia, inicio=inicio, fim=fim
    )
