...
policial_service_dep = Depends(get_policial_service)


@router.post("/", response_model=PolicialResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_policial(
    data: PolicialCreate, service: PolicialService = policial_service_dep
) -> PolicialResponse:
    try:
        return await service.cadastrar_policial(
...
async def obter_policial(
    policial_id: UUID, service: PolicialService = policial_service_dep
) -> PolicialResponse:
    try:
...
async def listar_policiais(
    ...
    status_policial: StatusAgente | None = None,
    service: PolicialService = policial_service_dep,
) -> list[PolicialResponse]:
...
async def atualizar_status_policial(
    policial_id: UUID,
    data: PolicialStatusUpdate,
    service: PolicialService = policial_service_dep,
) -> PolicialResponse:
    try:
        return await service.atualizar_status(
            policial_id=policial_id, status=data.status, motivo=data.motivo
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def ativar_porte_policial(
    policial_id: UUID,
    data: PolicialPorteUpdate,
    service: PolicialService = policial_service_dep,
) -> PolicialResponse:
    try:
        return await service.ativar_porte(
