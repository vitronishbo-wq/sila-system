...
vestigio_service_dep = Depends(get_vestigio_service)


@router.post("/", response_model=VestigioResponse, status_code=status.HTTP_201_CREATED)
async def registrar_vestigio(
    data: VestigioCreate, service: VestigioService = vestigio_service_dep
) -> VestigioResponse:
    try:
        return await service.registrar_vestigio(
            cadeia_custodia_id=data.cadeia_custodia_id,
            tipo=data.tipo,
...
async def obter_vestigio(
    vestigio_id: UUID, service: VestigioService = vestigio_service_dep
) -> VestigioResponse:
    try:
...
async def listar_vestigios(
    cadeia_custodia_id: UUID | None = None,
    status_vestigio: StatusVestigio | None = None,
    service: VestigioService = vestigio_service_dep,
) -> list[VestigioResponse]:
    return await service.listar_vestigios(
        cadeia_custodia_id=cadeia_custodia_id, status=status_vestigio
    )


@router.patch("/{vestigio_id}/status", response_model=VestigioResponse)
async def atualizar_status_vestigio(
    vestigio_id: UUID,
    data: VestigioStatusUpdate,
    service: VestigioService = vestigio_service_dep,
) -> VestigioResponse:
    try:
        return await service.atualizar_status(
            vestigio_id=vestigio_id, status=data.status, observacoes=data.observacoes
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
