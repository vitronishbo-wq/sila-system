...
investigacao_service_dep = Depends(get_investigacao_service)


@router.post("/", response_model=InvestigacaoResponse, status_code=status.HTTP_201_CREATED)
async def abrir_investigacao(
    data: InvestigacaoCreate, service: InvestigacaoService = investigacao_service_dep
) -> InvestigacaoResponse:
    try:
        return await service.abrir_investigacao(
            ocorrencia_id=data.ocorrencia_id,
            delegado_responsavel_id=data.delegado_responsavel_id,
            resumo=data.resumo,
...
async def obter_investigacao(
    investigacao_id: UUID, service: InvestigacaoService = investigacao_service_dep
) -> InvestigacaoResponse:
    try:
...
async def listar_investigacoes(
    ...
    status_investigacao: StatusInvestigacao | None = None,
    service: InvestigacaoService = investigacao_service_dep,
) -> list[InvestigacaoResponse]:
    return await service.listar_investigacoes(
        ocorrencia_id=ocorrencia_id, status=status_investigacao
    )


@router.patch("/{investigacao_id}/status", response_model=InvestigacaoResponse)
async def atualizar_status_investigacao(
    investigacao_id: UUID,
    data: InvestigacaoStatusUpdate,
    service: InvestigacaoService = investigacao_service_dep,
) -> InvestigacaoResponse:
    try:
        return await service.atualizar_status(
            investigacao_id=investigacao_id, status=data.status, observacoes=data.observacoes
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def remover_investigacao(
    investigacao_id: UUID, service: InvestigacaoService = investigacao_service_dep
) -> None:
    try:
