...
laudo_pericial_service_dep = Depends(get_laudo_pericial_service)


@router.post("/", response_model=LaudoPericialResponse, status_code=status.HTTP_201_CREATED)
async def emitir_laudo_pericial(
    data: LaudoPericialCreate, service: LaudoPericialService = laudo_pericial_service_dep
) -> LaudoPericialResponse:
    try:
        return await service.emitir_laudo(
...
async def obter_laudo_pericial(
    laudo_id: UUID, service: LaudoPericialService = laudo_pericial_service_dep
) -> LaudoPericialResponse:
    try:
...
async def listar_laudos_periciais(
    prova_id: UUID | None = None,
    tipo: TipoLaudo | None = None,
    status_laudo: StatusLaudo | None = None,
    service: LaudoPericialService = laudo_pericial_service_dep,
) -> list[LaudoPericialResponse]:
...
async def atualizar_status_laudo(
    laudo_id: UUID,
    data: LaudoPericialStatusUpdate,
    service: LaudoPericialService = laudo_pericial_service_dep,
) -> LaudoPericialResponse:
    try:
        return await service.atualizar_status(
            laudo_id=laudo_id, status=data.status, observacoes=data.observacoes
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def remover_laudo_pericial(
    laudo_id: UUID, service: LaudoPericialService = laudo_pericial_service_dep
) -> None:
    try:
