...
class Ocorrencia:
    ...
    @classmethod
    def registrar(cls, *, codigo_ocorrencia: str, unidade_id: UUID, tipo: TipoOcorrencia, prioridade: PrioridadeOcorrencia, data_ocorrencia: datetime, descricao: str, municipio: str, provincia: str, policial_responsavel_id: UUID | None=None, endereco: str | None=None, vitimas: int=0, suspeitos: int=0, preso_em_flagrante: bool=False, observacoes: str | None=None) -> 'Ocorrencia':
        ...
        return cls(id=uuid4(), codigo_ocorrencia=codigo_ocorrencia.strip(), unidade_id=unidade_id, tipo=tipo, status=StatusOcorrencia.REGISTRADA, prioridade=prioridade, data_ocorrencia=data_ocorrencia, descricao=descricao.strip(), municipio=municipio.strip(), provincia=provincia.strip(), data_registro=date.today(), policial_responsavel_id=policial_responsavel_id, endereco=endereco.strip() if endereco else None, vitimas=vitimas, suspeitos=suspeitos, preso_em_flagrante=preso_em_flagrante, observacoes=observacoes.strip() if observacoes else None, ativo=True)
