...
class Evidencia:
    ...
    @classmethod
    def registrar(cls, *, codigo_evidencia: str, vestigio_id: UUID, cadeia_custodia_id: UUID, tipo: TipoEvidencia, descricao: str, fonte: str, confiabilidade: int=3, analisado_por_id: UUID | None=None, observacoes: str | None=None) -> 'Evidencia':
        ...
        return cls(id=uuid4(), codigo_evidencia=codigo_evidencia.strip(), vestigio_id=vestigio_id, cadeia_custodia_id=cadeia_custodia_id, tipo=tipo, descricao=descricao.strip(), fonte=fonte.strip(), confiabilidade=confiabilidade, status=StatusEvidencia.REGISTRADA, data_registro=date.today(), analisado_por_id=analisado_por_id, observacoes=observacoes.strip() if observacoes else None, ativo=True)
