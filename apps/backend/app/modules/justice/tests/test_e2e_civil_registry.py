...
from apps.backend.app.modules.justice._deprecated.bounded_contexts.vital_events.application.services.birth_service import (
    BirthService,
)
from apps.backend.app.modules.justice._deprecated.bounded_contexts.vital_events.infrastructure.repositories.birth_repository import (
    BirthRepository,
)
...
async def test_citizen_registration_outbox(db_session):
    ...
    service = BirthService(
        repo=birth_repo, fuc=_StubFUC(), citizen_repo=citizen_repo, db_session=db_session
    )
    data = {
        "nub": "NUB-TEST-001",
        ...
        "bi_number": "000123456LA042",
        "request_id": "req-test",
    }
    await service.register_birth(data, data["bi_number"], request_id=data["request_id"])
    result = await db_session.execute(
        select(OutboxEvent).order_by(OutboxEvent.created_at.desc()).limit(1)
    )
