...
class CitizenRepository(BaseRepository, CitizenRepositoryPort):
    ...
    async def get_by_id(self, citizen_id: UUID) -> CitizenFUC | None:
        result = await self.session.execute(
            select(CitizenFUC).where(CitizenFUC.citizen_id == citizen_id)
        )
        return result.scalar_one_or_none()

    async def get_by_bi(self, bi_number: str) -> CitizenFUC | None:
        result = await self.session.execute(
            text(
    ...
    async def get_by_national_id_number(self, national_id_number: str) -> CitizenFUC | None:
        """Legacy alias for BI lookup."""
        return await self.get_by_bi(national_id_number)

    async def get_by_nif(self, nif: str) -> CitizenFUC | None:
    ...
    async def search_by_name(self, name: str, limit: int = 50) -> list[CitizenFUC]:
        result = await self.session.execute(
            select(CitizenFUC).where(CitizenFUC.full_name.ilike(f"%{name}%")).limit(limit)
        )
        return list(result.scalars().all())
    ...
    async def get(self, citizen_id: str) -> dict[str, Any] | None:
        ...
        return (
            ...
            else {"citizen_id": str(citizen.citizen_id)}
        )
