...
class CitizenAggregate:
    ...
    def register_birth(self, data: dict, bi_number: str) -> None:
        ...
        self.bi_record = BIRecord(
            citizen_fuc_id=self.id,
            bi_number=bi_number,
            issue_date=date.today(),
            expiry_date=None,
            status="active",
        )
        event = CitizenRegisteredEvent(
            aggregate_id=self.id, payload=data, metadata={"request_id": get_request_id()}
        )
        self.events.append(event)

    @classmethod
    def create_citizen(
        cls, data: dict, bi_number: str, citizen_id: str | None = None
    ) -> CitizenAggregate:
