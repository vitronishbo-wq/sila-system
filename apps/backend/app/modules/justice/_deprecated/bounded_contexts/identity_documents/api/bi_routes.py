...
fuc_client_dep = Depends(get_fuc_client)
identity_service_dep = Depends(get_identity_service)
...
async def get_identity_service(
    ...
    return IdentityRequestService(
        db=db, repository=repository, notification_service=notification_service
    )
...
async def emit_bi(
    ...
    fuc_client: CitizenFUCClient = fuc_client_dep,
    service: IdentityRequestService = identity_service_dep,
):
    """
    ...
    logger.info(
        "BI emission (SILA-API-001)",
        extra={"citizen_fuc_id": citizen_fuc_id, "request_id": str(request.id)},
    )
    return {
        "success": True,
...
async def renew_bi(
    ...
    """
    citizen_fuc_id = payload.get("citizen_fuc_id")
    payload.get("operator_id", str(current_user.id))
    logger.info(
        "BI renewal requested",
        extra={
            "bi_number": bi_number,
            "citizen_fuc_id": citizen_fuc_id,
    ...
    if not projection:
        logger.warning(
            "BI renewal failed: citizen not found in FUC",
            extra={"citizen_fuc_id": citizen_fuc_id, "user_id": str(current_user.id)},
        )
        raise HTTPException(status_code=404, detail="Cidadão não encontrado na base soberana.")
    request = await service.create_request(
        citizen_id=uuid.UUID(citizen_fuc_id) if isinstance(citizen_fuc_id, str) else citizen_fuc_id,
...
async def cancel_bi(
    bi_number: str, payload: dict[str, Any], current_user: User = current_user_dep
):
    """
    ...
    logger.info(
        "BI cancellation requested", extra={"bi_number": bi_number, "user_id": str(current_user.id)}
    )
    if not reason:
        logger.warning(
            "BI cancellation failed: missing reason",
            extra={"bi_number": bi_number, "user_id": str(current_user.id)},
        )
