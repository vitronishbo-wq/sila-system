...
from apps.backend.app.modules.justice.bounded_contexts.application.ports.bi_repository_port import (
    BIRepositoryPort,
)
from apps.backend.app.modules.justice.bounded_contexts.infrastructure.models.bi_record import (
    BIRecord,
)
...
class BIRepository(BIRepositoryPort):
    ...
    async def get_by_id(self, bi_id: uuid.UUID) -> BIRecord | None:
        """Recupera BI por ID."""
        result = await self.session.execute(select(BIRecord).where(BIRecord.id == bi_id))
        return result.scalar_one_or_none()

    async def get_by_number(self, bi_number: str) -> BIRecord | None:
    ...
    async def get_active_by_citizen(self, citizen_fuc_id: str) -> BIRecord | None:
        """Retorna o BI ativo do cidadão (se houver)."""
        result = await self.session.execute(
            select(BIRecord).where(
                and_(BIRecord.citizen_fuc_id == citizen_fuc_id, BIRecord.status == "active")
            )
        )
        return result.scalar_one_or_none()

    async def update_status(
        self, bi_id: uuid.UUID, new_status: str, reason: str | None = None
    ) -> BIRecord | None:
    ...
    async def list_all(self, limit: int = 100, offset: int = 0) -> list[BIRecord]:
        ...
        result = await self.session.execute(
            select(BIRecord).order_by(BIRecord.created_at.desc()).limit(limit).offset(offset)
        )
        return list(result.scalars().all())
