import logging

from fastapi import APIRouter, Depends, HTTPException

from apps.backend.app.api.deps import get_current_user, get_db
from apps.backend.app.modules.justice.bounded_contexts.application.services.marriage_service import (
    MarriageService,
)

logger = logging.getLogger("sila.registo_civil.api")
router = APIRouter(prefix="/marriage", tags=["Registo Civil - Eventos"])

current_user_dep = Depends(get_current_user)
db_dep = Depends(get_db)
marriage_service_dep = Depends(get_marriage_service)


def get_marriage_service(db=db_dep):
    from apps.backend.app.modules.justice.bounded_contexts.infrastructure.repositories.marriage_repository import (
        MarriageRepository,
    )
    from apps.backend.app.modules.justice.bounded_contexts.integrations.fuc_client import FUCClient

    return MarriageService(MarriageRepository(db), FUCClient())


@router.post("/")
async def register_marriage(
    payload: dict,
    service: MarriageService = marriage_service_dep,
    current_user=current_user_dep,
):
    """Registra um assento de casamento."""
    logger.info(
        f"Iniciando registro de casamento entre {payload.get('spouse1_id')} e {payload.get('spouse2_id')}"
    )
    try:
        result = await service.register(payload)
        logger.info(f"Casamento registrado com sucesso: {result.get('marriage_id')}")
        return result
    except Exception as e:
        logger.error(f"Erro ao registrar casamento: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e)) from e
