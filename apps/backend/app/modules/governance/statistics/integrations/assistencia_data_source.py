...
class AssistenciaDataSource(BaseDataSource):
    ...
    async def collect_metrics(self, data_ref: date) -> dict[str, float | int]:
        ...
        beneficiarios_ativos = int(
            await self._scalar(
                ...
                .where(
                    ...
                    self._status_in_ci(BeneficiarioModel.situacao, ("ativo",)),
                ),
                0,
            )
        )
        beneficios_ativos = int(
            await self._scalar(
                select(func.count())
                .select_from(BeneficioModel)
                .where(self._status_in_ci(BeneficioModel.status, ("ativo", "aprovado"))),
                0,
            )
        )
        cadastros_ativos = int(
            await self._scalar(
                select(func.count())
                .select_from(CadastroUnicoModel)
                .where(self._status_in_ci(CadastroUnicoModel.status, ("ativo",))),
                0,
            )
        )
        familias_baixa_renda = int(
            await self._scalar(
                select(func.count())
                .select_from(CadastroUnicoModel)
                .where(CadastroUnicoModel.renda_per_capita <= 250),
                0,
            )
        )
        renda_media = self._as_float(
            await self._scalar(select(func.avg(CadastroUnicoModel.renda_per_capita)), 0)
        )
        return {
            ...
            "renda_media_per_capita": round(renda_media, 2),
        }
