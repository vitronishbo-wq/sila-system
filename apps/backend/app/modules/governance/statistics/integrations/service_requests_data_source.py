...
class ServiceRequestsDataSource(BaseDataSource):
    ...
    async def collect_metrics(self, data_ref: date) -> dict[str, float | int]:
        ...
        requests_total_mes = int(
            await self._scalar(
                select(func.count())
                .select_from(ServiceRequestModel)
                .where(
                    ServiceRequestModel.created_at >= start_dt,
                    ServiceRequestModel.created_at < next_month_dt,
                ),
                0,
            )
        )
        requests_concluidas_mes = int(
            await self._scalar(
                select(func.count())
                .select_from(ServiceRequestModel)
                .where(
                    ...
                    ServiceRequestModel.created_at < next_month_dt,
                    self._status_in_ci(
                        ServiceRequestModel.status, ("completed", "concluida", "fechado")
                    ),
                ),
                0,
            )
        )
        requests_pendentes = int(
            await self._scalar(
                select(func.count())
                .select_from(ServiceRequestModel)
                .where(
        ...
        requests_sla_violado = int(
            await self._scalar(
                select(func.count())
                .select_from(ServiceRequestModel)
                .where(ServiceRequestModel.sla_breached.is_(True)),
                0,
            )
        )
        taxa_conclusao = (
            round(requests_concluidas_mes / requests_total_mes * 100, 2)
            if requests_total_mes > 0
            else 0.0
        )
        return {
            "requests_total_mes": requests_total_mes,
            "requests_concluidas_mes": requests_concluidas_mes,
            "requests_pendentes": requests_pendentes,
            "requests_sla_violado": requests_sla_violado,
            "taxa_conclusao_requests": taxa_conclusao,
        }
