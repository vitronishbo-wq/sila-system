...
from apps.backend.app.core.bridges.identity_bridge import BIEventRecord, BIRecord, CitizenFUC
...
SOURCE_REQUIREMENTS = {
    "educacao": {"educacao_turmas", "educacao_matriculas"},
    "juventude": {"juventude_jovens", "juventude_programas"},
    "emprego": {"emprego_candidatos", "emprego_ofertas", "emprego_contratos"},
    "saude": {"appointments", "vaccine_doses", "health_internamentos"},
    "assistencia": {
        "assistencia_social_beneficiarios",
        "assistencia_social_beneficios",
        "assistencia_social_cadastros_unicos",
    },
    "identidade": {"citizen_fuc", "bi_records", "bi_events"},
    "workflow": {"wf_definitions", "wf_states", "wf_transitions", "wf_instances", "wf_tasks"},
...
async def _seed_for_available_sources(db_session, available: set[str]) -> set[str]:
    ...
    if "citizen_fuc" in available:
        db_session.add(
            CitizenFUC(
                ...
                full_name="Cidadao Statistics Integracao",
                is_active=True,
                vital_status="alive",
    ...
    if SOURCE_REQUIREMENTS["identidade"].issubset(available):
        db_session.add(
            BIRecord(
                ...
                bi_number=f"{uuid4().hex[:12].upper()}",
        ...
        db_session.add(
            BIEventRecord(
                aggregate_id=str(citizen_id),
                event_type="BI_ISSUED",
                operator_id="stats-test",
                payload={"ok": True},
                event_metadata={},
            )
        )
    ...
    if SOURCE_REQUIREMENTS["juventude"].issubset(available):
        db_session.add(
            JovemModel(
                ...
                nome="Jovem Statistics",
                data_nascimento=date(today.year - 20, 1, 1),
                faixa_etaria="18_24",
                genero="M",
                naturalidade="Huambo",
                nacionalidade="Angolana",
                escolaridade="medio_completo",
                situacao_ocupacional="procura_emprego",
                endereco="Rua A",
                municipio="Huambo",
                provincia="Huambo",
                citizen_id=citizen_id,
                data_cadastro=today,
                ativo=True,
                acompanhamento_psicossocial=True,
            )
        )
        db_session.add(
            ProgramaJuvenilModel(
                ...
                nome="Programa Integracao Statistics",
                tipo="capacitacao",
    ...
    if SOURCE_REQUIREMENTS["emprego"].issubset(available):
        ...
        db_session.add(
            OfertaModel(
                id=uuid4(),
                numero_processo=f"OF-{uuid4().hex[:8]}",
                citizen_id=citizen_id,
                data_registro=today,
                service_type="vaga_emprego",
                status="aberta",
                observacoes=None,
                metadata_json={},
            )
        ...
        db_session.add(
            ContratoModel(
                id=uuid4(),
                numero_processo=f"CON-{uuid4().hex[:8]}",
                citizen_id=citizen_id,
                data_registro=today,
                service_type="contratacao",
                status="ativo",
                observacoes=None,
                metadata_json={},
            )
    ...
    if SOURCE_REQUIREMENTS["saude"].issubset(available):
        ...
        db_session.add(
            AppointmentModel(
                id=uuid4(),
                appointment_number=f"APT-{uuid4().hex[:8]}",
                citizen_id=citizen_id,
                created_by=uuid4(),
                doctor_id=None,
        ...
        db_session.add(
            VaccineDoseModel(
                ...
                applied_by=uuid4(),
                dose_number=1,
                batch_number=f"LOT-{uuid4().hex[:8]}",
    ...
    if SOURCE_REQUIREMENTS["assistencia"].issubset(available):
        ...
        db_session.add(
            CadastroUnicoModel(
                id=cadastro_id,
                codigo=f"CAD-{uuid4().hex[:8]}",
                citizen_id_responsavel=citizen_id,
                renda_per_capita=Decimal("180.00"),
                composicao_familiar=[],
        ...
        db_session.add(
            BeneficiarioModel(
                ...
                citizen_id=citizen_id,
                cadastro_unico_id=cadastro_id,
                faixa_vulnerabilidade="alta",
                situacao="ativo",
                data_cadastro=today,
                observacoes=None,
                ativo=True,
        ...
        db_session.add(
            BeneficioModel(
                id=uuid4(),
                codigo=f"BFC-{uuid4().hex[:8]}",
                beneficiario_id=beneficiario_id,
                programa_social_id=None,
                tipo="bpc_pcd",
                valor=Decimal("706.00"),
                status="aprovado",
                data_solicitacao=today,
                data_concessao=today,
                data_fim=None,
                motivo_status=None,
            )
        )
    ...
    if SOURCE_REQUIREMENTS["workflow"].issubset(available):
        ...
        db_session.add(
            WorkflowDefinitionModel(
                ...
                description="Integracao statistics",
                version=1,
    ...
    if SOURCE_REQUIREMENTS["service_requests"].issubset(available):
        db_session.add(
            ServiceRequestModel(
                ...
                service_type="education_enrollment",
                title="Pedido teste statistics",
                description="Integracao real",
                status="completed",
                priority="MEDIUM",
                channel="WEB",
                workflow_instance_id=None,
...
async def test_statistics_service_collect_with_real_repo_partial_schema(db_session) -> None:
    required_tables = set().union(
        *SOURCE_REQUIREMENTS.values(), {"statistics", "statistics_timeseries"}
    )
    available = await _available_tables(db_session, required_tables)
    seeded_sources = await _seed_for_available_sources(db_session, available)
    if not seeded_sources:
    ...
    successful_sources = {
        name for name, payload in result.items() if payload["status"] == "success"
    }
    ...
    if has_statistics_tables:
        persisted_success = [
            payload.get("persisted", False)
            for payload in result.values()
            if payload["status"] == "success"
        ]
        assert any(persisted_success)
        stats_total = int(
            (await db_session.execute(select(func.count()).select_from(StatisticModel))).scalar()
            or 0
        )
        ts_total = int(
            (await db_session.execute(select(func.count()).select_from(TimeSeriesModel))).scalar()
            or 0
        )
...
async def test_di_wiring_builds_statistics_service_with_all_sources(db_session) -> None:
    ...
    assert set(service.data_sources.keys()) == {
        "educacao",
        "juventude",
        "emprego",
        "saude",
