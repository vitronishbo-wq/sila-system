...
class KPIService:
    ...
    def __init__(self, db: Optional[AsyncSession]=None, stats_service: Optional[StatisticsService]=None, **_: Any):
        self.db = db
    ...
    async def get_dashboard_kpis(self) -> Dict[str, Any]:
        if not self.stats_service:
            return {'generated_at': datetime.utcnow().isoformat(), 'message': 'StatisticsService nao configurado'}
    ...
    async def get_growth_metrics(self, days: int=30) -> Dict[str, Any]:
        ...
        return {'periodo_dias': days, 'volume_atual': round(current_volume, 2), 'volume_periodo_anterior': round(previous_volume, 2), 'growth_percent': round(growth, 2), 'generated_at': datetime.utcnow().isoformat()}
