...
from apps.backend.app.modules.governance.statistics.application.ports.dashboard_repository_port import (
...
class SQLAlchemyDashboardRepository(DashboardRepositoryPort):
    ...
    async def create(self, dashboard: Dashboard) -> Dashboard:
        model = DashboardModel(
            ...
            tipo=dashboard.tipo.value,
            configuracoes=dashboard.configuracoes,
            kpi_ids=dashboard.kpi_ids,
            criado_por=dashboard.criado_por,
        )
    ...
    async def list_all(self, limit: int = 100, offset: int = 0) -> list[Dashboard]:
        stmt = (
            select(DashboardModel).order_by(DashboardModel.nome.asc()).offset(offset).limit(limit)
        )
    ...
    def _to_domain(model: DashboardModel) -> Dashboard:
        return Dashboard(
            ...
            tipo=TipoDashboard(model.tipo),
            configuracoes=model.configuracoes,
            kpi_ids=model.kpi_ids or [],
