...
from apps.backend.app.modules.governance.statistics.application.ports.timeseries_repository_port import (
...
class SQLAlchemyTimeSeriesRepository(TimeSeriesRepositoryPort):
    ...
    async def create(self, point: TimeSeries) -> TimeSeries:
        model = TimeSeriesModel(
            metrica_id=point.metrica_id,
            timestamp=point.timestamp,
            valor=point.valor,
            dimensao_1=point.dimensao_1,
            dimensao_2=point.dimensao_2,
            dimensao_3=point.dimensao_3,
            origem=point.origem,
        )
    ...
    async def latest_by_metrica(self, metrica_id: int) -> TimeSeries | None:
        stmt = (
            ...
            .where(TimeSeriesModel.metrica_id == metrica_id)
            .order_by(TimeSeriesModel.timestamp.desc())
            .limit(1)
        )
    ...
    def _to_domain(model: TimeSeriesModel) -> TimeSeries:
        return TimeSeries(
            id=model.id,
            metrica_id=model.metrica_id,
            timestamp=model.timestamp,
            valor=model.valor,
            dimensao_1=model.dimensao_1,
            dimensao_2=model.dimensao_2,
            dimensao_3=model.dimensao_3,
            origem=model.origem,
