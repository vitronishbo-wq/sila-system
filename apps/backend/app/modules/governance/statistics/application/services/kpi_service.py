...
class KPIService:
    ...
    def __init__(self, kpi_repository: KPIRepositoryPort, metrica_repository: MetricaRepositoryPort, event_bus: EventBus | None=None) -> None:
    ...
    async def criar_kpi(self, data: dict) -> KPI:
        ...
        if self.event_bus:
            self.event_bus.publish('kpi.criado', {'id': saved.id, 'nome': saved.nome, 'metrica_id': saved.metrica_id})
        return saved
    ...
    async def atualizar_kpi(self, kpi_id: int, data: dict) -> KPI:
        ...
        if self.event_bus:
            self.event_bus.publish('kpi.atualizado', {'id': updated.id, 'valor_atual': updated.valor_atual, 'performance': updated.calcular_performance()})
        return updated
    ...
    async def atualizar_valor_kpi(self, kpi_id: int, valor: float) -> KPI:
        ...
        updated = await self.kpi_repository.update(kpi)
        if self.event_bus:
            self.event_bus.publish('kpi.valor_atualizado', {'id': updated.id, 'valor_anterior': valor_anterior, 'valor_atual': valor, 'performance': updated.calcular_performance()})
        return updated

    async def listar_kpis(self, status: StatusKPI | None=None, limit: int=100, offset: int=0) -> list[KPI]:
        if status:
    ...
    async def calcular_performance_kpi(self, kpi_id: int) -> dict:
        ...
        return {'kpi_id': kpi.id, 'nome': kpi.nome, 'valor_atual': kpi.valor_atual, 'valor_alvo': kpi.valor_alvo, 'performance': kpi.calcular_performance(), 'status_cor': kpi.status_cor, 'dentro_limites': kpi.esta_within_limits()}
