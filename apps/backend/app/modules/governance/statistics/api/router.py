...
from .schemas.statistics_schema import StatisticCreateSchema, StatisticDetailSchema, StatisticSchema
...
@router.post(
    ...
    dependencies=[Depends(require_permission("statistics:admin"))],
)
async def create_statistic(
    payload: StatisticCreateSchema, service: StatisticsService = Depends(get_statistics_service)
):
...
@router.get(
    "/code/{code}",
    response_model=StatisticDetailSchema,
    dependencies=[Depends(require_permission("statistics:view"))],
)
async def get_statistic_by_code(
    code: str, service: StatisticsService = Depends(get_statistics_service)
):
...
async def get_statistic(
    statistic_id: int, service: StatisticsService = Depends(get_statistics_service)
):
...
@router.get(
    "/{statistic_id}/series",
    response_model=list[TimeSeriesSchema],
    dependencies=[Depends(require_permission("statistics:view"))],
)
...
@router.get(
    "/{statistic_id}/latest",
    response_model=TimeSeriesSchema | None,
    dependencies=[Depends(require_permission("statistics:view"))],
)
...
@router.delete(
    "/{statistic_id}/series/{timeseries_id}",
    dependencies=[Depends(require_permission("statistics:admin"))],
)
...
@router.get(
    "/{statistic_id}/analytics/average",
    dependencies=[Depends(require_permission("statistics:view"))],
)
...
@router.get(
    "/{statistic_id}/analytics/sum", dependencies=[Depends(require_permission("statistics:view"))]
)
async def get_sum(
    statistic_id: int,
    period_start: datetime = Query(...),
