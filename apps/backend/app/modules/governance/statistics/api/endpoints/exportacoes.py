...
async def criar_exportacao(data: ExportacaoCreate, service: ExportacaoService=Depends(get_exportacao_service)) -> ExportacaoResponse:
...
async def listar_exportacoes(pagina: int=Query(1, ge=1), tamanho_pagina: int=Query(20, ge=1, le=100), service: ExportacaoService=Depends(get_exportacao_service)) -> ExportacaoListResponse:
    offset = (pagina - 1) * tamanho_pagina
    itens = await service.listar(limit=tamanho_pagina, offset=offset)
    return ExportacaoListResponse(itens=[ExportacaoResponse.model_validate(i) for i in itens], total=len(itens), pagina=pagina, tamanho_pagina=tamanho_pagina)

@router.get('/{exportacao_id}', response_model=ExportacaoResponse)
async def obter_exportacao(exportacao_id: int, service: ExportacaoService=Depends(get_exportacao_service)) -> ExportacaoResponse:
    try:
...
async def atualizar_exportacao(exportacao_id: int, data: ExportacaoUpdate, service: ExportacaoService=Depends(get_exportacao_service)) -> ExportacaoResponse:
    try:
        return ExportacaoResponse.model_validate(await service.atualizar(exportacao_id, data.model_dump(exclude_unset=True)))
    except EstatisticaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))

@router.delete('/{exportacao_id}', status_code=status.HTTP_204_NO_CONTENT)
async def deletar_exportacao(exportacao_id: int, service: ExportacaoService=Depends(get_exportacao_service)) -> None:
    try:
