...
from apps.backend.app.modules.governance.statistics.api.schemas.alerta_schema import (
    AlertaCreate,
    AlertaListResponse,
    AlertaResponse,
    AlertaUpdate,
)
from apps.backend.app.modules.governance.statistics.application.services.alerta_service import (
    AlertaService,
)
...
alerta_service_dep = Depends(get_alerta_service)
...
async def criar_alerta(
    data: AlertaCreate, service: AlertaService = alerta_service_dep
) -> AlertaResponse:
...
async def listar_alertas(
    ...
    service: AlertaService = alerta_service_dep,
) -> AlertaListResponse:
    offset = (pagina - 1) * tamanho_pagina
    itens = await service.listar(limit=tamanho_pagina, offset=offset)
    return AlertaListResponse(
        itens=[AlertaResponse.model_validate(i) for i in itens],
        total=len(itens),
        pagina=pagina,
...
async def obter_alerta(
    alerta_id: int, service: AlertaService = alerta_service_dep
) -> AlertaResponse:
    try:
...
async def atualizar_alerta(
    alerta_id: int, data: AlertaUpdate, service: AlertaService = alerta_service_dep
) -> AlertaResponse:
    try:
        return AlertaResponse.model_validate(
            await service.atualizar(alerta_id, data.model_dump(exclude_unset=True))
        )
    except EstatisticaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
