...
async def criar_relatorio(data: RelatorioCreate, service: RelatorioService=Depends(get_relatorio_service)) -> RelatorioResponse:
...
async def listar_relatorios(pagina: int=Query(1, ge=1), tamanho_pagina: int=Query(20, ge=1, le=100), service: RelatorioService=Depends(get_relatorio_service)) -> RelatorioListResponse:
    offset = (pagina - 1) * tamanho_pagina
    itens = await service.listar(limit=tamanho_pagina, offset=offset)
    return RelatorioListResponse(itens=[RelatorioResponse.model_validate(i) for i in itens], total=len(itens), pagina=pagina, tamanho_pagina=tamanho_pagina)

@router.get('/{relatorio_id}', response_model=RelatorioResponse)
async def obter_relatorio(relatorio_id: int, service: RelatorioService=Depends(get_relatorio_service)) -> RelatorioResponse:
    try:
...
async def atualizar_relatorio(relatorio_id: int, data: RelatorioUpdate, service: RelatorioService=Depends(get_relatorio_service)) -> RelatorioResponse:
    try:
        return RelatorioResponse.model_validate(await service.atualizar(relatorio_id, data.model_dump(exclude_unset=True)))
    except EstatisticaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))

@router.delete('/{relatorio_id}', status_code=status.HTTP_204_NO_CONTENT)
async def deletar_relatorio(relatorio_id: int, service: RelatorioService=Depends(get_relatorio_service)) -> None:
    try:
