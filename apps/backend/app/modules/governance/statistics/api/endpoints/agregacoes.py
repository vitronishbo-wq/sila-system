...
from apps.backend.app.modules.governance.statistics.api.schemas.agregacao_schema import (
    AgregacaoCreate,
    AgregacaoListResponse,
    AgregacaoResponse,
    AgregacaoUpdate,
)
from apps.backend.app.modules.governance.statistics.application.services.agregacao_service import (
    AgregacaoService,
)
...
agregacao_service_dep = Depends(get_agregacao_service)
...
async def criar_agregacao(
    data: AgregacaoCreate, service: AgregacaoService = agregacao_service_dep
) -> AgregacaoResponse:
...
async def listar_agregacoes(
    pagina: int = pagina_query,
    tamanho_pagina: int = tamanho_pagina_query,
    service: AgregacaoService = agregacao_service_dep,
) -> AgregacaoListResponse:
    offset = (pagina - 1) * tamanho_pagina
    itens = await service.listar(limit=tamanho_pagina, offset=offset)
    return AgregacaoListResponse(
        itens=[AgregacaoResponse.model_validate(i) for i in itens],
        total=len(itens),
        pagina=pagina,
...
async def obter_agregacao(
    agregacao_id: int, service: AgregacaoService = agregacao_service_dep
) -> AgregacaoResponse:
    try:
...
async def atualizar_agregacao(
    agregacao_id: int,
    data: AgregacaoUpdate,
    service: AgregacaoService = agregacao_service_dep,
) -> AgregacaoResponse:
    try:
        return AgregacaoResponse.model_validate(
            await service.atualizar(agregacao_id, data.model_dump(exclude_unset=True))
        )
    except EstatisticaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
