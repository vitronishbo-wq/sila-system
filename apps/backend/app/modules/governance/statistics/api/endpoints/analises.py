...
from apps.backend.app.modules.governance.statistics.api.schemas.analise_schema import (
    AnaliseCreate,
    AnaliseListResponse,
    AnaliseResponse,
    AnaliseUpdate,
)
from apps.backend.app.modules.governance.statistics.application.services.analise_service import (
    AnaliseService,
)
...
analise_service_dep = Depends(get_analise_service)
...
async def criar_analise(
    data: AnaliseCreate, service: AnaliseService = analise_service_dep
) -> AnaliseResponse:
...
async def listar_analises(
    ...
    service: AnaliseService = analise_service_dep,
) -> AnaliseListResponse:
    offset = (pagina - 1) * tamanho_pagina
    itens = await service.listar(limit=tamanho_pagina, offset=offset)
    return AnaliseListResponse(
        itens=[AnaliseResponse.model_validate(i) for i in itens],
        total=len(itens),
        pagina=pagina,
...
async def obter_analise(
    analise_id: int, service: AnaliseService = analise_service_dep
) -> AnaliseResponse:
    try:
...
async def atualizar_analise(
    analise_id: int, data: AnaliseUpdate, service: AnaliseService = analise_service_dep
) -> AnaliseResponse:
    try:
        return AnaliseResponse.model_validate(
            await service.atualizar(analise_id, data.model_dump(exclude_unset=True))
        )
    except EstatisticaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
