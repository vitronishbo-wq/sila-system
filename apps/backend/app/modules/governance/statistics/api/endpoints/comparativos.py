...
async def criar_comparativo(data: ComparativoCreate, service: ComparativoService=Depends(get_comparativo_service)) -> ComparativoResponse:
...
async def listar_comparativos(pagina: int=Query(1, ge=1), tamanho_pagina: int=Query(20, ge=1, le=100), service: ComparativoService=Depends(get_comparativo_service)) -> ComparativoListResponse:
    offset = (pagina - 1) * tamanho_pagina
    itens = await service.listar(limit=tamanho_pagina, offset=offset)
    return ComparativoListResponse(itens=[ComparativoResponse.model_validate(i) for i in itens], total=len(itens), pagina=pagina, tamanho_pagina=tamanho_pagina)

@router.get('/{comparativo_id}', response_model=ComparativoResponse)
async def obter_comparativo(comparativo_id: int, service: ComparativoService=Depends(get_comparativo_service)) -> ComparativoResponse:
    try:
...
async def atualizar_comparativo(comparativo_id: int, data: ComparativoUpdate, service: ComparativoService=Depends(get_comparativo_service)) -> ComparativoResponse:
    try:
        return ComparativoResponse.model_validate(await service.atualizar(comparativo_id, data.model_dump(exclude_unset=True)))
    except EstatisticaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))

@router.delete('/{comparativo_id}', status_code=status.HTTP_204_NO_CONTENT)
async def deletar_comparativo(comparativo_id: int, service: ComparativoService=Depends(get_comparativo_service)) -> None:
    try:
