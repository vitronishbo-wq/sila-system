...
async def criar_tendencia(data: TendenciaCreate, service: TendenciaService=Depends(get_tendencia_service)) -> TendenciaResponse:
...
async def listar_tendencias(pagina: int=Query(1, ge=1), tamanho_pagina: int=Query(20, ge=1, le=100), service: TendenciaService=Depends(get_tendencia_service)) -> TendenciaListResponse:
    offset = (pagina - 1) * tamanho_pagina
    itens = await service.listar(limit=tamanho_pagina, offset=offset)
    return TendenciaListResponse(itens=[TendenciaResponse.model_validate(i) for i in itens], total=len(itens), pagina=pagina, tamanho_pagina=tamanho_pagina)

@router.get('/{tendencia_id}', response_model=TendenciaResponse)
async def obter_tendencia(tendencia_id: int, service: TendenciaService=Depends(get_tendencia_service)) -> TendenciaResponse:
    try:
...
async def atualizar_tendencia(tendencia_id: int, data: TendenciaUpdate, service: TendenciaService=Depends(get_tendencia_service)) -> TendenciaResponse:
    try:
        return TendenciaResponse.model_validate(await service.atualizar(tendencia_id, data.model_dump(exclude_unset=True)))
    except EstatisticaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))

@router.delete('/{tendencia_id}', status_code=status.HTTP_204_NO_CONTENT)
async def deletar_tendencia(tendencia_id: int, service: TendenciaService=Depends(get_tendencia_service)) -> None:
    try:
