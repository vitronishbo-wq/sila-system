...
from apps.backend.app.modules.governance.statistics.api.schemas.metrica_schema import (
    MetricaCreate,
    MetricaListaResponse,
    MetricaResponse,
    MetricaUpdate,
    MetricaValorUpdate,
)
from apps.backend.app.modules.governance.statistics.application.services.metrica_service import (
    MetricaService,
)
...
async def listar_metricas(
    tipo: TipoMetrica | None = tipo_query,
    fonte: FonteDados | None = fonte_query,
    pagina: int = pagina_query,
    tamanho_pagina: int = tamanho_pagina_query,
    service: MetricaService = metrica_service_dep,
) -> MetricaListaResponse:
    offset = (pagina - 1) * tamanho_pagina
    metricas = await service.listar_metricas(
        tipo=tipo, fonte=fonte, limit=tamanho_pagina, offset=offset
    )
    return MetricaListaResponse(
        metricas=[MetricaResponse.model_validate(m) for m in metricas],
        total=len(metricas),
        pagina=pagina,
...
async def obter_metrica(
    metrica_id: int, service: MetricaService = metrica_service_dep
) -> MetricaResponse:
    try:
...
async def atualizar_metrica(
    metrica_id: int, data: MetricaUpdate, service: MetricaService = metrica_service_dep
) -> MetricaResponse:
    try:
        return MetricaResponse.model_validate(
            await service.atualizar_metrica(metrica_id, data.model_dump(exclude_unset=True))
        )
    except EstatisticaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def atualizar_valor_metrica(
    metrica_id: int,
    data: MetricaValorUpdate,
    service: MetricaService = metrica_service_dep,
) -> MetricaResponse:
    try:
