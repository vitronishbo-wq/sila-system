...
class RequestService(BaseRequestService[ServiceRequest, RequestRepositoryPort], RequestServicePort):
    ...
    def __init__(
        ...
        educacao_client: DomainClientPort | None = None,
        juventude_client: DomainClientPort | None = None,
        emprego_client: DomainClientPort | None = None,
        saude_client: DomainClientPort | None = None,
        assistencia_client: DomainClientPort | None = None,
        identidade_client: DomainClientPort | None = None,
    ):
        super().__init__(
            db=db,
    ...
    async def _validate_create_request(
        ...
        service_value = (
            service_type.value if isinstance(service_type, ServiceType) else str(service_type)
        )
        if self.identidade_client is not None:
            ok, reason = await self.identidade_client.validate_payload(
                service_type=service_value, citizen_id=citizen_id, payload=payload
            )
            if not ok:
        ...
        if domain_client is None:
            return
        ok, reason = await domain_client.validate_payload(
            service_type=service_type.value, citizen_id=citizen_id, payload=payload
        )
        if not ok:
    ...
    async def change_status(
        ...
        return await self.update_status(
            request_id=request_id, new_status=new_status, updated_by=changed_by, reason=reason
        )
    ...
    def _validate_status_transition(
        self, current: ServiceRequestStatus, new: ServiceRequestStatus
    ) -> bool:
        """
        ...
        transitions = {
            ...
            ServiceRequestStatus.UNDER_REVIEW: [
                ...
                ServiceRequestStatus.REJECTED,
                ServiceRequestStatus.WAITING_INFO,
            ],
            ServiceRequestStatus.IN_PROGRESS: [
    ...
    async def _start_workflow(self, request: ServiceRequest):
        ...
        try:
            try:
                ...
                wf_instance = await self.workflow_engine.start_workflow(
                    definition_name=request.service_type.value,
                    context={
                        "citizen_id": str(request.citizen_id),
                        "request_id": str(request.id),
                        "request_number": request.request_number,
    ...
    async def _dispatch_submission(self, request: ServiceRequest) -> dict[str, Any] | None:
        ...
        if domain_client is None:
            return None
        return await domain_client.submit(
