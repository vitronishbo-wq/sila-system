...
class RequestLifecycleService:
    ...
    async def create_request(
        ...
        event = RequestEvent(
            request_id=saved.id,
            event_type="CREATED",
            payload={"service_type": service_type, "channel": channel, "priority": priority},
            actor_id=created_by,
        )
    ...
    async def transition_request(
        self, request_id: UUID, transition: str, actor_id: UUID, data: dict[str, Any] | None = None
    ) -> bool:
        ...
        success = await self.workflow_client.send_transition(
            request.workflow_instance_id, transition, actor_id, data
        )
        if success:
            event = RequestEvent(
