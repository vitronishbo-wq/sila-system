...
from ..infrastructure.models import ServiceRequestModel, RequestEventModel
...
async def create_service_request(
    request_data: ServiceRequestCreate, service: RequestService = Depends(get_request_service)
):
    """
    ...
    try:
        ...
        raise InvalidRequestError(reason=str(e)).to_http_exception() from e
...
async def get_service_request(
    request_id: UUID, service: RequestService = Depends(get_request_service)
):
    """
...
async def submit_service_request(
    ...
    """
    from apps.backend.app.core.exceptions import InvalidRequestError, UnauthorizedError
    ...
    try:
        ...
        raise UnauthorizedError(str(e)).to_http_exception() from e
    except Exception as e:
...
async def assign_service_request(
    ...
    try:
        ...
        request = await service.assign_request(
            request_id, assign_data.assigned_to_user_id, assigned_by
        )
        return request
    except Exception as e:
        raise InvalidRequestError(reason=str(e)).to_http_exception()
...
async def update_service_request_status(
    ...
    try:
        ...
        request = await service.change_status(
            request_id, status_data.status, changed_by, status_data.reason
        )
        return request
...
async def search_service_requests(
    search: ServiceRequestSearch, service: RequestService = Depends(get_request_service)
):
    """
    Pesquisa avançada de pedidos
    """
    filters = search.model_dump(
        exclude={"query", "skip", "limit", "start_date", "end_date"}, exclude_none=True
    )
    requests, total = await service.search_requests(
        search.query or "", filters, search.skip, search.limit
    )
...
async def get_requests_by_entity_id(entity_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    stmt = (
        ...
        .where(cast(ServiceRequestModel.metadata_, String).like(f'%"entity_id": "{entity_id}"%'))
