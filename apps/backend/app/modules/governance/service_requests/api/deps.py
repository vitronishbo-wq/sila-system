...
from apps.backend.app.domain.db import get_session
from apps.backend.app.api.deps import get_notification_service
...
async def get_request_service(db: AsyncSession=Depends(get_session), notification_service: NotificationService=Depends(get_notification_service)) -> RequestService:
    ...
    juventude_client = JuventudeClient(jovem_repo=make_juventude_jovem_repository(db), programa_repo=make_juventude_programa_repository(db))
    ...
    assistencia_client = AssistenciaClient(beneficiario_repo=make_assistencia_beneficiario_repository(db))
    return RequestService(db=db, repository=repository, notification_service=notification_service, identidade_client=identidade_client, educacao_client=educacao_client, juventude_client=juventude_client, emprego_client=emprego_client, saude_client=saude_client, assistencia_client=assistencia_client)

async def get_request_or_404(request_id: UUID, service: RequestService=Depends(get_request_service), identity=Depends(lambda: None)):
...
def require_request_permission(permission: str):
    ...
    async def decorator(request=Depends(get_request_or_404), identity=Depends(lambda: None)):
        from apps.backend.app.domain.exceptions import AccessDeniedError
        if permission == 'view':
