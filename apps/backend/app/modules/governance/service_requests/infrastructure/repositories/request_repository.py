...
class RequestRepository(RequestRepositoryPort):
    ...
    async def get_by_number(self, request_number: str) -> ServiceRequest | None:
        ...
        stmt = select(ServiceRequestModel).where(
            ServiceRequestModel.request_number == request_number
        )
        result = await self.db.execute(stmt)
        model = result.scalar_one_or_none()
        return self._to_domain(model) if model else None
    ...
    async def get_by_assignee(
        ...
        stmt = (
            select(ServiceRequestModel)
            .where(where_clause)
            .order_by(desc(ServiceRequestModel.priority), desc(ServiceRequestModel.created_at))
            .offset(skip)
            .limit(limit)
        )
        result = await self.db.execute(stmt)
    ...
    async def search(
        ...
        if query:
            where_clauses.append(
                or_(
                    ServiceRequestModel.request_number.ilike(f"%{query}%"),
                    ServiceRequestModel.title.ilike(f"%{query}%"),
                    ServiceRequestModel.description.ilike(f"%{query}%"),
                )
            )
        if filters:
            ...
            if filters.get("assigned_to"):
                where_clauses.append(
                    ServiceRequestModel.assigned_to_user_id == filters["assigned_to"]
                )
        combined_where = (
            where_clauses[0]
            if len(where_clauses) == 1
            else and_(*where_clauses)
            if where_clauses
            else True
        )
    ...
    async def get_next_sequence(self, year: int) -> int:
        ...
        stmt = (
            ...
            .where(ServiceRequestModel.request_number.like(pattern))
        )
        result = await self.db.execute(stmt)
        count = result.scalar() or 0
        return count + 1

    def _to_domain(self, model: ServiceRequestModel) -> ServiceRequest | None:
        ...
        legacy_status_map = {
            "RASCUNHO": ServiceRequestStatus.DRAFT.value,
            "RECEBIDO": ServiceRequestStatus.SUBMITTED.value,
            "SUBMETIDO": ServiceRequestStatus.SUBMITTED.value,
            "EM_ANALISE": ServiceRequestStatus.UNDER_REVIEW.value,
            "EM_ANDAMENTO": ServiceRequestStatus.IN_PROGRESS.value,
            "AGUARDANDO_INFO": ServiceRequestStatus.WAITING_INFO.value,
            "APROVADO": ServiceRequestStatus.APPROVED.value,
            "REJEITADO": ServiceRequestStatus.REJECTED.value,
            "CONCLUIDO": ServiceRequestStatus.COMPLETED.value,
            "CANCELADO": ServiceRequestStatus.CANCELLED.value,
            "EXPIRADO": ServiceRequestStatus.EXPIRED.value,
        }
        ...
        legacy_service_type_map = {
            "DOCUMENT_REQUEST": ServiceType.IDENTITY_BI.value,
            "DOCUMENT": ServiceType.IDENTITY_BI.value,
            "GENERAL": ServiceType.GENERAL_SUPPORT.value,
        }
        ...
        request = ServiceRequest(
            ...
            workflow_data=model.workflow_data or {},
