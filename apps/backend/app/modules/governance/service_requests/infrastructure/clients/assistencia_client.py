...
from apps.backend.app.modules.governance.service_requests.application.ports import AssistenciaClientPort
...
class AssistenciaClient(AssistenciaClientPort):
    ...
    async def submit(self, *, service_type: str, request_id: UUID, citizen_id: UUID, payload: dict[str, Any]) -> dict[str, Any]:
        beneficiario = await self._beneficiario_repo.get_by_citizen(citizen_id)
        return {'module': 'assistencia_social', 'service_type': service_type, 'request_id': str(request_id), 'citizen_id': str(citizen_id), 'beneficiario_id': str(beneficiario.id) if beneficiario else None, 'cadastro_unico_id': str(payload.get('cadastro_unico_id')) if payload.get('cadastro_unico_id') else None, 'accepted': True}