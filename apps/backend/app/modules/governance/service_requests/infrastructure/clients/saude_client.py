...
class SaudeClient(SaudeClientPort):
    ...
    async def validate_payload(
        ...
        if service_type not in {
            ServiceType.HEALTH_APPOINTMENT.value,
            ServiceType.HEALTH_EXAM.value,
            ServiceType.HEALTH_PRESCRIPTION.value,
        }:
            return (True, None)
    ...
    async def submit(
        ...
        return {
            ...
            "health_unit_id": str(payload.get("health_unit_id"))
            if payload.get("health_unit_id")
            else None,
            "accepted": True,
        }
