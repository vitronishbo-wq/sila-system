...
class JuventudeClient(JuventudeClientPort):
    ...
    async def submit(self, *, service_type: str, request_id: UUID, citizen_id: UUID, payload: dict[str, Any]) -> dict[str, Any]:
        return {'module': 'juventude', 'service_type': service_type, 'request_id': str(request_id), 'citizen_id': str(citizen_id), 'programa_id': str(payload.get('programa_id')) if payload.get('programa_id') else None, 'accepted': True}