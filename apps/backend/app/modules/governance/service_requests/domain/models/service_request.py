...
from ..enums import ServiceRequestStatus, ServiceType, RequestChannel, RequestPriority
...
class ServiceRequest:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.service_type, str):
            legacy_service_map = {'DOCUMENT_REQUEST': ServiceType.IDENTITY_BI, 'GENERAL': ServiceType.GENERAL_SUPPORT, 'DOCUMENT': ServiceType.IDENTITY_BI}
        ...
        if isinstance(self.priority, str):
            legacy_priority_map = {'NORMAL': RequestPriority.MEDIUM, 'MEDIUM': RequestPriority.MEDIUM, 'LOW': RequestPriority.LOW, 'HIGH': RequestPriority.HIGH, 'URGENT': RequestPriority.URGENT, 'CRITICAL': RequestPriority.CRITICAL}
    ...
    def change_status(self, new_status: ServiceRequestStatus, changed_by: UUID, reason: Optional[str]=None):
        ...
        self.metadata['status_history'].append({'from': old_status.value, 'to': new_status.value, 'by': str(changed_by), 'at': datetime.now().isoformat(), 'reason': reason})
    ...
    def to_dict(self) -> dict:
        ...
        return {'id': str(self.id), 'request_number': self.request_number, 'citizen_id': str(self.citizen_id), 'created_by_user_id': str(self.created_by_user_id), 'assigned_to_user_id': str(self.assigned_to_user_id) if self.assigned_to_user_id else None, 'service_type': self.service_type.value, 'title': self.title, 'description': self.description, 'status': self.status.value, 'priority': self.priority.value, 'channel': self.channel.value, 'workflow_instance_id': str(self.workflow_instance_id) if self.workflow_instance_id else None, 'workflow_data': self.workflow_data, 'metadata': self.metadata, 'tags': self.tags, 'created_at': self.created_at.isoformat(), 'updated_at': self.updated_at.isoformat() if self.updated_at else None, 'submitted_at': self.submitted_at.isoformat() if self.submitted_at else None, 'completed_at': self.completed_at.isoformat() if self.completed_at else None, 'deadline': self.deadline.isoformat() if self.deadline else None, 'sla_due_at': self.sla_due_at.isoformat() if self.sla_due_at else None, 'sla_breached': self.sla_breached, 'is_active': self.is_active, 'is_draft': self.is_draft, 'is_terminal': self.is_terminal}