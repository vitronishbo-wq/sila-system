...
from apps.backend.app.modules.governance.service_requests.application.services.request_service import (
    RequestService,
)
...
async def test_create_request_uses_identity_validation_first() -> None:
    ...
    with pytest.raises(ValueError, match="Cidadão inativo"):
        await service.create_request(
            citizen_id=uuid4(),
            created_by=uuid4(),
            service_type=ServiceType.EDUCATION_ENROLLMENT,
            title="Pedido de matrícula",
            description="Solicitação de matrícula escolar",
            channel=RequestChannel.WEB,
            priority=RequestPriority.MEDIUM,
            metadata={"turma_id": str(uuid4()), "ano_letivo_id": str(uuid4())},
        )
...
async def test_submit_request_dispatches_to_domain_client() -> None:
    ...
    educacao_client.submit.return_value = {
        "module": "educacao",
        "accepted": True,
