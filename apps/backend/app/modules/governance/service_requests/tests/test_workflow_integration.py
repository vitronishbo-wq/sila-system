...
from apps.backend.app.modules.governance.service_requests.integrations.workflow_client import (
    WorkflowClient,
)
...
async def test_start_workflow():
    ...
    response = await client.start_workflow(
        ...
        variables={"service_type": "DOCUMENT_REQUEST"},
        actor_id=uuid4(),
    )
...
async def test_workflow_transition():
    ...
    client = WorkflowClient()
    success = await client.send_transition(
        instance_id=uuid4(), transition="APPROVE", actor_id=uuid4()
    )
