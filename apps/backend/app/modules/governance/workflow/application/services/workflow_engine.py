...
from apps.backend.app.modules.governance.workflow.integrations.notification_client import (
    NotificationClient,
)
...
class WorkflowEngine:
    ...
    def __init__(
        ...
        notifications: NotificationClient | None = None,
        educacao_adapter: EducacaoAdapterPort | None = None,
        identidade_adapter: IdentidadeAdapterPort | None = None,
        juventude_adapter: JuventudeAdapterPort | None = None,
        emprego_adapter: EmpregoAdapterPort | None = None,
        saude_adapter: SaudeAdapterPort | None = None,
        assistencia_social_adapter: AssistenciaSocialAdapterPort | None = None,
    ):
        ...
        self.adapters = {
            "educacao": educacao_adapter,
            "identidade": identidade_adapter,
            "identity": identidade_adapter,
            "juventude": juventude_adapter,
            "emprego": emprego_adapter,
            "saude": saude_adapter,
            "assistencia": assistencia_social_adapter,
            "assistencia_social": assistencia_social_adapter,
        }
    ...
    async def start_workflow(
        ...
        history = WorkflowHistory.system(
            ...
            data={"workflow_code": workflow_code, "initial_state": initial_state.code},
        )
        ...
        self.notifications.notify_citizen(
            citizen_id, "Processo Iniciado", "Seu processo foi iniciado com sucesso"
        )
        return instance
    ...
    async def execute_transition(
        ...
        if not await self._evaluate_transition_condition(
        ...
        await self.workflow_repo.commit()
        self.notifications.notify_citizen(
            instance.citizen_id, "Processo Atualizado", "Seu processo mudou de status"
        )
        return instance
    ...
    async def assign_task(
        self, task_id: UUID, user_id: UUID, actor_id: UUID
    ) -> WorkflowTask | None:
        ...
        if task:
            ...
            self.notifications.notify_operator(
                user_id, "Nova Tarefa", f"Tarefa atribuída: {task.title}"
            )
        return task

    async def complete_task(
        self, task_id: UUID, result: dict[str, Any] | None = None, actor_id: UUID = None
    ) -> WorkflowTask | None:
            ...
            if task.transition_id:
                ...
                if transition and transition.transition_type.value == "automatic":
                    await self.execute_transition(
                        task.instance_id, transition.code, actor_id or task.assigned_to, result
                    )
                else:
                    await self.workflow_repo.commit()
            else:
    ...
    async def check_sla_breaches(self) -> list[WorkflowInstance]:
        ...
        if overdue_instances:
            ...
            self.notifications.notify_managers(
                "SLA Estourado", f"{len(overdue_instances)} processo(s) ultrapassaram o prazo"
            )
        ...
        for transition in transitions:
            if transition.transition_type.value == "automatic":
                await self.create_task(
                    ...
                    assigned_role=transition.assignment_value
                    if transition.assignment_type.value == "role"
                    else None,
    ...
    async def _evaluate_transition_condition(
        ...
        if " and " in expression:
            ...
            return all(
                await self._evaluate_transition_condition(part, instance, form_data)
                for part in parts
            )
        if " or " in expression:
        ...
        if expression.lower().startswith("not "):
            return not await self._evaluate_transition_condition(
                expression[4:].strip(), instance, form_data
            )
        ...
        if "." in expression:
            ...
            if method_name:
                result = await self._invoke_adapter_call(
                    adapter_name=adapter_name.strip(), method_name=method_name, payload=payload
                )
                return bool(result)
        ...
        for action_type, action_config in actions.items():
            if action_type == "update_variables":
                ...
                self.notifications.notify_citizen(
                    ...
                    title,
                    message,
                    self._resolve_value(data, payload) if data else None,
                )
    ...
    async def _run_adapter_actions(
        self, action_config: Any, instance: WorkflowInstance, form_data: dict[str, Any] | None
    ) -> None:
    ...
    async def _invoke_adapter_call(
        self, adapter_name: str, method_name: str, payload: dict[str, Any]
    ) -> Any:
    ...
    def _build_runtime_payload(
        instance: WorkflowInstance, form_data: dict[str, Any] | None = None
    ) -> dict[str, Any]:
