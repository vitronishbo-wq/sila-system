...
from apps.backend.app.core.bridges.society_repository_bridges import (
    ...
    make_assistencia_visita_repository,
    ...
    make_saude_appointment_repository,
)
from apps.backend.app.modules.governance.workflow.application.services.workflow_engine import (
    WorkflowEngine,
)
from apps.backend.app.modules.governance.workflow.infrastructure.adapters import (
...
from apps.backend.app.modules.governance.workflow.infrastructure.repositories.task_repository import (
    TaskRepository,
)
...
async def can_view_instance(instance_id: UUID, db: AsyncSession = db_dep):
...
async def get_workflow_engine(db: AsyncSession = db_dep) -> WorkflowEngine:
    """Compõe engine com dependências concretas na borda (API)."""
    workflow_repo = WorkflowRepository(db)
    task_repo = TaskRepository(db)
    educacao_adapter = EducacaoAdapter(
        matricula_repo=make_educacao_matricula_repository(db),
        turma_repo=make_educacao_turma_repository(db),
    )
    ...
    assistencia_social_adapter = AssistenciaSocialAdapter(
        beneficiario_repo=make_assistencia_beneficiario_repository(db),
        visita_repo=make_assistencia_visita_repository(db),
    )
    return WorkflowEngine(
        workflow_repo=workflow_repo,
        task_repo=task_repo,
        educacao_adapter=educacao_adapter,
        identidade_adapter=identidade_adapter,
        juventude_adapter=juventude_adapter,
        emprego_adapter=emprego_adapter,
        saude_adapter=saude_adapter,
        assistencia_social_adapter=assistencia_social_adapter,
    )