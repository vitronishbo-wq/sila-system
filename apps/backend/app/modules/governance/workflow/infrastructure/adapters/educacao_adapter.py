...
class EducacaoAdapter(EducacaoAdapterPort):
    ...
    async def has_matricula_ativa(self, citizen_id: UUID) -> bool:
        ...
        return any(
            str(getattr(item.status, "value", item.status)).lower() in ativos for item in matriculas
        )

    async def validar_disponibilidade_vaga(self, *, turma_id: UUID, ano_letivo_id: UUID) -> bool:
        turma = await self._turma_repo.get_by_id(turma_id, for_update=True)
        if turma is None or not turma.ativa:
            return False
        if turma.ano_letivo_id != ano_letivo_id:
            return False
        ocupacao = await self._turma_repo.count_matriculas_ativas(
            turma_id=turma_id, ano_letivo_id=ano_letivo_id
        )
