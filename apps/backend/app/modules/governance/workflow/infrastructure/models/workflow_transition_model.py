...
class WorkflowTransitionModel(Base):
    ...
    workflow_id = Column(
        UUID(as_uuid=True), ForeignKey("wf_definitions.id", ondelete="CASCADE"), nullable=False
    )
    from_state_id = Column(
        UUID(as_uuid=True), ForeignKey("wf_states.id", ondelete="CASCADE"), nullable=False
    )
    to_state_id = Column(
        UUID(as_uuid=True), ForeignKey("wf_states.id", ondelete="CASCADE"), nullable=False
    )
    ...
    from_state = relationship(
        "WorkflowStateModel", foreign_keys=[from_state_id], back_populates="transitions_from"
    )
    to_state = relationship(
        "WorkflowStateModel", foreign_keys=[to_state_id], back_populates="transitions_to"
    )
    __table_args__ = (
        Index("ix_wf_transitions_workflow_code", "workflow_id", "code", unique=True),
        Index("ix_wf_transitions_from_state", "from_state_id"),
