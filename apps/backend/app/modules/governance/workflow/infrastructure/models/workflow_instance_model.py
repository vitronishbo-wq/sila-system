...
class WorkflowInstanceModel(Base):
    ...
    tasks = relationship(
        "WorkflowTaskModel", back_populates="instance", cascade="all, delete-orphan"
    )
    history = relationship(
        "WorkflowHistoryModel", back_populates="instance", cascade="all, delete-orphan"
    )
    __table_args__ = (
        Index("ix_wf_instances_entity", "entity_type", "entity_id"),
        Index("ix_wf_instances_citizen", "citizen_id"),
        Index("ix_wf_instances_assigned", "assigned_to"),
        Index("ix_wf_instances_status", "status"),
        Index("ix_wf_instances_deadline", "deadline"),
