...
from apps.backend.app.modules.governance.workflow.domain.enums import (
    AssignmentType,
    TransitionType,
    WorkflowStatus,
)
...
class WorkflowRepository(WorkflowRepositoryPort):
    ...
    async def list_definitions(
        ...
        total_stmt = (
            ...
            .where(WorkflowDefinitionModel.is_active)
        )
        total = int((await self.db.execute(total_stmt)).scalar() or 0)
        stmt = (
            select(WorkflowDefinitionModel)
            .where(WorkflowDefinitionModel.is_active)
            .order_by(desc(WorkflowDefinitionModel.created_at))
            .offset(skip)
            .limit(limit)
        )
        result = await self.db.execute(stmt)
    ...
    async def get_initial_state(self, workflow_id: UUID) -> WorkflowState | None:
        stmt = select(WorkflowStateModel).where(
            WorkflowStateModel.workflow_id == workflow_id, WorkflowStateModel.is_initial
        )
        result = await self.db.execute(stmt)
        model = result.scalars().first()
    ...
    def _definition_to_domain(self, model: WorkflowDefinitionModel) -> WorkflowDefinition | None:
        if not model:
            return None
        return WorkflowDefinition(
            ...
            metadata=model.definition_metadata or {},
            tags=model.tags or [],
    ...
    def _definition_to_model(self, domain: WorkflowDefinition) -> WorkflowDefinitionModel:
        return WorkflowDefinitionModel(
            ...
            definition_metadata=domain.metadata,
            tags=domain.tags,
    ...
    def _state_to_domain(self, model: WorkflowStateModel) -> WorkflowState | None:
        if not model:
            return None
        return WorkflowState(
            ...
            metadata=model.state_metadata or {},
            created_at=model.created_at,
            updated_at=model.updated_at,
        )
    ...
    def _state_to_model(self, domain: WorkflowState) -> WorkflowStateModel:
        return WorkflowStateModel(
            ...
            state_metadata=domain.metadata,
            created_at=domain.created_at,
            updated_at=domain.updated_at,
        )

    def _transition_to_domain(self, model: WorkflowTransitionModel) -> WorkflowTransition | None:
        if not model:
            return None
        return WorkflowTransition(
            ...
            metadata=model.transition_metadata or {},
            created_at=model.created_at,
            updated_at=model.updated_at,
        )
    ...
    def _transition_to_model(self, domain: WorkflowTransition) -> WorkflowTransitionModel:
        return WorkflowTransitionModel(
            ...
            transition_metadata=domain.metadata,
            created_at=domain.created_at,
            updated_at=domain.updated_at,
        )

    def _instance_to_domain(self, model: WorkflowInstanceModel) -> WorkflowInstance | None:
        if not model:
            return None
        return WorkflowInstance(
            ...
            timeout_hours=model.timeout_hours,
            metadata=model.instance_metadata or {},
            updated_at=model.updated_at,
        )
    ...
    def _instance_to_model(self, domain: WorkflowInstance) -> WorkflowInstanceModel:
        return WorkflowInstanceModel(
            ...
            timeout_hours=domain.timeout_hours,
            instance_metadata=domain.metadata,
            updated_at=domain.updated_at,
        )

    def _history_to_domain(self, model: WorkflowHistoryModel) -> WorkflowHistory | None:
        if not model:
            return None
        return WorkflowHistory(
            ...
            data=model.data or {},
            metadata=model.history_metadata or {},
            created_at=model.created_at,
        )
    ...
    def _history_to_model(self, domain: WorkflowHistory) -> WorkflowHistoryModel:
        return WorkflowHistoryModel(
            ...
            data=domain.data,
            history_metadata=domain.metadata,
            created_at=domain.created_at,
        )
