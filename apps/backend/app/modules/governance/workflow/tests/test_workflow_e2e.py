...
def random_ids():
    ...
    return {'entity_id': str(uuid.uuid4()), 'citizen_id': str(uuid.uuid4()), 'created_by': str(uuid.uuid4())}
...
def test_create_definition(client):
    ...
    Uses PostgreSQL transactional isolation via db_session fixture.
    """
    payload = {'code': 'TEST_FLOW_2', 'name': 'Test Flow 2', 'description': 'Test workflow 2', 'version': 1}
...
def test_create_instance(client):
    ...
    Uses PostgreSQL transactional isolation via db_session fixture.
    """
    payload = {'workflow_code': 'TEST_FLOW_1', 'entity_type': 'SERVICE_REQUEST', 'entity_id': str(uuid.uuid4()), 'variables': {}}
...
def test_create_and_start_workflow(client, random_ids):
    ...
    Uses PostgreSQL transactional isolation via db_session fixture.
    """
    payload_start = {'workflow_code': 'TEST_FLOW_1', 'entity_type': 'SERVICE_REQUEST', 'entity_id': random_ids['entity_id'], 'variables': {'foo': 'bar'}}
    ...
    r = client.post(f'/api/v1/workflow/instances/{instance_id}/transitions', json={'transition_code': 'TO_DONE'})
    assert r.status_code == 200
    inst3 = r.json()
...
def test_task_lifecycle(client, random_ids):
    ...
    Uses PostgreSQL transactional isolation via db_session fixture.
    """
    payload_start = {'workflow_code': 'TEST_FLOW_1', 'entity_type': 'SERVICE_REQUEST', 'entity_id': str(uuid.uuid4()), 'variables': {}}
    r = client.post('/api/v1/workflow/start', json=payload_start)
    assert r.status_code == 200
    instance = r.json()
    ...
    if items:
        task = items[0]
        task_id = task.get('id')
        r = client.post(f'/api/v1/workflow/tasks/{task_id}/assign', json={'user_id': random_ids['created_by']})
        assert r.status_code == 200
