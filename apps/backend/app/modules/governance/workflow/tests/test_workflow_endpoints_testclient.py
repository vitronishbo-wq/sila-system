...
async def workflow_test_engine():
    ...
    engine = create_async_engine(
        _database_url(), echo=False, pool_pre_ping=True, poolclass=NullPool
    )
    yield engine
    ...
    async with workflow_test_engine.begin() as connection:
        await connection.run_sync(
            lambda sync_connection: Base.metadata.create_all(
                sync_connection, tables=required_tables, checkfirst=True
            )
        )
    yield async_sessionmaker(
        bind=workflow_test_engine, class_=AsyncSession, expire_on_commit=False, autocommit=False
    )


def _build_test_app(
    *, session_factory: async_sessionmaker[AsyncSession], user_id: UUID, citizen_id: UUID
) -> FastAPI:
    app = FastAPI()
        ...
        return IdentityContext(
            {
                ...
                "email": "workflow-test@sila.local",
            }
        )
...
) -> tuple[WorkflowDefinition, WorkflowState, WorkflowState]:
    async with session_factory() as session:
        repo = WorkflowRepository(session)
        definition = WorkflowDefinition(
            code=code, name=f"Workflow {code}", entity_type="SERVICE_REQUEST"
        )
        start_state = WorkflowState(
            workflow_id=definition.id, code="INICIO", name="Inicio", is_initial=True
        )
...
async def _seed_citizen(
    session_factory: async_sessionmaker[AsyncSession], citizen_id: UUID
) -> None:
    async with session_factory() as session:
        session.add(
            CitizenFUC(
...
@pytest.mark.asyncio
async def test_start_workflow_endpoint_with_testclient(
    session_factory: async_sessionmaker[AsyncSession],
):
    user_id = uuid4()
    citizen_id = uuid4()
    definition, _, _ = await _seed_workflow_definition(
        session_factory, code=f"WF_START_{uuid4().hex[:6]}"
    )
    ...
    with TestClient(app) as client:
        response = client.post(
            ...
            json={
                ...
                "entity_id": str(uuid4()),
                "variables": {"origem": "endpoint-test"},
            },
        )
...
@pytest.mark.asyncio
async def test_transition_endpoint_applies_condition_expression_and_call_adapter(
    session_factory: async_sessionmaker[AsyncSession],
):
    user_id = uuid4()
    citizen_id = uuid4()
    await _seed_citizen(session_factory, citizen_id)
    definition, _, end_state = await _seed_workflow_definition(
        session_factory,
        code=f"WF_TRANS_{uuid4().hex[:6]}",
    ...
    with TestClient(app) as client:
        ...
        transition_response = client.post(
            f"/api/v1/workflow/instances/{instance_id}/transitions",
            json={"transition_code": "AVANCAR", "form_data": {}},
        )
