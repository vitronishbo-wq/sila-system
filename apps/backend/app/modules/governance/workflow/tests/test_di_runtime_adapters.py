...
async def test_execute_transition_uses_adapter_condition_expression():
    ...
    definition = WorkflowDefinition(
        code="WF_INT", name="Workflow Integracao", entity_type="SERVICE_REQUEST", id=workflow_id
    )
    ...
    transition = WorkflowTransition(
        ...
        code="NEXT",
        name="Avancar",
        condition_expression="educacao.has_matricula_ativa",
    )
    to_state = WorkflowState(
        workflow_id=workflow_id, id=to_state_id, code="DONE", name="Concluido", is_final=True
    )
    ...
    engine = WorkflowEngine(
        ...
        notifications=SimpleNamespace(
            notify_citizen=lambda *args, **kwargs: None,
            notify_operator=lambda *args, **kwargs: None,
            notify_managers=lambda *args, **kwargs: None,
        ),
        educacao_adapter=educacao_stub,
    )
    result = await engine.execute_transition(
        instance_id=instance.id, transition_code="NEXT", actor_id=actor_id
    )
...
async def test_execute_transition_runs_adapter_action_and_stores_result():
    ...
    actor_id = uuid4()
    definition = WorkflowDefinition(
        code="WF_ACT", name="Workflow Actions", entity_type="SERVICE_REQUEST", id=workflow_id
    )
    instance = WorkflowInstance(
        workflow_id=workflow_id,
        current_state_id=from_state_id,
    ...
    transition = WorkflowTransition(
        ...
        post_actions={
            "call_adapter": {
                "adapter": "emprego",
                "method": "get_status_candidato",
                "args": {"citizen_id": "$citizen_id"},
                "save_as": "status_emprego",
            }
        },
    )
    to_state = WorkflowState(
        workflow_id=workflow_id, id=to_state_id, code="IN_PROGRESS", name="Em andamento"
    )
    workflow_repo = AsyncMock()
    task_repo = AsyncMock()
    workflow_repo.get_instance.return_value = instance
