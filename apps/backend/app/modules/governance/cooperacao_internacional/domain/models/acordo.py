...
class Acordo:
        ...
        self.partes.append(
            {
                ...
                "data_adesao": data_adesao.isoformat(),
                "assinante": assinante,
                "titulo_assinante": titulo_assinante,
            }
        )
    ...
    def ratificar(self, *, parte_id: UUID, data_ratificacao: date, instrumento: str) -> None:
        self.ratificacoes.append(
            {
                "parte_id": str(parte_id),
                "data_ratificacao": data_ratificacao.isoformat(),
    ...
    def denunciar(self, *, parte_id: UUID, data_denuncia: date, motivo: str) -> None:
        self.denuncias.append(
            {
                "parte_id": str(parte_id),
                "data_denuncia": data_denuncia.isoformat(),
                "motivo": motivo,
            }
        )
    ...
    def vigencia_restante_dias(self) -> int | None:
        if self.data_vigor is None or self.prazo_anos is None:
            return None
        fim = date(
            self.data_vigor.year + self.prazo_anos, self.data_vigor.month, self.data_vigor.day
        )
        if date.today() >= fim:
            return 0
