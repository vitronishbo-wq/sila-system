...
class ProjetoCooperacaoService:
    ...
    def __init__(self, *, projeto_repo: ProjetoCooperacaoRepositoryPort, outbox: InMemoryOutbox) -> None:
        self.projeto_repo = projeto_repo
        self.outbox = outbox

    async def criar_projeto(self, *, titulo: str, tipo: TipoProjeto, modalidade: ModalidadeCooperacao, orgao_responsavel_id: UUID, orgao_parceiro_id: UUID, pais_parceiro_id: UUID, data_inicio: date, data_fim: date, objetivo_geral: str, objetivos_especificos: list[str], orcamento_total: float, fonte_recursos: str, acordo_base_id: UUID | None=None) -> ProjetoCooperacao:
        projeto = ProjetoCooperacao(titulo=titulo, tipo=tipo, modalidade=modalidade, acordo_base_id=acordo_base_id, orgao_responsavel_id=orgao_responsavel_id, orgao_parceiro_id=orgao_parceiro_id, pais_parceiro_id=pais_parceiro_id, data_inicio=data_inicio, data_fim=data_fim, objetivo_geral=objetivo_geral, objetivos_especificos=objetivos_especificos, orcamento_total=orcamento_total, fonte_recursos=fonte_recursos)
    ...
    async def aprovar_projeto(self, *, projeto_id: UUID) -> ProjetoCooperacao:
        ...
        projeto.aprovar()
        await self.projeto_repo.save(projeto)
        evento = ProjetoCooperacaoAprovadoEvent(projeto_id=projeto.id, codigo_projeto=projeto.codigo_projeto, titulo=projeto.titulo, pais_parceiro_id=projeto.pais_parceiro_id, orcamento_total=projeto.orcamento_total)
