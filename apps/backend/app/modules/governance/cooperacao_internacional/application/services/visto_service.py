...
class VistoService:
    ...
    async def solicitar_visto(self, *, tipo: TipoVisto, categoria: CategoriaVisto, solicitante_cpf: str, solicitante_nome: str, solicitante_passaporte: str, pais_origem_id: UUID, pais_destino_id: UUID, data_entrada_prevista: date, data_saida_prevista: date, objetivo_viagem: str, consulato_emissor_id: UUID) -> Visto:
        visto = Visto(tipo=tipo, categoria=categoria, solicitante_cpf=solicitante_cpf, solicitante_nome=solicitante_nome, solicitante_passaporte=solicitante_passaporte, pais_origem_id=pais_origem_id, pais_destino_id=pais_destino_id, data_entrada_prevista=data_entrada_prevista, data_saida_prevista=data_saida_prevista, objetivo_viagem=objetivo_viagem, consulato_emissor_id=consulato_emissor_id)
        await self.visto_repo.save(visto)
        return visto

    async def analisar_visto(self, *, visto_id: UUID, analista: str, resultado: str, justificativa: str | None=None) -> Visto:
    ...
    async def aprovar_visto(self, *, visto_id: UUID, autoridade: str, validade_dias: int=90) -> Visto:
        visto = await self.visto_repo.get_by_id(visto_id)
        if visto is None:
            raise ValueError('Visto nao encontrado')
        visto.aprovar(autoridade=autoridade, validade_dias=validade_dias)
        await self.visto_repo.save(visto)
        evento = VistoAprovadoEvent(visto_id=visto.id, numero_processo=visto.numero_processo, solicitante_cpf=visto.solicitante_cpf, tipo=visto.tipo.value, data_validade=visto.data_validade.isoformat() if visto.data_validade else '')
