...
visto_service_dep = Depends(get_visto_service)


@router.post("/", response_model=VistoResponse, status_code=status.HTTP_201_CREATED)
async def solicitar_visto(
    payload: VistoCreate, service: VistoService = visto_service_dep
) -> VistoResponse:
    try:
...
async def analisar_visto(
    visto_id: UUID, payload: VistoAnaliseInput, service: VistoService = visto_service_dep
) -> VistoResponse:
    try:
...
async def aprovar_visto(
    visto_id: UUID, payload: VistoAprovarInput, service: VistoService = visto_service_dep
) -> VistoResponse:
    try:
...
async def emitir_visto(
    visto_id: UUID, service: VistoService = visto_service_dep
) -> VistoResponse:
    try:
...
async def negar_visto(
    visto_id: UUID, payload: VistoNegarInput, service: VistoService = visto_service_dep
) -> VistoResponse:
    try:
...
async def listar_vistos(service: VistoService = visto_service_dep) -> list[VistoResponse]:
