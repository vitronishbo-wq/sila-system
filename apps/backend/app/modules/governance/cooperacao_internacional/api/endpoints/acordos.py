...
acordo_service_dep = Depends(get_acordo_service)
dias_query = Query(90, ge=1, le=3650)


@router.post("/", response_model=AcordoResponse, status_code=status.HTTP_201_CREATED)
async def criar_acordo(
    payload: AcordoCreate, service: AcordoService = acordo_service_dep
) -> AcordoResponse:
    try:
...
async def listar_acordos(
    service: AcordoService = acordo_service_dep,
) -> list[AcordoResponse]:
...
async def assinar_acordo(
    acordo_id: UUID,
    payload: AcordoAssinarInput,
    service: AcordoService = acordo_service_dep,
) -> AcordoResponse:
    try:
        acordo = await service.assinar_acordo(
            acordo_id=acordo_id,
            partes=payload.model_dump()["partes"],
            local_assinatura=payload.local_assinatura,
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    return AcordoResponse.model_validate(acordo)
...
async def iniciar_vigor_acordo(
    acordo_id: UUID, payload: AcordoVigorInput, service: AcordoService = acordo_service_dep
) -> AcordoResponse:
    try:
...
async def acordos_vencimento_proximo(
    dias: int = dias_query, service: AcordoService = acordo_service_dep
) -> list[dict]:
