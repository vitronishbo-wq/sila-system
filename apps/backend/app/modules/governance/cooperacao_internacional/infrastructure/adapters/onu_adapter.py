...
class ONUAdapter:
    ...
    async def registrar_tratado(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self._client.post(
            f"{self.base_url}/treaties/register", json=payload, headers=self._headers()
        )
        response.raise_for_status()
        return response.json()

    @circuit_breaker(name="cooperacao_onu", failure_threshold=3, recovery_timeout=60)
    async def registrar_extincao(
        self, *, numero_acordo: str, data_extincao: date
    ) -> dict[str, Any]:
        if not self.base_url:
            return {"status": "simulado", "numero": numero_acordo}
        response = await self._client.post(
            f"{self.base_url}/treaties/{numero_acordo}/terminate",
            json={"data_extincao": data_extincao.isoformat()},
