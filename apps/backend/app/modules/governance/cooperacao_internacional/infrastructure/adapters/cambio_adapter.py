...
class CambioAdapter:
    ...
    async def cotar(self, *, moeda_origem: str, moeda_destino: str) -> dict[str, Any]:
        if not self.base_url:
            return {'moeda_origem': moeda_origem, 'moeda_destino': moeda_destino, 'taxa': 1.0, 'timestamp': datetime.utcnow().isoformat()}
        response = await self._client.get(f'{self.base_url}/fx/rate', params={'from': moeda_origem, 'to': moeda_destino}, headers=self._headers())
