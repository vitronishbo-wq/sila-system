...
class TratadoMonitorWorker:
    def __init__(
        self, *, acordo_repo, mre_adapter=None, onu_adapter=None, check_interval: int = 86400
    ) -> None:
        ...
        for acordo in acordos:
            ...
            if dias_restantes == 0:
                ...
                if self.onu is not None and acordo.tipo == TipoAcordo.TRATADO:
                    await self.onu.registrar_extincao(
                        numero_acordo=acordo.numero_registro, data_extincao=datetime.utcnow().date()
                    )
