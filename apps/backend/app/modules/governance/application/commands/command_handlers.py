...
from apps.backend.app.modules.governance.application.commands.governance_commands import (
...
class StartWorkflowHandler:
    ...
    async def handle(self, command: StartWorkflowCommand) -> Workflow:
        workflow = Workflow(
            ...
            total_steps=command.metadata.get("total_steps", 1) if command.metadata else 1,
