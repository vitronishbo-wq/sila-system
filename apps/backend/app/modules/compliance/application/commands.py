...
from apps.backend.app.modules.compliance.domain.events import (
    ComplianceInspectionConducted,
...
class CreateComplianceObligationCommandHandler:
    ...
    async def handle(
        ...
        try:
            event = ComplianceObligationCreated(
                aggregate_id=obligation_id,
                aggregate_type="ComplianceObligation",
                event_type="ComplianceObligationCreated",
                ...
                metadata={
                    ...
                    "command_name": "CreateComplianceObligation",
...
class ConductInspectionCommandHandler:
    ...
    async def handle(
        ...
        try:
            event = ComplianceInspectionConducted(
                aggregate_id=inspection_id,
                aggregate_type="ComplianceInspection",
                event_type="ComplianceInspectionConducted",
                ...
                metadata={
                    "user_id": str(user_id) if user_id else None,
                    "command_name": "ConductInspection",
                    "timestamp_utc": datetime.now(UTC).isoformat(),
                },
            )
            await self.event_bus.publish(event)
...
class ReportViolationCommandHandler:
    ...
    async def handle(
        ...
        try:
            event = ComplianceViolationReported(
                aggregate_id=violation_id,
                aggregate_type="ComplianceViolation",
                event_type="ComplianceViolationReported",
