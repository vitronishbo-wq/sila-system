...
from apps.backend.app.modules.compliance.application.commands.compliance_commands import (
...
class LogAuditHandler:
    ...
    async def handle(self, command: LogAuditCommand) -> AuditLog:
        audit_log = AuditLog(
            ...
            level=AuditLevel(command.level) if isinstance(command.level, str) else command.level,
