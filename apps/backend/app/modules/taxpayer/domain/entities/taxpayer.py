...
class Taxpayer:
    ...
    nif: str = ""  # Identificador único fiscal
    ...
    # === REFERÊNCIAS EXTERNAS (por ID, não strong references) ===
    ...
    # === ENTIDADES DO AGREGADO (armazenadas aqui, não em repositórios separados) ===
    ...
    def validate(self) -> None:
        ...
        # Validar que não há 2 declarations no mesmo período
        periods_seen = set()
        for declaration in self.declarations:
            period_key = f"{declaration.tax_type}:{declaration.tax_period}"
            if period_key in periods_seen:
                raise ValueError(
                    f"Não pode haver 2 declarações para o mesmo período: {period_key}"
                )
