...
class TaxDeclarationNumber:
    ...
    def __post_init__(self) -> None:
        ...
        # Validar que o tax_type no formato corresponde
        parts = self.value.split("-")
        if parts[0] != self.tax_type:
            raise ValueError(
                f"Tipo de imposto não corresponde: "
                f"{parts[0]} vs {self.tax_type}"
            )
        
        if int(parts[1]) != self.year:
            raise ValueError(
                f"Ano não corresponde: {parts[1]} vs {self.year}"
            )
    ...
    def generate(
        ...
        """
        if not re.match(r"^[A-Z]{2,3}$", tax_type):
