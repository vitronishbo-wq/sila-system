...
from apps.backend.app.modules.civil_protection.tests._fakes import InMemoryAtendimentoRepository, InMemoryBombeiroRepository, InMemoryCorporacaoRepository, InMemoryDespachoRepository, InMemoryOcorrenciaEmergencialRepository
...
def test_registrar_atendimento_conclui_despacho() -> None:
    ...
    async def scenario() -> None:
        ...
        corporacao = await corporacao_service.cadastrar_corporacao(nome='Corpo Atendimento', municipio='Luanda', provincia='Luanda', endereco='Rua Atendimento', comandante='Cmd Atendimento')
        bombeiro = await bombeiro_service.cadastrar_bombeiro(corporacao_id=corporacao.id, nome='Bombeiro A', data_nascimento=date.today() - timedelta(days=365 * 30), cpf='201.202.203-40', rg='RG201202')
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(corporacao_id=corporacao.id, bombeiro_responsavel_id=bombeiro.id, tipo=TipoOcorrenciaEmergencial.INUNDACAO, prioridade=PrioridadeAtendimento.ALTA, data_ocorrencia=datetime.now() - timedelta(hours=1), descricao='Inundacao com familias desalojadas', municipio='Luanda', provincia='Luanda')
        despacho = await despacho_service.registrar_despacho(ocorrencia_id=ocorrencia.id, bombeiro_responsavel_id=bombeiro.id)
        atendimento = await atendimento_service.registrar_atendimento(despacho_id=despacho.id, local_atendimento='Bairro Azul', vitimas_atendidas=3, desalojados_atendidos=5, equipe_responsavel_id=bombeiro.id)
...
def test_finalizar_atendimento_conclui_ocorrencia() -> None:
    ...
    async def scenario() -> None:
        ...
        despacho_service = DespachoService(despacho_repo=despacho_repo, ocorrencia_repo=ocorrencia_repo, bombeiro_repo=bombeiro_repo)
        atendimento_service = AtendimentoService(atendimento_repo=atendimento_repo, despacho_repo=despacho_repo, ocorrencia_repo=ocorrencia_repo, bombeiro_repo=bombeiro_repo)
        corporacao = await corporacao_service.cadastrar_corporacao(nome='Corpo Finalizacao', municipio='Bengo', provincia='Bengo', endereco='Rua Final', comandante='Cmd Final')
        bombeiro = await bombeiro_service.cadastrar_bombeiro(corporacao_id=corporacao.id, nome='Bombeiro F', data_nascimento=date.today() - timedelta(days=365 * 33), cpf='301.302.303-44', rg='RG301302')
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(corporacao_id=corporacao.id, bombeiro_responsavel_id=bombeiro.id, tipo=TipoOcorrenciaEmergencial.DESLIZAMENTO, prioridade=PrioridadeAtendimento.ALTA, data_ocorrencia=datetime.now() - timedelta(hours=2), descricao='Deslizamento em encosta residencial', municipio='Bengo', provincia='Bengo')
        despacho = await despacho_service.registrar_despacho(ocorrencia_id=ocorrencia.id, bombeiro_responsavel_id=bombeiro.id)
        atendimento = await atendimento_service.registrar_atendimento(despacho_id=despacho.id, local_atendimento='Encosta Sul', equipe_responsavel_id=bombeiro.id)
        finalizado = await atendimento_service.finalizar_atendimento(atendimento_id=atendimento.id, resumo='Atendimento concluido sem novos riscos')
