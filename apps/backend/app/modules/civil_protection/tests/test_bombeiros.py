...
def test_cadastrar_bombeiro_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        bombeiro = await bombeiro_service.cadastrar_bombeiro(
            ...
            nome="Joao Costa",
            data_nascimento=date.today() - timedelta(days=365 * 29),
            cpf="123.456.789-00",
            rg="RG123456",
            cargo=CargoBombeiro.SARGENTO,
        )
...
def test_cadastrar_bombeiro_falha_cpf_duplicado() -> None:
    ...
    async def scenario() -> None:
        ...
        corporacao = await corporacao_service.cadastrar_corporacao(
            nome="Corpo de Bombeiros Oeste",
            municipio="Huambo",
            provincia="Huambo",
            endereco="Rua Oeste",
            comandante="Comandante Oeste",
        )
        payload = dict(
            corporacao_id=corporacao.id,
            nome="Ana Rocha",
            data_nascimento=date.today() - timedelta(days=365 * 28),
            cpf="987.654.321-00",
            rg="RG998877",
        )
...
def test_atualizar_status_bombeiro() -> None:
    ...
    async def scenario() -> None:
        ...
        atualizado = await bombeiro_service.atualizar_status(
            bombeiro_id=bombeiro.id, status=StatusAgenteProtecao.LICENCA, motivo="Licenca medica"
        )
