...
def _tables():
    ...
    return [
        CorporacaoModel.__table__,
        BombeiroModel.__table__,
        OcorrenciaEmergencialModel.__table__,
        DespachoModel.__table__,
    ...
    async def scenario() -> None:
        async with _session_scope() as session:
            ...
            corporacao = await corporacao_service.cadastrar_corporacao(
                nome="Corpo de Bombeiros ORM",
                municipio="Luanda",
                provincia="Luanda",
                endereco="Rua ORM 100",
                comandante="Comandante ORM",
            )
            ...
            ocorrencia = await ocorrencia_service.registrar_ocorrencia(
                ...
                descricao="Teste integracao ORM protecao civil",
                municipio="Luanda",
                provincia="Luanda",
                vitimas=2,
                desalojados=3,
            ...
            atendimento = await atendimento_service.registrar_atendimento(
                ...
                local_atendimento="Bairro ORM",
                vitimas_atendidas=2,
                desalojados_atendidos=3,
                equipe_responsavel_id=bombeiro.id,
            )
            atendimento_finalizado = await atendimento_service.finalizar_atendimento(
                atendimento_id=atendimento.id, resumo="Fluxo completo encerrado"
            )
