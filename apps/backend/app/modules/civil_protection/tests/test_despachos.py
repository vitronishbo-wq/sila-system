...
def test_registrar_despacho_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        corporacao = await corporacao_service.cadastrar_corporacao(nome='Corpo Despacho', municipio='Luanda', provincia='Luanda', endereco='Rua Despacho', comandante='Cmd Despacho')
        bombeiro = await bombeiro_service.cadastrar_bombeiro(corporacao_id=corporacao.id, nome='Bombeiro D', data_nascimento=date.today() - timedelta(days=365 * 30), cpf='101.202.303-40', rg='RG101202')
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(corporacao_id=corporacao.id, bombeiro_responsavel_id=bombeiro.id, tipo=TipoOcorrenciaEmergencial.INCENDIO_URBANO, prioridade=PrioridadeAtendimento.ALTA, data_ocorrencia=datetime.now() - timedelta(hours=1), descricao='Incendio urbano em residencia', municipio='Luanda', provincia='Luanda')
...
def test_registrar_despacho_falha_bombeiro_outra_corporacao() -> None:
    ...
    async def scenario() -> None:
        ...
        ocorrencia_service = OcorrenciaEmergencialService(ocorrencia_repo=ocorrencia_repo, corporacao_repo=corporacao_repo, bombeiro_repo=bombeiro_repo)
        despacho_service = DespachoService(despacho_repo=despacho_repo, ocorrencia_repo=ocorrencia_repo, bombeiro_repo=bombeiro_repo)
        corp_a = await corporacao_service.cadastrar_corporacao(nome='Corpo A', municipio='Luanda', provincia='Luanda', endereco='Rua A', comandante='Cmd A')
        corp_b = await corporacao_service.cadastrar_corporacao(nome='Corpo B', municipio='Luanda', provincia='Luanda', endereco='Rua B', comandante='Cmd B')
        bombeiro_b = await bombeiro_service.cadastrar_bombeiro(corporacao_id=corp_b.id, nome='Bombeiro B', data_nascimento=date.today() - timedelta(days=365 * 31), cpf='111.222.333-99', rg='RG778899')
        ocorrencia = await ocorrencia_service.registrar_ocorrencia(corporacao_id=corp_a.id, tipo=TipoOcorrenciaEmergencial.DESABAMENTO, prioridade=PrioridadeAtendimento.MEDIA, data_ocorrencia=datetime.now() - timedelta(hours=1), descricao='Desabamento em edificio comercial', municipio='Luanda', provincia='Luanda')
        with pytest.raises(ValueError, match='nao pertence'):
            await despacho_service.registrar_despacho(ocorrencia_id=ocorrencia.id, bombeiro_responsavel_id=bombeiro_b.id)
    asyncio.run(scenario())