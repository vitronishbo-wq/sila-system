...
def test_cadastrar_corporacao_sucesso() -> None:
    ...
    async def scenario() -> None:
        service = CorporacaoService(
            corporacao_repo=InMemoryCorporacaoRepository(), request_service=FakeRequestService()
        )
        corporacao = await service.cadastrar_corporacao(
            nome="Corpo de Bombeiros Luanda Centro",
            municipio="Luanda",
            provincia="Luanda",
            endereco="Rua da Prevencao, 10",
            comandante="Comandante Silva",
            telefone="222100200",
        )
...
def test_cadastrar_corporacao_falha_nome_curto() -> None:
    ...
    async def scenario() -> None:
        ...
        with pytest.raises(ValueError, match="Nome"):
            await service.cadastrar_corporacao(
                nome="CB",
                municipio="Luanda",
                provincia="Luanda",
                endereco="Rua X",
                comandante="Comandante Y",
            )
...
def test_atualizar_status_corporacao() -> None:
    ...
    async def scenario() -> None:
        ...
        atualizada = await service.atualizar_status(
            ...
            status=StatusCorporacao.EM_REESTRUTURACAO,
            motivo="Reforma estrutural",
        )
