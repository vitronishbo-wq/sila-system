    ...
    async def handle(
        ...
        try:
            event = EmergencyAlertIssued(
                ...
                aggregate_type="EmergencyAlert",
                event_type="EmergencyAlertIssued",
                alert_id=alert_id,
                ...
                metadata={
                    ...
                    "command_name": "IssueEmergencyAlert",
...
class RecordDisasterEventCommandHandler:
    ...
    async def handle(
        ...
        try:
            event = DisasterEventOccurred(
                aggregate_id=disaster_id,
                aggregate_type="DisasterEvent",
                event_type="DisasterEventOccurred",
                ...
                metadata={
                    "user_id": str(user_id) if user_id else None,
                    "command_name": "RecordDisasterEvent",
                    "timestamp_utc": datetime.now(UTC).isoformat(),
                },
            )
            await self.event_bus.publish(event)
...
class InitiateEvacuationCommandHandler:
    ...
    async def handle(
        ...
        try:
            event = EvacuationInitiated(
                aggregate_id=evacuation_id,
                aggregate_type="Evacuation",
                ...
                metadata={
                    "user_id": str(user_id) if user_id else None,
                    "command_name": "InitiateEvacuation",
                    "timestamp_utc": datetime.now(UTC).isoformat(),
                },
            )
            await self.event_bus.publish(event)
