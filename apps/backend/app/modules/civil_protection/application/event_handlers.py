...
from apps.backend.app.modules.civil_protection.domain.events import (
    CivilProtectionResourceStatusChanged,
    DisasterEventOccurred,
...
async def handle_evacuation_initiated(event: EvacuationInitiated) -> None:
    ...
    logger.info(
        f"Evacuation initiated for zone {event.evacuation_zone} ({event.affected_population} people)"
    )
    pass
...
CIVIL_PROTECTION_EVENT_HANDLERS: dict[str, Callable[[DomainEvent], Awaitable[None]]] = {
    "EmergencyAlertIssued": handle_emergency_alert_issued,
    "DisasterEventOccurred": handle_disaster_event_occurred,
    "EvacuationInitiated": handle_evacuation_initiated,
    "CivilProtectionResourceStatusChanged": handle_resource_status_changed,
