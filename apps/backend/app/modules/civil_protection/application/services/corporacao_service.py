...
class CorporacaoService:
    def __init__(
        self,
        *,
        corporacao_repo: CorporacaoRepositoryPort,
    ...
    async def cadastrar_corporacao(
        ...
        *,
        ...
        corporacao = Corporacao.cadastrar(
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CADASTRO_CORPORACAO",
                entity_id=saved.id,
                citizen_id=citizen_id,
                numero_processo=saved.codigo_corporacao,
                metadata={
                    "codigo_corporacao": saved.codigo_corporacao,
                    "nome": saved.nome,
    ...
    async def listar_corporacoes(
        self, *, municipio: str | None = None, status: StatusCorporacao | None = None
    ) -> list[Corporacao]:
        if municipio:
    ...
    async def atualizar_status(
        self, *, corporacao_id: UUID, status: StatusCorporacao, motivo: str | None = None
    ) -> Corporacao:
