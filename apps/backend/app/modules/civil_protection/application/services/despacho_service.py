...
class DespachoService:
    ...
    async def registrar_despacho(
        ...
        *,
        ...
        despacho = Despacho.gerar(
            codigo_despacho=codigo_despacho,
            ocorrencia_id=ocorrencia.id,
            corporacao_id=ocorrencia.corporacao_id,
            bombeiro_responsavel_id=bombeiro_responsavel_id,
            meio_deslocamento=meio_deslocamento,
        ...
        if ocorrencia.status == StatusOcorrenciaEmergencial.RECEBIDA:
            ocorrencia.atualizar_status(
                StatusOcorrenciaEmergencial.EM_ATENDIMENTO,
                "Ocorrencia com despacho operacional registrado",
            )
            await self.ocorrencia_repo.save(ocorrencia)
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="REGISTRO_DESPACHO",
                entity_id=saved.id,
                citizen_id=citizen_id,
                numero_processo=saved.codigo_despacho,
                metadata={
                    "codigo_despacho": saved.codigo_despacho,
    ...
    async def listar_despachos(
        self, *, ocorrencia_id: UUID | None = None, status: StatusDespacho | None = None
    ) -> list[Despacho]:
    ...
    async def atualizar_status(
        self, *, despacho_id: UUID, status: StatusDespacho, observacoes: str | None = None
    ) -> Despacho:
