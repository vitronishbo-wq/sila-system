...
from app.modules.civil_protection.domain.enums import StatusAtendimento, StatusDespacho, StatusOcorrenciaEmergencial
from app.modules.civil_protection.domain.models.atendimento import Atendimento

class AtendimentoService:

    def __init__(self, *, atendimento_repo: AtendimentoRepositoryPort, despacho_repo: DespachoRepositoryPort, ocorrencia_repo: OcorrenciaEmergencialRepositoryPort, bombeiro_repo: BombeiroRepositoryPort, request_service: RequestServicePort | None=None) -> None:
    ...
    async def registrar_atendimento(self, *, despacho_id: UUID, local_atendimento: str, vitimas_atendidas: int=0, desalojados_atendidos: int=0, obitos_confirmados: int=0, equipe_responsavel_id: UUID | None=None, observacoes: str | None=None, citizen_id: UUID | None=None) -> Atendimento:
        ...
        atendimento = Atendimento.iniciar(codigo_atendimento=codigo_atendimento, despacho_id=despacho.id, ocorrencia_id=despacho.ocorrencia_id, local_atendimento=local_atendimento, vitimas_atendidas=vitimas_atendidas, desalojados_atendidos=desalojados_atendidos, obitos_confirmados=obitos_confirmados, equipe_responsavel_id=equipe_responsavel_id, observacoes=observacoes)
        ...
        if despacho.status in {StatusDespacho.GERADO, StatusDespacho.EM_DESLOCAMENTO}:
            despacho.atualizar_status(StatusDespacho.CONCLUIDO, 'Despacho operacional concluido com atendimento iniciado')
            await self.despacho_repo.save(despacho)
        if ocorrencia.status == StatusOcorrenciaEmergencial.RECEBIDA:
            ocorrencia.atualizar_status(StatusOcorrenciaEmergencial.EM_ATENDIMENTO, 'Atendimento iniciado para ocorrencia')
            await self.ocorrencia_repo.save(ocorrencia)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='REGISTRO_ATENDIMENTO', entity_id=saved.id, citizen_id=citizen_id, numero_processo=saved.codigo_atendimento, metadata={'codigo_atendimento': saved.codigo_atendimento, 'despacho_id': str(saved.despacho_id), 'status': saved.status.value})
        return saved

    async def finalizar_atendimento(self, *, atendimento_id: UUID, resumo: str | None=None, observacoes: str | None=None) -> Atendimento:
        ...
        if ocorrencia is not None and ocorrencia.status != StatusOcorrenciaEmergencial.CONCLUIDA:
            ocorrencia.atualizar_status(StatusOcorrenciaEmergencial.CONCLUIDA, 'Ocorrencia concluida apos finalizacao do atendimento')
            await self.ocorrencia_repo.save(ocorrencia)
        return saved
    ...
    async def listar_atendimentos(self, *, ocorrencia_id: UUID | None=None, despacho_id: UUID | None=None, status: StatusAtendimento | None=None) -> list[Atendimento]:
    ...
    async def atualizar_status(self, *, atendimento_id: UUID, status: StatusAtendimento, observacoes: str | None=None) -> Atendimento:
        atendimento = await self.buscar_atendimento(atendimento_id)
