...
class BombeiroService:
        ...
        *,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CADASTRO_BOMBEIRO",
                entity_id=saved.id,
    ...
    async def listar_bombeiros(
        self, *, corporacao_id: UUID | None = None, status: StatusAgenteProtecao | None = None
    ) -> list[Bombeiro]:
    ...
    async def atualizar_status(
        self, *, bombeiro_id: UUID, status: StatusAgenteProtecao, motivo: str | None = None
    ) -> Bombeiro:
