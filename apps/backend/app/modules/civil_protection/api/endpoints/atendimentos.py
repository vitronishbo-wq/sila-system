...
from apps.backend.app.modules.civil_protection.application.dto.atendimento_schema import (
    AtendimentoCreate,
...
atendimento_service_dep = Depends(get_atendimento_service)


@router.post("/", response_model=AtendimentoResponse, status_code=status.HTTP_201_CREATED)
async def registrar_atendimento(
    data: AtendimentoCreate, service: AtendimentoService = atendimento_service_dep
) -> AtendimentoResponse:
    try:
        return await service.registrar_atendimento(
            despacho_id=data.despacho_id,
            local_atendimento=data.local_atendimento,
            vitimas_atendidas=data.vitimas_atendidas,
            desalojados_atendidos=data.desalojados_atendidos,
            obitos_confirmados=data.obitos_confirmados,
            equipe_responsavel_id=data.equipe_responsavel_id,
...
async def finalizar_atendimento(
    atendimento_id: UUID,
    data: AtendimentoFinalizacao,
    service: AtendimentoService = atendimento_service_dep,
) -> AtendimentoResponse:
    try:
        return await service.finalizar_atendimento(
            atendimento_id=atendimento_id, resumo=data.resumo, observacoes=data.observacoes
        )
    except ValueError as exc:
...
async def obter_atendimento(
    atendimento_id: UUID, service: AtendimentoService = atendimento_service_dep
) -> AtendimentoResponse:
    try:
...
async def listar_atendimentos(
    ...
    return await service.listar_atendimentos(
        ocorrencia_id=ocorrencia_id, despacho_id=despacho_id, status=status_atendimento
    )
...
async def atualizar_status_atendimento(
    atendimento_id: UUID,
    data: AtendimentoStatusUpdate,
    service: AtendimentoService = atendimento_service_dep,
) -> AtendimentoResponse:
    try:
        return await service.atualizar_status(
            atendimento_id=atendimento_id, status=data.status, observacoes=data.observacoes
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
