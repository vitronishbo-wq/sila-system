...
from apps.backend.app.modules.civil_protection.application.dto.despacho_schema import (
    DespachoCreate,
    DespachoResponse,
...
despacho_service_dep = Depends(get_despacho_service)


@router.post("/", response_model=DespachoResponse, status_code=status.HTTP_201_CREATED)
async def registrar_despacho(
    data: DespachoCreate, service: DespachoService = despacho_service_dep
) -> DespachoResponse:
    try:
        return await service.registrar_despacho(
            ocorrencia_id=data.ocorrencia_id,
            bombeiro_responsavel_id=data.bombeiro_responsavel_id,
            meio_deslocamento=data.meio_deslocamento,
...
async def obter_despacho(
    despacho_id: UUID, service: DespachoService = despacho_service_dep
) -> DespachoResponse:
    try:
...
async def listar_despachos(
    ...
    status_despacho: StatusDespacho | None = None,
    service: DespachoService = despacho_service_dep,
...
async def atualizar_status_despacho(
    despacho_id: UUID,
    data: DespachoStatusUpdate,
    service: DespachoService = despacho_service_dep,
) -> DespachoResponse:
    try:
        return await service.atualizar_status(
            despacho_id=despacho_id, status=data.status, observacoes=data.observacoes
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
