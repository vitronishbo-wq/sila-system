...
from apps.backend.app.modules.civil_protection.application.dto.ocorrencia_emergencial_schema import (
...
router = APIRouter(
    prefix="/ocorrencias-emergenciais", tags=["Protecao Civil - Ocorrencias Emergenciais"]
)

ocorrencia_emergencial_service_dep = Depends(get_ocorrencia_emergencial_service)
...
async def registrar_ocorrencia(
    data: OcorrenciaEmergencialCreate,
    service: OcorrenciaEmergencialService = ocorrencia_emergencial_service_dep,
) -> OcorrenciaEmergencialResponse:
    try:
        return await service.registrar_ocorrencia(
            ...
            bombeiro_responsavel_id=data.bombeiro_responsavel_id,
            endereco=data.endereco,
            vitimas=data.vitimas,
            desalojados=data.desalojados,
            obitos=data.obitos,
...
async def listar_ocorrencias(
    ...
    tipo_ocorrencia: TipoOcorrenciaEmergencial | None = None,
    status_ocorrencia: StatusOcorrenciaEmergencial | None = None,
    service: OcorrenciaEmergencialService = ocorrencia_emergencial_service_dep,
) -> list[OcorrenciaEmergencialResponse]:
    return await service.listar_ocorrencias(
        corporacao_id=corporacao_id, tipo=tipo_ocorrencia, status=status_ocorrencia
    )
...
async def atualizar_status_ocorrencia(
    ocorrencia_id: UUID,
    data: OcorrenciaEmergencialStatusUpdate,
    service: OcorrenciaEmergencialService = ocorrencia_emergencial_service_dep,
) -> OcorrenciaEmergencialResponse:
    try:
        return await service.atualizar_status(
            ocorrencia_id=ocorrencia_id, status=data.status, observacoes=data.observacoes
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
