...
from apps.backend.app.modules.civil_protection.api.deps import get_corporacao_service
from apps.backend.app.modules.civil_protection.application.dto.corporacao_schema import (
    CorporacaoCreate,
    CorporacaoResponse,
...
corporacao_service_dep = Depends(get_corporacao_service)


@router.post("/", response_model=CorporacaoResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_corporacao(
    data: CorporacaoCreate, service: CorporacaoService = corporacao_service_dep
) -> CorporacaoResponse:
    try:
        return await service.cadastrar_corporacao(
            nome=data.nome,
...
async def obter_corporacao(
    corporacao_id: UUID, service: CorporacaoService = corporacao_service_dep
) -> CorporacaoResponse:
    try:
...
async def listar_corporacoes(
    municipio: str | None = None,
    status_corporacao: StatusCorporacao | None = None,
    service: CorporacaoService = corporacao_service_dep,
) -> list[CorporacaoResponse]:
...
async def atualizar_status_corporacao(
    corporacao_id: UUID,
    data: CorporacaoStatusUpdate,
    service: CorporacaoService = corporacao_service_dep,
) -> CorporacaoResponse:
    try:
        return await service.atualizar_status(
            corporacao_id=corporacao_id, status=data.status, motivo=data.motivo
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
