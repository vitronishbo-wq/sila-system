...
from apps.backend.app.modules.civil_protection.infrastructure.adapters.request_service_adapter import (
...
async def get_corporacao_service(session: AsyncSession = db_dep) -> CorporacaoService:
    return CorporacaoService(
...
async def get_bombeiro_service(session: AsyncSession = db_dep) -> BombeiroService:
    corporacao_repo = SQLAlchemyCorporacaoRepository(session)
    return BombeiroService(
...
async def get_ocorrencia_emergencial_service(
    session: AsyncSession = db_dep,
) -> OcorrenciaEmergencialService:
    corporacao_repo = SQLAlchemyCorporacaoRepository(session)
    bombeiro_repo = SQLAlchemyBombeiroRepository(session)
    return OcorrenciaEmergencialService(
...
async def get_despacho_service(session: AsyncSession = db_dep) -> DespachoService:
