...
class DocumentService:
    ...
    async def create_single_document(self, file: UploadFile, metadata: DocumentCreate, owner_id):
        file_path, size_bytes, checksum = await self._save_file(file)
        document = Document(title=metadata.title, description=metadata.description, owner_id=owner_id, status=DocumentStatus.PENDING)
        self.db.add(document)
        await self.db.flush()
        version = DocumentVersion(document_id=getattr(document, 'id', None), file_path=file_path, file_size=size_bytes, checksum=checksum, content_type=file.content_type)
        self.db.add(version)
        await self.db.flush()
