...
class CulturalAssetService:
    ...
    LEVEL_ORDER = {
        ClassificationLevel.MUNICIPAL: 1,
        ClassificationLevel.PROVINCIAL: 2,
        ClassificationLevel.NATIONAL: 3,
        ClassificationLevel.UNESCO: 4,
        ClassificationLevel.WORLD_HERITAGE: 5,
    }

    def __init__(
        self,
        *,
        asset_repository: CulturalAssetRepositoryPort,
        tourism_service: TourismServicePort | None = None,
    ) -> None:
    ...
    async def register_asset(
        ...
        *,
        ...
        existing = await self.asset_repository.get_by_name_and_province(
    ...
    async def list_assets(
        ...
        *,
        ...
        return await self.asset_repository.list_by_filters(
            asset_type=asset_type,
            province=province,
        ...
        *,
        ...
        if asset.classification_level is not None:
            ...
            if next_rank < current_rank:
                raise AssetAlreadyClassifiedError(
                    "Nao e possivel reclassificar para nivel inferior"
                )
        asset.classify(
            level=classification_level, authority=authority, classification_date=classification_date
        )
        saved = await self.asset_repository.save(asset)
        if self.tourism_service and saved.has_unesco_classification:
            await self.tourism_service.promote_asset(
                asset_id=saved.asset_id,
                promotion_data={
                    "name": saved.name,
                    "classification_level": saved.classification_level.value
                    if saved.classification_level
                    else None,
                },
            )
        return saved

    async def add_preservation_action(
        self,
        *,
        asset_id: UUID,
        ...
        asset = await self.get_asset_by_id(asset_id)
        action = asset.add_preservation_action(
    ...
    async def register_cultural_event(
        self,
        *,
        asset_id: UUID,
        name: str,
        ...
        if asset.is_protected and self.tourism_service:
            await self.tourism_service.promote_asset(
                ...
                promotion_data={"event_name": name, "event_date": event_date.isoformat()},
            )
    ...
    async def get_asset_inventory(
        ...
        *,
        ...
        assets = await self.asset_repository.list_by_filters(
            province=province, classification_level=classification_level
        )
        response: list[dict] = []
        for asset in assets:
            data = asset.to_dict()
    ...
    async def get_protected_assets_report(self, *, province: str | None = None) -> dict:
        ...
        return {
            ...
            "by_classification": {
                level.value: len([a for a in assets if a.classification_level == level])
                for level in ClassificationLevel
            },
            "unesco_count": len(unesco_assets),
            "unesco_assets": [asset.to_dict() for asset in unesco_assets],
            "by_province": self._group_by_province(assets),
