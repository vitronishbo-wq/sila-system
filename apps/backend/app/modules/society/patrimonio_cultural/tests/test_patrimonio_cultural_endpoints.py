...
from apps.backend.app.modules.society.patrimonio_cultural.api.router import (
    router as patrimonio_cultural_router,
)
...
def test_patrimonio_lifecycle_e2e() -> None:
    ...
    register_payload = {
        ...
        "asset_type": "museum_collection",
        "province": "Luanda",
        "municipality": "Ingombota",
        "latitude": -8.815,
        "longitude": 13.232,
        "historical_period": "Seculo XX",
        "description": "Principal museu de antropologia de Angola",
    }
    ...
    response = client.post(
        f"/patrimonio-cultural/assets/{asset_id}/classification", json=classify_payload
    )
    ...
    action_payload = {
        ...
        "description": "Restauro de fachada principal e sistema de climatizacao",
        "executed_by": "Instituto do Patrimonio Cultural",
        "cost": 150000.0,
        "funding_source": "Orcamento do Estado",
    }
    response = client.post(
        f"/patrimonio-cultural/assets/{asset_id}/preservation-actions", json=action_payload
    )
    assert response.status_code == 201
    event_payload = {
        "name": "Exposicao Temporaria: Culturas Bantu",
    ...
    assert response.status_code == 201
    response = client.get(
        "/patrimonio-cultural/assets/inventory",
        params={"province": "Luanda", "include_events": True},
    )
    assert response.status_code == 200
