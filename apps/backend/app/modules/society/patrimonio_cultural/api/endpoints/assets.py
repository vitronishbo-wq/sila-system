...
from apps.backend.app.modules.society.patrimonio_cultural.api.schemas import (
...
cultural_asset_service_dep = Depends(get_cultural_asset_service)
asset_status_query = Query(default=None, alias="status")
...
async def register_cultural_asset(
    payload: CulturalAssetCreateSchema,
    service: CulturalAssetService = cultural_asset_service_dep,
) -> dict:
    try:
...
async def list_cultural_assets(
    asset_type: AssetType | None = None,
    province: str | None = None,
    classification_level: ClassificationLevel | None = None,
    asset_status: AssetStatus | None = asset_status_query,
...
async def get_cultural_inventory(
    province: str | None = None,
    classification_level: ClassificationLevel | None = None,
    ...
    return await service.get_asset_inventory(
        province=province,
        classification_level=classification_level,
...
async def get_protected_assets_report(
    province: str | None = None, service: CulturalAssetService = cultural_asset_service_dep
) -> dict:
...
async def get_cultural_asset(
    asset_id: UUID, service: CulturalAssetService = cultural_asset_service_dep
) -> dict:
    try:
...
async def classify_cultural_asset(
    asset_id: UUID,
    payload: ClassificationRequestSchema,
    service: CulturalAssetService = cultural_asset_service_dep,
) -> dict:
    try:
        asset = await service.classify_asset(
            asset_id=asset_id,
            classification_level=payload.classification_level,
            authority=payload.authority,
            classification_date=payload.classification_date,
        )
        return asset.to_dict()
    except AssetNotFoundError as exc:
        _raise_404(exc)
...
async def add_preservation_action(
    asset_id: UUID,
    payload: PreservationActionSchema,
    service: CulturalAssetService = cultural_asset_service_dep,
) -> dict:
    try:
        action = await service.add_preservation_action(
            asset_id=asset_id,
...
async def register_cultural_event(
    asset_id: UUID,
    payload: CulturalEventSchema,
    service: CulturalAssetService = cultural_asset_service_dep,
) -> dict:
    try:
        event = await service.register_cultural_event(
            asset_id=asset_id,
            name=payload.name,
