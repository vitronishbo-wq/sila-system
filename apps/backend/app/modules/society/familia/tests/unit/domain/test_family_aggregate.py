...
def test_create_aggregate_with_valid_head() -> None:
    ...
    aggregate = FamilyAggregate(
        family_id=uuid4(), head_citizen_id=head_id, code="FAM-2026-000001", head_age=30
    )
...
def test_create_aggregate_with_minor_head_raises_error() -> None:
    with pytest.raises(HeadMustBeAdultError):
        FamilyAggregate(
            family_id=uuid4(), head_citizen_id=uuid4(), code="FAM-2026-000001", head_age=16
        )


def test_add_member_to_active_aggregate() -> None:
    aggregate = FamilyAggregate(
        family_id=uuid4(), head_citizen_id=uuid4(), code="FAM-2026-000001", head_age=40
    )
    member_id = uuid4()
