...
class FakeCitizenService:
    ...
    async def verify_civil_capacity(self, citizen_id):
        ...
        return (
            ...
            and (str(data.get("vital_status", "alive")).lower() in {"alive", "active"})
        )
...
async def test_family_lifecycle_e2e():
    ...
    child_id = uuid4()
    citizens = {
        head_id: {"full_name": "Joao Silva", "age": 35, "is_active": True, "vital_status": "alive"},
        spouse_id: {
    ...
    async def _get_aggregate_service():
        return FamilyAggregateService(
            ...
            civil_registry_service=civil_registry_service,
        )

    async def _get_query_service():
        return FamilyQueryService(
            repository=repo,
            projection_repository=NoopProjectionRepository(),
            citizen_service=citizen_service,
        )
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        create_payload = {
            "head_citizen_id": str(head_id),
            "members": [{"citizen_id": str(spouse_id), "role": "MEMBER"}],
            "metadata": {"jurisdiction_code": "AO-LUA"},
        }
        ...
        resp = await ac.post(
            f"/v1/familia/families/{family_id}/members",
            json={"citizen_id": str(child_id), "role": "DEPENDENT"},
        )
        ...
        assert deps["count"] == 1
        resp = await ac.put(
            f"/v1/familia/families/{family_id}/transfer-head",
            json={"new_head_citizen_id": str(spouse_id)},
        )
        assert resp.status_code == 200, resp.text
        transferred = resp.json()
        assert transferred["head_citizen_id"] == str(spouse_id)
        resp = await ac.request(
            "DELETE", f"/v1/familia/families/{family_id}", json={"reason": "divorce"}
        )
        assert resp.status_code == 200, resp.text
        dissolved = resp.json()
