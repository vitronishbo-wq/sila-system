...
from apps.backend.app.modules.society.familia.application.ports.outbox_repository_port import (
...
class FamilyAggregateService:
    def __init__(
        self,
        *,
        repository: FamilyAggregateRepositoryPort,
        outbox_repository: OutboxRepositoryPort,
        event_bus: EventBusPort,
        citizen_service: CitizenServicePort | None = None,
        civil_registry_service: CivilRegistryServicePort | None = None,
    ) -> None:
    ...
    async def create_aggregate(
        self, *, head_citizen_id: UUID, initial_members: list, metadata: dict
    ):
        ...
        if self._civil_registry_service and await self._civil_registry_service.is_deceased(
            head_citizen_id
        ):
            raise ValueError("Nao e possivel criar agregado para cidadao com obito registrado")
        await OneActiveFamilyRule().validate(
            citizen_id=head_citizen_id, family_repository=self._repository, exclude_family_id=None
        )
        ...
        aggregate = FamilyAggregate(
            family_id=uuid4(), code=code, head_citizen_id=head_citizen_id, metadata=metadata
        )
        for member in initial_members:
            ...
            await OneActiveFamilyRule().validate(
                ...
                family_repository=self._repository,
                exclude_family_id=aggregate.id,
            )
    ...
    async def add_member_to_aggregate(self, *, family_id: UUID, citizen_id: UUID, role: MemberRole):
        ...
        if not aggregate:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Agregado nao encontrado"
            )
        if self._citizen_service:
        ...
        await OneActiveFamilyRule().validate(
            citizen_id=citizen_id, family_repository=self._repository, exclude_family_id=family_id
        )
