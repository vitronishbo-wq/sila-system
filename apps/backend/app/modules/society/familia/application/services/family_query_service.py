...
from apps.backend.app.modules.society.familia.application.ports.projection_repository_port import (
...
class FamilyQueryService:
    def __init__(
        ...
        *,
        ...
        projection_repository: ProjectionRepositoryPort,
    ...
    async def get_active_family_for_citizen(self, citizen_id):
        ...
        if not family_id:
            raise HTTPException(
                ...
                detail="Nenhuma familia ativa encontrada para o cidadao",
            )
        return await self.get_aggregate(family_id)

    async def list_families_for_citizen(
        self, citizen_id, *, limit: int = 100, offset: int = 0
    ) -> list[dict]:
        family_ids = await self._repository.list_family_ids_for_citizen(
            citizen_id, limit=limit, offset=offset
        )
        results: list[dict] = []
    ...
    async def get_dependents_for_head(self, head_citizen_id):
        ...
        return {
            "citizen_id": head_citizen_id,
            "dependents": tree["dependents"],
            "count": len(tree["dependents"]),
        }
