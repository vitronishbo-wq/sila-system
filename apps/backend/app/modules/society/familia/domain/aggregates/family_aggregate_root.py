...
class FamilyAggregate:
    def __init__(
        ...
        *,
        ...
        code: str | None = None,
        head_citizen_id: UUID | None = None,
        head_age: int = 18,
    ...
    def add_member(
        self, *, citizen_id: UUID, role: MemberRole, joined_at: datetime | None = None
    ) -> None:
        ...
        self._pending_events.append(
            FamilyMemberAddedEvent(
                aggregate_id=self.id, citizen_id=citizen_id, role=role, occurred_at=self._updated_at
            )
        )
    ...
    def remove_member(self, *, citizen_id: UUID, reason: str | None = None) -> None:
        ...
        if citizen_id == self._head_citizen_id:
            raise ValueError(
                "Nao e possivel remover o chefe do agregado; use dissolve() ou transfer_head()"
            )
        member = next(
            (m for m in self._members if m.citizen_id == citizen_id and m.left_at is None), None
        )
        if member is None:
    ...
    def transfer_head(self, *, new_head_citizen_id: UUID) -> None:
        ...
        if self._head_citizen_id is None:
            raise ValueError("Aggregate sem chefe definido")
        target = next(
            (m for m in self._members if m.citizen_id == new_head_citizen_id and m.left_at is None),
            None,
        )
        if target is None:
        ...
        self._pending_events.append(
            FamilyHeadTransferredEvent(
                aggregate_id=self.id,
                old_head_citizen_id=old_head,
                new_head_citizen_id=new_head_citizen_id,
                occurred_at=self._updated_at,
            )
        )
        ...
        self._pending_events.append(
            FamilyDissolvedEvent(
                payload={
                    ...
                    "head_citizen_id": str(self.head_citizen_id),
                    "reason": reason,
                    "occurred_at": self._updated_at.isoformat(),
                }
            )
        )
    ...
    def reconstitute(cls, *, state: dict, members: list[FamilyMember]) -> FamilyAggregate:
        aggregate = cls(
            family_id=state["id"],
            code=state["code"],
            head_citizen_id=state["head_citizen_id"],
            status=state["status"],
