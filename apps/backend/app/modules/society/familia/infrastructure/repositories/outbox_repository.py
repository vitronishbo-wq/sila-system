        ...
        for event in events:
            ...
            if aggregate_uuid not in versions_cache:
                current = await self._session.execute(
                    select(func.max(FamilyOutboxEventModel.event_version)).where(
                        FamilyOutboxEventModel.aggregate_id == aggregate_uuid
                    )
                )
            ...
            self._session.add(
                FamilyOutboxEventModel(
                    ...
                    event_version=versions_cache[aggregate_uuid],
