...
from apps.backend.app.modules.society.familia.application.ports.family_aggregate_repository_port import (
    FamilyAggregateRepositoryPort,
)
from apps.backend.app.modules.society.familia.domain.aggregates.family_aggregate_root import (
    FamilyAggregate,
)
...
class SQLAlchemyFamilyAggregateRepository(FamilyAggregateRepositoryPort):
    ...
    async def save(self, aggregate: FamilyAggregate) -> None:
        ...
        if model is None:
            model = FamilyAggregateModel(
                id=aggregate.id,
                code=aggregate.code,
                head_citizen_id=aggregate.head_citizen_id,
        ...
        existing_rows = await self._session.execute(
            select(FamilyMemberModel).where(FamilyMemberModel.family_id == aggregate.id)
        )
        by_citizen_id: dict[UUID, FamilyMemberModel] = {
            row.citizen_id: row for row in existing_rows.scalars().all()
        }
    ...
    async def get_by_id(self, family_id: UUID) -> FamilyAggregate | None:
        ...
        return FamilyAggregate.reconstitute(
            state={
                "id": model.id,
                "code": model.code,
                "head_citizen_id": model.head_citizen_id,
                "status": FamilyStatus(model.status),
    ...
    async def find_active_family_for_citizen(
        self, citizen_id: UUID, exclude_family_id: UUID | None = None
    ) -> UUID | None:
        stmt = (
            select(FamilyAggregateModel.id)
            .join(FamilyMemberModel, FamilyMemberModel.family_id == FamilyAggregateModel.id)
            .where(
                FamilyMemberModel.citizen_id == citizen_id,
                FamilyMemberModel.left_at.is_(None),
                FamilyAggregateModel.status == FamilyStatus.ACTIVE.value,
            )
        )
    ...
    async def find_active_family_by_head(self, head_citizen_id: UUID) -> UUID | None:
        stmt = select(FamilyAggregateModel.id).where(
            FamilyAggregateModel.head_citizen_id == head_citizen_id,
            FamilyAggregateModel.status == FamilyStatus.ACTIVE.value,
        )
        result = await self._session.execute(stmt.limit(1))
        return result.scalar_one_or_none()

    async def list_family_ids_for_citizen(
        self, citizen_id: UUID, *, limit: int = 100, offset: int = 0
    ) -> list[UUID]:
        stmt = (
            select(FamilyAggregateModel.id)
            .join(FamilyMemberModel, FamilyMemberModel.family_id == FamilyAggregateModel.id)
            .where(FamilyMemberModel.citizen_id == citizen_id)
            .order_by(FamilyAggregateModel.created_at.desc())
            .limit(limit)
            .offset(offset)
        )
    ...
    async def get_next_sequence(self, year: int) -> int:
        result = await self._session.execute(
            select(func.count(FamilyAggregateModel.id)).where(
                FamilyAggregateModel.code.like(f"FAM-{year}-%")
            )
        )
