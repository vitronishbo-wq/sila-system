...
class RedisProjectionRepository(ProjectionRepositoryPort):
    ...
    async def get_family_composition(self, family_id: UUID) -> dict | None:
        ...
        return {
            "id": model.family_id,
            "code": model.composition_json.get("code", ""),
            "head_citizen_id": model.head_citizen_id,
            "status": model.composition_json.get("status", "ACTIVE"),
            "member_count": model.member_count,
            "created_at": model.composition_json.get("created_at"),
    ...
    async def upsert_family_composition(self, payload: dict) -> None:
        ...
        model = await self._session.get(FamilyCompositionViewModel, family_id)
        if model is None:
            model = FamilyCompositionViewModel(
                family_id=family_id,
                ...
                active_relationships=int(payload.get("active_relationships", 0)),
                composition_json=payload,
            )
