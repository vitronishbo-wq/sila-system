...
from apps.backend.app.modules.society.familia.api.deps import (
    get_family_aggregate_service,
    get_family_query_service,
)
from apps.backend.app.modules.society.familia.api.schemas.family_aggregate_schema import (
    FamilyAggregateCreateSchema,
    FamilyAggregateResponseSchema,
    FamilyDissolveSchema,
    FamilyMemberCreateSchema,
    FamilyTransferHeadSchema,
...
family_aggregate_service_dep = Depends(get_family_aggregate_service)
family_query_service_dep = Depends(get_family_query_service)
...
async def create_family_aggregate(
    payload: FamilyAggregateCreateSchema,
    service: FamilyAggregateService = family_aggregate_service_dep,
) -> FamilyAggregateResponseSchema:
    try:
        return await service.create_aggregate(
            head_citizen_id=payload.head_citizen_id,
            initial_members=payload.members,
...
async def list_families_for_citizen(
    citizen_id: UUID,
    limit: int = limit_query,
    offset: int = offset_query,
    service: FamilyQueryService = family_query_service_dep,
) -> list[FamilyAggregateResponseSchema]:
    try:
...
async def get_active_family_for_citizen(
    citizen_id: UUID, service: FamilyQueryService = family_query_service_dep
) -> FamilyAggregateResponseSchema:
    try:
...
async def get_dependents_for_citizen(
    citizen_id: UUID, service: FamilyQueryService = family_query_service_dep
) -> dict:
    try:
...
async def get_family_aggregate(
    family_id: UUID, service: FamilyQueryService = family_query_service_dep
) -> FamilyAggregateResponseSchema:
    try:
...
async def add_family_member(
    ...
    try:
        return await service.add_member_to_aggregate(
            family_id=family_id, citizen_id=payload.citizen_id, role=payload.role
        )
    except Exception as exc:
        raise to_http_error(exc) from exc
...
async def transfer_family_head(
    family_id: UUID,
    payload: FamilyTransferHeadSchema,
    service: FamilyAggregateService = family_aggregate_service_dep,
) -> FamilyAggregateResponseSchema:
    try:
        return await service.transfer_head(
            family_id=family_id, new_head_citizen_id=payload.new_head_citizen_id
        )
    except Exception as exc:
        raise to_http_error(exc) from exc
...
async def dissolve_family_aggregate(
    family_id: UUID,
    payload: FamilyDissolveSchema,
    service: FamilyAggregateService = family_aggregate_service_dep,
) -> FamilyAggregateResponseSchema:
    try:
