...
class AuxilioService:
    def __init__(
        self,
        *,
        auxilio_repo: AuxilioRepositoryPort,
    ...
    async def conceder_auxilio(
        ...
        *,
        ...
        auxilio = Auxilio.conceder(
            codigo_auxilio=codigo,
            jovem_id=jovem_id,
            tipo=tipo,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CONCESSAO_AUXILIO_JUVENTUDE",
                entity_id=saved.id,
                citizen_id=jovem.citizen_id,
                numero_processo=saved.codigo_auxilio,
                metadata={
                    "codigo_auxilio": saved.codigo_auxilio,
                    "jovem_id": str(jovem_id),
    ...
    async def listar_auxilios(
        ...
        *,
        ...
        status: StatusBeneficio | None = None,
    ) -> list[Auxilio]:
    ...
    async def atualizar_status(
        self, *, auxilio_id: UUID, status: StatusBeneficio, observacoes: str | None = None
    ) -> Auxilio:
