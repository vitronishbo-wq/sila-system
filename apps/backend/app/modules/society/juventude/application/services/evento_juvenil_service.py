...
class EventoJuvenilService:
    ...
    def __init__(self, *, evento_repo: EventoJuvenilRepositoryPort, jovem_repo: JovemRepositoryPort) -> None:
        self.evento_repo = evento_repo
        self.jovem_repo = jovem_repo

    async def criar_evento(self, *, titulo: str, tipo_evento: TipoEvento, area_interesse: AreaInteresse, data_evento: date, local: str, municipio: str, provincia: str, vagas: int | None=None, observacoes: str | None=None) -> EventoJuvenil:
        codigo = await self.evento_repo.next_codigo()
        item = EventoJuvenil.criar(codigo_evento=codigo, titulo=titulo, tipo_evento=tipo_evento, area_interesse=area_interesse, data_evento=data_evento, local=local, municipio=municipio, provincia=provincia, vagas=vagas, observacoes=observacoes)
