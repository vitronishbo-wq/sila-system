...
class PoliticaJuventudeService:
    ...
    async def criar_politica(self, *, nome: str, descricao: str, area_interesse: AreaInteresse, data_inicio: date, metas: dict[str, float] | None=None, indicadores: list[str] | None=None, data_fim: date | None=None, observacoes: str | None=None) -> PoliticaJuventude:
        codigo = await self.politica_repo.next_codigo()
        item = PoliticaJuventude.criar(codigo_politica=codigo, nome=nome, descricao=descricao, area_interesse=area_interesse, data_inicio=data_inicio, metas=metas, indicadores=indicadores, data_fim=data_fim, observacoes=observacoes)
    ...
    async def listar_politicas(self, *, status: StatusPoliticaJuventude | None=None, area: AreaInteresse | None=None) -> list[PoliticaJuventude]:
    ...
    async def atualizar_status(self, *, politica_id: UUID, status: StatusPoliticaJuventude) -> PoliticaJuventude:
