...
class InscricaoProgramaService:
    ...
    def __init__(self, *, inscricao_repo: InscricaoProgramaRepositoryPort, jovem_repo: JovemRepositoryPort, programa_repo: ProgramaRepositoryPort, request_service: RequestServicePort | None=None) -> None:
        self.inscricao_repo = inscricao_repo
        self.jovem_repo = jovem_repo
        self.programa_repo = programa_repo
        self.request_service = request_service

    async def inscrever_jovem(self, *, programa_id: UUID, jovem_id: UUID, prioridade: int=0, observacoes: str | None=None) -> InscricaoPrograma:
        ...
        inscricao = InscricaoPrograma.inscrever(codigo_inscricao=codigo, programa_id=programa_id, jovem_id=jovem_id, prioridade=prioridade, observacoes=observacoes)
        saved = await self.inscricao_repo.save(inscricao)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='INSCRICAO_PROGRAMA_JUVENTUDE', entity_id=saved.id, citizen_id=jovem.citizen_id, numero_processo=saved.codigo_inscricao, metadata={'programa_id': str(saved.programa_id), 'prioridade': saved.prioridade, 'status': saved.status.value})
        return saved
    ...
    async def listar_inscricoes(self, *, jovem_id: UUID | None=None, programa_id: UUID | None=None, status: StatusInscricao | None=None) -> list[InscricaoPrograma]:
    ...
    async def cancelar_inscricao(self, inscricao_id: UUID, motivo: str | None=None) -> InscricaoPrograma:
        item = await self.buscar_inscricao(inscricao_id)
