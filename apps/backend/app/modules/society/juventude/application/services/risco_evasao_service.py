...
class RiscoEvasaoService:
    def __init__(
        self,
        *,
        risco_repo: RiscoEvasaoRepositoryPort,
    ...
    async def avaliar_risco(self, *, jovem_id: UUID, observacoes: str | None = None) -> RiscoEvasao:
        ...
        risco = RiscoEvasao.avaliar(
            codigo_risco=codigo,
            jovem_id=jovem.id,
            citizen_id=jovem.citizen_id,
            matricula_ativa=matricula_ativa,
            situacao_ocupacional=jovem.situacao_ocupacional,
            vulnerabilidades=jovem.vulnerabilidades,
            observacoes=observacoes,
        )
        saved = await self.risco_repo.save(risco)
        if (
            self.request_service is not None
            and saved.citizen_id is not None
            and (saved.nivel_risco in {RiscoSocial.ALTO, RiscoSocial.CRITICO})
        ):
            await self.request_service.create_request(
                request_type="RISCO_EVASAO_JUVENTUDE",
                entity_id=saved.id,
                citizen_id=saved.citizen_id,
                numero_processo=saved.codigo_risco,
                metadata={
                    "jovem_id": str(saved.jovem_id),
                    "pontuacao": saved.pontuacao,
                    "nivel_risco": saved.nivel_risco.value,
                    "fatores": saved.fatores,
                },
            )
        return saved
