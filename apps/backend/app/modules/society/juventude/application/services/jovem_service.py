...
class JovemService:
    ...
    def __init__(self, *, jovem_repo: JovemRepositoryPort, citizen_service: CitizenServicePort | None=None, educacao_service: EducacaoServicePort | None=None, emprego_service: EmpregoServicePort | None=None, request_service: RequestServicePort | None=None) -> None:
    ...
    async def cadastrar_jovem(self, *, nome: str, data_nascimento: date, genero: str, naturalidade: str, escolaridade: Escolaridade, situacao_ocupacional: SituacaoOcupacional, endereco: str, municipio: str, provincia: str, nacionalidade: str='Angolana', telefone: str | None=None, email: str | None=None, citizen_id: UUID | None=None, observacoes: str | None=None) -> Jovem:
        ...
        if citizen_id is not None and self.educacao_service is not None and (situacao_ocupacional in {SituacaoOcupacional.ESTUDA, SituacaoOcupacional.ESTUDA_TRABALHA}):
        ...
        if citizen_id is not None and self.emprego_service is not None and (situacao_ocupacional in {SituacaoOcupacional.TRABALHA, SituacaoOcupacional.PROCURA_EMPREGO, SituacaoOcupacional.ESTUDA_TRABALHA}):
            await self.emprego_service.has_candidatura_ativa(citizen_id)
        registro = await self.jovem_repo.next_registro()
        jovem = Jovem.cadastrar(numero_registro=registro, nome=nome, data_nascimento=data_nascimento, genero=genero, naturalidade=naturalidade, escolaridade=escolaridade, situacao_ocupacional=situacao_ocupacional, endereco=endereco, municipio=municipio, provincia=provincia, nacionalidade=nacionalidade, telefone=telefone, email=email, citizen_id=citizen_id, observacoes=observacoes)
        saved = await self.jovem_repo.save(jovem)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='CADASTRO_JOVEM', entity_id=saved.id, citizen_id=citizen_id, numero_processo=saved.numero_registro, metadata={'registro': saved.numero_registro, 'nome': saved.nome, 'faixa_etaria': saved.faixa_etaria.value, 'escolaridade': saved.escolaridade.value, 'situacao_ocupacional': saved.situacao_ocupacional.value})
        return saved
    ...
    async def listar_jovens(self, *, faixa_etaria: FaixaEtaria | None=None, escolaridade: Escolaridade | None=None, situacao: SituacaoOcupacional | None=None, municipio: str | None=None, vulneravel: bool | None=None) -> list[Jovem]:
    ...
    async def atualizar_jovem(self, *, jovem_id: UUID, escolaridade: Escolaridade | None=None, situacao_ocupacional: SituacaoOcupacional | None=None, telefone: str | None=None, email: str | None=None, endereco: str | None=None, municipio: str | None=None, provincia: str | None=None, observacoes: str | None=None, ativo: bool | None=None) -> Jovem:
    ...
    async def adicionar_vulnerabilidade(self, *, jovem_id: UUID, vulnerabilidade: TipoVulnerabilidade) -> Jovem:
        jovem = await self.buscar_jovem(jovem_id)
