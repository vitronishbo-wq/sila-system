...
class BolsaEstudoService:
    def __init__(
        self,
        *,
        bolsa_repo: BolsaEstudoRepositoryPort,
        ...
        *,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CONCESSAO_BOLSA_JUVENTUDE",
                entity_id=saved.id,
                citizen_id=jovem.citizen_id,
                numero_processo=saved.codigo_bolsa,
                metadata={
                    "codigo_bolsa": saved.codigo_bolsa,
    ...
    async def listar_bolsas(
        self, *, jovem_id: UUID | None = None, apenas_ativas: bool | None = None
    ) -> list[BolsaEstudo]:
    ...
    async def encerrar_bolsa(
        self, *, bolsa_id: UUID, observacoes: str | None = None
    ) -> BolsaEstudo:
