...
class ProgramaService:
    ...
    def __init__(self, *, programa_repo: ProgramaRepositoryPort, request_service: RequestServicePort | None=None) -> None:
        self.programa_repo = programa_repo
        self.request_service = request_service

    async def criar_programa(self, *, nome: str, tipo: TipoPrograma, data_inicio: date, data_fim: date | None=None, vagas: int | None=None, municipio: str | None=None, provincia: str | None=None, observacoes: str | None=None) -> ProgramaJuvenil:
        codigo = await self.programa_repo.next_codigo()
        programa = ProgramaJuvenil.criar(codigo_programa=codigo, nome=nome, tipo=tipo, data_inicio=data_inicio, data_fim=data_fim, vagas=vagas, municipio=municipio, provincia=provincia, observacoes=observacoes)
        saved = await self.programa_repo.save(programa)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='CADASTRO_PROGRAMA_JUVENTUDE', entity_id=saved.id, numero_processo=saved.codigo_programa, metadata={'codigo_programa': saved.codigo_programa, 'nome': saved.nome, 'tipo': saved.tipo.value})
        return saved
    ...
    async def listar_programas(self, *, tipo: TipoPrograma | None=None, status: StatusPrograma | None=None) -> list[ProgramaJuvenil]:
        if tipo is not None:
    ...
    async def atualizar_status(self, *, programa_id: UUID, status: StatusPrograma) -> ProgramaJuvenil:
