...
class EstagioJuvenilModel(Base):
    __tablename__ = 'juventude_estagios'
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    codigo_estagio: Mapped[str] = mapped_column(String(50), unique=True, nullable=False, index=True)
    jovem_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True)
    instituicao: Mapped[str] = mapped_column(String(200), nullable=False, index=True)
    ...
    bolsa_auxilio: Mapped[float | None] = mapped_column(Numeric(12, 2), nullable=True)
