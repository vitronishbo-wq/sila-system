...
from apps.backend.app.modules.society.juventude.infrastructure.models.inscricao_programa_model import (
    InscricaoProgramaModel,
)
...
class SQLAlchemyInscricaoProgramaRepository(InscricaoProgramaRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_inscricao: str) -> InscricaoPrograma | None:
        stmt = select(InscricaoProgramaModel).where(
            InscricaoProgramaModel.codigo_inscricao == codigo_inscricao.strip()
        )
    ...
    async def list_by_jovem(self, jovem_id: UUID) -> list[InscricaoPrograma]:
        stmt = (
            select(InscricaoProgramaModel)
            .where(InscricaoProgramaModel.jovem_id == jovem_id)
            .order_by(InscricaoProgramaModel.data_cadastro.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(i) for i in rows]
    ...
    async def list_by_programa(self, programa_id: UUID) -> list[InscricaoPrograma]:
        stmt = (
            select(InscricaoProgramaModel)
            .where(InscricaoProgramaModel.programa_id == programa_id)
            .order_by(InscricaoProgramaModel.data_cadastro.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(i) for i in rows]
    ...
    async def list_by_status(self, status: StatusInscricao) -> list[InscricaoPrograma]:
        stmt = (
            select(InscricaoProgramaModel)
            .where(InscricaoProgramaModel.status == status.value)
            .order_by(InscricaoProgramaModel.data_cadastro.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(i) for i in rows]
    ...
    async def exists_active_by_jovem_programa(self, jovem_id: UUID, programa_id: UUID) -> bool:
        stmt = select(InscricaoProgramaModel.id).where(
            and_(
                InscricaoProgramaModel.jovem_id == jovem_id,
                InscricaoProgramaModel.programa_id == programa_id,
                InscricaoProgramaModel.status.in_(
                    [StatusInscricao.PENDENTE.value, StatusInscricao.CONFIRMADA.value]
                ),
            )
        )
    ...
    async def count_confirmadas_by_programa(self, programa_id: UUID) -> int:
        stmt = (
            select(func.count())
            .select_from(InscricaoProgramaModel)
            .where(
                InscricaoProgramaModel.programa_id == programa_id,
                InscricaoProgramaModel.status == StatusInscricao.CONFIRMADA.value,
            )
        )
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(InscricaoProgramaModel)
            .where(InscricaoProgramaModel.codigo_inscricao.like(f"INS/{ano}/%"))
        )
