...
class SQLAlchemyFormacaoRepository(FormacaoRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_formacao: str) -> FormacaoJuvenil | None:
        stmt = select(FormacaoJuvenilModel).where(FormacaoJuvenilModel.codigo_formacao == codigo_formacao.strip())
    ...
    async def list_by_jovem(self, jovem_id: UUID) -> list[FormacaoJuvenil]:
        stmt = select(FormacaoJuvenilModel).where(FormacaoJuvenilModel.jovem_id == jovem_id).order_by(FormacaoJuvenilModel.data_cadastro.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]

    async def list_by_programa(self, programa_id: UUID) -> list[FormacaoJuvenil]:
        stmt = select(FormacaoJuvenilModel).where(FormacaoJuvenilModel.programa_id == programa_id).order_by(FormacaoJuvenilModel.data_cadastro.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]

    async def list_by_status(self, status: StatusFormacao) -> list[FormacaoJuvenil]:
        stmt = select(FormacaoJuvenilModel).where(FormacaoJuvenilModel.status == status.value).order_by(FormacaoJuvenilModel.data_cadastro.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ano = date.today().year
        stmt = select(func.count()).select_from(FormacaoJuvenilModel).where(FormacaoJuvenilModel.codigo_formacao.like(f'FRM/{ano}/%'))
        count = (await self.session.execute(stmt)).scalar() or 0
        return f'FRM/{ano}/{count + 1:05d}'

    @staticmethod
    def _to_domain(model: FormacaoJuvenilModel) -> FormacaoJuvenil:
        return FormacaoJuvenil(id=model.id, codigo_formacao=model.codigo_formacao, jovem_id=model.jovem_id, programa_id=model.programa_id, nome_curso=model.nome_curso, instituicao=model.instituicao, carga_horaria=model.carga_horaria, data_inicio=model.data_inicio, data_fim=model.data_fim, certificado_emitido=model.certificado_emitido, status=StatusFormacao(model.status), data_cadastro=model.data_cadastro, observacoes=model.observacoes, ativo=model.ativo)