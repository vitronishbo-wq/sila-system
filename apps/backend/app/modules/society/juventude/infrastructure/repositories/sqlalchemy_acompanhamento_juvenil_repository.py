...
from apps.backend.app.modules.society.juventude.domain.models.acompanhamento_juvenil import (
    AcompanhamentoJuvenil,
)
from apps.backend.app.modules.society.juventude.infrastructure.models.acompanhamento_juvenil_model import (
    AcompanhamentoJuvenilModel,
)
...
class SQLAlchemyAcompanhamentoJuvenilRepository(AcompanhamentoJuvenilRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_acompanhamento: str) -> AcompanhamentoJuvenil | None:
        stmt = select(AcompanhamentoJuvenilModel).where(
            AcompanhamentoJuvenilModel.codigo_acompanhamento == codigo_acompanhamento.strip()
        )
        model = (await self.session.execute(stmt)).scalars().first()
        return self._to_domain(model) if model else None

    async def list_all(self) -> list[AcompanhamentoJuvenil]:
        stmt = select(AcompanhamentoJuvenilModel).order_by(
            AcompanhamentoJuvenilModel.data_registo.desc()
        )
    ...
    async def list_by_jovem(self, jovem_id: UUID) -> list[AcompanhamentoJuvenil]:
        stmt = (
            select(AcompanhamentoJuvenilModel)
            .where(AcompanhamentoJuvenilModel.jovem_id == jovem_id)
            .order_by(AcompanhamentoJuvenilModel.data_registo.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(i) for i in rows]
    ...
    async def list_by_status(self, status: StatusAcompanhamento) -> list[AcompanhamentoJuvenil]:
        stmt = (
            select(AcompanhamentoJuvenilModel)
            .where(AcompanhamentoJuvenilModel.status == status.value)
            .order_by(AcompanhamentoJuvenilModel.data_registo.desc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(i) for i in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(AcompanhamentoJuvenilModel)
            .where(AcompanhamentoJuvenilModel.codigo_acompanhamento.like(f"ACP/{ano}/%"))
        )
    ...
    def _to_domain(model: AcompanhamentoJuvenilModel) -> AcompanhamentoJuvenil:
        return AcompanhamentoJuvenil(
            id=model.id,
            codigo_acompanhamento=model.codigo_acompanhamento,
            ...
            proxima_revisao=model.proxima_revisao,
