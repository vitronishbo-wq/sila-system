...
class SQLAlchemyAuxilioRepository(AuxilioRepositoryPort):
    ...
    async def list_by_jovem(self, jovem_id: UUID) -> list[Auxilio]:
        stmt = select(AuxilioModel).where(AuxilioModel.jovem_id == jovem_id).order_by(AuxilioModel.data_cadastro.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]

    async def list_by_tipo(self, tipo: TipoAuxilio) -> list[Auxilio]:
        stmt = select(AuxilioModel).where(AuxilioModel.tipo == tipo.value).order_by(AuxilioModel.data_cadastro.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]

    async def list_by_status(self, status: StatusBeneficio) -> list[Auxilio]:
        stmt = select(AuxilioModel).where(AuxilioModel.status == status.value).order_by(AuxilioModel.data_cadastro.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ano = date.today().year
        stmt = select(func.count()).select_from(AuxilioModel).where(AuxilioModel.codigo_auxilio.like(f'AUX/{ano}/%'))
        count = (await self.session.execute(stmt)).scalar() or 0
        return f'AUX/{ano}/{count + 1:05d}'

    @staticmethod
    def _to_domain(model: AuxilioModel) -> Auxilio:
        return Auxilio(id=model.id, codigo_auxilio=model.codigo_auxilio, jovem_id=model.jovem_id, tipo=TipoAuxilio(model.tipo), data_inicio=model.data_inicio, data_fim=model.data_fim, valor_mensal=model.valor_mensal, status=StatusBeneficio(model.status), data_cadastro=model.data_cadastro, observacoes=model.observacoes, ativo=model.ativo)