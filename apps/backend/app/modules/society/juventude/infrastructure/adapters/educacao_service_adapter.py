...
class _MatriculaRepository(Protocol):
    ...
    async def get_by_citizen(self, citizen_id: UUID, ano_letivo_id: UUID | None=None) -> Sequence[_MatriculaSnapshot]:
        ...
...
class EducacaoServiceAdapter(EducacaoServicePort):
    ...
    async def has_matricula_ativa(self, citizen_id: UUID) -> bool:
        ...
        return any((item.status in {StatusMatricula.ATIVA, StatusMatricula.PENDENTE, StatusMatricula.ATIVA.value, StatusMatricula.PENDENTE.value} for item in matriculas))