...
class IntercambioJuvenil:
    ...
    @classmethod
    def solicitar(cls, *, codigo_intercambio: str, jovem_id: UUID, pais_destino: str, instituicao_destino: str, area_interesse: AreaInteresse, data_inicio: date, data_fim: date | None=None, observacoes: str | None=None) -> 'IntercambioJuvenil':
        ...
        return cls(id=uuid4(), codigo_intercambio=codigo_intercambio.strip(), jovem_id=jovem_id, pais_destino=pais_destino.strip(), instituicao_destino=instituicao_destino.strip(), area_interesse=area_interesse, data_inicio=data_inicio, data_fim=data_fim, data_cadastro=date.today(), observacoes=observacoes.strip() if observacoes else None, status=StatusIntercambio.SOLICITADO, ativo=True)
