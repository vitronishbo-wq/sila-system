...
class Auxilio:
    ...
    @classmethod
    def conceder(cls, *, codigo_auxilio: str, jovem_id: UUID, tipo: TipoAuxilio, data_inicio: date, valor_mensal: Decimal | None=None, data_fim: date | None=None, observacoes: str | None=None) -> 'Auxilio':
        ...
        return cls(id=uuid4(), codigo_auxilio=codigo_auxilio.strip(), jovem_id=jovem_id, tipo=tipo, data_inicio=data_inicio, status=StatusBeneficio.ATIVO, data_cadastro=date.today(), data_fim=data_fim, valor_mensal=valor_mensal, observacoes=observacoes.strip() if observacoes else None, ativo=True)
