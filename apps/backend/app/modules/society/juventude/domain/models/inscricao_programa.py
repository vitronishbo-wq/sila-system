...
class InscricaoPrograma:
    ...
    @classmethod
    def inscrever(cls, *, codigo_inscricao: str, programa_id: UUID, jovem_id: UUID, prioridade: int=0, observacoes: str | None=None) -> 'InscricaoPrograma':
        if prioridade < 0:
            raise ValueError('Prioridade da inscricao nao pode ser negativa')
        return cls(id=uuid4(), codigo_inscricao=codigo_inscricao.strip(), programa_id=programa_id, jovem_id=jovem_id, data_inscricao=date.today(), status=StatusInscricao.PENDENTE, data_cadastro=date.today(), prioridade=prioridade, observacoes=observacoes.strip() if observacoes else None, ativo=True)
