...
class ProgramaJuvenil:
    ...
    @classmethod
    def criar(cls, *, codigo_programa: str, nome: str, tipo: TipoPrograma, data_inicio: date, data_fim: date | None=None, vagas: int | None=None, municipio: str | None=None, provincia: str | None=None, observacoes: str | None=None) -> 'ProgramaJuvenil':
        ...
        return cls(id=uuid4(), codigo_programa=codigo_programa.strip(), nome=nome_normalizado, tipo=tipo, data_inicio=data_inicio, data_fim=data_fim, vagas=vagas, municipio=municipio.strip() if municipio else None, provincia=provincia.strip() if provincia else None, data_cadastro=date.today(), status=StatusPrograma.PLANEADO, observacoes=observacoes.strip() if observacoes else None, ativo=True)
