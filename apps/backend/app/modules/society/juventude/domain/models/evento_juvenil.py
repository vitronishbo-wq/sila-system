...
class EventoJuvenil:
    ...
    @classmethod
    def criar(cls, *, codigo_evento: str, titulo: str, tipo_evento: TipoEvento, area_interesse: AreaInteresse, data_evento: date, local: str, municipio: str, provincia: str, vagas: int | None=None, observacoes: str | None=None) -> 'EventoJuvenil':
        ...
        return cls(id=uuid4(), codigo_evento=codigo_evento.strip(), titulo=titulo.strip(), tipo_evento=tipo_evento, area_interesse=area_interesse, data_evento=data_evento, local=local.strip(), municipio=municipio.strip(), provincia=provincia.strip(), vagas=vagas, data_cadastro=date.today(), observacoes=observacoes.strip() if observacoes else None, status=StatusEvento.PLANEADO, ativo=True)
