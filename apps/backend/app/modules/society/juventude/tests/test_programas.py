...
def test_criar_programa_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        result = await service.criar_programa(nome='Programa Primeiro Emprego', tipo=TipoPrograma.PRIMEIRO_EMPREGO, data_inicio=date.today(), vagas=120, municipio='Luanda', provincia='Luanda')
...
def test_atualizar_status_programa() -> None:
    ...
    async def scenario() -> None:
        service = ProgramaService(programa_repo=InMemoryProgramaRepository())
        programa = await service.criar_programa(nome='Programa Lideranca Jovem', tipo=TipoPrograma.LIDERANCA, data_inicio=date.today())
        atualizado = await service.atualizar_status(programa_id=programa.id, status=StatusPrograma.ATIVO)
