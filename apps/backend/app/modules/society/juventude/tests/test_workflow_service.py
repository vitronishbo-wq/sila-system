...
class InMemoryWorkflowRepository(WorkflowRepositoryPort):
    ...
    async def exists_active_for_citizen(self, citizen_id: UUID, service_type: str) -> bool:
        active = {StatusFluxo.PENDENTE, StatusFluxo.CONFIRMADA, StatusFluxo.EM_ANALISE, StatusFluxo.APROVADA}
        return any((i.citizen_id == citizen_id and i.service_type == service_type and (i.status in active) for i in self._items.values()))
...
def test_workflow_create_e_concluir() -> None:
    ...
    async def scenario() -> None:
        citizen_id = uuid4()
        service = WorkflowService(repository=InMemoryWorkflowRepository(), process_prefix='JUV', citizen_service=FakeCitizenService(active=True))
        created = await service.create_record(service_type='inscricao_programa', citizen_id=citizen_id, instituicao_id=uuid4(), observacoes='solicitacao inicial')
...
def test_workflow_bloqueia_duplicado_ativo() -> None:
    ...
    async def scenario() -> None:
        citizen_id = uuid4()
        service = WorkflowService(repository=InMemoryWorkflowRepository(), process_prefix='JUV', citizen_service=FakeCitizenService(active=True))
        await service.create_record(service_type='inscricao_programa', citizen_id=citizen_id, instituicao_id=uuid4())
