...
from apps.backend.app.modules.society.juventude.tests._fakes import (
    FakeCitizenService,
    FakeEducacaoService,
    FakeEmpregoService,
    InMemoryJovemRepository,
    InMemoryProgramaRepository,
)
...
class InMemoryInscricaoRepository(InscricaoProgramaRepositoryPort):
    ...
    async def exists_active_by_jovem_programa(self, jovem_id: UUID, programa_id: UUID) -> bool:
        return any(
            i.jovem_id == jovem_id
            and i.programa_id == programa_id
            and (i.status in {StatusInscricao.PENDENTE, StatusInscricao.CONFIRMADA})
    ...
    async def count_confirmadas_by_programa(self, programa_id: UUID) -> int:
        return sum(
            1
            for i in self._items.values()
            if i.programa_id == programa_id and i.status == StatusInscricao.CONFIRMADA
        )
...
def test_bolsa_estudo_fluxo_basico() -> None:
    ...
    async def scenario() -> None:
        ...
        jovem_service = JovemService(
            jovem_repo=jovem_repo,
            citizen_service=FakeCitizenService(active=True),
            educacao_service=FakeEducacaoService(matricula_ativa=True),
            emprego_service=FakeEmpregoService(candidatura_ativa=True),
        )
        jovem = await jovem_service.cadastrar_jovem(
            nome="Jovem Bolsa",
            data_nascimento=date.today() - timedelta(days=365 * 19),
            genero="F",
            naturalidade="Luanda",
            escolaridade=Escolaridade.MEDIO_COMPLETO,
            situacao_ocupacional=SituacaoOcupacional.ESTUDA,
            endereco="Rua A",
            municipio="Luanda",
            provincia="Luanda",
            citizen_id=uuid4(),
        ...
        bolsa = await service.conceder_bolsa(
            jovem_id=jovem.id,
            tipo=TipoBolsa.PERMANENCIA,
            valor_mensal=Decimal("25000.00"),
...
def test_estagio_fluxo_basico() -> None:
    ...
    async def scenario() -> None:
        ...
        jovem = await jovem_service.cadastrar_jovem(
            nome="Jovem Estagio",
            data_nascimento=date.today() - timedelta(days=365 * 21),
            genero="M",
            naturalidade="Benguela",
            escolaridade=Escolaridade.SUPERIOR_INCOMPLETO,
            situacao_ocupacional=SituacaoOcupacional.ESTUDA_TRABALHA,
            endereco="Rua B",
            municipio="Benguela",
            provincia="Benguela",
            citizen_id=uuid4(),
        )
        service = EstagioJuvenilService(
            estagio_repo=InMemoryEstagioRepository(), jovem_repo=jovem_repo
        )
        estagio = await service.registrar_estagio(
            jovem_id=jovem.id,
            instituicao="Empresa Teste",
            area_interesse=AreaInteresse.TECNOLOGIA,
            cargo="Assistente",
...
def test_inscricao_programa_valida_vagas_e_duplicidade() -> None:
    ...
    async def scenario() -> None:
        ...
        jovem = await jovem_service.cadastrar_jovem(
            nome="Jovem Inscricao",
            data_nascimento=date.today() - timedelta(days=365 * 20),
            genero="M",
            naturalidade="Huambo",
            escolaridade=Escolaridade.MEDIO_COMPLETO,
            situacao_ocupacional=SituacaoOcupacional.PROCURA_EMPREGO,
            endereco="Rua C",
            municipio="Huambo",
            provincia="Huambo",
            citizen_id=uuid4(),
        ...
        service = InscricaoProgramaService(
            inscricao_repo=InMemoryInscricaoRepository(),
            jovem_repo=jovem_repo,
        ...
        try:
            ...
            raise AssertionError("Esperava erro de duplicidade")
        except ValueError:
            assert True
