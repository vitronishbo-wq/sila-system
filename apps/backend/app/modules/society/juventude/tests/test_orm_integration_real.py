...
def _tables():
    ...
    from apps.backend.app.modules.society.juventude.infrastructure.models.jovem_model import (
        JovemModel,
    )
    ...
    return [
        JovemModel.__table__,
        AuxilioModel.__table__,
        ProgramaJuvenilModel.__table__,
        FormacaoJuvenilModel.__table__,
    ]
    ...
    async def scenario() -> None:
        async with _session_scope() as session:
            ...
            formacao_service = FormacaoService(
                formacao_repo=formacao_repo, jovem_repo=jovem_repo, programa_repo=programa_repo
            )
            ...
            auxilio = await auxilio_service.conceder_auxilio(
                jovem_id=jovem.id,
                tipo=TipoAuxilio.ALIMENTACAO,
                data_inicio=date.today(),
                valor_mensal=Decimal("25000.00"),
            )
            programa = await programa_service.criar_programa(
                nome="Programa ORM Juventude",
                tipo=TipoPrograma.CAPACITACAO,
                data_inicio=date.today(),
                vagas=50,
                municipio="Luanda",
                provincia="Luanda",
            )
            formacao = await formacao_service.registrar_formacao(
                jovem_id=jovem.id,
                programa_id=programa.id,
                nome_curso="Curso ORM Juventude",
                instituicao="Centro ORM",
                carga_horaria=40,
                data_inicio=date.today(),
            )
