...
from apps.backend.app.core.bridges.society_repository_bridges import (
    make_educacao_matricula_repository,
)
from apps.backend.app.modules.society.emprego.infrastructure.repositories.sqlalchemy_candidato_repository import (
...
from apps.backend.app.modules.society.juventude.application.services.auxilio_service import (
    AuxilioService,
)
from apps.backend.app.modules.society.juventude.application.services.bolsa_estudo_service import (
    BolsaEstudoService,
)
from apps.backend.app.modules.society.juventude.application.services.empreendedorismo_juvenil_service import (
    EmpreendedorismoJuvenilService,
)
from apps.backend.app.modules.society.juventude.application.services.estagio_juvenil_service import (
    EstagioJuvenilService,
)
from apps.backend.app.modules.society.juventude.application.services.evento_juvenil_service import (
    EventoJuvenilService,
)
from apps.backend.app.modules.society.juventude.application.services.formacao_service import (
    FormacaoService,
)
from apps.backend.app.modules.society.juventude.application.services.inscricao_programa_service import (
    InscricaoProgramaService,
)
from apps.backend.app.modules.society.juventude.application.services.intercambio_juvenil_service import (
    IntercambioJuvenilService,
)
from apps.backend.app.modules.society.juventude.application.services.jovem_service import (
    JovemService,
)
...
from apps.backend.app.modules.society.juventude.application.services.risco_evasao_service import (
    RiscoEvasaoService,
)
from apps.backend.app.modules.society.juventude.application.services.saude_juvenil_service import (
    SaudeJuvenilService,
)
from apps.backend.app.modules.society.juventude.application.services.voluntariado_service import (
    VoluntariadoService,
)
from apps.backend.app.modules.society.juventude.infrastructure.adapters.citizen_service_adapter import (
    CitizenServiceAdapter,
)
from apps.backend.app.modules.society.juventude.infrastructure.adapters.educacao_service_adapter import (
    EducacaoServiceAdapter,
)
from apps.backend.app.modules.society.juventude.infrastructure.adapters.emprego_service_adapter import (
    EmpregoServiceAdapter,
)
from apps.backend.app.modules.society.juventude.infrastructure.adapters.request_service_adapter import (
...
async def get_jovem_service(session: AsyncSession = db_dep) -> JovemService:
    return JovemService(
        jovem_repo=SQLAlchemyJovemRepository(session),
        citizen_service=_get_citizen_service(session),
        educacao_service=_get_educacao_service(session),
        emprego_service=_get_emprego_service(session),
        request_service=_get_request_service(session),
    )


async def get_auxilio_service(session: AsyncSession = db_dep) -> AuxilioService:
    return AuxilioService(
...
async def get_programa_service(session: AsyncSession = db_dep) -> ProgramaService:
...
async def get_formacao_service(session: AsyncSession = db_dep) -> FormacaoService:
...
async def get_bolsa_estudo_service(session: AsyncSession = db_dep) -> BolsaEstudoService:
    return BolsaEstudoService(
        bolsa_repo=SQLAlchemyBolsaEstudoRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
        request_service=_get_request_service(session),
    )


async def get_estagio_juvenil_service(
    session: AsyncSession = db_dep,
) -> EstagioJuvenilService:
    return EstagioJuvenilService(
        estagio_repo=SQLAlchemyEstagioJuvenilRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
        request_service=_get_request_service(session),
    )


async def get_inscricao_programa_service(
    session: AsyncSession = db_dep,
) -> InscricaoProgramaService:
    return InscricaoProgramaService(
        inscricao_repo=SQLAlchemyInscricaoProgramaRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
        programa_repo=SQLAlchemyProgramaRepository(session),
        request_service=_get_request_service(session),
    )
...
async def get_intercambio_juvenil_service(
    session: AsyncSession = db_dep,
) -> IntercambioJuvenilService:
    return IntercambioJuvenilService(
        intercambio_repo=SQLAlchemyIntercambioJuvenilRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
    )


async def get_mentor_service(session: AsyncSession = db_dep) -> MentorService:
    return MentorService(mentor_repo=SQLAlchemyMentorRepository(session))


async def get_evento_juvenil_service(
    session: AsyncSession = db_dep,
) -> EventoJuvenilService:
    return EventoJuvenilService(
        evento_repo=SQLAlchemyEventoJuvenilRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
    )


async def get_voluntariado_service(session: AsyncSession = db_dep) -> VoluntariadoService:
    return VoluntariadoService(
        voluntariado_repo=SQLAlchemyVoluntariadoRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
    )


async def get_empreendedorismo_juvenil_service(
    session: AsyncSession = db_dep,
) -> EmpreendedorismoJuvenilService:
    return EmpreendedorismoJuvenilService(
        empreendedorismo_repo=SQLAlchemyEmpreendedorismoJuvenilRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
    )


async def get_saude_juvenil_service(session: AsyncSession = db_dep) -> SaudeJuvenilService:
    return SaudeJuvenilService(
        saude_repo=SQLAlchemySaudeJuvenilRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
    )


async def get_acompanhamento_juvenil_service(
    session: AsyncSession = db_dep,
) -> AcompanhamentoJuvenilService:
    return AcompanhamentoJuvenilService(
        acompanhamento_repo=SQLAlchemyAcompanhamentoJuvenilRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
    )


async def get_politica_juventude_service(
    session: AsyncSession = db_dep,
) -> PoliticaJuventudeService:
    return PoliticaJuventudeService(politica_repo=SQLAlchemyPoliticaJuventudeRepository(session))


async def get_risco_evasao_service(session: AsyncSession = db_dep) -> RiscoEvasaoService:
    return RiscoEvasaoService(
        risco_repo=SQLAlchemyRiscoEvasaoRepository(session),
        jovem_repo=SQLAlchemyJovemRepository(session),
        educacao_service=_get_educacao_service(session),
        request_service=_get_request_service(session),
    )