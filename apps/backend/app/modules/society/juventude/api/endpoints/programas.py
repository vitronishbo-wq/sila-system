...
from apps.backend.app.modules.society.juventude.api.deps import get_programa_service
...
programa_service_dep = Depends(get_programa_service)


@router.post("/", response_model=ProgramaResponse, status_code=status.HTTP_201_CREATED)
async def criar_programa(
    data: ProgramaCreate, service: ProgramaService = programa_service_dep
) -> ProgramaResponse:
    try:
...
async def obter_programa(
    programa_id: UUID, service: ProgramaService = programa_service_dep
) -> ProgramaResponse:
    try:
...
async def listar_programas(
    ...
    status_filtro: StatusPrograma | None = None,
    service: ProgramaService = programa_service_dep,
...
async def atualizar_status_programa(
    programa_id: UUID,
    data: ProgramaStatusUpdate,
    service: ProgramaService = programa_service_dep,
) -> ProgramaResponse:
    try:
