...
evento_juvenil_service_dep = Depends(get_evento_juvenil_service)


@router.post("/", response_model=EventoJuvenilResponse, status_code=status.HTTP_201_CREATED)
async def criar_evento(
    data: EventoJuvenilCreate, service: EventoJuvenilService = evento_juvenil_service_dep
) -> EventoJuvenilResponse:
    try:
        return await service.criar_evento(
            titulo=data.titulo,
...
async def obter_evento(
    evento_id: UUID, service: EventoJuvenilService = evento_juvenil_service_dep
) -> EventoJuvenilResponse:
    try:
...
async def listar_eventos(
    status_filtro: StatusEvento | None = None,
    service: EventoJuvenilService = evento_juvenil_service_dep,
) -> list[EventoJuvenilResponse]:
