...
jovem_service_dep = Depends(get_jovem_service)


@router.post("/", response_model=JovemResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_jovem(
    data: JovemCreate, service: JovemService = jovem_service_dep
) -> JovemResponse:
    try:
        return await service.cadastrar_jovem(
            nome=data.nome,
...
async def obter_jovem(
    jovem_id: UUID, service: JovemService = jovem_service_dep
) -> JovemResponse:
    try:
...
async def listar_jovens(
    ...
    service: JovemService = jovem_service_dep,
) -> list[JovemResponse]:
    return await service.listar_jovens(
...
async def atualizar_jovem(
    jovem_id: UUID, data: JovemUpdate, service: JovemService = jovem_service_dep
) -> JovemResponse:
    try:
        return await service.atualizar_jovem(
            jovem_id=jovem_id,
            escolaridade=data.escolaridade,
            situacao_ocupacional=data.situacao_ocupacional,
            telefone=data.telefone,
...
async def adicionar_vulnerabilidade(
    jovem_id: UUID, data: VulnerabilidadeAdd, service: JovemService = jovem_service_dep
) -> JovemResponse:
    try:
        return await service.adicionar_vulnerabilidade(
            jovem_id=jovem_id, vulnerabilidade=data.vulnerabilidade
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def remover_jovem(jovem_id: UUID, service: JovemService = jovem_service_dep) -> None:
    try:
