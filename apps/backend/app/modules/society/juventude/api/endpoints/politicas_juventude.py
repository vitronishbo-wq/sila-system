...
async def criar_politica(data: PoliticaJuventudeCreate, service: PoliticaJuventudeService=Depends(get_politica_juventude_service)) -> PoliticaJuventudeResponse:
    try:
        return await service.criar_politica(nome=data.nome, descricao=data.descricao, area_interesse=data.area_interesse, data_inicio=data.data_inicio, metas=data.metas, indicadores=data.indicadores, data_fim=data.data_fim, observacoes=data.observacoes)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))

@router.get('/{politica_id}', response_model=PoliticaJuventudeResponse)
async def obter_politica(politica_id: UUID, service: PoliticaJuventudeService=Depends(get_politica_juventude_service)) -> PoliticaJuventudeResponse:
    try:
...
async def listar_politicas(status_filtro: StatusPoliticaJuventude | None=None, area_filtro: AreaInteresse | None=None, service: PoliticaJuventudeService=Depends(get_politica_juventude_service)) -> list[PoliticaJuventudeResponse]:
...
async def atualizar_status_politica(politica_id: UUID, data: PoliticaJuventudeStatusUpdate, service: PoliticaJuventudeService=Depends(get_politica_juventude_service)) -> PoliticaJuventudeResponse:
    try:
...
async def remover_politica(politica_id: UUID, service: PoliticaJuventudeService=Depends(get_politica_juventude_service)) -> None:
    try:
