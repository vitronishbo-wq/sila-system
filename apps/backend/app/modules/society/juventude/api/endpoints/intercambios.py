...
intercambio_juvenil_service_dep = Depends(get_intercambio_juvenil_service)


@router.post("/", response_model=IntercambioJuvenilResponse, status_code=status.HTTP_201_CREATED)
async def solicitar_intercambio(
    data: IntercambioJuvenilCreate,
    service: IntercambioJuvenilService = intercambio_juvenil_service_dep,
) -> IntercambioJuvenilResponse:
    try:
        return await service.solicitar_intercambio(
            jovem_id=data.jovem_id,
            pais_destino=data.pais_destino,
            instituicao_destino=data.instituicao_destino,
...
async def listar_intercambios(
    jovem_id: UUID | None = None,
    status_filtro: StatusIntercambio | None = None,
    service: IntercambioJuvenilService = intercambio_juvenil_service_dep,
) -> list[IntercambioJuvenilResponse]:
...
async def atualizar_status_intercambio(
    intercambio_id: UUID,
    data: IntercambioJuvenilStatusUpdate,
    service: IntercambioJuvenilService = intercambio_juvenil_service_dep,
) -> IntercambioJuvenilResponse:
    try:
