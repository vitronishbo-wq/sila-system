...
risco_evasao_service_dep = Depends(get_risco_evasao_service)


@router.post("/avaliar", response_model=RiscoEvasaoResponse, status_code=status.HTTP_201_CREATED)
async def avaliar_risco(
    data: RiscoEvasaoAvaliar, service: RiscoEvasaoService = risco_evasao_service_dep
) -> RiscoEvasaoResponse:
    try:
...
async def obter_risco_ativo_jovem(
    jovem_id: UUID, service: RiscoEvasaoService = risco_evasao_service_dep
) -> RiscoEvasaoResponse:
    try:
...
async def obter_risco(
    risco_id: UUID, service: RiscoEvasaoService = risco_evasao_service_dep
) -> RiscoEvasaoResponse:
    try:
...
async def listar_riscos(
    nivel: RiscoSocial | None = None,
    service: RiscoEvasaoService = risco_evasao_service_dep,
) -> list[RiscoEvasaoResponse]:
