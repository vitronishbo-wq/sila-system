...
async def cadastrar_mentor(data: MentorCreate, service: MentorService=Depends(get_mentor_service)) -> MentorResponse:
    try:
        return await service.cadastrar_mentor(nome=data.nome, tipo_mentoria=data.tipo_mentoria, area_interesse=data.area_interesse, email=data.email, telefone=data.telefone, observacoes=data.observacoes)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))

@router.get('/{mentor_id}', response_model=MentorResponse)
async def obter_mentor(mentor_id: UUID, service: MentorService=Depends(get_mentor_service)) -> MentorResponse:
    try:
...
async def listar_mentores(status_filtro: StatusMentoria | None=None, service: MentorService=Depends(get_mentor_service)) -> list[MentorResponse]:
...
async def atribuir_jovem(mentor_id: UUID, data: MentorAtribuirJovem, service: MentorService=Depends(get_mentor_service)) -> MentorResponse:
    try:
...
async def atualizar_status_mentor(mentor_id: UUID, data: MentorStatusUpdate, service: MentorService=Depends(get_mentor_service)) -> MentorResponse:
    try:
...
async def remover_mentor(mentor_id: UUID, service: MentorService=Depends(get_mentor_service)) -> None:
    try:
