...
async def iniciar_voluntariado(data: VoluntariadoCreate, service: VoluntariadoService=Depends(get_voluntariado_service)) -> VoluntariadoResponse:
    try:
        return await service.iniciar_voluntariado(jovem_id=data.jovem_id, organizacao=data.organizacao, causa=data.causa, carga_horaria_total=data.carga_horaria_total, data_inicio=data.data_inicio, data_fim=data.data_fim, observacoes=data.observacoes)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))

@router.get('/{voluntariado_id}', response_model=VoluntariadoResponse)
async def obter_voluntariado(voluntariado_id: UUID, service: VoluntariadoService=Depends(get_voluntariado_service)) -> VoluntariadoResponse:
    try:
...
async def listar_voluntariados(jovem_id: UUID | None=None, status_filtro: StatusVoluntariado | None=None, service: VoluntariadoService=Depends(get_voluntariado_service)) -> list[VoluntariadoResponse]:
...
async def atualizar_status_voluntariado(voluntariado_id: UUID, data: VoluntariadoStatusUpdate, service: VoluntariadoService=Depends(get_voluntariado_service)) -> VoluntariadoResponse:
    try:
...
async def remover_voluntariado(voluntariado_id: UUID, service: VoluntariadoService=Depends(get_voluntariado_service)) -> None:
    try:
