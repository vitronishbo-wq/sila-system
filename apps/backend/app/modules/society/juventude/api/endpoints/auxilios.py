...
auxilio_service_dep = Depends(get_auxilio_service)


@router.post("/", response_model=AuxilioResponse, status_code=status.HTTP_201_CREATED)
async def conceder_auxilio(
    data: AuxilioCreate, service: AuxilioService = auxilio_service_dep
) -> AuxilioResponse:
    try:
        return await service.conceder_auxilio(
...
async def obter_auxilio(
    auxilio_id: UUID, service: AuxilioService = auxilio_service_dep
) -> AuxilioResponse:
    try:
...
async def listar_auxilios(
    ...
    tipo: TipoAuxilio | None = None,
    status_filtro: StatusBeneficio | None = None,
    service: AuxilioService = auxilio_service_dep,
) -> list[AuxilioResponse]:
...
async def atualizar_status_auxilio(
    auxilio_id: UUID,
    data: AuxilioStatusUpdate,
    service: AuxilioService = auxilio_service_dep,
) -> AuxilioResponse:
    try:
        return await service.atualizar_status(
            auxilio_id=auxilio_id, status=data.status, observacoes=data.observacoes
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
