...
saude_juvenil_service_dep = Depends(get_saude_juvenil_service)


@router.post("/", response_model=SaudeJuvenilResponse, status_code=status.HTTP_201_CREATED)
async def registrar_saude(
    data: SaudeJuvenilCreate, service: SaudeJuvenilService = saude_juvenil_service_dep
) -> SaudeJuvenilResponse:
    try:
        return await service.registrar_saude(
            jovem_id=data.jovem_id,
            tipo_registo=data.tipo_registo,
...
async def obter_registo_saude(
    registo_id: UUID, service: SaudeJuvenilService = saude_juvenil_service_dep
) -> SaudeJuvenilResponse:
    try:
...
async def listar_registos_saude(
    jovem_id: UUID | None = None,
    status_filtro: StatusAcompanhamento | None = None,
    service: SaudeJuvenilService = saude_juvenil_service_dep,
) -> list[SaudeJuvenilResponse]:
...
async def atualizar_acompanhamento_saude(
    registo_id: UUID,
    data: SaudeJuvenilStatusUpdate,
    service: SaudeJuvenilService = saude_juvenil_service_dep,
) -> SaudeJuvenilResponse:
    try:
...
async def remover_registo_saude(
    registo_id: UUID, service: SaudeJuvenilService = saude_juvenil_service_dep
) -> None:
    try:
