...
inscricao_programa_service_dep = Depends(get_inscricao_programa_service)


@router.post("/", response_model=InscricaoProgramaResponse, status_code=status.HTTP_201_CREATED)
async def inscrever_jovem(
    data: InscricaoProgramaCreate,
    service: InscricaoProgramaService = inscricao_programa_service_dep,
) -> InscricaoProgramaResponse:
    try:
        return await service.inscrever_jovem(
...
async def obter_inscricao(
    inscricao_id: UUID, service: InscricaoProgramaService = inscricao_programa_service_dep
) -> InscricaoProgramaResponse:
    try:
...
async def listar_inscricoes(
    ...
    status_filtro: StatusInscricao | None = None,
    service: InscricaoProgramaService = inscricao_programa_service_dep,
) -> list[InscricaoProgramaResponse]:
    return await service.listar_inscricoes(
...
async def cancelar_inscricao(
    inscricao_id: UUID,
    data: InscricaoProgramaCancelar,
    service: InscricaoProgramaService = inscricao_programa_service_dep,
) -> InscricaoProgramaResponse:
    try:
