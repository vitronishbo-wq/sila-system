...
empreendedorismo_juvenil_service_dep = Depends(get_empreendedorismo_juvenil_service)


@router.post(
    "/", response_model=EmpreendedorismoJuvenilResponse, status_code=status.HTTP_201_CREATED
)
async def registrar_empreendimento(
    data: EmpreendedorismoJuvenilCreate,
    service: EmpreendedorismoJuvenilService = empreendedorismo_juvenil_service_dep,
) -> EmpreendedorismoJuvenilResponse:
    try:
        return await service.registrar_empreendimento(
...
async def listar_empreendimentos(
    ...
    status_filtro: StatusEmpreendimento | None = None,
    service: EmpreendedorismoJuvenilService = empreendedorismo_juvenil_service_dep,
) -> list[EmpreendedorismoJuvenilResponse]:
...
async def atualizar_status_empreendimento(
    empreendimento_id: UUID,
    data: EmpreendedorismoJuvenilStatusUpdate,
    service: EmpreendedorismoJuvenilService = empreendedorismo_juvenil_service_dep,
) -> EmpreendedorismoJuvenilResponse:
    try:
        return await service.atualizar_status(
            empreendimento_id=empreendimento_id, status=data.status
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def remover_empreendimento(
    empreendimento_id: UUID,
    service: EmpreendedorismoJuvenilService = empreendedorismo_juvenil_service_dep,
) -> None:
    try:
