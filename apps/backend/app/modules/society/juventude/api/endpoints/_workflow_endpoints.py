...
def build_workflow_router(*, tag: str, get_service: Callable, create_schema: Any, response_schema: Any, concluir_schema: Any, cancelar_schema: Any, routes: list[tuple[str, str]]) -> APIRouter:
    ...
    def register_create(path: str, service_type: str) -> None:
        ...
        async def create(data: create_schema, service=Depends(get_service), _: dict=Depends(get_current_user)):
            try:
                return await service.create_record(service_type=service_type, citizen_id=data.citizen_id, instituicao_id=data.instituicao_id, observacoes=data.observacoes, metadata=data.metadata)
    ...
    async def list_by_citizen(citizen_id: UUID, service_type: str | None=None, service=Depends(get_service), _: dict=Depends(get_current_user)):
    ...
    async def conclude(record_id: UUID, data: concluir_schema, service=Depends(get_service), _: dict=Depends(get_current_user)):
        try:
            return await service.conclude_record(record_id, data.resumo)
        except ValueError as exc:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))

    @router.post('/workflow/{record_id}/cancelar', response_model=response_schema)
    async def cancel(record_id: UUID, data: cancelar_schema, service=Depends(get_service), _: dict=Depends(get_current_user)):
        try:
            return await service.cancel_record(record_id, data.motivo)
        except ValueError as exc:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
    return router