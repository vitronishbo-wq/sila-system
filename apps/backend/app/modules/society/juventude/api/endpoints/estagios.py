...
estagio_juvenil_service_dep = Depends(get_estagio_juvenil_service)


@router.post("/", response_model=EstagioJuvenilResponse, status_code=status.HTTP_201_CREATED)
async def registrar_estagio(
    data: EstagioJuvenilCreate,
    service: EstagioJuvenilService = estagio_juvenil_service_dep,
) -> EstagioJuvenilResponse:
    try:
        return await service.registrar_estagio(
            ...
            bolsa_auxilio=data.bolsa_auxilio,
...
async def obter_estagio(
    estagio_id: UUID, service: EstagioJuvenilService = estagio_juvenil_service_dep
) -> EstagioJuvenilResponse:
    try:
...
async def listar_estagios(
    ...
    status_filtro: StatusEstagio | None = None,
    service: EstagioJuvenilService = estagio_juvenil_service_dep,
) -> list[EstagioJuvenilResponse]:
...
async def atualizar_status_estagio(
    estagio_id: UUID,
    data: EstagioJuvenilStatusUpdate,
    service: EstagioJuvenilService = estagio_juvenil_service_dep,
) -> EstagioJuvenilResponse:
    try:
