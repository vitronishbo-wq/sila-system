...
from apps.backend.app.modules.society.assistencia_social.application.services import (
    AtendimentoService,
    BeneficiarioService,
    BeneficioService,
...
from apps.backend.app.modules.society.assistencia_social.infrastructure.adapters import (
    CitizenServiceAdapter,
    EducacaoServiceAdapter,
    EmpregoServiceAdapter,
    JuventudeServiceAdapter,
    RequestServiceAdapter,
    SaudeServiceAdapter,
)
from apps.backend.app.modules.society.assistencia_social.infrastructure.repositories import (
...
def _bridges(session: AsyncSession):
    return (
        CitizenServiceAdapter(CitizenRepository(session)),
        EducacaoServiceAdapter(make_educacao_matricula_repository(session)),
        SaudeServiceAdapter(),
        JuventudeServiceAdapter(SQLAlchemyJovemRepository(session)),
        EmpregoServiceAdapter(SQLAlchemyCandidatoRepository(session)),
        RequestServiceAdapter(ServiceRequestLifecycleBridge(session)),
    )


async def get_beneficiario_service(session: AsyncSession = db_dep) -> BeneficiarioService:
    ...
    return BeneficiarioService(
        ...
        cadastro_unico_repo=SQLAlchemyCadastroUnicoRepository(session),
        request_service=request,
    )


async def get_cadastro_unico_service(
    session: AsyncSession = db_dep,
) -> CadastroUnicoService:
    citizen, educacao, _, juventude, _, request = _bridges(session)
    return CadastroUnicoService(
        cadastro_repo=SQLAlchemyCadastroUnicoRepository(session),
        citizen_service=citizen,
        educacao_service=educacao,
        juventude_service=juventude,
        request_service=request,
    )


async def get_programa_social_service(
    session: AsyncSession = db_dep,
) -> ProgramaSocialService:
    return ProgramaSocialService(programa_repo=SQLAlchemyProgramaSocialRepository(session))


async def get_beneficio_service(session: AsyncSession = db_dep) -> BeneficioService:
    _, _, saude, _, emprego, request = _bridges(session)
    return BeneficioService(
        beneficio_repo=SQLAlchemyBeneficioRepository(session),
        beneficiario_repo=SQLAlchemyBeneficiarioRepository(session),
...
async def get_atendimento_service(session: AsyncSession = db_dep) -> AtendimentoService:
...
async def get_visita_domiciliar_service(
    session: AsyncSession = db_dep,
) -> VisitaDomiciliarService:
    _, _, saude, _, _, request = _bridges(session)
    return VisitaDomiciliarService(
...
async def get_situacao_rua_service(session: AsyncSession = db_dep) -> SituacaoRuaService:
    _, _, _, _, _, request = _bridges(session)
    return SituacaoRuaService(
...
async def get_crianca_risco_service(session: AsyncSession = db_dep) -> CriancaRiscoService:
    _, educacao, _, _, _, request = _bridges(session)
    return CriancaRiscoService(
...
async def get_idoso_vulneravel_service(
    session: AsyncSession = db_dep,
) -> IdosoVulneravelService:
    _, _, saude, _, _, request = _bridges(session)
    return IdosoVulneravelService(
...
async def get_pcd_service(session: AsyncSession = db_dep) -> PCDService:
    _, _, saude, _, _, request = _bridges(session)
    return PCDService(
        pcd_repo=SQLAlchemyPCDRepository(session),
        beneficiario_repo=SQLAlchemyBeneficiarioRepository(session),
