...
from apps.backend.app.modules.society.assistencia_social.api.schemas.cadastro_unico_schema import (
    CadastroUnicoCreate,
    CadastroUnicoCreateResponse,
    CadastroUnicoResponse,
)
...
cadastro_unico_service_dep = Depends(get_cadastro_unico_service)


@router.post("/", response_model=CadastroUnicoCreateResponse, status_code=status.HTTP_201_CREATED)
async def registrar_cadastro_unico(
    data: CadastroUnicoCreate, service: CadastroUnicoService = cadastro_unico_service_dep
) -> CadastroUnicoCreateResponse:
    try:
        cadastro, programas, alertas = await service.registrar_cadastro(
            citizen_id_responsavel=data.citizen_id_responsavel,
            renda_per_capita=data.renda_per_capita,
            composicao_familiar=data.composicao_familiar,
            condicoes_moradia=data.condicoes_moradia,
            acesso_agua=data.acesso_agua,
            acesso_energia=data.acesso_energia,
            observacoes=data.observacoes,
        )
        return CadastroUnicoCreateResponse(
            cadastro=CadastroUnicoResponse.model_validate(cadastro),
...
async def obter_cadastro_unico(
    cadastro_id: UUID, service: CadastroUnicoService = cadastro_unico_service_dep
) -> CadastroUnicoResponse:
    try:
...
async def listar_cadastros_unicos(
    service: CadastroUnicoService = cadastro_unico_service_dep,
) -> list[CadastroUnicoResponse]:
    return await service.listar_cadastros()


@router.delete("/{cadastro_id}", status_code=status.HTTP_204_NO_CONTENT)
async def remover_cadastro_unico(
    cadastro_id: UUID, service: CadastroUnicoService = cadastro_unico_service_dep
) -> None:
    try:
