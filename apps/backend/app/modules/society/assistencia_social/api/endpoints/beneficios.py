...
from apps.backend.app.modules.society.assistencia_social.api.schemas.beneficio_schema import (
    BeneficioBpcPcdCreate,
    BeneficioCreate,
    BeneficioMotivo,
    BeneficioResponse,
)
...
beneficio_service_dep = Depends(get_beneficio_service)


@router.post("/", response_model=BeneficioResponse, status_code=status.HTTP_201_CREATED)
async def solicitar_beneficio(
    data: BeneficioCreate, service: BeneficioService = beneficio_service_dep
) -> BeneficioResponse:
    try:
        return await service.solicitar_beneficio(
...
async def conceder_bpc_pcd(
    data: BeneficioBpcPcdCreate, service: BeneficioService = beneficio_service_dep
) -> BeneficioResponse:
    try:
        return await service.conceder_bpc_pcd(
            beneficiario_id=data.beneficiario_id, pcd_id=data.pcd_id, valor=data.valor
        )
    except ValueError as exc:
        raise_http_for_value_error(exc)
...
async def obter_beneficio(
    beneficio_id: UUID, service: BeneficioService = beneficio_service_dep
) -> BeneficioResponse:
    try:
...
async def listar_beneficios(
    beneficiario_id: UUID | None = None, service: BeneficioService = beneficio_service_dep
...
async def negar_beneficio(
    beneficio_id: UUID,
    data: BeneficioMotivo,
    service: BeneficioService = beneficio_service_dep,
) -> BeneficioResponse:
    try:
