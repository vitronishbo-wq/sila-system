...
async def criar_programa_social(data: ProgramaSocialCreate, service: ProgramaSocialService=Depends(get_programa_social_service)) -> ProgramaSocialResponse:
    return await service.criar_programa(nome=data.nome, publico_alvo=data.publico_alvo, criterio_renda_max=data.criterio_renda_max, valor_base=data.valor_base, vagas=data.vagas, data_inicio=data.data_inicio, observacoes=data.observacoes)

@router.get('/{programa_id}', response_model=ProgramaSocialResponse)
async def obter_programa_social(programa_id: UUID, service: ProgramaSocialService=Depends(get_programa_social_service)) -> ProgramaSocialResponse:
    try:
...
async def listar_programas_sociais(service: ProgramaSocialService=Depends(get_programa_social_service)) -> list[ProgramaSocialResponse]:
    return await service.listar_programas()

@router.patch('/{programa_id}/ativar', response_model=ProgramaSocialResponse)
async def ativar_programa_social(programa_id: UUID, service: ProgramaSocialService=Depends(get_programa_social_service)) -> ProgramaSocialResponse:
    try:
...
async def suspender_programa_social(programa_id: UUID, data: ProgramaSocialSuspender, service: ProgramaSocialService=Depends(get_programa_social_service)) -> ProgramaSocialResponse:
    try:
...
async def encerrar_programa_social(programa_id: UUID, data: ProgramaSocialEncerrar, service: ProgramaSocialService=Depends(get_programa_social_service)) -> ProgramaSocialResponse:
    try:
        return await service.encerrar_programa(programa_id, data_fim=data.data_fim, motivo=data.motivo)
    except ValueError as exc:
        raise_http_for_value_error(exc)

@router.delete('/{programa_id}', status_code=status.HTTP_204_NO_CONTENT)
async def remover_programa_social(programa_id: UUID, service: ProgramaSocialService=Depends(get_programa_social_service)) -> None:
    try:
