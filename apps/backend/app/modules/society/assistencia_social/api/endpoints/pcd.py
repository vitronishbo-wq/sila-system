...
from apps.backend.app.modules.society.assistencia_social.api.schemas.pcd_schema import (
    PCDCreate,
    PCDResponse,
)
...
pcd_service_dep = Depends(get_pcd_service)


@router.post("/", response_model=PCDResponse, status_code=status.HTTP_201_CREATED)
async def registrar_pcd(
    data: PCDCreate, service: PCDService = pcd_service_dep
) -> PCDResponse:
    try:
        return await service.registrar_pcd(
            beneficiario_id=data.beneficiario_id,
            citizen_id_pcd=data.citizen_id_pcd,
...
async def obter_pcd(item_id: UUID, service: PCDService = pcd_service_dep) -> PCDResponse:
    try:
...
async def listar_pcd(
    beneficiario_id: UUID | None = None, service: PCDService = pcd_service_dep
) -> list[PCDResponse]:
...
async def ativar_bpc(item_id: UUID, service: PCDService = pcd_service_dep) -> PCDResponse:
    try:
...
async def encerrar_pcd(
    item_id: UUID, service: PCDService = pcd_service_dep
) -> PCDResponse:
    try:
...
async def remover_pcd(item_id: UUID, service: PCDService = pcd_service_dep) -> None:
    try:
