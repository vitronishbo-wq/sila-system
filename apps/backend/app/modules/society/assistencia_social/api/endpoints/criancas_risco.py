...
async def registrar_crianca_risco(data: CriancaRiscoCreate, service: CriancaRiscoService=Depends(get_crianca_risco_service)) -> CriancaRiscoResponse:
    try:
        return await service.registrar_crianca_risco(beneficiario_id=data.beneficiario_id, citizen_id_crianca=data.citizen_id_crianca, idade=data.idade, motivo=data.motivo, escolarizada=data.escolarizada)
    except ValueError as exc:
        raise_http_for_value_error(exc)

@router.get('/{item_id}', response_model=CriancaRiscoResponse)
async def obter_crianca_risco(item_id: UUID, service: CriancaRiscoService=Depends(get_crianca_risco_service)) -> CriancaRiscoResponse:
    try:
...
async def listar_criancas_risco(beneficiario_id: UUID | None=None, service: CriancaRiscoService=Depends(get_crianca_risco_service)) -> list[CriancaRiscoResponse]:
...
async def encerrar_crianca_risco(item_id: UUID, service: CriancaRiscoService=Depends(get_crianca_risco_service)) -> CriancaRiscoResponse:
    try:
...
async def remover_crianca_risco(item_id: UUID, service: CriancaRiscoService=Depends(get_crianca_risco_service)) -> None:
    try:
