...
def test_beneficiario_rejeita_duplicidade_ativa_e_permite_inativar() -> None:
    ...
    async def scenario() -> None:
        ...
        service = BeneficiarioService(
            beneficiario_repo=repo,
            citizen_service=FakeCitizenService(active=True),
            cadastro_unico_repo=InMemoryCadastroUnicoRepo(),
            request_service=request,
        )
        primeiro = await service.cadastrar_beneficiario(
            citizen_id=citizen_id, faixa_vulnerabilidade=FaixaVulnerabilidade.MEDIA
        )
        assert primeiro.numero_registro.startswith("BEN/")
        assert len(request.calls) == 1
        erro = None
        try:
            await service.cadastrar_beneficiario(
                citizen_id=citizen_id, faixa_vulnerabilidade=FaixaVulnerabilidade.ALTA
            )
