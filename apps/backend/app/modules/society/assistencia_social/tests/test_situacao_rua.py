...
def test_situacao_rua_registra_e_encerra() -> None:
    ...
    async def scenario() -> None:
        ...
        service = SituacaoRuaService(
            situacao_repo=InMemorySituacaoRuaRepo(),
            beneficiario_repo=beneficiario_repo,
            request_service=request,
        )
        situacao = await service.registrar_situacao(
            beneficiario_id=beneficiario.id,
            localizacao="Mercado Municipal",
            motivo="perda de moradia",
        )
