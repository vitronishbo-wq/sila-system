...
def test_visita_domiciliar_gera_recomendacao_quando_sem_cobertura() -> None:
    ...
    async def scenario() -> None:
        ...
        service = VisitaDomiciliarService(
            visita_repo=InMemoryVisitaRepo(),
            beneficiario_repo=beneficiario_repo,
            saude_service=FakeSaudeService(cobertura=False),
            request_service=request,
        )
        visita = await service.registrar_visita(
            beneficiario_id=beneficiario.id,
            assistente_social_id=uuid4(),
            condicoes_moradia="residencia em area sem saneamento",
            recomendacoes=["orientacao inicial"],
        )
