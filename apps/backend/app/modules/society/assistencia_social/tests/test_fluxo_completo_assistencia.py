...
def test_fluxo_basico_assistencia_social() -> None:
    ...
    async def scenario() -> None:
        ...
        cadastro, _, _ = await cadastro_service.registrar_cadastro(
            citizen_id_responsavel=uuid4(),
            renda_per_capita=Decimal("120.00"),
            composicao_familiar=[],
            condicoes_moradia="CEDIDA",
            acesso_agua=True,
            acesso_energia=False,
        )
        ...
        programa = await programa_service.criar_programa(
            nome="Auxilio Familiar",
            publico_alvo=PublicoAlvo.FAMILIA_BAIXA_RENDA,
            criterio_renda_max=Decimal("150.00"),
        ...
        beneficio_service = BeneficioService(
            ...
            emprego_service=FakeEmpregoService(),
            request_service=request,
        )
        beneficio = await beneficio_service.solicitar_beneficio(
            beneficiario_id=beneficiario.id,
            tipo=TipoBeneficio.AUXILIO_NUTRICIONAL,
            valor=Decimal("80.00"),
            programa_social_id=programa.id,
        )
        atendimento_service = AtendimentoService(
            atendimento_repo=InMemoryAtendimentoRepo(),
            beneficiario_repo=beneficiario_repo,
            request_service=request,
        )
        ...
        visita_service = VisitaDomiciliarService(
            visita_repo=InMemoryVisitaRepo(),
            beneficiario_repo=beneficiario_repo,
        ...
        visita = await visita_service.registrar_visita(
            ...
            condicoes_moradia="RESIDENCIA SEM SANEAMENTO",
        )
