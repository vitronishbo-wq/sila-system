...
def test_crianca_risco_dispara_solicitacao_quando_fora_da_escola() -> None:
    ...
    async def scenario() -> None:
        ...
        service = CriancaRiscoService(crianca_repo=InMemoryCriancaRiscoRepo(), beneficiario_repo=beneficiario_repo, educacao_service=FakeEducacaoService(estudantes_ativos=set()), request_service=request)
        registro = await service.registrar_crianca_risco(beneficiario_id=beneficiario.id, citizen_id_crianca=uuid4(), idade=12, motivo='risco de evasao', escolarizada=True)
        assert registro.escolarizada is False
        assert any((call['request_type'] == 'ASSISTENCIA_CRIANCA_FORA_ESCOLA' for call in request.calls))
        erro = None
        try:
            await service.registrar_crianca_risco(beneficiario_id=beneficiario.id, citizen_id_crianca=uuid4(), idade=18, motivo='fora da faixa', escolarizada=False)
