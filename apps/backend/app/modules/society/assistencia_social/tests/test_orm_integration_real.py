...
from apps.backend.app.modules.society.assistencia_social.application.services.atendimento_service import (
    AtendimentoService,
)
...
from apps.backend.app.modules.society.assistencia_social.application.services.pcd_service import (
    PCDService,
)
...
def _tables():
    ...
    from apps.backend.app.modules.society.assistencia_social.infrastructure.models.idoso_vulneravel_model import (
        IdosoVulneravelModel,
    )
    ...
    return [
        CadastroUnicoModel.__table__,
        BeneficiarioModel.__table__,
        ProgramaSocialModel.__table__,
        BeneficioModel.__table__,
        AtendimentoModel.__table__,
        VisitaDomiciliarModel.__table__,
        SituacaoRuaModel.__table__,
        CriancaRiscoModel.__table__,
        IdosoVulneravelModel.__table__,
        PCDModel.__table__,
    ]
...
def test_fluxo_real_orm_assistencia_social() -> None:
    ...
    async def scenario() -> None:
        async with _session_scope() as session:
            ...
            cadastro_service = CadastroUnicoService(
                cadastro_repo=cadastro_repo,
                citizen_service=FakeCitizenService(active=True),
                educacao_service=FakeEducacaoService(set()),
                juventude_service=FakeJuventudeService(set()),
                request_service=None,
            )
            beneficiario_service = BeneficiarioService(
                beneficiario_repo=beneficiario_repo,
                citizen_service=FakeCitizenService(active=True),
                cadastro_unico_repo=cadastro_repo,
                request_service=None,
            )
            ...
            visita_service = VisitaDomiciliarService(
                visita_repo=visita_repo,
                beneficiario_repo=beneficiario_repo,
                saude_service=FakeSaudeService(cobertura=False),
                request_service=None,
            )
            situacao_service = SituacaoRuaService(
                situacao_repo=situacao_repo,
                beneficiario_repo=beneficiario_repo,
                request_service=None,
            )
            crianca_service = CriancaRiscoService(
                crianca_repo=crianca_repo,
                beneficiario_repo=beneficiario_repo,
                educacao_service=FakeEducacaoService(set()),
                request_service=None,
            )
            idoso_service = IdosoVulneravelService(
                idoso_repo=idoso_repo,
                beneficiario_repo=beneficiario_repo,
                saude_service=FakeSaudeService(cobertura=False),
                request_service=None,
            )
            pcd_service = PCDService(
                pcd_repo=pcd_repo,
                beneficiario_repo=beneficiario_repo,
                saude_service=FakeSaudeService(),
                request_service=None,
            )
            responsavel_id = uuid4()
            cadastro, programas, _alertas = await cadastro_service.registrar_cadastro(
                citizen_id_responsavel=responsavel_id,
                renda_per_capita=Decimal("80.00"),
                composicao_familiar=[{"idade": 5}, {"idade": 68}],
            ...
            beneficiario = await beneficiario_service.cadastrar_beneficiario(
                citizen_id=uuid4(),
                faixa_vulnerabilidade=FaixaVulnerabilidade.ALTA,
                cadastro_unico_id=cadastro.id,
            )
            assert beneficiario.numero_registro.startswith("BEN/")
            programa = await programa_service.criar_programa(
                nome="Apoio Integrado ORM",
            ...
            atendimento = await atendimento_service.registrar_atendimento(
                ...
                tipo=TipoAtendimento.SOCIAL,
                descricao="atendimento inicial",
                responsavel_id=uuid4(),
            )
            assert atendimento.codigo.startswith("ATD/")
            visita = await visita_service.registrar_visita(
                beneficiario_id=beneficiario.id,
                assistente_social_id=uuid4(),
                condicoes_moradia="moradia fragil",
            )
            assert visita.codigo.startswith("VIS/")
            assert any("cobertura de saude" in rec.lower() for rec in visita.recomendacoes)
            situacao = await situacao_service.registrar_situacao(
                beneficiario_id=beneficiario.id, localizacao="centro urbano", motivo="desalojamento"
            )
            ...
            crianca = await crianca_service.registrar_crianca_risco(
                beneficiario_id=beneficiario.id,
            ...
            idoso = await idoso_service.registrar_idoso(
                beneficiario_id=beneficiario.id,
                citizen_id_idoso=uuid4(),
                idade=72,
                dependencia=True,
                precisa_cuidados=True,
            )
            assert idoso.codigo.startswith("IDO/")
            pcd = await pcd_service.registrar_pcd(
                beneficiario_id=beneficiario.id,
                citizen_id_pcd=beneficiario.citizen_id,
    ...
    try:
        ...
        if "Could not refresh instance" in str(exc):
            pytest.skip(
                "Ambiente de BD sem garantia de leitura imediata apos commit (refresh intermitente em sessao async)."
            )
        raise
