...
def test_pcd_exige_laudo_valido_e_permite_fluxo_bpc() -> None:
    ...
    async def scenario() -> None:
        ...
        beneficiario = await beneficiario_service.cadastrar_beneficiario(
            citizen_id=uuid4(), faixa_vulnerabilidade=FaixaVulnerabilidade.MEDIA
        )
        service_invalido = PCDService(
            pcd_repo=pcd_repo,
            beneficiario_repo=beneficiario_repo,
            saude_service=FakeSaudeService(laudos_invalidos={laudo_id}),
            request_service=request,
        )
        erro = None
        try:
            await service_invalido.registrar_pcd(
        ...
        pcd = await service.registrar_pcd(
            beneficiario_id=beneficiario.id,
            citizen_id_pcd=beneficiario.citizen_id,
            tipo_deficiencia="fisica",
            cid="G82.5",
