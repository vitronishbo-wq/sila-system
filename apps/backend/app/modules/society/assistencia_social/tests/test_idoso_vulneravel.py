...
def test_idoso_vulneravel_valida_idade_e_cobertura_saude() -> None:
    ...
    async def scenario() -> None:
        ...
        service = IdosoVulneravelService(
            idoso_repo=InMemoryIdosoRepo(),
            beneficiario_repo=beneficiario_repo,
        ...
        assert "60" in erro
        registro = await service.registrar_idoso(
            beneficiario_id=beneficiario.id,
            citizen_id_idoso=uuid4(),
            idade=70,
            dependencia=True,
        ...
        assert any(
            call["request_type"] == "ASSISTENCIA_IDOSO_SEM_COBERTURA_SAUDE"
