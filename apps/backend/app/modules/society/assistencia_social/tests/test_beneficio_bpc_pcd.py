...
def test_fluxo_bpc_pcd_aprova_e_ativa_pcd() -> None:
    ...
    async def scenario() -> None:
        ...
        beneficiario = await beneficiario_service.cadastrar_beneficiario(
            citizen_id=uuid4(), faixa_vulnerabilidade=FaixaVulnerabilidade.EXTREMA
        )
        ...
        beneficio_service = BeneficioService(
            ...
            pcd_repo=pcd_repo,
            saude_service=FakeSaudeService(),
            emprego_service=FakeEmpregoService(previdenciarios=set()),
            request_service=request_service,
        )
        beneficio = await beneficio_service.conceder_bpc_pcd(
            beneficiario_id=beneficiario.id, pcd_id=pcd.id, valor=Decimal("706.00")
        )
