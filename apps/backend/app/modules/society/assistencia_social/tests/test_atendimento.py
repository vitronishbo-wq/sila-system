...
def test_atendimento_exige_beneficiario_e_persiste_fluxo() -> None:
    ...
    async def scenario() -> None:
        ...
        erro = None
        try:
            await atendimento_service.registrar_atendimento(
        ...
        beneficiario = await beneficiario_service.cadastrar_beneficiario(
            citizen_id=uuid4(), faixa_vulnerabilidade=FaixaVulnerabilidade.BAIXA
        )
        ...
        atendimento = await service.registrar_atendimento(
            beneficiario_id=beneficiario.id,
            tipo=TipoAtendimento.SOCIAL,
            descricao="visita no CRAS",
            responsavel_id=uuid4(),
            encaminhamentos=[{"tipo": "retorno"}],
        )
