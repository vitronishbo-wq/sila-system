...
from apps.backend.app.modules.society.assistencia_social.api.deps import (
    get_beneficiario_service,
    get_beneficio_service,
    get_cadastro_unico_service,
)
from apps.backend.app.modules.society.assistencia_social.api.router import (
    router as assistencia_social_router,
)
...
def test_endpoint_cadastro_unico_retorna_201_com_programas() -> None:
    ...
    response = client.post(
        "/assistencia-social/cadastros-unicos/",
        json={
            "citizen_id_responsavel": str(uuid4()),
            "renda_per_capita": "80.00",
            "composicao_familiar": [{"idade": 4}, {"idade": 66}],
...
def test_endpoint_obter_beneficiario_retorna_404_quando_nao_encontrado() -> None:
    service = SimpleNamespace(
        buscar_beneficiario=AsyncMock(side_effect=ValueError("Beneficiario nao encontrado"))
    )
...
def test_endpoint_conceder_bpc_pcd_retorna_400_para_laudo_invalido() -> None:
    service = SimpleNamespace(
        conceder_bpc_pcd=AsyncMock(
            side_effect=ValueError("Laudo medico invalido para concessao de BPC")
        )
    )
    client = _build_client({get_beneficio_service: service})
    response = client.post(
        "/assistencia-social/beneficios/bpc-pcd",
        json={"beneficiario_id": str(uuid4()), "pcd_id": str(uuid4()), "valor": "706.00"},
    )
...
def test_endpoint_listar_beneficios_retorna_lista() -> None:
    ...
    beneficio = Beneficio.solicitar(
        codigo="BNF/2026/000001",
