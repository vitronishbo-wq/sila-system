...
from apps.backend.app.modules.society.assistencia_social.application.ports.idoso_vulneravel_repository_port import (
...
class SQLAlchemyIdosoVulneravelRepository(IdosoVulneravelRepositoryPort):
    ...
    async def list_by_beneficiario(self, beneficiario_id: UUID) -> list[IdosoVulneravel]:
        stmt = (
            select(IdosoVulneravelModel)
            .where(IdosoVulneravelModel.beneficiario_id == beneficiario_id)
            .order_by(IdosoVulneravelModel.data_registro.desc())
        )
    ...
    def _to_domain(model: IdosoVulneravelModel) -> IdosoVulneravel:
        return IdosoVulneravel(
            id=model.id,
            codigo=model.codigo,
            beneficiario_id=model.beneficiario_id,
            citizen_id_idoso=model.citizen_id_idoso,
            idade=model.idade,
            dependencia=model.dependencia,
            precisa_cuidados=model.precisa_cuidados,
