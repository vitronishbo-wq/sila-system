...
from apps.backend.app.modules.society.assistencia_social.domain.enums import PublicoAlvo
...
class ProgramaSocialService:
    ...
    async def criar_programa(
        ...
        *,
        ...
        programa = ProgramaSocial.criar(
            codigo=codigo,
            nome=nome,
            publico_alvo=publico_alvo,
            criterio_renda_max=criterio_renda_max,
    ...
    async def suspender_programa(
        self, programa_id: UUID, motivo: str | None = None
    ) -> ProgramaSocial:
        item = await self.buscar_programa(programa_id)
        item.suspender(motivo)
        return await self.programa_repo.save(item)

    async def encerrar_programa(
        self, programa_id: UUID, *, data_fim: date, motivo: str | None = None
    ) -> ProgramaSocial:
        item = await self.buscar_programa(programa_id)
