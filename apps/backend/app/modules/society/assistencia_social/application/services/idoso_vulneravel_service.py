...
from apps.backend.app.modules.society.assistencia_social.application.ports import BeneficiarioRepositoryPort, IdosoVulneravelRepositoryPort, RequestServicePort, SaudeServicePort
...
class IdosoVulneravelService:
    ...
    def __init__(self, *, idoso_repo: IdosoVulneravelRepositoryPort, beneficiario_repo: BeneficiarioRepositoryPort, saude_service: SaudeServicePort, request_service: RequestServicePort | None=None) -> None:
        self.idoso_repo = idoso_repo
        self.beneficiario_repo = beneficiario_repo
        self.saude_service = saude_service
        self.request_service = request_service

    async def registrar_idoso(self, *, beneficiario_id: UUID, citizen_id_idoso: UUID, idade: int, dependencia: bool, precisa_cuidados: bool) -> IdosoVulneravel:
        ...
        item = IdosoVulneravel.registrar(codigo=codigo, beneficiario_id=beneficiario_id, citizen_id_idoso=citizen_id_idoso, idade=idade, dependencia=dependencia, precisa_cuidados=precisa_cuidados)
        saved = await self.idoso_repo.save(item)
        cobertura_ok = await self.saude_service.verificar_cobertura_idoso(citizen_id=citizen_id_idoso)
        if self.request_service is not None and (not cobertura_ok):
            await self.request_service.create_request(request_type='ASSISTENCIA_IDOSO_SEM_COBERTURA_SAUDE', entity_id=saved.id, citizen_id=beneficiario.citizen_id, numero_processo=saved.codigo)
        return saved
