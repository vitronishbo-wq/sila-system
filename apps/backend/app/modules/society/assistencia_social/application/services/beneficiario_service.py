...
from apps.backend.app.modules.society.assistencia_social.application.ports import BeneficiarioRepositoryPort, CadastroUnicoRepositoryPort, CitizenServicePort, RequestServicePort
...
class BeneficiarioService:
    ...
    def __init__(self, *, beneficiario_repo: BeneficiarioRepositoryPort, citizen_service: CitizenServicePort, cadastro_unico_repo: CadastroUnicoRepositoryPort, request_service: RequestServicePort | None=None) -> None:
    ...
    async def cadastrar_beneficiario(self, *, citizen_id: UUID, faixa_vulnerabilidade: FaixaVulnerabilidade, cadastro_unico_id: UUID | None=None, observacoes: str | None=None) -> Beneficiario:
        ...
        entity = Beneficiario.cadastrar(numero_registro=codigo, citizen_id=citizen_id, cadastro_unico_id=cadastro_unico_id, faixa_vulnerabilidade=faixa_vulnerabilidade, observacoes=observacoes)
        saved = await self.beneficiario_repo.save(entity)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='ASSISTENCIA_CADASTRO_BENEFICIARIO', entity_id=saved.id, citizen_id=saved.citizen_id, numero_processo=saved.numero_registro, metadata={'faixa_vulnerabilidade': saved.faixa_vulnerabilidade.value})
        return saved
    ...
    async def suspender_beneficiario(self, beneficiario_id: UUID, motivo: str | None=None) -> Beneficiario:
        item = await self.buscar_beneficiario(beneficiario_id)
        item.suspender(motivo)
        return await self.beneficiario_repo.save(item)

    async def inativar_beneficiario(self, beneficiario_id: UUID, motivo: str | None=None) -> Beneficiario:
        item = await self.buscar_beneficiario(beneficiario_id)
