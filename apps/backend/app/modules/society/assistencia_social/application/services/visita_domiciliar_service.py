...
from apps.backend.app.modules.society.assistencia_social.application.ports import BeneficiarioRepositoryPort, RequestServicePort, SaudeServicePort, VisitaDomiciliarRepositoryPort
...
class VisitaDomiciliarService:
    ...
    def __init__(self, *, visita_repo: VisitaDomiciliarRepositoryPort, beneficiario_repo: BeneficiarioRepositoryPort, saude_service: SaudeServicePort, request_service: RequestServicePort | None=None) -> None:
        self.visita_repo = visita_repo
        self.beneficiario_repo = beneficiario_repo
        self.saude_service = saude_service
        self.request_service = request_service

    async def registrar_visita(self, *, beneficiario_id: UUID, assistente_social_id: UUID, condicoes_moradia: str, observacoes: str | None=None, recomendacoes: list[str] | None=None, resultado: ResultadoVisita=ResultadoVisita.RETORNO_NECESSARIO) -> VisitaDomiciliar:
        ...
        possui_cobertura = await self.saude_service.verificar_cobertura_idoso(citizen_id=beneficiario.citizen_id)
        ...
        entity = VisitaDomiciliar.registrar(codigo=codigo, beneficiario_id=beneficiario_id, assistente_social_id=assistente_social_id, condicoes_moradia=condicoes_moradia, observacoes=observacoes, recomendacoes=recs, resultado=resultado)
        saved = await self.visita_repo.save(entity)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='ASSISTENCIA_VISITA_DOMICILIAR', entity_id=saved.id, citizen_id=beneficiario.citizen_id, numero_processo=saved.codigo, metadata={'resultado': saved.resultado.value})
        return saved
