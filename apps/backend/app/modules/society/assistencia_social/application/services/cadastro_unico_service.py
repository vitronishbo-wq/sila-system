...
class CadastroUnicoService:
    def __init__(
        self,
        *,
        cadastro_repo: CadastroUnicoRepositoryPort,
        citizen_service: CitizenServicePort,
        educacao_service: EducacaoServicePort,
        juventude_service: JuventudeServicePort,
    ...
    async def registrar_cadastro(
        ...
        *,
    ...
    ) -> tuple[CadastroUnico, list[str], list[str]]:
        ...
        cadastro = CadastroUnico.registrar(
            codigo=codigo,
        ...
        for membro in composicao_familiar:
            ...
            if citizen_id is None:
                continue
            if 4 <= idade <= 17 and (
                not await self.educacao_service.is_estudante_ativo(citizen_id)
            ):
        ...
        if self.request_service is not None:
            for idx, alerta in enumerate(alertas, start=1):
                await self.request_service.create_request(
                    request_type="ASSISTENCIA_ALERTA_FAMILIAR",
                    entity_id=saved.id,
                    citizen_id=saved.citizen_id_responsavel,
                    numero_processo=f"{saved.codigo}-AL{idx:02d}",
                    metadata={"alerta": alerta},
                )
