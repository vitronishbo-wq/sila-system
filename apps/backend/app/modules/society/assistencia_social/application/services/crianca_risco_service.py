...
from apps.backend.app.modules.society.assistencia_social.application.ports import BeneficiarioRepositoryPort, CriancaRiscoRepositoryPort, EducacaoServicePort, RequestServicePort
...
class CriancaRiscoService:
    ...
    def __init__(self, *, crianca_repo: CriancaRiscoRepositoryPort, beneficiario_repo: BeneficiarioRepositoryPort, educacao_service: EducacaoServicePort, request_service: RequestServicePort | None=None) -> None:
        self.crianca_repo = crianca_repo
        self.beneficiario_repo = beneficiario_repo
        self.educacao_service = educacao_service
        self.request_service = request_service

    async def registrar_crianca_risco(self, *, beneficiario_id: UUID, citizen_id_crianca: UUID, idade: int, motivo: str, escolarizada: bool) -> CriancaRisco:
        ...
        item = CriancaRisco.registrar(codigo=codigo, beneficiario_id=beneficiario_id, citizen_id_crianca=citizen_id_crianca, idade=idade, motivo=motivo, escolarizada=status_escolar)
        saved = await self.crianca_repo.save(item)
        if self.request_service is not None and (not status_escolar):
            await self.request_service.create_request(request_type='ASSISTENCIA_CRIANCA_FORA_ESCOLA', entity_id=saved.id, citizen_id=beneficiario.citizen_id, numero_processo=saved.codigo, metadata={'citizen_id_crianca': str(citizen_id_crianca)})
        return saved
    ...
    async def listar_criancas_risco(self, beneficiario_id: UUID | None=None) -> list[CriancaRisco]:
