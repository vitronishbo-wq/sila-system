...
class SituacaoRuaService:
    ...
    def __init__(self, *, situacao_repo: SituacaoRuaRepositoryPort, beneficiario_repo: BeneficiarioRepositoryPort, request_service: RequestServicePort | None=None) -> None:
        self.situacao_repo = situacao_repo
        self.beneficiario_repo = beneficiario_repo
        self.request_service = request_service

    async def registrar_situacao(self, *, beneficiario_id: UUID, localizacao: str, motivo: str) -> SituacaoRua:
        ...
        item = SituacaoRua.registrar(codigo=codigo, beneficiario_id=beneficiario_id, localizacao=localizacao, motivo=motivo)
        saved = await self.situacao_repo.save(item)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='ASSISTENCIA_SITUACAO_RUA', entity_id=saved.id, citizen_id=beneficiario.citizen_id, numero_processo=saved.codigo)
        return saved
