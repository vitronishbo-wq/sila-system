...
from apps.backend.app.modules.society.assistencia_social.application.ports import BeneficiarioRepositoryPort, PCDRepositoryPort, RequestServicePort, SaudeServicePort
...
class PCDService:
    ...
    def __init__(self, *, pcd_repo: PCDRepositoryPort, beneficiario_repo: BeneficiarioRepositoryPort, saude_service: SaudeServicePort, request_service: RequestServicePort | None=None) -> None:
        self.pcd_repo = pcd_repo
        self.beneficiario_repo = beneficiario_repo
        self.saude_service = saude_service
        self.request_service = request_service

    async def registrar_pcd(self, *, beneficiario_id: UUID, citizen_id_pcd: UUID, tipo_deficiencia: str, cid: str, grau_deficiencia: str, laudo_id: UUID) -> PessoaComDeficiencia:
        ...
        laudo_valido = await self.saude_service.validar_laudo_pcd(citizen_id=citizen_id_pcd, laudo_id=laudo_id, cid=cid)
        if not laudo_valido:
            raise ValueError('Laudo medico invalido para registro PcD')
        codigo = next_codigo('PCD', len(await self.pcd_repo.list_all()))
        item = PessoaComDeficiencia.registrar(codigo=codigo, beneficiario_id=beneficiario_id, citizen_id_pcd=citizen_id_pcd, tipo_deficiencia=tipo_deficiencia, cid=cid, grau_deficiencia=grau_deficiencia, laudo_id=laudo_id)
        saved = await self.pcd_repo.save(item)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='ASSISTENCIA_REGISTRO_PCD', entity_id=saved.id, citizen_id=beneficiario.citizen_id, numero_processo=saved.codigo, metadata={'cid': saved.cid, 'grau_deficiencia': saved.grau_deficiencia})
        return saved
