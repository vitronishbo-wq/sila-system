...
class BeneficioService:
    ...
    def __init__(self, *, beneficio_repo: BeneficioRepositoryPort, beneficiario_repo: BeneficiarioRepositoryPort, programa_repo: ProgramaSocialRepositoryPort, pcd_repo: PCDRepositoryPort, saude_service: SaudeServicePort, emprego_service: EmpregoServicePort, request_service: RequestServicePort | None=None) -> None:
    ...
    async def solicitar_beneficio(self, *, beneficiario_id: UUID, tipo: TipoBeneficio, valor: Decimal, programa_social_id: UUID | None=None) -> Beneficio:
        ...
        entity = Beneficio.solicitar(codigo=codigo, beneficiario_id=beneficiario_id, programa_social_id=programa_social_id, tipo=tipo, valor=valor)
        saved = await self.beneficio_repo.save(entity)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='ASSISTENCIA_SOLICITACAO_BENEFICIO', entity_id=saved.id, citizen_id=beneficiario.citizen_id, numero_processo=saved.codigo, metadata={'tipo': saved.tipo.value, 'valor': str(saved.valor)})
        return saved

    async def conceder_bpc_pcd(self, *, beneficiario_id: UUID, pcd_id: UUID, valor: Decimal) -> Beneficio:
        pcd = await self.pcd_repo.get_by_id(pcd_id)
        if pcd is None:
            raise ValueError('Registro PcD nao encontrado')
        if not await self.saude_service.validar_laudo_pcd(citizen_id=pcd.citizen_id_pcd, laudo_id=pcd.laudo_id, cid=pcd.cid):
        ...
        beneficio = await self.solicitar_beneficio(beneficiario_id=beneficiario_id, tipo=TipoBeneficio.BPC_PCD, valor=valor)
        beneficio.aprovar()
