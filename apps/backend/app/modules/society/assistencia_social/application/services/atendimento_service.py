...
from app.modules.society.assistencia_social.application.ports import AtendimentoRepositoryPort, BeneficiarioRepositoryPort, RequestServicePort
from app.modules.society.assistencia_social.application.services._codegen import next_codigo
from app.modules.society.assistencia_social.domain.enums import TipoAtendimento
from app.modules.society.assistencia_social.domain.models import Atendimento

class AtendimentoService:

    def __init__(self, *, atendimento_repo: AtendimentoRepositoryPort, beneficiario_repo: BeneficiarioRepositoryPort, request_service: RequestServicePort | None=None) -> None:
        self.atendimento_repo = atendimento_repo
        self.beneficiario_repo = beneficiario_repo
        self.request_service = request_service

    async def registrar_atendimento(self, *, beneficiario_id: UUID, tipo: TipoAtendimento, descricao: str, responsavel_id: UUID, encaminhamentos: list[dict] | None=None) -> Atendimento:
        ...
        entity = Atendimento.registrar(codigo=codigo, beneficiario_id=beneficiario_id, tipo=tipo, descricao=descricao, responsavel_id=responsavel_id, encaminhamentos=encaminhamentos)
        saved = await self.atendimento_repo.save(entity)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='ASSISTENCIA_ATENDIMENTO', entity_id=saved.id, citizen_id=beneficiario.citizen_id, numero_processo=saved.codigo, metadata={'tipo': saved.tipo.value})
        return saved
