...
from apps.backend.app.modules.society.domain.events import (
    WelfareApplicationSubmitted,
    WelfareEligibilityApproved,
    ...
    async def handle(
        ...
        try:
            event = WelfareApplicationSubmitted(
                ...
                aggregate_type="WelfareApplication",
                event_type="WelfareApplicationSubmitted",
                ...
                metadata={
                    ...
                    "command_name": "SubmitWelfareApplication",
...
class ApproveWelfareEligibilityCommandHandler:
    ...
    async def handle(
        ...
        application_id: UUID,
        approved: bool,
        monthly_benefit_amount: Decimal = Decimal("0"),
        ...
        try:
            event = WelfareEligibilityApproved(
                aggregate_id=approval_id,
                aggregate_type="WelfareEligibility",
                event_type="WelfareEligibilityApproved",
                ...
                metadata={
                    "user_id": str(user_id) if user_id else None,
                    "command_name": "ApproveWelfareEligibility",
                    "timestamp_utc": datetime.now(UTC).isoformat(),
                },
            )
            await self.event_bus.publish(event)
    ...
    async def handle(
        ...
        try:
            event = WelfarePaymentProcessed(
                ...
                aggregate_type="WelfarePayment",
                event_type="WelfarePaymentProcessed",
                ...
                metadata={
                    "user_id": str(user_id) if user_id else None,
                    "command_name": "ProcessWelfarePayment",
                    "timestamp_utc": datetime.now(UTC).isoformat(),
                },
            )
            await self.event_bus.publish(event)
