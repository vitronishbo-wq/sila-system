...
def build_workflow_router(*, tag: str, get_service: Callable, create_schema: Any, action_schema: Any, cancel_schema: Any, response_schema: Any, routes: list[tuple[str, str]]) -> APIRouter:
    router = APIRouter(tags=[tag])

    def _register_create(path: str, service_type: str) -> None:

        async def _create(data: create_schema, service=Depends(get_service)):
            try:
                return await service.criar_registro(service_type=service_type, citizen_id=data.citizen_id, observacoes=data.observacoes, metadata=data.metadata)
