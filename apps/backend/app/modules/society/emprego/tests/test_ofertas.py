...
async def test_criar_oferta_sucesso():
    citizen_id = uuid4()
    repository = SimpleNamespace(exists_active_for_citizen=AsyncMock(return_value=False), next_numero_processo=AsyncMock(return_value='OFER/2026/0001'), save=AsyncMock(side_effect=lambda item: item))
    ...
    service = OfertaService(repository=repository, citizen_service=citizen_service, request_service=request_service)
    record = await service.criar_registro(service_type='oferta_emprego', citizen_id=citizen_id, observacoes='Oferta para atendimento geral', metadata={'setor': 'servicos'})
...
def test_endpoint_criar_oferta_retorna_201():
    citizen_id = uuid4()
    item = WorkflowEmpregoRecord(id=uuid4(), numero_processo='OFER/2026/0004', citizen_id=citizen_id, data_registro=date.today(), service_type='oferta_emprego')
    service = SimpleNamespace(criar_registro=AsyncMock(return_value=item))
    client = _build_client(service)
    response = client.post('/emprego/ofertas', json={'citizen_id': str(citizen_id), 'observacoes': 'Oferta publica', 'metadata': {'setor': 'servicos'}})
...
def test_endpoint_concluir_oferta_retorna_200():
    item_id = uuid4()
    citizen_id = uuid4()
    concluido = WorkflowEmpregoRecord(id=item_id, numero_processo='OFER/2026/0004', citizen_id=citizen_id, data_registro=date.today(), service_type='oferta_emprego', status=WorkflowStatus.CONCLUIDA, observacoes='Concluida no portal')
    service = SimpleNamespace(concluir_registro=AsyncMock(return_value=concluido))
    client = _build_client(service)
    response = client.post(f'/emprego/workflow/{item_id}/concluir', json={'actor_id': str(uuid4()), 'observacoes': 'Concluida no portal'})
