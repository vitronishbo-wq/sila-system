...
from app.modules.society.emprego.api.deps import get_formacao_service
from app.modules.society.emprego.api.endpoints.formacoes import router as formacoes_router
from app.modules.society.emprego.application.services.formacao_service import FormacaoService
...
async def test_cancelar_formacao_define_status_e_motivo():
    item = WorkflowEmpregoRecord(id=uuid4(), numero_processo='FORM/2026/0008', citizen_id=uuid4(), data_registro=date.today(), service_type='formacao_profissional')
    ...
    updated = await service.cancelar_registro(item_id=item.id, actor_id=uuid4(), motivo='Documentacao incompleta')
...
def test_endpoint_cancelar_formacao_retorna_200():
    item_id = uuid4()
    item = WorkflowEmpregoRecord(id=item_id, numero_processo='FORM/2026/0010', citizen_id=uuid4(), data_registro=date.today(), service_type='formacao_profissional', status=WorkflowStatus.CANCELADA, observacoes='Cancelada')
    service = SimpleNamespace(cancelar_registro=AsyncMock(return_value=item))
    client = _build_client(service)
    response = client.post(f'/emprego/workflow/{item_id}/cancelar', json={'actor_id': str(uuid4()), 'motivo': 'Cancelada'})
