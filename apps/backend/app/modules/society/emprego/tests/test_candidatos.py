...
from apps.backend.app.modules.society.emprego.api.endpoints.candidatos import (
...
def _build_service(candidato_repo, citizen_repo, request_service) -> CandidatoService:
    return CandidatoService(
        candidato_repo=candidato_repo, citizen_repo=citizen_repo, request_service=request_service
    )
...
async def test_registrar_candidato_sucesso():
    citizen_id = uuid4()
    candidato_repo = SimpleNamespace(
        get_by_citizen=AsyncMock(return_value=None),
        next_numero_processo=AsyncMock(return_value="CAND/2026/0001"),
        save=AsyncMock(side_effect=lambda candidato: candidato),
    )
    ...
    result = await service.registrar_candidato(
        citizen_id=citizen_id,
        escolaridade=Escolaridade.SECUNDARIA,
        situacao=SituacaoProfissional.DESEMPREGADO,
...
async def test_registrar_candidato_cidadao_inexistente():
    ...
    with pytest.raises(CitizenNotFoundError):
        await service.registrar_candidato(
            citizen_id=uuid4(),
            escolaridade=Escolaridade.SECUNDARIA,
            situacao=SituacaoProfissional.DESEMPREGADO,
            areas_interesse=["administracao"],
        )
...
async def test_registrar_candidato_bloqueia_duplicado_ativo():
    citizen_id = uuid4()
    existente = Candidato.criar(
        citizen_id=citizen_id,
        escolaridade=Escolaridade.BASICA,
        situacao=SituacaoProfissional.EMPREGADO,
        areas_interesse=["logistica"],
        numero_processo="CAND/2026/0007",
    )
    candidato_repo = SimpleNamespace(
...
async def test_desativar_candidato_conclui_solicitacao():
    citizen_id = uuid4()
    actor_id = uuid4()
    candidato = Candidato.criar(
        citizen_id=citizen_id,
        escolaridade=Escolaridade.TECNICA,
        situacao=SituacaoProfissional.PRIMEIRO_EMPREGO,
        areas_interesse=["suporte"],
        numero_processo="CAND/2026/0011",
    )
    candidato_repo = SimpleNamespace(
        get_by_id=AsyncMock(return_value=candidato), save=AsyncMock(side_effect=lambda item: item)
    )
    ...
    service = _build_service(candidato_repo, citizen_repo, request_service)
    updated = await service.desativar_candidato(
        candidato_id=candidato.id, motivo="Solicitado pelo candidato", actor_id=actor_id
    )
...
def test_endpoint_registrar_candidato_retorna_201():
    ...
    candidato = Candidato.criar(
        ...
        situacao=SituacaoProfissional.DESEMPREGADO,
        areas_interesse=["administracao"],
        numero_processo="CAND/2026/0002",
    )
...
def test_endpoint_registrar_candidato_retorna_404_para_cidadao_inexistente():
    service = SimpleNamespace(
        registrar_candidato=AsyncMock(side_effect=CitizenNotFoundError("Cidadao nao encontrado"))
    )
    client = _build_client(service)
    response = client.post(
        "/emprego/candidatos/",
