...
from apps.backend.app.modules.society.emprego.api.endpoints.concursos import (
    router as concursos_router,
)
from apps.backend.app.modules.society.emprego.application.services.concurso_service import (
    ConcursoService,
)
...
async def test_criar_concurso_bloqueia_duplicidade_ativa():
    ...
    with pytest.raises(ValueError, match="registro ativo"):
        await service.criar_registro(
            service_type="concurso_publico",
            citizen_id=uuid4(),
            metadata={"concurso": "tecnico superior"},
        )
