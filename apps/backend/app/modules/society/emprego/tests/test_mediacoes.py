...
from apps.backend.app.modules.society.emprego.api.endpoints.mediacoes import (
    router as mediacoes_router,
)
from apps.backend.app.modules.society.emprego.application.services.mediacao_service import (
    MediacaoService,
)
...
async def test_concluir_mediacao_sucesso():
    item = WorkflowEmpregoRecord(
        id=uuid4(),
        numero_processo="MEDI/2026/0003",
        citizen_id=uuid4(),
        data_registro=date.today(),
        service_type="mediacao",
        status=WorkflowStatus.EM_ANALISE,
    )
    ...
    updated = await service.concluir_registro(
        item_id=item.id, actor_id=uuid4(), observacoes="Acordo concluido"
    )
...
def test_endpoint_listar_mediacoes_por_cidadao():
    ...
    records = [
        WorkflowEmpregoRecord(
            id=uuid4(),
            numero_processo="MEDI/2026/0009",
