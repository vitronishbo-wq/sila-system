...
from apps.backend.app.modules.society.emprego.api.endpoints.certificacoes import (
    router as certificacoes_router,
)
from apps.backend.app.modules.society.emprego.application.services.certificacao_service import (
    CertificacaoService,
)
...
async def test_listar_certificacoes_por_cidadao():
    ...
    records = [
        WorkflowEmpregoRecord(
            id=uuid4(),
            numero_processo="CERT/2026/0004",
            citizen_id=citizen_id,
            data_registro=date.today(),
            service_type="certificacao_profissional",
        )
    ]
...
def test_endpoint_listar_certificacoes_por_cidadao_retorna_200():
    ...
    records = [
        WorkflowEmpregoRecord(
            id=uuid4(),
            numero_processo="CERT/2026/0020",
            citizen_id=citizen_id,
            data_registro=date.today(),
            service_type="certificacao_profissional",
        )
