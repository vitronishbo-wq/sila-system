...
class InMemoryRequestService(RequestServicePort):
    ...
    async def create_request(self, *, request_type: str, entity_id: UUID, citizen_id: UUID, numero_processo: str, metadata: dict | None=None):
        self.created.append({'request_type': request_type, 'entity_id': entity_id, 'citizen_id': citizen_id, 'numero_processo': numero_processo, 'metadata': metadata or {}})
        return uuid4()

    async def complete_request(self, *, entity_id: UUID, actor_id: UUID, metadata: dict | None=None) -> bool:
        self.completed.append({'entity_id': entity_id, 'actor_id': actor_id, 'metadata': metadata or {}})
        return True
...
class InMemoryWorkflowRepository(WorkflowRepositoryPort):
    ...
    async def exists_active_for_citizen(self, citizen_id: UUID, service_type: str) -> bool:
        ...
        return any((item.citizen_id == citizen_id and item.service_type == service_type and (item.status in active) for item in self.by_id.values()))
...
@pytest.fixture
def e2e_client():
    citizens = {'candidato': uuid4(), 'oferta': uuid4(), 'mediacao': uuid4(), 'formacao': uuid4(), 'trabalho': uuid4(), 'concurso': uuid4(), 'certificacao': uuid4()}
    ...
    candidato_service = CandidatoService(candidato_repo=candidato_repo, citizen_repo=citizen_service, request_service=request_service)
    oferta_service = OfertaService(repository=workflow_repo, citizen_service=citizen_service, request_service=request_service)
    mediacao_service = MediacaoService(repository=workflow_repo, citizen_service=citizen_service, request_service=request_service)
    formacao_service = FormacaoService(repository=workflow_repo, citizen_service=citizen_service, request_service=request_service)
    trabalho_service = TrabalhoService(repository=workflow_repo, citizen_service=citizen_service, request_service=request_service)
    concurso_service = ConcursoService(repository=workflow_repo, citizen_service=citizen_service, request_service=request_service)
    certificacao_service = CertificacaoService(repository=workflow_repo, citizen_service=citizen_service, request_service=request_service)
    app = FastAPI()
...
def test_e2e_candidatos_fluxo_completo(e2e_client):
    ...
    created = client.post('/emprego/candidatos/', json={'citizen_id': str(citizen_id), 'escolaridade': Escolaridade.SECUNDARIA.value, 'situacao': SituacaoProfissional.DESEMPREGADO.value, 'areas_interesse': ['administracao', 'vendas']})
    ...
    listed = client.get('/emprego/candidatos', params={'situacao': SituacaoProfissional.DESEMPREGADO.value})
    assert listed.status_code == 200
    assert any((item['id'] == candidato_id for item in listed.json()))
    deactivated = client.post(f'/emprego/candidatos/{candidato_id}/desativar', json={'actor_id': str(uuid4()), 'motivo': 'Encerramento voluntario'})
...
def test_e2e_ofertas_fluxo_criar_consultar_concluir(e2e_client):
    ...
    created = client.post('/emprego/ofertas', json={'citizen_id': str(citizen_id), 'observacoes': 'Oferta de emprego', 'metadata': {'setor': 'servicos'}})
    assert created.status_code == 201
    created_payload = created.json()
    ...
    concluded = client.post(f'/emprego/workflow/{item_id}/concluir', json={'actor_id': str(uuid4()), 'observacoes': 'Processo concluido'})
...
def test_e2e_fluxo_criacao_por_dominios_workflow(e2e_client):
    client, citizens, _ = e2e_client
    cases = [('/emprego/mediacoes', citizens['mediacao'], 'mediacao', 'MEDI'), ('/emprego/formacoes/profissional', citizens['formacao'], 'formacao_profissional', 'FORM'), ('/emprego/trabalho/reclamacoes', citizens['trabalho'], 'reclamacao_trabalhista', 'TRAB'), ('/emprego/concursos/inscricoes', citizens['concurso'], 'concurso_publico', 'CONC'), ('/emprego/certificacoes/profissionais', citizens['certificacao'], 'certificacao_profissional', 'CERT')]
    for path, citizen_id, service_type, prefix in cases:
        response = client.post(path, json={'citizen_id': str(citizen_id), 'metadata': {'origem': 'e2e'}})
