...
from apps.backend.app.modules.society.emprego.api.endpoints.trabalhistas import (
    router as trabalhistas_router,
)
from apps.backend.app.modules.society.emprego.application.services.trabalho_service import (
    TrabalhoService,
)
...
async def test_criar_registro_trabalhista_rejeita_cidadao_inativo():
    ...
    with pytest.raises(ValueError, match="nao encontrado ou inativo"):
        await service.criar_registro(
            service_type="reclamacao_trabalhista",
            citizen_id=uuid4(),
            observacoes="Queixa formal",
...
def test_endpoint_criar_trabalhista_retorna_400_para_regra_negocio():
    ...
    response = client.post(
        "/emprego/trabalho/reclamacoes",
        json={"citizen_id": str(uuid4()), "metadata": {"origem": "portal"}},
    )
