...
class CandidatoService:
    def __init__(
        self,
        *,
        candidato_repo: CandidatoRepositoryPort,
    ...
    async def registrar_candidato(
        ...
        *,
        ...
        await self.request_service.create_request(
            request_type="registro_candidato_emprego",
            entity_id=saved.id,
            citizen_id=saved.citizen_id,
            numero_processo=saved.numero_processo,
            metadata={
                "escolaridade": saved.escolaridade.value,
                "situacao": saved.situacao.value,
                "areas_interesse": saved.areas_interesse,
            },
        )
        return saved

    async def obter_por_id(self, candidato_id: UUID) -> Candidato | None:
    ...
    async def buscar_candidatos(
        ...
        *,
        ...
        return await self.candidato_repo.list_by_filtros(
            escolaridade=escolaridade, situacao=situacao, area_interesse=area
        )

    async def desativar_candidato(
        self, *, candidato_id: UUID, motivo: str, actor_id: UUID
    ) -> Candidato:
        ...
        await self.request_service.complete_request(
            ...
            actor_id=actor_id,
            metadata={"status": updated.status.value, "motivo": motivo},
        )
        return updated
