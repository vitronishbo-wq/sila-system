...
class WorkflowService:
    ...
    async def concluir_registro(
        self, *, item_id: UUID, actor_id: UUID, observacoes: str | None = None
    ) -> WorkflowEmpregoRecord:
        ...
        if self.request_service is not None:
            await self.request_service.complete_request(
                entity_id=item_id,
                actor_id=actor_id,
                metadata={"service_type": updated.service_type, "status": updated.status.value},
            )
        return updated

    async def cancelar_registro(
        self, *, item_id: UUID, actor_id: UUID, motivo: str
    ) -> WorkflowEmpregoRecord:
        item = await self.repository.get_by_id(item_id)
        if not item:
            raise ValueError("Registro nao encontrado")
