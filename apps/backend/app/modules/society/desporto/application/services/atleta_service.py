...
class AtletaService:
    ...
    async def cadastrar_atleta(self, *, nome: str, data_nascimento: date, naturalidade: str, nacionalidade: str, tipo: TipoAtleta, modalidades: list[ModalidadeDesportiva], citizen_id: UUID | None=None, posicoes: list[PosicaoAtleta] | None=None, pe_preferencial: PePreferencial | None=None, altura_cm: int | None=None, peso_kg: Decimal | None=None, clube_atual_id: UUID | None=None, numero_camisola: int | None=None, ultimo_exame_id: UUID | None=None, observacoes: str | None=None) -> Atleta:
        ...
        atleta = Atleta.cadastrar(numero_registro=registro, nome=nome, data_nascimento=data_nascimento, naturalidade=naturalidade, nacionalidade=nacionalidade, tipo=tipo, modalidades=modalidades, citizen_id=citizen_id, posicoes=posicoes, pe_preferencial=pe_preferencial, altura_cm=altura_cm, peso_kg=peso_kg, clube_atual_id=clube_atual_id, numero_camisola=numero_camisola, ultimo_exame_id=ultimo_exame_id, observacoes=observacoes)
        saved = await self.atleta_repo.save(atleta)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='CADASTRO_ATLETA', entity_id=saved.id, citizen_id=citizen_id, metadata={'registro': saved.numero_registro, 'nome': saved.nome, 'tipo': saved.tipo.value, 'modalidades': [item.value for item in saved.modalidades]}, numero_processo=saved.numero_registro)
        return saved
    ...
    async def listar_atletas(self, *, tipo: TipoAtleta | None=None, modalidade: ModalidadeDesportiva | None=None, status: StatusAtleta | None=None, clube_id: UUID | None=None, somente_ativos: bool=True) -> list[Atleta]:
    ...
    async def atualizar_atleta(self, *, atleta_id: UUID, nome: str | None=None, tipo: TipoAtleta | None=None, modalidades: list[ModalidadeDesportiva] | None=None, posicoes: list[PosicaoAtleta] | None=None, pe_preferencial: PePreferencial | None=None, altura_cm: int | None=None, peso_kg: Decimal | None=None, clube_atual_id: UUID | None=None, numero_camisola: int | None=None, status: StatusAtleta | None=None, ultimo_exame_id: UUID | None=None, ativo: bool | None=None, observacoes: str | None=None) -> Atleta:
        item = await self.buscar_atleta(atleta_id)
        if ultimo_exame_id is not None and self.saude_service is not None:
            exame_ok = await self.saude_service.exame_exists(ultimo_exame_id)
            if not exame_ok:
                raise ValueError('Exame medico informado nao encontrado')
        item.atualizar(nome=nome, tipo=tipo, modalidades=modalidades, posicoes=posicoes, pe_preferencial=pe_preferencial, altura_cm=altura_cm, peso_kg=peso_kg, clube_atual_id=clube_atual_id, numero_camisola=numero_camisola, status=status, ultimo_exame_id=ultimo_exame_id, ativo=ativo, observacoes=observacoes)
