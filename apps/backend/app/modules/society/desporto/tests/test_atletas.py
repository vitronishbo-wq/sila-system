...
def test_cadastrar_atleta_sucesso() -> None:
    ...
    async def scenario() -> None:
        service = AtletaService(
            atleta_repo=InMemoryAtletaRepository(),
            citizen_service=FakeCitizenService(active=True),
            saude_service=FakeSaudeService(exists=True),
            request_service=FakeRequestService(),
        )
        result = await service.cadastrar_atleta(
            nome="Joao Atleta",
...
def test_cadastrar_atleta_falha_cidadao_inativo() -> None:
    ...
    async def scenario() -> None:
        service = AtletaService(
            atleta_repo=InMemoryAtletaRepository(),
            citizen_service=FakeCitizenService(active=False),
            saude_service=FakeSaudeService(exists=True),
            request_service=FakeRequestService(),
        )
        with pytest.raises(ValueError, match="Cidadao"):
            await service.cadastrar_atleta(
                nome="Atleta Invalido",
                data_nascimento=date.today() - timedelta(days=365 * 20),
                naturalidade="Luanda",
                nacionalidade="Angolana",
                tipo=TipoAtleta.AMADOR,
                modalidades=[ModalidadeDesportiva.ATLETISMO],
                citizen_id=uuid4(),
            )
...
def test_registrar_lesao_e_recuperar() -> None:
    ...
    async def scenario() -> None:
        ...
        atleta = await service.cadastrar_atleta(
            nome="Atleta Teste",
            data_nascimento=date.today() - timedelta(days=365 * 25),
            naturalidade="Benguela",
            nacionalidade="Angolana",
            tipo=TipoAtleta.PROFISSIONAL,
            modalidades=[ModalidadeDesportiva.BASQUETEBOL],
        )
