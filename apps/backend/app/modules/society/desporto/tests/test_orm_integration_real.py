...
from apps.backend.app.modules.society.desporto.tests._fakes import (
    FakeEducacaoService,
    FakeObrasPublicasService,
    FakeTurismoService,
)


def _tables():
    from apps.backend.app.modules.society.desporto.infrastructure.models.atleta_model import (
        AtletaModel,
    )
    from apps.backend.app.modules.society.desporto.infrastructure.models.clube_model import (
        ClubeModel,
    )
    ...
    return [
        AtletaModel.__table__,
        CompeticaoModel.__table__,
        ClubeModel.__table__,
        JogoModel.__table__,
    ]
...
def test_fluxo_real_orm_atleta_competicao() -> None:
    ...
    async def scenario() -> None:
        async with _session_scope() as session:
            ...
            competicao_service = CompeticaoService(
                ...
                obras_publicas_service=FakeObrasPublicasService(exists=True),
            )
            ...
            atleta = await atleta_service.cadastrar_atleta(
                nome="Atleta Integracao",
                data_nascimento=date.today() - timedelta(days=365 * 24),
                naturalidade="Luanda",
                nacionalidade="Angolana",
                tipo=TipoAtleta.PROFISSIONAL,
                modalidades=[ModalidadeDesportiva.FUTEBOL],
            )
            competicao = await competicao_service.cadastrar_competicao(
                nome="Liga Integracao",
                tipo=TipoCompeticao.LIGA,
                modalidade=ModalidadeDesportiva.FUTEBOL,
            ...
            competicao_pub = await competicao_service.atualizar_competicao(
                competicao_id=competicao.id, status=StatusCompeticao.EM_ANDAMENTO
            )
            clube_casa = await clube_service.cadastrar_clube(
                nome="Clube ORM A",
                sigla="ORMA",
                tipo=TipoClube.PROFISSIONAL,
                modalidade_principal=ModalidadeDesportiva.FUTEBOL,
                municipio="Luanda",
                provincia="Luanda",
            )
            clube_fora = await clube_service.cadastrar_clube(
                nome="Clube ORM B",
                sigla="ORMB",
                tipo=TipoClube.PROFISSIONAL,
                modalidade_principal=ModalidadeDesportiva.FUTEBOL,
            ...
            jogo = await jogo_service.agendar_jogo(
                competicao_id=competicao.id,
                clube_casa_id=clube_casa.id,
                clube_fora_id=clube_fora.id,
                data_jogo=date(2026, 9, 12),
                local="Estadio Integracao",
                municipio="Luanda",
                provincia="Luanda",
            )
            jogo_final = await jogo_service.registrar_resultado(
                jogo_id=jogo.id, placar_casa=1, placar_fora=0
            )
