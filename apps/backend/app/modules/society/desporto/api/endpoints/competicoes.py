...
from apps.backend.app.modules.society.desporto.api.schemas.competicao_schema import (
    CompeticaoCreate,
    CompeticaoResponse,
    CompeticaoUpdate,
)
...
competicao_service_dep = Depends(get_competicao_service)


@router.post("/", response_model=CompeticaoResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_competicao(
    data: CompeticaoCreate, service: CompeticaoService = competicao_service_dep
) -> CompeticaoResponse:
    try:
...
async def obter_competicao(
    competicao_id: UUID, service: CompeticaoService = competicao_service_dep
) -> CompeticaoResponse:
    try:
...
async def listar_competicoes(
    ...
    status_filtro: StatusCompeticao | None = None,
    data_inicio: date | None = None,
    data_fim: date | None = None,
    somente_ativas: bool = True,
    service: CompeticaoService = competicao_service_dep,
) -> list[CompeticaoResponse]:
...
async def atualizar_competicao(
    competicao_id: UUID,
    data: CompeticaoUpdate,
    service: CompeticaoService = competicao_service_dep,
) -> CompeticaoResponse:
    try:
        return await service.atualizar_competicao(
