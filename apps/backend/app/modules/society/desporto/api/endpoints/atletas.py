...
from apps.backend.app.modules.society.desporto.api.schemas.atleta_schema import (
    AtletaCreate,
    AtletaResponse,
    AtletaUpdate,
)
...
atleta_service_dep = Depends(get_atleta_service)


@router.post("/", response_model=AtletaResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_atleta(
    data: AtletaCreate, service: AtletaService = atleta_service_dep
) -> AtletaResponse:
    try:
        return await service.cadastrar_atleta(
            nome=data.nome,
            ...
            pe_preferencial=data.pe_preferencial,
            altura_cm=data.altura_cm,
            peso_kg=data.peso_kg,
            clube_atual_id=data.clube_atual_id,
            numero_camisola=data.numero_camisola,
            ultimo_exame_id=data.ultimo_exame_id,
...
async def obter_atleta(
    atleta_id: UUID, service: AtletaService = atleta_service_dep
) -> AtletaResponse:
    try:
...
async def listar_atletas(
    ...
    status_filtro: StatusAtleta | None = None,
    clube_id: UUID | None = None,
    somente_ativos: bool = True,
    service: AtletaService = atleta_service_dep,
) -> list[AtletaResponse]:
    return await service.listar_atletas(
        tipo=tipo,
...
async def atualizar_atleta(
    atleta_id: UUID, data: AtletaUpdate, service: AtletaService = atleta_service_dep
) -> AtletaResponse:
    try:
