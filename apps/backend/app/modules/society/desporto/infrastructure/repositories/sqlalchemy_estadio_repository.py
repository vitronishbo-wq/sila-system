...
from apps.backend.app.modules.society.desporto.infrastructure.models.estadio_model import (
    EstadioModel,
)
...
class SQLAlchemyEstadioRepository(EstadioRepositoryPort):
    ...
    async def list_by_municipio(self, municipio: str) -> list[Estadio]:
        stmt = (
            select(EstadioModel)
            .where(func.lower(EstadioModel.municipio) == municipio.strip().lower())
            .order_by(EstadioModel.nome.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(EstadioModel)
            .where(EstadioModel.codigo_estadio.like(f"EST/{year}/%"))
        )
    ...
    def _to_domain(model: EstadioModel) -> Estadio:
        return Estadio(
            id=model.id,
            codigo_estadio=model.codigo_estadio,
            nome=model.nome,
            tipo=TipoEstadio(model.tipo),
            municipio=model.municipio,
            provincia=model.provincia,
            capacidade=model.capacidade,
            estado_relvado=EstadoRelvado(model.estado_relvado),
            data_cadastro=model.data_cadastro,
            codigo_obra_instalacao=model.codigo_obra_instalacao,
            clube_mandante_id=model.clube_mandante_id,
            ativo=model.ativo,
