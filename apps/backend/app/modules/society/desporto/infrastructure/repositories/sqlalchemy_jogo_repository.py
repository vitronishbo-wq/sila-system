...
class SQLAlchemyJogoRepository(JogoRepositoryPort):
    ...
    async def list_by_competicao(self, competicao_id: UUID) -> list[Jogo]:
        stmt = (
            select(JogoModel)
            .where(JogoModel.competicao_id == competicao_id)
            .order_by(JogoModel.data_jogo.asc(), JogoModel.codigo_jogo.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_clube(self, clube_id: UUID) -> list[Jogo]:
        stmt = (
            select(JogoModel)
            .where(or_(JogoModel.clube_casa_id == clube_id, JogoModel.clube_fora_id == clube_id))
            .order_by(JogoModel.data_jogo.asc(), JogoModel.codigo_jogo.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_status(self, status: StatusJogo) -> list[Jogo]:
        stmt = (
            select(JogoModel)
            .where(JogoModel.status == status.value)
            .order_by(JogoModel.data_jogo.asc(), JogoModel.codigo_jogo.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_periodo(self, data_inicio: date, data_fim: date) -> list[Jogo]:
        stmt = (
            select(JogoModel)
            .where(JogoModel.data_jogo >= data_inicio)
            .where(JogoModel.data_jogo <= data_fim)
            .order_by(JogoModel.data_jogo.asc(), JogoModel.codigo_jogo.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(JogoModel)
            .where(JogoModel.codigo_jogo.like(f"JOG/{ano}/%"))
        )
    ...
    def _to_domain(model: JogoModel) -> Jogo:
        return Jogo(
            id=model.id,
            codigo_jogo=model.codigo_jogo,
            competicao_id=model.competicao_id,
            clube_casa_id=model.clube_casa_id,
            clube_fora_id=model.clube_fora_id,
            data_jogo=model.data_jogo,
            local=model.local,
            municipio=model.municipio,
            provincia=model.provincia,
            data_cadastro=model.data_cadastro,
            status=StatusJogo(model.status),
            placar_casa=model.placar_casa,
            placar_fora=model.placar_fora,
            ...
            publico_presente=model.publico_presente,
            ativo=model.ativo,
