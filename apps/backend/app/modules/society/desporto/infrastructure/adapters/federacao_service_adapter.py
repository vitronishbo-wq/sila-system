...
from apps.backend.app.modules.society.desporto.infrastructure.resilience.circuit_breaker import (
    circuit_breaker,
)
...
class FederacaoServiceAdapter(FederacaoServicePort):
    ...
    async def validar_clube_federado(self, codigo_clube: str) -> bool:
        ...
        response = await self._client.get(
            f"{self.base_url}/clubes/{codigo_clube}/federado", headers=self._headers()
        )
