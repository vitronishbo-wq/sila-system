...
from apps.backend.app.modules.society.desporto.infrastructure.models.outbox_event_model import (
    OutboxEventModel,
)
...
class InMemoryOutboxRepository(OutboxRepositoryPort):
    ...
    async def append(self, event: object) -> None:
        ...
        async with self._lock:
            self._events.append(
                OutboxMessage(id=None, event=event, event_name=event_name, payload=payload)
            )
...
class SQLAlchemyOutboxRepository(OutboxRepositoryPort):
    ...
    async def append(self, event: object) -> None:
        event_name, payload = serialize_event(event)
        idempotency_key = str(
            payload.get("event_id") or f"{event_name}-{datetime.now(UTC).isoformat()}"
        )
        ...
        async with self._session_factory() as session:
            async with session.begin():
                stmt = (
                    ...
                    .where(
                        OutboxEventModel.processed.is_(False),
    ...
    async def mark_failed(
        ...
        async with self._session_factory() as session:
            ...
            row.locked_by = None
            row.locked_at = None
            row.available_at = datetime.now(UTC) + timedelta(seconds=max(retry_delay_seconds, 1))
            await session.commit()
