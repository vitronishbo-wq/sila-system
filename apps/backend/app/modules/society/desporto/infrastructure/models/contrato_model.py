...
class ContratoModel(Base):
    ...
    codigo_contrato: Mapped[str] = mapped_column(String(50), nullable=False, unique=True, index=True)
    atleta_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True)
    clube_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True)
    ...
    salario_mensal: Mapped[Decimal] = mapped_column(Numeric(14, 2), nullable=False)
    clausula_rescisao: Mapped[Decimal | None] = mapped_column(Numeric(14, 2), nullable=True)
