...
class ClubeModel(Base):
    __tablename__ = 'desporto_clubes'
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    codigo_clube: Mapped[str] = mapped_column(String(50), unique=True, nullable=False, index=True)
    nome: Mapped[str] = mapped_column(String(200), nullable=False, index=True)
    sigla: Mapped[str] = mapped_column(String(10), nullable=False, index=True)
    tipo: Mapped[str] = mapped_column(String(30), nullable=False, index=True)
    modalidade_principal: Mapped[str] = mapped_column(String(30), nullable=False, index=True)
