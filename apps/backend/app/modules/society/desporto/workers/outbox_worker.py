...
class OutboxWorker:
    ...
    async def process_once(self) -> int:
        messages = await self.outbox_repo.pop_batch(
            self.batch_size, worker_id=self.worker_id, lock_ttl_seconds=self.lock_ttl_seconds
        )
        if not messages:
            return 0
        processed = 0
        for message in messages:
            try:
                ...
                await self.outbox_repo.mark_failed(
                    message, error=str(exc), retry_delay_seconds=self.retry_delay_seconds
                )
                self.stats["failed"] = int(self.stats["failed"]) + 1
        self.stats["last_batch_at"] = datetime.now(UTC).isoformat()
        return processed
