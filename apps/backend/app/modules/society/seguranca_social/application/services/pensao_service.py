...
class PensaoService:
    def __init__(
        self,
        *,
        pensao_repo: PensaoRepositoryPort,
    ...
    async def solicitar_pensao(
        ...
        *,
        ...
        existentes = await self.pensao_repo.list_by_filtros(
            beneficiario_id=beneficiario_id, tipo=tipo
        )
        if any(item.status != StatusPensao.CANCELADA for item in existentes):
            raise PensaoAlreadyExistsError(
                "Ja existe pensao nao cancelada deste tipo para o beneficiario"
            )
        ...
        await self.request_service.create_request(
            request_type="solicitacao_pensao",
            entity_id=saved.id,
            ...
            metadata={
                ...
                "valor_mensal": str(saved.valor_mensal),
            },
        )
        return saved
    ...
    async def aprovar_pensao(self, *, pensao_id: UUID, actor_id: UUID) -> Pensao:
        ...
        await self.request_service.complete_request(
            entity_id=updated.id, actor_id=actor_id, metadata={"status": updated.status.value}
        )
        return updated
    ...
    async def obter_por_id(self, pensao_id: UUID) -> Pensao | None:
    ...
    async def listar_pensoes(
        ...
        *,
        ...
        return await self.pensao_repo.list_by_filtros(
            beneficiario_id=beneficiario_id, tipo=tipo, status=status
        )
