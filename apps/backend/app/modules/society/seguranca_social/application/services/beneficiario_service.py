...
class BeneficiarioService:
    def __init__(
        ...
        *,
        ...
        citizen_repo: CitizenServicePort,
    ...
    async def inscrever_beneficiario(
        self, *, citizen_id: UUID, tipo: TipoBeneficiario, regime: RegimeSegurancaSocial
    ) -> Beneficiario:
        ...
        if tipo == TipoBeneficiario.DESEMPREGADO:
            ...
            if not candidato_registrado:
                raise CandidatoEmpregoRequiredError(
                    "Cidadao nao esta registrado como candidato no modulo emprego"
                )
        numero = await self.beneficiario_repo.next_numero_beneficiario(date.today().year)
        beneficiario = Beneficiario.criar(
            citizen_id=citizen_id, tipo=tipo, regime=regime, numero_beneficiario=numero
        )
        saved = await self.beneficiario_repo.save(beneficiario)
        await self.request_service.create_request(
            request_type="inscricao_seguranca_social",
            entity_id=saved.id,
            citizen_id=saved.citizen_id,
            numero_processo=saved.numero_beneficiario,
            metadata={"tipo": saved.tipo.value, "regime": saved.regime.value},
        )
        return saved
    ...
    async def ativar_beneficiario(self, *, beneficiario_id: UUID) -> Beneficiario:
        ...
        await self.request_service.complete_request(
            entity_id=beneficiario_id,
            actor_id=updated.citizen_id,
            metadata={"ativacao_data": date.today().isoformat()},
        )
        return updated

    async def obter_por_id(self, beneficiario_id: UUID) -> Beneficiario | None:
