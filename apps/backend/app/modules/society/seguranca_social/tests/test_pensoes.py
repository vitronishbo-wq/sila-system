...
from apps.backend.app.modules.society.seguranca_social.api.endpoints.pensoes import (
    router as pensoes_router,
)
from apps.backend.app.modules.society.seguranca_social.application.services.pensao_service import (
    PensaoService,
)
...
def _build_service(pensao_repo, beneficiario_repo, request_service) -> PensaoService:
    return PensaoService(
        pensao_repo=pensao_repo,
...
async def test_solicitar_pensao_sucesso():
    beneficiario = Beneficiario.criar(
        ...
        numero_beneficiario="BEN/2026/0001",
    )
    beneficiario.ativar()
    pensao_repo = SimpleNamespace(
        list_by_filtros=AsyncMock(return_value=[]),
        next_numero_processo=AsyncMock(return_value="PEN/2026/0001"),
        save=AsyncMock(side_effect=lambda pensao: pensao),
    )
    ...
    result = await service.solicitar_pensao(
        beneficiario_id=beneficiario.id, tipo=TipoPensao.VELHICE, valor_mensal=Decimal("25000.00")
    )
...
async def test_solicitar_pensao_beneficiario_nao_ativo():
    beneficiario = Beneficiario.criar(
        ...
        tipo=TipoBeneficiario.IDOSO,
        regime=RegimeSegurancaSocial.GERAL,
        numero_beneficiario="BEN/2026/0005",
    )
    pensao_repo = SimpleNamespace(
        list_by_filtros=AsyncMock(return_value=[]),
    ...
    with pytest.raises(BeneficiarioNotEligibleError):
        await service.solicitar_pensao(
            beneficiario_id=beneficiario.id,
            tipo=TipoPensao.VELHICE,
...
async def test_aprovar_pensao_conclui_request():
    pensao = Pensao.solicitar(
        beneficiario_id=uuid4(),
        tipo=TipoPensao.INVALIDEZ,
        valor_mensal=Decimal("12000.00"),
        numero_processo="PEN/2026/0008",
    )
    pensao_repo = SimpleNamespace(
        get_by_id=AsyncMock(return_value=pensao), save=AsyncMock(side_effect=lambda item: item)
    )
...
async def test_solicitar_pensao_bloqueia_duplicado_ativo():
    beneficiario = Beneficiario.criar(
        ...
        tipo=TipoBeneficiario.IDOSO,
        regime=RegimeSegurancaSocial.GERAL,
        numero_beneficiario="BEN/2026/0012",
    )
    beneficiario.ativar()
    existente = Pensao.solicitar(
        beneficiario_id=beneficiario.id,
        tipo=TipoPensao.VELHICE,
        valor_mensal=Decimal("11111.00"),
        numero_processo="PEN/2026/0014",
    )
    pensao_repo = SimpleNamespace(
        list_by_filtros=AsyncMock(return_value=[existente]),
        next_numero_processo=AsyncMock(),
        save=AsyncMock(),
    )
    beneficiario_repo = SimpleNamespace(get_by_id=AsyncMock(return_value=beneficiario))
...
def test_endpoint_solicitar_pensao_retorna_201():
    pensao = Pensao.solicitar(
        beneficiario_id=uuid4(),
        tipo=TipoPensao.VELHICE,
        valor_mensal=Decimal("30000.00"),
        numero_processo="PEN/2026/0002",
    )
    ...
    response = client.post(
        ...
        json={
            "beneficiario_id": str(pensao.beneficiario_id),
            "tipo": "velhice",
...
def test_endpoint_aprovar_pensao_retorna_404_quando_inexistente():
    service = SimpleNamespace(
        aprovar_pensao=AsyncMock(side_effect=PensaoNotFoundError("nao encontrada"))
    )
    client = _build_client(service)
    response = client.post(
        f"/seguranca-social/pensoes/{uuid4()}/aprovar", json={"actor_id": str(uuid4())}
    )
...
def test_endpoint_solicitar_pensao_retorna_409_para_duplicado():
    service = SimpleNamespace(
        solicitar_pensao=AsyncMock(side_effect=PensaoAlreadyExistsError("duplicada"))
    )
    client = _build_client(service)
    response = client.post(
        "/seguranca-social/pensoes/",
        json={"beneficiario_id": str(uuid4()), "tipo": "velhice", "valor_mensal": "30000.00"},
    )
