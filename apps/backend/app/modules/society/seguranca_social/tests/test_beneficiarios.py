...
async def test_inscrever_beneficiario_sucesso():
    citizen_id = uuid4()
    beneficiario_repo = SimpleNamespace(get_by_citizen=AsyncMock(return_value=None), next_numero_beneficiario=AsyncMock(return_value='BEN/2026/0001'), save=AsyncMock(side_effect=lambda beneficiario: beneficiario))
    ...
    result = await service.inscrever_beneficiario(citizen_id=citizen_id, tipo=TipoBeneficiario.IDOSO, regime=RegimeSegurancaSocial.GERAL)
...
async def test_inscrever_beneficiario_cidadao_inexistente():
    beneficiario_repo = SimpleNamespace(get_by_citizen=AsyncMock(), next_numero_beneficiario=AsyncMock(), save=AsyncMock())
    ...
    with pytest.raises(CitizenNotFoundError):
        await service.inscrever_beneficiario(citizen_id=uuid4(), tipo=TipoBeneficiario.IDOSO, regime=RegimeSegurancaSocial.GERAL)

@pytest.mark.asyncio
async def test_inscrever_beneficiario_bloqueia_duplicado_ativo_ou_pendente():
    citizen_id = uuid4()
    existente = Beneficiario.criar(citizen_id=citizen_id, tipo=TipoBeneficiario.TRABALHADOR, regime=RegimeSegurancaSocial.GERAL, numero_beneficiario='BEN/2026/0004')
    beneficiario_repo = SimpleNamespace(get_by_citizen=AsyncMock(return_value=existente), next_numero_beneficiario=AsyncMock(), save=AsyncMock())
    citizen_repo = SimpleNamespace(get_citizen=AsyncMock(return_value={'id': citizen_id}))
    emprego_service = SimpleNamespace(is_candidato_registrado=AsyncMock(return_value=True))
    request_service = SimpleNamespace(create_request=AsyncMock(), complete_request=AsyncMock())
    service = _build_service(beneficiario_repo, citizen_repo, emprego_service, request_service)
    with pytest.raises(BeneficiarioAlreadyExistsError):
        await service.inscrever_beneficiario(citizen_id=citizen_id, tipo=TipoBeneficiario.TRABALHADOR, regime=RegimeSegurancaSocial.GERAL)

@pytest.mark.asyncio
async def test_inscrever_beneficiario_desempregado_exige_candidato_emprego():
    citizen_id = uuid4()
    beneficiario_repo = SimpleNamespace(get_by_citizen=AsyncMock(return_value=None), next_numero_beneficiario=AsyncMock(), save=AsyncMock())
    citizen_repo = SimpleNamespace(get_citizen=AsyncMock(return_value={'id': citizen_id}))
    emprego_service = SimpleNamespace(is_candidato_registrado=AsyncMock(return_value=False))
    request_service = SimpleNamespace(create_request=AsyncMock(), complete_request=AsyncMock())
    service = _build_service(beneficiario_repo, citizen_repo, emprego_service, request_service)
    with pytest.raises(CandidatoEmpregoRequiredError):
        await service.inscrever_beneficiario(citizen_id=citizen_id, tipo=TipoBeneficiario.DESEMPREGADO, regime=RegimeSegurancaSocial.GERAL)

@pytest.mark.asyncio
async def test_ativar_beneficiario_conclui_request():
    citizen_id = uuid4()
    beneficiario = Beneficiario.criar(citizen_id=citizen_id, tipo=TipoBeneficiario.IDOSO, regime=RegimeSegurancaSocial.GERAL, numero_beneficiario='BEN/2026/0010')
    beneficiario_repo = SimpleNamespace(get_by_id=AsyncMock(return_value=beneficiario), save=AsyncMock(side_effect=lambda item: item))
...
def test_endpoint_inscrever_beneficiario_retorna_201():
    citizen_id = uuid4()
    beneficiario = Beneficiario.criar(citizen_id=citizen_id, tipo=TipoBeneficiario.IDOSO, regime=RegimeSegurancaSocial.GERAL, numero_beneficiario='BEN/2026/0002')
    service = SimpleNamespace(inscrever_beneficiario=AsyncMock(return_value=beneficiario))
    client = _build_client(service)
    response = client.post('/seguranca-social/beneficiarios/', json={'citizen_id': str(citizen_id), 'tipo': 'idoso', 'regime': 'geral'})
...
def test_endpoint_inscrever_beneficiario_retorna_409_para_duplicado():
    service = SimpleNamespace(inscrever_beneficiario=AsyncMock(side_effect=BeneficiarioAlreadyExistsError('ja existe')))
    client = _build_client(service)
    response = client.post('/seguranca-social/beneficiarios/', json={'citizen_id': str(uuid4()), 'tipo': 'idoso', 'regime': 'geral'})
