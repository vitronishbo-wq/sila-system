...
async def inscrever_beneficiario(data: BeneficiarioCreate, service: BeneficiarioService=Depends(get_beneficiario_service)):
    try:
        return await service.inscrever_beneficiario(citizen_id=data.citizen_id, tipo=data.tipo, regime=data.regime)
...
async def ativar_beneficiario(beneficiario_id: UUID, service: BeneficiarioService=Depends(get_beneficiario_service)):
    try:
...
async def obter_beneficiario(beneficiario_id: UUID, service: BeneficiarioService=Depends(get_beneficiario_service)):
...
async def listar_beneficiarios(filtros: BeneficiarioFilter=Depends(), service: BeneficiarioService=Depends(get_beneficiario_service)):
    return await service.listar_beneficiarios(tipo=filtros.tipo, estado=filtros.estado, regime=filtros.regime)