...
from apps.backend.app.core.bridges.emprego_bridge import SQLAlchemyCandidatoRepository
from apps.backend.app.modules.society.seguranca_social.application.services import (
    BeneficiarioService,
    PensaoService,
)
from apps.backend.app.modules.society.seguranca_social.infrastructure.adapters import (
...
async def get_beneficiario_service(session=db_dep) -> BeneficiarioService:
...
async def get_pensao_service(session=db_dep) -> PensaoService:
    _, request, _ = _bridges(session)
    return PensaoService(
        pensao_repo=SQLAlchemyPensaoRepository(session),
        beneficiario_repo=SQLAlchemyBeneficiarioRepository(session),
        request_service=request,
    )