...
from apps.backend.app.modules.society.seguranca_social.application.ports import RequestServicePort
...
class RequestServiceAdapter(RequestServicePort):
    ...
    async def create_request(self, *, request_type: str, entity_id: UUID, citizen_id: UUID, numero_processo: str, metadata: dict[str, Any] | None=None) -> Optional[UUID]:
        label = f'SegurancaSocial/{request_type}'
        return await self.bridge.create_education_request(entity_id=entity_id, citizen_id=citizen_id, numero_processo=numero_processo, escola_nome=label, ano_letivo=str(date.today().year))
