# Module Architecture

Domain: seguranca_social

Path:
/home/dev03wsl/sila-system/apps/backend/app/modules/seguranca_social

## Purpose

Gerir capacidades do dominio governamental `seguranca_social` no contexto federado `social`.

## Core Responsibilities

- Modelar regras de negocio do modulo `seguranca_social`.
- Orquestrar casos de uso na camada `application`.
- Expor capacidades publicas via camada `api`.

## Architecture

DDD + Clean Architecture

Layers:

- domain
- application
- infrastructure
- api
- tests

## Key Entities

- Beneficiario
- EstadoBeneficiario
- Pensao
- Periodicidade
- RegimeSegurancaSocial
- StatusPensao
- TipoBeneficiario
- TipoPensao
- TipoPrestacao

## Use Cases

- BeneficiarioService
- PensaoService
- aprovar_pensao
- ativar_beneficiario
- cancelar_pensao
- inscrever_beneficiario
- listar_beneficiarios
- listar_pensoes
- obter_por_id
- solicitar_pensao
- suspender_pensao

## Public API

- Nenhum endpoint detectado automaticamente no diretório `api`.

## Dependencies

- Sem dependencias externas de modulo detectadas.

## Notes for AI Agents

Regras importantes para IA:

- Nao acessar outros dominios diretamente.
- Usar `application` services/handlers como orquestracao.
- Manter separacao DDD entre `domain`, `application`, `infrastructure` e `api`.

_Auto-generated by `scripts/ai/generate_module_architecture_docs.py`. Nao editar manualmente; regenerar via `make architecture-docs`._
