...
from apps.backend.app.modules.society.cultura.application.services.evento_cultural_service import (
...
def _tables():
    ...
    from apps.backend.app.modules.society.cultura.infrastructure.models.evento_cultural_model import (
        EventoCulturalModel,
    )
    ...
    return [
        ArtistaModel.__table__,
        BemCulturalModel.__table__,
        EventoCulturalModel.__table__,
        GrupoArtisticoModel.__table__,
        PatrimonioImaterialModel.__table__,
    ]
...
def test_fluxo_real_orm_artista_bem_evento() -> None:
    ...
    async def scenario() -> None:
        async with _session_scope() as session:
            from apps.backend.app.modules.society.cultura.application.services.grupo_artistico_service import (
            ...
            evento_service = EventoCulturalService(
                evento_repo=evento_repo,
                artista_repo=artista_repo,
                turismo_service=FakeTurismoService(exists=True),
                educacao_service=FakeEducacaoService(exists=True),
            )
            grupo_service = GrupoArtisticoService(
                grupo_repo=grupo_repo,
                artista_repo=artista_repo,
                educacao_service=FakeEducacaoService(exists=True),
            )
            patrimonio_service = PatrimonioImaterialService(
                patrimonio_repo=patrimonio_repo,
                turismo_service=FakeTurismoService(exists=True),
                educacao_service=FakeEducacaoService(exists=True),
            )
            artista = await artista_service.cadastrar_artista(
                nome="Artista Integracao", tipo=[TipoArtista.MUSICO, TipoArtista.ESCRITOR]
            )
            bem = await bem_service.cadastrar_bem(
                nome="Monumento Integracao",
                tipo=TipoPatrimonio.HISTORICO,
                descricao="Monumento de valor historico",
                localizacao="Centro",
                municipio="Luanda",
                provincia="Luanda",
            )
            evento = await evento_service.cadastrar_evento(
                nome="Festival Integracao",
                tipo=TipoEventoCultural.FESTIVAL,
                descricao="Evento integrado",
                data_inicio=date(2026, 7, 1),
                data_fim=date(2026, 7, 2),
                local="Memorial",
                municipio="Luanda",
                provincia="Luanda",
                realizador_id=artista.id,
                atracao_turistica_id=uuid4(),
                instituicao_educacional_id=uuid4(),
            )
            tombado = await bem_service.tombar_bem(bem.id)
            evento_atualizado = await evento_service.atualizar_evento(
                evento_id=evento.id, status=StatusEventoCultural.PUBLICADO
            )
            grupo = await grupo_service.cadastrar_grupo(
                nome="Grupo Integracao",
                tipo=TipoGrupoArtistico.COLETIVO,
                lider_artista_id=artista.id,
                instituicao_educacional_id=uuid4(),
            )
            patrimonio = await patrimonio_service.registrar_patrimonio(
                nome="Tradicao Integracao",
                categoria=CategoriaPatrimonioImaterial.TRADICAO,
                descricao="Tradicao comunitaria viva e registrada no sistema.",
                comunidade="Comunidade Integracao",
                municipio="Luanda",
                provincia="Luanda",
                atracao_turistica_id=uuid4(),
                instituicao_educacional_id=uuid4(),
            )
            patrimonio_atualizado = await patrimonio_service.atualizar_patrimonio(
                patrimonio_id=patrimonio.id, status=StatusPatrimonioImaterial.REGISTRADO
            )
