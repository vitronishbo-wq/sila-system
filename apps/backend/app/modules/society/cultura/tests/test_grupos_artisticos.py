...
from apps.backend.app.modules.society.cultura.tests._fakes import (
    ...
    InMemoryGrupoArtisticoRepository,
)
...
def test_grupo_artistico_cadastro_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        grupo = await service.cadastrar_grupo(
            nome="Coletivo Kilamba",
            tipo=TipoGrupoArtistico.COLETIVO,
            lider_artista_id=lider.id,
            municipio="Luanda",
            provincia="Luanda",
...
def test_grupo_artistico_rejeita_lider_inexistente() -> None:
    ...
    async def scenario() -> None:
        service = GrupoArtisticoService(
            grupo_repo=InMemoryGrupoArtisticoRepository(), artista_repo=InMemoryArtistaRepository()
        )
        with pytest.raises(ValueError, match="Artista lider"):
            await service.cadastrar_grupo(
                nome="Grupo Sem Lider", tipo=TipoGrupoArtistico.BANDA, lider_artista_id=uuid4()
            )
...
def test_grupo_artistico_filtro_tipo() -> None:
    ...
    async def scenario() -> None:
        ...
        await service.cadastrar_grupo(
            nome="Orquestra Nacional", tipo=TipoGrupoArtistico.ORQUESTRA, lider_artista_id=lider.id
        )
        await service.cadastrar_grupo(
            nome="Companhia Teatral", tipo=TipoGrupoArtistico.COMPANHIA, lider_artista_id=lider.id
        )
