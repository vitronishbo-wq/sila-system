...
def test_bem_cultural_fluxo_cadastro_tombamento() -> None:
    ...
    async def scenario() -> None:
        service = BemCulturalService(
            bem_repo=InMemoryBemCulturalRepository(), request_service=FakeRequestService()
        )
        bem = await service.cadastrar_bem(
            nome="Fortaleza de Sao Miguel",
            tipo=TipoPatrimonio.HISTORICO,
            descricao="Fortificacao historica de Luanda.",
...
def test_bem_cultural_filtro_por_municipio() -> None:
    ...
    async def scenario() -> None:
        ...
        await service.cadastrar_bem(
            nome="Bem Luanda",
            tipo=TipoPatrimonio.ARTISTICO,
