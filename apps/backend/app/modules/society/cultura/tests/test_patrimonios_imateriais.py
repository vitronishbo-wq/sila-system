...
from apps.backend.app.modules.society.cultura.tests._fakes import (
    ...
    FakeRequestService,
    FakeTurismoService,
    InMemoryPatrimonioImaterialRepository,
)
...
def test_patrimonio_imaterial_registro_sucesso() -> None:
    ...
    async def scenario() -> None:
        service = PatrimonioImaterialService(
            patrimonio_repo=InMemoryPatrimonioImaterialRepository(),
        ...
        patrimonio = await service.registrar_patrimonio(
            nome="Ritual da Kianda",
            categoria=CategoriaPatrimonioImaterial.RITUAL,
            descricao="Ritual ancestral de relacao com as aguas e memoria local.",
            comunidade="Ilha de Luanda",
            municipio="Luanda",
            provincia="Luanda",
...
def test_patrimonio_imaterial_rejeita_atracao_inexistente() -> None:
    ...
    async def scenario() -> None:
        service = PatrimonioImaterialService(
            patrimonio_repo=InMemoryPatrimonioImaterialRepository(),
            turismo_service=FakeTurismoService(exists=False),
        )
        with pytest.raises(ValueError, match="Atracao turistica"):
            await service.registrar_patrimonio(
                nome="Tradicao X",
                categoria=CategoriaPatrimonioImaterial.TRADICAO,
                descricao="Descricao valida e suficiente para registro formal.",
...
def test_patrimonio_imaterial_filtro_status() -> None:
    ...
    async def scenario() -> None:
        ...
        patrimonio = await service.registrar_patrimonio(
            nome="Lenda do Morro",
            categoria=CategoriaPatrimonioImaterial.EXPRESSAO_ORAL,
            descricao="Narrativa oral transmitida por varias geracoes na regiao.",
            comunidade="Bie",
            municipio="Kuito",
            provincia="Bie",
        )
        await service.atualizar_patrimonio(
