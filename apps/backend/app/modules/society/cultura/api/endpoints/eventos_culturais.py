...
from apps.backend.app.modules.society.cultura.api.deps import get_evento_cultural_service
from apps.backend.app.modules.society.cultura.api.schemas.evento_cultural_schema import (
    EventoCulturalCreate,
    EventoCulturalResponse,
    EventoCulturalUpdate,
)
...
evento_cultural_service_dep = Depends(get_evento_cultural_service)


@router.post("/", response_model=EventoCulturalResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_evento(
    data: EventoCulturalCreate,
    service: EventoCulturalService = evento_cultural_service_dep,
) -> EventoCulturalResponse:
    try:
        return await service.cadastrar_evento(
            nome=data.nome,
            tipo=data.tipo,
...
async def obter_evento(
    evento_id: UUID, service: EventoCulturalService = evento_cultural_service_dep
) -> EventoCulturalResponse:
    try:
...
async def listar_eventos(
    ...
    status_evento: StatusEventoCultural | None = None,
    data_inicio: date | None = None,
    data_fim: date | None = None,
    somente_ativos: bool = True,
    service: EventoCulturalService = evento_cultural_service_dep,
) -> list[EventoCulturalResponse]:
...
async def atualizar_evento(
    evento_id: UUID,
    data: EventoCulturalUpdate,
    service: EventoCulturalService = evento_cultural_service_dep,
) -> EventoCulturalResponse:
    try:
        return await service.atualizar_evento(
            evento_id=evento_id,
            nome=data.nome,
            tipo=data.tipo,
            descricao=data.descricao,
