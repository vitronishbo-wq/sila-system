...
from apps.backend.app.modules.society.cultura.api.schemas.projeto_cultural_schema import (
    ProjetoAprovacaoRequest,
    ProjetoCulturalCreate,
    ProjetoCulturalResponse,
    ProjetoCulturalUpdate,
    ProjetoExecucaoRequest,
)
...
projeto_cultural_service_dep = Depends(get_projeto_cultural_service)


@router.post("/", response_model=ProjetoCulturalResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_projeto(
    data: ProjetoCulturalCreate,
    service: ProjetoCulturalService = projeto_cultural_service_dep,
) -> ProjetoCulturalResponse:
    try:
        return await service.cadastrar_projeto(
            ...
            proponente_cpf_cnpj=data.proponente_cpf_cnpj,
            proponente_nome=data.proponente_nome,
            resumo=data.resumo,
...
async def obter_projeto(
    projeto_id: UUID, service: ProjetoCulturalService = projeto_cultural_service_dep
) -> ProjetoCulturalResponse:
    try:
...
async def listar_projetos(
    ...
    status_projeto: StatusProjetoCultural | None = None,
    data_inicio: date | None = None,
    data_fim: date | None = None,
    somente_ativos: bool = True,
    service: ProjetoCulturalService = projeto_cultural_service_dep,
) -> list[ProjetoCulturalResponse]:
...
async def atualizar_projeto(
    projeto_id: UUID,
    data: ProjetoCulturalUpdate,
    service: ProjetoCulturalService = projeto_cultural_service_dep,
) -> ProjetoCulturalResponse:
    try:
        return await service.atualizar_projeto(
...
async def aprovar_projeto(
    projeto_id: UUID,
    data: ProjetoAprovacaoRequest,
    service: ProjetoCulturalService = projeto_cultural_service_dep,
) -> ProjetoCulturalResponse:
    try:
        return await service.aprovar_projeto(
            projeto_id=projeto_id, valor_aprovado=data.valor_aprovado
        )
    except ValueError as exc:
        message = str(exc).lower()
        code = (
