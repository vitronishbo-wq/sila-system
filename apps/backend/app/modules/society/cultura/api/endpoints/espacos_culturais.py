...
from apps.backend.app.modules.society.cultura.api.schemas.espaco_cultural_schema import (
    EspacoCulturalCreate,
    EspacoCulturalResponse,
    EspacoCulturalUpdate,
)
...
espaco_cultural_service_dep = Depends(get_espaco_cultural_service)


@router.post("/", response_model=EspacoCulturalResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_espaco(
    data: EspacoCulturalCreate,
    service: EspacoCulturalService = espaco_cultural_service_dep,
) -> EspacoCulturalResponse:
    try:
        return await service.cadastrar_espaco(
            nome=data.nome,
            tipo=data.tipo,
            ...
            orgao_gestor=data.orgao_gestor,
            ano_inauguracao=data.ano_inauguracao,
...
async def obter_espaco(
    espaco_id: UUID, service: EspacoCulturalService = espaco_cultural_service_dep
) -> EspacoCulturalResponse:
    try:
...
async def listar_espacos(
    ...
    service: EspacoCulturalService = espaco_cultural_service_dep,
) -> list[EspacoCulturalResponse]:
    return await service.listar_espacos(
        tipo=tipo, municipio=municipio, somente_ativos=somente_ativos
    )
...
async def atualizar_espaco(
    espaco_id: UUID,
    data: EspacoCulturalUpdate,
    service: EspacoCulturalService = espaco_cultural_service_dep,
) -> EspacoCulturalResponse:
    try:
        return await service.atualizar_espaco(
            espaco_id=espaco_id,
            nome=data.nome,
            tipo=data.tipo,
            municipio=data.municipio,
