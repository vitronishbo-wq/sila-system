...
from apps.backend.app.modules.society.cultura.api.schemas.grupo_artistico_schema import (
    GrupoArtisticoCreate,
    GrupoArtisticoResponse,
    GrupoArtisticoUpdate,
)
...
grupo_artistico_service_dep = Depends(get_grupo_artistico_service)


@router.post("/", response_model=GrupoArtisticoResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_grupo(
    data: GrupoArtisticoCreate,
    service: GrupoArtisticoService = grupo_artistico_service_dep,
) -> GrupoArtisticoResponse:
    try:
        return await service.cadastrar_grupo(
            nome=data.nome,
            tipo=data.tipo,
            lider_artista_id=data.lider_artista_id,
...
async def obter_grupo(
    grupo_id: UUID, service: GrupoArtisticoService = grupo_artistico_service_dep
) -> GrupoArtisticoResponse:
    try:
...
async def atualizar_grupo(
    grupo_id: UUID,
    data: GrupoArtisticoUpdate,
    service: GrupoArtisticoService = grupo_artistico_service_dep,
) -> GrupoArtisticoResponse:
    try:
        return await service.atualizar_grupo(
            grupo_id=grupo_id,
            nome=data.nome,
            tipo=data.tipo,
            lider_artista_id=data.lider_artista_id,
