...
class PatrimonioImaterialModel(Base):
    __tablename__ = 'cultura_patrimonios_imateriais'
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    registro_pni: Mapped[str] = mapped_column(String(50), unique=True, nullable=False, index=True)
    nome: Mapped[str] = mapped_column(String(200), nullable=False, index=True)
    categoria: Mapped[str] = mapped_column(String(40), nullable=False, index=True)
    descricao: Mapped[str] = mapped_column(Text, nullable=False)
    comunidade: Mapped[str] = mapped_column(String(200), nullable=False)
    ...
    plano_salvaguarda: Mapped[str | None] = mapped_column(Text, nullable=True)
