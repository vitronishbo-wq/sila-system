...
class SQLAlchemyEventoCulturalRepository(EventoCulturalRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_evento: str) -> EventoCultural | None:
        stmt = select(EventoCulturalModel).where(
            EventoCulturalModel.codigo_evento == codigo_evento.strip()
        )
        model = (await self.session.execute(stmt)).scalars().first()
        return self._to_domain(model) if model else None

    async def list_all(self) -> list[EventoCultural]:
        stmt = select(EventoCulturalModel).order_by(
            EventoCulturalModel.data_inicio.asc(), EventoCulturalModel.nome.asc()
        )
    ...
    async def list_by_tipo(self, tipo: TipoEventoCultural) -> list[EventoCultural]:
        stmt = (
            select(EventoCulturalModel)
            .where(EventoCulturalModel.tipo == tipo.value)
            .order_by(EventoCulturalModel.data_inicio.asc(), EventoCulturalModel.nome.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_municipio(self, municipio: str) -> list[EventoCultural]:
        stmt = (
            select(EventoCulturalModel)
            .where(func.lower(EventoCulturalModel.municipio) == municipio.strip().lower())
            .order_by(EventoCulturalModel.data_inicio.asc(), EventoCulturalModel.nome.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_status(self, status: StatusEventoCultural) -> list[EventoCultural]:
        stmt = (
            select(EventoCulturalModel)
            .where(EventoCulturalModel.status == status.value)
            .order_by(EventoCulturalModel.data_inicio.asc(), EventoCulturalModel.nome.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_periodo(self, data_inicio: date, data_fim: date) -> list[EventoCultural]:
        stmt = (
            select(EventoCulturalModel)
            .where(EventoCulturalModel.data_inicio >= data_inicio)
            .where(EventoCulturalModel.data_fim <= data_fim)
            .order_by(EventoCulturalModel.data_inicio.asc(), EventoCulturalModel.nome.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(EventoCulturalModel)
            .where(EventoCulturalModel.codigo_evento.like(f"EVT/{ano}/%"))
        )
    ...
    def _to_domain(model: EventoCulturalModel) -> EventoCultural:
        return EventoCultural(
            ...
            nome=model.nome,
            tipo=TipoEventoCultural(model.tipo),
            ...
            realizador_id=model.realizador_id,
            data_cadastro=model.data_cadastro,
            status=StatusEventoCultural(model.status),
            atracao_turistica_id=model.atracao_turistica_id,
            instituicao_educacional_id=model.instituicao_educacional_id,
            entrada_gratuita=model.entrada_gratuita,
            valor_ingresso=model.valor_ingresso,
            publico_estimado=model.publico_estimado,
            ativo=model.ativo,
