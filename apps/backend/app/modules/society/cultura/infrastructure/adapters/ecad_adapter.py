...
class EcadAdapter:
    ...
    async def registrar_execucao_publica(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self._client.post(f'{self.base_url}/execucoes-publicas', json=payload, headers=self._headers())
    ...
    async def calcular_direitos(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self._client.post(f'{self.base_url}/direitos/calcular', json=payload, headers=self._headers())
        response.raise_for_status()
        return response.json()