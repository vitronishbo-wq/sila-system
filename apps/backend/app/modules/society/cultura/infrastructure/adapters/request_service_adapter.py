...
class RequestServiceAdapter(RequestServicePort):
    ...
    async def create_request(self, *, request_type: str, entity_id: UUID, metadata: dict[str, Any] | None=None, citizen_id: UUID | None=None, numero_processo: str | None=None) -> UUID | None:
        if citizen_id is None:
            return None
        return await self._bridge.create_education_request(entity_id=entity_id, citizen_id=citizen_id, numero_processo=numero_processo or str(entity_id), escola_nome=f'Cultura/{request_type}', ano_letivo=str(date.today().year))