...
class IphanAdapter:
    ...
    async def registrar_bem_tombado(self, dados: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self._client.post(
            f"{self.base_url}/bens-tombados", json=dados, headers=self._headers()
        )
    ...
    async def consultar_bem(self, codigo_iphan: str) -> dict[str, Any] | None:
        ...
        response = await self._client.get(
            f"{self.base_url}/bens-tombados/{codigo_iphan}", headers=self._headers()
        )
    ...
    @circuit_breaker(name="cultura_iphan", failure_threshold=3, recovery_timeout=60)
    async def solicitar_tombamento(
        self, *, bem_id: UUID, justificativa: str, documentacao: list[str]
    ) -> dict[str, Any]:
        payload = {
            "bem_id": str(bem_id),
        ...
        response = await self._client.post(
            f"{self.base_url}/solicitacoes-tombamento", json=payload, headers=self._headers()
        )
        response.raise_for_status()
        return response.json()
    ...
    async def registrar_intervencao(
        ...
        *,
        ...
        response = await self._client.post(
            f"{self.base_url}/bens-tombados/{bem_id}/intervencoes",
            json=payload,
