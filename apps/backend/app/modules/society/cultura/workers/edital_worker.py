...
from apps.backend.app.modules.society.cultura.domain.enums import FaseEditalCultural
...
class EditalWorker:
    ...
    async def _monitorar_editais(self) -> None:
        ...
        for edital in editais:
            ...
            if (
                edital.fase == FaseEditalCultural.PUBLICADO
                and agora >= edital.data_inicio_inscricoes
            ):
                edital.abrir_inscricoes()
                await self.edital_repo.save(edital)
                self.stats["fases_alteradas"] += 1
            if (
                edital.fase == FaseEditalCultural.INSCRICOES_ABERTAS
                and agora > edital.data_fim_inscricoes
            ):
