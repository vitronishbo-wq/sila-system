...
class GrupoArtisticoService:
    def __init__(
        self,
        *,
        grupo_repo: GrupoArtisticoRepositoryPort,
    ...
    async def cadastrar_grupo(
        ...
        *,
        ...
        grupo = GrupoArtistico.criar(
            codigo_grupo=codigo,
            nome=nome,
            tipo=tipo,
            lider_artista_id=lider_artista_id,
            descricao=descricao,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CADASTRO_GRUPO_ARTISTICO",
                entity_id=saved.id,
                citizen_id=lider.citizen_id if lider else None,
                metadata={
                    "codigo_grupo": saved.codigo_grupo,
                    "nome": saved.nome,
                    "tipo": saved.tipo.value,
                },
                numero_processo=saved.codigo_grupo,
            )
        return saved
