...
class EditalService:
    ...
    def __init__(self, *, edital_repo: EditalRepositoryPort, projeto_repo: ProjetoCulturalRepositoryPort, request_service: RequestServicePort | None=None, minc_adapter=None, turismo_adapter=None) -> None:
    ...
    async def publicar_edital(self, *, numero: str, titulo: str, tipo: TipoEditalCultural, orgao_responsavel_id: UUID, valor_total: Decimal, data_publicacao: datetime, data_inicio_inscricoes: datetime, data_fim_inscricoes: datetime, vagas: int, descricao: str, criterios: list[str], documentos_necessarios: list[str]) -> Edital:
        existing = await self.edital_repo.get_by_numero(numero)
        if existing is not None:
            raise ValueError('Ja existe edital com este numero')
        edital = Edital.publicar(numero=numero, titulo=titulo, tipo=tipo, orgao_responsavel_id=orgao_responsavel_id, valor_total=valor_total, data_publicacao=data_publicacao, data_inicio_inscricoes=data_inicio_inscricoes, data_fim_inscricoes=data_fim_inscricoes, vagas=vagas, descricao=descricao, criterios=criterios, documentos_necessarios=documentos_necessarios)
        saved = await self.edital_repo.save(edital)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='PUBLICACAO_EDITAL_CULTURAL', entity_id=saved.id, numero_processo=saved.numero, metadata={'tipo': saved.tipo.value, 'vagas': saved.vagas})
        if self.minc_adapter is not None:
            await self.minc_adapter.publicar_edital({'id': str(saved.id), 'numero': saved.numero, 'titulo': saved.titulo, 'tipo': saved.tipo.value, 'valor_total': float(saved.valor_total), 'data_fim_inscricoes': saved.data_fim_inscricoes.isoformat()})
        if self.turismo_adapter is not None and saved.tipo.value in {'festival', 'circulacao'}:
            await self.turismo_adapter.notificar_evento({'nome': saved.titulo, 'inicio': saved.data_inicio_inscricoes.isoformat(), 'fim': saved.data_fim_inscricoes.isoformat()})
        await event_bus.publish(EditalPublicadoEvent(edital_id=saved.id, numero=saved.numero, titulo=saved.titulo, tipo=saved.tipo.value, valor_total=float(saved.valor_total), data_fim_inscricoes=saved.data_fim_inscricoes.isoformat()))
        return saved
    ...
    async def listar_editais(self, *, tipo: TipoEditalCultural | None=None, fase: FaseEditalCultural | None=None, data_inicio: datetime | None=None, data_fim: datetime | None=None, somente_ativos: bool=True) -> list[Edital]:
        if tipo is not None:
