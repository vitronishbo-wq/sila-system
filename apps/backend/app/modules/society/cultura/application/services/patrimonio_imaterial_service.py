...
class PatrimonioImaterialService:
    ...
    def __init__(self, *, patrimonio_repo: PatrimonioImaterialRepositoryPort, turismo_service: TurismoServicePort | None=None, educacao_service: EducacaoServicePort | None=None, request_service: RequestServicePort | None=None) -> None:
        self.patrimonio_repo = patrimonio_repo
        self.turismo_service = turismo_service
        self.educacao_service = educacao_service
        self.request_service = request_service

    async def registrar_patrimonio(self, *, nome: str, categoria: CategoriaPatrimonioImaterial, descricao: str, comunidade: str, municipio: str, provincia: str, atracao_turistica_id: UUID | None=None, instituicao_educacional_id: UUID | None=None, plano_salvaguarda: str | None=None, observacoes: str | None=None) -> PatrimonioImaterial:
        ...
        patrimonio = PatrimonioImaterial.registrar(registro_pni=registro, nome=nome, categoria=categoria, descricao=descricao, comunidade=comunidade, municipio=municipio, provincia=provincia, atracao_turistica_id=atracao_turistica_id, instituicao_educacional_id=instituicao_educacional_id, plano_salvaguarda=plano_salvaguarda, observacoes=observacoes)
        saved = await self.patrimonio_repo.save(patrimonio)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='REGISTRO_PATRIMONIO_IMATERIAL', entity_id=saved.id, metadata={'registro_pni': saved.registro_pni, 'categoria': saved.categoria.value, 'municipio': saved.municipio}, numero_processo=saved.registro_pni)
        return saved
    ...
    async def listar_patrimonios(self, *, categoria: CategoriaPatrimonioImaterial | None=None, municipio: str | None=None, status: StatusPatrimonioImaterial | None=None, somente_ativos: bool=True) -> list[PatrimonioImaterial]:
    ...
    async def atualizar_patrimonio(self, *, patrimonio_id: UUID, nome: str | None=None, categoria: CategoriaPatrimonioImaterial | None=None, descricao: str | None=None, comunidade: str | None=None, municipio: str | None=None, provincia: str | None=None, atracao_turistica_id: UUID | None=None, instituicao_educacional_id: UUID | None=None, plano_salvaguarda: str | None=None, status: StatusPatrimonioImaterial | None=None, ativo: bool | None=None, observacoes: str | None=None) -> PatrimonioImaterial:
        item = await self.buscar_patrimonio(patrimonio_id)
        await self._validar_integracoes(atracao_turistica_id=atracao_turistica_id, instituicao_educacional_id=instituicao_educacional_id)
        item.atualizar(nome=nome, categoria=categoria, descricao=descricao, comunidade=comunidade, municipio=municipio, provincia=provincia, atracao_turistica_id=atracao_turistica_id, instituicao_educacional_id=instituicao_educacional_id, plano_salvaguarda=plano_salvaguarda, status=status, ativo=ativo, observacoes=observacoes)
