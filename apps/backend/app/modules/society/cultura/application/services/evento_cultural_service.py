...
class EventoCulturalService:
    def __init__(
        self,
        *,
        evento_repo: EventoCulturalRepositoryPort,
        ...
        *,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CADASTRO_EVENTO_CULTURAL",
                entity_id=saved.id,
                citizen_id=realizador.citizen_id,
                metadata={
                    "codigo_evento": saved.codigo_evento,
                    "tipo": saved.tipo.value,
                    "municipio": saved.municipio,
                },
                numero_processo=saved.codigo_evento,
            )
        await event_bus.publish(
            EventoProgramadoEvent(
                ...
                nome=saved.nome,
                tipo=saved.tipo.value,
                data_inicio=saved.data_inicio.isoformat(),
                data_fim=saved.data_fim.isoformat(),
                local=saved.local,
                capacidade=saved.publico_estimado or 0,
            )
        )
        return saved
