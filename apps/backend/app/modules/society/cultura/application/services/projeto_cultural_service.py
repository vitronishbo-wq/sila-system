...
class ProjetoCulturalService:
    ...
    async def cadastrar_projeto(
        ...
        *,
        ...
        projeto = ProjetoCultural.cadastrar(
            codigo_projeto=codigo,
            titulo=titulo,
            tipo=tipo,
            natureza=natureza,
            proponente_cpf_cnpj=proponente_cpf_cnpj,
            proponente_nome=proponente_nome,
            resumo=resumo,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CADASTRO_PROJETO_CULTURAL",
                entity_id=saved.id,
                numero_processo=saved.codigo_projeto,
                metadata={
                    "codigo_projeto": saved.codigo_projeto,
    ...
    async def listar_projetos(
        ...
        *,
        ...
        status: StatusProjetoCultural | None = None,
        data_inicio: date | None = None,
        data_fim: date | None = None,
        somente_ativos: bool = True,
    ) -> list[ProjetoCultural]:
        if tipo is not None:
    ...
    async def aprovar_projeto(
        self, *, projeto_id: UUID, valor_aprovado: Decimal
    ) -> ProjetoCultural:
        ...
        await event_bus.publish(
            ProjetoAprovadoEvent(
                ...
                codigo_projeto=saved.codigo_projeto,
                titulo=saved.titulo,
                proponente=saved.proponente_nome,
                valor_aprovado=float(valor_aprovado),
                edital_id=saved.edital_id,
            )
        )
        return saved
