...
class BemCultural:
    ...
    @classmethod
    def cadastrar(cls, *, registro: str, nome: str, tipo: TipoPatrimonio, descricao: str, localizacao: str, municipio: str, provincia: str, coordenadas_lat: Decimal | None=None, coordenadas_long: Decimal | None=None, observacoes: str | None=None) -> 'BemCultural':
        ...
        return cls(id=uuid4(), registro_ipat=registro.strip(), nome=nome_normalizado, tipo=tipo, descricao=descricao.strip(), localizacao=localizacao.strip(), municipio=municipio.strip(), provincia=provincia.strip(), data_cadastro=date.today(), coordenadas_lat=coordenadas_lat, coordenadas_long=coordenadas_long, observacoes=observacoes.strip() if observacoes else None)

    def atualizar(self, *, nome: str | None=None, tipo: TipoPatrimonio | None=None, descricao: str | None=None, localizacao: str | None=None, municipio: str | None=None, provincia: str | None=None, coordenadas_lat: Decimal | None=None, coordenadas_long: Decimal | None=None, ativo: bool | None=None, observacoes: str | None=None) -> None:
        if nome is not None:
            nome_normalizado = nome.strip()
            if len(nome_normalizado) < 3:
                raise ValueError('Nome do bem cultural deve ter pelo menos 3 caracteres')
