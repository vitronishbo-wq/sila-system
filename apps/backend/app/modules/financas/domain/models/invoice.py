...
class Invoice:
    ...
    def change_status(self, new_status: InvoiceStatus, reason: str = "") -> Dict[str, Any]:
        ...
        """
        valid_transitions = {
            InvoiceStatus.PENDING: [InvoiceStatus.PAID, InvoiceStatus.CANCELLED, InvoiceStatus.OVERDUE],
        ...
        if new_status not in valid_transitions.get(self.status, []):
            from app.modules.financas.exceptions import InvalidInvoiceStateError
            raise InvalidInvoiceStateError(
                current_status=self.status.value,
                action=f"mudar para {new_status.value}"
            )
        ...
        # Retorna dicionário com os dados da mudança para o serviço criar o log de auditoria
        return {
            "entity_type": "INVOICE",
            "entity_id": self.id,
            "action": "STATUS_CHANGE",
            "previous_state": {"status": old_status.value},
            "new_state": {"status": new_status.value, "reason": reason},
