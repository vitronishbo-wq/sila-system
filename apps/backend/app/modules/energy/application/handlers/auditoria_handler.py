...
class AuditoriaHandler:
    ...
    async def on_fatura_gerada(self, event: FaturaGeradaEvent) -> None:
        if self._aneel_adapter is None:
            return
        await self._aneel_adapter.registrar_evento_qualidade(
            {
                "tipo_evento": "fatura_gerada",
                "fatura_id": str(event.fatura_id),
                "numero_fatura": event.numero_fatura,
                "valor_total": str(event.valor_total),
    ...
    async def on_qualidade_inconforme(self, event: QualidadeInconformeEvent) -> None:
        ...
        await self._aneel_adapter.registrar_evento_qualidade(
            {
                "tipo_evento": "qualidade_inconforme",
                "ponto_medicao_id": str(event.ponto_medicao_id),
                "parametro": event.parametro,
                "valor_medido": str(event.valor_medido),
                "valor_referencia": str(event.valor_referencia),
                "desvio_percentual": str(event.desvio_percentual),
            }
        )
