...
class GeracaoService:
    ...
    def __init__(self, *, central_repo: CentralGeradoraRepositoryPort, sub_repo: SubestacaoRepositoryPort, linha_repo: LinhaTransmissaoRepositoryPort, repo_timeout_seconds: float=1.0, slow_log_threshold_seconds: float=0.5) -> None:
    ...
    async def get_visao_consolidada(self) -> dict[str, Any]:
        ...
        subestacoes, erro_subestacoes = await self._safe_fetch('subestacoes', self._sub_repo.list_all)
        ...
        if elapsed > self._slow_log_threshold_seconds:
            logger.warning('Dashboard de geracao lento', extra={'elapsed_seconds': round(elapsed, 4), 'repo_timeout_seconds': self._repo_timeout_seconds, 'degraded': bool(erros)})
        return {'centrais': centrais, 'subestacoes': subestacoes, 'linhas': linhas, 'errors': erros, 'degraded': bool(erros)}
    ...
    async def _safe_fetch(self, nome: str, callback) -> tuple[list[Any], str | None]:
        try:
            ...
            logger.error('Timeout ao consultar repositorio de energia', extra={'repo': nome, 'timeout_seconds': self._repo_timeout_seconds})
