...
class FaturamentoService:
    ...
    async def gerar_fatura_por_consumo(
        self, consumo_id: UUID, *, data_referencia: date | None = None
    ) -> FaturaEnergia:
        ...
        if any(
            item.consumo_id == consumo_id and item.status != StatusFaturaEnergia.CANCELADA
    ...
    async def registrar_pagamento(
        ...
        *,
        ...
        item.registrar_pagamento(
            data_pagamento=data_pagamento, valor_pago=valor_pago, metodo_pagamento=metodo_pagamento
        )
    ...
    async def listar(
        ...
        *,
        ...
        items = await self._fatura_repo.list(
            unidade_consumidora_id=unidade_consumidora_id, cpf_titular=cpf_titular, status=status
        )
