...
from apps.backend.app.modules.energy.application.events.registry import (
    EnergiaEventRegistry,
    serialize_event,
)
from apps.backend.app.modules.energy.application.ports.outbox_repository_port import (
    OutboxRepositoryPort,
)
from apps.backend.app.modules.energy.infrastructure.models.outbox_event_model import (
...
class SQLAlchemyOutboxRepository(OutboxRepositoryPort):
    ...
    async def enqueue(self, event: Any) -> OutboxMessage:
        ...
        model = EnergiaOutboxEventModel(
            id=uuid4(),
            event_name=event_name,
            event_version=int(payload.get("version", 1)),
            schema_version=1,
            topic=topic,
            payload=payload,
            headers={},
    ...
    async def get_pending(self, *, limit: int = 100) -> list[OutboxMessage]:
        async with self._session_factory() as session:
            result = await session.execute(
                select(EnergiaOutboxEventModel)
                .where(EnergiaOutboxEventModel.processed.is_(False))
                .order_by(EnergiaOutboxEventModel.created_at.asc())
                .limit(limit)
            )
    ...
    async def mark_done(self, message_id: UUID) -> None:
        async with self._session_factory() as session:
            result = await session.execute(
                select(EnergiaOutboxEventModel).where(EnergiaOutboxEventModel.id == message_id)
            )
