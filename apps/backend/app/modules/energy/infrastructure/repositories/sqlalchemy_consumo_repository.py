...
class SQLAlchemyConsumoRepository(ConsumoRepositoryPort):
    ...
    async def save(self, item: ConsumoEnergia) -> ConsumoEnergia:
        model = ConsumoEnergiaModel(
            id=item.id,
            unidade_consumidora_id=item.unidade_consumidora_id,
            medidor_id=item.medidor_id,
            data_leitura=item.data_leitura,
            leitura_kwh=item.leitura_kwh,
            leitura_anterior_kwh=item.leitura_anterior_kwh,
            consumo_periodo_kwh=item.consumo_periodo_kwh,
            tipo_leitura=item.tipo_leitura,
            classe_tarifaria=item.classe_tarifaria,
    ...
    async def get_last_by_unidade(self, unidade_consumidora_id: UUID) -> ConsumoEnergia | None:
        values = [
            ...
            if item.unidade_consumidora_id == unidade_consumidora_id
        ]
        if not values:
            return None
    ...
    async def list(
        ...
        if unidade_consumidora_id:
            values = [
                item for item in values if item.unidade_consumidora_id == unidade_consumidora_id
            ]
        if cpf_titular:
    ...
    def _to_domain(model: ConsumoEnergiaModel) -> ConsumoEnergia:
        return ConsumoEnergia(
            id=model.id,
            unidade_consumidora_id=model.unidade_consumidora_id,
            medidor_id=model.medidor_id,
            data_leitura=model.data_leitura,
            leitura_kwh=Decimal(model.leitura_kwh),
            leitura_anterior_kwh=Decimal(model.leitura_anterior_kwh)
            if model.leitura_anterior_kwh is not None
            else None,
            consumo_periodo_kwh=Decimal(model.consumo_periodo_kwh),
            tipo_leitura=model.tipo_leitura,
            classe_tarifaria=model.classe_tarifaria,
            cpf_titular=model.cpf_titular,
        )
