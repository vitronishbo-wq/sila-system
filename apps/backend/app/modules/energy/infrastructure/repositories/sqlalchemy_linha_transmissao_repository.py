...
class SQLAlchemyLinhaTransmissaoRepository(LinhaTransmissaoRepositoryPort):
    ...
    def _to_domain(model: LinhaTransmissaoModel) -> LinhaTransmissao:
        return LinhaTransmissao(
            id=model.id,
            origem_id=model.origem_id,
            origem_tipo=model.origem_tipo,
            destino_id=model.destino_id,
            destino_tipo=model.destino_tipo,
            capacidade_mw=model.capacidade_mw,
            extensao_km=model.extensao_km,
            status=model.status,
