...
class SQLAlchemyFaturaRepository(FaturaRepositoryPort):
    ...
    async def save(self, item: FaturaEnergia) -> FaturaEnergia:
        model = FaturaEnergiaModel(
            ...
            bandeira_tarifaria=item.bandeira_tarifaria,
            valor_consumo=item.valor_consumo,
            valor_bandeira=item.valor_bandeira,
            valor_iluminacao_publica=item.valor_iluminacao_publica,
    ...
    def _to_domain(model: FaturaEnergiaModel) -> FaturaEnergia:
        return FaturaEnergia(
            ...
            consumo_id=model.consumo_id,
            unidade_consumidora_id=model.unidade_consumidora_id,
            cpf_titular=model.cpf_titular,
            mes_referencia=model.mes_referencia,
            consumo_kwh=Decimal(model.consumo_kwh),
            tarifa_kwh=Decimal(model.tarifa_kwh),
            bandeira_tarifaria=model.bandeira_tarifaria,
            valor_consumo=Decimal(model.valor_consumo),
            valor_bandeira=Decimal(model.valor_bandeira),
            valor_iluminacao_publica=Decimal(model.valor_iluminacao_publica),
