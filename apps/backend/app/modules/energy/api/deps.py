...
from apps.backend.app.modules.energy.application.events.definitions import (
    FaturaGeradaEvent,
    LeituraRealizadaEvent,
    QualidadeInconformeEvent,
)
from apps.backend.app.modules.energy.application.handlers import (
    AuditoriaHandler,
    FaturamentoHandler,
)
from apps.backend.app.modules.energy.application.services import (
...
from apps.backend.app.modules.energy.infrastructure.repositories import (
...
central_geradora_service_singleton = CentralGeradoraService(
    repository=central_geradora_repo_singleton
)
...
linha_transmissao_service_singleton = LinhaTransmissaoService(
    repository=linha_transmissao_repo_singleton
)
geracao_service_singleton = GeracaoService(
    central_repo=central_geradora_repo_singleton,
    sub_repo=subestacao_repo_singleton,
    linha_repo=linha_transmissao_repo_singleton,
)
...
consumo_service_singleton = ConsumoService(
    consumo_repo=consumo_repo_singleton, outbox_repo=outbox_repo_singleton
)
faturamento_service_singleton = FaturamentoService(
    fatura_repo=fatura_repo_singleton,
    consumo_repo=consumo_repo_singleton,
    outbox_repo=outbox_repo_singleton,
    ons_adapter=ons_adapter_singleton,
)
faturamento_handler_singleton = FaturamentoHandler(
    faturamento_service=faturamento_service_singleton
)
...
def _configure_subscriptions() -> None:
    ...
    if _subscriptions_configured:
        return
    event_bus.subscribe(
        LeituraRealizadaEvent.event_name, faturamento_handler_singleton.on_leitura_realizada
    )
    event_bus.subscribe(FaturaGeradaEvent.event_name, auditoria_handler_singleton.on_fatura_gerada)
    event_bus.subscribe(
        QualidadeInconformeEvent.event_name, auditoria_handler_singleton.on_qualidade_inconforme
    )
