...
from apps.backend.app.modules.energy.api.schemas.fatura_schema import (
    FaturaEnergiaResponse,
    FaturaGerarPorConsumoInput,
...
@router.post(
    "/gerar-por-consumo", response_model=FaturaEnergiaResponse, status_code=status.HTTP_201_CREATED
)
async def gerar_por_consumo(
    data: FaturaGerarPorConsumoInput, service: FaturamentoService = faturamento_service_dep
):
    try:
        return await service.gerar_fatura_por_consumo(
            data.consumo_id, data_referencia=data.data_referencia
        )
...
async def listar_faturas(
    ...
    status_fatura: StatusFaturaEnergia | None = None,
    service: FaturamentoService = faturamento_service_dep,
):
    return await service.listar(
        unidade_consumidora_id=unidade_consumidora_id, cpf_titular=cpf_titular, status=status_fatura
    )