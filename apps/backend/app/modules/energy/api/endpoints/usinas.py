...
from apps.backend.app.modules.energy.api.schemas.usina_schema import (
    UsinaCreate,
    UsinaDataInput,
    UsinaMotivoInput,
    UsinaPotenciaInput,
...
usina_service_dep = Depends(get_usina_service)


@router.post("/", response_model=UsinaResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_usina(data: UsinaCreate, service: UsinaService = usina_service_dep):
    try:
        return await service.cadastrar_usina(
            codigo_aneel=data.codigo_aneel,
            nome=data.nome,
            fonte=data.fonte,
            tipo=data.tipo,
            potencia_instalada_mw=data.potencia_instalada_mw,
...
async def iniciar_construcao(
    usina_id: UUID, data: UsinaDataInput, service: UsinaService = usina_service_dep
):
    try:
...
async def paralisar_usina(
    usina_id: UUID, data: UsinaMotivoInput, service: UsinaService = usina_service_dep
):
    try:
...
async def atualizar_potencia_fiscalizada(
    usina_id: UUID, data: UsinaPotenciaInput, service: UsinaService = usina_service_dep
):
    try:
...
async def obter_usina(usina_id: UUID, service: UsinaService = usina_service_dep):
    try:
...
async def listar_usinas(
    status_usina: StatusUsina | None = None,
    fonte: FonteEnergia | None = None,
    provincia: str | None = None,
    service: UsinaService = usina_service_dep,
):
