...
async def registrar_leitura(
    data: ConsumoLeituraInput, service: ConsumoService = consumo_service_dep
):
    try:
        return await service.registrar_leitura(
            unidade_consumidora_id=data.unidade_consumidora_id,
            leitura_kwh=data.leitura_kwh,
            data_leitura=data.data_leitura,
            tipo_leitura=data.tipo_leitura,
            classe_tarifaria=data.classe_tarifaria,
...
async def obter_consumo(consumo_id: UUID, service: ConsumoService = consumo_service_dep):
    try:
