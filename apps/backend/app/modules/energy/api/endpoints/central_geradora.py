...
central_geradora_service_dep = Depends(get_central_geradora_service)


@router.post("/", response_model=CentralGeradoraResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar(
    data: CentralGeradoraCreate,
    service: CentralGeradoraService = central_geradora_service_dep,
):
    try:
        return await service.cadastrar(
            nome=data.nome,
            tipo=data.tipo,
            capacidade_instalada_mw=data.capacidade_instalada_mw,
