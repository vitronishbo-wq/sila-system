...
subestacao_service_dep = Depends(get_subestacao_service)


@router.post("/", response_model=SubestacaoResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar(
    data: SubestacaoCreate, service: SubestacaoService = subestacao_service_dep
):
    try:
        return await service.cadastrar(
            nome=data.nome,
            tensao_nominal_kv=data.tensao_nominal_kv,
            classe_tensao=data.classe_tensao,
...
async def iniciar_construcao(
    id: UUID,
    data: SubestacaoDataInput,
    service: SubestacaoService = subestacao_service_dep,
):
    try:
