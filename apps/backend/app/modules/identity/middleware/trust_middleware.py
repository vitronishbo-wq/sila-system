...
from apps.backend.app.modules.identity.domain.trust_score import TrustScore
...
class TrustEvaluationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> JSONResponse:
        ...
        logger.info(
            f"Trust evaluation: endpoint={request.url.path} risk_level={risk_level} total_score={total_score:.2f} client={request.client}"
        )
        ...
        if trust.block_access():
            logger.warning(
                f"CRITICAL_RISK blocked: endpoint={request.url.path} score={total_score:.2f} client={request.client}"
            )
            return JSONResponse(
                status_code=403,
                content={
        ...
        if request.url.path in HIGH_ASSURANCE_ENDPOINTS:
            if total_score < 0.95:
                logger.warning(
                    f"High-assurance denied: endpoint={request.url.path} score={total_score:.2f} required=0.95"
                )
                return JSONResponse(
                    status_code=403,
                    content={
        ...
        if request.url.path in MFA_REQUIRED_ENDPOINTS and total_score < 0.8:
            if not mfa_token:
                logger.warning(
                    f"MFA required but missing: endpoint={request.url.path} score={total_score:.2f}"
                )
                return JSONResponse(
                    status_code=401,
                    content={
