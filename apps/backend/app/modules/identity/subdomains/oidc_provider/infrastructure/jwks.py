...
def _resolve_key_paths() -> tuple[str | None, str | None]:
...
def load_provider_config() -> ProviderConfig:
    ...
    algorithm = (
        os.getenv("OIDC_PROVIDER_ALGORITHM") or os.getenv("OIDC_ALGORITHM") or DEFAULT_ALGORITHM
    )
    token_ttl_seconds = int(os.getenv("OIDC_PROVIDER_TOKEN_TTL", str(DEFAULT_TOKEN_TTL_SECONDS)))
    return ProviderConfig(
        issuer=issuer, audience=audience, algorithm=algorithm, token_ttl_seconds=token_ttl_seconds
    )
...
def issue_access_token(
    subject: str, claims: dict[str, Any], scope: str | None = None
) -> dict[str, Any]:
    if jwt is None:
        raise RuntimeError("python-jose is required to issue OIDC provider tokens")
    ...
    token = jwt.encode(
        payload, keys.private_key_pem, algorithm=config.algorithm, headers={"kid": keys.kid}
    )
    return {
        "access_token": token,
        "token_type": "Bearer",
