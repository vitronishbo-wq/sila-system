...
class Identity:
    ...
    def verify(self) -> Dict[str, Any]:
        ...
        return {'entity_type': 'IDENTITY', 'entity_id': self.id, 'action': 'IDENTITY_VERIFIED', 'previous_state': {'status': old_status.value}, 'new_state': {'status': self.status.value}, 'timestamp': self.verified_at}
    ...
    def reject(self, reason: str='') -> Dict[str, Any]:
        ...
        return {'entity_type': 'IDENTITY', 'entity_id': self.id, 'action': 'IDENTITY_REJECTED', 'previous_state': {'status': old_status.value}, 'new_state': {'status': self.status.value, 'reason': reason}, 'timestamp': self.updated_at}