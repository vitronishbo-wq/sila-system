...
class TrustScore:
    ...
    def update_score(self, new_score: float, reason: str='') -> Dict[str, Any]:
        ...
        return {'entity_type': 'TRUST_SCORE', 'entity_id': self.id, 'action': 'TRUST_SCORE_UPDATED', 'previous_state': {'score': old_score, 'level': old_level.value}, 'new_state': {'score': new_score, 'level': self.level.value}, 'timestamp': self.updated_at}