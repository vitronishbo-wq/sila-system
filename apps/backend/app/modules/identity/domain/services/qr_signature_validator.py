...
from apps.backend.app.modules.identity.domain.exceptions import (
    DomainException,
...
class QRSignatureValidator:
    ...
    def verify_signature(payload: bytes, signature: bytes, public_key: bytes) -> bool:
        ...
        try:
            ...
            raise DomainExceptionFactory.invalid_argument(
                f"Invalid signature/public_key format: {str(e)}"
            ) from e
