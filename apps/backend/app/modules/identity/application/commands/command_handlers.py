...
from apps.backend.app.modules.identity.application.commands.identity_commands import (
...
class CreateIdentityHandler:
    ...
    async def handle(self, command: CreateIdentityCommand) -> Identity:
        ...
        await self.publisher.publish(
            "identity.created", {"identity_id": saved.id, "citizen_id": saved.citizen_id}
        )
        return saved
...
class UpdateTrustScoreHandler:
    ...
    async def handle(self, command: UpdateTrustScoreCommand) -> TrustScore:
        trust_score = await self.repo.get_by_citizen(command.citizen_id) or TrustScore(
            id=str(uuid.uuid4()), citizen_id=command.citizen_id, score=0.0
        )
        audit_data = trust_score.update_score(
            command.new_score, command.metadata.get("reason", "") if command.metadata else ""
        )
