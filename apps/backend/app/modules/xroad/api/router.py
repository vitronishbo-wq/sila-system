...
from apps.backend.app.modules.xroad.domain.envelope import SILAEnvelope, SILAEnvelopeResponse, ServiceType, OriginMinistry
...
async def justice_verify_birth_notice(request: Request, envelope: SILAEnvelope) -> SILAEnvelopeResponse:
    """
...
async def justice_check_death_status(request: Request, envelope: SILAEnvelope) -> SILAEnvelopeResponse:
    """
...
async def justice_batch_identity_validation(request: Request, envelopes: list[SILAEnvelope]) -> list[SILAEnvelopeResponse]:
    """
...
async def health_validate_bi_status(request: Request, envelope: SILAEnvelope) -> SILAEnvelopeResponse:
    """
...
async def health_verify_citizenship(request: Request, envelope: SILAEnvelope) -> SILAEnvelopeResponse:
    """
...
async def services_discovery() -> dict:
    ...
    return {'status': 'online', 'services': services['available_services'], 'service_count': services['total'], 'interop_policies': services['interop_policy_count']}
...
async def xroad_health() -> dict:
    ...
    try:
        xroad = get_xroad()
        envelope = SILAEnvelope(sender_service=OriginMinistry.MINJUS, receiver_service=OriginMinistry.MINSA, service_type=ServiceType.HEALTH_CHECK, payload={})
        response = await xroad.exchange(envelope)
        return {'status': 'healthy', 'xroad_service': 'online', 'timestamp': response.timestamp, 'audit_id': response.audit_record_id}
    except Exception as e:
...
async def audit_trail_query(message_id: str) -> dict:
    ...
    return {'message_id': message_id, 'audit_trail': [{'status': 'RECEIVED', 'timestamp': '2026-03-11T14:45:00Z'}, {'status': 'VALIDATED', 'timestamp': '2026-03-11T14:45:01Z'}, {'status': 'PROCESSED', 'timestamp': '2026-03-11T14:45:05Z'}, {'status': 'ARCHIVED', 'timestamp': '2026-03-11T14:45:06Z'}]}
...
async def list_interoperability_policies() -> dict:
    ...
    for (sender, receiver), policy in xroad.policies.items():
        ...
        policies[key] = {'services': [s.value for s in policy.allowed_service_types], 'requires_high_trust': policy.requires_high_trust_score, 'timeout_seconds': policy.timeout_seconds, 'requires_signature': policy.requires_signature}
