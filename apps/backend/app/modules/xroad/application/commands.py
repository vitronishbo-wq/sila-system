...
from apps.backend.app.modules.xroad.domain.events import (
    XRoadServiceAccessApproved,
    XRoadServiceInvokedSuccessfully,
    XRoadServiceRegistered,
)
...
class RegisterXRoadServiceCommandHandler:
    ...
    async def handle(
        ...
        try:
            event = XRoadServiceRegistered(
                aggregate_id=service_id,
                aggregate_type="XRoadService",
                event_type="XRoadServiceRegistered",
                ...
                metadata={
                    ...
                    "command_name": "RegisterXRoadService",
...
class ApproveXRoadServiceAccessCommandHandler:
    ...
    async def handle(
        ...
        try:
            event = XRoadServiceAccessApproved(
                aggregate_id=approval_id,
                aggregate_type="XRoadServiceAccess",
                event_type="XRoadServiceAccessApproved",
                ...
                metadata={
                    "user_id": str(user_id) if user_id else None,
                    "command_name": "ApproveXRoadServiceAccess",
                    "timestamp_utc": datetime.now(UTC).isoformat(),
                },
            )
            await self.event_bus.publish(event)
...
class InvokeXRoadServiceCommandHandler:
    ...
    async def handle(
        ...
        try:
            event = XRoadServiceInvokedSuccessfully(
                aggregate_id=invocation_id,
                aggregate_type="XRoadServiceInvocation",
                event_type="XRoadServiceInvokedSuccessfully",
                invocation_id=invocation_id,
                service_id=service_id,
                consumer_id=consumer_id,
                invocation_date=datetime.now(UTC),
                request_parameters=request_parameters,
                response_time_ms=response_time_ms,
                correlation_id=correlation_id,
                metadata={
                    "user_id": str(user_id) if user_id else None,
                    "command_name": "InvokeXRoadService",
                    "timestamp_utc": datetime.now(UTC).isoformat(),
                },
            )
            await self.event_bus.publish(event)
