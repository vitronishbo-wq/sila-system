...
from apps.backend.app.modules.payment.application.services.payment_service import PaymentService
...
class PaymentWebhookService:
    ...
    async def process_webhook(self, webhook_id: int, payload_body: bytes, headers: dict):
        ...
        if not payment:
            ...
            return {'status': 'skipped', 'reason': 'Payment not found'}
        status_map = {'completed': PaymentStatus.COMPLETED, 'failed': PaymentStatus.FAILED, 'pending': PaymentStatus.PENDING}
        ...
        await self.payment_service.update_payment_status(payment_id=getattr(payment, 'id', None), status=status_value, provider_reference=provider_reference)
