...
from apps.backend.app.modules.payment.application.commands.payment_commands import CreatePaymentCommand, RefundPaymentCommand, ProcessWebhookCommand, UpdatePaymentStatusCommand
...
class CreatePaymentHandler:
    ...
    async def handle(self, command: CreatePaymentCommand) -> Payment:
        ...
        payment = Payment(id=None, citizen_id=command.citizen_id, reference=command.reference, amount=command.amount, method=command.method, status='PENDING', description=command.description or '', metadata=command.metadata or {})
...
class RefundPaymentHandler:
    ...
    async def handle(self, command: RefundPaymentCommand) -> Payment:
        ...
        refund_result = await self.provider.refund(payment.provider_reference, command.amount or payment.amount)
...
class ProcessWebhookHandler:
    ...
    async def handle(self, command: ProcessWebhookCommand) -> Optional[Payment]:
