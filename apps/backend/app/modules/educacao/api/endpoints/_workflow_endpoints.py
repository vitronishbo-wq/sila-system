...
def build_workflow_router(*, tag: str, get_service: Callable, create_schema: Any, response_schema: Any, concluir_schema: Any, cancelar_schema: Any, routes: list[tuple[str, str]]) -> APIRouter:
    ...
    async def conclude(record_id: UUID, data: concluir_schema, service=Depends(get_service), user: dict=Depends(get_current_user)):
        try:
    ...
    async def cancel(record_id: UUID, data: cancelar_schema, service=Depends(get_service), user: dict=Depends(get_current_user)):
        try:
