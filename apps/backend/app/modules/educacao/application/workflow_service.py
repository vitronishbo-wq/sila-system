...
class WorkflowService:
    ...
    def __init__(self, repository: WorkflowRepositoryPort, process_prefix: str, citizen_repo: CitizenRepositoryPort | None=None, request_service: ServiceRequestLifecycleBridge | None=None):
        self.repository = repository
        self.process_prefix = process_prefix
        self.citizen_repo = citizen_repo
        self.request_service = request_service

    async def create_record(self, *, service_type: str, citizen_id: UUID, instituicao_id: UUID, observacoes: Optional[str]=None, metadata: Optional[dict]=None) -> WorkflowRecord:
        ...
        if self.request_service is not None:
            await self.request_service.create_education_request(entity_id=saved.id, citizen_id=saved.citizen_id, numero_processo=saved.numero_processo, escola_nome=service_type.replace('_', ' '), ano_letivo=str(year))
        return saved

    async def conclude_record(self, record_id: UUID, actor_id: UUID, resumo: Optional[str]=None) -> WorkflowRecord:
        ...
        if self.request_service is not None:
            await self.request_service.mark_education_request_completed(entity_id=updated.id, actor_id=actor_id, metadata={'status': 'concluida', 'service_type': updated.service_type})
        return updated
    ...
    async def cancel_record(self, record_id: UUID, actor_id: UUID, motivo: str) -> WorkflowRecord:
        ...
        updated = await self.repository.save(record)
        if self.request_service is not None:
            await self.request_service.mark_education_request_completed(entity_id=updated.id, actor_id=actor_id, metadata={'status': 'cancelada', 'service_type': updated.service_type, 'motivo': motivo})
        return updated
