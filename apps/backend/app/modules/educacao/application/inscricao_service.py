...
class InscricaoService:
    def __init__(
        self,
        inscricao_repo: InscricaoRepositoryPort,
    ...
    async def criar_inscricao_basica(
        self, *, citizen_id: UUID, escola_id: UUID, observacoes: str | None = None
    ):
        return await self._criar_inscricao(
            tipo=TipoInscricao.BASICA,
    ...
    async def confirmar_inscricao(self, inscricao_id: UUID, actor_id: UUID):
        ...
        await self.request_service.mark_education_request_completed(
            ...
            actor_id=actor_id,
            metadata={"status": "confirmada", "tipo": updated.tipo.value},
        )
        return updated
    ...
    async def cancelar_inscricao(self, inscricao_id: UUID, actor_id: UUID, motivo: str):
        ...
        await self.request_service.mark_education_request_completed(
            entity_id=updated.id,
            actor_id=actor_id,
            metadata={"status": "cancelada", "motivo": motivo, "tipo": updated.tipo.value},
        )
        return updated
    ...
    async def _criar_inscricao(
        self, *, tipo: TipoInscricao, citizen_id: UUID, escola_id: UUID, observacoes: str | None
    ):
