...
class TransferenciaService:
    def __init__(
        self,
        repository: TransferenciaRepositoryPort,
    ...
    async def solicitar_transferencia(
        ...
        *,
        ...
        """Solicitar transferência COM validação pessimista.
        
        PASSO 6: Usa lock para validar capacidade, prevenindo race condition.
        """
        ...
        ocupacao_turma = await self.turma_repo.count_matriculas_ativas(
            turma_destino_id, matricula.ano_letivo_id, for_update=False
        )
        ...
        if transferencia_ativa is not None:
            raise TransferenciaDuplicadaError(
                "Ja existe solicitacao ativa de transferencia para esta matricula"
            )
        numero = await self.repository.next_numero_processo(
            date.today().year, self.service_type, self.process_prefix
        )
        record = WorkflowRecord(
            id=uuid4(),
            numero_processo=numero,
            service_type=self.service_type,
            citizen_id=matricula.citizen_id,
            instituicao_id=escola_destino_id,
            data_registo=date.today(),
            status=StatusFluxo.EM_ANALISE,
            observacoes=observacoes,
            metadata={
                "matricula_origem_id": str(matricula.id),
                "escola_origem_id": str(matricula.escola_id),
                "turma_origem_id": str(matricula.turma_id),
                "escola_destino_id": str(escola_destino_id),
                "turma_destino_id": str(turma_destino_id),
                "ano_letivo_id": str(matricula.ano_letivo_id),
                "motivo": motivo.strip(),
            },
        )
        ...
        if self.request_service is not None:
            await self.request_service.create_education_request(
                ...
                escola_nome=escola_destino.nome,
                ano_letivo=str(matricula.ano_letivo_id),
            )
        return saved

    async def aprovar_transferencia(
        self, *, transferencia_id: UUID, actor_id: UUID, resumo: str | None = None
    ) -> WorkflowRecord:
        """Aprovar transferência COM lock pessimista na turma destino.
        
        PASSO 6: Esta é a operação crítica onde happens a reserva de vaga.
        Sem lock pessimista, múltiplas aprovações simultâneas podem causar overbooking:
        
        ERRADO (sem lock):
        - Processo A: SELECT ocupacao=28 FROM turma WHERE id=X
        - Processo B: SELECT ocupacao=28 FROM turma WHERE id=X  
        - Processo A: IF ocupacao < capacidade(30): INSERT nova_matricula
        - Processo B: IF ocupacao < capacidade(30): INSERT nova_matricula
        - RESULTADO: ocupacao=30 (correto) MAS 2 INSERT simultâneos = overbooking!
        
        CORRETO (com lock):
        - Processo A: SELECT ocupacao=28 FROM turma WHERE id=X FOR UPDATE (lock)
        - Processo B: SELECT ocupacao=28 FROM turma WHERE id=X FOR UPDATE (aguarda)
        - Processo A: IF 28 < 30: INSERT, UPDATE ocupacao=29, COMMIT (libera lock)
        - Processo B: SELECT ocupacao=29 FROM turma (agora vê estado atualizado)
        - Processo B: IF 29 < 30: INSERT, UPDATE ocupacao=30, COMMIT
        """
        ...
        ano_letivo_id = self._metadata_uuid(metadata, "ano_letivo_id")
        ...
        if matricula_origem.status != StatusMatricula.ATIVA:
            raise InvalidMatriculaStateError(
                "Matricula de origem precisa estar ativa para concluir a transferencia"
            )
        turma_destino = await self.turma_repo.get_by_id(turma_destino_id)
        self._validar_turma_destino(
            turma_destino=turma_destino,
        ...
        # PASSO 6: Lock pessimista na turma destino para validar capacidade
        # ⚠️ CRÍTICO: Com for_update=True, garante atomicidade
        ocupacao_turma = await self.turma_repo.count_matriculas_ativas(
            turma_destino_id, ano_letivo_id, for_update=True
        )
        if ocupacao_turma >= turma_destino.capacidade:
            raise TurmaSemVagasError("Turma destino sem vagas disponiveis")
        ...
        numero_nova_matricula = await self.matricula_repo.next_numero_processo(
            date.today().year, escola_destino_id
        )
        nova_matricula = Matricula(
            id=uuid4(),
            numero_processo=numero_nova_matricula,
            citizen_id=matricula_origem.citizen_id,
            escola_id=escola_destino_id,
            turma_id=turma_destino_id,
            ano_letivo_id=matricula_origem.ano_letivo_id,
            data_matricula=date.today(),
            status=StatusMatricula.ATIVA,
            observacoes=f"Matricula criada por transferencia {transferencia.numero_processo}",
        )
        ...
        if self.request_service is not None:
            await self.request_service.mark_education_request_completed(
                ...
                actor_id=actor_id,
                metadata={"status": "aprovada", "nova_matricula_id": str(saved_nova_matricula.id)},
            )
        return updated

    async def rejeitar_transferencia(
        self, *, transferencia_id: UUID, actor_id: UUID, motivo: str
    ) -> WorkflowRecord:
        transferencia = await self.repository.get_by_id(transferencia_id)
        if not transferencia:
            raise TransferenciaNotFoundError("Transferencia nao encontrada")
        ...
        if self.request_service is not None:
            await self.request_service.mark_education_request_completed(
                entity_id=updated.id,
                actor_id=actor_id,
                metadata={"status": "rejeitada", "motivo": motivo.strip()},
            )
        return updated
    ...
    def _validar_estado_processavel(status: StatusFluxo) -> None:
        if status in {StatusFluxo.REJEITADA, StatusFluxo.CANCELADA, StatusFluxo.APROVADA}:
            raise TransferenciaEstadoInvalidoError(
                f"Transferencia ja finalizada com status {status.value}"
            )

    @staticmethod
    def _validar_turma_destino(
        *, turma_destino, turma_destino_id: UUID, escola_destino_id: UUID, ano_letivo_id: UUID
    ) -> None:
