...
from apps.backend.app.modules.educacao.domain.events import EducationInstitutionRegistered, StudentEnrolled, AcademicDegreeAwarded, EducationInstitutionStatusChanged, TeacherCertificationIssued
...
async def handle_education_institution_status_changed(event: EducationInstitutionStatusChanged) -> None:
...
async def handle_teacher_certification_issued(event: TeacherCertificationIssued) -> None:
    ...
    logger.info(f'Certification {event.certification_number} issued to teacher {event.aggregate_id}')
    pass
EDUCACAO_EVENT_HANDLERS: dict[str, Callable[[DomainEvent], Awaitable[None]]] = {'EducationInstitutionRegistered': handle_education_institution_registered, 'StudentEnrolled': handle_student_enrolled, 'AcademicDegreeAwarded': handle_academic_degree_awarded, 'EducationInstitutionStatusChanged': handle_education_institution_status_changed, 'TeacherCertificationIssued': handle_teacher_certification_issued}