...
class MatriculaService:
    ...
    async def criar_matricula(
        ...
        *,
        ...
        turma = await self.turma_repo.get_by_id(turma_id, for_update=True)
        if not turma:
        ...
        if nascimento is None:
            raise IdadeMinimaNaoAtendidaError(
                "Cidadao sem data de nascimento para validacao de idade minima"
            )
        ...
        if idade_cidadao < idade_minima:
            raise IdadeMinimaNaoAtendidaError(
                f"Idade minima nao atendida para a turma: minimo {idade_minima}, atual {idade_cidadao}"
            )
    ...
    def _idade_minima_por_ciclo(ciclos: list[CicloEnsino]) -> int:
        base = {
            CicloEnsino.PRE_ESCOLAR: 4,
            CicloEnsino.PRIMARIO: 6,
            CicloEnsino.SECUNDARIO_1: 12,
            CicloEnsino.SECUNDARIO_2: 15,
            CicloEnsino.TECNICO: 15,
            CicloEnsino.FORMACAO: 15,
        }
        if not ciclos:
            return 6
