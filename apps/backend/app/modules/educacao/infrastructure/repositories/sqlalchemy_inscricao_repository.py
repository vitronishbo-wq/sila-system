...
from apps.backend.app.modules.educacao.application.ports.inscricao_repository_port import (
    InscricaoEntity,
...
class SQLAlchemyInscricaoRepository(InscricaoRepositoryPort):
    ...
    async def exists_active_for_citizen(self, citizen_id: UUID, tipo: TipoInscricao) -> bool:
        stmt = select(InscricaoModel.id).where(
            and_(
                ...
                InscricaoModel.status.in_(
                    [StatusFluxo.PENDENTE.value, StatusFluxo.CONFIRMADA.value]
                ),
            )
        )
    ...
    async def next_numero_processo(self, ano: int, tipo: TipoInscricao) -> str:
        ...
        count_stmt = (
            select(func.count())
            .select_from(InscricaoModel)
            .where(InscricaoModel.numero_processo.like(f"{prefix}%"))
        )
