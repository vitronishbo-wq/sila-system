...
from apps.backend.app.modules.educacao.infrastructure.models.transferencia_model import (
    TransferenciaModel,
)
...
class SQLAlchemyTransferenciaRepository(SQLAlchemyWorkflowRepository, TransferenciaRepositoryPort):
    ...
    async def get_active_by_matricula(self, matricula_id: UUID) -> WorkflowRecord | None:
        stmt = (
            ...
            .where(
                TransferenciaModel.service_type == "transferencia",
                TransferenciaModel.metadata_json["matricula_origem_id"].astext == str(matricula_id),
                TransferenciaModel.status.in_(
                    [
            ...
            .order_by(TransferenciaModel.data_registo.desc())
            .limit(1)
        )
