...
def test_concurso_domain_transitions():
    item = DomainRecord(
        id=uuid4(),
        numero_processo="CNC/2026/0001",
        service_type="concurso_inscricao",
        citizen_id=uuid4(),
...
def test_concurso_service_create_and_complete():
    ...
    saved = WorkflowRecord(
        id=uuid4(),
        numero_processo="CNC/CONCURSO_INSCRICAO/2026/0002",
        service_type="concurso_inscricao",
    ...
    asyncio.run(
        service.create_record(
            service_type="concurso_inscricao", citizen_id=citizen_id, instituicao_id=instituicao_id
        )
    )
