...
from apps.backend.app.modules.educacao.domain.models.candidato_emprego import DomainRecord


def test_emprego_domain_transitions():
    item = DomainRecord(
        id=uuid4(),
        numero_processo="EMP/2026/0001",
        service_type="candidato_emprego",
        citizen_id=uuid4(),
...
def test_emprego_service_create_and_complete():
    ...
    saved = WorkflowRecord(
        id=uuid4(),
        numero_processo="EMP/CANDIDATO_EMPREGO/2026/0002",
        service_type="candidato_emprego",
    ...
    asyncio.run(
        service.create_record(
            service_type="candidato_emprego", citizen_id=citizen_id, instituicao_id=instituicao_id
        )
    )
