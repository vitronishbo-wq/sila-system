...
def test_propina_service_create_and_complete():
    citizen_id = uuid4()
    instituicao_id = uuid4()
    saved = WorkflowRecord(id=uuid4(), numero_processo='PRP/PROPINA/2026/0002', service_type='propina', citizen_id=citizen_id, instituicao_id=instituicao_id, data_registo=date.today())
    ...
    service = PropinaService(repository=repository, citizen_repo=SimpleNamespace(get_by_id=AsyncMock(return_value=SimpleNamespace(id=citizen_id))), request_service=SimpleNamespace(create_education_request=AsyncMock(return_value=uuid4()), mark_education_request_completed=AsyncMock(return_value=True)))
    asyncio.run(service.create_record(service_type='propina', citizen_id=citizen_id, instituicao_id=instituicao_id))
