...
from app.modules.educacao.domain.models.certificado_conclusao import DomainRecord
from app.modules.educacao.domain.models._workflow_record import WorkflowRecord

def test_certificado_domain_transitions():
    item = DomainRecord(id=uuid4(), numero_processo='CRT/2026/0001', service_type='certificado_conclusao', citizen_id=uuid4(), instituicao_id=uuid4(), data_registo=date.today())
    item.confirmar()
...
def test_certificado_service_create_and_complete():
    ...
    saved = WorkflowRecord(id=uuid4(), numero_processo='CRT/CERTIFICADO_CONCLUSAO/2026/0002', service_type='certificado_conclusao', citizen_id=citizen_id, instituicao_id=instituicao_id, data_registo=date.today())
    ...
    service = CertificadoService(repository=repository, citizen_repo=citizen_repo, request_service=request_service)
    asyncio.run(service.create_record(service_type='certificado_conclusao', citizen_id=citizen_id, instituicao_id=instituicao_id))
