...
def _build_service(*, turma: Turma, citizen_birth_date: date, ocupacao: int) -> MatriculaService:
    matricula_repo = SimpleNamespace(
        ...
        next_numero_processo=AsyncMock(return_value="2026/0012/0001"),
        save=AsyncMock(side_effect=lambda matricula: matricula),
    )
    turma_repo = SimpleNamespace(
        get_by_id=AsyncMock(return_value=turma),
        count_matriculas_ativas=AsyncMock(return_value=ocupacao),
    )
    escola_repo = SimpleNamespace(
        get_by_id=AsyncMock(
            return_value=SimpleNamespace(
                id=turma.escola_id, nome="Escola Integrada", ciclos=[CicloEnsino.PRIMARIO]
            )
        )
    )
    citizen_repo = SimpleNamespace(
        get_by_id=AsyncMock(
            return_value=SimpleNamespace(citizen_id=uuid4(), birth_date=citizen_birth_date)
        )
    )
...
def test_criar_matricula_rejeita_idade_abaixo_minimo() -> None:
    ...
    service = _build_service(
        turma=turma, citizen_birth_date=date.today() - timedelta(days=365 * 5), ocupacao=0
    )
    try:
        asyncio.run(
        ...
        raise AssertionError("Esperava IdadeMinimaNaoAtendidaError")
...
def test_criar_matricula_rejeita_turma_sem_vagas() -> None:
    ...
    service = _build_service(
        turma=turma, citizen_birth_date=date.today() - timedelta(days=365 * 14), ocupacao=1
    )
    try:
        asyncio.run(
            service.criar_matricula(
        ...
        raise AssertionError("Esperava TurmaSemVagasError")
...
def test_criar_matricula_aplica_regras_e_rastreia_request() -> None:
    ...
    service = _build_service(
        turma=turma, citizen_birth_date=date.today() - timedelta(days=365 * 15), ocupacao=4
    )
    matricula = asyncio.run(
        service.criar_matricula(
