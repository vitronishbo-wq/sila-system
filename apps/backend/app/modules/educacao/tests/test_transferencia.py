...
def _base_fixture():
    ...
    matricula = Matricula(
        id=uuid4(),
        numero_processo="2026/0012/0001",
        citizen_id=citizen_id,
        escola_id=escola_origem_id,
        turma_id=turma_origem_id,
...
def test_solicitar_transferencia_fluxo_real() -> None:
    ...
    record = SimpleNamespace(
        id=uuid4(),
        numero_processo="TRF/TRANSFERENCIA/2026/0001",
        service_type="transferencia",
        citizen_id=fx.citizen_id,
        instituicao_id=fx.escola_destino_id,
    ...
    repository = SimpleNamespace(
        get_active_by_matricula=AsyncMock(return_value=None),
        next_numero_processo=AsyncMock(return_value=record.numero_processo),
        save=AsyncMock(return_value=record),
    )
    service = TransferenciaService(
        repository=repository,
        matricula_repo=SimpleNamespace(get_by_id=AsyncMock(return_value=fx.matricula)),
        escola_repo=SimpleNamespace(
            get_by_id=AsyncMock(return_value=SimpleNamespace(nome="Destino"))
        ),
        turma_repo=SimpleNamespace(
            get_by_id=AsyncMock(return_value=fx.turma_destino),
            count_matriculas_ativas=AsyncMock(return_value=10),
        ),
        citizen_repo=SimpleNamespace(
            get_by_id=AsyncMock(return_value=SimpleNamespace(id=fx.citizen_id))
        ),
    ...
    created = asyncio.run(
        service.solicitar_transferencia(
            matricula_id=fx.matricula.id,
            escola_destino_id=fx.escola_destino_id,
            turma_destino_id=fx.turma_destino_id,
...
def test_solicitar_transferencia_bloqueia_duplicada_por_matricula() -> None:
    fx = _base_fixture()
    repository = SimpleNamespace(
        get_active_by_matricula=AsyncMock(return_value=SimpleNamespace(id=uuid4())),
        next_numero_processo=AsyncMock(),
        save=AsyncMock(),
    )
    service = TransferenciaService(
        repository=repository,
        ...
        turma_repo=SimpleNamespace(
            get_by_id=AsyncMock(return_value=fx.turma_destino),
            count_matriculas_ativas=AsyncMock(return_value=2),
        ),
        citizen_repo=SimpleNamespace(
            get_by_id=AsyncMock(return_value=SimpleNamespace(id=fx.citizen_id))
        ),
        request_service=SimpleNamespace(create_education_request=AsyncMock()),
    )
    try:
        asyncio.run(
            service.solicitar_transferencia(
        ...
        raise AssertionError("Esperava TransferenciaDuplicadaError")
...
def test_aprovar_transferencia_cria_nova_matricula_e_encerra_antiga() -> None:
    ...
    transferencia = SimpleNamespace(
        ...
        observacoes=None,
        metadata={
            "matricula_origem_id": str(fx.matricula.id),
            "escola_origem_id": str(fx.escola_origem_id),
            "turma_origem_id": str(fx.turma_origem_id),
            "escola_destino_id": str(fx.escola_destino_id),
            "turma_destino_id": str(fx.turma_destino_id),
            "ano_letivo_id": str(fx.ano_letivo_id),
        },
    )
    ...
    service = TransferenciaService(
        repository=SimpleNamespace(
            get_by_id=AsyncMock(return_value=transferencia),
            save=AsyncMock(side_effect=lambda item: item),
        ),
        matricula_repo=SimpleNamespace(
            get_by_id=AsyncMock(return_value=fx.matricula),
            save=AsyncMock(side_effect=save_matricula),
            next_numero_processo=AsyncMock(return_value="2026/0007/0042"),
        ),
        escola_repo=SimpleNamespace(
            get_by_id=AsyncMock(return_value=SimpleNamespace(nome="Destino"))
        ),
        turma_repo=SimpleNamespace(
            get_by_id=AsyncMock(return_value=fx.turma_destino),
            count_matriculas_ativas=AsyncMock(return_value=12),
        ),
        citizen_repo=None,
        request_service=SimpleNamespace(
            mark_education_request_completed=AsyncMock(return_value=True)
    ...
    updated = asyncio.run(
        service.aprovar_transferencia(
            transferencia_id=transferencia.id, actor_id=uuid4(), resumo="Aprovada pela direcao"
        )
    )
