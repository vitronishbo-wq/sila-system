...
from app.modules.educacao.application.services.inscricao_service import InscricaoService
...
def test_inscricao_basica_domain_transitions():
    inscricao = InscricaoBasica(
        id=uuid4(),
        numero_processo="INS/BASICA/2026/0001",
        citizen_id=uuid4(),
        escola_id=uuid4(),
...
def test_inscricao_service_cria_e_rastreia_request():
    citizen_id = uuid4()
    escola_id = uuid4()
    saved = InscricaoBasica(
        id=uuid4(),
        numero_processo="INS/BASICA/2026/0002",
        citizen_id=citizen_id,
        escola_id=escola_id,
        data_inscricao=date.today(),
    )
    inscricao_repo = SimpleNamespace(
    ...
    escola_repo = SimpleNamespace(
        get_by_id=AsyncMock(return_value=SimpleNamespace(nome="Escola Teste")),
    )
