...
@dataclass
class Prescription:
    """Prescricao medica - Aggregate Root."""
    ...
    def __post_init__(self) -> None:
        if not self.items:
            raise ValueError('Prescricao deve ter pelo menos um item')
    ...
    def add_item(self, item: PrescriptionItem) -> None:
        """Adiciona item a prescricao."""
        self.items.append(item)
        self.updated_at = datetime.now()

    def remove_item(self, medication_code: str) -> None:
        """Remove item da prescricao."""
        self.items = [i for i in self.items if i.medication_code != medication_code]
        self.updated_at = datetime.now()

    def dispense(self, items_dispensed: List[str] | None=None) -> None:
        """Registra dispensacao."""
    ...
    def to_dict(self) -> dict:
        return {'id': str(self.id), 'prescription_number': self.prescription_number, 'citizen_id': str(self.citizen_id), 'doctor_id': str(self.doctor_id), 'appointment_id': str(self.appointment_id) if self.appointment_id else None, 'health_unit_id': str(self.health_unit_id), 'items': [{'medication_code': i.medication_code, 'medication_name': i.medication_name, 'dosage': i.dosage, 'frequency': i.frequency, 'duration_days': i.duration_days, 'quantity': i.quantity, 'unit': i.unit, 'notes': i.notes, 'medication_type': i.medication_type.value} for i in self.items], 'clinical_notes': self.clinical_notes, 'recommendations': self.recommendations, 'issue_date': self.issue_date.isoformat(), 'expiry_date': self.expiry_date.isoformat() if self.expiry_date else None, 'status': self.status.value, 'is_active': self.is_active, 'is_expired': self.is_expired, 'workflow_instance_id': str(self.workflow_instance_id) if self.workflow_instance_id else None, 'created_at': self.created_at.isoformat(), 'updated_at': self.updated_at.isoformat() if self.updated_at else None, 'dispensed_at': self.dispensed_at.isoformat() if self.dispensed_at else None}