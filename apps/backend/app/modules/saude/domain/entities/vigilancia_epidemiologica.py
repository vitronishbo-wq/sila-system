...
class VigilanciaEpidemiologica:
    ...
    def start_investigation(self) -> None:
        if self.status != VigilanciaStatus.REPORTED:
            raise ValueError(f'Nao e possivel iniciar investigacao em status {self.status}')
        self.status = VigilanciaStatus.INVESTIGATING
        self.investigating_at = datetime.utcnow()

    def contain(self) -> None:
        if self.status not in {VigilanciaStatus.REPORTED, VigilanciaStatus.INVESTIGATING}:
    ...
    def close(self, summary: str) -> None:
        if self.status not in {VigilanciaStatus.CONTAINED, VigilanciaStatus.INVESTIGATING}:
    ...
    def to_dict(self) -> dict:
        return {'id': str(self.id), 'health_unit_id': str(self.health_unit_id), 'reported_by': str(self.reported_by), 'disease_name': self.disease_name, 'municipality': self.municipality, 'suspected_cases': self.suspected_cases, 'confirmed_cases': self.confirmed_cases, 'status': self.status.value, 'notes': self.notes, 'created_at': self.created_at.isoformat(), 'investigating_at': self.investigating_at.isoformat() if self.investigating_at else None, 'contained_at': self.contained_at.isoformat() if self.contained_at else None, 'closed_at': self.closed_at.isoformat() if self.closed_at else None, 'closure_summary': self.closure_summary}