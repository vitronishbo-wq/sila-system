...
class OperationsService:
    ...
    async def generate_payment(self, order_id, citizen_id):
        ...
        payment = SimpleNamespace(
            id=uuid4(),
            order_id=order.id,
            reference=f"SIM-{str(order.id).replace('-', '')[:8].upper()}",
            amount=Decimal(getattr(order, "total_amount", Decimal("0"))),
    ...
    async def complete_order(self, order_id, citizen_id):
        ...
        if not getattr(order, "receipt_number", None):
            order.receipt_number = (
                f"RCP-{datetime.now(UTC):%Y%m%d}-{str(order.id).replace('-', '')[:8].upper()}"
            )
        order.proof_payload = {
            "receipt_number": order.receipt_number,
            "order_id": str(order.id),
            "citizen_id": str(order.citizen_id),
            "service_id": str(getattr(order, "service_id", "")),
            "amount": float(getattr(order, "total_amount", Decimal("0"))),
            "status": order.status,
