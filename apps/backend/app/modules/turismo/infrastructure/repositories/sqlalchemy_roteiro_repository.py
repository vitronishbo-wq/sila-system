...
from app.modules.turismo.domain.models.roteiro import Roteiro
from app.modules.turismo.infrastructure.models.roteiro_model import RoteiroModel
...
class SQLAlchemyRoteiroRepository(RoteiroRepositoryPort):
    ...
    async def save(self, roteiro: Roteiro) -> Roteiro:
        model = RoteiroModel(
            ...
            municipio_origem=roteiro.municipio_origem,
            provincia_origem=roteiro.provincia_origem,
            duracao_horas=roteiro.duracao_horas,
            pontos_parada=list(roteiro.pontos_parada),
            acessivel=roteiro.acessivel,
            ativo=roteiro.ativo,
            valor_estimado=roteiro.valor_estimado,
            meios_transporte_sugeridos=list(roteiro.meios_transporte_sugeridos or []),
            parceiros_comerciais=list(roteiro.parceiros_comerciais or []),
    ...
    def _to_domain(model: RoteiroModel) -> Roteiro:
        return Roteiro(
            ...
            municipio_origem=model.municipio_origem,
            provincia_origem=model.provincia_origem,
            duracao_horas=model.duracao_horas,
            pontos_parada=list(model.pontos_parada),
            acessivel=model.acessivel,
            ativo=model.ativo,
            valor_estimado=model.valor_estimado,
            meios_transporte_sugeridos=list(model.meios_transporte_sugeridos or []),
            parceiros_comerciais=list(model.parceiros_comerciais or []),
