...
class PesquisadorService:
    def __init__(
        ...
        *,
        ...
        instituicao_repo: InstituicaoPesquisaRepositoryPort | None = None,
    ) -> None:
    ...
    async def listar_pesquisadores(
        self, *, instituicao_id: UUID | None = None
    ) -> list[Pesquisador]:
    ...
    async def vincular_instituicao(
        ...
        if self.instituicao_repo is not None:
            ...
            if instituicao is None:
                raise ValueError("Instituicao de pesquisa nao encontrada")
        atualizado = await self.pesquisador_repo.vincular_instituicao(
        ...
        pesquisador.atualizar_vinculo(
            status_vinculo=StatusVinculoPesquisador.ATIVO, unidade_pesquisa_id=unidade_pesquisa_id
        )
        return await self.pesquisador_repo.save(pesquisador)

    async def encerrar_vinculo(
        self, *, pesquisador_id: UUID, data_fim_vinculo: date | None = None
    ) -> Pesquisador:
        pesquisador = await self.buscar_pesquisador(pesquisador_id)
