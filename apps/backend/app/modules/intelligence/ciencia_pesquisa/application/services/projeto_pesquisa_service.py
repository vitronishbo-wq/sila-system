...
from app.modules.intelligence.ciencia_pesquisa.domain.enums import AreaConhecimento
...
class ProjetoPesquisaService:
    ...
    def __init__(self, *, projeto_repo: ProjetoPesquisaRepositoryPort, instituicao_repo: InstituicaoPesquisaRepositoryPort, pesquisador_repo: PesquisadorRepositoryPort) -> None:
        self.projeto_repo = projeto_repo
        self.instituicao_repo = instituicao_repo
        self.pesquisador_repo = pesquisador_repo

    async def cadastrar_projeto(self, *, titulo: str, resumo: str, instituicao_id: UUID, coordenador_id: UUID, equipe_pesquisadores_ids: list[UUID] | None=None, area_conhecimento: AreaConhecimento=AreaConhecimento.MULTIDISCIPLINAR, data_inicio: date | None=None, data_fim_prevista: date | None=None, palavras_chave: list[str] | None=None, orcamento_previsto: float | None=None, codigo_projeto: str | None=None) -> ProjetoPesquisa:
        ...
        projeto = ProjetoPesquisa.cadastrar(codigo_projeto=codigo, titulo=titulo, resumo=resumo, instituicao_id=instituicao_id, coordenador_id=coordenador_id, equipe_pesquisadores_ids=equipe_ids, area_conhecimento=area_conhecimento, data_inicio=data_inicio, data_fim_prevista=data_fim_prevista, palavras_chave=palavras_chave, orcamento_previsto=orcamento_previsto)
    ...
    async def listar_projetos(self, *, instituicao_id: UUID | None=None, pesquisador_id: UUID | None=None) -> list[ProjetoPesquisa]:
    ...
    async def vincular_pesquisadores(self, *, projeto_id: UUID, pesquisador_ids: list[UUID]) -> ProjetoPesquisa:
        for pesquisador_id in pesquisador_ids:
            pesquisador = await self.pesquisador_repo.get_by_id(pesquisador_id)
            if pesquisador is None:
                raise ValueError('Pesquisador da equipe nao encontrado')
        atualizado = await self.projeto_repo.vincular_pesquisadores(projeto_id=projeto_id, pesquisador_ids=pesquisador_ids)
    ...
    async def encerrar_projeto(self, *, projeto_id: UUID, data_fim_real: date | None=None) -> ProjetoPesquisa:
        projeto = await self.buscar_projeto(projeto_id)
