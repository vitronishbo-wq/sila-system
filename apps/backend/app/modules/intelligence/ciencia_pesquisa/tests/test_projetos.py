...
def test_cadastrar_projeto_sucesso() -> None:
    ...
    async def scenario() -> None:
        ...
        service = ProjetoPesquisaService(projeto_repo=projeto_repo, instituicao_repo=instituicao_repo, pesquisador_repo=pesquisador_repo)
        instituicao = await _seed_instituicao(instituicao_repo)
        coordenador = await _seed_pesquisador(pesquisador_repo, 'Laura Mendes', 'BI010101', 'laura@incp.ao')
        projeto = await service.cadastrar_projeto(titulo='Projeto Integrado de Biotecnologia', resumo='Pesquisa aplicada para desenvolvimento de biotecnologia em saude.', instituicao_id=instituicao.id, coordenador_id=coordenador.id, area_conhecimento=AreaConhecimento.CIENCIAS_BIOLOGICAS, data_inicio=date(2026, 3, 2), data_fim_prevista=date(2027, 3, 2), palavras_chave=['biotecnologia', 'inovacao'])
...
def test_cadastrar_projeto_falha_sem_instituicao() -> None:
    ...
    async def scenario() -> None:
        service = ProjetoPesquisaService(projeto_repo=InMemoryProjetoPesquisaRepository(), instituicao_repo=InMemoryInstituicaoPesquisaRepository(), pesquisador_repo=InMemoryPesquisadorRepository())
        coordenador_repo = service.pesquisador_repo
        coordenador = await _seed_pesquisador(coordenador_repo, 'Paulo Costa', 'BI020202', 'paulo@incp.ao')
        with pytest.raises(ValueError, match='Instituicao'):
            await service.cadastrar_projeto(titulo='Projeto sem Instituicao', resumo='Resumo valido para o cadastro do projeto.', instituicao_id=uuid4(), coordenador_id=coordenador.id)
    asyncio.run(scenario())
...
def test_vincular_e_encerrar_projeto() -> None:
    ...
    async def scenario() -> None:
        ...
        service = ProjetoPesquisaService(projeto_repo=projeto_repo, instituicao_repo=instituicao_repo, pesquisador_repo=pesquisador_repo)
        instituicao = await _seed_instituicao(instituicao_repo)
        coordenador = await _seed_pesquisador(pesquisador_repo, 'Rita Dias', 'BI030303', 'rita@incp.ao')
        pesquisador2 = await _seed_pesquisador(pesquisador_repo, 'Joao Pires', 'BI040404', 'joao@incp.ao')
        projeto = await service.cadastrar_projeto(titulo='Projeto de Agricultura de Precisao', resumo='Pesquisa de tecnicas e sensores para agricultura de precisao.', instituicao_id=instituicao.id, coordenador_id=coordenador.id, area_conhecimento=AreaConhecimento.CIENCIAS_AGRARIAS)
        atualizado = await service.vincular_pesquisadores(projeto_id=projeto.id, pesquisador_ids=[pesquisador2.id])
        assert pesquisador2.id in atualizado.equipe_pesquisadores_ids
        encerrado = await service.encerrar_projeto(projeto_id=projeto.id, data_fim_real=date(2026, 12, 31))
...
def test_fluxo_status_projeto() -> None:
    ...
    async def scenario() -> None:
        ...
        service = ProjetoPesquisaService(projeto_repo=projeto_repo, instituicao_repo=instituicao_repo, pesquisador_repo=pesquisador_repo)
        instituicao = await _seed_instituicao(instituicao_repo)
        coordenador = await _seed_pesquisador(pesquisador_repo, 'Andre Lima', 'BI050505', 'andre@incp.ao')
        projeto = await service.cadastrar_projeto(titulo='Projeto de Energia Limpa', resumo='Pesquisa sobre modelos sustentaveis de energia limpa.', instituicao_id=instituicao.id, coordenador_id=coordenador.id, area_conhecimento=AreaConhecimento.ENGENHARIAS)
