...
def test_contract_pesquisador_repository_crud_e_vinculacao() -> None:
    ...
    async def scenario() -> None:
        ...
        pesquisador = Pesquisador.cadastrar(
            ...
            documento_identificacao="BI123456",
            email_institucional="ana.silva@inst.ao",
            area_conhecimento=AreaConhecimento.CIENCIAS_SAUDE,
            nivel_formacao=NivelFormacao.DOUTOR,
            tipo_vinculo=TipoVinculoPesquisador.EFETIVO,
        )
        ...
        vinculado = await repo.vincular_instituicao(
            pesquisador_id=saved.id, instituicao_id=instituicao_id, unidade_pesquisa_id=None
        )
...
def test_contract_instituicao_repository_crud() -> None:
    ...
    async def scenario() -> None:
        ...
        instituicao = InstituicaoPesquisa.cadastrar(
            sigla="INIS",
            nome="Instituto Nacional de Investigacao em Saude",
            nif="500000001",
            ...
            provincia="Luanda",
            municipio="Luanda",
            endereco="Rua da Ciencia, 10",
            email_institucional="contato@inis.ao",
        )
...
def test_contract_projeto_repository_crud_e_vinculacao() -> None:
    ...
    async def scenario() -> None:
        ...
        projeto = ProjetoPesquisa.cadastrar(
            codigo_projeto=await repo.next_codigo(),
            titulo="Projeto de Vigilancia Epidemiologica",
            resumo="Projeto para ampliar a capacidade de pesquisa epidemiologica.",
        ...
        atualizado = await repo.vincular_pesquisadores(
            projeto_id=saved.id, pesquisador_ids=[pesquisador_extra]
        )
