...
from apps.backend.app.modules.intelligence.ciencia_pesquisa.api.deps import (
    get_projeto_pesquisa_service,
)
from apps.backend.app.modules.intelligence.ciencia_pesquisa.api.schemas.projeto_pesquisa_schema import (
    ProjetoPesquisaCreate,
    ProjetoPesquisaEncerrarInput,
    ProjetoPesquisaResponse,
    ProjetoPesquisaVincularPesquisadoresInput,
)
...
projeto_pesquisa_service_dep = Depends(get_projeto_pesquisa_service)
...
async def cadastrar_projeto(
    data: ProjetoPesquisaCreate,
    service: ProjetoPesquisaService = projeto_pesquisa_service_dep,
) -> ProjetoPesquisaResponse:
    try:
        return await service.cadastrar_projeto(
            titulo=data.titulo,
            resumo=data.resumo,
            instituicao_id=data.instituicao_id,
            coordenador_id=data.coordenador_id,
            equipe_pesquisadores_ids=data.equipe_pesquisadores_ids,
            area_conhecimento=data.area_conhecimento,
            data_inicio=data.data_inicio,
            data_fim_prevista=data.data_fim_prevista,
            palavras_chave=data.palavras_chave,
            orcamento_previsto=data.orcamento_previsto,
...
async def obter_projeto(
    projeto_id: UUID, service: ProjetoPesquisaService = projeto_pesquisa_service_dep
) -> ProjetoPesquisaResponse:
    try:
...
async def listar_projetos(
    ...
    service: ProjetoPesquisaService = projeto_pesquisa_service_dep,
) -> list[ProjetoPesquisaResponse]:
    return await service.listar_projetos(
        instituicao_id=instituicao_id, pesquisador_id=pesquisador_id
    )
...
async def vincular_pesquisadores(
    projeto_id: UUID,
    data: ProjetoPesquisaVincularPesquisadoresInput,
    service: ProjetoPesquisaService = projeto_pesquisa_service_dep,
) -> ProjetoPesquisaResponse:
    try:
        return await service.vincular_pesquisadores(
            projeto_id=projeto_id, pesquisador_ids=data.pesquisador_ids
        )
    except ValueError as exc:
        raise HTTPException(status_code=_status_code_for_error(str(exc)), detail=str(exc)) from exc
...
async def iniciar_execucao_projeto(
    projeto_id: UUID, service: ProjetoPesquisaService = projeto_pesquisa_service_dep
) -> ProjetoPesquisaResponse:
    try:
...
async def encerrar_projeto(
    projeto_id: UUID,
    data: ProjetoPesquisaEncerrarInput,
    service: ProjetoPesquisaService = projeto_pesquisa_service_dep,
) -> ProjetoPesquisaResponse:
    try:
        return await service.encerrar_projeto(
            projeto_id=projeto_id, data_fim_real=data.data_fim_real
        )
    except ValueError as exc:
        raise HTTPException(status_code=_status_code_for_error(str(exc)), detail=str(exc)) from exc
