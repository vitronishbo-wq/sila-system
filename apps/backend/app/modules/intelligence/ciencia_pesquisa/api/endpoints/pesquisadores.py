...
from apps.backend.app.modules.intelligence.ciencia_pesquisa.api.schemas.pesquisador_schema import (
    PesquisadorCreate,
    PesquisadorEncerrarVinculoInput,
    PesquisadorResponse,
    PesquisadorVincularInstituicaoInput,
)
...
pesquisador_service_dep = Depends(get_pesquisador_service)
...
async def cadastrar_pesquisador(
    data: PesquisadorCreate, service: PesquisadorService = pesquisador_service_dep
) -> PesquisadorResponse:
    try:
        return await service.cadastrar_pesquisador(
            ...
            unidade_pesquisa_id=data.unidade_pesquisa_id,
            area_conhecimento=data.area_conhecimento,
            nivel_formacao=data.nivel_formacao,
            tipo_vinculo=data.tipo_vinculo,
            data_inicio_vinculo=data.data_inicio_vinculo,
...
async def obter_pesquisador(
    pesquisador_id: UUID, service: PesquisadorService = pesquisador_service_dep
) -> PesquisadorResponse:
    try:
...
async def listar_pesquisadores(
    ...
    service: PesquisadorService = pesquisador_service_dep,
) -> list[PesquisadorResponse]:
...
async def vincular_instituicao(
    pesquisador_id: UUID,
    data: PesquisadorVincularInstituicaoInput,
    service: PesquisadorService = pesquisador_service_dep,
) -> PesquisadorResponse:
    try:
        return await service.vincular_instituicao(
...
async def encerrar_vinculo(
    pesquisador_id: UUID,
    data: PesquisadorEncerrarVinculoInput,
    service: PesquisadorService = pesquisador_service_dep,
) -> PesquisadorResponse:
    try:
        return await service.encerrar_vinculo(
            pesquisador_id=pesquisador_id, data_fim_vinculo=data.data_fim_vinculo
        )
    except ValueError as exc:
        raise HTTPException(status_code=_status_code_for_error(str(exc)), detail=str(exc)) from exc
