...
class PlanoClassificacaoResponseSchema(BaseModel):
    ...
    def from_domain(cls, plano) -> "PlanoClassificacaoResponseSchema":
        ...
        return cls(
            ...
            orgao_id=plano.orgao_id,
            ativo=plano.ativo,
            data_criacao=plano.data_criacao,
            data_atualizacao=plano.data_atualizacao,
            classes_count=len(plano.classes) if plano.classes else 0,
        )
...
class ClasseCreateSchema(BaseModel):
    ...
    descricao: str | None = Field(None, max_length=500, description="Descrição da classe")
...
class ClasseResponseSchema(BaseModel):
    ...
    def from_domain(cls, classe) -> "ClasseResponseSchema":
        ...
        return cls(
            id=classe.id,
            plano_id=classe.plano_id,
            codigo=classe.codigo,
            titulo=classe.titulo,
            descricao=classe.descricao,
            grau=classe.grau,
            prazo_guarda_corrente=classe.prazo_guarda_corrente,
            prazo_guarda_intermediaria=classe.prazo_guarda_intermediaria,
            destino_final=classe.destino_final,
            ativo=classe.ativo,
            data_criacao=classe.data_criacao,
        )
...
class SubclasseCreateSchema(BaseModel):
    ...
    descricao: str | None = Field(None, max_length=500, description="Descrição da subclasse")
    prazo_guarda_corrente: int | None = Field(
        None,
        gt=0,
        description="Prazo de guarda corrente em anos (herda da classe se não informado)",
    )
    prazo_guarda_intermediaria: int | None = Field(
        None, ge=0, description="Prazo de guarda intermediária em anos"
    )
    destino_final: str | None = Field(
        None, description="Destino final (herda da classe se não informado)"
    )
...
class SubclasseResponseSchema(BaseModel):
    ...
    def from_domain(cls, subclasse) -> "SubclasseResponseSchema":
        ...
        return cls(
            id=subclasse.id,
            classe_id=subclasse.classe_id,
            codigo=subclasse.codigo,
            titulo=subclasse.titulo,
            descricao=subclasse.descricao,
            prazo_guarda_corrente=subclasse.prazo_guarda_corrente,
            prazo_guarda_intermediaria=subclasse.prazo_guarda_intermediaria,
            destino_final=subclasse.destino_final,
            ativo=subclasse.ativo,
            data_criacao=subclasse.data_criacao,
        )
...
class ClasseUpdateSchema(BaseModel):
    ...
    titulo: str | None = Field(None, min_length=5, max_length=255, description="Novo título")
    descricao: str | None = Field(None, max_length=500, description="Nova descrição")
    prazo_guarda_corrente: int | None = Field(
