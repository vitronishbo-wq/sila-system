...
class TabelaTemporalidadeResponseSchema(BaseModel):
    ...
    def from_domain(cls, tabela) -> "TabelaTemporalidadeResponseSchema":
        ...
        return cls(
            ...
            orgao_id=tabela.orgao_id,
            base_legal=tabela.base_legal,
            ativo=tabela.ativo,
            data_criacao=tabela.data_criacao,
            data_atualizacao=tabela.data_atualizacao,
            regras_count=len(tabela.regras) if tabela.regras else 0,
        )
...
class RegraTemporalidadeCreateSchema(BaseModel):
    ...
    classe_id: UUID | None = Field(
        None, description="ID da classe (opcional se subclasse informada)"
    )
    subclasse_id: UUID | None = Field(
        None, description="ID da subclasse (opcional se classe informada)"
    )
    prazo_guarda_corrente: int = Field(..., gt=0, description="Prazo de guarda corrente em anos")
    prazo_guarda_intermediaria: int | None = Field(
        None, ge=0, description="Prazo de guarda intermediária em anos"
    )
    destino_final: str = Field(..., description="Destino final (eliminação ou guarda permanente)")
    observacoes: str | None = Field(None, max_length=500, description="Observações sobre a regra")
...
class RegraTemporalidadeResponseSchema(BaseModel):
    ...
    subclasse_id: UUID | None = None
    prazo_guarda_corrente: int
    prazo_guarda_intermediaria: int | None = None
    destino_final: str
    ...
    def from_domain(cls, regra) -> "RegraTemporalidadeResponseSchema":
        ...
        return cls(
            id=regra.id,
            tabela_id=regra.tabela_id,
            classe_id=regra.classe_id,
            subclasse_id=regra.subclasse_id,
            prazo_guarda_corrente=regra.prazo_guarda_corrente,
            prazo_guarda_intermediaria=regra.prazo_guarda_intermediaria,
            destino_final=regra.destino_final,
            observacoes=regra.observacoes,
            ativo=regra.ativo,
            data_criacao=regra.data_criacao,
            data_atualizacao=regra.data_atualizacao,
        )
...
class EventoTemporalidadeCreateSchema(BaseModel):
    ...
    tipo_evento: str = Field(
        ..., description="Tipo do evento (início_guarda, transferência, eliminação)"
    )
    data_evento: datetime = Field(..., description="Data do evento")
    responsavel_id: str = Field(..., description="ID do responsável pelo evento")
    observacoes: str | None = Field(None, max_length=500, description="Observações do evento")
...
class EventoTemporalidadeResponseSchema(BaseModel):
    ...
    def from_domain(cls, evento) -> "EventoTemporalidadeResponseSchema":
        ...
        return cls(
            ...
            regra_id=evento.regra_id,
            tipo_evento=evento.tipo_evento,
            data_evento=evento.data_evento,
            responsavel_id=evento.responsavel_id,
            observacoes=evento.observacoes,
            data_registro=evento.data_registro,
        )
...
class TabelaTemporalidadeUpdateSchema(BaseModel):
    ...
    titulo: str | None = Field(None, min_length=5, max_length=255, description="Novo título")
    descricao: str | None = Field(None, max_length=1000, description="Nova descrição")
    base_legal: str | None = Field(None, min_length=5, description="Nova base legal")
    ativo: bool | None = Field(None, description="Novo status ativo")


class RegraTemporalidadeUpdateSchema(BaseModel):
    """Schema para atualização de regra de temporalidade."""

    prazo_guarda_corrente: int | None = Field(
        None, gt=0, description="Novo prazo de guarda corrente"
    )
    prazo_guarda_intermediaria: int | None = Field(
        None, ge=0, description="Novo prazo de guarda intermediária"
    )
    destino_final: str | None = Field(None, description="Novo destino final")
    observacoes: str | None = Field(None, max_length=500, description="Novas observações")
    ativo: bool | None = Field(None, description="Novo status ativo")
...
class AplicarTemporalidadeSchema(BaseModel):
    ...
    observacoes: str | None = Field(None, max_length=500, description="Observações da aplicação")
