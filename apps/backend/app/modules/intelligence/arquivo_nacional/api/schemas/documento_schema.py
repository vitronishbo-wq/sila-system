...
class DocumentoCreateSchema(BaseModel):
    ...
    localizacao_fisica: Optional[Dict[str, Any]] = Field(default_factory=dict, description='Localização física')
...
class DocumentoResponseSchema(BaseModel):
    ...
    localizacao_fisica: Optional[Dict[str, Any]] = None
    ...
    def from_domain(cls, documento) -> 'DocumentoResponseSchema':
        ...
        return cls(id=documento.id, codigo_referencia=documento.codigo_referencia, titulo=documento.titulo, tipo=documento.tipo.value, suporte=documento.suporte.value, data_producao=documento.data_producao, data_registro=documento.data_registro, orgao_produtor_id=documento.orgao_produtor_id, classificacao_id=documento.classificacao_id, num_paginas=documento.num_paginas, estado_conservacao=documento.estado_conservacao.value, grau_restricao=documento.grau_restricao.value, fase_arquivistica=documento.fase_arquivistica.value, status=documento.status, digitalizado=documento.arquivo_digital_path is not None, versao=documento.versao, descricao=documento.descricao, localizacao_fisica=documento.localizacao_fisica)
...
class DocumentoRestricaoSchema(BaseModel):
    ...
    validade_ate: Optional[datetime] = Field(None, description='Data de validade da restrição')
...
class DocumentoConservacaoSchema(BaseModel):
    ...
    intervencao_descricao: Optional[str] = Field(None, description='Descrição da intervenção')
