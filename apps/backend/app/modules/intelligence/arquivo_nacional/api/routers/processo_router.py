...
async def criar_processo(processo_data: ProcessoCreateSchema, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
...
async def obter_processo(processo_id: UUID, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
...
async def listar_processos(skip: int=Query(0, ge=0), limit: int=Query(100, ge=1, le=1000), numero: Optional[str]=None, assunto: Optional[str]=None, status: Optional[str]=None, orgao_origem_id: Optional[UUID]=None, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
    """Lista processos com filtros opcionais."""
    processos = await service.listar_processos(db, skip=skip, limit=limit, numero=numero, assunto=assunto, status=status, orgao_origem_id=orgao_origem_id)
...
async def atualizar_processo(processo_id: UUID, processo_data: ProcessoCreateSchema, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
...
async def excluir_processo(processo_id: UUID, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
    """Exclui um processo."""
...
async def tramitar_processo(processo_id: UUID, tramitacao_data: ProcessoTramitacaoSchema, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
...
async def receber_processo(processo_id: UUID, recebimento_data: ProcessoRecebimentoSchema, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
...
async def encerrar_processo(processo_id: UUID, encerramento_data: ProcessoEncerramentoSchema, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
    """Encerra um processo."""
    try:
...
async def arquivar_processo(processo_id: UUID, arquivamento_data: ProcessoArquivamentoSchema, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
    """Arquiva um processo."""
    try:
...
async def autuar_documento_processo(processo_id: UUID, autuacao_data: ProcessoAutuarDocumentoSchema, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
...
async def obter_historico_tramitacao(processo_id: UUID, db: AsyncSession=Depends(get_db), service: ProcessoService=Depends()):
