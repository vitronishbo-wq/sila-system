...
from apps.backend.app.modules.intelligence.arquivo_nacional.application.services.tabela_temporalidade_service import (
    TabelaTemporalidadeService,
)
...
async def criar_tabela_temporalidade(
    tabela_data: TabelaTemporalidadeCreateSchema,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
async def obter_tabela_temporalidade(
    tabela_id: UUID,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
async def listar_tabelas_temporalidade(
    ...
    tabelas = await service.listar_tabelas_temporalidade(
...
async def atualizar_tabela_temporalidade(
    tabela_id: UUID,
    tabela_data: TabelaTemporalidadeUpdateSchema,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
async def excluir_tabela_temporalidade(
    tabela_id: UUID,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
@router.post(
    "/{tabela_id}/regras", response_model=RegraTemporalidadeResponseSchema, status_code=201
)
async def criar_regra_temporalidade(
    tabela_id: UUID,
    regra_data: RegraTemporalidadeCreateSchema,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
async def listar_regras_tabela(
    tabela_id: UUID,
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=1000),
    ativo: bool | None = None,
...
async def obter_regra_temporalidade(
    regra_id: UUID,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
async def atualizar_regra_temporalidade(
    regra_id: UUID,
    regra_data: RegraTemporalidadeUpdateSchema,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
async def excluir_regra_temporalidade(
    regra_id: UUID,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
async def registrar_evento_temporalidade(
    evento_data: EventoTemporalidadeCreateSchema,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
async def aplicar_temporalidade(
    aplicacao_data: AplicarTemporalidadeSchema,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
...
async def obter_status_temporalidade_documento(
    documento_id: UUID,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
    ...
    if not status:
        raise HTTPException(
            status_code=404, detail="Documento não encontrado ou sem temporalidade aplicada"
        )
    return status


@router.get(
    "/regras/aplicaveis/{documento_id}", response_model=list[RegraTemporalidadeResponseSchema]
)
async def obter_regras_aplicaveis_documento(
    documento_id: UUID,
    db: AsyncSession = Depends(get_db),
    service: TabelaTemporalidadeService = Depends(),
):
