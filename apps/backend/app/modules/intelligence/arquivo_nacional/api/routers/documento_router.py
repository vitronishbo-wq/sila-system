...
async def criar_documento(documento_data: DocumentoCreateSchema, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
...
async def obter_documento(documento_id: UUID, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
...
async def listar_documentos(skip: int=Query(0, ge=0), limit: int=Query(100, ge=1, le=1000), numero: Optional[str]=None, titulo: Optional[str]=None, tipo: Optional[str]=None, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
    """Lista documentos com filtros opcionais."""
    documentos = await service.listar_documentos(db, skip=skip, limit=limit, numero=numero, titulo=titulo, tipo=tipo)
...
async def atualizar_documento(documento_id: UUID, documento_data: DocumentoCreateSchema, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
...
async def excluir_documento(documento_id: UUID, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
    """Exclui um documento."""
...
async def digitalizar_documento(documento_id: UUID, digitalizacao_data: DocumentoDigitalizacaoSchema, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
...
async def restringir_documento(documento_id: UUID, restricao_data: DocumentoRestricaoSchema, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
...
async def conservar_documento(documento_id: UUID, conservacao_data: DocumentoConservacaoSchema, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
...
async def alterar_fase_documento(documento_id: UUID, fase_data: DocumentoFaseSchema, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
...
async def eliminar_documento(documento_id: UUID, eliminacao_data: DocumentoEliminacaoSchema, db: AsyncSession=Depends(get_db), service: DocumentoService=Depends()):
