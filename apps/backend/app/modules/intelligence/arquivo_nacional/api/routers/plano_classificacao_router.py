...
async def criar_plano_classificacao(plano_data: PlanoClassificacaoCreateSchema, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def obter_plano_classificacao(plano_id: UUID, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def listar_planos_classificacao(skip: int=Query(0, ge=0), limit: int=Query(100, ge=1, le=1000), codigo: Optional[str]=None, titulo: Optional[str]=None, ativo: Optional[bool]=None, orgao_id: Optional[UUID]=None, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
    """Lista planos de classificação com filtros opcionais."""
    planos = await service.listar_planos_classificacao(db, skip=skip, limit=limit, codigo=codigo, titulo=titulo, ativo=ativo, orgao_id=orgao_id)
...
async def atualizar_plano_classificacao(plano_id: UUID, plano_data: PlanoClassificacaoUpdateSchema, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def excluir_plano_classificacao(plano_id: UUID, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def criar_classe(plano_id: UUID, classe_data: ClasseCreateSchema, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def listar_classes_plano(plano_id: UUID, skip: int=Query(0, ge=0), limit: int=Query(100, ge=1, le=1000), ativo: Optional[bool]=None, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def obter_classe(classe_id: UUID, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def atualizar_classe(classe_id: UUID, classe_data: ClasseUpdateSchema, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def excluir_classe(classe_id: UUID, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
    """Exclui uma classe."""
    sucesso = await service.excluir_classe(classe_id, db)
    if not sucesso:
        raise HTTPException(status_code=404, detail='Classe não encontrada')

@router.post('/classes/{classe_id}/subclasses', response_model=SubclasseResponseSchema, status_code=201)
async def criar_subclasse(classe_id: UUID, subclasse_data: SubclasseCreateSchema, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def listar_subclasses_classe(classe_id: UUID, skip: int=Query(0, ge=0), limit: int=Query(100, ge=1, le=1000), ativo: Optional[bool]=None, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
    """Lista subclasses de uma classe."""
    subclasses = await service.listar_subclasses_classe(classe_id, db, skip=skip, limit=limit, ativo=ativo)
...
async def obter_subclasse(subclasse_id: UUID, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def atualizar_subclasse(subclasse_id: UUID, subclasse_data: SubclasseUpdateSchema, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
...
async def excluir_subclasse(subclasse_id: UUID, db: AsyncSession=Depends(get_db), service: PlanoClassificacaoService=Depends()):
