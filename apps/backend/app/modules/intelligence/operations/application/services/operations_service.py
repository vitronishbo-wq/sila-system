...
from app.domain.catalog.models.service import Service
...
class OperationsService:
    ...
    async def list_services(self) -> list[dict]:
        stmt = select(Service).options(selectinload(Service.module)).where(Service.is_active.is_(True), or_(Service.is_public.is_(True), Service.scope == 'public')).order_by(Service.business_priority.asc(), Service.name.asc())
        ...
        for row in rows:
            ...
            visibility = getattr(row, 'visibility', None) or ('PUBLIC' if row.is_public else 'INTERNAL')
            result.append({'id': row.id, 'code': row.code, 'name': row.name, 'description': description, 'price': float(row.price or 0), 'estimated_days': row.estimated_days, 'workflow_definition_key': workflow_key, 'required_documents': required_documents, 'visibility': visibility, 'version': int(getattr(row, 'version', 1) or 1), 'module_slug': module_slug, 'module_title': module_title, 'is_essential': bool(row.is_essential), 'icon_slug': row.icon_slug, 'active': bool(row.is_active)})
        return result
    ...
    async def create_order(self, citizen_id: UUID, service_id: UUID) -> OperationalOrderModel:
        ...
        order = OperationalOrderModel(citizen_id=citizen_id, service_id=service.id, workflow_instance_id=uuid4(), total_amount=amount, status=OrderStatus.DRAFT.value, status_history=[self._history_entry(None, OrderStatus.DRAFT.value, 'Order created')])
        self.db.add(order)
        await self.db.commit()
        await self.db.refresh(order)
        return await self._get_order(order.id, citizen_id)

    async def add_documents(self, order_id: UUID, citizen_id: UUID, documents: list[dict]) -> OperationalOrderModel:
        ...
        for item in documents:
            doc = OperationalOrderDocumentModel(order_id=order.id, filename=item['filename'], content_type=item['content_type'], size_bytes=item['size_bytes'], uri=item.get('uri'))
            self.db.add(doc)
        await self.db.commit()
    ...
    async def generate_payment(self, order_id: UUID, citizen_id: UUID) -> OperationalPaymentModel:
        ...
        pending_stmt = select(OperationalPaymentModel).where(OperationalPaymentModel.order_id == order.id, OperationalPaymentModel.status == PaymentStatus.PENDING.value)
        existing = (await self.db.execute(pending_stmt)).scalars().first()
        if existing:
            return existing
        reference = f'SIM-{order.id.hex[:8].upper()}-{uuid4().hex[:6].upper()}'
        payment = OperationalPaymentModel(order_id=order.id, reference=reference, amount=order.total_amount, status=PaymentStatus.PENDING.value, provider='FAKE_BANK')
        self.db.add(payment)
    ...
    async def confirm_payment(self, reference: str) -> OperationalPaymentModel:
        stmt = select(OperationalPaymentModel).options(selectinload(OperationalPaymentModel.order)).where(OperationalPaymentModel.reference == reference).with_for_update()
        ...
        if reference.startswith('SIM'):
            ...
            if payment.order.status != OrderStatus.PAID.value:
                self._transition_order(payment.order, OrderStatus.PAID, f'Payment confirmed by provider {payment.provider}')
        else:
    ...
    async def complete_order(self, order_id: UUID, citizen_id: UUID) -> OperationalOrderModel:
        ...
        if not order.receipt_number:
            order.receipt_number = f'RCP-{datetime.now(timezone.utc):%Y%m%d}-{order.id.hex[:8].upper()}'
        order.proof_payload = {'receipt_number': order.receipt_number, 'order_id': str(order.id), 'citizen_id': str(order.citizen_id), 'service_id': str(order.service_id), 'amount': float(order.total_amount), 'status': order.status, 'issued_at': datetime.now(timezone.utc).isoformat()}
        await self.db.commit()
        return await self._get_order(order_id, citizen_id)
    ...
    async def _get_order(self, order_id: UUID, citizen_id: UUID) -> OperationalOrderModel:
        stmt = select(OperationalOrderModel).options(selectinload(OperationalOrderModel.documents), selectinload(OperationalOrderModel.payments)).where(OperationalOrderModel.id == order_id, OperationalOrderModel.citizen_id == citizen_id)
        order = (await self.db.execute(stmt)).scalars().first()
        if not order:
            raise ValueError('Order not found')
        return order

    async def _get_order_for_update(self, order_id: UUID, citizen_id: UUID) -> OperationalOrderModel:
        stmt = select(OperationalOrderModel).options(selectinload(OperationalOrderModel.documents), selectinload(OperationalOrderModel.payments)).where(OperationalOrderModel.id == order_id, OperationalOrderModel.citizen_id == citizen_id).with_for_update()
        order = (await self.db.execute(stmt)).scalars().first()
        if not order:
            raise ValueError('Order not found')
        return order

    def _transition_order(self, order: OperationalOrderModel, target_status: OrderStatus, reason: str) -> None:
    ...
    def _history_entry(from_status: str | None, to_status: str, reason: str) -> dict:
        return {'from': from_status, 'to': to_status, 'reason': reason, 'at': datetime.now(timezone.utc).isoformat()}