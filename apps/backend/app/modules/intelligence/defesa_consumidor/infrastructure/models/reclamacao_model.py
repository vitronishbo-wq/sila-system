...
class ReclamacaoModel(Base):
    ...
    __table_args__ = (Index('ix_dc_reclamacoes_status_data', 'status', 'data_abertura'), Index('ix_dc_reclamacoes_consumidor_status', 'consumidor_id', 'status'))
    ...
    data_abertura: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)
    ...
    def to_dict(self) -> dict:
        return {'id': self.id, 'protocolo': self.protocolo, 'consumidor_id': self.consumidor_id, 'estabelecimento_id': self.estabelecimento_id, 'produto_servico': self.produto_servico, 'descricao': self.descricao, 'categoria': self.categoria, 'status': self.status, 'prioridade': self.prioridade, 'valor_reclamado': self.valor_reclamado, 'data_abertura': self.data_abertura, 'data_resolucao': self.data_resolucao, 'resolvido': self.resolvido, 'descricao_resposta': self.descricao_resposta, 'criado_em': self.criado_em, 'atualizado_em': self.atualizado_em}