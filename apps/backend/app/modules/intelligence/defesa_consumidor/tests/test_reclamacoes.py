...
class _FakeRepo:
    ...
    async def list_by_estabelecimento(self, estabelecimento_id, limit=50, offset=0):
        return [r for r in self._db.values() if r["estabelecimento_id"] == estabelecimento_id][
            :limit
        ]
    ...
    async def list_prioritarias(self, prioridade, limit=50):
        return [
            r for r in self._db.values() if r["prioridade"] == prioridade and (not r["resolvido"])
        ][:limit]
...
async def test_fluxo_reclamacao_basico():
    ...
    created = await service.criar_reclamacao(
        {
            "consumidor_id": 1,
            "estabelecimento_id": 2,
            "produto_servico": "Notebook XY",
            "descricao": "Produto sem funcionar apos 3 dias",
