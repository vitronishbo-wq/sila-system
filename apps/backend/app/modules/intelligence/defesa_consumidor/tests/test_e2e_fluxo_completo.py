...
def test_fluxo_completo_reclamacao():
    ...
    data = {'consumidor_id': 1, 'estabelecimento_id': 1, 'produto_servico': 'Notebook XY', 'descricao': 'Produto nao funcionou apos 3 dias', 'categoria': 'produto_defectuoso', 'valor_reclamado': 1500.0, 'prioridade': 'media'}
    ...
    resp = client.patch(f'/api/v1/defesa_consumidor/reclamacoes/{rid}/status', params={'novo_status': 'em_analise'})
    assert resp.status_code == 200
    resp = client.patch(f'/api/v1/defesa_consumidor/reclamacoes/{rid}/escalar')
    assert resp.status_code == 200
    resp = client.patch(f'/api/v1/defesa_consumidor/reclamacoes/{rid}/finalizar', params={'resolvido': True})
    assert resp.status_code == 200