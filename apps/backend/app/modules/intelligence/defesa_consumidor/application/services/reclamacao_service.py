...
from apps.backend.app.modules.intelligence.defesa_consumidor.application.ports.reclamacao_repository_port import (
    ReclamacaoRepositoryPort,
)
from apps.backend.app.modules.intelligence.defesa_consumidor.domain.exceptions import (
...
class ReclamacaoService:
    ...
    async def criar_reclamacao(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        await self.event_bus.publish(
            "defesa_consumidor.reclamacao.criada",
            {
                "id": reclamacao.get("id"),
                "protocolo": reclamacao.get("protocolo"),
                "consumidor_id": reclamacao.get("consumidor_id"),
                "estabelecimento_id": reclamacao.get("estabelecimento_id"),
                "categoria": reclamacao.get("categoria"),
                "data_abertura": self._iso(reclamacao.get("data_abertura")),
            },
        )
        return reclamacao
    ...
    async def atualizar_status(self, reclamacao_id: int, novo_status: str) -> dict[str, Any]:
        ...
        await self.event_bus.publish(
            "defesa_consumidor.reclamacao.status_atualizado",
            {
                "id": reclamacao_id,
                "protocolo": reclamacao.get("protocolo"),
    ...
    async def finalizar_reclamacao(
        self, reclamacao_id: int, resolvido: bool = True, descricao_resposta: str | None = None
    ) -> dict[str, Any]:
        reclamacao = await self.obter_reclamacao(reclamacao_id)
        if reclamacao.get("resolvido"):
            raise ReclamacaoJaEncerradaException(reclamacao_id)
        ...
        updated = await self.repository.update(reclamacao)
        await self.event_bus.publish(
            "defesa_consumidor.reclamacao.finalizada",
            {
                "id": reclamacao_id,
                "protocolo": reclamacao.get("protocolo"),
                "resolvido": resolvido,
                "dias_resolucao": dias_aberto,
                "data_resolucao": data_resolucao.isoformat(),
            },
        )
        return updated
    ...
    async def escalar_prioridade(self, reclamacao_id: int) -> dict[str, Any]:
        ...
        updated = await self.repository.update(reclamacao)
        await self.event_bus.publish(
            "defesa_consumidor.reclamacao.prioridade_escalada",
            {
                "id": reclamacao_id,
                "protocolo": reclamacao.get("protocolo"),
                "prioridade_anterior": atual,
                "prioridade_nova": nova,
            },
        )
        return updated

    async def listar_por_consumidor(
        self, consumidor_id: int, limit: int = 50
    ) -> list[dict[str, Any]]:
        return await self.repository.list_by_consumidor(consumidor_id, limit)

    async def listar_prioritarias(
        self, prioridade: str = "alta", limit: int = 50
    ) -> list[dict[str, Any]]:
