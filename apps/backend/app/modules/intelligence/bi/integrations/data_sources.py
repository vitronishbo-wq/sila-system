...
class FinancasDataSource:
    ...
    def __init__(self, db: AsyncSession):
        if InvoiceModel is None or PaymentModel is None:
            raise RuntimeError(
                "Modelos financeiros indisponiveis para BI (InvoiceModel/PaymentModel)."
            )
        self.db = db
    ...
    async def collect_metrics(self, data_ref: date) -> dict[str, float | int]:
        ...
        faturas_pendentes = int(
            await _scalar(
                ...
                .where(func.lower(InvoiceModel.status).in_(("pending", "issued", "overdue"))),
            )
        )
        faturas_pagas = int(
            await _scalar(
                ...
                select(func.count())
                .select_from(InvoiceModel)
                .where(func.lower(InvoiceModel.status) == "paid"),
            )
        )
        valor_faturado_total = _as_float(
            await _scalar(self.db, select(func.coalesce(func.sum(InvoiceModel.amount), 0)))
        )
        pagamentos_confirmados_mes = int(
            await _scalar(
                self.db,
                select(func.count())
                .select_from(PaymentModel)
                .where(
                    PaymentModel.created_at >= start_dt,
                    PaymentModel.created_at < next_month_dt,
                    func.lower(PaymentModel.status).in_(("completed", "reconciled")),
                ),
            )
        )
        valor_pago_mes = _as_float(
            await _scalar(
                self.db,
        ...
        return {
            ...
            "valor_pago_mes": round(valor_pago_mes, 2),
        }
...
class FinancasPublicasDataSource:
    ...
    async def collect_metrics(self, data_ref: date) -> dict[str, float | int]:
        ...
        orcamentos_total = int(
            await _scalar(self.db, select(func.count()).select_from(OrcamentoModel))
        )
        orcamentos_ativos = int(
            await _scalar(
                ...
                .where(
                    func.lower(OrcamentoModel.status).in_(
                        ("active", "ativo", "vigente", "em_execucao", "aprovado")
                    )
                ),
            )
        )
        receitas_previstas = _as_float(
            await _scalar(self.db, select(func.coalesce(func.sum(ReceitaModel.valor_previsto), 0)))
        )
        receitas_arrecadadas = _as_float(
            await _scalar(
                self.db, select(func.coalesce(func.sum(ReceitaModel.valor_arrecadado), 0))
            )
        )
        despesas_previstas = _as_float(
            await _scalar(self.db, select(func.coalesce(func.sum(DespesaModel.valor_previsto), 0)))
        )
        despesas_pagas = _as_float(
            await _scalar(self.db, select(func.coalesce(func.sum(DespesaModel.valor_pago), 0)))
        )
        execucao_despesa = (
            despesas_pagas / despesas_previstas * 100 if despesas_previstas > 0 else 0.0
        )
        cobertura_receita = (
            receitas_arrecadadas / receitas_previstas * 100 if receitas_previstas > 0 else 0.0
        )
        return {
            "orcamentos_total": orcamentos_total,
            "orcamentos_ativos": orcamentos_ativos,
            "receitas_previstas_total": round(receitas_previstas, 2),
            "receitas_arrecadadas_total": round(receitas_arrecadadas, 2),
            "despesas_previstas_total": round(despesas_previstas, 2),
            "despesas_pagas_total": round(despesas_pagas, 2),
            "execucao_despesa_percentual": round(execucao_despesa, 2),
            "cobertura_receita_percentual": round(cobertura_receita, 2),
        }
...
class DataSources:
    ...
    DOMAIN_ALIASES = {
        ...
        "financas-publicas": "financas_publicas",
    }
