...
class DashboardService:
    ...
    async def get_dashboard_domain(
        self, domain: str, data_ref: date | None = None
    ) -> dict[str, Any]:
    ...
    async def get_dashboard_executivo(
        self, data_ref: date | None = None, domains: list[str] | None = None
    ) -> dict[str, Any]:
        ref = data_ref or date.today()
        ...
        return {
            ...
            "generated_at": datetime.utcnow().isoformat(),
            "summary": summary,
            "domains": data_by_domain,
            "unavailable_domains": sorted(set(unavailable_domains)),
            "errors": errors,
        }
    ...
    def _build_executive_summary(
        ...
        return {
            "domains_requested": len(data_by_domain) + len(unavailable_domains) + len(errors),
            "domains_with_data": len(data_by_domain),
            "domains_unavailable": len(set(unavailable_domains)),
            "domains_with_error": len(errors),
            "total_indicadores": sum(len(metrics) for metrics in data_by_domain.values()),
            "indicadores_chave": {
                "cidadaos_ativos": _sum_metric_fragment(data_by_domain, "cidadaos_ativos"),
                "atendimentos_mes": _sum_metric_fragment(data_by_domain, "atendimentos_mes"),
                "requests_total_mes": _sum_metric_fragment(data_by_domain, "requests_total_mes"),
                "valor_pago_mes": _sum_metric_fragment(data_by_domain, "valor_pago_mes"),
                "despesas_pagas_total": _sum_metric_fragment(
                    data_by_domain, "despesas_pagas_total"
                ),
            },
        }
