...
REQUIRED_TABLES_BY_DOMAIN = {
    "financas": {"financas_invoices", "financas_payments"},
    "financas_publicas": {
        "financas_publicas_orcamentos",
        "financas_publicas_receitas",
        "financas_publicas_despesas",
    },
}
...
async def test_data_sources_collect_metrics_with_real_orm(db_session) -> None:
    ...
    if not eligible_domains:
        ...
        return
    assert tested_domains, (
        "Dominios com tabela fisica existem, mas nenhum adapter BI conseguiu coletar metricas."
    )
