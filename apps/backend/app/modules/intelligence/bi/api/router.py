...
from apps.backend.app.modules.intelligence.bi.api.deps import (
    get_dashboard_service,
    get_kpi_service,
    require_permission,
)
from apps.backend.app.modules.intelligence.bi.api.schemas.dashboard_schema import (
    DashboardDomainDataResponse,
    DashboardExecutiveResponse,
)
from apps.backend.app.modules.intelligence.bi.api.schemas.kpi_schema import (
    KPIConsolidatedResponse,
    KPIDomainResponse,
)
...
def _ensure_domain_source(is_available: bool, domain: str) -> None:
    if not is_available:
        raise HTTPException(
            ...
            detail=f"Fonte de dados indisponivel para o dominio '{domain}'. Verifique integracao/adapters do modulo no BI.",
        )
...
async def _domain_dashboard(domain: str, service: DashboardService, data_ref: date | None) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Falha ao montar dashboard do dominio '{domain}': {exc}",
        ) from exc
...
async def _domain_kpis(domain: str, service: KPIService, data_ref: date | None) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Falha ao coletar KPIs do dominio '{domain}': {exc}",
        )
...
@router.get(
    "/kpis",
    response_model=KPIConsolidatedResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_consolidated_kpis(
    data_ref: date | None = Query(None),
    domains: str | None = Query(
        None, description="Lista de dominios separados por virgula (ex: educacao,saude,financas)."
    ),
...
@router.get(
    "/kpis/domain/{domain}",
    response_model=KPIDomainResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_kpis_by_domain(
    domain: str, data_ref: date | None = Query(None), service: KPIService = Depends(get_kpi_service)
):
...
async def get_kpis_educacao(
    data_ref: date | None = Query(None), service: KPIService = Depends(get_kpi_service)
):
...
async def get_kpis_assistencia(
    data_ref: date | None = Query(None), service: KPIService = Depends(get_kpi_service)
):
...
@router.get(
    "/kpis/service-requests",
    response_model=KPIDomainResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_kpis_service_requests(
    data_ref: date | None = Query(None), service: KPIService = Depends(get_kpi_service)
):
...
@router.get(
    "/kpis/financas-publicas",
    response_model=KPIDomainResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_kpis_financas_publicas(
    data_ref: date | None = Query(None), service: KPIService = Depends(get_kpi_service)
):
...
async def get_dashboard_executivo(
    data_ref: date | None = Query(None),
    domains: str | None = Query(
        None, description="Lista de dominios separados por virgula para filtrar o painel executivo."
    ),
...
@router.get(
    "/dashboards/domain/{domain}",
    response_model=DashboardDomainDataResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_dashboard_by_domain(
    domain: str,
    data_ref: date | None = Query(None),
    service: DashboardService = Depends(get_dashboard_service),
):
...
async def get_dashboard_educacao(
    data_ref: date | None = Query(None), service: DashboardService = Depends(get_dashboard_service)
):
    return await _domain_dashboard(domain="educacao", service=service, data_ref=data_ref)


@router.get(
    "/dashboards/juventude",
    response_model=DashboardDomainDataResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_dashboard_juventude(
    data_ref: date | None = Query(None), service: DashboardService = Depends(get_dashboard_service)
):
...
async def get_dashboard_emprego(
    data_ref: date | None = Query(None), service: DashboardService = Depends(get_dashboard_service)
):
...
@router.get(
    "/dashboards/assistencia",
    response_model=DashboardDomainDataResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_dashboard_assistencia(
    data_ref: date | None = Query(None), service: DashboardService = Depends(get_dashboard_service)
):
...
@router.get(
    "/dashboards/identidade",
    response_model=DashboardDomainDataResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_dashboard_identidade(
    data_ref: date | None = Query(None), service: DashboardService = Depends(get_dashboard_service)
):
...
@router.get(
    "/dashboards/service-requests",
    response_model=DashboardDomainDataResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_dashboard_service_requests(
    data_ref: date | None = Query(None), service: DashboardService = Depends(get_dashboard_service)
):
...
@router.get(
    "/dashboards/financas-publicas",
    response_model=DashboardDomainDataResponse,
    dependencies=[Depends(require_permission("bi:view"))],
)
async def get_dashboard_financas_publicas(
    data_ref: date | None = Query(None), service: DashboardService = Depends(get_dashboard_service)
):
