...
from apps.backend.app.modules.resources.pecuaria.application.ports import (
...
from apps.backend.app.modules.resources.pecuaria.infrastructure.models.propriedade_pecuaria_model import (
    PropriedadePecuariaModel,
)
...
class SQLAlchemyPropriedadePecuariaRepository(PropriedadePecuariaRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_propriedade: str) -> PropriedadePecuaria | None:
        stmt = select(PropriedadePecuariaModel).where(
            PropriedadePecuariaModel.codigo_propriedade == codigo_propriedade
        )
    ...
    async def list_by_pecuarista(
        ...
        rows = (
            (await self.session.execute(stmt.order_by(PropriedadePecuariaModel.created_at.desc())))
            .scalars()
            .all()
        )
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(PropriedadePecuariaModel)
            .where(PropriedadePecuariaModel.codigo_propriedade.like(f"PROP/{ano}/%"))
        )
    ...
    def _to_domain(model: PropriedadePecuariaModel) -> PropriedadePecuaria:
        return PropriedadePecuaria(
            id=model.id,
            codigo_propriedade=model.codigo_propriedade,
            pecuarista_id=model.pecuarista_id,
            nome=model.nome,
