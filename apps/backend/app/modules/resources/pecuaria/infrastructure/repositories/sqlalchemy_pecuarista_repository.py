...
from apps.backend.app.modules.resources.pecuaria.infrastructure.models.pecuarista_model import (
    PecuaristaModel,
)
...
class SQLAlchemyPecuaristaRepository(PecuaristaRepositoryPort):
    ...
    async def get_by_cadastro(self, cadastro_pecuarista: str) -> Pecuarista | None:
        stmt = select(PecuaristaModel).where(
            PecuaristaModel.cadastro_pecuarista == cadastro_pecuarista
        )
    ...
    async def list_by_status(self, status: StatusPecuarista | None = None) -> list[Pecuarista]:
        ...
        rows = (
            (await self.session.execute(stmt.order_by(PecuaristaModel.created_at.desc())))
            .scalars()
            .all()
        )
    ...
    async def next_cadastro(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(PecuaristaModel)
            .where(PecuaristaModel.cadastro_pecuarista.like(f"PEC/{ano}/%"))
        )
    ...
    def _to_domain(model: PecuaristaModel) -> Pecuarista:
        return Pecuarista(
            id=model.id,
            cadastro_pecuarista=model.cadastro_pecuarista,
            nome=model.nome,
            documento=model.documento,
            documento_tipo=model.documento_tipo,
            data_cadastro=model.data_cadastro,
            status=StatusPecuarista(model.status),
