...
class PecuaristaService:
    def __init__(
        self,
        pecuarista_repo: PecuaristaRepositoryPort,
    ...
    async def cadastrar_pecuarista(
        ...
        *,
        ...
        if existente and existente.status in {
            StatusPecuarista.PENDENTE,
            StatusPecuarista.ATIVO,
            StatusPecuarista.SUSPENSO,
        }:
        ...
        if self.request_service and citizen_id:
            await self.request_service.create_request(
                request_type="cadastro_pecuarista",
                entity_id=saved.id,
                citizen_id=citizen_id,
                numero_processo=saved.cadastro_pecuarista,
                metadata={"documento": saved.documento},
            )
        return saved

    async def ativar_pecuarista(
        self, cadastro_pecuarista: str, actor_id: UUID | None = None
    ) -> Pecuarista:
        ...
        if self.request_service and actor_id:
            await self.request_service.complete_request(
                entity_id=saved.id,
                actor_id=actor_id,
                metadata={"status": saved.status.value, "data": str(date.today())},
            )
        return saved
