...
async def test_rebanho_service_cadastro_sucesso():
    repository = SimpleNamespace(next_codigo=AsyncMock(return_value='REB/2026/000001'), save=AsyncMock(side_effect=lambda item: item), get_by_codigo=AsyncMock(), list_by_propriedade=AsyncMock())
    service = RebanhoService(repository=repository)
    item = await service.cadastrar(propriedade_id=uuid4(), tipo_animal=TipoAnimal.BOVINO, descricao='Matrizes leiteiras', quantidade_animais=10)
    assert item.codigo_rebanho.startswith('REB/')

def test_endpoint_cadastrar_rebanho_retorna_201():
    mock_item = Rebanho.criar(propriedade_id=uuid4(), tipo_animal=TipoAnimal.BOVINO, descricao='Matrizes leiteiras', quantidade_animais=10)
    ...
    response = client.post('/pecuaria/rebanhos/', json={'propriedade_id': str(mock_item.propriedade_id), 'tipo_animal': 'bovino', 'descricao': 'Matrizes leiteiras', 'quantidade_animais': 10})
