...
from apps.backend.app.modules.resources.pecuaria.application.services.producao_service import (
    ProducaoService,
)
...
from apps.backend.app.modules.resources.pecuaria.application.services.sanidade_service import (
    SanidadeService,
)
from apps.backend.app.modules.resources.pecuaria.infrastructure.adapters import (
    CitizenServiceAdapter,
    RequestServiceAdapter,
)
from apps.backend.app.modules.resources.pecuaria.infrastructure.repositories import (
    SQLAlchemyAnimalRepository,
    SQLAlchemyPecuaristaRepository,
    SQLAlchemyPropriedadePecuariaRepository,
    SQLAlchemyRebanhoRepository,
)
...
async def get_pecuarista_service(session: AsyncSession = db_dep) -> PecuaristaService:
    ...
    return PecuaristaService(
        pecuarista_repo=repository, citizen_service=citizen_service, request_service=request_service
    )


async def get_propriedade_service(session: AsyncSession = db_dep) -> PropriedadeService:
    repository = SQLAlchemyPropriedadePecuariaRepository(session)
    return PropriedadeService(repository=repository)


async def get_rebanho_service(session: AsyncSession = db_dep) -> RebanhoService:
    repository = SQLAlchemyRebanhoRepository(session)
    return RebanhoService(repository=repository)


async def get_animal_service(session: AsyncSession = db_dep) -> AnimalService:
    ...
    return AnimalService(
        animal_repo=animal_repo, propriedade_repo=propriedade_repo, rebanho_repo=rebanho_repo
    )
