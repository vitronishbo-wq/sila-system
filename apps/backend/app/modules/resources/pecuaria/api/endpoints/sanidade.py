...
sanidade_service_dep = Depends(get_sanidade_service)


@router.post("/vacinas", response_model=VacinaResponse, status_code=status.HTTP_201_CREATED)
async def aplicar_vacina(
    data: VacinaCreate, service: SanidadeService = sanidade_service_dep
):
    return await service.aplicar_vacina(
...
async def listar_vacinas(
    animal_id: UUID | None = None, service: SanidadeService = sanidade_service_dep
):
