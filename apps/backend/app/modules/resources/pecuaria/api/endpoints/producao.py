...
async def registrar_producao_leite(
    data: ProducaoLeiteCreate, service: ProducaoService = producao_service_dep
):
    try:
        return await service.registrar_leite(
            propriedade_id=data.propriedade_id, litros=data.litros, data_producao=data.data_producao
        )
...
async def registrar_producao_carne(
    data: ProducaoCarneCreate, service: ProducaoService = producao_service_dep
):
    try:
        return await service.registrar_carne(
            propriedade_id=data.propriedade_id, quilos=data.quilos, data_producao=data.data_producao
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
...
async def listar_producoes(
    propriedade_id: UUID | None = None, service: ProducaoService = producao_service_dep
):
