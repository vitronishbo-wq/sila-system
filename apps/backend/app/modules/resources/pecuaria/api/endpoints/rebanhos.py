...
rebanho_service_dep = Depends(get_rebanho_service)


@router.post("/", response_model=RebanhoResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_rebanho(
    data: RebanhoCreate, service: RebanhoService = rebanho_service_dep
):
    try:
...
async def obter_rebanho(
    codigo_rebanho: str, service: RebanhoService = rebanho_service_dep
):
    try:
...
async def listar_rebanhos(
    propriedade_id: UUID | None = None, service: RebanhoService = rebanho_service_dep
):
