...
from app.modules.resources.aguas_saneamento.application.events.registry import serialize_event
...
from app.modules.resources.aguas_saneamento.infrastructure.models.outbox_event_model import OutboxEventModel
...
class SQLAlchemyOutboxRepository(OutboxRepositoryPort):
    ...
    async def enqueue(self, event: Any) -> OutboxMessage:
        ...
        model = OutboxEventModel(id=uuid4(), event_name=event_name, event_version=int(payload.get('version', 1)), schema_version=1, topic=topic, payload=payload, headers={}, processed=False, retries=0, created_at=datetime.now(timezone.utc))
    ...
    async def mark_done(self, message_id: UUID) -> None:
        async with self._session_factory() as session:
            result = await session.execute(select(OutboxEventModel).where(OutboxEventModel.id == message_id))
