...
class MessagePublisherPort(ABC):
    ...
    @abstractmethod
    async def publish(self, *, topic: str, payload: dict[str, Any], headers: dict[str, Any] | None=None, key: str | None=None) -> str:
        pass
...
class RabbitMQPublisher(MessagePublisherPort):
    ...
    async def _connect(self) -> None:
        ...
        try:
            ...
            raise RuntimeError('Dependencia ausente: instale aio-pika para RabbitMQ publisher') from exc
        self._connection = await aio_pika.connect_robust(self._url)
        self._channel = await self._connection.channel()
        self._exchange = await self._channel.declare_exchange(self._exchange_name, type=aio_pika.ExchangeType.TOPIC, durable=True)
    ...
    async def publish(self, *, topic: str, payload: dict[str, Any], headers: dict[str, Any] | None=None, key: str | None=None) -> str:
        ...
        message = aio_pika.Message(body=body, content_type='application/json', headers=headers or {}, message_id=message_id, delivery_mode=aio_pika.DeliveryMode.PERSISTENT)
...
class KafkaPublisher(MessagePublisherPort):
    ...
    async def _connect(self) -> None:
        ...
        try:
            ...
            raise RuntimeError('Dependencia ausente: instale aiokafka para Kafka publisher') from exc
    ...
    async def publish(self, *, topic: str, payload: dict[str, Any], headers: dict[str, Any] | None=None, key: str | None=None) -> str:
        ...
        metadata = await self._producer.send_and_wait(topic, value=body, key=key.encode('utf-8') if key else None, headers=kafka_headers)
...
class BrokerPublisherFactory:
    ...
    def from_env() -> MessagePublisherPort:
        ...
        if driver == 'rabbitmq':
            return RabbitMQPublisher(url=os.getenv('AGUAS_RABBITMQ_URL', 'amqp://guest:guest@127.0.0.1:5672/'), exchange=os.getenv('AGUAS_RABBITMQ_EXCHANGE', 'sila.events'))
        if driver == 'kafka':
            return KafkaPublisher(bootstrap_servers=os.getenv('AGUAS_KAFKA_BOOTSTRAP_SERVERS', '127.0.0.1:9092'))
        if driver == 'nats':
