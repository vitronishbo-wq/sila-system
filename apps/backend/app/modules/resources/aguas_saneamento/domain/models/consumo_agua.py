...
class ConsumoAgua:
    ...
    @classmethod
    def registrar(cls, *, abastecimento_id: UUID, titular_id: UUID, referencia: str, categoria: CategoriaConsumo, volume_m3: Decimal, unidade_volume: str, hidrometro_id: UUID | None=None, leitura_anterior: Decimal | None=None, leitura_atual: Decimal | None=None, data_leitura: date | None=None) -> 'ConsumoAgua':
        ...
        if leitura_anterior is not None and leitura_atual is not None and (leitura_atual < leitura_anterior):
            raise ValueError('Leitura atual nao pode ser menor que leitura anterior')
        return cls(id=uuid4(), codigo_consumo='', abastecimento_id=abastecimento_id, titular_id=titular_id, referencia=referencia, categoria=categoria, volume_m3=volume_m3.quantize(Decimal('0.01')), unidade_volume=unidade_volume.strip(), data_leitura=data_leitura or date.today(), status=StatusConsumo.REGISTRADO, data_registro=date.today(), hidrometro_id=hidrometro_id, leitura_anterior=leitura_anterior.quantize(Decimal('0.01')) if leitura_anterior is not None else None, leitura_atual=leitura_atual.quantize(Decimal('0.01')) if leitura_atual is not None else None)
