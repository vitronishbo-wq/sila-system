...
class Outorga:
    ...
    def requerer(
        ...
        *,
        ...
        return cls(
            id=uuid4(),
            numero_outorga=numero_outorga,
            tipo=tipo,
            status=StatusOutorga.REQUERIDA,
            requerente_id=requerente_id,
            requerente_tipo=requerente_tipo.strip(),
            corpo_hidrico_id=corpo_hidrico_id,
            tipo_captacao=tipo_captacao,
            vazao=vazao.quantize(Decimal("0.01")),
            unidade_vazao=unidade_vazao.strip(),
            tempo_captacao=tempo_captacao,
            periodo_captacao=periodo_captacao.strip() if periodo_captacao else None,
    ...
    def atualizar_status_vencimento(self) -> None:
        if (
            self.status == StatusOutorga.DEFERIDA
            and self.data_validade_fim
            and (date.today() > self.data_validade_fim)
        ):
