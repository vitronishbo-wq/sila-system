...
from apps.backend.app.modules.resources.aguas_saneamento.api.endpoints.consumo import (
    router as consumo_router,
)
...
async def test_consumo_service_fluxo_sucesso():
    ...
    item = await service.registrar(
        abastecimento_id=uuid4(),
        titular_id=uuid4(),
        referencia="2026-03",
        categoria=CategoriaConsumo.RESIDENCIAL,
        volume_m3=Decimal("18.75"),
        unidade_volume="m3",
        leitura_anterior=Decimal("100.00"),
        leitura_atual=Decimal("118.75"),
    )
...
def test_endpoint_registrar_consumo_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_consumo="CON/2026/000001",
        abastecimento_id=uuid4(),
        titular_id=uuid4(),
        referencia="2026-03",
        categoria=CategoriaConsumo.RESIDENCIAL,
    ...
    response = client.post(
        "/aguas-saneamento/consumo/",
        json={
            "abastecimento_id": str(uuid4()),
            "titular_id": str(uuid4()),
            "referencia": "2026-03",
            "categoria": "residencial",
            "volume_m3": "18.75",
            "unidade_volume": "m3",
            "leitura_anterior": "100.00",
            "leitura_atual": "118.75",
        },
    )
...
def test_endpoint_obter_consumo_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(side_effect=ConsumoNotFoundError("Consumo nao encontrado"))
    )
    app = FastAPI()
    app.include_router(consumo_router, prefix="/aguas-saneamento")
    app.dependency_overrides[get_consumo_service] = lambda: service
