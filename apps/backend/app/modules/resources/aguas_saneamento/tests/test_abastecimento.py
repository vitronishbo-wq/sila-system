...
async def test_abastecimento_service_fluxo_sucesso():
    ...
    item = await service.registrar(infraestrutura_id=uuid4(), nome_sistema='Sistema Central Luanda', provincia='Luanda', municipio='Talatona')
...
def test_endpoint_registrar_abastecimento_retorna_201():
    mock_item = SimpleNamespace(id=uuid4(), codigo_abastecimento='ABS/2026/000001', infraestrutura_id=uuid4(), nome_sistema='Sistema Central Luanda', provincia='Luanda', municipio='Talatona', status=StatusAbastecimento.PLANEJADO, data_registro=date(2026, 3, 1), data_inicio_operacao=None, data_interrupcao=None, motivo_interrupcao=None, observacoes=None)
    ...
    response = client.post('/aguas-saneamento/abastecimento/', json={'infraestrutura_id': str(uuid4()), 'nome_sistema': 'Sistema Central Luanda', 'provincia': 'Luanda', 'municipio': 'Talatona'})
    assert response.status_code == 201
    assert response.json()['codigo_abastecimento'] == 'ABS/2026/000001'

def test_endpoint_obter_abastecimento_retorna_404():
    service = SimpleNamespace(obter_por_codigo=AsyncMock(side_effect=AbastecimentoNotFoundError('Abastecimento nao encontrado')))
    app = FastAPI()
    app.include_router(abastecimento_router, prefix='/aguas-saneamento')
    app.dependency_overrides[get_abastecimento_service] = lambda: service
    client = TestClient(app)
