...
class _FakePublisher:
    ...
    async def publish(self, *, topic, payload, headers=None, key=None) -> str:
        self.calls.append(
            {"topic": topic, "payload": payload, "headers": headers or {}, "key": key}
        )
        return "MSG-123"
...
async def test_financas_gateway_publica_evento_fatura_emitida():
    ...
    result = await gateway.registrar_fatura_agua(
        fatura_id=uuid4(),
        ...
        data_vencimento=date(2026, 3, 14),
    )
...
async def test_financas_gateway_publica_evento_pagamento():
    fake = _FakePublisher()
    gateway = FinancasGateway(publisher=fake)
    result = await gateway.registrar_pagamento_fatura_agua(
        fatura_id=uuid4(),
        numero_fatura="FAT/2026/000001",
        titular_id=uuid4(),
