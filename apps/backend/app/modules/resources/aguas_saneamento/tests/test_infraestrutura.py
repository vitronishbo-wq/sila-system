...
async def test_infraestrutura_service_fluxo_sucesso():
    ...
    infraestrutura = await service.registrar(tipo=TipoInfraestrutura.ETA, nome='ETA Luanda Sul', provincia='Luanda', municipio='Belas', capacidade=Decimal('2500.00'), unidade_capacidade='m3/dia')
    ...
    infraestrutura = await service.manutencao(infraestrutura.codigo_infraestrutura, motivo='Manutencao preventiva')
    assert infraestrutura.status == StatusInfraestrutura.MANUTENCAO
    infraestrutura = await service.reativar(infraestrutura.codigo_infraestrutura, motivo='Servico concluido')
    assert infraestrutura.status == StatusInfraestrutura.OPERACIONAL
    infraestrutura = await service.desativar(infraestrutura.codigo_infraestrutura, motivo='Encerramento operacional')
    assert infraestrutura.status == StatusInfraestrutura.INATIVA

def test_endpoint_registrar_infraestrutura_retorna_201():
    mock_item = SimpleNamespace(id=uuid4(), codigo_infraestrutura='INF/2026/000001', tipo=TipoInfraestrutura.ETA, nome='ETA Luanda Sul', provincia='Luanda', municipio='Belas', status=StatusInfraestrutura.PLANEJADA, data_registro=date(2026, 3, 1), capacidade=Decimal('2500.00'), unidade_capacidade='m3/dia', outorga_id=None, latitude=None, longitude=None, data_operacao=None, observacoes=None)
    ...
    response = client.post('/aguas-saneamento/infraestrutura/', json={'tipo': 'eta', 'nome': 'ETA Luanda Sul', 'provincia': 'Luanda', 'municipio': 'Belas', 'capacidade': '2500.00', 'unidade_capacidade': 'm3/dia'})
    assert response.status_code == 201
    assert response.json()['codigo_infraestrutura'] == 'INF/2026/000001'

def test_endpoint_obter_infraestrutura_retorna_404():
    service = SimpleNamespace(obter_por_codigo=AsyncMock(side_effect=InfraestruturaNotFoundError('Infraestrutura nao encontrada')))
    app = FastAPI()
    app.include_router(infraestrutura_router, prefix='/aguas-saneamento')
    app.dependency_overrides[get_infraestrutura_service] = lambda: service
    client = TestClient(app)
