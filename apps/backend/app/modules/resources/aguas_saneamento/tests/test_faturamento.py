...
async def test_faturamento_service_fluxo_sucesso():
    ...
    item = await service.emitir(consumo_id=uuid4(), titular_id=uuid4(), referencia='2026-03', volume_m3=Decimal('18.75'), tarifa_m3=Decimal('45.50'), data_vencimento=date.today() + timedelta(days=10))
    assert item.status == StatusFatura.EMITIDA
    assert item.valor_total == Decimal('853.12')
    item = await service.registrar_pagamento(item.numero_fatura, data_pagamento=date.today(), valor_pago=item.valor_total, metodo_pagamento=MetodoPagamento.MULTICAIXA)
...
def test_endpoint_emitir_fatura_retorna_201():
    mock_item = SimpleNamespace(id=uuid4(), numero_fatura='FAT/2026/000001', consumo_id=uuid4(), titular_id=uuid4(), referencia='2026-03', volume_m3=Decimal('18.75'), tarifa_m3=Decimal('45.50'), valor_total=Decimal('853.12'), status=StatusFatura.EMITIDA, data_emissao=date(2026, 3, 1), data_vencimento=date(2026, 3, 10), data_pagamento=None, valor_pago=None, metodo_pagamento=None, observacoes=None)
    ...
    response = client.post('/aguas-saneamento/faturas/', json={'consumo_id': str(uuid4()), 'titular_id': str(uuid4()), 'referencia': '2026-03', 'volume_m3': '18.75', 'tarifa_m3': '45.50', 'data_vencimento': '2026-03-10'})
    assert response.status_code == 201
    assert response.json()['numero_fatura'] == 'FAT/2026/000001'

def test_endpoint_obter_fatura_retorna_404():
    service = SimpleNamespace(obter_por_numero=AsyncMock(side_effect=FaturaNotFoundError('Fatura nao encontrada')))
    app = FastAPI()
    app.include_router(faturas_router, prefix='/aguas-saneamento')
    app.dependency_overrides[get_faturamento_service] = lambda: service
    client = TestClient(app)
