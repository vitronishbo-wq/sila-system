...
async def test_outbox_sqlalchemy_repository_roundtrip(db_session_factory):
    async with db_session_factory() as session:
        ...
        assert exists.scalar_one_or_none() is not None, (
            "Tabela fisica ausente: aguas_saneamento_outbox_events. Execute migration 20260304_040_aguas_saneamento_outbox_events."
        )
    repo = SQLAlchemyOutboxRepository(session_factory=db_session_factory)
    fatura = FaturaAgua.emitir(
        consumo_id=uuid4(),
        titular_id=uuid4(),
        referencia="2026-03",
        volume_m3=Decimal("12.50"),
        tarifa_m3=Decimal("23.40"),
