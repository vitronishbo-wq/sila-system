...
class AbastecimentoService:
    ...
    async def registrar(
        self, *, infraestrutura_id: UUID, nome_sistema: str, provincia: str, municipio: str
    ) -> AbastecimentoAgua:
        existentes = await self._abastecimento_repo.list(
            infraestrutura_id=infraestrutura_id,
            provincia=provincia.strip(),
            municipio=municipio.strip(),
        )
        ativos = {
            StatusAbastecimento.PLANEJADO,
            StatusAbastecimento.OPERACIONAL,
            StatusAbastecimento.INTERROMPIDO,
        }
        if any(
            item.nome_sistema.lower() == nome_sistema.strip().lower() and item.status in ativos
            for item in existentes
        ):
            raise AbastecimentoAlreadyExistsError("Ja existe sistema de abastecimento equivalente")
        item = AbastecimentoAgua.registrar(
            infraestrutura_id=infraestrutura_id,
    ...
    async def iniciar_operacao(
        self, codigo_abastecimento: str, *, data_inicio_operacao: date | None = None
    ) -> AbastecimentoAgua:
    ...
    async def listar(
        ...
        *,
        ...
        return await self._abastecimento_repo.list(
            infraestrutura_id=infraestrutura_id,
            status=status,
            provincia=provincia,
            municipio=municipio,
