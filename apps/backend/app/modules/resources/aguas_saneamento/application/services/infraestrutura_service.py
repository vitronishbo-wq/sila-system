...
class InfraestruturaService:
    ...
    async def registrar(
        ...
        *,
        ...
        existentes = await self._infraestrutura_repo.list(
            tipo=tipo, provincia=provincia.strip(), municipio=municipio.strip()
        )
        ativos = {
            StatusInfraestrutura.PLANEJADA,
            StatusInfraestrutura.OPERACIONAL,
            StatusInfraestrutura.MANUTENCAO,
            StatusInfraestrutura.INTERDITADA,
        }
        if any(
            item.nome.lower() == nome.strip().lower() and item.status in ativos
            for item in existentes
        ):
            raise InfraestruturaAlreadyExistsError(
                "Ja existe infraestrutura equivalente registrada"
            )
        item = InfraestruturaHidrica.registrar(
            tipo=tipo,
            nome=nome,
            provincia=provincia,
            municipio=municipio,
    ...
    async def ativar(
        self, codigo_infraestrutura: str, *, data_operacao: date | None = None
    ) -> InfraestruturaHidrica:
    ...
    async def reativar(
        self, codigo_infraestrutura: str, *, motivo: str | None = None
    ) -> InfraestruturaHidrica:
        item = await self._obter_ou_erro(codigo_infraestrutura)
    ...
    async def listar(
        ...
        *,
        ...
        return await self._infraestrutura_repo.list(
            tipo=tipo, status=status, provincia=provincia, municipio=municipio
        )
