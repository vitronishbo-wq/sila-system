...
class OutorgaService:
    ...
    async def requerer(
        ...
        *,
        ...
        ativos = {
            StatusOutorga.REQUERIDA,
            StatusOutorga.EM_ANALISE,
            StatusOutorga.DEFERIDA,
            StatusOutorga.SUSPENSA,
        }
        if any(
            item.corpo_hidrico_id == corpo_hidrico_id and item.status in ativos
            for item in existentes
        ):
            raise OutorgaAlreadyExistsError(
                "Ja existe outorga ativa para o mesmo requerente e corpo hidrico"
            )
        item = Outorga.requerer(
            tipo=tipo,
            requerente_id=requerente_id,
            requerente_tipo=requerente_tipo,
            corpo_hidrico_id=corpo_hidrico_id,
            tipo_captacao=tipo_captacao,
            vazao=vazao,
            unidade_vazao=unidade_vazao,
            tempo_captacao=tempo_captacao,
