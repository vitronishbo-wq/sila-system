...
from apps.backend.app.modules.resources.aguas_saneamento.infrastructure.persistence.repository import (
...
class FaturamentoService:
    def __init__(
        self, *, fatura_repo: FaturaRepositoryPort, outbox_repo: OutboxRepositoryPort | None = None
    ) -> None:
    ...
    async def emitir(
        ...
        *,
        ...
        saved = await self._outbox.commit_with_event(
            persist=self._fatura_repo.save(item), event=FaturaEmitidaEvent.from_fatura(item)
        )
        return saved
    ...
    async def registrar_pagamento(
        ...
        *,
        ...
        saved = await self._outbox.commit_with_event(
            persist=self._fatura_repo.save(item),
            event=FaturaPagamentoRegistradoEvent.from_fatura(item),
        )
        return saved
    ...
    async def listar(
        ...
        *,
        ...
        items = await self._fatura_repo.list(
            consumo_id=consumo_id, titular_id=titular_id, referencia=referencia, status=status
        )
        for item in items:
            item.atualizar_status_vencimento()
        return items
