...
class ConsumoService:
    ...
    async def registrar(
        ...
        *,
        ...
        existentes = await self._consumo_repo.list(
            abastecimento_id=abastecimento_id, titular_id=titular_id, referencia=referencia
        )
        ...
        if any(item.status in ativos for item in existentes):
            raise ConsumoAlreadyExistsError(
                "Ja existe consumo registrado para titular e referencia"
            )
        item = ConsumoAgua.registrar(
