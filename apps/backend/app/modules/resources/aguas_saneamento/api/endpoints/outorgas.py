...
from apps.backend.app.modules.resources.aguas_saneamento.api.schemas.outorga_schema import (
    OutorgaCreate,
    OutorgaDeferimentoInput,
    OutorgaMotivoInput,
    OutorgaRenovacaoInput,
    OutorgaResponse,
)
...
from apps.backend.app.modules.resources.aguas_saneamento.exceptions import (
    OutorgaAlreadyExistsError,
    OutorgaNotFoundError,
)

router = APIRouter(prefix="/outorgas", tags=["Aguas Saneamento - Outorgas"])
outorga_service_dep = Depends(get_outorga_service)


@router.post("/", response_model=OutorgaResponse, status_code=status.HTTP_201_CREATED)
async def requerer_outorga(
    data: OutorgaCreate, service: OutorgaService = outorga_service_dep
):
    try:
        return await service.requerer(
            tipo=data.tipo,
            requerente_id=data.requerente_id,
            requerente_tipo=data.requerente_tipo,
            corpo_hidrico_id=data.corpo_hidrico_id,
            tipo_captacao=data.tipo_captacao,
            vazao=data.vazao,
            unidade_vazao=data.unidade_vazao,
            tempo_captacao=data.tempo_captacao,
            periodo_captacao=data.periodo_captacao,
...
async def iniciar_analise_outorga(
    numero_outorga: str, service: OutorgaService = outorga_service_dep
):
    try:
...
async def deferir_outorga(
    numero_outorga: str,
    data: OutorgaDeferimentoInput,
    service: OutorgaService = outorga_service_dep,
):
    try:
        return await service.deferir(
            numero_outorga,
            data_validade_inicio=data.data_validade_inicio,
            data_validade_fim=data.data_validade_fim,
...
async def indeferir_outorga(
    numero_outorga: str,
    data: OutorgaMotivoInput,
    service: OutorgaService = outorga_service_dep,
):
    try:
...
async def renovar_outorga(
    numero_outorga: str,
    data: OutorgaRenovacaoInput,
    service: OutorgaService = outorga_service_dep,
):
    try:
...
async def listar_outorgas(
    ...
    status_outorga: StatusOutorga | None = None,
    service: OutorgaService = outorga_service_dep,
):
