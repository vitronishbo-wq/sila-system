...
async def registrar_consumo(data: ConsumoCreate, service: ConsumoService=Depends(get_consumo_service)):
    try:
        return await service.registrar(abastecimento_id=data.abastecimento_id, titular_id=data.titular_id, referencia=data.referencia, categoria=data.categoria, volume_m3=data.volume_m3, unidade_volume=data.unidade_volume, hidrometro_id=data.hidrometro_id, leitura_anterior=data.leitura_anterior, leitura_atual=data.leitura_atual, data_leitura=data.data_leitura)
...
async def validar_consumo(codigo_consumo: str, service: ConsumoService=Depends(get_consumo_service)):
    try:
...
async def faturar_consumo(codigo_consumo: str, service: ConsumoService=Depends(get_consumo_service)):
    try:
...
async def cancelar_consumo(codigo_consumo: str, data: ConsumoMotivoInput, service: ConsumoService=Depends(get_consumo_service)):
    try:
...
async def obter_consumo(codigo_consumo: str, service: ConsumoService=Depends(get_consumo_service)):
    try:
...
async def listar_consumos(abastecimento_id: UUID | None=None, titular_id: UUID | None=None, referencia: str | None=None, status_consumo: StatusConsumo | None=None, service: ConsumoService=Depends(get_consumo_service)):
    return await service.listar(abastecimento_id=abastecimento_id, titular_id=titular_id, referencia=referencia, status=status_consumo)