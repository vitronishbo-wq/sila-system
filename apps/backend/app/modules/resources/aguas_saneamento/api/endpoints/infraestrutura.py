...
from apps.backend.app.modules.resources.aguas_saneamento.api.schemas.infraestrutura_schema import (
    InfraestruturaAtivacaoInput,
    InfraestruturaCreate,
    InfraestruturaMotivoInput,
...
from apps.backend.app.modules.resources.aguas_saneamento.exceptions import (
    InfraestruturaAlreadyExistsError,
    InfraestruturaNotFoundError,
)

router = APIRouter(prefix="/infraestrutura", tags=["Aguas Saneamento - Infraestrutura"])
infraestrutura_service_dep = Depends(get_infraestrutura_service)


@router.post("/", response_model=InfraestruturaResponse, status_code=status.HTTP_201_CREATED)
async def registrar_infraestrutura(
    data: InfraestruturaCreate, service: InfraestruturaService = infraestrutura_service_dep
):
    try:
...
async def ativar_infraestrutura(
    codigo_infraestrutura: str,
    data: InfraestruturaAtivacaoInput,
    service: InfraestruturaService = infraestrutura_service_dep,
):
    try:
...
async def manutencao_infraestrutura(
    codigo_infraestrutura: str,
    data: InfraestruturaMotivoInput,
    service: InfraestruturaService = infraestrutura_service_dep,
):
    try:
...
async def interditar_infraestrutura(
    codigo_infraestrutura: str,
    data: InfraestruturaMotivoInput,
    service: InfraestruturaService = infraestrutura_service_dep,
):
...
async def reativar_infraestrutura(
    codigo_infraestrutura: str,
    data: InfraestruturaMotivoInput | None = None,
    service: InfraestruturaService = infraestrutura_service_dep,
):
    try:
...
async def desativar_infraestrutura(
    codigo_infraestrutura: str,
    data: InfraestruturaMotivoInput,
    service: InfraestruturaService = infraestrutura_service_dep,
):
...
async def obter_infraestrutura(
    codigo_infraestrutura: str, service: InfraestruturaService = infraestrutura_service_dep
):
    try:
...
async def listar_infraestruturas(
    ...
    status_infraestrutura: StatusInfraestrutura | None = None,
    provincia: str | None = None,
    municipio: str | None = None,
    service: InfraestruturaService = infraestrutura_service_dep,
):
    return await service.listar(
        tipo=tipo, status=status_infraestrutura, provincia=provincia, municipio=municipio
    )
