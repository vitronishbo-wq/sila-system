...
from apps.backend.app.modules.resources.aguas_saneamento.api.schemas.abastecimento_schema import (
    AbastecimentoCreate,
    AbastecimentoMotivoInput,
    AbastecimentoOperacaoInput,
...
abastecimento_service_dep = Depends(get_abastecimento_service)


@router.post("/", response_model=AbastecimentoResponse, status_code=status.HTTP_201_CREATED)
async def registrar_abastecimento(
    data: AbastecimentoCreate, service: AbastecimentoService = abastecimento_service_dep
):
    try:
        return await service.registrar(
            infraestrutura_id=data.infraestrutura_id,
...
async def iniciar_operacao_abastecimento(
    codigo_abastecimento: str,
    data: AbastecimentoOperacaoInput,
    service: AbastecimentoService = abastecimento_service_dep,
):
    try:
        return await service.iniciar_operacao(
            codigo_abastecimento, data_inicio_operacao=data.data_inicio_operacao
        )
...
async def interromper_abastecimento(
    codigo_abastecimento: str,
    data: AbastecimentoMotivoInput,
    service: AbastecimentoService = abastecimento_service_dep,
):
    try:
...
async def retomar_abastecimento(
    codigo_abastecimento: str, service: AbastecimentoService = abastecimento_service_dep
):
    try:
...
async def encerrar_abastecimento(
    codigo_abastecimento: str,
    data: AbastecimentoMotivoInput,
    service: AbastecimentoService = abastecimento_service_dep,
):
...
async def listar_abastecimentos(
    ...
    status_abastecimento: StatusAbastecimento | None = None,
