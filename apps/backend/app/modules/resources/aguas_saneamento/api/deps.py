...
infraestrutura_service_singleton = InfraestruturaService(infraestrutura_repo=infraestrutura_repo_singleton)
abastecimento_repo_singleton = SQLAlchemyAbastecimentoRepository()
abastecimento_service_singleton = AbastecimentoService(abastecimento_repo=abastecimento_repo_singleton)
...
financas_integration_handler_singleton = FinancasIntegrationHandler(gateway=financas_gateway_singleton)
event_bus.subscribe(FaturaEmitidaEvent.event_name, financas_integration_handler_singleton.on_fatura_emitida)
event_bus.subscribe(FaturaPagamentoRegistradoEvent.event_name, financas_integration_handler_singleton.on_fatura_pagamento_registrado)
outbox_worker_singleton = OutboxWorker(outbox_repo=outbox_repo_singleton, event_bus=event_bus)
faturamento_service_singleton = FaturamentoService(fatura_repo=fatura_repo_singleton, outbox_repo=outbox_repo_singleton)
