...
from apps.backend.app.modules.resources.florestas.infrastructure.adapters._integration_runtime import (
    invoke_async_method,
)
...
class ComercioExternoServiceAdapter(ComercioExternoServicePort):
    ...
    async def integration_payload(self) -> dict[str, Any]:
        ...
        return {
            "provider": type(self.dependency).__name__ if self.dependency is not None else "none",
            "capability": "controle_exportacao_madeira",
            "exportadores_habilitados": len(exportadores)
            if ok and isinstance(exportadores, list)
            else None,
        }
