...
class SQLAlchemyPlanoManejoFlorestalRepository(PlanoManejoFlorestalRepositoryPort):
    ...
    async def get_by_numero(self, numero_pmfs: str) -> PlanoManejoFlorestal | None:
        stmt = select(PlanoManejoFlorestalModel).where(PlanoManejoFlorestalModel.numero_pmfs == numero_pmfs)
        model = (await self.session.execute(stmt)).scalars().first()
        return self._to_domain(model) if model else None

    async def list_by_unidade(self, unidade_manejo_id: UUID) -> list[PlanoManejoFlorestal]:
        stmt = select(PlanoManejoFlorestalModel).where(PlanoManejoFlorestalModel.unidade_manejo_id == unidade_manejo_id)
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(row) for row in rows]

    @staticmethod
    def _to_domain(model: PlanoManejoFlorestalModel) -> PlanoManejoFlorestal:
        return PlanoManejoFlorestal(id=model.id, numero_pmfs=model.numero_pmfs, unidade_manejo_id=model.unidade_manejo_id, responsavel_tecnico_id=model.responsavel_tecnico_id, responsavel_tecnico_registro=model.responsavel_tecnico_registro, status=StatusPlanoManejo(model.status), data_submissao=model.data_submissao, data_aprovacao=model.data_aprovacao, data_validade=model.data_validade, analista_responsavel_id=model.analista_responsavel_id, volume_anual_estimado_m3=model.volume_anual_estimado_m3, ciclo_corte_anos=model.ciclo_corte_anos, area_anual_ha=model.area_anual_ha, parecer_tecnico=model.parecer_tecnico, observacoes=model.observacoes)
