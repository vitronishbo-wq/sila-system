...
from apps.backend.app.modules.resources.florestas.infrastructure.models.inventario_florestal_model import (
    InventarioFlorestalModel,
)
...
class SQLAlchemyInventarioFlorestalRepository(InventarioFlorestalRepositoryPort):
    ...
    async def list_by_unidade(self, unidade_manejo_id: UUID) -> list[InventarioFlorestal]:
        stmt = select(InventarioFlorestalModel).where(
            InventarioFlorestalModel.unidade_manejo_id == unidade_manejo_id
        )
    ...
    def _to_domain(model: InventarioFlorestalModel) -> InventarioFlorestal:
        return InventarioFlorestal(
            ...
            data_inventario=model.data_inventario,
            volume_estimado_m3=model.volume_estimado_m3,
            area_inventariada_ha=model.area_inventariada_ha,
            status=model.status,
