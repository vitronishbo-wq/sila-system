...
class EstatisticaFlorestalService:
    ...
    def __init__(self, *, operador_repo: ConcessionarioFlorestalRepositoryPort, unidade_repo: UnidadeManejoRepositoryPort, plano_repo: PlanoManejoFlorestalRepositoryPort, inventario_repo: InventarioFlorestalRepositoryPort, ambiente_service: AmbienteServicePort, gestao_fundiaria_service: GestaoFundiariaServicePort, agricultura_service: AgriculturaServicePort, energia_service: EnergiaServicePort, comercio_externo_service: ComercioExternoServicePort, geosampa_service: GeosampaServicePort):
        ...
        self._integracoes: dict[str, Any] = {'ambiente': ambiente_service, 'gestao_fundiaria': gestao_fundiaria_service, 'agricultura': agricultura_service, 'energia': energia_service, 'comercio_externo': comercio_externo_service, 'geosampa': geosampa_service}
    ...
    async def gerar_resumo_operacional(self) -> dict[str, Any]:
        ...
        return {'total_operadores': total_operadores, 'total_unidades_manejo': total_unidades, 'total_planos_manejo': total_planos, 'total_inventarios': total_inventarios, 'media_unidades_por_operador': round(total_unidades / total_operadores, 2) if total_operadores else 0.0, 'media_planos_por_unidade': round(total_planos / total_unidades, 2) if total_unidades else 0.0}

    async def gerar_status_integracao(self) -> dict[str, Any]:
        items = await asyncio.gather(*[self._coletar_status_modulo(nome, porta) for nome, porta in self._integracoes.items()])
        total = len(items)
        ativos = sum((1 for item in items if item['available']))
        return {'integracao_ok': ativos == total, 'modulos_ativos': ativos, 'modulos_totais': total, 'modulos': items}

    async def gerar_dashboard(self) -> dict[str, Any]:
        operacional, integracao = await asyncio.gather(self.gerar_resumo_operacional(), self.gerar_status_integracao())
