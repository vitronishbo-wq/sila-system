...
async def test_submeter_plano_sucesso():
    unidade = UnidadeManejo(
        id=uuid4(),
        codigo_um="UM/ABC/2026/0001",
        nome="UM Norte",
        area_total_ha=Decimal("100.0"),
        area_manejo_ha=Decimal("80.0"),
        area_preservacao_ha=Decimal("20.0"),
        tipo_manejo=TipoManejo.SUSTENTAVEL,
        ciclo_corte=TipoCicloCorte.DECENAL,
        operador_id=uuid4(),
        imovel_id=uuid4(),
        data_criacao=date.today(),
    )
    service = PlanoManejoService(plano_repo=_PlanoRepo(), unidade_repo=_UnidadeRepo(unidade))
    plano = await service.submeter_plano(
        unidade_manejo_id=unidade.id,
        responsavel_tecnico_id=uuid4(),
        responsavel_tecnico_registro="ENG-123",
        volume_anual_estimado_m3=Decimal("1500"),
        ciclo_corte_anos=10,
        area_anual_ha=Decimal("50"),
    )
