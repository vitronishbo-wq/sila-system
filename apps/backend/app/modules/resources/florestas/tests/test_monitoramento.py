...
class _FakeEstatisticaService:
    async def gerar_dashboard(self):
        return {
            "operacional": {
                "total_operadores": 3,
                "total_unidades_manejo": 6,
                "total_planos_manejo": 4,
                "total_inventarios": 9,
                "media_unidades_por_operador": 2.0,
                "media_planos_por_unidade": 0.67,
            },
            "integracao": {
                "integracao_ok": True,
                "modulos_ativos": 6,
                "modulos_totais": 6,
                "modulos": [
                    {"modulo": "ambiente", "available": True, "detalhes": {"status": "ok"}}
                ],
            },
        }
...
async def test_resumo_operacional_agrega_metricas():
    ...
    service = EstatisticaFlorestalService(
        operador_repo=_OperadorRepo([SimpleNamespace(id=op1), SimpleNamespace(id=op2)]),
        unidade_repo=_UnidadeRepo(
            {
                op1: [SimpleNamespace(id=um1), SimpleNamespace(id=um2)],
                op2: [SimpleNamespace(id=um3)],
            }
        ),
        plano_repo=_PlanoRepo({um1: [1], um2: [1, 2], um3: []}),
        inventario_repo=_InventarioRepo({um1: [1], um2: [1], um3: [1, 2, 3]}),
        ambiente_service=_ExternalOk(),
        gestao_fundiaria_service=_ExternalOk(),
        agricultura_service=_ExternalOk(),
        energia_service=_ExternalOk(),
        comercio_externo_service=_ExternalOk(),
        geosampa_service=_ExternalOk(),
    )
...
async def test_status_integracao_identifica_modulo_indisponivel():
    service = EstatisticaFlorestalService(
        operador_repo=_OperadorRepo([]),
        unidade_repo=_UnidadeRepo({}),
        plano_repo=_PlanoRepo({}),
        inventario_repo=_InventarioRepo({}),
        ambiente_service=_ExternalOk(),
        gestao_fundiaria_service=_ExternalDown(),
        agricultura_service=_ExternalOk(),
        energia_service=_ExternalOk(),
        comercio_externo_service=_ExternalOk(),
        geosampa_service=_ExternalOk(),
...
@pytest.mark.parametrize(
    ("adapter", "expected_capability", "metric_key", "metric_value"),
    [
        (
            AmbienteServiceAdapter(_ListDependency(3)),
            "licenciamento_ambiental",
            "car_registrados",
            3,
        ),
        (
            GestaoFundiariaServiceAdapter(_ListDependency(4)),
            "cadastro_imoveis_car",
            "imoveis_registrados",
            4,
        ),
        (
            AgriculturaServiceAdapter(_ListDependency(5)),
            "correlacao_uso_solo",
            "propriedades_rurais",
            5,
        ),
        (
            ComercioExternoServiceAdapter(_ListDependency(6)),
            "controle_exportacao_madeira",
            "exportadores_habilitados",
            6,
        ),
    ],
)
async def test_contract_payload_integracao_lista(
    adapter, expected_capability, metric_key, metric_value
):
...
@pytest.mark.parametrize(
    "adapter",
    [
        AmbienteServiceAdapter(None),
        GestaoFundiariaServiceAdapter(None),
        AgriculturaServiceAdapter(None),
        ComercioExternoServiceAdapter(None),
    ],
)
