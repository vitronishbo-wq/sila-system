...
inventario_service_dep = Depends(get_inventario_service)


@router.post("/", response_model=InventarioFlorestalResponse, status_code=201)
async def registrar_inventario(
    data: InventarioFlorestalCreate, service: InventarioService = inventario_service_dep
):
    return await service.registrar_inventario(
        unidade_manejo_id=data.unidade_manejo_id,
        volume_estimado_m3=data.volume_estimado_m3,
        area_inventariada_ha=data.area_inventariada_ha,
...
async def listar_inventarios(
    unidade_manejo_id: UUID, service: InventarioService = inventario_service_dep
):
