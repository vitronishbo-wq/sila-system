...
operador_florestal_service_dep = Depends(get_operador_florestal_service)


@router.post("/", response_model=OperadorFlorestalResponse, status_code=201)
async def cadastrar_operador(
    data: OperadorFlorestalCreate,
    service: OperadorFlorestalService = operador_florestal_service_dep,
):
    try:
        return await service.cadastrar_operador(
            nome=data.nome, nif=data.nif, tipo_operador=data.tipo_operador
        )
