...
async def test_cadastro_service_fluxo_car_sucesso():
    ...
    proprietario = await service.cadastrar_proprietario(
        nome="Maria Silva", documento="BI1234567", telefone="923000111"
    )
    imovel = await service.cadastrar_imovel(
        proprietario_id=proprietario.id,
        nome="Fazenda Verde",
        provincia="Huambo",
        municipio="Caala",
        area_total=Decimal("150.0"),
        bioma=Bioma.SAVANA,
        tipo_imovel=TipoImovel.MEDIA_PROPRIEDADE,
    )
    car = await service.criar_car(
        imovel_id=imovel.id,
        proprietario_id=proprietario.id,
        area_total=Decimal("150.0"),
        bioma=Bioma.SAVANA,
    ...
    car = await service.atualizar_areas(
        numero_car=car.numero_car,
        area_preservacao_permanente=Decimal("20"),
        area_reserva_legal=Decimal("30"),
        area_uso_alternativo=Decimal("70"),
        area_consolidada=Decimal("20"),
    )
...
def test_endpoint_criar_car_retorna_201():
    mock_item = SimpleNamespace(
        ...
        area_total="150.00",
        area_preservacao_permanente="0",
        area_reserva_legal="0",
        area_uso_alternativo="0",
        area_consolidada="0",
        bioma="savana",
        tipo_imovel="media_propriedade",
        status="pendente",
        data_cadastro="2026-02-28",
        data_analise=None,
        data_aprovacao=None,
        analista_id=None,
        observacoes=None,
...
def test_endpoint_obter_car_retorna_404():
    service = SimpleNamespace(
        obter_por_numero=AsyncMock(side_effect=CARNotFoundError("CAR nao encontrado"))
    )
    app = FastAPI()
    app.include_router(car_router, prefix="/ambiente")
    app.dependency_overrides[get_cadastro_service] = lambda: service
