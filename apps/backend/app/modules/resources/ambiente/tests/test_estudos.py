...
from apps.backend.app.modules.resources.ambiente.api.deps import (
    get_condicionante_service,
    get_estudo_service,
)
...
async def test_fluxo_estudo_e_condicionante_sucesso():
    ...
    condicionante_service = CondicionanteService(
        licenca_repo=licenca_repo, condicionante_repo=condicionante_repo
    )
    proprietario = await cadastro_service.cadastrar_proprietario(
        nome="Domingos Miala", documento="BI1234500"
    )
    imovel = await cadastro_service.cadastrar_imovel(
        proprietario_id=proprietario.id,
        nome="Fazenda Rio Azul",
        provincia="Benguela",
        municipio="Lobito",
        area_total=Decimal("120"),
        bioma=Bioma.SAVANA,
        tipo_imovel=TipoImovel.MEDIA_PROPRIEDADE,
    )
    ...
    licenca = await licenciamento_service.requerer_licenca(
        numero_car=car.numero_car, tipo=TipoLicenca.PREVIA, atividade="Projeto de agroindustria"
    )
    ...
    estudo = await estudo_service.submeter(
        numero_licenca=licenca.numero_licenca,
        tipo=TipoEstudoAmbiental.EIA,
        descricao="EIA da planta de processamento",
        responsavel_tecnico="Eng. Ambiental Kunda",
    )
    ...
    condicionante = await condicionante_service.criar(
        numero_licenca=licenca.numero_licenca,
        descricao="Implantar faixa de protecao ciliar",
        prazo_dias=180,
    )
    assert condicionante.status == StatusCondicionante.PENDENTE
    condicionante = await condicionante_service.iniciar_cumprimento(
        condicionante.codigo_condicionante
    )
    assert condicionante.status == StatusCondicionante.EM_CUMPRIMENTO
    condicionante = await condicionante_service.registrar_cumprimento(
        condicionante.codigo_condicionante, evidencia="Relatorio fotografico"
    )
...
def test_endpoint_submeter_estudo_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        numero_estudo="EST/2026/000001",
        numero_licenca="LIC/2026/000001",
        tipo=TipoEstudoAmbiental.EIA,
        descricao="Estudo de impacto da captação",
        responsavel_tecnico="Tecnico A",
        status=StatusEstudoAmbiental.SUBMETIDO,
        data_submissao=date(2026, 2, 28),
        data_analise=None,
        data_aprovacao=None,
        analista_id=None,
        observacoes=None,
...
def test_endpoint_obter_estudo_retorna_404():
    service = SimpleNamespace(
        obter_por_numero=AsyncMock(
            side_effect=EstudoNotFoundError("Estudo ambiental nao encontrado")
        )
    )
    app = FastAPI()
    app.include_router(estudos_router, prefix="/ambiente")
...
def test_endpoint_criar_condicionante_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_condicionante="COND/2026/000001",
        numero_licenca="LIC/2026/000001",
        descricao="Implementar monitoramento trimestral",
        prazo_dias=90,
        status=StatusCondicionante.PENDENTE,
        data_criacao=date(2026, 2, 28),
        data_limite=date(2026, 5, 29),
...
def test_endpoint_obter_condicionante_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(
            side_effect=CondicionanteNotFoundError("Condicionante nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(condicionantes_router, prefix="/ambiente")
