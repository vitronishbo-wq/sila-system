...
async def test_fiscalizacao_service_fluxo_sucesso():
    ...
    proprietario = await cadastro_service.cadastrar_proprietario(
        nome="Fatima Cangola", documento="BI11223344"
    )
    ...
    licenca = await licenciamento_service.requerer_licenca(
        numero_car=car.numero_car, tipo=TipoLicenca.PREVIA, atividade="Ampliação de área produtiva"
    )
    ...
    fiscalizacao = await fiscalizacao_service.agendar(
        ...
        localidade="Perimetro Sul",
        objetivo="Verificar cumprimento de condicionantes",
        fiscal_responsavel="Fiscal A",
    ...
    fiscalizacao = await fiscalizacao_service.concluir(
        fiscalizacao.numero_fiscalizacao, "Nao foram identificadas irregularidades graves"
    )
...
def test_endpoint_agendar_fiscalizacao_retorna_201():
    mock_item = SimpleNamespace(
        ...
        localidade="Perimetro Sul",
        objetivo="Fiscalizacao ordinaria",
        fiscal_responsavel="Fiscal A",
        status=StatusFiscalizacao.AGENDADA,
        data_agendada=date(2026, 2, 28),
    ...
    response = client.post(
        "/ambiente/fiscalizacoes/",
        json={
            "numero_licenca": "LIC/2026/000001",
            "localidade": "Perimetro Sul",
            "objetivo": "Fiscalizacao ordinaria",
            "fiscal_responsavel": "Fiscal A",
            "data_agendada": "2026-02-28",
        },
    )
...
def test_endpoint_obter_fiscalizacao_retorna_404():
    service = SimpleNamespace(
        obter_por_numero=AsyncMock(
            side_effect=FiscalizacaoNotFoundError("Fiscalizacao nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(fiscalizacoes_router, prefix="/ambiente")
