...
from apps.backend.app.modules.resources.ambiente.api.schemas.car_schema import (
    CARAprovacaoInput,
    CARAreasInput,
    CARCreate,
    CARPendenciaInput,
...
cadastro_service_dep = Depends(get_cadastro_service)


@router.post(
    "/proprietarios", response_model=ProprietarioResponse, status_code=status.HTTP_201_CREATED
)
async def cadastrar_proprietario(
    data: ProprietarioCreate, service: CadastroService = cadastro_service_dep
):
    try:
        return await service.cadastrar_proprietario(
            nome=data.nome, documento=data.documento, telefone=data.telefone, email=data.email
        )
...
async def cadastrar_imovel(
    data: ImovelCreate, service: CadastroService = cadastro_service_dep
):
    try:
        return await service.cadastrar_imovel(
...
async def criar_car(data: CARCreate, service: CadastroService = cadastro_service_dep):
    try:
        return await service.criar_car(
...
async def atualizar_areas_car(
    numero_car: str, data: CARAreasInput, service: CadastroService = cadastro_service_dep
):
    try:
        return await service.atualizar_areas(
            numero_car=numero_car,
            area_preservacao_permanente=data.area_preservacao_permanente,
            area_reserva_legal=data.area_reserva_legal,
            area_uso_alternativo=data.area_uso_alternativo,
            area_consolidada=data.area_consolidada,
        )
...
async def submeter_car(numero_car: str, service: CadastroService = cadastro_service_dep):
    try:
...
async def aprovar_car(
    numero_car: str,
    data: CARAprovacaoInput,
    service: CadastroService = cadastro_service_dep,
):
    try:
...
async def solicitar_pendencia_car(
    numero_car: str,
    data: CARPendenciaInput,
    service: CadastroService = cadastro_service_dep,
):
    try:
...
async def obter_car(numero_car: str, service: CadastroService = cadastro_service_dep):
    try:
...
async def listar_cars(
    status_car: StatusCAR | None = None, service: CadastroService = cadastro_service_dep
):
