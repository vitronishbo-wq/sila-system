...
from apps.backend.app.modules.resources.ambiente.api.schemas.multa_schema import (
    MultaCancelamentoInput,
    MultaCreate,
    MultaParcelamentoInput,
    MultaResponse,
)
...
async def aplicar_multa(
    data: MultaCreate, service: PenalidadeService = penalidade_service_dep
):
    try:
        return await service.aplicar_multa(
            numero_auto_infracao=data.numero_auto_infracao,
            valor=data.valor,
            dias_vencimento=data.dias_vencimento,
        )
...
async def parcelar_multa(
    numero_multa: str,
    data: MultaParcelamentoInput,
    service: PenalidadeService = penalidade_service_dep,
):
    try:
        return await service.parcelar_multa(
            numero_multa, quantidade_parcelas=data.quantidade_parcelas
        )
...
async def pagar_multa(
    numero_multa: str, service: PenalidadeService = penalidade_service_dep
):
    try:
...
async def cancelar_multa(
    numero_multa: str,
    data: MultaCancelamentoInput,
    service: PenalidadeService = penalidade_service_dep,
):
    try:
...
async def listar_multas(
    numero_auto_infracao: str | None = None,
    status_multa: StatusMulta | None = None,
    service: PenalidadeService = penalidade_service_dep,
):
    return await service.listar_multas(
        numero_auto_infracao=numero_auto_infracao, status=status_multa
    )