...
async def criar_condicionante(data: CondicionanteCreate, service: CondicionanteService=Depends(get_condicionante_service)):
    try:
        return await service.criar(numero_licenca=data.numero_licenca, descricao=data.descricao, prazo_dias=data.prazo_dias)
...
async def iniciar_cumprimento(codigo_condicionante: str, service: CondicionanteService=Depends(get_condicionante_service)):
    try:
...
async def concluir_condicionante(codigo_condicionante: str, data: CondicionanteCumprimentoInput, service: CondicionanteService=Depends(get_condicionante_service)):
    try:
...
async def descumprir_condicionante(codigo_condicionante: str, data: CondicionanteDescumprimentoInput, service: CondicionanteService=Depends(get_condicionante_service)):
    try:
...
async def obter_condicionante(codigo_condicionante: str, service: CondicionanteService=Depends(get_condicionante_service)):
    try:
...
async def listar_condicionantes(numero_licenca: str | None=None, status_condicionante: StatusCondicionante | None=None, service: CondicionanteService=Depends(get_condicionante_service)):
