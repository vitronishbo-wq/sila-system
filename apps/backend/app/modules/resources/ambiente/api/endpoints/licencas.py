...
async def requerer_licenca(data: LicencaCreate, service: LicenciamentoService=Depends(get_licenciamento_service)):
    try:
        return await service.requerer_licenca(numero_car=data.numero_car, tipo=data.tipo, atividade=data.atividade)
...
async def iniciar_analise(numero_licenca: str, service: LicenciamentoService=Depends(get_licenciamento_service)):
    try:
...
async def deferir_licenca(numero_licenca: str, data: LicencaDeferimentoInput, service: LicenciamentoService=Depends(get_licenciamento_service)):
    try:
        return await service.deferir(numero_licenca, analista_id=data.analista_id, data_validade=data.data_validade, condicionantes=data.condicionantes)
    except LicencaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def indeferir_licenca(numero_licenca: str, data: LicencaIndeferimentoInput, service: LicenciamentoService=Depends(get_licenciamento_service)):
    try:
        return await service.indeferir(numero_licenca, analista_id=data.analista_id, motivo=data.motivo)
    except LicencaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def suspender_licenca(numero_licenca: str, data: LicencaSuspensaoInput, service: LicenciamentoService=Depends(get_licenciamento_service)):
    try:
...
async def cancelar_licenca(numero_licenca: str, data: LicencaCancelamentoInput, service: LicenciamentoService=Depends(get_licenciamento_service)):
    try:
...
async def obter_licenca(numero_licenca: str, service: LicenciamentoService=Depends(get_licenciamento_service)):
    try:
...
async def listar_licencas(numero_car: str | None=None, tipo: TipoLicenca | None=None, status_licenca: StatusLicenca | None=None, service: LicenciamentoService=Depends(get_licenciamento_service)):
