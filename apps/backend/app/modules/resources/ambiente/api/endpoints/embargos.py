...
from apps.backend.app.modules.resources.ambiente.api.schemas.embargo_schema import (
    EmbargoCreate,
    EmbargoLevantamentoInput,
    EmbargoResponse,
    EmbargoSuspensaoInput,
)
...
async def aplicar_embargo(
    data: EmbargoCreate, service: PenalidadeService = penalidade_service_dep
):
    try:
        return await service.aplicar_embargo(
            numero_auto_infracao=data.numero_auto_infracao, motivo=data.motivo
        )
...
async def suspender_embargo(
    numero_embargo: str,
    data: EmbargoSuspensaoInput,
...
async def obter_embargo(
    numero_embargo: str, service: PenalidadeService = penalidade_service_dep
):
    try:
...
async def listar_embargos(
    ...
    status_embargo: StatusEmbargo | None = None,
    service: PenalidadeService = penalidade_service_dep,
):
    return await service.listar_embargos(
        numero_auto_infracao=numero_auto_infracao, status=status_embargo
    )