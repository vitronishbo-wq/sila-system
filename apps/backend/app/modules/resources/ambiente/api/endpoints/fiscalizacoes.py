...
from apps.backend.app.modules.resources.ambiente.api.schemas.fiscalizacao_schema import (
    FiscalizacaoCancelamentoInput,
    FiscalizacaoConclusaoInput,
    FiscalizacaoCreate,
...
async def agendar_fiscalizacao(
    ...
    try:
        return await service.agendar(
            ...
            fiscal_responsavel=data.fiscal_responsavel,
...
async def iniciar_fiscalizacao(
    numero_fiscalizacao: str, service: FiscalizacaoService = fiscalizacao_service_dep
):
    try:
...
async def concluir_fiscalizacao(
    numero_fiscalizacao: str,
    data: FiscalizacaoConclusaoInput,
    service: FiscalizacaoService = fiscalizacao_service_dep,
):
    try:
...
async def cancelar_fiscalizacao(
    numero_fiscalizacao: str,
    data: FiscalizacaoCancelamentoInput,
    service: FiscalizacaoService = fiscalizacao_service_dep,
):
    try:
...
async def listar_fiscalizacoes(
    numero_licenca: str | None = None,
    status_fiscalizacao: StatusFiscalizacao | None = None,
    service: FiscalizacaoService = fiscalizacao_service_dep,
):
