...
class Multa:
    ...
    @classmethod
    def aplicar(
        cls, *, numero_auto_infracao: str, valor: Decimal, dias_vencimento: int = 30
    ) -> Multa:
        if valor <= Decimal("0"):
        ...
        return cls(
            ...
            status=StatusMulta.APLICADA,
            data_aplicacao=data_aplicacao,
            data_vencimento=data_aplicacao + timedelta(days=dias_vencimento),
        )
    ...
    def atualizar_vencimento(self) -> None:
        if (
            self.status in {StatusMulta.APLICADA, StatusMulta.PARCELADA}
            and date.today() > self.data_vencimento
        ):
