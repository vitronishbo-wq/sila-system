...
class CAR:
    ...
    @classmethod
    def criar(cls, *, imovel_id: UUID, proprietario_id: UUID, area_total: Decimal, bioma: Bioma, tipo_imovel: TipoImovel) -> 'CAR':
        if area_total <= Decimal('0'):
            raise ValueError('Area total deve ser maior que zero')
        return cls(id=uuid4(), numero_car='', imovel_id=imovel_id, proprietario_id=proprietario_id, area_total=area_total.quantize(Decimal('0.01')), area_preservacao_permanente=Decimal('0'), area_reserva_legal=Decimal('0'), area_uso_alternativo=Decimal('0'), area_consolidada=Decimal('0'), bioma=bioma, tipo_imovel=tipo_imovel, status=StatusCAR.PENDENTE, data_cadastro=date.today())

    def definir_areas(self, *, area_preservacao_permanente: Decimal, area_reserva_legal: Decimal, area_uso_alternativo: Decimal, area_consolidada: Decimal) -> None:
        valores = [area_preservacao_permanente, area_reserva_legal, area_uso_alternativo, area_consolidada]
