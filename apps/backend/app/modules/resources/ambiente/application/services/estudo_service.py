...
class EstudoService:
    ...
    def __init__(self, *, licenca_repo: LicencaRepositoryPort, estudo_repo: EstudoRepositoryPort) -> None:
        self._licenca_repo = licenca_repo
        self._estudo_repo = estudo_repo

    async def submeter(self, *, numero_licenca: str, tipo: TipoEstudoAmbiental, descricao: str, responsavel_tecnico: str) -> EstudoImpacto:
        licenca = await self._licenca_repo.get_by_numero(numero_licenca)
        if not licenca:
            raise LicencaNotFoundError('Licenca ambiental nao encontrada')
        item = EstudoImpacto.submeter(numero_licenca=numero_licenca, tipo=tipo, descricao=descricao, responsavel_tecnico=responsavel_tecnico)
    ...
    async def solicitar_complementacao(self, numero_estudo: str, analista_id: UUID, motivo: str) -> EstudoImpacto:
        item = await self._estudo_repo.get_by_numero(numero_estudo)
        if not item:
            raise EstudoNotFoundError('Estudo ambiental nao encontrado')
    ...
    async def listar(self, *, numero_licenca: str | None=None, tipo: TipoEstudoAmbiental | None=None, status: StatusEstudoAmbiental | None=None) -> list[EstudoImpacto]:
