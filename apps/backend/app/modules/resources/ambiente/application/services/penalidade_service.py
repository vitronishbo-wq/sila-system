...
class PenalidadeService:
    def __init__(
        self,
        *,
        fiscalizacao_repo: FiscalizacaoRepositoryPort,
        auto_infracao_repo: AutoInfracaoRepositoryPort,
        embargo_repo: EmbargoRepositoryPort,
        multa_repo: MultaRepositoryPort,
    ) -> None:
    ...
    async def lavrar_auto(
        ...
        *,
        ...
        if fiscalizacao.status not in {
            StatusFiscalizacao.EM_ANDAMENTO,
            StatusFiscalizacao.CONCLUIDA,
        }:
            raise ValueError("Fiscalizacao deve estar em andamento ou concluida para lavrar auto")
        item = AutoInfracao.lavrar(
            numero_fiscalizacao=numero_fiscalizacao,
            tipo=tipo,
            descricao=descricao,
            fiscal_id=fiscal_id,
    ...
    async def julgar_auto(
        self, numero_auto: str, *, mantido: bool, observacoes: str | None = None
    ) -> AutoInfracao:
        item = await self._auto_infracao_repo.get_by_numero(numero_auto)
        if not item:
            raise AutoInfracaoNotFoundError("Auto de infracao nao encontrado")
    ...
    async def levantar_embargo(
        self, numero_embargo: str, *, observacoes: str | None = None
    ) -> Embargo:
        item = await self._embargo_repo.get_by_numero(numero_embargo)
        if not item:
            raise EmbargoNotFoundError("Embargo nao encontrado")
    ...
    async def aplicar_multa(
        self, *, numero_auto_infracao: str, valor: Decimal, dias_vencimento: int = 30
    ) -> Multa:
        auto = await self._auto_infracao_repo.get_by_numero(numero_auto_infracao)
        if not auto:
            raise AutoInfracaoNotFoundError("Auto de infracao nao encontrado")
        item = Multa.aplicar(
            numero_auto_infracao=numero_auto_infracao, valor=valor, dias_vencimento=dias_vencimento
        )
    ...
    async def listar_autos(
        ...
        *,
        ...
        return await self._auto_infracao_repo.list(
            numero_fiscalizacao=numero_fiscalizacao, tipo=tipo, status=status
        )
    ...
    async def listar_embargos(
        self, *, numero_auto_infracao: str | None = None, status: StatusEmbargo | None = None
    ) -> list[Embargo]:
        return await self._embargo_repo.list(
            numero_auto_infracao=numero_auto_infracao, status=status
        )
    ...
    async def listar_multas(
        ...
        items = await self._multa_repo.list(
            numero_auto_infracao=numero_auto_infracao, status=status
        )
        for item in items:
            item.atualizar_vencimento()
