...
class LicenciamentoService:
    ...
    async def requerer_licenca(
        self, *, numero_car: str, tipo: TipoLicenca, atividade: str
    ) -> LicencaAmbiental:
        ...
        ativos = {
            StatusLicenca.REQUERIDA,
            StatusLicenca.EM_ANALISE,
            StatusLicenca.DEFERIDA,
    ...
    async def deferir(
        ...
        *,
        ...
        if not item:
            raise LicencaNotFoundError("Licenca ambiental nao encontrada")
        item.deferir(
            analista_id=analista_id, data_validade=data_validade, condicionantes=condicionantes
        )
        return await self._licenca_repo.save(item)

    async def indeferir(
        self, numero_licenca: str, *, analista_id: UUID, motivo: str
    ) -> LicencaAmbiental:
        item = await self._licenca_repo.get_by_numero(numero_licenca)
        if not item:
            raise LicencaNotFoundError("Licenca ambiental nao encontrada")
