...
class CondicionanteService:
    ...
    def __init__(self, *, licenca_repo: LicencaRepositoryPort, condicionante_repo: CondicionanteRepositoryPort) -> None:
    ...
    async def criar(self, *, numero_licenca: str, descricao: str, prazo_dias: int) -> Condicionante:
        ...
        item = Condicionante.criar(numero_licenca=numero_licenca, descricao=descricao, prazo_dias=prazo_dias)
    ...
    async def registrar_cumprimento(self, codigo_condicionante: str, evidencia: str | None=None) -> Condicionante:
        item = await self._condicionante_repo.get_by_codigo(codigo_condicionante)
        if not item:
            raise CondicionanteNotFoundError('Condicionante nao encontrada')
    ...
    async def listar(self, *, numero_licenca: str | None=None, status: StatusCondicionante | None=None) -> list[Condicionante]:
