...
async def test_cadastrar_embarcacao_sucesso():
    repo = SimpleNamespace(
        next_inscricao=AsyncMock(return_value="LUANDA/2026/0001"),
    ...
    result = await service.cadastrar_embarcacao(
        nome="Mar Azul",
        tipo=TipoEmbarcacao.ARTESANAL,
        comprimento=Decimal("10.5"),
        arqueacao_bruta=Decimal("20.2"),
        porto_registro="LUANDA",
...
def test_endpoint_obter_embarcacao_404():
    service = SimpleNamespace(
        buscar_embarcacao=AsyncMock(side_effect=ValueError("Embarcacao nao encontrada"))
    )
    app = FastAPI()
