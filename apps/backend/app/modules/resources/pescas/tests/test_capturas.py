...
async def test_registrar_captura_sucesso():
    licenca = SimpleNamespace(status=StatusLicenca.DEFERIDA, data_validade=date.today() + timedelta(days=1))
    ...
    result = await service.registrar_captura(embarcacao_id=uuid4(), licenca_id=uuid4(), zona_pesca_id=uuid4(), especie_id=uuid4(), quantidade_kg=Decimal('100.5'), arte_pesca_id=uuid4())
    assert result.quantidade_kg == Decimal('100.5')

def test_endpoint_registrar_captura_400():
    service = SimpleNamespace(registrar_captura=AsyncMock(side_effect=ValueError('Licenca vencida')))
    app = FastAPI()
    app.include_router(capturas_router, prefix='/pescas')
    app.dependency_overrides[get_captura_service] = lambda: service
    client = TestClient(app)
    response = client.post('/pescas/capturas/', json={'embarcacao_id': str(uuid4()), 'licenca_id': str(uuid4()), 'zona_pesca_id': str(uuid4()), 'especie_id': str(uuid4()), 'quantidade_kg': '10.0', 'arte_pesca_id': str(uuid4())})
