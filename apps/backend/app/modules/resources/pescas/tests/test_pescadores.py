...
async def test_cadastrar_pescador_sucesso():
    repo = SimpleNamespace(
        get_by_numero_registro=AsyncMock(return_value=None),
        next_registro=AsyncMock(return_value="PES/2026/000001"),
    ...
    item = await service.cadastrar_pescador(
        nome="Mario", tipo=TipoPescador.ARTESANAL, citizen_id=uuid4()
    )
...
def test_endpoint_obter_pescador_404():
    service = SimpleNamespace(
        buscar_pescador=AsyncMock(side_effect=ValueError("Pescador nao encontrado"))
    )
    app = FastAPI()
