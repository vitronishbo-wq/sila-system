...
async def test_emitir_licenca_sucesso():
    ...
    licenca_repo = SimpleNamespace(
        next_numero=AsyncMock(return_value="LIC/2026/000001"),
        save=AsyncMock(side_effect=lambda i: i),
    )
    service = LicenciamentoPescaService(licenca_repo=licenca_repo, embarcacao_repo=embarcacao_repo)
    result = await service.emitir_licenca(
        embarcacao_id=embarcacao_id,
        titular_id=titular_id,
        modalidade_autorizada="arrasto",
        zona_pesca_id=zona_id,
    )
