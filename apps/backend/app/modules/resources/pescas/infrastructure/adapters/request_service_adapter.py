...
from app.modules.resources.pescas.application.ports import RequestServicePort
...
class RequestServiceAdapter(RequestServicePort):
    ...
    async def create_request(self, *, request_type: str, entity_id: UUID, citizen_id: UUID, numero_processo: str, metadata: dict[str, Any] | None=None) -> UUID:
        return await self._bridge.create_education_request(entity_id=entity_id, citizen_id=citizen_id, numero_processo=numero_processo, escola_nome=f'Pescas/{request_type}', ano_letivo=str(date.today().year))
