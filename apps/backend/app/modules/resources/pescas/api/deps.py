...
class _EspecieCatalogService:
    ...
    async def cadastrar(self, *, nome_comum: str, nome_cientifico: str, codigo_fao: str) -> Especie:
        item = Especie.cadastrar(nome_comum=nome_comum, nome_cientifico=nome_cientifico, codigo_fao=codigo_fao)
...
async def get_pescador_service(session: AsyncSession=Depends(get_db)) -> PescadorService:
    ...
    return PescadorService(pescador_repo=repository, citizen_service=citizen_service, request_service=request_service)

async def get_armador_service() -> ArmadorService:
    return ArmadorService(repository=_armador_repo_singleton)

async def get_embarcacao_service(session: AsyncSession=Depends(get_db)) -> EmbarcacaoService:
    repository = SQLAlchemyEmbarcacaoRepository(session)
    citizen_service = CitizenServiceAdapter(CitizenRepository(session))
    request_service = RequestServiceAdapter(ServiceRequestLifecycleBridge(session))
    return EmbarcacaoService(embarcacao_repo=repository, citizen_service=citizen_service, request_service=request_service)

async def get_licenciamento_pesca_service(session: AsyncSession=Depends(get_db)) -> LicenciamentoPescaService:
    return LicenciamentoPescaService(licenca_repo=SQLAlchemyLicencaPescaRepository(session), embarcacao_repo=SQLAlchemyEmbarcacaoRepository(session))

async def get_captura_service(session: AsyncSession=Depends(get_db)) -> CapturaService:
    return CapturaService(captura_repo=SQLAlchemyCapturaRepository(session), licenca_repo=SQLAlchemyLicencaPescaRepository(session))
