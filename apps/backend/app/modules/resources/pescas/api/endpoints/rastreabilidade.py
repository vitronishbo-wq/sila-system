...
async def registrar_evento(data: RastreabilidadeCreate, service: RastreabilidadeService=Depends(get_rastreabilidade_service)):
    return await service.registrar_evento(lote_codigo=data.lote_codigo, origem_captura_id=data.origem_captura_id, etapa=data.etapa, operador=data.operador)

@router.get('/', response_model=list[RastreabilidadeResponse])
async def listar_eventos(service: RastreabilidadeService=Depends(get_rastreabilidade_service)):
