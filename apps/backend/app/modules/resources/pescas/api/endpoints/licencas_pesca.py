...
async def emitir_licenca(data: LicencaPescaCreate, service: LicenciamentoPescaService=Depends(get_licenciamento_pesca_service)):
    try:
        return await service.emitir_licenca(embarcacao_id=data.embarcacao_id, titular_id=data.titular_id, modalidade_autorizada=data.modalidade_autorizada, zona_pesca_id=data.zona_pesca_id, validade_dias=data.validade_dias)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))

@router.get('/{licenca_id}', response_model=LicencaPescaResponse)
async def obter_licenca(licenca_id: UUID, service: LicenciamentoPescaService=Depends(get_licenciamento_pesca_service)):
    try:
...
async def listar_licencas_validas(service: LicenciamentoPescaService=Depends(get_licenciamento_pesca_service)):
