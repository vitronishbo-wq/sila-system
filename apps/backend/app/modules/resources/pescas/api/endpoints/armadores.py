...
async def cadastrar_armador(data: ArmadorCreate, service: ArmadorService=Depends(get_armador_service)):
    try:
...
async def obter_armador(armador_id: UUID, service: ArmadorService=Depends(get_armador_service)):
    try:
...
async def listar_armadores(filtros: ArmadorFilter=Depends(), service: ArmadorService=Depends(get_armador_service)):
