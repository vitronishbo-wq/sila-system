...
from apps.backend.app.modules.resources.pescas.api.schemas.captura_schema import (
    CapturaCreate,
    CapturaResponse,
)
...
captura_service_dep = Depends(get_captura_service)
embarcacao_id_query = Query(...)


@router.post("/", response_model=CapturaResponse, status_code=status.HTTP_201_CREATED)
async def registrar_captura(
    data: CapturaCreate, service: CapturaService = captura_service_dep
):
    try:
        return await service.registrar_captura(
            ...
            arte_pesca_id=data.arte_pesca_id,
        )
...
async def obter_captura(captura_id: UUID, service: CapturaService = captura_service_dep):
    try:
...
async def listar_capturas(
    embarcacao_id: UUID = embarcacao_id_query, service: CapturaService = captura_service_dep
):
