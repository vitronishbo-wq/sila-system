...
class SQLAlchemyLoteProducaoRepository(LoteProducaoRepositoryPort):
    ...
    async def list_by_unidade(self, unidade_processamento_id: UUID) -> list[LoteProducao]:
        stmt = (
            select(LoteProducaoModel)
            .where(LoteProducaoModel.unidade_processamento_id == unidade_processamento_id)
            .order_by(LoteProducaoModel.codigo_lote.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_produto(self, produto_processado_id: UUID) -> list[LoteProducao]:
        stmt = (
            select(LoteProducaoModel)
            .where(LoteProducaoModel.produto_processado_id == produto_processado_id)
            .order_by(LoteProducaoModel.codigo_lote.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_status(self, status: StatusLoteProducao) -> list[LoteProducao]:
        stmt = (
            select(LoteProducaoModel)
            .where(LoteProducaoModel.status == status.value)
            .order_by(LoteProducaoModel.codigo_lote.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_periodo(self, data_inicio: date, data_fim: date) -> list[LoteProducao]:
        stmt = (
            select(LoteProducaoModel)
            .where(LoteProducaoModel.data_producao >= data_inicio)
            .where(LoteProducaoModel.data_producao <= data_fim)
            .order_by(LoteProducaoModel.data_producao.asc(), LoteProducaoModel.codigo_lote.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(LoteProducaoModel)
            .where(LoteProducaoModel.codigo_lote.like(f"LOT/{ano}/%"))
        )
    ...
    def _to_domain(model: LoteProducaoModel) -> LoteProducao:
        return LoteProducao(
            ...
            data_producao=model.data_producao,
            quantidade_kg=model.quantidade_kg,
            status=StatusLoteProducao(model.status),
            destino_mercado=MercadoDestino(model.destino_mercado),
            data_validade=model.data_validade,
            turno=model.turno,
            temperatura_armazenamento_c=model.temperatura_armazenamento_c,
