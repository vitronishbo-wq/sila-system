...
class SQLAlchemyInspecaoRepository(InspecaoSanitariaIndustrialRepositoryPort):
    ...
    async def list_by_unidade(
        ...
        stmt = (
            select(InspecaoModel)
            .where(InspecaoModel.unidade_processamento_id == unidade_processamento_id)
            .order_by(InspecaoModel.codigo_inspecao.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_status(self, status: StatusInspecao) -> list[InspecaoSanitariaIndustrial]:
        stmt = (
            select(InspecaoModel)
            .where(InspecaoModel.status == status.value)
            .order_by(InspecaoModel.codigo_inspecao.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_lote(self, lote_producao_id: UUID) -> list[InspecaoSanitariaIndustrial]:
        stmt = (
            select(InspecaoModel)
            .where(InspecaoModel.lote_producao_id == lote_producao_id)
            .order_by(InspecaoModel.codigo_inspecao.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_periodo(
        ...
        stmt = (
            select(InspecaoModel)
            .where(InspecaoModel.data_agendada >= data_inicio)
            .where(InspecaoModel.data_agendada <= data_fim)
            .order_by(InspecaoModel.data_agendada.asc(), InspecaoModel.codigo_inspecao.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(InspecaoModel)
            .where(InspecaoModel.codigo_inspecao.like(f"INS/{ano}/%"))
        )
    ...
    def _to_domain(model: InspecaoModel) -> InspecaoSanitariaIndustrial:
        return InspecaoSanitariaIndustrial(
            id=model.id,
            codigo_inspecao=model.codigo_inspecao,
            unidade_processamento_id=model.unidade_processamento_id,
            data_agendada=model.data_agendada,
            selo_inspecao=TipoSeloInspecao(model.selo_inspecao),
            status=StatusInspecao(model.status),
            fiscal_id=model.fiscal_id,
            lote_producao_id=model.lote_producao_id,
            data_realizacao=model.data_realizacao,
            pontuacao=model.pontuacao,
            inconformidades=list(model.inconformidades) if model.inconformidades else None,
