...
class SQLAlchemyProdutoProcessadoRepository(ProdutoProcessadoRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_produto: str) -> ProdutoProcessado | None:
        stmt = select(ProdutoProcessadoModel).where(
            ProdutoProcessadoModel.codigo_produto == codigo_produto.strip()
        )
    ...
    async def list_by_unidade(self, unidade_processamento_id: UUID) -> list[ProdutoProcessado]:
        stmt = (
            select(ProdutoProcessadoModel)
            .where(ProdutoProcessadoModel.unidade_processamento_id == unidade_processamento_id)
            .order_by(ProdutoProcessadoModel.codigo_produto.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_tipo(self, tipo_produto: TipoProdutoProcessado) -> list[ProdutoProcessado]:
        stmt = (
            select(ProdutoProcessadoModel)
            .where(ProdutoProcessadoModel.tipo_produto == tipo_produto.value)
            .order_by(ProdutoProcessadoModel.codigo_produto.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_by_destino(self, mercado_destino: MercadoDestino) -> list[ProdutoProcessado]:
        stmt = (
            select(ProdutoProcessadoModel)
            .where(ProdutoProcessadoModel.mercado_destino == mercado_destino.value)
            .order_by(ProdutoProcessadoModel.codigo_produto.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(ProdutoProcessadoModel)
            .where(ProdutoProcessadoModel.codigo_produto.like(f"PRD/{ano}/%"))
        )
    ...
    def _to_domain(model: ProdutoProcessadoModel) -> ProdutoProcessado:
        return ProdutoProcessado(
            ...
            unidade_processamento_id=model.unidade_processamento_id,
            nome_comercial=model.nome_comercial,
            tipo_produto=TipoProdutoProcessado(model.tipo_produto),
            tipo_processamento=TipoProcessamento(model.tipo_processamento),
            peso_liquido_kg=model.peso_liquido_kg,
            rendimento_percentual=model.rendimento_percentual,
            mercado_destino=MercadoDestino(model.mercado_destino),
