...
class SQLAlchemyUnidadeProcessamentoRepository(UnidadeProcessamentoRepositoryPort):
    ...
    async def save(self, unidade: UnidadeProcessamento) -> UnidadeProcessamento:
        ...
        model.certificacoes = (
            [str(item) for item in unidade.certificacoes] if unidade.certificacoes else None
        )
    ...
    async def get_by_cnpj(self, cnpj: str) -> UnidadeProcessamento | None:
        stmt = select(UnidadeProcessamentoModel).where(
            UnidadeProcessamentoModel.cnpj == cnpj.strip()
        )
    ...
    async def list_by_municipio(self, municipio: str) -> list[UnidadeProcessamento]:
        stmt = (
            select(UnidadeProcessamentoModel)
            .where(func.lower(UnidadeProcessamentoModel.municipio) == municipio.strip().lower())
            .order_by(UnidadeProcessamentoModel.razao_social.asc())
        )
    ...
    async def list_by_tipo(self, tipo: TipoProcessamento) -> list[UnidadeProcessamento]:
        stmt = (
            select(UnidadeProcessamentoModel)
            .where(UnidadeProcessamentoModel.tipo_processamento.contains([tipo.value]))
            .order_by(UnidadeProcessamentoModel.razao_social.asc())
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def list_all(self) -> list[UnidadeProcessamento]:
        stmt = select(UnidadeProcessamentoModel).order_by(
            UnidadeProcessamentoModel.razao_social.asc()
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self, provincia: str) -> str:
        ...
        stmt = (
            select(func.count())
            .select_from(UnidadeProcessamentoModel)
            .where(UnidadeProcessamentoModel.cnpj.like("%"))
        )
    ...
    def _to_domain(model: UnidadeProcessamentoModel) -> UnidadeProcessamento:
        return UnidadeProcessamento(
            ...
            tipo_processamento=[TipoProcessamento(item) for item in model.tipo_processamento],
            classificacao=ClassificacaoIndustrial(model.classificacao),
            capacidade_kg_dia=model.capacidade_kg_dia,
            area_total_m2=model.area_total_m2,
            area_producao_m2=model.area_producao_m2,
            area_armazenagem_m2=model.area_armazenagem_m2,
            capacidade_frigorifica_m3=model.capacidade_frigorifica_m3,
            temperatura_media=model.temperatura_media,
            numero_funcionarios=model.numero_funcionarios,
            ...
            data_inauguracao=model.data_inauguracao,
            licenca_operacao_id=model.licenca_operacao_id,
            alvara_sanitario_id=model.alvara_sanitario_id,
            certificacoes=[UUID(item) for item in model.certificacoes]
            if model.certificacoes
            else None,
