...
async def _seed_unidade(unidade_repo: InMemoryUnidadeRepository, cnpj: str, nome: str):
    unidade = UnidadeProcessamento.cadastrar(
        ...
        capacidade_kg_dia=Decimal("3500"),
        area_total_m2=Decimal("650"),
        area_producao_m2=Decimal("230"),
        area_armazenagem_m2=Decimal("180"),
...
def test_lote_crud_com_filtros() -> None:
    ...
    async def scenario() -> None:
        ...
        atualizado = await lote_service.atualizar_lote(
            lote_id=lote.id,
            status=StatusLoteProducao.CONCLUIDO,
            observacoes="Processamento finalizado",
        )
...
def test_lote_rejeita_produto_de_outra_unidade() -> None:
    ...
    async def scenario() -> None:
        ...
        produto = await produto_service.cadastrar_produto(
            ...
            nome_comercial="Pescada Seca",
            tipo_produto=TipoProdutoProcessado.SECO,
            tipo_processamento=TipoProcessamento.SECO,
            peso_liquido_kg=Decimal("1.0"),
            rendimento_percentual=Decimal("62"),
        ...
        with pytest.raises(ValueError, match="nao pertence"):
            await lote_service.cadastrar_lote(
                ...
                produto_processado_id=produto.id,
                data_producao=date(2026, 2, 20),
                quantidade_kg=Decimal("300"),
