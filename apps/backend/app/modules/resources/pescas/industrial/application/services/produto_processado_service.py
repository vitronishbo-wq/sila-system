...
class ProdutoProcessadoService:
    ...
    def __init__(self, *, produto_repo: ProdutoProcessadoRepositoryPort, unidade_repo: UnidadeProcessamentoRepositoryPort, request_service: RequestServicePort | None=None) -> None:
        self.produto_repo = produto_repo
        self.unidade_repo = unidade_repo
        self.request_service = request_service

    async def cadastrar_produto(self, *, unidade_processamento_id: UUID, nome_comercial: str, tipo_produto: TipoProdutoProcessado, tipo_processamento: TipoProcessamento, peso_liquido_kg: Decimal, rendimento_percentual: Decimal, mercado_destino: MercadoDestino, observacoes: str | None=None) -> ProdutoProcessado:
        ...
        if any((item.nome_comercial.lower() == normalized_name and item.tipo_produto == tipo_produto for item in existentes)):
            raise ValueError('Produto ja cadastrado para a unidade com este nome e tipo')
        codigo = await self.produto_repo.next_codigo()
        produto = ProdutoProcessado.cadastrar(codigo_produto=codigo, unidade_processamento_id=unidade_processamento_id, nome_comercial=nome_comercial, tipo_produto=tipo_produto, tipo_processamento=tipo_processamento, peso_liquido_kg=peso_liquido_kg, rendimento_percentual=rendimento_percentual, mercado_destino=mercado_destino, observacoes=observacoes)
        saved = await self.produto_repo.save(produto)
        if self.request_service is not None:
            await self.request_service.create_request(request_type='CADASTRO_PRODUTO_PROCESSADO', entity_id=saved.id, metadata={'codigo_produto': saved.codigo_produto, 'tipo_produto': saved.tipo_produto.value, 'mercado_destino': saved.mercado_destino.value}, numero_processo=saved.codigo_produto)
        return saved
    ...
    async def listar_produtos(self, *, unidade_processamento_id: UUID | None=None, tipo_produto: TipoProdutoProcessado | None=None, mercado_destino: MercadoDestino | None=None, apenas_ativos: bool=True) -> list[ProdutoProcessado]:
    ...
    async def atualizar_produto(self, *, produto_id: UUID, nome_comercial: str | None=None, tipo_produto: TipoProdutoProcessado | None=None, tipo_processamento: TipoProcessamento | None=None, peso_liquido_kg: Decimal | None=None, rendimento_percentual: Decimal | None=None, mercado_destino: MercadoDestino | None=None, ativo: bool | None=None, observacoes: str | None=None) -> ProdutoProcessado:
        item = await self.buscar_produto(produto_id)
        item.atualizar(nome_comercial=nome_comercial, tipo_produto=tipo_produto, tipo_processamento=tipo_processamento, peso_liquido_kg=peso_liquido_kg, rendimento_percentual=rendimento_percentual, mercado_destino=mercado_destino, ativo=ativo, observacoes=observacoes)
