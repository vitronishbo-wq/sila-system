...
class LoteProducaoService:
    ...
    async def cadastrar_lote(
        ...
        *,
        ...
        lote = LoteProducao.criar(
            codigo_lote=codigo,
            unidade_processamento_id=unidade_processamento_id,
            produto_processado_id=produto_processado_id,
            data_producao=data_producao,
            quantidade_kg=quantidade_kg,
            destino_mercado=destino_mercado,
            data_validade=data_validade,
            turno=turno,
            temperatura_armazenamento_c=temperatura_armazenamento_c,
            observacoes=observacoes,
        )
        saved = await self.lote_repo.save(lote)
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="ABERTURA_LOTE_PRODUCAO",
                entity_id=saved.id,
                metadata={
                    "codigo_lote": saved.codigo_lote,
                    "produto_id": str(saved.produto_processado_id),
                    "quantidade_kg": str(saved.quantidade_kg),
                },
                numero_processo=saved.codigo_lote,
            )
        return saved
    ...
    async def atualizar_lote(
        ...
        *,
        lote_id: UUID,
        quantidade_kg: Decimal | None = None,
        status: StatusLoteProducao | None = None,
