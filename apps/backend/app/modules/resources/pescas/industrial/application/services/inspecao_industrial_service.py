...
class InspecaoIndustrialService:
    def __init__(
        self,
        *,
        inspecao_repo: InspecaoSanitariaIndustrialRepositoryPort,
        unidade_repo: UnidadeProcessamentoRepositoryPort,
        lote_repo: LoteProducaoRepositoryPort,
    ...
    async def agendar_inspecao(
        ...
        *,
        ...
        inspecao = InspecaoSanitariaIndustrial.agendar(
            codigo_inspecao=codigo,
            unidade_processamento_id=unidade_processamento_id,
            data_agendada=data_agendada,
            selo_inspecao=selo_inspecao,
            fiscal_id=fiscal_id,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="AGENDAMENTO_INSPECAO_SANITARIA",
                entity_id=saved.id,
                metadata={
                    "codigo_inspecao": saved.codigo_inspecao,
                    "status": saved.status.value,
                    "selo": saved.selo_inspecao.value,
                },
                numero_processo=saved.codigo_inspecao,
                citizen_id=fiscal_id,
            )
        return saved
