...
class UnidadeProcessamentoService:
    ...
    def __init__(self, *, unidade_repo: UnidadeProcessamentoRepositoryPort, pescas_service: PescasServicePort, industria_service: IndustriaServicePort, citizen_service: CitizenServicePort | None=None, request_service: RequestServicePort | None=None) -> None:
    ...
    async def cadastrar_unidade(self, *, cnpj: str, razao_social: str, tipo_processamento: list[TipoProcessamento], classificacao: ClassificacaoIndustrial, capacidade_kg_dia: Decimal, area_total_m2: Decimal, area_producao_m2: Decimal, area_armazenagem_m2: Decimal, numero_funcionarios: int, endereco: str, municipio: str, provincia: str, armador_id: UUID | None=None, responsavel_tecnico_id: UUID | None=None) -> UnidadeProcessamento:
        ...
        unidade = UnidadeProcessamento.cadastrar(cnpj=cnpj, razao_social=razao_social, tipo_processamento=tipo_processamento, classificacao=classificacao, capacidade_kg_dia=capacidade_kg_dia, area_total_m2=area_total_m2, area_producao_m2=area_producao_m2, area_armazenagem_m2=area_armazenagem_m2, numero_funcionarios=numero_funcionarios, endereco=endereco, municipio=municipio, provincia=provincia)
        ...
        if self.request_service is not None:
            await self.request_service.create_request(request_type='CADASTRO_UNIDADE_PROCESSAMENTO', entity_id=saved.id, metadata={'cnpj': saved.cnpj, 'razao_social': saved.razao_social, 'municipio': saved.municipio, 'capacidade_kg_dia': str(saved.capacidade_kg_dia)}, citizen_id=responsavel_tecnico_id, numero_processo=saved.cnpj)
        return saved
    ...
    async def atualizar_capacidade(self, unidade_id: UUID, capacidade_kg_dia: Decimal) -> UnidadeProcessamento:
