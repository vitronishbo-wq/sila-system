...
from apps.backend.app.modules.resources.pescas.industrial.api.deps import (
    get_unidade_processamento_service,
)
from apps.backend.app.modules.resources.pescas.industrial.api.schemas.unidade_processamento_schema import (
    UnidadeProcessamentoCreate,
    UnidadeProcessamentoResponse,
    UnidadeProcessamentoUpdateCapacidade,
)
...
router = APIRouter(
    prefix="/unidades-processamento", tags=["Pescas Industriais - Unidades de Processamento"]
)

unidade_processamento_service_dep = Depends(get_unidade_processamento_service)


@router.post("/", response_model=UnidadeProcessamentoResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_unidade(
    data: UnidadeProcessamentoCreate,
    service: UnidadeProcessamentoService = unidade_processamento_service_dep,
) -> UnidadeProcessamentoResponse:
    try:
        return await service.cadastrar_unidade(
            ...
            capacidade_kg_dia=data.capacidade_kg_dia,
            area_total_m2=data.area_total_m2,
            area_producao_m2=data.area_producao_m2,
            area_armazenagem_m2=data.area_armazenagem_m2,
            numero_funcionarios=data.numero_funcionarios,
            endereco=data.endereco,
            municipio=data.municipio,
            provincia=data.provincia,
            armador_id=data.armador_id,
...
async def listar_unidades(
    ...
    tipo: TipoProcessamento | None = None,
    service: UnidadeProcessamentoService = unidade_processamento_service_dep,
) -> list[UnidadeProcessamentoResponse]:
    if tipo is not None:
...
async def atualizar_capacidade(
    unidade_id: UUID,
    data: UnidadeProcessamentoUpdateCapacidade,
    service: UnidadeProcessamentoService = unidade_processamento_service_dep,
) -> UnidadeProcessamentoResponse:
    try:
