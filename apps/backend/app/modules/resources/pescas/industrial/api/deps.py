...
from apps.backend.app.modules.resources.pescas.industrial.infrastructure.adapters.citizen_service_adapter import (
    CitizenServiceAdapter,
)
from apps.backend.app.modules.resources.pescas.industrial.infrastructure.adapters.industria_service_adapter import (
    IndustriaServiceAdapter,
)
from apps.backend.app.modules.resources.pescas.industrial.infrastructure.adapters.pescas_service_adapter import (
    PescasServiceAdapter,
)
from apps.backend.app.modules.resources.pescas.industrial.infrastructure.adapters.request_service_adapter import (
...
async def get_unidade_processamento_service(
    ...
    return UnidadeProcessamentoService(
        unidade_repo=unidade_repo,
        pescas_service=pescas_service,
        industria_service=industria_service,
        citizen_service=citizen_service,
        request_service=request_service,
    )


async def get_produto_processado_service(
    session: AsyncSession = db_dep,
) -> ProdutoProcessadoService:
    request_service = RequestServiceAdapter(ServiceRequestLifecycleBridge(session))
    return ProdutoProcessadoService(
...
async def get_lote_producao_service(session: AsyncSession = db_dep) -> LoteProducaoService:
    request_service = RequestServiceAdapter(ServiceRequestLifecycleBridge(session))
...
async def get_inspecao_industrial_service(
    session: AsyncSession = db_dep,
) -> InspecaoIndustrialService:
    request_service = RequestServiceAdapter(ServiceRequestLifecycleBridge(session))
    return InspecaoIndustrialService(
