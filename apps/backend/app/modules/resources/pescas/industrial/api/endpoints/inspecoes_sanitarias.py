...
from apps.backend.app.modules.resources.pescas.industrial.api.deps import (
    get_inspecao_industrial_service,
)
from apps.backend.app.modules.resources.pescas.industrial.api.schemas.inspecao_schema import (
    InspecaoCreate,
    InspecaoResponse,
    InspecaoStatusUpdate,
)
...
router = APIRouter(
    prefix="/inspecoes-sanitarias", tags=["Pescas Industriais - Inspecoes Sanitarias"]
)

inspecao_industrial_service_dep = Depends(get_inspecao_industrial_service)


@router.post("/", response_model=InspecaoResponse, status_code=status.HTTP_201_CREATED)
async def agendar_inspecao(
    data: InspecaoCreate,
    service: InspecaoIndustrialService = inspecao_industrial_service_dep,
) -> InspecaoResponse:
    try:
        return await service.agendar_inspecao(
            unidade_processamento_id=data.unidade_processamento_id,
            data_agendada=data.data_agendada,
            selo_inspecao=data.selo_inspecao,
...
async def obter_inspecao(
    inspecao_id: UUID, service: InspecaoIndustrialService = inspecao_industrial_service_dep
) -> InspecaoResponse:
    try:
...
async def listar_inspecoes(
    ...
    service: InspecaoIndustrialService = inspecao_industrial_service_dep,
) -> list[InspecaoResponse]:
    return await service.listar_inspecoes(
...
async def atualizar_status_inspecao(
    inspecao_id: UUID,
    data: InspecaoStatusUpdate,
    service: InspecaoIndustrialService = inspecao_industrial_service_dep,
) -> InspecaoResponse:
    try:
