...
class InspecaoSanitariaIndustrial:
    ...
    def concluir(
        ...
        *,
        ...
        self.inconformidades = [
            item.strip() for item in inconformidades or [] if item.strip()
        ] or None
        self.status = (
            StatusInspecao.APROVADA
            if aprovada and (not self.inconformidades)
            else StatusInspecao.REPROVADA
        )
    ...
    def atualizar_status(self, status: StatusInspecao, observacoes: str | None = None) -> None:
        ...
        if status in {
            StatusInspecao.APROVADA,
            StatusInspecao.REPROVADA,
            StatusInspecao.INTERDITADA,
        }:
