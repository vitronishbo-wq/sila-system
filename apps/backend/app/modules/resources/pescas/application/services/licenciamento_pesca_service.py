...
class LicenciamentoPescaService:
    ...
    def __init__(self, licenca_repo: LicencaPescaRepositoryPort, embarcacao_repo: EmbarcacaoRepositoryPort):
        self.licenca_repo = licenca_repo
        self.embarcacao_repo = embarcacao_repo

    async def emitir_licenca(self, *, embarcacao_id: UUID, titular_id: UUID, modalidade_autorizada: str, zona_pesca_id: UUID, validade_dias: int=365) -> LicencaPesca:
        ...
        hoje = date.today()
        item = LicencaPesca.emitir(numero_licenca=numero_licenca, embarcacao_id=embarcacao_id, titular_id=titular_id, data_emissao=hoje, data_validade=hoje + timedelta(days=validade_dias), modalidade_autorizada=modalidade_autorizada, zona_pesca_id=zona_pesca_id)
