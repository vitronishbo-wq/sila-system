...
class PescadorService:
    ...
    def __init__(self, pescador_repo: PescadorRepositoryPort, citizen_service: CitizenServicePort, request_service: RequestServicePort):
        self.pescador_repo = pescador_repo
        self.citizen_service = citizen_service
        self.request_service = request_service

    async def cadastrar_pescador(self, *, nome: str, tipo: TipoPescador, citizen_id, numero_registro: str | None=None) -> Pescador:
        ...
        item = Pescador.cadastrar(nome=nome, numero_registro=registro, tipo=tipo, citizen_id=citizen_id)
        saved = await self.pescador_repo.save(item)
        await self.request_service.create_request(request_type='CADASTRO_PESCADOR', entity_id=saved.id, citizen_id=citizen_id, numero_processo=registro, metadata={'tipo': tipo.value, 'nome': nome})
        return saved
