...
class CreditoRural:
    ...
    @classmethod
    def solicitar(cls, *, codigo_produtor: str, finalidade: str, valor_solicitado: float) -> 'CreditoRural':
        if valor_solicitado <= 0:
            raise ValueError('Valor solicitado deve ser maior que zero')
        return cls(id=uuid4(), codigo_credito='', codigo_produtor=codigo_produtor, finalidade=finalidade, valor_solicitado=round(valor_solicitado, 2), valor_aprovado=None, status=StatusCredito.SOLICITADO, data_solicitacao=date.today())
