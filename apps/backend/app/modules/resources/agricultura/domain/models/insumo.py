...
class Insumo:
    ...
    @classmethod
    def criar(cls, *, nome: str, tipo: TipoInsumo, unidade_medida: str, quantidade_inicial: float, custo_unitario: float) -> 'Insumo':
        if quantidade_inicial < 0:
            raise ValueError('Quantidade inicial nao pode ser negativa')
        if custo_unitario < 0:
            raise ValueError('Custo unitario nao pode ser negativo')
        return cls(id=uuid4(), codigo_insumo='', nome=nome, tipo=tipo, unidade_medida=unidade_medida, quantidade_estoque=round(quantidade_inicial, 3), custo_unitario=round(custo_unitario, 2), data_registro=date.today(), ativo=True)
