...
def test_endpoint_agendar_assistencia_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_assistencia="AST/2026/000001",
        codigo_propriedade="PROP/2026/000001",
        tecnico_nome="Eng. Antonio",
        objetivo="Avaliacao de solo",
        status="agendada",
        data_agendamento="2026-02-28",
...
def test_endpoint_obter_assistencia_retorna_404():
    service = SimpleNamespace(
        obter=AsyncMock(side_effect=AssistenciaNotFoundError("Assistencia tecnica nao encontrada"))
    )
    app = FastAPI()
    app.include_router(assistencia_router, prefix="/agricultura")
    app.dependency_overrides[get_assistencia_service] = lambda: service
