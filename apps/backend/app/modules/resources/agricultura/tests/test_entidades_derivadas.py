...
def test_plantio_fluxo_planejar_executar_cancelar():
    plantio = Plantio.planejar(codigo_safra='SAF/2026/000001', codigo_talhao='TAL/2026/000001', area_plantada_ha=5.0, quantidade_semente=120.0)
    ...
    try:
        ...
        assert False, 'cancelamento de plantio executado deveria falhar'
    except ValueError as exc:
        assert 'executado' in str(exc)

def test_colheita_calcula_quantidade_liquida():
    colheita = Colheita.registrar(codigo_safra='SAF/2026/000001', codigo_talhao='TAL/2026/000001', quantidade_colhida_ton=18.75, perdas_ton=1.25, umidade_percentual=13.2)
    assert colheita.quantidade_liquida_ton == 17.5

def test_colheita_invalida_quando_perda_excede_quantidade():
    try:
        Colheita.registrar(codigo_safra='SAF/2026/000001', codigo_talhao='TAL/2026/000001', quantidade_colhida_ton=2.0, perdas_ton=2.5)
        assert False, 'perda maior que quantidade deveria falhar'
    except ValueError as exc:
        assert 'Perdas' in str(exc)

def test_equipamento_fluxo_uso_manutencao_inativacao():
    equipamento = Equipamento.cadastrar(nome='Trator 110cv', tipo=TipoEquipamento.TRATOR, fabricante='AgroTech', modelo='AT-110', ano_fabricacao=2023)
...
def test_equipamento_ano_fabricacao_invalido():
    try:
        Equipamento.cadastrar(nome='Pulverizador XP', tipo=TipoEquipamento.PULVERIZADOR, ano_fabricacao=1940)
        assert False, 'ano invalido deveria falhar'
    except ValueError as exc:
