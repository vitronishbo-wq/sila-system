...
async def test_certificacao_service_fluxo_solicitar_aprovar():
    ...
    prop = await propriedade_service.cadastrar(
        ...
        area_total_ha=120,
        area_cultivavel_ha=95,
    )
    service = CertificacaoService(propriedade_service=propriedade_service)
    cert = await service.solicitar(
        codigo_propriedade=prop.codigo_propriedade, tipo="boas_praticas", orgao_emissor="MINAGRIP"
    )
...
def test_endpoint_solicitar_certificacao_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_certificacao="CER/2026/000001",
        codigo_propriedade="PROP/2026/000001",
        tipo="boas_praticas",
        orgao_emissor="MINAGRIP",
        status="solicitada",
        data_solicitacao="2026-02-28",
        data_emissao=None,
        data_validade=None,
...
def test_endpoint_obter_certificacao_retorna_404():
    service = SimpleNamespace(
        obter=AsyncMock(side_effect=CertificacaoNotFoundError("Certificacao nao encontrada"))
    )
    app = FastAPI()
    app.include_router(certificacoes_router, prefix="/agricultura")
    app.dependency_overrides[get_certificacao_service] = lambda: service
