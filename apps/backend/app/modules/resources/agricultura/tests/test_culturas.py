...
async def test_producao_service_cadastra_cultura():
    ...
    item = await service.cadastrar_cultura(
        nome="Milho", tipo=TipoCultura.GRAOS, ciclo_dias=120, produtividade_estimada_ton_ha=6.2
    )
...
def test_endpoint_cadastrar_cultura_retorna_201():
    mock_item = SimpleNamespace(
        ...
        nome="Milho",
        tipo=TipoCultura.GRAOS,
        ciclo_dias=120,
        produtividade_estimada_ton_ha=6.2,
        data_registro="2026-02-28",
        ativa=True,
    )
...
def test_endpoint_obter_cultura_retorna_404():
    service = SimpleNamespace(
        obter_cultura=AsyncMock(side_effect=CulturaNotFoundError("Cultura nao encontrada"))
    )
    app = FastAPI()
    app.include_router(culturas_router, prefix="/agricultura")
    app.dependency_overrides[get_producao_service] = lambda: service
