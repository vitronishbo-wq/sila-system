...
async def test_operacao_service_registrar_com_consumo_insumo():
    ...
    prop = await propriedade_service.cadastrar(
        ...
        area_total_ha=90,
        area_cultivavel_ha=70,
    )
    cultura = await producao_service.cadastrar_cultura(
        nome="Feijao", tipo=TipoCultura.GRAOS, ciclo_dias=110, produtividade_estimada_ton_ha=2.1
    )
...
def test_endpoint_registrar_operacao_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_operacao="OPE/2026/000001",
        codigo_safra="SAF/2026/000001",
        tipo="plantio",
        descricao="Plantio inicial",
        data_operacao="2026-02-28T00:00:00",
        codigo_insumo="INS/2026/000001",
        quantidade_insumo=12.0,
    )
    ...
    response = client.post(
        "/agricultura/operacoes/",
        json={
            "codigo_safra": "SAF/2026/000001",
            "tipo": "plantio",
            "descricao": "Plantio inicial",
            "codigo_insumo": "INS/2026/000001",
            "quantidade_insumo": 12,
        },
    )
...
def test_endpoint_obter_operacao_retorna_404():
    service = SimpleNamespace(
        obter=AsyncMock(side_effect=OperacaoNotFoundError("Operacao nao encontrada"))
    )
    app = FastAPI()
    app.include_router(operacoes_router, prefix="/agricultura")
    app.dependency_overrides[get_operacao_service] = lambda: service
