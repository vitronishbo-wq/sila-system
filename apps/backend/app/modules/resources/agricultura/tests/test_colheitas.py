...
async def test_colheita_service_registrar():
    ...
    prop = await propriedade_service.cadastrar(
        produtor_id=uuid4(),
        nome="Fazenda Colheita",
        tipo=TipoPropriedade.PROPRIO,
        area_total_ha=70,
        area_cultivavel_ha=52,
    )
    talhao = await talhao_service.cadastrar(
        codigo_propriedade=prop.codigo_propriedade, nome="Talhao C", area_ha=14
    )
    cultura = await producao_service.cadastrar_cultura(
        nome="Milho", tipo=TipoCultura.GRAOS, ciclo_dias=110, produtividade_estimada_ton_ha=3.2
    )
...
def test_endpoint_registrar_colheita_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_colheita="COL/2026/000001",
        codigo_safra="SAF/2026/000001",
        codigo_talhao="TAL/2026/000001",
        quantidade_colhida_ton=18.0,
        perdas_ton=1.5,
        quantidade_liquida_ton=16.5,
        data_colheita="2026-02-28",
...
def test_endpoint_obter_colheita_retorna_404():
    service = SimpleNamespace(
        obter=AsyncMock(side_effect=ColheitaNotFoundError("Colheita nao encontrada"))
    )
    app = FastAPI()
    app.include_router(colheitas_router, prefix="/agricultura")
    app.dependency_overrides[get_colheita_service] = lambda: service
