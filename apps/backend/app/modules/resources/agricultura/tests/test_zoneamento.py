...
async def test_zoneamento_service_fluxo_completo_cadastro_ambiental():
    ...
    prop = await propriedade_service.cadastrar(
        ...
        area_total_ha=70,
        area_cultivavel_ha=48,
    )
    service = ZoneamentoService(propriedade_service=propriedade_service)
    zoneamento = await service.registrar_zoneamento(
        codigo_propriedade=prop.codigo_propriedade,
        zona=TipoZonaAgricola.USO_MISTO,
        aptidao_solo=AptidaoSolo.MEDIA,
        area_zoneada_ha=45,
        culturas_recomendadas=["milho", "feijao"],
    )
    assert zoneamento.status == StatusZoneamento.ATIVO
    cadastro = await service.registrar_cadastro_ambiental(
        codigo_zoneamento=zoneamento.codigo_zoneamento,
        reserva_legal_percentual=20,
        app_percentual=12,
        area_protecao_ha=9,
    )
    assert cadastro.status == StatusCadastroAmbiental.PENDENTE
    cadastro = await service.registrar_pendencia(
        cadastro.codigo_cadastro_ambiental, pendencia="Falta memorial descritivo"
    )
    assert cadastro.status == StatusCadastroAmbiental.COM_PENDENCIA
    with pytest.raises(ValueError):
        await service.validar_cadastro(
            cadastro.codigo_cadastro_ambiental, numero_processo="PROC-123"
        )
    cadastro = await service.sanar_pendencias(cadastro.codigo_cadastro_ambiental)
    assert cadastro.status == StatusCadastroAmbiental.PENDENTE
    cadastro = await service.validar_cadastro(
        cadastro.codigo_cadastro_ambiental, numero_processo="PROC-123"
    )
...
def test_endpoint_registrar_zoneamento_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_zoneamento="ZON/2026/000001",
        codigo_propriedade="PROP/2026/000001",
        zona=TipoZonaAgricola.USO_MISTO,
        aptidao_solo=AptidaoSolo.MEDIA,
        area_zoneada_ha=45.0,
        status=StatusZoneamento.ATIVO,
        data_zoneamento=date(2026, 2, 28),
        culturas_recomendadas=["milho", "feijao"],
        restricoes=[],
        validade_ate=None,
        observacoes=None,
...
def test_endpoint_obter_zoneamento_retorna_404():
    service = SimpleNamespace(
        obter_zoneamento=AsyncMock(side_effect=ZoneamentoNotFoundError("Zoneamento nao encontrado"))
    )
    app = FastAPI()
    app.include_router(zoneamento_router, prefix="/agricultura")
    app.dependency_overrides[get_zoneamento_service] = lambda: service
...
def test_endpoint_registrar_cadastro_ambiental_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_cadastro_ambiental="CAR/2026/000001",
        codigo_zoneamento="ZON/2026/000001",
        codigo_propriedade="PROP/2026/000001",
        reserva_legal_percentual=20.0,
        app_percentual=12.0,
        area_protecao_ha=9.0,
        status=StatusCadastroAmbiental.PENDENTE,
        data_registro=date(2026, 2, 28),
    ...
    client = TestClient(app)
    response = client.post(
        "/agricultura/zoneamento/ZON/2026/000001/cadastro-ambiental",
        json={"reserva_legal_percentual": 20, "app_percentual": 12, "area_protecao_ha": 9},
    )
    assert response.status_code == 201
