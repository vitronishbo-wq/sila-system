...
async def test_comercializacao_service_registrar_venda():
    ...
    prop = await propriedade_service.cadastrar(
        ...
        area_total_ha=80,
        area_cultivavel_ha=65,
    )
    cultura = await producao_service.cadastrar_cultura(
        nome="Milho", tipo=TipoCultura.GRAOS, ciclo_dias=120, produtividade_estimada_ton_ha=3
    )
    ...
    venda = await service.registrar_venda(
...
def test_endpoint_registrar_venda_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_comercializacao="COM/2026/000001",
...
def test_endpoint_obter_venda_retorna_404():
    service = SimpleNamespace(
        obter=AsyncMock(side_effect=ComercializacaoNotFoundError("Comercializacao nao encontrada"))
    )
    app = FastAPI()
    app.include_router(comercializacao_router, prefix="/agricultura")
    app.dependency_overrides[get_comercializacao_service] = lambda: service
