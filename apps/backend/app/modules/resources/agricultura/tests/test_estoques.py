...
async def test_estoque_service_criar_e_listar_baixo():
    ...
    insumo = await insumo_service.cadastrar(nome='NPK', tipo=TipoInsumo.FERTILIZANTE, unidade_medida='kg', quantidade_inicial=50, custo_unitario=15)
...
def test_endpoint_criar_controle_estoque_retorna_201():
    mock_item = SimpleNamespace(id=uuid4(), codigo_estoque='EST/2026/000001', codigo_insumo='INS/2026/000001', quantidade_atual=40.0, quantidade_minima=60.0, status='baixo', data_atualizacao='2026-02-28T00:00:00')
    ...
    response = client.post('/agricultura/estoques/', json={'codigo_insumo': 'INS/2026/000001', 'quantidade_minima': 60})
    assert response.status_code == 201
    assert response.json()['codigo_estoque'] == 'EST/2026/000001'

def test_endpoint_obter_estoque_retorna_404():
    service = SimpleNamespace(obter=AsyncMock(side_effect=EstoqueNotFoundError('Controle de estoque nao encontrado')))
    app = FastAPI()
    app.include_router(estoques_router, prefix='/agricultura')
    app.dependency_overrides[get_estoque_service] = lambda: service
    client = TestClient(app)
