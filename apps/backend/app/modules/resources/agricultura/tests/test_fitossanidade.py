...
def test_endpoint_registrar_ocorrencia_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_ocorrencia="FIT/2026/000001",
        codigo_propriedade="PROP/2026/000001",
        praga_doenca="Lagarta",
        descricao="Ataque no talhao 2",
        severidade="alta",
        status="aberta",
        data_registro="2026-02-28T00:00:00",
        cultura_afetada=None,
...
def test_endpoint_obter_ocorrencia_retorna_404():
    service = SimpleNamespace(
        obter=AsyncMock(
            side_effect=OcorrenciaNotFoundError("Ocorrencia fitossanitaria nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(fitossanidade_router, prefix="/agricultura")
