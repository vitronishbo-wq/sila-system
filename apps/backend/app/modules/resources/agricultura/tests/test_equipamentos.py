...
async def test_equipamento_service_fluxo_uso():
    ...
    equipamento = await service.cadastrar(
        nome="Trator XP", tipo=TipoEquipamento.TRATOR, ano_fabricacao=2024
    )
...
def test_endpoint_cadastrar_equipamento_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_equipamento="EQP/2026/000001",
        nome="Trator XP",
        tipo="trator",
        fabricante="AgroTech",
        modelo="XP-90",
...
def test_endpoint_obter_equipamento_retorna_404():
    service = SimpleNamespace(
        obter=AsyncMock(side_effect=EquipamentoNotFoundError("Equipamento nao encontrado"))
    )
    app = FastAPI()
    app.include_router(equipamentos_router, prefix="/agricultura")
    app.dependency_overrides[get_equipamento_service] = lambda: service
