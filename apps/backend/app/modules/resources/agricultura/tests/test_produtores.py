...
async def test_cadastrar_produtor_sucesso():
    ...
    repo = SimpleNamespace(
        get_by_documento=AsyncMock(return_value=None),
        next_cadastro=AsyncMock(return_value="AGR/2026/000001"),
    ...
    result = await service.cadastrar_produtor(
        nome="Joao Silva",
        documento="123456789LA",
        documento_tipo="BI",
        tipo=TipoProdutor.FAMILIAR,
...
async def test_cadastrar_produtor_documento_duplicado():
    existente = Produtor.criar(
        nome="Maria", documento="DUP", documento_tipo="BI", tipo=TipoProdutor.PEQUENO
    )
    existente.status = StatusProdutor.ATIVO
    repo = SimpleNamespace(
        get_by_documento=AsyncMock(return_value=existente),
        next_cadastro=AsyncMock(),
        save=AsyncMock(),
    )
    service = ProdutorService(repo)
    with pytest.raises(ProdutorAlreadyExistsError):
        await service.cadastrar_produtor(
            nome="Outra", documento="DUP", documento_tipo="BI", tipo=TipoProdutor.PEQUENO
        )
...
async def test_cadastrar_produtor_cidadao_inativo():
    repo = SimpleNamespace(
        get_by_documento=AsyncMock(return_value=None),
        next_cadastro=AsyncMock(return_value="AGR/2026/000010"),
        save=AsyncMock(),
    )
...
def test_endpoint_cadastrar_produtor_retorna_201():
    ...
    response = client.post(
        ...
        json={
            ...
            "documento_tipo": "BI",
            "tipo": "cooperativa",
            "citizen_id": str(produtor.citizen_id),
        },
    )
