...
class ZoneamentoService:
    ...
    async def registrar_zoneamento(
        ...
        *,
        ...
        item = Zoneamento.registrar(
            codigo_propriedade=codigo_propriedade,
            zona=zona,
            aptidao_solo=aptidao_solo,
            area_zoneada_ha=area_zoneada_ha,
            culturas_recomendadas=culturas_recomendadas,
    ...
    async def registrar_cadastro_ambiental(
        ...
        *,
        ...
        existente = next(
            (
                item
                for item in self._cadastros.values()
                if item.codigo_zoneamento == codigo_zoneamento
            ),
            None,
        )
        if existente:
            raise ValueError("Ja existe cadastro ambiental para este zoneamento")
        cadastro = CadastroAmbiental.registrar(
            codigo_zoneamento=codigo_zoneamento,
            codigo_propriedade=zoneamento.codigo_propriedade,
            reserva_legal_percentual=reserva_legal_percentual,
            app_percentual=app_percentual,
            area_protecao_ha=area_protecao_ha,
    ...
    async def validar_cadastro(
        self, codigo_cadastro_ambiental: str, *, numero_processo: str
    ) -> CadastroAmbiental:
    ...
    async def registrar_pendencia(
        self, codigo_cadastro_ambiental: str, *, pendencia: str
    ) -> CadastroAmbiental:
        item = self._cadastros.get(codigo_cadastro_ambiental)
        if not item:
            raise CadastroAmbientalNotFoundError("Cadastro ambiental nao encontrado")
