...
class TalhaoService:
    ...
    async def cadastrar(self, *, codigo_propriedade: str, nome: str, area_ha: float, tipo_solo: str | None=None, irrigado: bool=False) -> Talhao:
        await self._propriedade_service.obter(codigo_propriedade)
        item = Talhao.criar(codigo_propriedade=codigo_propriedade, nome=nome, area_ha=area_ha, tipo_solo=tipo_solo, irrigado=irrigado)
    ...
    async def listar(self, *, codigo_propriedade: str | None=None, status: StatusTalhao | None=None) -> list[Talhao]:
