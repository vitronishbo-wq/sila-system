...
from apps.backend.app.modules.resources.agricultura.application.ports import (
    CitizenServicePort,
...
class ProdutorService:
    def __init__(
        self,
        produtor_repo: ProdutorRepositoryPort,
    ...
    async def cadastrar_produtor(
        ...
        *,
        ...
        if existente and existente.status in {
            StatusProdutor.PENDENTE,
            StatusProdutor.ATIVO,
            StatusProdutor.SUSPENSO,
        }:
        ...
        if self.request_service and citizen_id:
            await self.request_service.create_request(
                request_type="cadastro_produtor",
                entity_id=saved.id,
                citizen_id=citizen_id,
                numero_processo=saved.cadastro_produtor,
                metadata={"tipo": saved.tipo.value, "documento": saved.documento},
            )
        return saved

    async def ativar_produtor(
        self, cadastro_produtor: str, actor_id: UUID | None = None
    ) -> Produtor:
        ...
        if self.request_service and actor_id:
            await self.request_service.complete_request(
                entity_id=saved.id, actor_id=actor_id, metadata={"status": saved.status.value}
            )
        return saved
    ...
    async def listar(self, status: StatusProdutor | None = None) -> list[Produtor]:
