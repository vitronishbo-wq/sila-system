...
class AssistenciaService:
    ...
    async def agendar(self, *, codigo_propriedade: str, tecnico_nome: str, objetivo: str) -> AssistenciaTecnica:
        await self._propriedade_service.obter(codigo_propriedade)
        item = AssistenciaTecnica.agendar(codigo_propriedade=codigo_propriedade, tecnico_nome=tecnico_nome, objetivo=objetivo)
        item.codigo_assistencia = self._next_codigo()
        self._items[item.codigo_assistencia] = item
        return item

    async def concluir(self, codigo_assistencia: str, *, recomendacoes: str | None=None) -> AssistenciaTecnica:
