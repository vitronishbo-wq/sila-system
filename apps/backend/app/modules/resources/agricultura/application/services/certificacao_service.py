...
class CertificacaoService:
    ...
    async def solicitar(
        self, *, codigo_propriedade: str, tipo: str, orgao_emissor: str
    ) -> Certificacao:
        await self._propriedade_service.obter(codigo_propriedade)
        item = Certificacao.solicitar(
            codigo_propriedade=codigo_propriedade, tipo=tipo, orgao_emissor=orgao_emissor
        )
    ...
    async def aprovar(
        self, codigo_certificacao: str, *, data_validade: date | None = None
    ) -> Certificacao:
