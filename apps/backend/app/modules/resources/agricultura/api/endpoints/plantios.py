...
async def planejar_plantio(data: PlantioCreate, service: PlantioService=Depends(get_plantio_service)):
    try:
        return await service.planejar(codigo_safra=data.codigo_safra, codigo_talhao=data.codigo_talhao, area_plantada_ha=data.area_plantada_ha, quantidade_semente=data.quantidade_semente)
...
async def executar_plantio(codigo_plantio: str, service: PlantioService=Depends(get_plantio_service)):
    try:
...
async def cancelar_plantio(codigo_plantio: str, data: PlantioCancelamentoInput, service: PlantioService=Depends(get_plantio_service)):
    try:
...
async def obter_plantio(codigo_plantio: str, service: PlantioService=Depends(get_plantio_service)):
    try:
...
async def listar_plantios(codigo_safra: str | None=None, codigo_talhao: str | None=None, status_plantio: StatusPlantio | None=None, service: PlantioService=Depends(get_plantio_service)):
    return await service.listar(codigo_safra=codigo_safra, codigo_talhao=codigo_talhao, status=status_plantio)