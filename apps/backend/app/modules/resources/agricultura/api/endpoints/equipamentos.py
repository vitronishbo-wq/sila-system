...
equipamento_service_dep = Depends(get_equipamento_service)


@router.post("/", response_model=EquipamentoResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_equipamento(
    data: EquipamentoCreate, service: EquipamentoService = equipamento_service_dep
):
    try:
...
async def iniciar_uso(
    codigo_equipamento: str, service: EquipamentoService = equipamento_service_dep
):
    try:
...
async def registrar_uso(
    ...
    data: UsoEquipamentoInput,
    service: EquipamentoService = equipamento_service_dep,
):
    try:
...
async def inativar_equipamento(
    codigo_equipamento: str, service: EquipamentoService = equipamento_service_dep
):
    try:
...
async def listar_equipamentos(
    status_equipamento: StatusEquipamento | None = None,
    tipo_equipamento: TipoEquipamento | None = None,
    service: EquipamentoService = equipamento_service_dep,
):
