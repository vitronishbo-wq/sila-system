...
async def registrar_venda(data: ComercializacaoCreate, service: ComercializacaoService=Depends(get_comercializacao_service)):
    try:
        return await service.registrar_venda(codigo_safra=data.codigo_safra, comprador=data.comprador, quantidade_ton=data.quantidade_ton, preco_unitario=data.preco_unitario)
...
async def obter_venda(codigo_comercializacao: str, service: ComercializacaoService=Depends(get_comercializacao_service)):
    try:
...
async def listar_vendas(codigo_safra: str | None=None, service: ComercializacaoService=Depends(get_comercializacao_service)):
