...
from apps.backend.app.modules.resources.agricultura.exceptions import (
    AssistenciaNotFoundError,
    PropriedadeNotFoundError,
)

router = APIRouter(prefix="/assistencia", tags=["Agricultura - assistencia"])
assistencia_service_dep = Depends(get_assistencia_service)


@router.post("/", response_model=AssistenciaResponse, status_code=status.HTTP_201_CREATED)
async def agendar_assistencia(
    data: AssistenciaCreate, service: AssistenciaService = assistencia_service_dep
):
    try:
        return await service.agendar(
...
async def concluir_assistencia(
    codigo_assistencia: str,
    data: AssistenciaConclusaoInput,
    service: AssistenciaService = assistencia_service_dep,
):
    try:
...
async def cancelar_assistencia(
    codigo_assistencia: str,
    data: AssistenciaCancelamentoInput,
    service: AssistenciaService = assistencia_service_dep,
):
    try:
...
async def obter_assistencia(
    codigo_assistencia: str, service: AssistenciaService = assistencia_service_dep
):
    try:
