...
async def cadastrar_cultura(
    data: CulturaCreate, service: ProducaoService = producao_service_dep
):
    try:
        return await service.cadastrar_cultura(
            nome=data.nome,
            tipo=data.tipo,
            ciclo_dias=data.ciclo_dias,
            produtividade_estimada_ton_ha=data.produtividade_estimada_ton_ha,
        )
...
async def obter_cultura(
    codigo_cultura: str, service: ProducaoService = producao_service_dep
):
    try:
...
async def listar_culturas(service: ProducaoService = producao_service_dep):
