...
propriedade_service_dep = Depends(get_propriedade_service)


@router.post("/", response_model=PropriedadeResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_propriedade(
    data: PropriedadeCreate, service: PropriedadeService = propriedade_service_dep
):
    try:
        return await service.cadastrar(
            ...
            area_cultivavel_ha=data.area_cultivavel_ha,
...
async def obter_propriedade(
    codigo_propriedade: str, service: PropriedadeService = propriedade_service_dep
):
    try:
...
async def listar_propriedades(
    produtor_id: UUID | None = None, service: PropriedadeService = propriedade_service_dep
):
