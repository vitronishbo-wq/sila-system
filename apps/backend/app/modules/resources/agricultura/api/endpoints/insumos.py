...
insumo_service_dep = Depends(get_insumo_service)


@router.post("/", response_model=InsumoResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_insumo(
    data: InsumoCreate, service: InsumoService = insumo_service_dep
):
    try:
...
async def registrar_entrada(
    codigo_insumo: str,
    data: MovimentoInsumoInput,
    service: InsumoService = insumo_service_dep,
):
    try:
...
async def obter_insumo(codigo_insumo: str, service: InsumoService = insumo_service_dep):
    try:
...
async def listar_insumos(service: InsumoService = insumo_service_dep):
