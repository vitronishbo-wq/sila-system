...
class Tender:
    ...
    def close(self) -> Dict[str, Any]:
        ...
        return {'entity_type': 'TENDER', 'entity_id': self.id, 'action': 'TENDER_CLOSED', 'previous_state': {'status': old_status.value}, 'new_state': {'status': self.status.value}, 'timestamp': self.updated_at}
    ...
    def award(self, supplier_id: str) -> Dict[str, Any]:
        ...
        self.updated_at = datetime.utcnow()
        return {'entity_type': 'TENDER', 'entity_id': self.id, 'action': 'TENDER_AWARDED', 'previous_state': {'status': old_status.value, 'awarded_to': old_awarded}, 'new_state': {'status': self.status.value, 'awarded_to': supplier_id}, 'timestamp': self.updated_at}