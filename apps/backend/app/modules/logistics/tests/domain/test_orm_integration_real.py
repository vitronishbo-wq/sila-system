...
async def _assert_table_exists(db_session: AsyncSession, table_name: str) -> None:
    ...
    assert result.scalar_one_or_none() is not None, (
        f"Tabela fisica ausente: {table_name}. Execute migration 20260304_039_transportes_logistica_linhas_bilhetagem."
    )


@pytest.mark.asyncio
@pytest.mark.integration
async def test_repositories_transportes_logistica_roundtrip_orm_real(
    ...
    veiculo = await veiculo_repo.save(
        Veiculo.cadastrar(
            placa="LD-20-30-AA",
    ...
    linha = Linha.criar(
        codigo=await linha_repo.next_codigo(),
        nome="Linha ORM Real",
        modal=ModalTransporte.RODOVIARIO,
        tipo_viagem=TipoViagem.URBANA,
        origem="Luanda",
        destino="Cacuaco",
        itinerario=[{"ordem": 1, "ponto": "Mutamba"}],
        extensao_km=Decimal("24.00"),
        tempo_estimado_minutos=65,
        dias_operacao=["seg", "ter", "qua"],
    ...
    viagem = await viagem_repo.save(
        Viagem.programar(
            linha_id=linha.id,
            veiculo_id=veiculo.id,
            motorista_id=uuid4(),
            data_hora_saida=saida,
            data_hora_chegada_prevista=chegada_prevista,
            origem="Luanda",
            destino="Bengo",
            itinerario=[{"ordem": 1, "ponto": "Cacuaco"}],
        )
    )
    viagem.iniciar()
    ...
    evento_bilhetagem = await bilhetagem_repo.save(
        BilhetagemEletronica.registrar_evento(
            codigo_bilhete="ORM-TICKET-001",
            viagem_id=viagem.id,
    ...
    frota = Frota.criar(
        codigo_frota=await frota_repo.next_codigo(),
        nome="Frota ORM Real",
        operadora_id=uuid4(),
        municipio="Luanda",
        provincia="Luanda",
    ...
    frota.registrar_manutencao(
        Manutencao.registrar(
            veiculo_id=uuid4(), tipo="preventiva", oficina="Oficina ORM", custo=Decimal("100000.00")
        )
    )
    frota.atualizar_tarifa(
        Tarifa.definir(tipo=TipoTarifa.PUBLICA, valor=Decimal("250.00"), motivo="Atualizacao anual")
    )
    frota.registrar_fiscalizacao(
        FiscalizacaoTransporte.registrar(
            fiscal_id=uuid4(), conformidade=True, apontamentos="Frota regular"
        )
    )
