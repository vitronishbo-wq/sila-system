...
async def test_operacao_analytics_calcula_demanda_e_qualidade():
    ...
    viagem_1 = Viagem.programar(
        linha_id=uuid4(),
        veiculo_id=uuid4(),
        motorista_id=uuid4(),
        data_hora_saida=base,
        data_hora_chegada_prevista=base + timedelta(minutes=40),
        origem="Centro",
        destino="Viana",
    )
    ...
    viagem_2 = Viagem.programar(
        linha_id=viagem_1.linha_id,
        veiculo_id=uuid4(),
        motorista_id=uuid4(),
        data_hora_saida=base + timedelta(hours=1),
        data_hora_chegada_prevista=base + timedelta(hours=1, minutes=45),
        origem="Centro",
        destino="Viana",
    )
    ...
    viagem_3 = Viagem.programar(
        linha_id=viagem_1.linha_id,
        veiculo_id=uuid4(),
        motorista_id=uuid4(),
        data_hora_saida=base + timedelta(hours=2),
        data_hora_chegada_prevista=base + timedelta(hours=2, minutes=35),
        origem="Centro",
        destino="Viana",
    )
    ...
    evento_1 = BilhetagemEletronica.registrar_evento(
        codigo_bilhete="BILH-A",
        viagem_id=viagem_1.id,
        tipo_tarifa=TipoTarifa.PUBLICA,
        valor_pago=Decimal("250.00"),
        forma_pagamento="cartao",
    )
    evento_2 = BilhetagemEletronica.registrar_evento(
        codigo_bilhete="BILH-B",
        viagem_id=viagem_3.id,
        tipo_tarifa=TipoTarifa.PUBLICA,
        valor_pago=Decimal("300.00"),
        forma_pagamento="qrcode",
    )
    await bilhetagem_repo.save(evento_1)
    await bilhetagem_repo.save(evento_2)
    demanda = await service.calcular_demanda(
        linha_id=viagem_1.linha_id,
        data_inicio=base - timedelta(minutes=5),
    ...
    qualidade = await service.calcular_qualidade(
        linha_id=viagem_1.linha_id,
        data_inicio=base - timedelta(minutes=5),
        data_fim=base + timedelta(hours=3),
    )
...
def test_endpoint_analytics_demanda_retorna_200():
    demanda = SimpleNamespace(
        viagens_total=5,
        passageiros_total=650,
        passageiros_media_por_viagem=Decimal("130.00"),
        arrecadacao_total=Decimal("1500.00"),
    )
