...
async def test_linha_service_fluxo_completo():
    ...
    workflow_adapter = SimpleNamespace(iniciar_fluxo=AsyncMock(return_value='WF-TRN-LIN-001'), registrar_evento=AsyncMock(return_value=None))
    service = LinhaService(linha_repo=SQLAlchemyLinhaRepository(), veiculo_repo=SQLAlchemyVeiculoRepository(), geosampa_adapter=geosampa_adapter, urbanismo_adapter=urbanismo_adapter, obras_publicas_adapter=obras_adapter, workflow_adapter=workflow_adapter)
    veiculo = await service.cadastrar_veiculo(placa='LD-10-20-AA', tipo=TipoVeiculo.ONIBUS, marca='Volvo', modelo='B340', ano_fabricacao=2022, ano_modelo=2023, proprietario_id=uuid4(), proprietario_tipo='cnpj', data_aquisicao=date(2025, 1, 15), capacidade_passageiros=70, operadora_id=uuid4())
    linha = await service.criar_linha(nome='Linha Centro', modal=ModalTransporte.RODOVIARIO, tipo_viagem=TipoViagem.URBANA, origem='Centro', destino='Viana', itinerario=[{'ponto': 'Mutamba'}, {'ponto': 'Cazenga'}], extensao_km=Decimal('18.50'), tempo_estimado_minutos=60, dias_operacao=['seg', 'ter', 'qua', 'qui', 'sex'], horario_inicio='05:30', horario_fim='22:00', tarifa_base=Decimal('250.00'), operadora_id=uuid4(), codigo_corredor='CRD-NORTE-01')
    ...
    linha = await service.registrar_indicadores(linha.codigo, demanda_media_diaria=1200, ocupacao_media=Decimal('73.25'), regularidade=Decimal('96.00'), pontualidade=Decimal('93.40'))
...
def test_endpoint_criar_linha_retorna_201():
    linha = Linha.criar(codigo='LIN/2026/000001', nome='Linha Centro', modal=ModalTransporte.RODOVIARIO, tipo_viagem=TipoViagem.URBANA, origem='Centro', destino='Viana', itinerario=[{'ponto': 'Mutamba'}], extensao_km=Decimal('12.00'), tempo_estimado_minutos=45, dias_operacao=['seg', 'ter'], horario_inicio='05:30', horario_fim='22:00', tarifa_base=Decimal('250.00'), operadora_id=uuid4())
    ...
    response = client.post('/transportes-logistica/linhas/', json={'nome': 'Linha Centro', 'modal': 'rodoviario', 'tipo_viagem': 'urbana', 'origem': 'Centro', 'destino': 'Viana', 'itinerario': [{'ponto': 'Mutamba'}], 'extensao_km': '12.00', 'tempo_estimado_minutos': 45, 'dias_operacao': ['seg', 'ter'], 'horario_inicio': '05:30', 'horario_fim': '22:00', 'tarifa_base': '250.00', 'operadora_id': str(uuid4())})
