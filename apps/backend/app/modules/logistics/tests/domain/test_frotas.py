...
async def test_frota_service_fluxo_completo():
    workflow_adapter = SimpleNamespace(
        iniciar_fluxo=AsyncMock(return_value="WF-TRN-001"),
        registrar_evento=AsyncMock(return_value=None),
    )
    service_requests_adapter = SimpleNamespace(
        abrir_solicitacao=AsyncMock(return_value="SRQ-TRN-001")
    )
    financas_adapter = SimpleNamespace(
        validar_tarifa=AsyncMock(return_value=True),
        registrar_despesa_manutencao=AsyncMock(return_value=True),
    )
    seguranca_adapter = SimpleNamespace(validar_regularidade_veiculo=AsyncMock(return_value=True))
    service = FrotaService(
        frota_repo=SQLAlchemyFrotaRepository(),
        workflow_adapter=workflow_adapter,
        service_requests_adapter=service_requests_adapter,
        financas_adapter=financas_adapter,
        seguranca_publica_adapter=seguranca_adapter,
    )
    frota = await service.criar_frota(
        nome="Frota Metropolitana Norte",
        operadora_id=uuid4(),
        municipio="Luanda",
        provincia="Luanda",
    )
    ...
    frota = await service.adicionar_veiculo(
        frota.codigo_frota, veiculo_id=uuid4(), placa="LD-45-67-AB", tipo="onibus", capacidade=80
    )
    assert len(frota.veiculos) == 1
    frota = await service.registrar_manutencao(
        frota.codigo_frota,
        veiculo_id=uuid4(),
        tipo="preventiva",
    ...
    frota = await service.atualizar_tarifa(
        frota.codigo_frota,
        tipo_tarifa=TipoTarifa.PUBLICA,
        valor=Decimal("250.00"),
        motivo="Recomposicao de custos operacionais",
    )
    assert len(frota.tarifas) == 1
    frota = await service.registrar_fiscalizacao(
        frota.codigo_frota, fiscal_id=uuid4(), conformidade=True, apontamentos="Operacao regular"
    )
...
def test_endpoint_criar_frota_retorna_201():
    item = SimpleNamespace(
        id=uuid4(),
        codigo_frota="FRT/2026/000001",
        nome="Frota Centro",
        operadora_id=uuid4(),
        municipio="Luanda",
        provincia="Luanda",
        status=StatusFrota.ATIVA,
        data_cadastro=date(2026, 3, 4),
...
def test_endpoint_obter_frota_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(side_effect=FrotaNotFoundError("Frota nao encontrada"))
    )
    app = FastAPI()
    app.include_router(frotas_router, prefix="/transportes-logistica")
    app.dependency_overrides[get_frota_service] = lambda: service
