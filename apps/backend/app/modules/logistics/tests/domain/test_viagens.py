...
async def test_viagem_service_fluxo_sucesso():
    ...
    viagem = await service.programar_viagem(
        linha_id=uuid4(),
        veiculo_id=uuid4(),
        motorista_id=uuid4(),
...
async def test_viagem_service_detecta_conflito_de_veiculo():
    ...
    await service.programar_viagem(
        linha_id=uuid4(),
        veiculo_id=veiculo_id,
        motorista_id=uuid4(),
        data_hora_saida=base,
        data_hora_chegada_prevista=base + timedelta(hours=3),
        origem="Luanda",
        destino="Viana",
    )
    with pytest.raises(ViagemConflictError):
        await service.programar_viagem(
            ...
            data_hora_chegada_prevista=base + timedelta(hours=4),
            origem="Luanda",
            destino="Caxito",
        )
...
def test_endpoint_programar_viagem_retorna_201():
    viagem = Viagem.programar(
        linha_id=uuid4(),
        veiculo_id=uuid4(),
        motorista_id=uuid4(),
        data_hora_saida=datetime(2026, 3, 1, 8, 0, 0),
        data_hora_chegada_prevista=datetime(2026, 3, 1, 10, 0, 0),
        origem="Luanda",
        destino="Bengo",
        itinerario=[{"ordem": 1, "ponto": "Cacuaco"}],
    )
    ...
    response = client.post(
        "/transportes-logistica/viagens/",
        json={
            "linha_id": str(viagem.linha_id),
            "veiculo_id": str(viagem.veiculo_id),
            "motorista_id": str(viagem.motorista_id),
            "data_hora_saida": "2026-03-01T08:00:00",
            "data_hora_chegada_prevista": "2026-03-01T10:00:00",
            "origem": "Luanda",
            "destino": "Bengo",
            "itinerario": [{"ordem": 1, "ponto": "Cacuaco"}],
        },
...
def test_endpoint_obter_viagem_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(side_effect=ViagemNotFoundError("Viagem nao encontrada"))
    )
    app = FastAPI()
    app.include_router(viagens_router, prefix="/transportes-logistica")
    app.dependency_overrides[get_viagem_service] = lambda: service
