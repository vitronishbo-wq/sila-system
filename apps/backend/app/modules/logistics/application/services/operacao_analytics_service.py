...
class OperacaoAnalyticsService:
    def __init__(
        self,
        *,
        viagem_repo: ViagemRepositoryPort,
        bilhetagem_repo: BilhetagemRepositoryPort | None = None,
    ) -> None:
    ...
    async def calcular_demanda(
        ...
        *,
        ...
        viagens_filtradas = self._filtrar_periodo(
            viagens, data_inicio=data_inicio, data_fim=data_fim
        )
        ...
        passageiros_media = (
            Decimal(passageiros_total) / Decimal(total_viagens)
            if total_viagens > 0
            else Decimal("0")
        ).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        arrecadacao_total = Decimal("0.00")
        if self._bilhetagem_repo:
            eventos = await self._bilhetagem_repo.list(
                viagem_id=None, data_inicio=data_inicio, data_fim=data_fim
            )
            if linha_id:
        ...
        return DemandaOperacional(
            viagens_total=total_viagens,
            passageiros_total=passageiros_total,
            passageiros_media_por_viagem=passageiros_media,
            arrecadacao_total=arrecadacao_total.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP),
        )
        ...
        *,
        ...
        for viagem in viagens_filtradas:
            ...
            if (
                ...
                and viagem.data_hora_chegada_real is not None
                and (viagem.data_hora_chegada_real > viagem.data_hora_chegada_prevista)
            ):
                is_atrasada = True
            if is_atrasada:
                viagens_atrasadas += 1
        ...
        return QualidadeServico(
            viagens_total=total_viagens,
            viagens_concluidas=viagens_concluidas,
            viagens_canceladas=viagens_canceladas,
            viagens_atrasadas=viagens_atrasadas,
            pontualidade_percentual=pontualidade,
            taxa_cancelamento_percentual=taxa_cancelamento,
            taxa_atraso_percentual=taxa_atraso,
        )

    @staticmethod
    def _filtrar_periodo(
        viagens: list, *, data_inicio: datetime | None, data_fim: datetime | None
    ) -> list:
        filtradas = viagens
        if data_inicio:
    ...
    def _percentual(numerador: int, denominador: int) -> Decimal:
        if denominador <= 0:
            return Decimal("0.00")
        return (Decimal(numerador) / Decimal(denominador) * Decimal("100")).quantize(
