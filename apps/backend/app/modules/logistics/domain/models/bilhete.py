    ...
    def emitir(
        ...
        *,
        ...
        return cls(
            ...
            valor_integral=valor_integral.quantize(Decimal("0.01")),
            forma_pagamento=forma_pagamento.strip().lower(),
            data_hora_emissao=datetime.utcnow(),
            passageiro_id=passageiro_id,
            passageiro_nome=passageiro_nome.strip() if passageiro_nome else None,
            passageiro_documento=passageiro_documento.strip() if passageiro_documento else None,
            desconto=desconto,
            assento=assento.strip().upper() if assento else None,
