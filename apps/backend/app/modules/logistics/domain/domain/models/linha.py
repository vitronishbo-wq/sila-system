...
from app.modules.logistics.domain.enums import ModalTransporte, StatusLinha, TipoViagem
...
class Linha:
    ...
    @classmethod
    def criar(cls, *, codigo: str, nome: str, modal: ModalTransporte, tipo_viagem: TipoViagem, origem: str, destino: str, itinerario: list[dict[str, Any]], extensao_km: Decimal, tempo_estimado_minutos: int, dias_operacao: list[str], horario_inicio: str, horario_fim: str, tarifa_base: Decimal, operadora_id: UUID, frequencia_media_minutos: int | None=None, observacoes: str | None=None) -> 'Linha':
        if not codigo.strip():
        ...
        return cls(id=uuid4(), codigo=codigo.strip(), nome=nome.strip(), modal=modal, tipo_viagem=tipo_viagem, origem=origem.strip(), destino=destino.strip(), itinerario=itinerario, extensao_km=extensao_km.quantize(Decimal('0.01')), tempo_estimado_minutos=tempo_estimado_minutos, dias_operacao=dias_operacao, horario_inicio=horario_inicio, horario_fim=horario_fim, tarifa_base=tarifa_base.quantize(Decimal('0.01')), operadora_id=operadora_id, status=StatusLinha.EM_IMPLANTACAO, data_cadastro=date.today(), frequencia_media_minutos=frequencia_media_minutos, observacoes=observacoes.strip() if observacoes else None)
    ...
    def adicionar_veiculo(self, *, veiculo_id: UUID, placa: str) -> None:
        ...
        self.veiculos_ativos.append({'veiculo_id': str(veiculo_id), 'placa': normalized, 'data_vinculo': date.today().isoformat()})
    ...
    def registrar_indicadores_operacionais(self, *, demanda_media_diaria: int | None, ocupacao_media: Decimal | None, regularidade: Decimal | None, pontualidade: Decimal | None) -> None:
        self.demanda_media_diaria = demanda_media_diaria
        self.ocupacao_media = ocupacao_media.quantize(Decimal('0.01')) if ocupacao_media is not None else None
        self.regularidade = regularidade.quantize(Decimal('0.01')) if regularidade is not None else None
        self.pontualidade = pontualidade.quantize(Decimal('0.01')) if pontualidade is not None else None
        self.data_atualizacao = date.today()
