...
async def test_bilhetagem_service_registro_e_reconciliacao():
    financas_adapter = SimpleNamespace(registrar_receita_bilhetagem=AsyncMock(return_value='REC-TRN-0001'), reconciliar_lancamento=AsyncMock(return_value=True))
    workflow_adapter = SimpleNamespace(registrar_evento=AsyncMock(return_value=None))
    service = BilhetagemService(bilhetagem_repo=SQLAlchemyBilhetagemRepository(), financas_adapter=financas_adapter, workflow_adapter=workflow_adapter)
    evento = await service.registrar_evento(codigo_bilhete='BILH-0001', viagem_id=uuid4(), tipo_tarifa=TipoTarifa.PUBLICA, valor_pago=Decimal('250.00'), forma_pagamento='cartao')
    ...
    reconciliado = await service.reconciliar_evento(evento.id, confirmado=True, referencia_externa='BANK-REF-001')
...
def test_endpoint_registrar_evento_bilhetagem_retorna_201():
    evento = BilhetagemEletronica.registrar_evento(codigo_bilhete='BILH-0001', viagem_id=uuid4(), tipo_tarifa=TipoTarifa.PUBLICA, valor_pago=Decimal('250.00'), forma_pagamento='cartao')
    ...
    response = client.post('/transportes-logistica/bilhetagem/eventos', json={'codigo_bilhete': 'BILH-0001', 'viagem_id': str(uuid4()), 'tipo_tarifa': 'publica', 'valor_pago': '250.00', 'forma_pagamento': 'cartao', 'metadata': {'catraca': 'A1'}})
