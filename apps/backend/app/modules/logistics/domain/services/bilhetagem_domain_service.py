...
class BilhetagemDomainService:
    def __init__(
        self,
        *,
        bilhetagem_repo: BilhetagemRepositoryPort,
    ...
    async def registrar_evento(
        ...
        *,
        ...
        if self._financas_adapter:
            lancamento_id = await self._financas_adapter.registrar_receita_bilhetagem(
                evento_id=evento.id, valor=evento.valor_pago, forma_pagamento=evento.forma_pagamento
            )
        ...
        await self._registrar_evento_workflow(
            workflow_id=f"ticket:{saved.codigo_bilhete}",
            evento="bilhetagem_evento_registrado",
            payload={
                "evento_id": str(saved.id),
                "valor_pago": str(saved.valor_pago),
                "forma_pagamento": saved.forma_pagamento,
            },
        )
        return saved

    async def reconciliar_evento(
        self, evento_id: UUID, *, confirmado: bool, referencia_externa: str | None = None
    ) -> BilhetagemEletronica:
        ...
        if self._financas_adapter:
            reconciliado = await self._financas_adapter.reconciliar_lancamento(
                lancamento_id=evento.lancamento_financeiro_id,
        ...
        await self._registrar_evento_workflow(
            workflow_id=f"ticket:{saved.codigo_bilhete}",
            evento="bilhetagem_evento_reconciliado",
            payload={
                "evento_id": str(saved.id),
                "status_reconciliacao": saved.status_reconciliacao.value,
                "referencia_externa": referencia_externa,
            },
        )
        return saved
    ...
    async def listar_eventos(
        ...
        *,
        ...
        return await self._bilhetagem_repo.list(
            viagem_id=viagem_id,
            codigo_bilhete=codigo_bilhete,
            status_reconciliacao=status_reconciliacao,
