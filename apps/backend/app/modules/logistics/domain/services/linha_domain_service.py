...
class LinhaDomainService:
    def __init__(
        self,
        *,
        linha_repo: LinhaRepositoryPort,
        veiculo_repo: VeiculoRepositoryPort,
        geosampa_adapter: GeosampaServicePort | None = None,
        urbanismo_adapter: UrbanismoServicePort | None = None,
    ...
    async def criar_linha(
        ...
        *,
        ...
        if self._urbanismo_adapter:
            zona_valida = await self._urbanismo_adapter.validar_zoneamento_rota(
                origem=origem, destino=destino
            )
            if not zona_valida:
                raise ValueError("Rota nao validada por urbanismo")
        if codigo_corredor and self._obras_publicas_adapter:
            corredor_valido = await self._obras_publicas_adapter.validar_corredor(
        ...
        linha.registrar_evento_auditoria(
            evento="linha_criada", payload={"codigo_corredor": codigo_corredor}
        )
        saved = await self._linha_repo.save(linha)
        if self._workflow_adapter:
            workflow_id = await self._workflow_adapter.iniciar_fluxo(
                entidade="transportes_logistica_linha",
                referencia_id=saved.id,
                contexto={"codigo_linha": saved.codigo},
            )
            saved.registrar_evento_auditoria(
                evento="workflow_iniciado", payload={"workflow_id": workflow_id}
            )
    ...
    async def vincular_veiculo(self, codigo_linha: str, *, placa: str) -> Linha:
        ...
        linha.registrar_evento_auditoria(
            evento="veiculo_vinculado",
            payload={"veiculo_id": str(veiculo.id), "placa": veiculo.placa},
        )
        saved = await self._linha_repo.save(linha)
        await self._registrar_evento_workflow(
            workflow_id=saved.codigo,
            evento="linha_veiculo_vinculado",
            payload={"veiculo_id": str(veiculo.id)},
        )
        return saved
    ...
    async def atualizar_tarifa(self, codigo_linha: str, *, valor: Decimal) -> Linha:
        ...
        linha.registrar_evento_auditoria(
            evento="tarifa_atualizada", payload={"valor": str(linha.tarifa_base)}
        )
        saved = await self._linha_repo.save(linha)
        await self._registrar_evento_workflow(
            workflow_id=saved.codigo,
            evento="linha_tarifa_atualizada",
            payload={"valor": str(saved.tarifa_base)},
        )
        return saved

    async def registrar_indicadores(
        self,
        codigo_linha: str,
        *,
        ...
        if not linha:
            raise LinhaNotFoundError("Linha nao encontrada")
        linha.registrar_indicadores_operacionais(
        ...
        await self._registrar_evento_workflow(
            workflow_id=saved.codigo,
            evento="linha_indicadores_atualizados",
            payload={
                "demanda_media_diaria": demanda_media_diaria,
                "ocupacao_media": str(ocupacao_media) if ocupacao_media is not None else None,
                "regularidade": str(regularidade) if regularidade is not None else None,
                "pontualidade": str(pontualidade) if pontualidade is not None else None,
            },
        )
        return saved
    ...
    async def listar_linhas(
        ...
        *,
        ...
        return await self._linha_repo.list(
            status=status, modal=modal, operadora_id=operadora_id, origem=origem, destino=destino
        )
    ...
    async def _registrar_evento_workflow(
        self, *, workflow_id: str, evento: str, payload: dict
    ) -> None:
        if not self._workflow_adapter:
            return
        await self._workflow_adapter.registrar_evento(
            workflow_id=workflow_id, evento=evento, payload=payload
        )
