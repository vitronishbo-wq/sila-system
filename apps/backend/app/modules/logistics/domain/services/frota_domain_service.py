...
from apps.backend.app.modules.logistics.domain.exceptions import (
    FrotaAlreadyExistsError,
    FrotaNotFoundError,
)
from apps.backend.app.modules.logistics.domain.models import (
...
class FrotaDomainService:
    ...
    async def criar_frota(
        ...
        *,
        ...
        if self._service_requests_adapter:
            request_id = await self._service_requests_adapter.abrir_solicitacao(
                {
                    "tipo": "frota_operacional",
                    "codigo_frota": saved.codigo_frota,
        ...
        if self._workflow_adapter:
            workflow_id = await self._workflow_adapter.iniciar_fluxo(
                entidade="transportes_logistica_frota",
                referencia_id=saved.id,
                contexto={"codigo_frota": saved.codigo_frota},
            )
            saved.registrar_evento_auditoria(
                evento="frota_criada", payload={"workflow_id": workflow_id}
            )
            saved = await self._frota_repo.save(saved)
        return saved
    ...
    async def adicionar_veiculo(
        ...
        *,
        ...
        if self._seguranca_publica_adapter:
            regular = await self._seguranca_publica_adapter.validar_regularidade_veiculo(
                placa=placa
            )
            if not regular:
        ...
        item.registrar_evento_auditoria(
            evento="veiculo_adicionado",
            payload={"veiculo_id": str(veiculo_id), "placa": placa.upper()},
        )
        saved = await self._frota_repo.save(item)
        await self._registrar_evento_workflow(
            saved, evento="frota_veiculo_adicionado", payload={"veiculo_id": str(veiculo_id)}
        )
        return saved
    ...
    async def registrar_manutencao(
        ...
        *,
        ...
        item = await self._obter_ou_erro(codigo_frota)
        manutencao = Manutencao.registrar(
        ...
        item.registrar_evento_auditoria(
            evento="manutencao_registrada",
            payload={"manutencao_id": str(manutencao.id), "veiculo_id": str(veiculo_id)},
        )
        if self._financas_adapter:
            registrado = await self._financas_adapter.registrar_despesa_manutencao(
                frota_id=item.id,
                veiculo_id=veiculo_id,
                valor=manutencao.custo,
                descricao=f"Manutencao {manutencao.tipo}",
            )
            if not registrado:
        ...
        await self._registrar_evento_workflow(
            saved,
            evento="frota_manutencao_registrada",
            payload={"manutencao_id": str(manutencao.id), "custo": str(manutencao.custo)},
        )
        return saved
    ...
    async def atualizar_tarifa(
        ...
        *,
        ...
        item = await self._obter_ou_erro(codigo_frota)
        if self._financas_adapter:
            tarifa_valida = await self._financas_adapter.validar_tarifa(
                valor=valor, tipo=tipo_tarifa.value
            )
            if not tarifa_valida:
                raise ValueError("Tarifa invalida para politica financeira vigente")
        tarifa = Tarifa.definir(
            tipo=tipo_tarifa,
            valor=valor,
        ...
        item.registrar_evento_auditoria(
            evento="tarifa_atualizada",
            payload={
                "tarifa_id": str(tarifa.id),
                "tipo": tarifa.tipo.value,
        ...
        await self._registrar_evento_workflow(
            saved,
            evento="frota_tarifa_atualizada",
            payload={"tarifa_id": str(tarifa.id), "tipo": tarifa.tipo.value},
        )
        return saved
    ...
    async def registrar_fiscalizacao(
        ...
        *,
        ...
        item = await self._obter_ou_erro(codigo_frota)
        fiscalizacao = FiscalizacaoTransporte.registrar(
            fiscal_id=fiscal_id,
        ...
        item.registrar_evento_auditoria(
            evento="fiscalizacao_registrada",
            payload={"fiscalizacao_id": str(fiscalizacao.id), "conformidade": conformidade},
        )
        saved = await self._frota_repo.save(item)
        await self._registrar_evento_workflow(
            saved,
            evento="frota_fiscalizacao_registrada",
            payload={"fiscalizacao_id": str(fiscalizacao.id), "conformidade": conformidade},
        )
        return saved
    ...
    async def listar(
        ...
        *,
        ...
        return await self._frota_repo.list(
    ...
    async def _registrar_evento_workflow(self, item: Frota, *, evento: str, payload: dict) -> None:
        ...
        await self._workflow_adapter.registrar_evento(
            workflow_id=item.codigo_frota, evento=evento, payload=payload
        )
