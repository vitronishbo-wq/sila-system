...
from app.modules.logistics.application.ports.financas_service_port import FinancasServicePort
...
class FinancasServiceAdapter(FinancasServicePort):
    ...
    async def registrar_despesa_manutencao(self, *, frota_id: UUID, veiculo_id: UUID, valor: Decimal, descricao: str) -> bool:
        _ = (frota_id, veiculo_id, descricao)
        return valor > Decimal('0')

    async def registrar_receita_bilhetagem(self, *, evento_id: UUID, valor: Decimal, forma_pagamento: str) -> str:
        if valor <= Decimal('0'):
    ...
    async def reconciliar_lancamento(self, *, lancamento_id: str, confirmado: bool, referencia_externa: str | None=None) -> bool:
