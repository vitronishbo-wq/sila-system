...
class SQLAlchemyFrotaRepository(FrotaRepositoryPort):
    ...
    async def save(self, item: Frota) -> Frota:
        if self._session:
            existing = await self._session.execute(select(FrotaModel).where(FrotaModel.codigo_frota == item.codigo_frota))
            model = existing.scalars().first()
            if model is None:
                model = FrotaModel(id=item.id, codigo_frota=item.codigo_frota, nome=item.nome, operadora_id=item.operadora_id, municipio=item.municipio, provincia=item.provincia, status=item.status.value, data_cadastro=item.data_cadastro, data_atualizacao=item.data_atualizacao, observacoes=item.observacoes, veiculos=item.veiculos, manutencoes=item.manutencoes, fiscalizacoes=item.fiscalizacoes, tarifas=item.tarifas, trilha_auditoria=item.trilha_auditoria)
    ...
    async def get_by_codigo(self, codigo_frota: str) -> Frota | None:
        if self._session:
            result = await self._session.execute(select(FrotaModel).where(FrotaModel.codigo_frota == codigo_frota))
            model = result.scalars().first()
            return self._to_domain(model) if model else None
        return self._items.get(codigo_frota)

    async def list(self, *, status: StatusFrota | None=None, operadora_id: UUID | None=None, municipio: str | None=None, provincia: str | None=None) -> list[Frota]:
        if self._session:
            ...
            if municipio:
                statement = statement.where(func.lower(FrotaModel.municipio) == municipio.strip().lower())
            if provincia:
                statement = statement.where(func.lower(FrotaModel.provincia) == provincia.strip().lower())
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            prefix = f'FRT/{year}/'
            result = await self._session.execute(select(func.count()).select_from(FrotaModel).where(FrotaModel.codigo_frota.like(f'{prefix}%')))
    ...
    def _to_domain(model: FrotaModel) -> Frota:
        return Frota(id=model.id, codigo_frota=model.codigo_frota, nome=model.nome, operadora_id=model.operadora_id, municipio=model.municipio, provincia=model.provincia, status=StatusFrota(model.status), data_cadastro=model.data_cadastro, data_atualizacao=model.data_atualizacao, observacoes=model.observacoes, veiculos=list(model.veiculos or []), manutencoes=list(model.manutencoes or []), fiscalizacoes=list(model.fiscalizacoes or []), tarifas=list(model.tarifas or []), trilha_auditoria=list(model.trilha_auditoria or []))
