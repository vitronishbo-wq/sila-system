...
class SQLAlchemyViagemRepository(ViagemRepositoryPort):
    ...
    async def save(self, viagem: Viagem) -> Viagem:
        if self._session:
            existing = await self._session.execute(
                select(ViagemModel).where(ViagemModel.id == viagem.id)
            )
            ...
            if model is None:
                model = ViagemModel(
                    ...
                    data_hora_chegada_prevista=viagem.data_hora_chegada_prevista,
                    origem=viagem.origem,
                    destino=viagem.destino,
                    itinerario=deepcopy(viagem.itinerario),
                    status=viagem.status.value,
                    data_hora_chegada_real=viagem.data_hora_chegada_real,
                    paradas=deepcopy(viagem.paradas),
                    passageiros_embarcados=viagem.passageiros_embarcados,
                    passageiros_desembarcados=viagem.passageiros_desembarcados,
                    passageiros_transbordo=viagem.passageiros_transbordo,
                    carga=deepcopy(viagem.carga),
                    volume_carga=viagem.volume_carga,
                    peso_carga=viagem.peso_carga,
                    valor_frete=viagem.valor_frete,
                    quilometragem_inicial=viagem.quilometragem_inicial,
                    quilometragem_final=viagem.quilometragem_final,
                    consumo_combustivel=viagem.consumo_combustivel,
        ...
        *,
        ...
        if self._session:
            ...
            result = await self._session.execute(
                statement.order_by(ViagemModel.data_hora_saida.asc())
            )
    ...
    async def has_active_for_veiculo(
        self,
        *,
        veiculo_id: UUID,
        inicio: datetime,
        fim: datetime,
        ignore_viagem_id: UUID | None = None,
    ) -> bool:
        statuses_ativos = {
            StatusViagem.PROGRAMADA.value,
            StatusViagem.CONFIRMADA.value,
            StatusViagem.EM_ANDAMENTO.value,
            StatusViagem.ATRASADA.value,
        }
        if self._session:
    ...
    def _to_domain(model: ViagemModel) -> Viagem:
        return Viagem(
            id=model.id,
            linha_id=model.linha_id,
            veiculo_id=model.veiculo_id,
            motorista_id=model.motorista_id,
            data_hora_saida=model.data_hora_saida,
            data_hora_chegada_prevista=model.data_hora_chegada_prevista,
            origem=model.origem,
            destino=model.destino,
            itinerario=deepcopy(model.itinerario),
            status=StatusViagem(model.status),
            data_hora_chegada_real=model.data_hora_chegada_real,
            paradas=deepcopy(model.paradas),
            passageiros_embarcados=model.passageiros_embarcados,
            passageiros_desembarcados=model.passageiros_desembarcados,
            passageiros_transbordo=model.passageiros_transbordo,
            carga=deepcopy(model.carga),
            volume_carga=Decimal(model.volume_carga) if model.volume_carga is not None else None,
            peso_carga=Decimal(model.peso_carga) if model.peso_carga is not None else None,
            valor_frete=Decimal(model.valor_frete) if model.valor_frete is not None else None,
            quilometragem_inicial=model.quilometragem_inicial,
            quilometragem_final=model.quilometragem_final,
            consumo_combustivel=Decimal(model.consumo_combustivel)
            if model.consumo_combustivel is not None
            else None,
