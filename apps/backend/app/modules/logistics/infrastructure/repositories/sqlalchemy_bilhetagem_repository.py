...
class SQLAlchemyBilhetagemRepository(BilhetagemRepositoryPort):
    ...
    async def save(self, item: BilhetagemEletronica) -> BilhetagemEletronica:
        if self._session:
            existing = await self._session.execute(
                select(BilhetagemEventoModel).where(BilhetagemEventoModel.id == item.id)
            )
    ...
    async def get_by_id(self, evento_id: UUID) -> BilhetagemEletronica | None:
        if self._session:
            result = await self._session.execute(
                select(BilhetagemEventoModel).where(BilhetagemEventoModel.id == evento_id)
            )
    ...
    async def get_by_codigo_bilhete(self, codigo_bilhete: str) -> BilhetagemEletronica | None:
        ...
        if self._session:
            result = await self._session.execute(
                select(BilhetagemEventoModel)
                .where(BilhetagemEventoModel.codigo_bilhete == normalized)
                .order_by(BilhetagemEventoModel.data_evento.desc())
            )
            model = result.scalars().first()
            return self._to_domain(model) if model else None
        ...
        if self._session:
            ...
            if codigo_bilhete:
                statement = statement.where(
                    BilhetagemEventoModel.codigo_bilhete == codigo_bilhete.strip()
                )
            if status_reconciliacao:
                statement = statement.where(
                    BilhetagemEventoModel.status_reconciliacao == status_reconciliacao.value
                )
            if data_inicio:
            ...
            result = await self._session.execute(
                statement.order_by(BilhetagemEventoModel.data_evento.asc())
            )
    ...
    def _to_model(item: BilhetagemEletronica) -> BilhetagemEventoModel:
        return BilhetagemEventoModel(
            id=item.id,
            codigo_bilhete=item.codigo_bilhete,
            viagem_id=item.viagem_id,
            tipo_tarifa=item.tipo_tarifa.value,
            valor_pago=item.valor_pago,
            forma_pagamento=item.forma_pagamento,
            data_evento=item.data_evento,
            status_reconciliacao=item.status_reconciliacao.value,
    ...
    def _to_domain(model: BilhetagemEventoModel) -> BilhetagemEletronica:
        return BilhetagemEletronica(
            id=model.id,
            codigo_bilhete=model.codigo_bilhete,
            viagem_id=model.viagem_id,
            tipo_tarifa=TipoTarifa(model.tipo_tarifa),
            valor_pago=Decimal(model.valor_pago),
            forma_pagamento=model.forma_pagamento,
            data_evento=model.data_evento,
            status_reconciliacao=StatusReconciliacaoFinanceira(model.status_reconciliacao),
            lancamento_financeiro_id=model.lancamento_financeiro_id,
