...
from apps.backend.app.modules.logistics.domain.ports import VeiculoRepositoryPort
from apps.backend.app.modules.logistics.infrastructure.orm import VeiculoModel
...
class SQLAlchemyVeiculoRepository(VeiculoRepositoryPort):
    ...
    async def save(self, item: Veiculo) -> Veiculo:
        if self._session:
            existing = await self._session.execute(
                select(VeiculoModel).where(VeiculoModel.placa == item.placa)
            )
    ...
    async def get_by_id(self, veiculo_id: UUID) -> Veiculo | None:
        if self._session:
            result = await self._session.execute(
                select(VeiculoModel).where(VeiculoModel.id == veiculo_id)
            )
    ...
    async def get_by_placa(self, placa: str) -> Veiculo | None:
        ...
        if self._session:
            result = await self._session.execute(
                select(VeiculoModel).where(VeiculoModel.placa == normalized)
            )
            model = result.scalars().first()
            return self._to_domain(model) if model else None
    ...
    def _to_model(item: Veiculo) -> VeiculoModel:
        return VeiculoModel(
            ...
            capacidade_carga_kg=item.capacidade_carga_kg,
            capacidade_carga_m3=item.capacidade_carga_m3,
            comprimento=item.comprimento,
            largura=item.largura,
            altura=item.altura,
            peso_bruto_total=item.peso_bruto_total,
            numero_eixos=item.numero_eixos,
            combustivel=item.combustivel,
            consumo_medio=item.consumo_medio,
            operadora_id=item.operadora_id,
            licenciamento=deepcopy(item.licenciamento),
            seguro=deepcopy(item.seguro),
            rastreador_id=item.rastreador_id,
            data_ultima_manutencao=item.data_ultima_manutencao,
            data_proxima_manutencao=item.data_proxima_manutencao,
    ...
    def _to_domain(model: VeiculoModel) -> Veiculo:
        return Veiculo(
            id=model.id,
            placa=model.placa,
            tipo=TipoVeiculo(model.tipo),
            marca=model.marca,
            modelo=model.modelo,
            ano_fabricacao=model.ano_fabricacao,
            ano_modelo=model.ano_modelo,
            proprietario_id=model.proprietario_id,
            proprietario_tipo=model.proprietario_tipo,
            data_aquisicao=model.data_aquisicao,
            status=StatusVeiculoOperacional(model.status),
            capacidade_passageiros=model.capacidade_passageiros,
            capacidade_carga_kg=Decimal(model.capacidade_carga_kg)
            if model.capacidade_carga_kg is not None
            else None,
            capacidade_carga_m3=Decimal(model.capacidade_carga_m3)
            if model.capacidade_carga_m3 is not None
            else None,
            comprimento=Decimal(model.comprimento) if model.comprimento is not None else None,
            largura=Decimal(model.largura) if model.largura is not None else None,
            altura=Decimal(model.altura) if model.altura is not None else None,
            peso_bruto_total=Decimal(model.peso_bruto_total)
            if model.peso_bruto_total is not None
            else None,
            numero_eixos=model.numero_eixos,
            combustivel=model.combustivel,
            consumo_medio=Decimal(model.consumo_medio) if model.consumo_medio is not None else None,
            operadora_id=model.operadora_id,
            licenciamento=deepcopy(model.licenciamento),
            seguro=deepcopy(model.seguro),
            rastreador_id=model.rastreador_id,
            data_ultima_manutencao=model.data_ultima_manutencao,
            data_proxima_manutencao=model.data_proxima_manutencao,
