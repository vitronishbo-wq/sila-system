...
bilhetagem_service_dep = Depends(get_bilhetagem_service)
...
def _ensure_financas_adapter(service: BilhetagemService) -> None:
    if not _has_capability(service, "has_financas_adapter"):
        raise HTTPException(
            ...
            detail="Adapter de Financas indisponivel para bilhetagem reconciliavel.",
        )


@router.post(
    "/eventos", response_model=BilhetagemEventoResponse, status_code=status.HTTP_201_CREATED
)
async def registrar_evento(
    data: BilhetagemEventoCreate, service: BilhetagemService = bilhetagem_service_dep
):
    _ensure_financas_adapter(service)
    try:
        return await service.registrar_evento(
            codigo_bilhete=data.codigo_bilhete,
...
async def reconciliar_evento(
    evento_id: UUID,
    data: BilhetagemReconciliacaoInput,
    service: BilhetagemService = bilhetagem_service_dep,
):
    _ensure_financas_adapter(service)
    try:
        return await service.reconciliar_evento(
            evento_id, confirmado=data.confirmado, referencia_externa=data.referencia_externa
        )
...
async def obter_evento(
    evento_id: UUID, service: BilhetagemService = bilhetagem_service_dep
):
    try:
