...
frota_service_dep = Depends(get_frota_service)
...
def _ensure_criacao_adapters(service: FrotaService) -> None:
    if not _has_capability(service, "has_workflow_adapter"):
        raise HTTPException(
            ...
            detail="Adapter de Workflow indisponivel para criacao da frota.",
        )
    if not _has_capability(service, "has_service_requests_adapter"):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Service Requests indisponivel para criacao da frota.",
        )


@router.post("/", response_model=FrotaResponse, status_code=status.HTTP_201_CREATED)
async def criar_frota(data: FrotaCreate, service: FrotaService = frota_service_dep):
    _ensure_criacao_adapters(service)
    try:
        return await service.criar_frota(
            nome=data.nome,
            operadora_id=data.operadora_id,
            municipio=data.municipio,
            provincia=data.provincia,
            codigo_frota=data.codigo_frota,
...
async def adicionar_veiculo_frota(
    codigo_frota: str,
    data: FrotaAdicionarVeiculoInput,
    service: FrotaService = frota_service_dep,
):
    try:
        return await service.adicionar_veiculo(
            codigo_frota,
...
async def registrar_manutencao_frota(
    codigo_frota: str,
    data: FrotaManutencaoInput,
    service: FrotaService = frota_service_dep,
):
    try:
        return await service.registrar_manutencao(
            codigo_frota,
            veiculo_id=data.veiculo_id,
            tipo=data.tipo,
...
async def atualizar_tarifa_frota(
    codigo_frota: str, data: FrotaTarifaInput, service: FrotaService = frota_service_dep
):
    try:
        return await service.atualizar_tarifa(
            codigo_frota,
...
async def registrar_fiscalizacao_frota(
    codigo_frota: str,
    data: FrotaFiscalizacaoInput,
    service: FrotaService = frota_service_dep,
):
    try:
...
async def obter_frota(codigo_frota: str, service: FrotaService = frota_service_dep):
    try:
...
async def listar_frotas(
    status_frota: StatusFrota | None = None,
    ...
    return await service.listar(
        status=status_frota, operadora_id=operadora_id, municipio=municipio, provincia=provincia
    )
