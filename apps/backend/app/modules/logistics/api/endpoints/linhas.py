...
linha_service_dep = Depends(get_linha_service)
...
def _ensure_criacao_adapters(service: LinhaService) -> None:
    if not _has_capability(service, "has_geosampa_adapter"):
        raise HTTPException(
            ...
            detail="Adapter Geosampa indisponivel para criacao da linha.",
        )
    if not _has_capability(service, "has_urbanismo_adapter"):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter Urbanismo indisponivel para criacao da linha.",
        )
    if not _has_capability(service, "has_workflow_adapter"):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter Workflow indisponivel para trilha da linha.",
        )


@router.post("/", response_model=LinhaResponse, status_code=status.HTTP_201_CREATED)
async def criar_linha(data: LinhaCreate, service: LinhaService = linha_service_dep):
    _ensure_criacao_adapters(service)
    try:
        return await service.criar_linha(
            nome=data.nome,
            modal=data.modal,
            ...
            codigo=data.codigo,
            frequencia_media_minutos=data.frequencia_media_minutos,
...
async def cadastrar_veiculo(
    data: VeiculoCreate, service: LinhaService = linha_service_dep
):
    try:
        return await service.cadastrar_veiculo(
            placa=data.placa,
            tipo=data.tipo,
            marca=data.marca,
            modelo=data.modelo,
            ...
            proprietario_tipo=data.proprietario_tipo,
            data_aquisicao=data.data_aquisicao,
            capacidade_passageiros=data.capacidade_passageiros,
            operadora_id=data.operadora_id,
            observacoes=data.observacoes,
        )
...
async def vincular_veiculo(
    codigo_linha: str,
    data: LinhaVincularVeiculoInput,
    service: LinhaService = linha_service_dep,
):
    try:
...
async def atualizar_tarifa(
    codigo_linha: str, data: LinhaTarifaInput, service: LinhaService = linha_service_dep
):
    try:
...
async def registrar_indicadores(
    codigo_linha: str,
    data: LinhaIndicadoresInput,
    service: LinhaService = linha_service_dep,
):
    try:
        return await service.registrar_indicadores(
            codigo_linha,
            demanda_media_diaria=data.demanda_media_diaria,
            ocupacao_media=data.ocupacao_media,
            regularidade=data.regularidade,
...
async def listar_linhas(
    status_linha: StatusLinha | None = None,
    ...
    return await service.listar_linhas(
        status=status_linha, modal=modal, operadora_id=operadora_id, origem=origem, destino=destino
    )


@router.get("/veiculos/{placa}", response_model=VeiculoResponse)
async def obter_veiculo(placa: str, service: LinhaService = linha_service_dep):
    try:
...
async def listar_veiculos(
    status_veiculo: StatusVeiculoOperacional | None = None,
    tipo: TipoVeiculo | None = None,
    operadora_id: UUID | None = None,
    service: LinhaService = linha_service_dep,
):
    return await service.listar_veiculos(
        status=status_veiculo, tipo=tipo, operadora_id=operadora_id
    )


@router.get("/{codigo_linha:path}", response_model=LinhaResponse)
async def obter_linha(codigo_linha: str, service: LinhaService = linha_service_dep):
    try:
