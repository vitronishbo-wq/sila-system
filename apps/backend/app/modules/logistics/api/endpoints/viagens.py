...
from apps.backend.app.modules.logistics.domain.exceptions import (
    InvalidViagemStateError,
    ViagemConflictError,
    ViagemNotFoundError,
)

router = APIRouter(prefix="/viagens", tags=["Transportes Logistica - Viagens"])
viagem_service_dep = Depends(get_viagem_service)


@router.post("/", response_model=ViagemResponse, status_code=status.HTTP_201_CREATED)
async def programar_viagem(
    data: ViagemCreate, service: ViagemService = viagem_service_dep
):
    try:
        return await service.programar_viagem(
            ...
            data_hora_chegada_prevista=data.data_hora_chegada_prevista,
            origem=data.origem,
...
async def iniciar_viagem(viagem_id: UUID, service: ViagemService = viagem_service_dep):
    try:
...
async def concluir_viagem(
    viagem_id: UUID, data: ViagemConcluirInput, service: ViagemService = viagem_service_dep
):
    try:
...
async def cancelar_viagem(
    viagem_id: UUID, data: ViagemCancelarInput, service: ViagemService = viagem_service_dep
):
    try:
...
async def obter_viagem(viagem_id: UUID, service: ViagemService = viagem_service_dep):
    try:
...
async def listar_viagens(
    status_viagem: StatusViagem | None = None,
    linha_id: UUID | None = None,
    veiculo_id: UUID | None = None,
    service: ViagemService = viagem_service_dep,
):
