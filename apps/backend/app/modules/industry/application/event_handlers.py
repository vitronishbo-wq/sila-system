...
from apps.backend.app.modules.industry.domain.events import (
    EnvironmentalPermitIssued,
    IndustrialFacilityRegistered,
    IndustrialFacilityStatusChanged,
    SafetyCertificationIssued,
    SafetyInspectionConducted,
)
...
async def handle_environmental_permit_issued(event: EnvironmentalPermitIssued) -> None:
    ...
    logger.info(
        f"Environmental permit {event.aggregate_id} issued for facility {event.aggregate_id}"
    )
    pass
...
async def handle_safety_certification_issued(event: SafetyCertificationIssued) -> None:
    ...
    logger.info(
        f"Safety certification {event.aggregate_id} issued for facility {event.aggregate_id}"
    )
    pass


INDUSTRY_EVENT_HANDLERS: dict[str, Callable[[DomainEvent], Awaitable[None]]] = {
    "IndustrialFacilityRegistered": handle_industrial_facility_registered,
    "EnvironmentalPermitIssued": handle_environmental_permit_issued,
    "SafetyInspectionConducted": handle_safety_inspection_conducted,
    "IndustrialFacilityStatusChanged": handle_industrial_facility_status_changed,
