...
class EstabelecimentoIndustrialService:
    ...
    async def cadastrar(self, *, cnpj: str, razao_social: str, ramo: RamoIndustrial, porte: PorteIndustrial, tipo: TipoEstabelecimento, cnae_principal: str, data_abertura: date, endereco: str, bairro: str, municipio: str, provincia: str) -> EstabelecimentoIndustrial:
        existente = await self._repository.get_by_cnpj(cnpj)
        if existente:
            raise EstabelecimentoIndustrialAlreadyExistsError('Estabelecimento industrial ja cadastrado para o CNPJ informado')
        item = EstabelecimentoIndustrial.cadastrar(cnpj=cnpj, razao_social=razao_social, ramo=ramo, porte=porte, tipo=tipo, cnae_principal=cnae_principal, data_abertura=data_abertura, endereco=endereco, bairro=bairro, municipio=municipio, provincia=provincia)
        salvo = await self._repository.save(item)
        logger.info('industria.estabelecimento.cadastrado id=%s cnpj=%s ramo=%s porte=%s', str(salvo.id), salvo.cnpj, salvo.ramo.value, salvo.porte.value)
        return salvo
    ...
    async def iniciar_atividades(self, id: UUID, *, data_inicio: date) -> EstabelecimentoIndustrial:
        ...
        salvo = await self._repository.save(item)
        logger.info('industria.estabelecimento.status id=%s status=%s acao=iniciar_atividades', str(salvo.id), salvo.status.value)
        return salvo
    ...
    async def suspender_atividades(self, id: UUID, *, motivo: str) -> EstabelecimentoIndustrial:
        ...
        try:
            ...
            raise InvalidEstabelecimentoIndustrialStateError(str(exc)) from exc
        salvo = await self._repository.save(item)
        logger.info('industria.estabelecimento.status id=%s status=%s acao=suspender_atividades', str(salvo.id), salvo.status.value)
        return salvo
    ...
    async def paralisar(self, id: UUID, *, motivo: str) -> EstabelecimentoIndustrial:
        ...
        try:
            ...
            raise InvalidEstabelecimentoIndustrialStateError(str(exc)) from exc
        salvo = await self._repository.save(item)
        logger.info('industria.estabelecimento.status id=%s status=%s acao=paralisar', str(salvo.id), salvo.status.value)
        return salvo
    ...
    async def reativar(self, id: UUID) -> EstabelecimentoIndustrial:
        ...
        try:
            ...
            raise InvalidEstabelecimentoIndustrialStateError(str(exc)) from exc
        salvo = await self._repository.save(item)
        logger.info('industria.estabelecimento.status id=%s status=%s acao=reativar', str(salvo.id), salvo.status.value)
        return salvo
    ...
    async def definir_porte(self, id: UUID, *, porte: PorteIndustrial) -> EstabelecimentoIndustrial:
        ...
        salvo = await self._repository.save(item)
        logger.info('industria.estabelecimento.porte id=%s porte=%s', str(salvo.id), salvo.porte.value)
        return salvo

    async def obter_por_id(self, id: UUID) -> EstabelecimentoIndustrial:
        return await self._obter_ou_erro(id)

    async def listar(self, *, status: StatusEstabelecimento | None=None, ramo: RamoIndustrial | None=None, municipio: str | None=None) -> list[EstabelecimentoIndustrial]:
    ...
    async def _obter_ou_erro(self, id: UUID) -> EstabelecimentoIndustrial:
        ...
        if not item:
            raise EstabelecimentoIndustrialNotFoundError('Estabelecimento industrial nao encontrado')
        return item
