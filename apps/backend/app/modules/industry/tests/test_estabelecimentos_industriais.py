...
from apps.backend.app.modules.industry.api.endpoints.estabelecimentos_industriais import (
...
from apps.backend.app.modules.industry.infrastructure.repositories import (
...
async def test_service_fluxo_principal_estabelecimento():
    service = EstabelecimentoIndustrialService(
        repository=SQLAlchemyEstabelecimentoIndustrialRepository()
    )
    item = await service.cadastrar(
        cnpj="50010020000199",
        razao_social="Fabrica Luanda Norte SA",
        ramo=RamoIndustrial.TRANSFORMACAO,
        porte=PorteIndustrial.MEDIA,
        tipo=TipoEstabelecimento.MATRIZ,
        cnae_principal="1031-7/00",
...
async def test_service_detecta_cnpj_duplicado():
    ...
    payload = dict(
        cnpj="50010020000188",
        razao_social="Metalurgica Benguela SA",
        ramo=RamoIndustrial.METALURGICA,
        porte=PorteIndustrial.GRANDE,
        tipo=TipoEstabelecimento.MATRIZ,
        cnae_principal="2599-3/01",
        data_abertura=date(2026, 2, 1),
        endereco="Zona Industrial A, lote 4",
        bairro="Centro",
        municipio="Lobito",
        provincia="Benguela",
    )
...
def test_endpoint_cadastrar_retorna_201():
    item = EstabelecimentoIndustrial.cadastrar(
        cnpj="50010020000177",
        razao_social="Textil Huambo SA",
        ramo=RamoIndustrial.TEXTIL,
        porte=PorteIndustrial.PEQUENA,
        tipo=TipoEstabelecimento.FILIAL,
        cnae_principal="1311-1/00",
        data_abertura=date(2026, 2, 15),
        endereco="Rua das Industrias, 10",
        bairro="Cidade Alta",
        municipio="Huambo",
        provincia="Huambo",
    )
    ...
    response = client.post(
        "/industria/estabelecimentos_industriais/",
        json={
            "cnpj": item.cnpj,
...
def test_endpoint_obter_por_id_retorna_404():
    service = SimpleNamespace(
        obter_por_id=AsyncMock(
            side_effect=EstabelecimentoIndustrialNotFoundError(
...
def test_http_full_cycle_estabelecimento_industrial():
    ...
    create_resp = client.post(
        ...
        json={
            "cnpj": "50010020000166",
            "razao_social": "Alimentos Sul SA",
            "ramo": "alimentar",
            "porte": "media",
            "tipo": "matriz",
            "cnae_principal": "1099-6/01",
            "data_abertura": "2026-01-20",
            "endereco": "Estrada Nacional KM 12",
    ...
    iniciar_resp = client.patch(
        f"/industria/estabelecimentos_industriais/{item_id}/iniciar_atividades",
        json={"data": "2026-03-01"},
    )
    assert iniciar_resp.status_code == 200
    assert iniciar_resp.json()["status"] == "ativo"
    ramo_resp = client.patch(
        f"/industria/estabelecimentos_industriais/{item_id}/associar_ramo", json={"ramo": "quimica"}
    )
    ...
    porte_resp = client.patch(
        f"/industria/estabelecimentos_industriais/{item_id}/definir_porte", json={"porte": "grande"}
    )
    ...
    assert any(
        ...
        and (item["ramo"] == "quimica")
        and (item["porte"] == "grande")
        for item in itens
    )
...
def test_http_auditoria_mantem_sequencia_eventos():
    ...
    create_resp = client.post(
        ...
        json={
            "cnpj": "50010020000155",
            "razao_social": "Quimica Centro SA",
            "ramo": "transformacao",
            "porte": "pequena",
            "tipo": "matriz",
            "cnae_principal": "2013-4/00",
            "data_abertura": "2026-01-05",
            "endereco": "Rua 5, lote 8",
    ...
    assert (
        client.patch(
            f"/industria/estabelecimentos_industriais/{item_id}/associar_ramo",
            json={"ramo": "quimica"},
        ).status_code
        == 200
    )
    assert (
        client.patch(
            f"/industria/estabelecimentos_industriais/{item_id}/definir_porte",
            json={"porte": "media"},
        ).status_code
        == 200
    )
    assert (
        client.patch(
            f"/industria/estabelecimentos_industriais/{item_id}/suspender_atividades",
            json={"motivo": "Parada tecnica"},
        ).status_code
        == 200
    )
    assert (
        client.patch(f"/industria/estabelecimentos_industriais/{item_id}/reativar").status_code
        == 200
    )
    ...
    expected_markers = [
        "Cadastro inicial do estabelecimento industrial",
        "Transicao de status: licenciamento -> ativo",
        "Ramo associado:",
        "Porte definido:",
        "Transicao de status: ativo -> suspenso",
        "Transicao de status: suspenso/paralisado -> ativo",
    ]
    current_index = 0
