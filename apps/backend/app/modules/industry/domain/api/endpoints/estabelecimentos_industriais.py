...
async def cadastrar_estabelecimento_industrial(data: EstabelecimentoIndustrialCreate, service: EstabelecimentoIndustrialService=Depends(get_estabelecimento_industrial_service)):
    try:
        return await service.cadastrar(cnpj=data.cnpj, razao_social=data.razao_social, ramo=data.ramo, porte=data.porte, tipo=data.tipo, cnae_principal=data.cnae_principal, data_abertura=data.data_abertura, endereco=data.endereco, bairro=data.bairro, municipio=data.municipio, provincia=data.provincia)
...
async def iniciar_atividades(item_id: UUID, data: DataInput, service: EstabelecimentoIndustrialService=Depends(get_estabelecimento_industrial_service)):
    try:
...
async def suspender_atividades(item_id: UUID, data: MotivoInput, service: EstabelecimentoIndustrialService=Depends(get_estabelecimento_industrial_service)):
    try:
...
async def paralisar(item_id: UUID, data: MotivoInput, service: EstabelecimentoIndustrialService=Depends(get_estabelecimento_industrial_service)):
    try:
...
async def reativar(item_id: UUID, service: EstabelecimentoIndustrialService=Depends(get_estabelecimento_industrial_service)):
    try:
...
async def associar_ramo(item_id: UUID, data: RamoInput, service: EstabelecimentoIndustrialService=Depends(get_estabelecimento_industrial_service)):
    try:
...
async def definir_porte(item_id: UUID, data: PorteInput, service: EstabelecimentoIndustrialService=Depends(get_estabelecimento_industrial_service)):
    try:
...
async def obter_por_id(item_id: UUID, service: EstabelecimentoIndustrialService=Depends(get_estabelecimento_industrial_service)):
    try:
...
async def listar(status_estabelecimento: StatusEstabelecimento | None=None, ramo: RamoIndustrial | None=None, municipio: str | None=None, service: EstabelecimentoIndustrialService=Depends(get_estabelecimento_industrial_service)):
