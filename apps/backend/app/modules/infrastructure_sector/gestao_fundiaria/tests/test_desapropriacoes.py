...
async def test_desapropriacao_service_fluxo_sucesso() -> None:
    ...
    service = DesapropriacaoService(
        desapropriacao_repo=SQLAlchemyDesapropriacaoRepository(),
        imovel_repo=imovel_repo,
        ambiente_adapter=_AmbienteValido(),
    )
    imovel = await imovel_service.cadastrar(
        tipo=TipoImovel.RURAL,
        natureza=NaturezaImovel.PRIVADO,
        area_total=Decimal("2000.00"),
        endereco="Estrada 100",
        bairro="Periferia",
        municipio="Huambo",
        provincia="Huambo",
    )
    processo = await service.instaurar(
        imovel_inscricao=imovel.inscricao_imobiliaria,
        tipo=TipoDesapropriacao.REFORMA_AGRARIA,
        ente_publico="MINAGRIF",
        finalidade="Projeto de reforma",
        valor_indenizacao=Decimal("200000.00"),
    )
...
def test_endpoint_instaurar_desapropriacao_retorna_201() -> None:
    mock_item = SimpleNamespace(
        id="060b40bf-0842-4329-aeec-7ea7b4f6ea27",
        numero_processo="DSP/2026/000001",
        imovel_inscricao="IMV/2026/000010",
        tipo=TipoDesapropriacao.UTILIDADE_PUBLICA,
        ente_publico="Governo Provincial",
        finalidade="Construir hospital",
        valor_indenizacao=Decimal("500000.00"),
        data_inicio=date(2026, 3, 4),
        status=StatusDesapropriacao.INSTAURADA,
        ativo=True,
        data_decreto=None,
        data_pagamento=None,
    ...
    service = SimpleNamespace(
        has_ambiente_adapter=lambda: True, instaurar=AsyncMock(return_value=mock_item)
    )
    app = FastAPI()
    ...
    response = client.post(
        "/gestao-fundiaria/desapropriacoes/",
        json={
            "imovel_inscricao": "IMV/2026/000010",
            "tipo": "utilidade_publica",
            "ente_publico": "Governo Provincial",
            "finalidade": "Construir hospital",
            "valor_indenizacao": "500000.00",
        },
    )
...
def test_endpoint_obter_desapropriacao_retorna_404() -> None:
    service = SimpleNamespace(
        obter_por_numero_processo=AsyncMock(
            side_effect=DesapropriacaoNotFoundError("Desapropriacao nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(desapropriacoes_router, prefix="/gestao-fundiaria")
