...
async def test_proprietario_service_fluxo_sucesso():
    ...
    item = await service.cadastrar(
        nome="Joao Manuel",
        documento="BI1234567LA045",
        tipo_pessoa=TipoPessoa.SINGULAR,
        tipo_titularidade=TipoTitularidade.PROPRIETARIO,
        email="joao@example.gov.ao",
    )
    assert item.numero_cadastro.startswith("PRP/")
    assert item.ativo is True
    item = await service.atualizar_titularidade(
        item.numero_cadastro,
        tipo_titularidade=TipoTitularidade.COTITULAR,
        percentual_titularidade=Decimal("50.00"),
    )
    assert item.tipo_titularidade == TipoTitularidade.COTITULAR
    assert item.percentual_titularidade == Decimal("50.00")
    item = await service.atualizar_contato(
        item.numero_cadastro, telefone="+244923000111", endereco="Rua B, 20 - Luanda"
    )
...
def test_endpoint_cadastrar_proprietario_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        numero_cadastro="PRP/2026/000001",
        nome="Joao Manuel",
        documento="BI1234567LA045",
        tipo_pessoa=TipoPessoa.SINGULAR,
        tipo_titularidade=TipoTitularidade.PROPRIETARIO,
    ...
    response = client.post(
        "/gestao-fundiaria/proprietarios/",
        json={
            "nome": "Joao Manuel",
            "documento": "BI1234567LA045",
            "tipo_pessoa": "singular",
            "tipo_titularidade": "proprietario",
            "email": "joao@example.gov.ao",
        },
    )
...
def test_endpoint_obter_proprietario_retorna_404():
    service = SimpleNamespace(
        obter_por_numero_cadastro=AsyncMock(
            side_effect=ProprietarioNotFoundError("Proprietario nao encontrado")
        )
    )
    app = FastAPI()
    app.include_router(proprietarios_router, prefix="/gestao-fundiaria")
