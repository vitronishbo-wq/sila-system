...
async def test_oneracoes_desapropriacoes_repositories_com_orm_real(db_session: AsyncSession) -> None:
    ...
    imovel = await imovel_repo.save(Imovel.cadastrar(tipo=TipoImovel.URBANO, natureza=NaturezaImovel.PRIVADO, area_total=Decimal('750.00'), endereco='Rua do ORM, 1', bairro='Centro', municipio='Luanda', provincia='Luanda', inscricao_imobiliaria=f'IMV/{date.today().year}/{uuid4().hex[:6]}'))
    numero_oneracao = await oneracao_repo.next_numero()
    oneracao = await oneracao_repo.save(Oneracao.registrar(numero_oneracao=numero_oneracao, imovel_inscricao=imovel.inscricao_imobiliaria, tipo=TipoOneracao.HIPOTECA, credor_nome='Banco Integracao', valor=Decimal('250000.00')))
    ...
    desapropriacao = await desapropriacao_repo.save(Desapropriacao.instaurar(numero_processo=numero_processo, imovel_inscricao=imovel.inscricao_imobiliaria, tipo=TipoDesapropriacao.UTILIDADE_PUBLICA, ente_publico='Governo Provincial', finalidade='Ampliacao viaria', valor_indenizacao=Decimal('980000.00')))
    ...
    desapropriacoes_imovel = await desapropriacao_repo.list(imovel_inscricao=imovel.inscricao_imobiliaria)
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_slice_matricula_georreferenciamento_cross_modulo_com_orm_real(db_session: AsyncSession) -> None:
    await _assert_table_exists(db_session, 'gestao_fundiaria_imoveis')
    ...
    imovel = await imovel_repo.save(Imovel.cadastrar(tipo=TipoImovel.RURAL, natureza=NaturezaImovel.PRIVADO, area_total=Decimal('5000.00'), endereco='Via Rural 20', bairro='Zona 7', municipio='Huambo', provincia='Huambo', inscricao_imobiliaria=f'IMV/{date.today().year}/{uuid4().hex[:6]}'))
    justica_adapter = AsyncMock()
    justica_adapter.validar_matricula = AsyncMock(return_value=True)
    matricula_service = MatriculaImovelService(matricula_repo=matricula_repo, imovel_repo=imovel_repo, justica_adapter=justica_adapter)
    geosampa_adapter = AsyncMock()
    geosampa_adapter.validar_coordenadas = AsyncMock(return_value=True)
    georreferenciamento_service = GeorreferenciamentoService(georreferenciamento_repo=geo_repo, imovel_repo=imovel_repo, geosampa_adapter=geosampa_adapter)
    matricula = await matricula_service.registrar(imovel_inscricao=imovel.inscricao_imobiliaria, tipo_registro=TipoRegistro.MATRICULA, cartorio_nome='Cartorio Integracao', livro='10', folha='22', comarca='Huambo', provincia='Huambo', proprietario_documento='BI998877')
    georreferenciamento = await georreferenciamento_service.registrar(imovel_inscricao=imovel.inscricao_imobiliaria, latitude=Decimal('-12.50000000'), longitude=Decimal('15.70000000'), precisao_metros=Decimal('1.00'), area_calculada=Decimal('5000.15'))
