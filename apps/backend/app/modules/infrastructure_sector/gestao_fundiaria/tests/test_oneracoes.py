...
async def test_oneracao_service_fluxo_sucesso() -> None:
    ...
    oneracao_service = OneracaoService(oneracao_repo=SQLAlchemyOneracaoRepository(), imovel_repo=imovel_repo, justica_adapter=_JusticaComLitigio())
    imovel = await imovel_service.cadastrar(tipo=TipoImovel.URBANO, natureza=NaturezaImovel.PRIVADO, area_total=Decimal('300.00'), endereco='Rua 1', bairro='Centro', municipio='Luanda', provincia='Luanda')
    item = await oneracao_service.registrar(imovel_inscricao=imovel.inscricao_imobiliaria, tipo=TipoOneracao.PENHORA, credor_nome='Banco Publico', valor=Decimal('15000.00'))
...
def test_endpoint_registrar_oneracao_retorna_201() -> None:
    mock_item = SimpleNamespace(id='3f2eb5f0-8308-45a5-a357-f6c2d2f8facc', numero_oneracao='ONR/2026/000001', imovel_inscricao='IMV/2026/000001', tipo=TipoOneracao.HIPOTECA, credor_nome='Banco X', valor=Decimal('1000.00'), data_registro=date(2026, 3, 4), status=StatusOneracao.ATIVA, ativo=True, documento_credor=None, moeda='AOA', data_vencimento=None, descricao=None, data_atualizacao=None, observacoes=None)
    ...
    response = client.post('/gestao-fundiaria/oneracoes/', json={'imovel_inscricao': 'IMV/2026/000001', 'tipo': 'hipoteca', 'credor_nome': 'Banco X', 'valor': '1000.00'})
...
def test_endpoint_penhora_sem_adapter_retorna_503() -> None:
    ...
    app.dependency_overrides[get_oneracao_service] = lambda: service
    client = TestClient(app)
    response = client.post('/gestao-fundiaria/oneracoes/', json={'imovel_inscricao': 'IMV/2026/000001', 'tipo': 'penhora', 'credor_nome': 'Tribunal', 'valor': '1000.00'})
...
def test_endpoint_obter_oneracao_retorna_404() -> None:
    service = SimpleNamespace(obter_por_numero=AsyncMock(side_effect=OneracaoNotFoundError('Oneracao nao encontrada')))
    app = FastAPI()
    app.include_router(oneracoes_router, prefix='/gestao-fundiaria')
    app.dependency_overrides[get_oneracao_service] = lambda: service
    client = TestClient(app)
