...
async def test_imovel_service_fluxo_sucesso():
    ...
    item = await service.atualizar_proprietario(
        item.inscricao_imobiliaria, proprietario_id=proprietario_id
    )
...
def test_endpoint_cadastrar_imovel_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        inscricao_imobiliaria="IMV/2026/000001",
        tipo=TipoImovel.URBANO,
        natureza=NaturezaImovel.PRIVADO,
...
def test_endpoint_obter_imovel_retorna_404():
    service = SimpleNamespace(
        obter_por_inscricao=AsyncMock(side_effect=ImovelNotFoundError("Imovel nao encontrado"))
    )
    app = FastAPI()
    app.include_router(imoveis_router, prefix="/gestao-fundiaria")
    app.dependency_overrides[get_imovel_service] = lambda: service
