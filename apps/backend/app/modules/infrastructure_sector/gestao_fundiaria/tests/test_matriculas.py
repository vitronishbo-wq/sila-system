...
async def test_matricula_service_fluxo_sucesso() -> None:
    ...
    service = MatriculaImovelService(matricula_repo=SQLAlchemyMatriculaImovelRepository(), imovel_repo=imovel_repo, justica_adapter=_JusticaValida())
    imovel = await imovel_service.cadastrar(tipo=TipoImovel.URBANO, natureza=NaturezaImovel.PRIVADO, area_total=Decimal('450.00'), endereco='Rua da Matricula, 10', bairro='Centro', municipio='Huambo', provincia='Huambo')
    matricula = await service.registrar(imovel_inscricao=imovel.inscricao_imobiliaria, tipo_registro=TipoRegistro.MATRICULA, cartorio_nome='Cartorio Central', livro='L1', folha='F100', comarca='Huambo', provincia='Huambo', proprietario_documento='BI123456')
...
def test_endpoint_registrar_matricula_retorna_201() -> None:
    mock_item = SimpleNamespace(id=uuid4(), numero_matricula='MAT/2026/000001', imovel_inscricao='IMV/2026/000001', tipo_registro=TipoRegistro.MATRICULA, cartorio_nome='Cartorio Central', livro='L1', folha='F100', comarca='Huambo', provincia='Huambo', data_registro=date(2026, 3, 4), status=StatusMatriculaImovel.ATIVA, ativo=True, proprietario_documento='BI123456', data_atualizacao=None, observacoes=None)
    service = SimpleNamespace(has_justica_adapter=lambda: True, registrar=AsyncMock(return_value=mock_item))
    app = FastAPI()
    ...
    response = client.post('/gestao-fundiaria/matriculas/', json={'imovel_inscricao': 'IMV/2026/000001', 'tipo_registro': 'matricula', 'cartorio_nome': 'Cartorio Central', 'livro': 'L1', 'folha': 'F100', 'comarca': 'Huambo', 'provincia': 'Huambo', 'proprietario_documento': 'BI123456'})
...
def test_endpoint_registrar_matricula_sem_adapter_retorna_503() -> None:
    ...
    app.dependency_overrides[get_matricula_service] = lambda: service
    client = TestClient(app)
    response = client.post('/gestao-fundiaria/matriculas/', json={'imovel_inscricao': 'IMV/2026/000001', 'tipo_registro': 'matricula', 'cartorio_nome': 'Cartorio Central', 'livro': 'L1', 'folha': 'F100', 'comarca': 'Huambo', 'provincia': 'Huambo'})
...
def test_endpoint_obter_matricula_retorna_404() -> None:
    service = SimpleNamespace(obter_por_numero=AsyncMock(side_effect=MatriculaImovelNotFoundError('Matricula nao encontrada')))
    app = FastAPI()
    app.include_router(matriculas_router, prefix='/gestao-fundiaria')
    app.dependency_overrides[get_matricula_service] = lambda: service
    client = TestClient(app)
