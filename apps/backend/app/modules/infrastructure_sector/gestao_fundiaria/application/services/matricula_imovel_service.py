...
class MatriculaImovelService:
    ...
    def __init__(self, *, matricula_repo: MatriculaImovelRepositoryPort, imovel_repo: ImovelRepositoryPort, justica_adapter: JusticaServicePort | None=None) -> None:
    ...
    async def registrar(self, *, imovel_inscricao: str, tipo_registro: TipoRegistro, cartorio_nome: str, livro: str, folha: str, comarca: str, provincia: str, proprietario_documento: str | None=None, numero_matricula: str | None=None) -> MatriculaImovel:
        ...
        if self._justica_adapter:
            valido = await self._justica_adapter.validar_matricula(numero_matricula=numero, cartorio_nome=cartorio_nome, livro=livro, folha=folha)
            if not valido:
                raise ValueError('Matricula invalida segundo o modulo Justica')
        matricula = MatriculaImovel.registrar(numero_matricula=numero, imovel_inscricao=imovel_inscricao, tipo_registro=tipo_registro, cartorio_nome=cartorio_nome, livro=livro, folha=folha, comarca=comarca, provincia=provincia, proprietario_documento=proprietario_documento)
    ...
    async def listar(self, *, imovel_inscricao: str | None=None, status: StatusMatriculaImovel | None=None, ativo: bool | None=None) -> list[MatriculaImovel]:
        return await self._matricula_repo.list(imovel_inscricao=imovel_inscricao, status=status, ativo=ativo)
