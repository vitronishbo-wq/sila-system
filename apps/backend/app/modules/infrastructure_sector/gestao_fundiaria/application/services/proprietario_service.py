...
class ProprietarioService:
    ...
    async def cadastrar(self, *, nome: str, documento: str, tipo_pessoa: TipoPessoa, tipo_titularidade: TipoTitularidade, percentual_titularidade: Decimal | None=None, email: str | None=None, telefone: str | None=None, endereco: str | None=None) -> Proprietario:
        ...
        item = Proprietario.cadastrar(nome=nome, documento=documento, tipo_pessoa=tipo_pessoa, tipo_titularidade=tipo_titularidade, numero_cadastro=await self._proprietario_repo.next_numero_cadastro(), percentual_titularidade=percentual_titularidade, email=email, telefone=telefone, endereco=endereco)
        return await self._proprietario_repo.save(item)

    async def atualizar_contato(self, numero_cadastro: str, *, email: str | None=None, telefone: str | None=None, endereco: str | None=None) -> Proprietario:
        item = await self._obter_ou_erro(numero_cadastro)
        item.atualizar_contato(email=email, telefone=telefone, endereco=endereco)
        return await self._proprietario_repo.save(item)

    async def atualizar_titularidade(self, numero_cadastro: str, *, tipo_titularidade: TipoTitularidade, percentual_titularidade: Decimal | None=None) -> Proprietario:
        item = await self._obter_ou_erro(numero_cadastro)
        item.atualizar_titularidade(tipo_titularidade=tipo_titularidade, percentual_titularidade=percentual_titularidade)
        return await self._proprietario_repo.save(item)
    ...
    async def listar(self, *, tipo_pessoa: str | None=None, ativo: bool | None=None) -> list[Proprietario]:
