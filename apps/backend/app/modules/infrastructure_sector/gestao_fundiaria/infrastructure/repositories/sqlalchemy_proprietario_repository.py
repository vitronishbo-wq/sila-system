...
class SQLAlchemyProprietarioRepository(ProprietarioRepositoryPort):
    ...
    async def save(self, item: Proprietario) -> Proprietario:
        if self._session:
            existing = await self._session.execute(select(ProprietarioModel).where(ProprietarioModel.numero_cadastro == item.numero_cadastro))
            model = existing.scalars().first()
            if model is None:
                model = ProprietarioModel(id=item.id, numero_cadastro=item.numero_cadastro, nome=item.nome, documento=item.documento, tipo_pessoa=item.tipo_pessoa.value, tipo_titularidade=item.tipo_titularidade.value, data_cadastro=item.data_cadastro, ativo=item.ativo, percentual_titularidade=item.percentual_titularidade, email=item.email, telefone=item.telefone, endereco=item.endereco, data_atualizacao=item.data_atualizacao, observacoes=item.observacoes)
    ...
    async def get_by_numero_cadastro(self, numero_cadastro: str) -> Proprietario | None:
        if self._session:
            result = await self._session.execute(select(ProprietarioModel).where(ProprietarioModel.numero_cadastro == numero_cadastro))
    ...
    async def get_by_documento(self, documento: str) -> Proprietario | None:
        if self._session:
            result = await self._session.execute(select(ProprietarioModel).where(ProprietarioModel.documento == documento))
            model = result.scalars().first()
            return self._to_domain(model) if model else None
    ...
    async def list(self, *, tipo_pessoa: str | None=None, ativo: bool | None=None) -> list[Proprietario]:
        if self._session:
            statement = select(ProprietarioModel)
            if tipo_pessoa:
                statement = statement.where(func.lower(ProprietarioModel.tipo_pessoa) == tipo_pessoa.strip().lower())
            if ativo is not None:
                statement = statement.where(ProprietarioModel.ativo == ativo)
            result = await self._session.execute(statement.order_by(ProprietarioModel.numero_cadastro.asc()))
    ...
    async def next_numero_cadastro(self) -> str:
        if self._session:
            ...
            prefix = f'PRP/{year}/'
            result = await self._session.execute(select(func.count()).select_from(ProprietarioModel).where(ProprietarioModel.numero_cadastro.like(f'{prefix}%')))
    ...
    def _to_domain(model: ProprietarioModel) -> Proprietario:
        return Proprietario(id=model.id, numero_cadastro=model.numero_cadastro, nome=model.nome, documento=model.documento, tipo_pessoa=TipoPessoa(model.tipo_pessoa), tipo_titularidade=TipoTitularidade(model.tipo_titularidade), data_cadastro=model.data_cadastro, ativo=model.ativo, percentual_titularidade=Decimal(model.percentual_titularidade) if model.percentual_titularidade is not None else None, email=model.email, telefone=model.telefone, endereco=model.endereco, data_atualizacao=model.data_atualizacao, observacoes=model.observacoes)