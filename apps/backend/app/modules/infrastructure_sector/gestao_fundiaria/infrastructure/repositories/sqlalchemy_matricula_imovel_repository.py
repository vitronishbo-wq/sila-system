...
from apps.backend.app.modules.infrastructure_sector.gestao_fundiaria.infrastructure.models.matricula_imovel_model import (
    MatriculaImovelModel,
)
...
class SQLAlchemyMatriculaImovelRepository(MatriculaImovelRepositoryPort):
    ...
    async def save(self, item: MatriculaImovel) -> MatriculaImovel:
        if self._session:
            existing = await self._session.execute(
                select(MatriculaImovelModel).where(
                    MatriculaImovelModel.numero_matricula == item.numero_matricula
                )
            )
            model = existing.scalars().first()
            if model is None:
                model = MatriculaImovelModel(
                    id=item.id,
                    ...
                    ativo=item.ativo,
                    proprietario_documento=item.proprietario_documento,
    ...
    async def get_by_numero(self, numero_matricula: str) -> MatriculaImovel | None:
        if self._session:
            result = await self._session.execute(
                select(MatriculaImovelModel).where(
                    MatriculaImovelModel.numero_matricula == numero_matricula
                )
            )
        ...
        if self._session:
            ...
            if imovel_inscricao:
                statement = statement.where(
                    MatriculaImovelModel.imovel_inscricao == imovel_inscricao.strip()
                )
            if status:
            ...
            result = await self._session.execute(
                statement.order_by(MatriculaImovelModel.numero_matricula.asc())
            )
    ...
    async def next_numero(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(MatriculaImovelModel)
                .where(MatriculaImovelModel.numero_matricula.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: MatriculaImovelModel) -> MatriculaImovel:
        return MatriculaImovel(
            id=model.id,
            numero_matricula=model.numero_matricula,
            imovel_inscricao=model.imovel_inscricao,
            tipo_registro=TipoRegistro(model.tipo_registro),
            cartorio_nome=model.cartorio_nome,
            livro=model.livro,
            folha=model.folha,
            comarca=model.comarca,
            provincia=model.provincia,
            data_registro=model.data_registro,
            status=StatusMatriculaImovel(model.status),
            ativo=model.ativo,
            proprietario_documento=model.proprietario_documento,
