...
from apps.backend.app.modules.infrastructure_sector.gestao_fundiaria.infrastructure.models.imovel_model import (
    ImovelModel,
)
...
class SQLAlchemyImovelRepository(ImovelRepositoryPort):
        ...
        if self._session:
            existing = await self._session.execute(
                select(ImovelModel).where(
                    ImovelModel.inscricao_imobiliaria == item.inscricao_imobiliaria
                )
            )
            ...
            if model is None:
                model = ImovelModel(
                    ...
                    proprietario_atual_id=item.proprietario_atual_id,
    ...
    async def get_by_inscricao(self, inscricao_imobiliaria: str) -> Imovel | None:
        if self._session:
            result = await self._session.execute(
                select(ImovelModel).where(
                    ImovelModel.inscricao_imobiliaria == inscricao_imobiliaria
                )
            )
        ...
        if self._session:
            ...
            if proprietario_atual_id:
                statement = statement.where(
                    ImovelModel.proprietario_atual_id == proprietario_atual_id
                )
            if municipio:
                statement = statement.where(
                    func.lower(ImovelModel.municipio) == municipio.strip().lower()
                )
            if provincia:
                statement = statement.where(
                    func.lower(ImovelModel.provincia) == provincia.strip().lower()
                )
            if ativo is not None:
                statement = statement.where(ImovelModel.ativo == ativo)
            result = await self._session.execute(
                statement.order_by(ImovelModel.inscricao_imobiliaria.asc())
            )
            return [self._to_domain(model) for model in result.scalars().all()]
        values = list(self._items.values())
        if proprietario_atual_id:
            values = [
                item for item in values if item.proprietario_atual_id == proprietario_atual_id
            ]
        if municipio:
    ...
    async def next_inscricao(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(ImovelModel)
                .where(ImovelModel.inscricao_imobiliaria.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: ImovelModel) -> Imovel:
        return Imovel(
            id=model.id,
            inscricao_imobiliaria=model.inscricao_imobiliaria,
            tipo=TipoImovel(model.tipo),
            natureza=NaturezaImovel(model.natureza),
            regime=RegimePropriedade(model.regime),
            situacao=SituacaoDominial(model.situacao),
            ...
            area_privativa=Decimal(model.area_privativa)
            if model.area_privativa is not None
            else None,
            area_construida=Decimal(model.area_construida)
            if model.area_construida is not None
            else None,
            area_terreno=Decimal(model.area_terreno) if model.area_terreno is not None else None,
            cep=model.cep,
            ...
            proprietario_atual_id=model.proprietario_atual_id,
