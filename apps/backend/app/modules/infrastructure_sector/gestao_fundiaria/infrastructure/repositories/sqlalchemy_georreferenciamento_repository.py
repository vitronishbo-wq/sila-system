...
class SQLAlchemyGeorreferenciamentoRepository(GeorreferenciamentoRepositoryPort):
    ...
    async def save(self, item: Georreferenciamento) -> Georreferenciamento:
        if self._session:
            existing = await self._session.execute(select(GeorreferenciamentoModel).where(GeorreferenciamentoModel.codigo_geo == item.codigo_geo))
            model = existing.scalars().first()
            if model is None:
                model = GeorreferenciamentoModel(id=item.id, codigo_geo=item.codigo_geo, imovel_inscricao=item.imovel_inscricao, latitude=item.latitude, longitude=item.longitude, sistema_referencia=item.sistema_referencia, data_registro=item.data_registro, precisao_metros=item.precisao_metros, area_calculada=item.area_calculada, validado=item.validado, ativo=item.ativo, data_atualizacao=item.data_atualizacao, observacoes=item.observacoes)
    ...
    async def get_by_codigo(self, codigo_geo: str) -> Georreferenciamento | None:
        if self._session:
            result = await self._session.execute(select(GeorreferenciamentoModel).where(GeorreferenciamentoModel.codigo_geo == codigo_geo))
    ...
    async def list(self, *, imovel_inscricao: str | None=None, validado: bool | None=None, ativo: bool | None=None) -> list[Georreferenciamento]:
        if self._session:
            ...
            if imovel_inscricao:
                statement = statement.where(GeorreferenciamentoModel.imovel_inscricao == imovel_inscricao.strip())
            ...
            result = await self._session.execute(statement.order_by(GeorreferenciamentoModel.codigo_geo.asc()))
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            prefix = f'GEO/{year}/'
            result = await self._session.execute(select(func.count()).select_from(GeorreferenciamentoModel).where(GeorreferenciamentoModel.codigo_geo.like(f'{prefix}%')))
    ...
    def _to_domain(model: GeorreferenciamentoModel) -> Georreferenciamento:
        return Georreferenciamento(id=model.id, codigo_geo=model.codigo_geo, imovel_inscricao=model.imovel_inscricao, latitude=Decimal(model.latitude), longitude=Decimal(model.longitude), sistema_referencia=model.sistema_referencia, data_registro=model.data_registro, precisao_metros=Decimal(model.precisao_metros) if model.precisao_metros is not None else None, area_calculada=Decimal(model.area_calculada) if model.area_calculada is not None else None, validado=model.validado, ativo=model.ativo, data_atualizacao=model.data_atualizacao, observacoes=model.observacoes)