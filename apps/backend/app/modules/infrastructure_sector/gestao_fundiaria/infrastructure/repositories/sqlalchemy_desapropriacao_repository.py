...
from apps.backend.app.modules.infrastructure_sector.gestao_fundiaria.infrastructure.models.desapropriacao_model import (
    DesapropriacaoModel,
)
...
class SQLAlchemyDesapropriacaoRepository(DesapropriacaoRepositoryPort):
    ...
    async def save(self, item: Desapropriacao) -> Desapropriacao:
        if self._session:
            existing = await self._session.execute(
                select(DesapropriacaoModel).where(
                    DesapropriacaoModel.numero_processo == item.numero_processo
                )
            )
            model = existing.scalars().first()
            if model is None:
                model = DesapropriacaoModel(
                    id=item.id,
                    ...
                    valor_indenizacao=item.valor_indenizacao,
                    data_inicio=item.data_inicio,
                    status=item.status.value,
                    ativo=item.ativo,
                    data_decreto=item.data_decreto,
    ...
    async def get_by_numero_processo(self, numero_processo: str) -> Desapropriacao | None:
        if self._session:
            result = await self._session.execute(
                select(DesapropriacaoModel).where(
                    DesapropriacaoModel.numero_processo == numero_processo
                )
            )
        ...
        if self._session:
            ...
            if imovel_inscricao:
                statement = statement.where(
                    DesapropriacaoModel.imovel_inscricao == imovel_inscricao.strip()
                )
            if status:
            ...
            result = await self._session.execute(
                statement.order_by(DesapropriacaoModel.numero_processo.asc())
            )
    ...
    async def next_numero_processo(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(DesapropriacaoModel)
                .where(DesapropriacaoModel.numero_processo.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: DesapropriacaoModel) -> Desapropriacao:
        return Desapropriacao(
            ...
            tipo=TipoDesapropriacao(model.tipo),
            ente_publico=model.ente_publico,
            finalidade=model.finalidade,
            valor_indenizacao=Decimal(model.valor_indenizacao),
            data_inicio=model.data_inicio,
            status=StatusDesapropriacao(model.status),
            ativo=model.ativo,
            data_decreto=model.data_decreto,
