...
from apps.backend.app.modules.infrastructure_sector.gestao_fundiaria.infrastructure.models.oneracao_model import (
    OneracaoModel,
)
...
class SQLAlchemyOneracaoRepository(OneracaoRepositoryPort):
    ...
    async def save(self, item: Oneracao) -> Oneracao:
        if self._session:
            existing = await self._session.execute(
                select(OneracaoModel).where(OneracaoModel.numero_oneracao == item.numero_oneracao)
            )
            model = existing.scalars().first()
            if model is None:
                model = OneracaoModel(
                    id=item.id,
                    numero_oneracao=item.numero_oneracao,
    ...
    async def get_by_numero(self, numero_oneracao: str) -> Oneracao | None:
        if self._session:
            result = await self._session.execute(
                select(OneracaoModel).where(OneracaoModel.numero_oneracao == numero_oneracao)
            )
        ...
        if self._session:
            ...
            if imovel_inscricao:
                statement = statement.where(
                    OneracaoModel.imovel_inscricao == imovel_inscricao.strip()
                )
            if status:
            ...
            result = await self._session.execute(
                statement.order_by(OneracaoModel.numero_oneracao.asc())
            )
    ...
    async def next_numero(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(OneracaoModel)
                .where(OneracaoModel.numero_oneracao.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: OneracaoModel) -> Oneracao:
        return Oneracao(
            id=model.id,
            numero_oneracao=model.numero_oneracao,
            imovel_inscricao=model.imovel_inscricao,
            tipo=TipoOneracao(model.tipo),
            credor_nome=model.credor_nome,
            valor=Decimal(model.valor),
            data_registro=model.data_registro,
            status=StatusOneracao(model.status),
            ativo=model.ativo,
            documento_credor=model.documento_credor,
            moeda=model.moeda,
