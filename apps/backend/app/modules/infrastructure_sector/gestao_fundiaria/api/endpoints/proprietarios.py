...
proprietario_service_dep = Depends(get_proprietario_service)


@router.post("/", response_model=ProprietarioResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_proprietario(
    data: ProprietarioCreate, service: ProprietarioService = proprietario_service_dep
):
    try:
        return await service.cadastrar(
            ...
            tipo_titularidade=data.tipo_titularidade,
            percentual_titularidade=data.percentual_titularidade,
            email=data.email,
...
async def atualizar_contato_proprietario(
    numero_cadastro: str,
    data: ProprietarioContatoInput,
    service: ProprietarioService = proprietario_service_dep,
):
    try:
        return await service.atualizar_contato(
            numero_cadastro, email=data.email, telefone=data.telefone, endereco=data.endereco
        )
...
async def atualizar_titularidade_proprietario(
    numero_cadastro: str,
    data: ProprietarioTitularidadeInput,
    service: ProprietarioService = proprietario_service_dep,
):
    try:
...
async def desativar_proprietario(
    numero_cadastro: str,
    data: ProprietarioMotivoInput,
    service: ProprietarioService = proprietario_service_dep,
):
    try:
...
async def obter_proprietario(
    numero_cadastro: str, service: ProprietarioService = proprietario_service_dep
):
    try:
