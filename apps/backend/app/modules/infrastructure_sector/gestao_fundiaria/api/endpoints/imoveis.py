...
async def atualizar_area_imovel(
    inscricao_imobiliaria: str,
    data: ImovelAreaInput,
    service: ImovelService = imovel_service_dep,
):
    try:
...
async def atualizar_proprietario_imovel(
    inscricao_imobiliaria: str,
    data: ImovelProprietarioInput,
    service: ImovelService = imovel_service_dep,
):
    try:
        return await service.atualizar_proprietario(
            inscricao_imobiliaria, proprietario_id=data.proprietario_id
        )
    except ImovelNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def atualizar_situacao_imovel(
    inscricao_imobiliaria: str,
    data: ImovelSituacaoInput,
    service: ImovelService = imovel_service_dep,
):
    try:
...
async def vincular_matricula_imovel(
    inscricao_imobiliaria: str,
    data: ImovelMatriculaInput,
    service: ImovelService = imovel_service_dep,
):
    try:
        return await service.vincular_matricula(
            inscricao_imobiliaria, matricula_id=data.matricula_id
        )
    except ImovelNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def desativar_imovel(
    inscricao_imobiliaria: str,
    data: ImovelMotivoInput,
    service: ImovelService = imovel_service_dep,
):
    try:
...
async def obter_imovel(
    inscricao_imobiliaria: str, service: ImovelService = imovel_service_dep
):
    try:
...
async def listar_imoveis(
    ...
    return await service.listar(
        proprietario_atual_id=proprietario_atual_id,
        municipio=municipio,
        provincia=provincia,
        ativo=ativo,
    )
