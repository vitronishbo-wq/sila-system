...
def _ensure_justica_adapter(service: MatriculaImovelService) -> None:
    if not service.has_justica_adapter():
        raise HTTPException(
            ...
            detail="Adapter de Justica indisponivel para operacoes de matricula imobiliaria. Verifique a integracao do modulo.",
        )


@router.post("/", response_model=MatriculaResponse, status_code=status.HTTP_201_CREATED)
async def registrar_matricula(
    data: MatriculaCreate, service: MatriculaImovelService = matricula_service_dep
):
...
async def transferir_matricula(
    numero_matricula: str,
    data: MatriculaTransferenciaInput,
    service: MatriculaImovelService = matricula_service_dep,
):
    try:
...
async def obter_matricula(
    numero_matricula: str, service: MatriculaImovelService = matricula_service_dep
):
    try:
