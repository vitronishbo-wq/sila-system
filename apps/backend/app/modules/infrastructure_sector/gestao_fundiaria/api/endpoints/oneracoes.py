...
oneracao_service_dep = Depends(get_oneracao_service)
...
def _ensure_adapter_for_penhora(service: OneracaoService, tipo: TipoOneracao) -> None:
    if tipo == TipoOneracao.PENHORA and (not service.has_justica_adapter()):
        raise HTTPException(
            ...
            detail="Adapter de Justica indisponivel para registrar oneracao do tipo 'penhora'. Verifique a integracao do modulo.",
        )


@router.post("/", response_model=OneracaoResponse, status_code=status.HTTP_201_CREATED)
async def registrar_oneracao(
    data: OneracaoCreate, service: OneracaoService = oneracao_service_dep
):
    ...
    try:
        return await service.registrar(
            ...
            valor=data.valor,
            documento_credor=data.documento_credor,
            data_vencimento=data.data_vencimento,
            descricao=data.descricao,
            numero_oneracao=data.numero_oneracao,
        )
...
async def baixar_oneracao(
    numero_oneracao: str,
    data: OneracaoMotivoInput,
    service: OneracaoService = oneracao_service_dep,
):
    try:
...
async def obter_oneracao(
    numero_oneracao: str, service: OneracaoService = oneracao_service_dep
):
    try:
