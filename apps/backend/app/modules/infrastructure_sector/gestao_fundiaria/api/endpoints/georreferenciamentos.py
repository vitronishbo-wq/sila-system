...
from apps.backend.app.modules.infrastructure_sector.gestao_fundiaria.api.schemas.georreferenciamento_schema import (
    GeorreferenciamentoCreate,
    GeorreferenciamentoMotivoInput,
    GeorreferenciamentoPontoInput,
...
georreferenciamento_service_dep = Depends(get_georreferenciamento_service)
...
def _ensure_geosampa_adapter(service: GeorreferenciamentoService) -> None:
    if not service.has_geosampa_adapter():
        raise HTTPException(
            ...
            detail="Adapter de Geosampa indisponivel para operacoes de georreferenciamento. Verifique a integracao do modulo.",
        )


@router.post("/", response_model=GeorreferenciamentoResponse, status_code=status.HTTP_201_CREATED)
async def registrar_georreferenciamento(
    data: GeorreferenciamentoCreate,
    service: GeorreferenciamentoService = georreferenciamento_service_dep,
):
    ...
    try:
        return await service.registrar(
            ...
            sistema_referencia=data.sistema_referencia,
            precisao_metros=data.precisao_metros,
            area_calculada=data.area_calculada,
            codigo_geo=data.codigo_geo,
        )
...
async def atualizar_ponto_georreferenciado(
    codigo_geo: str,
    data: GeorreferenciamentoPontoInput,
    service: GeorreferenciamentoService = georreferenciamento_service_dep,
):
    _ensure_geosampa_adapter(service)
    try:
        return await service.atualizar_ponto(
            codigo_geo, latitude=data.latitude, longitude=data.longitude
        )
...
async def invalidar_georreferenciamento(
    codigo_geo: str,
    data: GeorreferenciamentoMotivoInput,
    service: GeorreferenciamentoService = georreferenciamento_service_dep,
):
    try:
...
async def obter_georreferenciamento(
    codigo_geo: str, service: GeorreferenciamentoService = georreferenciamento_service_dep
):
    try:
...
async def listar_georreferenciamentos(
