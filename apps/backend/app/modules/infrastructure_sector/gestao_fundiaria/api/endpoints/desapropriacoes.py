...
from apps.backend.app.modules.infrastructure_sector.gestao_fundiaria.api.deps import (
    get_desapropriacao_service,
)
from apps.backend.app.modules.infrastructure_sector.gestao_fundiaria.api.schemas.desapropriacao_schema import (
    DesapropriacaoCreate,
    DesapropriacaoDecretoInput,
    DesapropriacaoMotivoInput,
    DesapropriacaoPagamentoInput,
    DesapropriacaoResponse,
)
...
from apps.backend.app.modules.infrastructure_sector.gestao_fundiaria.exceptions import (
    DesapropriacaoAlreadyExistsError,
    DesapropriacaoNotFoundError,
    ImovelNotFoundError,
)

router = APIRouter(prefix="/desapropriacoes", tags=["Gestao Fundiaria - Desapropriacoes"])
desapropriacao_service_dep = Depends(get_desapropriacao_service)


def _ensure_adapter_for_reforma_agraria(
    service: DesapropriacaoService, tipo: TipoDesapropriacao
) -> None:
    if tipo == TipoDesapropriacao.REFORMA_AGRARIA and (not service.has_ambiente_adapter()):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Ambiente indisponivel para desapropriacao do tipo 'reforma_agraria'. Verifique a integracao do modulo.",
        )


@router.post("/", response_model=DesapropriacaoResponse, status_code=status.HTTP_201_CREATED)
async def instaurar_desapropriacao(
    data: DesapropriacaoCreate, service: DesapropriacaoService = desapropriacao_service_dep
):
    _ensure_adapter_for_reforma_agraria(service, data.tipo)
    try:
        return await service.instaurar(
            imovel_inscricao=data.imovel_inscricao,
            tipo=data.tipo,
            ente_publico=data.ente_publico,
...
async def decretar_desapropriacao(
    numero_processo: str,
    data: DesapropriacaoDecretoInput,
    service: DesapropriacaoService = desapropriacao_service_dep,
):
    try:
...
async def registrar_pagamento_desapropriacao(
    numero_processo: str,
    data: DesapropriacaoPagamentoInput,
    service: DesapropriacaoService = desapropriacao_service_dep,
):
    try:
        return await service.registrar_pagamento(
            numero_processo, data_pagamento=data.data_pagamento
        )
    except DesapropriacaoNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except ValueError as exc:
...
async def encerrar_desapropriacao(
    numero_processo: str,
    data: DesapropriacaoMotivoInput,
    service: DesapropriacaoService = desapropriacao_service_dep,
):
    try:
...
async def cancelar_desapropriacao(
    numero_processo: str,
    data: DesapropriacaoMotivoInput,
    service: DesapropriacaoService = desapropriacao_service_dep,
):
...
async def obter_desapropriacao(
    numero_processo: str, service: DesapropriacaoService = desapropriacao_service_dep
):
    try:
...
async def listar_desapropriacoes(
