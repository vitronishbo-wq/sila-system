...
class MatriculaImovel:
    ...
    @classmethod
    def registrar(cls, *, numero_matricula: str, imovel_inscricao: str, tipo_registro: TipoRegistro, cartorio_nome: str, livro: str, folha: str, comarca: str, provincia: str, proprietario_documento: str | None=None) -> 'MatriculaImovel':
        ...
        return cls(id=uuid4(), numero_matricula=numero_matricula.strip(), imovel_inscricao=imovel_inscricao.strip(), tipo_registro=tipo_registro, cartorio_nome=cartorio_nome.strip(), livro=livro.strip(), folha=folha.strip(), comarca=comarca.strip(), provincia=provincia.strip(), data_registro=date.today(), status=StatusMatriculaImovel.ATIVA, ativo=True, proprietario_documento=proprietario_documento.strip() if proprietario_documento else None)
