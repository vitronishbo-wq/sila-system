...
class Oneracao:
    ...
    @classmethod
    def registrar(cls, *, numero_oneracao: str, imovel_inscricao: str, tipo: TipoOneracao, credor_nome: str, valor: Decimal, documento_credor: str | None=None, data_vencimento: date | None=None, descricao: str | None=None) -> 'Oneracao':
        ...
        return cls(id=uuid4(), numero_oneracao=numero_oneracao.strip(), imovel_inscricao=imovel_inscricao.strip(), tipo=tipo, credor_nome=credor_nome.strip(), valor=valor.quantize(Decimal('0.01')), data_registro=date.today(), documento_credor=documento_credor.strip() if documento_credor else None, data_vencimento=data_vencimento, descricao=descricao.strip() if descricao else None, status=StatusOneracao.ATIVA, ativo=True)
