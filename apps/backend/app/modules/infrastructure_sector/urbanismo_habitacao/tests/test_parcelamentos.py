...
async def test_parcelamento_service_fluxo_sucesso():
    ...
    item = await service.criar(
        nome="Parcelamento ZRU Sul",
        tipo=TipoParcelamento.LOTEAMENTO,
        plano_diretor_id=uuid4(),
        zoneamento_id=uuid4(),
        provincia="Luanda",
        area_total=Decimal("250000.00"),
        quantidade_unidades_prevista=220,
        municipio="Luanda",
    )
...
def test_endpoint_criar_parcelamento_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_parcelamento="PAR/2026/000001",
        nome="Parcelamento ZRU Sul",
        tipo=TipoParcelamento.LOTEAMENTO,
        ...
        quantidade_unidades_prevista=220,
        municipio="Luanda",
        area_publica_prevista=None,
        area_sistema_viario_prevista=None,
        quantidade_unidades_resultante=None,
    ...
    response = client.post(
        "/urbanismo-habitacao/parcelamentos/",
        json={
            "nome": "Parcelamento ZRU Sul",
            "tipo": "loteamento",
...
def test_endpoint_obter_parcelamento_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(
            side_effect=ParcelamentoNotFoundError("Parcelamento nao encontrado")
        )
    )
    app = FastAPI()
    app.include_router(parcelamentos_router, prefix="/urbanismo-habitacao")
