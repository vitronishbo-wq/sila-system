...
async def test_operacao_urbana_service_fluxo_sucesso():
    ...
    item = await service.atualizar_execucao(
        item.codigo_operacao,
        percentual_execucao=Decimal("42.50"),
        investimento_executado=Decimal("45000000.00"),
    )
    ...
    item = await service.concluir(
        item.codigo_operacao, data_fim_real=date.today() + timedelta(days=1)
    )
...
def test_endpoint_criar_operacao_urbana_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_operacao="OPU/2026/000001",
        nome="Operacao Integrada Marginal Sul",
        tipo=TipoOperacaoUrbana.OPERACAO_URBANA_CONSORCIADA,
        status=StatusOperacaoUrbana.ELABORACAO,
        plano_diretor_id=uuid4(),
        orgao_responsavel_id=uuid4(),
        provincia="Luanda",
        municipio="Luanda",
        area_intervencao=None,
        investimento_previsto=Decimal("150000000.00"),
        investimento_executado=None,
        data_inicio_prevista=None,
        data_fim_prevista=None,
        data_inicio_real=None,
        data_fim_real=None,
        percentual_execucao=Decimal("0"),
    ...
    response = client.post(
        "/urbanismo-habitacao/operacoes-urbanas/",
        json={
            "nome": "Operacao Integrada Marginal Sul",
            "tipo": "operacao_urbana_consorciada",
            "plano_diretor_id": str(uuid4()),
            "orgao_responsavel_id": str(uuid4()),
            "provincia": "Luanda",
            "municipio": "Luanda",
            "investimento_previsto": "150000000.00",
        },
    )
...
def test_endpoint_obter_operacao_urbana_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(
            side_effect=OperacaoUrbanaNotFoundError("Operacao urbana nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(operacoes_urbanas_router, prefix="/urbanismo-habitacao")
