...
async def test_plano_diretor_service_fluxo_sucesso():
    ...
    item = await service.criar(
        nome="Plano Diretor de Luanda 2035",
    ...
    item = await service.aprovar_camara(
        item.codigo_plano, lei_aprovacao="Lei Municipal 12/2026", ano_aprovacao=2026
    )
    ...
    item = await service.definir_validade(
        item.codigo_plano, data_inicio=date.today(), data_fim=date.today() + timedelta(days=3650)
    )
...
def test_endpoint_criar_plano_diretor_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_plano="PD/2026/000001",
        nome="Plano Diretor de Luanda 2035",
        tipo=TipoPlanoDiretor.MUNICIPAL,
        status=StatusPlanoDiretor.ELABORACAO,
        provincia="Luanda",
        ano_elaboracao=2026,
        orgao_responsavel_id=uuid4(),
        municipio="Luanda",
        ano_aprovacao=None,
        ano_publicacao=None,
        periodo_validade_inicio=None,
        periodo_validade_fim=None,
        lei_aprovacao=None,
        participantes_consulta=None,
        audiencias_publicas=None,
        documento_url=None,
        mapa_url=None,
        area_total_urbana=None,
        area_total_rural=None,
        populacao_estimada=None,
        densidade_media=None,
        macrozoneamento=None,
        diretrizes_gerais=None,
        objetivos_estrategicos=None,
        observacoes=None,
    ...
    response = client.post(
        "/urbanismo-habitacao/planos-diretores/",
        json={
            "nome": "Plano Diretor de Luanda 2035",
            "tipo": "municipal",
            "provincia": "Luanda",
...
def test_endpoint_obter_plano_diretor_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(
            side_effect=PlanoDiretorNotFoundError("Plano diretor nao encontrado")
        )
    )
    app = FastAPI()
    app.include_router(planos_diretores_router, prefix="/urbanismo-habitacao")
