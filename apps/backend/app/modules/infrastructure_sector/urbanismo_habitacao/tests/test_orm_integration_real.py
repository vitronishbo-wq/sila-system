...
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.domain.models.loteamento import (
    Loteamento,
)
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.domain.models.operacao_urbana import (
    OperacaoUrbana,
)
...
async def _assert_table_exists(db_session: AsyncSession, table_name: str) -> None:
    ...
    assert result.scalar_one_or_none() is not None, (
        f"Tabela fisica ausente: {table_name}. Execute migration 20260304_036_urbanismo_habitacao_orm_core."
    )


@pytest.mark.asyncio
@pytest.mark.integration
async def test_repositories_urbanismo_habitacao_roundtrip_orm_real(
    ...
    plano = await plano_repo.save(
        PlanoDiretor.criar(
            codigo_plano=await plano_repo.next_codigo(),
            nome="Plano Diretor ORM Real",
            tipo=TipoPlanoDiretor.MUNICIPAL,
            provincia="Luanda",
            ano_elaboracao=2026,
            orgao_responsavel_id=uuid4(),
        )
    )
    zoneamento = await zone_repo.save(
        Zoneamento.criar(
            codigo_zoneamento=await zone_repo.next_codigo(),
            nome="Zona Residencial ORM",
            tipo_zona=TipoZona.RESIDENCIAL,
            plano_diretor_id=plano.id,
            provincia="Luanda",
            usos_permitidos=[UsoPermitido.HABITACIONAL, UsoPermitido.SERVICOS],
        )
    )
    operacao = await operacao_repo.save(
        OperacaoUrbana.criar(
            codigo_operacao=await operacao_repo.next_codigo(),
            nome="Operacao Urbana ORM",
            tipo=TipoOperacaoUrbana.OPERACAO_URBANA_CONSORCIADA,
            plano_diretor_id=plano.id,
            orgao_responsavel_id=uuid4(),
            provincia="Luanda",
            investimento_previsto=Decimal("2500000.00"),
        )
    )
    parcelamento = await parcelamento_repo.save(
        Parcelamento.criar(
            codigo_parcelamento=await parcelamento_repo.next_codigo(),
            nome="Parcelamento ORM",
            tipo=TipoParcelamento.LOTEAMENTO,
            plano_diretor_id=plano.id,
            zoneamento_id=zoneamento.id,
            provincia="Luanda",
            area_total=Decimal("55000.00"),
            quantidade_unidades_prevista=180,
        )
    )
    loteamento = await loteamento_repo.save(
        Loteamento.criar(
            codigo_loteamento=await loteamento_repo.next_codigo(),
            nome="Loteamento ORM",
            tipo=TipoLoteamento.ABERTO,
            parcelamento_id=parcelamento.id,
            plano_diretor_id=plano.id,
            zoneamento_id=zoneamento.id,
            provincia="Luanda",
            area_total=Decimal("42000.00"),
            quantidade_lotes_prevista=120,
        )
    )
    licenca = await licenca_repo.save(
        LicencaUrbanistica.criar(
            codigo_licenca=await licenca_repo.next_codigo(),
            numero_processo="PROC-URB-ORM-001",
    ...
    alvara = await alvara_repo.save(
        Alvara.criar(
            codigo_alvara=await alvara_repo.next_codigo(),
            numero_processo="PROC-ALV-ORM-001",
            tipo=TipoAlvara.CONSTRUCAO,
            licenca_urbanistica_id=licenca.id,
            requerente_id=uuid4(),
            provincia="Luanda",
        )
    )
    habite = await habite_repo.save(
        HabiteSe.criar(
            codigo_habite_se=await habite_repo.next_codigo(),
            numero_processo="PROC-HBT-ORM-001",
            tipo=TipoHabiteSe.TOTAL,
            alvara_id=alvara.id,
            requerente_id=uuid4(),
            provincia="Luanda",
        )
