...
async def test_zoneamento_service_fluxo_sucesso():
    ...
    item = await service.atualizar_parametros(
        item.codigo_zoneamento,
        coeficiente_aproveitamento_max=Decimal("2.50"),
        taxa_ocupacao_max=Decimal("0.70"),
        gabarito_maximo=18,
    )
...
def test_endpoint_criar_zoneamento_retorna_201():
    mock_item = SimpleNamespace(
        ...
        usos_permitidos=[UsoPermitido.HABITACIONAL, UsoPermitido.SERVICOS],
        municipio="Luanda",
        coeficiente_aproveitamento_max=None,
        taxa_ocupacao_max=None,
        gabarito_maximo=None,
        recuo_frontal_minimo=None,
        permeabilidade_minima=None,
        area_lote_minima=None,
    ...
    response = client.post(
        "/urbanismo-habitacao/zoneamento/",
        json={
            "nome": "Zona Residencial Central",
            "tipo_zona": "residencial",
            "plano_diretor_id": str(uuid4()),
            "provincia": "Luanda",
            "usos_permitidos": ["habitacional", "servicos"],
            "municipio": "Luanda",
        },
    )
...
def test_endpoint_obter_zoneamento_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(side_effect=ZoneamentoNotFoundError("Zoneamento nao encontrado"))
    )
    app = FastAPI()
    app.include_router(zoneamento_router, prefix="/urbanismo-habitacao")
    app.dependency_overrides[get_zoneamento_service] = lambda: service
