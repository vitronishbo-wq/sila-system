...
async def test_licenciamento_urbano_service_fluxo_sucesso():
    ...
    item = await service.criar(
        numero_processo="PROC-URB-2026-0001",
        tipo_alvara=TipoAlvara.CONSTRUCAO,
        requerente_id=uuid4(),
        zoneamento_id=uuid4(),
        provincia="Luanda",
        municipio="Luanda",
        endereco_obra="Rua da Missao, 120",
        area_construida_prevista=Decimal("850.50"),
    )
    ...
    item = await service.iniciar_analise(item.codigo_licenca)
    assert item.status == StatusLicencaUrbanistica.EM_ANALISE
    item = await service.deferir(
        item.codigo_licenca,
        data_emissao=date.today(),
        data_validade=date.today() + timedelta(days=365),
        tecnico_responsavel_id=uuid4(),
    )
...
def test_endpoint_criar_licenca_urbanistica_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_licenca="LIC/2026/000001",
        numero_processo="PROC-URB-2026-0001",
        tipo_alvara=TipoAlvara.CONSTRUCAO,
        status=StatusLicencaUrbanistica.REQUERIDA,
        requerente_id=uuid4(),
        zoneamento_id=uuid4(),
        provincia="Luanda",
        municipio="Luanda",
    ...
    response = client.post(
        "/urbanismo-habitacao/licencas-urbanisticas/",
        json={
            "numero_processo": "PROC-URB-2026-0001",
            "tipo_alvara": "construcao",
            "requerente_id": str(uuid4()),
            "zoneamento_id": str(uuid4()),
            "provincia": "Luanda",
            "municipio": "Luanda",
            "endereco_obra": "Rua da Missao, 120",
            "area_construida_prevista": "850.50",
        },
    )
...
def test_endpoint_obter_licenca_urbanistica_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(
            side_effect=LicencaUrbanisticaNotFoundError("Licenca urbanistica nao encontrada")
        )
    )
    app = FastAPI()
    app.include_router(licencas_urbanisticas_router, prefix="/urbanismo-habitacao")
...
def test_endpoint_deferir_licenca_sem_adapter_critico_retorna_503():
    ...
    client = TestClient(app)
    response = client.post(
        "/urbanismo-habitacao/licencas-urbanisticas/LIC/2026/000001/deferir",
        json={
            "data_emissao": "2026-03-04",
            "data_validade": "2027-03-04",
            "tecnico_responsavel_id": str(uuid4()),
        },
    )
