...
async def test_loteamento_service_fluxo_sucesso():
    ...
    item = await service.criar(
        nome="Loteamento Nova Centralidade",
        tipo=TipoLoteamento.ABERTO,
        parcelamento_id=uuid4(),
        plano_diretor_id=uuid4(),
        zoneamento_id=uuid4(),
        provincia="Luanda",
        area_total=Decimal("180000.00"),
        quantidade_lotes_prevista=300,
        municipio="Luanda",
    )
    ...
    item = await service.registrar_implantacao(
        item.codigo_loteamento, quantidade_lotes_implantada=140
    )
    ...
    assert item.status == StatusLoteamento.EM_IMPLANTACAO
    item = await service.registrar_implantacao(
        item.codigo_loteamento, quantidade_lotes_implantada=300
    )
    item = await service.concluir(
        item.codigo_loteamento, data_fim_real=date.today() + timedelta(days=1)
    )
...
def test_endpoint_criar_loteamento_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_loteamento="LOT/2026/000001",
        nome="Loteamento Nova Centralidade",
        tipo=TipoLoteamento.ABERTO,
        ...
        quantidade_lotes_prevista=300,
        municipio="Luanda",
        quantidade_lotes_implantada=0,
        area_lotes=None,
        area_verde=None,
        area_institucional=None,
    ...
    response = client.post(
        "/urbanismo-habitacao/loteamentos/",
        json={
            "nome": "Loteamento Nova Centralidade",
            "tipo": "aberto",
            "parcelamento_id": str(uuid4()),
            "plano_diretor_id": str(uuid4()),
            "zoneamento_id": str(uuid4()),
            "provincia": "Luanda",
            "area_total": "180000.00",
            "quantidade_lotes_prevista": 300,
            "municipio": "Luanda",
        },
    )
...
def test_endpoint_obter_loteamento_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(side_effect=LoteamentoNotFoundError("Loteamento nao encontrado"))
    )
    app = FastAPI()
    app.include_router(loteamentos_router, prefix="/urbanismo-habitacao")
    app.dependency_overrides[get_loteamento_service] = lambda: service
