...
class LicenciamentoUrbanoService:
    def __init__(
        self,
        *,
        licenca_repo: LicencaUrbanisticaRepositoryPort,
    ...
    async def criar(
        ...
        *,
        ...
        if existente:
            raise LicencaUrbanisticaAlreadyExistsError(
                "Ja existe licenca urbanistica com este codigo"
            )
        item = LicencaUrbanistica.criar(
            ...
            area_construida_prevista=area_construida_prevista,
        )
        saved = await self._licenca_repo.save(item)
        if self._request_service:
            req_id = await self._request_service.create(
                {
                    "tipo": "licenciamento_urbanistico",
                    "referencia_id": str(saved.id),
        ...
        if self._workflow_adapter:
            wf_id = await self._workflow_adapter.iniciar_fluxo(
                entidade="urbanismo_licenca",
                referencia_id=saved.id,
                contexto={"codigo_licenca": saved.codigo_licenca},
            )
    ...
    async def deferir(
        ...
        *,
        ...
        if self._financas_adapter:
            taxa = await self._financas_adapter.calcular_taxa_licenciamento(
                tipo_alvara=saved.tipo_alvara,
                area_construida_prevista=saved.area_construida_prevista,
            )
            cobranca_id = await self._financas_adapter.registrar_cobranca(
                referencia_id=saved.id,
                descricao=f"Taxa licenciamento {saved.codigo_licenca}",
                valor=taxa,
            )
        ...
        if self._workflow_adapter:
            await self._workflow_adapter.registrar_evento(
                workflow_id=saved.codigo_licenca,
                evento="licenca_deferida",
    ...
    async def listar(
        ...
        *,
        ...
        return await self._licenca_repo.list(
            status=status, tipo_alvara=tipo_alvara, provincia=provincia
        )
