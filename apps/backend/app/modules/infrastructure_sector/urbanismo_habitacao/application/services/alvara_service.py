...
class AlvaraService:
    ...
    async def criar(
        ...
        *,
        ...
        item = Alvara.criar(
            codigo_alvara=codigo,
            numero_processo=numero_processo,
            tipo=tipo,
            licenca_urbanistica_id=licenca_urbanistica_id,
    ...
    async def deferir(
        self, codigo_alvara: str, *, data_emissao: date, data_validade: date, analista_id: UUID
    ) -> Alvara:
        item = await self._obter_ou_erro(codigo_alvara)
        item.deferir(
            data_emissao=data_emissao, data_validade=data_validade, analista_id=analista_id
        )
        return await self._alvara_repo.save(item)
