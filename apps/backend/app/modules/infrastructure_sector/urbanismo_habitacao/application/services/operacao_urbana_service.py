...
class OperacaoUrbanaService:
    ...
    async def iniciar_execucao(
        self, codigo_operacao: str, *, data_inicio_real: date
    ) -> OperacaoUrbana:
        item = await self._obter_ou_erro(codigo_operacao)
    ...
    async def atualizar_execucao(
        ...
        *,
        ...
        item = await self._obter_ou_erro(codigo_operacao)
        item.atualizar_execucao(
            percentual_execucao=percentual_execucao, investimento_executado=investimento_executado
        )
        return await self._operacao_urbana_repo.save(item)
