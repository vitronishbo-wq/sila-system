...
class PlanoDiretorService:
    ...
    async def criar(
        ...
        *,
        ...
        item = PlanoDiretor.criar(
            codigo_plano=codigo,
            nome=nome,
            tipo=tipo,
            provincia=provincia,
            ano_elaboracao=ano_elaboracao,
    ...
    async def realizar_audiencia_publica(
        self, codigo_plano: str, *, participantes: int
    ) -> PlanoDiretor:
        item = await self._obter_ou_erro(codigo_plano)
    ...
    async def aprovar_camara(
        self, codigo_plano: str, *, lei_aprovacao: str, ano_aprovacao: int
    ) -> PlanoDiretor:
        item = await self._obter_ou_erro(codigo_plano)
    ...
    async def definir_validade(
        self, codigo_plano: str, *, data_inicio: date, data_fim: date
    ) -> PlanoDiretor:
        item = await self._obter_ou_erro(codigo_plano)
