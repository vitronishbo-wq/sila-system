...
class HabiteSeService:
    ...
    async def criar(
        ...
        *,
        ...
        item = HabiteSe.criar(
            ...
            area_vistoriada=area_vistoriada,
        )
        return await self._habite_se_repo.save(item)

    async def agendar_vistoria(
        self, codigo_habite_se: str, *, data_vistoria: date, tecnico_vistoriador_id: UUID
    ) -> HabiteSe:
        item = await self._obter_ou_erro(codigo_habite_se)
        item.agendar_vistoria(
            data_vistoria=data_vistoria, tecnico_vistoriador_id=tecnico_vistoriador_id
        )
        return await self._habite_se_repo.save(item)
    ...
    async def emitir(
        self, codigo_habite_se: str, *, data_emissao: date, data_validade: date | None = None
    ) -> HabiteSe:
        item = await self._obter_ou_erro(codigo_habite_se)
