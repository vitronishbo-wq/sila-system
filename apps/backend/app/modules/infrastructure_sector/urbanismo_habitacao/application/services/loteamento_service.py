...
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.application.ports.ambiente_service_port import (
    AmbienteServicePort,
)
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.application.ports.financas_service_port import (
    FinancasServicePort,
)
...
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.application.ports.loteamento_repository_port import (
    LoteamentoRepositoryPort,
)
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.application.ports.obras_publicas_service_port import (
    ObrasPublicasServicePort,
)
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.application.ports.transportes_service_port import (
    TransportesServicePort,
)
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.application.ports.workflow_service_port import (
    WorkflowServicePort,
)
...
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.exceptions import (
    LoteamentoAlreadyExistsError,
    LoteamentoNotFoundError,
)
...
class LoteamentoService:
    def __init__(
        self,
        *,
        loteamento_repo: LoteamentoRepositoryPort,
        gestao_fundiaria_adapter: GestaoFundiariaServicePort | None = None,
        ambiente_adapter: AmbienteServicePort | None = None,
        obras_publicas_adapter: ObrasPublicasServicePort | None = None,
        aguas_saneamento_adapter: AguasSaneamentoServicePort | None = None,
        transportes_adapter: TransportesServicePort | None = None,
    ...
    async def criar(
        ...
        *,
        ...
        item = Loteamento.criar(
            codigo_loteamento=codigo,
            nome=nome,
            tipo=tipo,
        ...
        if self._financas_adapter:
            taxa = await self._financas_adapter.calcular_taxa_loteamento(
                area_total=area_total, quantidade_lotes=quantidade_lotes_prevista
            )
            item.observacoes = f"taxa_loteamento:{taxa}"
        saved = await self._loteamento_repo.save(item)
        if self._workflow_adapter:
            workflow_id = await self._workflow_adapter.iniciar_fluxo(
                entidade="urbanismo_loteamento",
                referencia_id=saved.id,
                contexto={"codigo_loteamento": saved.codigo_loteamento},
            )
    ...
    async def aprovar(self, codigo_loteamento: str) -> Loteamento:
        ...
        if self._aguas_saneamento_adapter:
            agua_ok = await self._aguas_saneamento_adapter.validar_capacidade_atendimento(
                zoneamento_id=item.zoneamento_id, quantidade_lotes=item.quantidade_lotes_prevista
            )
            if not agua_ok:
                raise ValueError("Capacidade de agua e saneamento insuficiente para aprovacao")
        if self._transportes_adapter:
            impacto_ok = await self._transportes_adapter.validar_impacto_viario(
                zoneamento_id=item.zoneamento_id, quantidade_lotes=item.quantidade_lotes_prevista
            )
            if not impacto_ok:
        ...
        saved = await self._loteamento_repo.save(item)
        if self._workflow_adapter:
            await self._workflow_adapter.registrar_evento(
                workflow_id=saved.codigo_loteamento,
                evento="loteamento_aprovado",
                payload={"status": saved.status.value},
            )
        return saved

    async def iniciar_implantacao(
        self, codigo_loteamento: str, *, data_inicio_real: date
    ) -> Loteamento:
        item = await self._obter_ou_erro(codigo_loteamento)
        item.iniciar_implantacao(data_inicio_real=data_inicio_real)
        return await self._loteamento_repo.save(item)

    async def registrar_implantacao(
        self, codigo_loteamento: str, *, quantidade_lotes_implantada: int
    ) -> Loteamento:
        item = await self._obter_ou_erro(codigo_loteamento)
    ...
    async def listar(
        self,
        *,
        status: StatusLoteamento | None = None,
        tipo: TipoLoteamento | None = None,
