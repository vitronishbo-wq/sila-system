...
class ZoneamentoService:
    ...
    async def criar(self, *, nome: str, tipo_zona: TipoZona, plano_diretor_id: UUID, provincia: str, usos_permitidos: list[UsoPermitido], municipio: str | None=None, codigo_zoneamento: str | None=None) -> Zoneamento:
        ...
        item = Zoneamento.criar(codigo_zoneamento=codigo, nome=nome, tipo_zona=tipo_zona, plano_diretor_id=plano_diretor_id, provincia=provincia, usos_permitidos=usos_permitidos, municipio=municipio)
    ...
    async def atualizar_parametros(self, codigo_zoneamento: str, *, usos_permitidos: list[UsoPermitido] | None=None, coeficiente_aproveitamento_max: Decimal | None=None, taxa_ocupacao_max: Decimal | None=None, gabarito_maximo: int | None=None, recuo_frontal_minimo: Decimal | None=None, permeabilidade_minima: Decimal | None=None, area_lote_minima: Decimal | None=None) -> Zoneamento:
        item = await self._obter_ou_erro(codigo_zoneamento)
        item.atualizar_parametros(usos_permitidos=usos_permitidos, coeficiente_aproveitamento_max=coeficiente_aproveitamento_max, taxa_ocupacao_max=taxa_ocupacao_max, gabarito_maximo=gabarito_maximo, recuo_frontal_minimo=recuo_frontal_minimo, permeabilidade_minima=permeabilidade_minima, area_lote_minima=area_lote_minima)
        return await self._zoneamento_repo.save(item)

    async def obter_por_codigo(self, codigo_zoneamento: str) -> Zoneamento:
        return await self._obter_ou_erro(codigo_zoneamento)

    async def listar(self, *, status: StatusZoneamento | None=None, tipo_zona: TipoZona | None=None, provincia: str | None=None) -> list[Zoneamento]:
        return await self._zoneamento_repo.list(status=status, tipo_zona=tipo_zona, provincia=provincia)
