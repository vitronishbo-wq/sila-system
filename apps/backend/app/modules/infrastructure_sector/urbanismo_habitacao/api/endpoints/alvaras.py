...
async def criar_alvara(data: AlvaraCreate, service: AlvaraService=Depends(get_alvara_service)):
    try:
        return await service.criar(numero_processo=data.numero_processo, tipo=data.tipo, licenca_urbanistica_id=data.licenca_urbanistica_id, requerente_id=data.requerente_id, provincia=data.provincia, municipio=data.municipio, endereco_obra=data.endereco_obra, area_autorizada=data.area_autorizada, codigo_alvara=data.codigo_alvara)
...
async def iniciar_analise_alvara(codigo_alvara: str, service: AlvaraService=Depends(get_alvara_service)):
    try:
...
async def registrar_pendencia_alvara(codigo_alvara: str, data: AlvaraMotivoInput, service: AlvaraService=Depends(get_alvara_service)):
    try:
...
async def deferir_alvara(codigo_alvara: str, data: AlvaraDeferimentoInput, service: AlvaraService=Depends(get_alvara_service)):
    try:
        return await service.deferir(codigo_alvara, data_emissao=data.data_emissao, data_validade=data.data_validade, analista_id=data.analista_id)
    except AlvaraNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def indeferir_alvara(codigo_alvara: str, data: AlvaraMotivoInput, service: AlvaraService=Depends(get_alvara_service)):
    try:
...
async def cancelar_alvara(codigo_alvara: str, data: AlvaraMotivoInput, service: AlvaraService=Depends(get_alvara_service)):
    try:
...
async def obter_alvara(codigo_alvara: str, service: AlvaraService=Depends(get_alvara_service)):
    try:
...
async def listar_alvaras(status_alvara: StatusAlvara | None=None, tipo_alvara: TipoAlvara | None=None, provincia: str | None=None, service: AlvaraService=Depends(get_alvara_service)):
