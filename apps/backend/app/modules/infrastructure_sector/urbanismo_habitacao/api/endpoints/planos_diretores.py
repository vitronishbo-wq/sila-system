...
async def criar_plano_diretor(data: PlanoDiretorCreate, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
    try:
        return await service.criar(nome=data.nome, tipo=data.tipo, provincia=data.provincia, ano_elaboracao=data.ano_elaboracao, orgao_responsavel_id=data.orgao_responsavel_id, municipio=data.municipio, codigo_plano=data.codigo_plano)
...
async def iniciar_consulta_publica(codigo_plano: str, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
    try:
...
async def realizar_audiencia_publica(codigo_plano: str, data: PlanoDiretorAudienciaInput, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
    try:
        return await service.realizar_audiencia_publica(codigo_plano, participantes=data.participantes)
    except PlanoDiretorNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def aprovar_camara(codigo_plano: str, data: PlanoDiretorAprovacaoCamaraInput, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
    try:
        return await service.aprovar_camara(codigo_plano, lei_aprovacao=data.lei_aprovacao, ano_aprovacao=data.ano_aprovacao)
    except PlanoDiretorNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def aprovar_prefeitura(codigo_plano: str, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
    try:
...
async def sancionar(codigo_plano: str, data: PlanoDiretorSancaoInput, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
    try:
...
async def publicar(codigo_plano: str, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
    try:
...
async def definir_validade(codigo_plano: str, data: PlanoDiretorValidadeInput, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
    try:
        return await service.definir_validade(codigo_plano, data_inicio=data.data_inicio, data_fim=data.data_fim)
    except PlanoDiretorNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def obter_plano_diretor(codigo_plano: str, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
    try:
...
async def listar_planos_diretores(status_plano: StatusPlanoDiretor | None=None, tipo_plano: TipoPlanoDiretor | None=None, provincia: str | None=None, service: PlanoDiretorService=Depends(get_plano_diretor_service)):
