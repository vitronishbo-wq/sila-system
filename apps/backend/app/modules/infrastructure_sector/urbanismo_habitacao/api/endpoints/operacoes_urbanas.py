...
async def criar_operacao_urbana(data: OperacaoUrbanaCreate, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
    try:
        return await service.criar(nome=data.nome, tipo=data.tipo, plano_diretor_id=data.plano_diretor_id, orgao_responsavel_id=data.orgao_responsavel_id, provincia=data.provincia, municipio=data.municipio, area_intervencao=data.area_intervencao, investimento_previsto=data.investimento_previsto, data_inicio_prevista=data.data_inicio_prevista, data_fim_prevista=data.data_fim_prevista, codigo_operacao=data.codigo_operacao)
...
async def aprovar_operacao_urbana(codigo_operacao: str, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
    try:
...
async def iniciar_execucao_operacao_urbana(codigo_operacao: str, data: OperacaoUrbanaInicioInput, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
    try:
        return await service.iniciar_execucao(codigo_operacao, data_inicio_real=data.data_inicio_real)
    except OperacaoUrbanaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def atualizar_execucao_operacao_urbana(codigo_operacao: str, data: OperacaoUrbanaExecucaoInput, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
    try:
        return await service.atualizar_execucao(codigo_operacao, percentual_execucao=data.percentual_execucao, investimento_executado=data.investimento_executado)
    except OperacaoUrbanaNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def concluir_operacao_urbana(codigo_operacao: str, data: OperacaoUrbanaConclusaoInput, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
    try:
...
async def suspender_operacao_urbana(codigo_operacao: str, data: OperacaoUrbanaMotivoInput, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
    try:
...
async def retomar_operacao_urbana(codigo_operacao: str, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
    try:
...
async def cancelar_operacao_urbana(codigo_operacao: str, data: OperacaoUrbanaMotivoInput, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
    try:
...
async def obter_operacao_urbana(codigo_operacao: str, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
    try:
...
async def listar_operacoes_urbanas(status_operacao: StatusOperacaoUrbana | None=None, tipo_operacao: TipoOperacaoUrbana | None=None, provincia: str | None=None, service: OperacaoUrbanaService=Depends(get_operacao_urbana_service)):
