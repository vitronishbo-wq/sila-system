...
async def criar_parcelamento(data: ParcelamentoCreate, service: ParcelamentoService=Depends(get_parcelamento_service)):
    try:
        return await service.criar(nome=data.nome, tipo=data.tipo, plano_diretor_id=data.plano_diretor_id, zoneamento_id=data.zoneamento_id, provincia=data.provincia, area_total=data.area_total, quantidade_unidades_prevista=data.quantidade_unidades_prevista, municipio=data.municipio, area_publica_prevista=data.area_publica_prevista, area_sistema_viario_prevista=data.area_sistema_viario_prevista, codigo_parcelamento=data.codigo_parcelamento)
...
async def iniciar_analise_parcelamento(codigo_parcelamento: str, service: ParcelamentoService=Depends(get_parcelamento_service)):
    try:
...
async def aprovar_parcelamento(codigo_parcelamento: str, service: ParcelamentoService=Depends(get_parcelamento_service)):
    try:
...
async def iniciar_execucao_parcelamento(codigo_parcelamento: str, service: ParcelamentoService=Depends(get_parcelamento_service)):
    try:
...
async def concluir_parcelamento(codigo_parcelamento: str, data: ParcelamentoConclusaoInput, service: ParcelamentoService=Depends(get_parcelamento_service)):
    try:
        return await service.concluir(codigo_parcelamento, quantidade_unidades_resultante=data.quantidade_unidades_resultante)
    except ParcelamentoNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def cancelar_parcelamento(codigo_parcelamento: str, data: ParcelamentoMotivoInput, service: ParcelamentoService=Depends(get_parcelamento_service)):
    try:
...
async def obter_parcelamento(codigo_parcelamento: str, service: ParcelamentoService=Depends(get_parcelamento_service)):
    try:
...
async def listar_parcelamentos(status_parcelamento: StatusParcelamento | None=None, tipo_parcelamento: TipoParcelamento | None=None, provincia: str | None=None, service: ParcelamentoService=Depends(get_parcelamento_service)):
    return await service.listar(status=status_parcelamento, tipo=tipo_parcelamento, provincia=provincia)