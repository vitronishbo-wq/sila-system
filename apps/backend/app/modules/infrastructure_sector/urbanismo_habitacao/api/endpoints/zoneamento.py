...
async def criar_zoneamento(data: ZoneamentoCreate, service: ZoneamentoService=Depends(get_zoneamento_service)):
    try:
        return await service.criar(nome=data.nome, tipo_zona=data.tipo_zona, plano_diretor_id=data.plano_diretor_id, provincia=data.provincia, usos_permitidos=data.usos_permitidos, municipio=data.municipio, codigo_zoneamento=data.codigo_zoneamento)
...
async def iniciar_consulta_publica(codigo_zoneamento: str, service: ZoneamentoService=Depends(get_zoneamento_service)):
    try:
...
async def aprovar_zoneamento(codigo_zoneamento: str, service: ZoneamentoService=Depends(get_zoneamento_service)):
    try:
...
async def vigorar_zoneamento(codigo_zoneamento: str, data: ZoneamentoVigenciaInput, service: ZoneamentoService=Depends(get_zoneamento_service)):
    try:
        return await service.vigorar(codigo_zoneamento, data_inicio_vigencia=data.data_inicio_vigencia)
    except ZoneamentoNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def suspender_zoneamento(codigo_zoneamento: str, data: ZoneamentoMotivoInput, service: ZoneamentoService=Depends(get_zoneamento_service)):
    try:
...
async def revogar_zoneamento(codigo_zoneamento: str, data: ZoneamentoMotivoInput, service: ZoneamentoService=Depends(get_zoneamento_service)):
    try:
...
async def atualizar_parametros_zoneamento(codigo_zoneamento: str, data: ZoneamentoParametrosInput, service: ZoneamentoService=Depends(get_zoneamento_service)):
    try:
        return await service.atualizar_parametros(codigo_zoneamento, usos_permitidos=data.usos_permitidos, coeficiente_aproveitamento_max=data.coeficiente_aproveitamento_max, taxa_ocupacao_max=data.taxa_ocupacao_max, gabarito_maximo=data.gabarito_maximo, recuo_frontal_minimo=data.recuo_frontal_minimo, permeabilidade_minima=data.permeabilidade_minima, area_lote_minima=data.area_lote_minima)
    except ZoneamentoNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def listar_zoneamentos(status_zoneamento: StatusZoneamento | None=None, tipo_zona: TipoZona | None=None, provincia: str | None=None, service: ZoneamentoService=Depends(get_zoneamento_service)):
