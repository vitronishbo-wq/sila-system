...
async def agendar_vistoria_habite_se(
    codigo_habite_se: str,
    data: HabiteSeVistoriaInput,
    service: HabiteSeService = habite_se_service_dep,
):
    try:
        return await service.agendar_vistoria(
            codigo_habite_se,
            data_vistoria=data.data_vistoria,
            tecnico_vistoriador_id=data.tecnico_vistoriador_id,
        )
...
async def aprovar_vistoria_habite_se(
    codigo_habite_se: str, service: HabiteSeService = habite_se_service_dep
):
    try:
...
async def reprovar_vistoria_habite_se(
    codigo_habite_se: str,
    data: HabiteSeMotivoInput,
    service: HabiteSeService = habite_se_service_dep,
):
    try:
...
async def emitir_habite_se(
    codigo_habite_se: str,
    data: HabiteSeEmissaoInput,
    service: HabiteSeService = habite_se_service_dep,
):
    try:
        return await service.emitir(
            codigo_habite_se, data_emissao=data.data_emissao, data_validade=data.data_validade
        )
    except HabiteSeNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except ValueError as exc:
...
async def listar_habite_se(
    status_habite_se: StatusHabiteSe | None = None,
    tipo_habite_se: TipoHabiteSe | None = None,
