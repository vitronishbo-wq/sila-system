...
loteamento_service_dep = Depends(get_loteamento_service)
...
def _ensure_loteamento_create_adapters(service: LoteamentoService) -> None:
    if not service.has_gestao_fundiaria_adapter():
        raise HTTPException(
            ...
            detail="Adapter de Gestao Fundiaria indisponivel para criar loteamento. Verifique a integracao do modulo.",
        )
    if not service.has_financas_adapter():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Financas indisponivel para calcular taxas de loteamento. Verifique a integracao do modulo.",
        )
    if not service.has_workflow_adapter():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Workflow indisponivel para iniciar fluxo do loteamento. Verifique a integracao do modulo.",
        )
...
def _ensure_loteamento_aprovacao_adapters(service: LoteamentoService) -> None:
    if not service.has_ambiente_adapter():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Ambiente indisponivel para aprovacao do loteamento.",
        )
    if not service.has_obras_publicas_adapter():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Obras Publicas indisponivel para aprovacao do loteamento.",
        )
    if not service.has_aguas_saneamento_adapter():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Aguas e Saneamento indisponivel para aprovacao do loteamento.",
        )
    if not service.has_transportes_adapter():
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Transportes indisponivel para aprovacao do loteamento.",
        )


@router.post("/", response_model=LoteamentoResponse, status_code=status.HTTP_201_CREATED)
async def criar_loteamento(
    data: LoteamentoCreate, service: LoteamentoService = loteamento_service_dep
):
    ...
    try:
        return await service.criar(
            nome=data.nome,
            tipo=data.tipo,
            parcelamento_id=data.parcelamento_id,
            plano_diretor_id=data.plano_diretor_id,
            zoneamento_id=data.zoneamento_id,
            provincia=data.provincia,
            area_total=data.area_total,
            quantidade_lotes_prevista=data.quantidade_lotes_prevista,
            municipio=data.municipio,
            area_lotes=data.area_lotes,
            area_verde=data.area_verde,
            area_institucional=data.area_institucional,
            data_inicio_prevista=data.data_inicio_prevista,
            data_fim_prevista=data.data_fim_prevista,
            codigo_loteamento=data.codigo_loteamento,
        )
...
async def aprovar_loteamento(
    codigo_loteamento: str, service: LoteamentoService = loteamento_service_dep
):
...
async def iniciar_implantacao_loteamento(
    codigo_loteamento: str,
    data: LoteamentoInicioInput,
    service: LoteamentoService = loteamento_service_dep,
):
    try:
        return await service.iniciar_implantacao(
            codigo_loteamento, data_inicio_real=data.data_inicio_real
        )
    except LoteamentoNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except ValueError as exc:
...
async def registrar_implantacao_loteamento(
    codigo_loteamento: str,
    data: LoteamentoImplantacaoInput,
    service: LoteamentoService = loteamento_service_dep,
):
    try:
        return await service.registrar_implantacao(
            codigo_loteamento, quantidade_lotes_implantada=data.quantidade_lotes_implantada
        )
    except LoteamentoNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except ValueError as exc:
...
async def concluir_loteamento(
    codigo_loteamento: str,
    data: LoteamentoConclusaoInput,
    service: LoteamentoService = loteamento_service_dep,
):
    try:
...
async def suspender_loteamento(
    codigo_loteamento: str,
    data: LoteamentoMotivoInput,
    service: LoteamentoService = loteamento_service_dep,
):
    try:
...
async def listar_loteamentos(
    status_loteamento: StatusLoteamento | None = None,
    tipo_loteamento: TipoLoteamento | None = None,
