...
class LicencaUrbanisticaModel(Base):
    __tablename__ = 'urbanismo_habitacao_licencas_urbanisticas'
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    codigo_licenca: Mapped[str] = mapped_column(String(40), unique=True, nullable=False, index=True)
    numero_processo: Mapped[str] = mapped_column(String(80), nullable=False, index=True)
    tipo_alvara: Mapped[str] = mapped_column(String(40), nullable=False, index=True)
    ...
    area_construida_prevista: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), nullable=True)
    ...
    tecnico_responsavel_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True, index=True)
