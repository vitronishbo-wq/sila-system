...
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.infrastructure.models.loteamento_model import (
    LoteamentoModel,
)
...
class SQLAlchemyLoteamentoRepository(LoteamentoRepositoryPort):
    ...
    async def save(self, item: Loteamento) -> Loteamento:
        if self._session:
            existing = await self._session.execute(
                select(LoteamentoModel).where(
                    LoteamentoModel.codigo_loteamento == item.codigo_loteamento
                )
            )
            model = existing.scalars().first()
            if model is None:
                model = LoteamentoModel(
                    id=item.id,
                    codigo_loteamento=item.codigo_loteamento,
                    nome=item.nome,
                    ...
                    quantidade_lotes_prevista=item.quantidade_lotes_prevista,
                    municipio=item.municipio,
                    quantidade_lotes_implantada=item.quantidade_lotes_implantada,
                    area_lotes=item.area_lotes,
                    area_verde=item.area_verde,
                    area_institucional=item.area_institucional,
    ...
    async def get_by_codigo(self, codigo_loteamento: str) -> Loteamento | None:
        if self._session:
            result = await self._session.execute(
                select(LoteamentoModel).where(
                    LoteamentoModel.codigo_loteamento == codigo_loteamento
                )
            )
        ...
        if self._session:
            ...
            if provincia:
                statement = statement.where(
                    func.lower(LoteamentoModel.provincia) == provincia.strip().lower()
                )
            result = await self._session.execute(
                statement.order_by(LoteamentoModel.codigo_loteamento.asc())
            )
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(LoteamentoModel)
                .where(LoteamentoModel.codigo_loteamento.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: LoteamentoModel) -> Loteamento:
        return Loteamento(
            id=model.id,
            codigo_loteamento=model.codigo_loteamento,
            nome=model.nome,
            tipo=TipoLoteamento(model.tipo),
            status=StatusLoteamento(model.status),
            parcelamento_id=model.parcelamento_id,
            plano_diretor_id=model.plano_diretor_id,
            zoneamento_id=model.zoneamento_id,
            provincia=model.provincia,
            area_total=Decimal(model.area_total),
            quantidade_lotes_prevista=model.quantidade_lotes_prevista,
            municipio=model.municipio,
            quantidade_lotes_implantada=model.quantidade_lotes_implantada,
            area_lotes=Decimal(model.area_lotes) if model.area_lotes is not None else None,
            area_verde=Decimal(model.area_verde) if model.area_verde is not None else None,
            area_institucional=Decimal(model.area_institucional)
            if model.area_institucional is not None
            else None,
