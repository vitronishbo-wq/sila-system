...
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.infrastructure.models.alvara_model import (
    AlvaraModel,
)
...
class SQLAlchemyAlvaraRepository(AlvaraRepositoryPort):
    ...
    async def save(self, item: Alvara) -> Alvara:
        if self._session:
            existing = await self._session.execute(
                select(AlvaraModel).where(AlvaraModel.codigo_alvara == item.codigo_alvara)
            )
            model = existing.scalars().first()
            if model is None:
                model = AlvaraModel(
                    id=item.id,
                    codigo_alvara=item.codigo_alvara,
                    numero_processo=item.numero_processo,
                    tipo=item.tipo.value,
                    status=item.status.value,
                    licenca_urbanistica_id=item.licenca_urbanistica_id,
                    requerente_id=item.requerente_id,
                    provincia=item.provincia,
                    municipio=item.municipio,
                    endereco_obra=item.endereco_obra,
                    area_autorizada=item.area_autorizada,
    ...
    async def get_by_codigo(self, codigo_alvara: str) -> Alvara | None:
        if self._session:
            result = await self._session.execute(
                select(AlvaraModel).where(AlvaraModel.codigo_alvara == codigo_alvara)
            )
        ...
        if self._session:
            ...
            if provincia:
                statement = statement.where(
                    func.lower(AlvaraModel.provincia) == provincia.strip().lower()
                )
            result = await self._session.execute(
                statement.order_by(AlvaraModel.codigo_alvara.asc())
            )
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(AlvaraModel)
                .where(AlvaraModel.codigo_alvara.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: AlvaraModel) -> Alvara:
        return Alvara(
            id=model.id,
            codigo_alvara=model.codigo_alvara,
            numero_processo=model.numero_processo,
            tipo=TipoAlvara(model.tipo),
            status=StatusAlvara(model.status),
            licenca_urbanistica_id=model.licenca_urbanistica_id,
            requerente_id=model.requerente_id,
            provincia=model.provincia,
            municipio=model.municipio,
            endereco_obra=model.endereco_obra,
            area_autorizada=Decimal(model.area_autorizada)
            if model.area_autorizada is not None
            else None,
