...
class SQLAlchemyHabiteSeRepository(HabiteSeRepositoryPort):
    ...
    async def save(self, item: HabiteSe) -> HabiteSe:
        if self._session:
            existing = await self._session.execute(select(HabiteSeModel).where(HabiteSeModel.codigo_habite_se == item.codigo_habite_se))
            model = existing.scalars().first()
            if model is None:
                model = HabiteSeModel(id=item.id, codigo_habite_se=item.codigo_habite_se, numero_processo=item.numero_processo, tipo=item.tipo.value, status=item.status.value, alvara_id=item.alvara_id, requerente_id=item.requerente_id, provincia=item.provincia, municipio=item.municipio, endereco_imovel=item.endereco_imovel, area_vistoriada=item.area_vistoriada, data_requerimento=item.data_requerimento, data_vistoria=item.data_vistoria, data_emissao=item.data_emissao, data_validade=item.data_validade, tecnico_vistoriador_id=item.tecnico_vistoriador_id, observacoes=item.observacoes, data_atualizacao=item.data_atualizacao)
    ...
    async def get_by_codigo(self, codigo_habite_se: str) -> HabiteSe | None:
        if self._session:
            result = await self._session.execute(select(HabiteSeModel).where(HabiteSeModel.codigo_habite_se == codigo_habite_se))
    ...
    async def list(self, *, status: StatusHabiteSe | None=None, tipo: TipoHabiteSe | None=None, provincia: str | None=None) -> list[HabiteSe]:
        if self._session:
            ...
            if provincia:
                statement = statement.where(func.lower(HabiteSeModel.provincia) == provincia.strip().lower())
            result = await self._session.execute(statement.order_by(HabiteSeModel.codigo_habite_se.asc()))
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            prefix = f'HBT/{year}/'
            result = await self._session.execute(select(func.count()).select_from(HabiteSeModel).where(HabiteSeModel.codigo_habite_se.like(f'{prefix}%')))
    ...
    def _to_domain(model: HabiteSeModel) -> HabiteSe:
        return HabiteSe(id=model.id, codigo_habite_se=model.codigo_habite_se, numero_processo=model.numero_processo, tipo=TipoHabiteSe(model.tipo), status=StatusHabiteSe(model.status), alvara_id=model.alvara_id, requerente_id=model.requerente_id, provincia=model.provincia, municipio=model.municipio, endereco_imovel=model.endereco_imovel, area_vistoriada=Decimal(model.area_vistoriada) if model.area_vistoriada is not None else None, data_requerimento=model.data_requerimento, data_vistoria=model.data_vistoria, data_emissao=model.data_emissao, data_validade=model.data_validade, tecnico_vistoriador_id=model.tecnico_vistoriador_id, observacoes=model.observacoes, data_atualizacao=model.data_atualizacao)