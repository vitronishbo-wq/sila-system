...
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.application.ports.plano_diretor_repository_port import (
    PlanoDiretorRepositoryPort,
)
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.domain.enums import (
    StatusPlanoDiretor,
    TipoPlanoDiretor,
)
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.domain.models.plano_diretor import (
    PlanoDiretor,
)
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.infrastructure.models.plano_diretor_model import (
    PlanoDiretorModel,
)
...
class SQLAlchemyPlanoDiretorRepository(PlanoDiretorRepositoryPort):
    ...
    async def save(self, item: PlanoDiretor) -> PlanoDiretor:
        if self._session:
            existing = await self._session.execute(
                select(PlanoDiretorModel).where(PlanoDiretorModel.codigo_plano == item.codigo_plano)
            )
            model = existing.scalars().first()
            if model is None:
                model = PlanoDiretorModel(
                    id=item.id,
                    ...
                    ano_elaboracao=item.ano_elaboracao,
                    orgao_responsavel_id=item.orgao_responsavel_id,
                    municipio=item.municipio,
                    ano_aprovacao=item.ano_aprovacao,
                    ano_publicacao=item.ano_publicacao,
                    periodo_validade_inicio=item.periodo_validade_inicio,
                    periodo_validade_fim=item.periodo_validade_fim,
                    lei_aprovacao=item.lei_aprovacao,
                    participantes_consulta=item.participantes_consulta,
                    audiencias_publicas=item.audiencias_publicas,
                    documento_url=item.documento_url,
                    mapa_url=item.mapa_url,
                    area_total_urbana=item.area_total_urbana,
                    area_total_rural=item.area_total_rural,
                    populacao_estimada=item.populacao_estimada,
                    densidade_media=item.densidade_media,
                    macrozoneamento=item.macrozoneamento,
                    diretrizes_gerais=item.diretrizes_gerais,
                    objetivos_estrategicos=item.objetivos_estrategicos,
    ...
    async def get_by_codigo(self, codigo_plano: str) -> PlanoDiretor | None:
        if self._session:
            result = await self._session.execute(
                select(PlanoDiretorModel).where(PlanoDiretorModel.codigo_plano == codigo_plano)
            )
    ...
    async def list(
        self,
        *,
        status: StatusPlanoDiretor | None = None,
        tipo: TipoPlanoDiretor | None = None,
        ...
        if self._session:
            ...
            if provincia:
                statement = statement.where(
                    func.lower(PlanoDiretorModel.provincia) == provincia.strip().lower()
                )
            result = await self._session.execute(
                statement.order_by(PlanoDiretorModel.codigo_plano.asc())
            )
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(PlanoDiretorModel)
                .where(PlanoDiretorModel.codigo_plano.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: PlanoDiretorModel) -> PlanoDiretor:
        return PlanoDiretor(
            id=model.id,
            codigo_plano=model.codigo_plano,
            nome=model.nome,
            tipo=TipoPlanoDiretor(model.tipo),
            status=StatusPlanoDiretor(model.status),
            provincia=model.provincia,
            ano_elaboracao=model.ano_elaboracao,
            orgao_responsavel_id=model.orgao_responsavel_id,
            municipio=model.municipio,
            ano_aprovacao=model.ano_aprovacao,
            ano_publicacao=model.ano_publicacao,
            periodo_validade_inicio=model.periodo_validade_inicio,
            periodo_validade_fim=model.periodo_validade_fim,
            lei_aprovacao=model.lei_aprovacao,
            participantes_consulta=model.participantes_consulta,
            audiencias_publicas=model.audiencias_publicas,
            documento_url=model.documento_url,
            mapa_url=model.mapa_url,
            area_total_urbana=Decimal(model.area_total_urbana)
            if model.area_total_urbana is not None
            else None,
            area_total_rural=Decimal(model.area_total_rural)
            if model.area_total_rural is not None
            else None,
            populacao_estimada=model.populacao_estimada,
            densidade_media=Decimal(model.densidade_media)
            if model.densidade_media is not None
            else None,
            macrozoneamento=model.macrozoneamento,
            diretrizes_gerais=model.diretrizes_gerais,
            objetivos_estrategicos=model.objetivos_estrategicos,
