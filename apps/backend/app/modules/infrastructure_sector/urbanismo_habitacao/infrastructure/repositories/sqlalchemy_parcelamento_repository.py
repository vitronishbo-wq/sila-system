...
class SQLAlchemyParcelamentoRepository(ParcelamentoRepositoryPort):
    ...
    async def save(self, item: Parcelamento) -> Parcelamento:
        if self._session:
            existing = await self._session.execute(select(ParcelamentoModel).where(ParcelamentoModel.codigo_parcelamento == item.codigo_parcelamento))
            model = existing.scalars().first()
            if model is None:
                model = ParcelamentoModel(id=item.id, codigo_parcelamento=item.codigo_parcelamento, nome=item.nome, tipo=item.tipo.value, status=item.status.value, plano_diretor_id=item.plano_diretor_id, zoneamento_id=item.zoneamento_id, provincia=item.provincia, area_total=item.area_total, quantidade_unidades_prevista=item.quantidade_unidades_prevista, municipio=item.municipio, area_publica_prevista=item.area_publica_prevista, area_sistema_viario_prevista=item.area_sistema_viario_prevista, quantidade_unidades_resultante=item.quantidade_unidades_resultante, data_cadastro=item.data_cadastro, data_atualizacao=item.data_atualizacao, observacoes=item.observacoes)
    ...
    async def get_by_codigo(self, codigo_parcelamento: str) -> Parcelamento | None:
        if self._session:
            result = await self._session.execute(select(ParcelamentoModel).where(ParcelamentoModel.codigo_parcelamento == codigo_parcelamento))
    ...
    async def list(self, *, status: StatusParcelamento | None=None, tipo: TipoParcelamento | None=None, provincia: str | None=None) -> list[Parcelamento]:
        if self._session:
            ...
            if provincia:
                statement = statement.where(func.lower(ParcelamentoModel.provincia) == provincia.strip().lower())
            result = await self._session.execute(statement.order_by(ParcelamentoModel.codigo_parcelamento.asc()))
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            prefix = f'PAR/{year}/'
            result = await self._session.execute(select(func.count()).select_from(ParcelamentoModel).where(ParcelamentoModel.codigo_parcelamento.like(f'{prefix}%')))
    ...
    def _to_domain(model: ParcelamentoModel) -> Parcelamento:
        return Parcelamento(id=model.id, codigo_parcelamento=model.codigo_parcelamento, nome=model.nome, tipo=TipoParcelamento(model.tipo), status=StatusParcelamento(model.status), plano_diretor_id=model.plano_diretor_id, zoneamento_id=model.zoneamento_id, provincia=model.provincia, area_total=Decimal(model.area_total), quantidade_unidades_prevista=model.quantidade_unidades_prevista, municipio=model.municipio, area_publica_prevista=Decimal(model.area_publica_prevista) if model.area_publica_prevista is not None else None, area_sistema_viario_prevista=Decimal(model.area_sistema_viario_prevista) if model.area_sistema_viario_prevista is not None else None, quantidade_unidades_resultante=model.quantidade_unidades_resultante, data_cadastro=model.data_cadastro, data_atualizacao=model.data_atualizacao, observacoes=model.observacoes)