...
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.infrastructure.models.operacao_urbana_model import (
    OperacaoUrbanaModel,
)
...
class SQLAlchemyOperacaoUrbanaRepository(OperacaoUrbanaRepositoryPort):
    ...
    async def save(self, item: OperacaoUrbana) -> OperacaoUrbana:
        if self._session:
            existing = await self._session.execute(
                select(OperacaoUrbanaModel).where(
                    OperacaoUrbanaModel.codigo_operacao == item.codigo_operacao
                )
            )
            model = existing.scalars().first()
            if model is None:
                model = OperacaoUrbanaModel(
                    id=item.id,
                    codigo_operacao=item.codigo_operacao,
                    nome=item.nome,
                    ...
                    area_intervencao=item.area_intervencao,
                    investimento_previsto=item.investimento_previsto,
                    investimento_executado=item.investimento_executado,
                    ...
                    percentual_execucao=item.percentual_execucao,
    ...
    async def get_by_codigo(self, codigo_operacao: str) -> OperacaoUrbana | None:
        if self._session:
            result = await self._session.execute(
                select(OperacaoUrbanaModel).where(
                    OperacaoUrbanaModel.codigo_operacao == codigo_operacao
                )
            )
    ...
    async def list(
        self,
        *,
        status: StatusOperacaoUrbana | None = None,
        tipo: TipoOperacaoUrbana | None = None,
        ...
        if self._session:
            ...
            if provincia:
                statement = statement.where(
                    func.lower(OperacaoUrbanaModel.provincia) == provincia.strip().lower()
                )
            result = await self._session.execute(
                statement.order_by(OperacaoUrbanaModel.codigo_operacao.asc())
            )
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(OperacaoUrbanaModel)
                .where(OperacaoUrbanaModel.codigo_operacao.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: OperacaoUrbanaModel) -> OperacaoUrbana:
        return OperacaoUrbana(
            id=model.id,
            codigo_operacao=model.codigo_operacao,
            nome=model.nome,
            tipo=TipoOperacaoUrbana(model.tipo),
            status=StatusOperacaoUrbana(model.status),
            plano_diretor_id=model.plano_diretor_id,
            orgao_responsavel_id=model.orgao_responsavel_id,
            provincia=model.provincia,
            municipio=model.municipio,
            area_intervencao=Decimal(model.area_intervencao)
            if model.area_intervencao is not None
            else None,
            investimento_previsto=Decimal(model.investimento_previsto)
            if model.investimento_previsto is not None
            else None,
            investimento_executado=Decimal(model.investimento_executado)
            if model.investimento_executado is not None
            else None,
            ...
            percentual_execucao=Decimal(model.percentual_execucao),
