...
from apps.backend.app.modules.infrastructure_sector.urbanismo_habitacao.infrastructure.models.licenca_urbanistica_model import (
    LicencaUrbanisticaModel,
)
...
class SQLAlchemyLicencaUrbanisticaRepository(LicencaUrbanisticaRepositoryPort):
    ...
    async def save(self, item: LicencaUrbanistica) -> LicencaUrbanistica:
        if self._session:
            existing = await self._session.execute(
                select(LicencaUrbanisticaModel).where(
                    LicencaUrbanisticaModel.codigo_licenca == item.codigo_licenca
                )
            )
            model = existing.scalars().first()
            if model is None:
                model = LicencaUrbanisticaModel(
                    id=item.id,
                    codigo_licenca=item.codigo_licenca,
                    numero_processo=item.numero_processo,
                    tipo_alvara=item.tipo_alvara.value,
                    status=item.status.value,
                    requerente_id=item.requerente_id,
                    zoneamento_id=item.zoneamento_id,
                    provincia=item.provincia,
                    municipio=item.municipio,
                    endereco_obra=item.endereco_obra,
                    area_construida_prevista=item.area_construida_prevista,
    ...
    async def get_by_codigo(self, codigo_licenca: str) -> LicencaUrbanistica | None:
        if self._session:
            result = await self._session.execute(
                select(LicencaUrbanisticaModel).where(
                    LicencaUrbanisticaModel.codigo_licenca == codigo_licenca
                )
            )
        ...
        if self._session:
            ...
            if tipo_alvara:
                statement = statement.where(
                    LicencaUrbanisticaModel.tipo_alvara == tipo_alvara.value
                )
            if provincia:
                statement = statement.where(
                    func.lower(LicencaUrbanisticaModel.provincia) == provincia.strip().lower()
                )
            result = await self._session.execute(
                statement.order_by(LicencaUrbanisticaModel.codigo_licenca.asc())
            )
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(LicencaUrbanisticaModel)
                .where(LicencaUrbanisticaModel.codigo_licenca.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: LicencaUrbanisticaModel) -> LicencaUrbanistica:
        return LicencaUrbanistica(
            id=model.id,
            codigo_licenca=model.codigo_licenca,
            numero_processo=model.numero_processo,
            tipo_alvara=TipoAlvara(model.tipo_alvara),
            status=StatusLicencaUrbanistica(model.status),
            requerente_id=model.requerente_id,
            zoneamento_id=model.zoneamento_id,
            provincia=model.provincia,
            municipio=model.municipio,
            endereco_obra=model.endereco_obra,
            area_construida_prevista=Decimal(model.area_construida_prevista)
            if model.area_construida_prevista is not None
            else None,
            data_requerimento=model.data_requerimento,
            data_emissao=model.data_emissao,
            data_validade=model.data_validade,
            tecnico_responsavel_id=model.tecnico_responsavel_id,
