...
class LicencaUrbanistica:
    ...
    def criar(
        ...
        *,
        ...
        return cls(
            id=uuid4(),
            codigo_licenca=codigo_licenca.strip(),
            ...
            endereco_obra=endereco_obra.strip() if endereco_obra else None,
            area_construida_prevista=area_construida_prevista,
        )

    def iniciar_analise(self) -> None:
        if self.status not in {
            StatusLicencaUrbanistica.REQUERIDA,
            StatusLicencaUrbanistica.PENDENTE_DOCUMENTACAO,
        }:
    ...
    def deferir(
        self, *, data_emissao: date, data_validade: date, tecnico_responsavel_id: UUID
    ) -> None:
        if self.status != StatusLicencaUrbanistica.EM_ANALISE:
            raise ValueError("Licenca precisa estar em analise")
    ...
    def indeferir(self, *, motivo: str) -> None:
        if self.status not in {
            StatusLicencaUrbanistica.EM_ANALISE,
            StatusLicencaUrbanistica.PENDENTE_DOCUMENTACAO,
        }:
