...
class SQLAlchemyOutboxRepository(OutboxRepositoryPort):
    ...
    async def enqueue(self, event: Any) -> OutboxMessage:
        ...
        row = OutboxEventModel(id=uuid4(), event_name=event_name, topic=topic, payload=payload, headers={}, processed=False, retries=0, created_at=datetime.now(timezone.utc))
    ...
    async def get_pending(self, *, limit: int=100) -> list[OutboxMessage]:
        result = await self.session.execute(select(OutboxEventModel).where(OutboxEventModel.processed.is_(False)).order_by(OutboxEventModel.created_at.asc()).limit(limit))
        rows = result.scalars().all()
        return [self._to_message(row) for row in rows]

    async def mark_done(self, message_id: UUID) -> None:
        result = await self.session.execute(select(OutboxEventModel).where(OutboxEventModel.id == message_id))
    ...
    def _to_message(row: OutboxEventModel) -> OutboxMessage:
        return OutboxMessage(id=row.id, event_name=row.event_name, topic=row.topic, payload=dict(row.payload or {}), headers=dict(row.headers or {}), processed=bool(row.processed), retries=int(row.retries or 0), created_at=row.created_at, processed_at=row.processed_at, last_error=row.last_error)