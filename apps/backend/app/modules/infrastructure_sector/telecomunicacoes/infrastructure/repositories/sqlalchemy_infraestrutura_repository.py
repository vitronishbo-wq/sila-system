...
class SQLAlchemyInfraestruturaRepository(InfraestruturaRepositoryPort):
    ...
    async def get_by_codigo(self, codigo_infra: str) -> InfraestruturaTelco | None:
        stmt = select(InfraestruturaTelcoModel).where(InfraestruturaTelcoModel.codigo_infra == codigo_infra.strip())
        model = (await self.session.execute(stmt)).scalars().first()
        return self._to_domain(model) if model else None

    async def list_all(self) -> list[InfraestruturaTelco]:
        stmt = select(InfraestruturaTelcoModel).order_by(InfraestruturaTelcoModel.data_implantacao.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]

    async def list_by_operadora(self, operadora_id: UUID) -> list[InfraestruturaTelco]:
        stmt = select(InfraestruturaTelcoModel).where(InfraestruturaTelcoModel.operadora_id == operadora_id).order_by(InfraestruturaTelcoModel.data_implantacao.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]

    async def list_by_municipio(self, municipio: str) -> list[InfraestruturaTelco]:
        normalized = municipio.strip().lower()
        stmt = select(InfraestruturaTelcoModel).where(func.lower(InfraestruturaTelcoModel.municipio) == normalized).order_by(InfraestruturaTelcoModel.data_implantacao.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]

    async def list_ativas(self) -> list[InfraestruturaTelco]:
        stmt = select(InfraestruturaTelcoModel).where(InfraestruturaTelcoModel.ativo.is_(True), InfraestruturaTelcoModel.status == StatusInfraestrutura.ATIVA.value).order_by(InfraestruturaTelcoModel.data_implantacao.desc())
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(item) for item in rows]
    ...
    async def next_codigo(self) -> str:
        ano = date.today().year
        stmt = select(func.count()).select_from(InfraestruturaTelcoModel).where(InfraestruturaTelcoModel.codigo_infra.like(f'INF/{ano}/%'))
        count = (await self.session.execute(stmt)).scalar() or 0
        return f'INF/{ano}/{count + 1:05d}'

    @staticmethod
    def _to_domain(model: InfraestruturaTelcoModel) -> InfraestruturaTelco:
        return InfraestruturaTelco(id=model.id, codigo_infra=model.codigo_infra, operadora_id=model.operadora_id, tipo=TipoInfraestrutura(model.tipo), identificador=model.identificador, municipio=model.municipio, provincia=model.provincia, data_implantacao=model.data_implantacao, status=StatusInfraestrutura(model.status), latitude=float(model.latitude) if model.latitude is not None else None, longitude=float(model.longitude) if model.longitude is not None else None, capacidade=model.capacidade, observacoes=model.observacoes, ativo=model.ativo)