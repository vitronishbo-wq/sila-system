...
class SQLAlchemyFaturaRepository(FaturaRepositoryPort):
    ...
    async def get_by_numero(self, numero_fatura: str) -> FaturaTelecom | None:
        stmt = select(FaturaTelecomModel).where(FaturaTelecomModel.numero_fatura == numero_fatura.strip())
        model = (await self.session.execute(stmt)).scalars().first()
        return self._to_domain(model) if model else None

    async def list_by_assinante(self, assinante_id: UUID) -> list[FaturaTelecom]:
        stmt = select(FaturaTelecomModel).where(FaturaTelecomModel.assinante_id == assinante_id).order_by(FaturaTelecomModel.data_emissao.desc())
    ...
    async def next_numero(self) -> str:
        ...
        stmt = select(func.count()).select_from(FaturaTelecomModel).where(FaturaTelecomModel.numero_fatura.like(f'FAT/{year}/%'))
        count = (await self.session.execute(stmt)).scalar() or 0
        return f'FAT/{year}/{count + 1:06d}'

    @staticmethod
    def _to_domain(model: FaturaTelecomModel) -> FaturaTelecom:
        return FaturaTelecom(id=model.id, numero_fatura=model.numero_fatura, assinante_id=model.assinante_id, referencia=model.referencia, consumo_total_gb=Decimal(model.consumo_total_gb), franquia_gb=Decimal(model.franquia_gb), excedente_gb=Decimal(model.excedente_gb), valor_plano=Decimal(model.valor_plano), valor_excedente=Decimal(model.valor_excedente), valor_total=Decimal(model.valor_total), data_emissao=model.data_emissao, data_vencimento=model.data_vencimento, status=StatusFaturaTelecom(model.status), data_pagamento=model.data_pagamento, valor_pago=Decimal(model.valor_pago) if model.valor_pago is not None else None)