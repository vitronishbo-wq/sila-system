...
class AnatelAdapter:
    ...
    def __init__(self, *, base_url: str | None=None, api_key: str | None=None, timeout_seconds: float=10.0, failure_threshold: int=3, recovery_timeout: int=30, client: Any | None=None) -> None:
        self._base_url = (base_url or os.getenv('TELECOM_ANATEL_BASE_URL') or 'http://127.0.0.1:8090').rstrip('/')
        self._api_key = api_key or os.getenv('TELECOM_ANATEL_API_KEY')
        self._timeout_seconds = timeout_seconds
        self._client = client
        self._breaker = CircuitBreaker(failure_threshold=failure_threshold, recovery_timeout=recovery_timeout)
    ...
    async def registrar_reclamacao(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        return await self._guarded_request(method='POST', path='/api/v1/reclamacoes', payload=dict(payload))

    async def registrar_alerta_qualidade(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        return await self._guarded_request(method='POST', path='/api/v1/qualidade/alertas', payload=dict(payload))
    ...
    async def _send_http(self, *, method: str, url: str, payload: dict[str, Any], headers: dict[str, str]) -> httpx.Response:
        try:
            if self._client is not None:
                return await self._client.request(method=method, url=url, json=payload, headers=headers, timeout=self._timeout_seconds)
