...
class QualidadeServico:
    ...
    def registrar(
        ...
        *,
        ...
        return cls(
            id=uuid4(),
            codigo_medicao=codigo_medicao.strip(),
            operadora_id=operadora_id,
            servico=servico,
            data_medicao=data_medicao,
            disponibilidade_percentual=disponibilidade_percentual,
            latencia_ms=latencia_ms,
            jitter_ms=jitter_ms,
            perda_pacotes_percentual=perda_pacotes_percentual,
            velocidade_download_mbps=velocidade_download_mbps,
            velocidade_upload_mbps=velocidade_upload_mbps,
            status=StatusQualidadeServico.ALERTA,
    ...
    def avaliar_conformidade(self, sla: SLA | None = None) -> None:
        if sla is None:
            ...
            conforme = (
                self.disponibilidade_percentual >= sla.disponibilidade_min_percentual
                and self.latencia_ms <= sla.latencia_max_ms
                and (self.jitter_ms <= sla.jitter_max_ms)
                and (self.perda_pacotes_percentual <= sla.perda_pacotes_max_percentual)
                and (self.velocidade_download_mbps >= sla.velocidade_download_min_mbps)
                and (self.velocidade_upload_mbps >= sla.velocidade_upload_min_mbps)
            )
            self.status = (
                StatusQualidadeServico.CONFORME if conforme else StatusQualidadeServico.CRITICO
            )
