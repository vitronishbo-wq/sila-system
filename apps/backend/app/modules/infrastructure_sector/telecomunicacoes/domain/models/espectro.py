...
class Espectro:
    ...
    @classmethod
    def registrar(cls, *, codigo_espectro: str, tipo: TipoEspectro, frequencia_inicial_mhz: float, frequencia_final_mhz: float, servico_principal: TipoServico, municipio: str, provincia: str, outorga_id: UUID | None=None, observacoes: str | None=None) -> 'Espectro':
        ...
        return cls(id=uuid4(), codigo_espectro=codigo_espectro.strip(), tipo=tipo, frequencia_inicial_mhz=frequencia_inicial_mhz, frequencia_final_mhz=frequencia_final_mhz, largura_banda_mhz=largura_banda, servico_principal=servico_principal, municipio=municipio.strip(), provincia=provincia.strip(), status=status, outorga_id=outorga_id, observacoes=observacoes.strip() if observacoes else None, ativo=True)
