...
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.services.assinante_service import (
    AssinanteService,
)
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.services.espectro_service import (
    EspectroService,
)
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.services.indicador_qualidade_service import (
    IndicadorQualidadeService,
)
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.services.infraestrutura_service import (
    InfraestruturaService,
)
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.services.operadora_service import (
    OperadoraService,
)
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.services.outorga_espectro_service import (
    OutorgaEspectroService,
)
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.services.qualidade_servico_service import (
    QualidadeServicoService,
)
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.services.sla_service import (
    SLAService,
)
...
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.tests._fakes import (
    FakeCitizenService,
)
...
def _tables():
    ...
    if not hasattr(OperadoraModel, "__table__"):
        pytest.skip(
            "ORM mappers limpos por conftest global apos import de modelos; executar este teste sem tests/conftest ou revisar clear_mappers global."
        )
    return [
        OperadoraModel.__table__,
        AssinanteModel.__table__,
        InfraestruturaTelcoModel.__table__,
        OutorgaEspectroModel.__table__,
        EspectroModel.__table__,
        SLAModel.__table__,
        QualidadeServicoModel.__table__,
        IndicadorQualidadeModel.__table__,
    ]
...
def test_fluxo_real_orm_telecom_completo_com_qualidade_sla_indicadores() -> None:
    ...
    async def scenario() -> None:
        async with _session_scope() as session:
            ...
            assinante_service = AssinanteService(
                assinante_repo=assinante_repo,
                operadora_repo=operadora_repo,
                citizen_service=FakeCitizenService(active=True),
            )
            infraestrutura_service = InfraestruturaService(
                infraestrutura_repo=infraestrutura_repo, operadora_repo=operadora_repo
            )
            outorga_service = OutorgaEspectroService(
                outorga_repo=outorga_repo, operadora_repo=operadora_repo
            )
            espectro_service = EspectroService(
                espectro_repo=espectro_repo, outorga_repo=outorga_repo
            )
            sla_service = SLAService(sla_repo=sla_repo, operadora_repo=operadora_repo)
            qualidade_service = QualidadeServicoService(
                qualidade_repo=qualidade_repo,
                sla_repo=sla_repo,
                operadora_repo=operadora_repo,
                assinante_repo=assinante_repo,
            )
            indicador_service = IndicadorQualidadeService(
                indicador_repo=indicador_repo,
                qualidade_repo=qualidade_repo,
                operadora_repo=operadora_repo,
            )
            operadora = await operadora_service.cadastrar_operadora(
                cnpj="44.444.444/0001-44",
                razao_social="Fibra Angola SA",
                tipo=TipoOperadora.PROVEDOR,
                servicos_autorizados=[TipoServico.INTERNET_FIXA, TipoServico.INTERNET_MOVEL],
                endereco="Avenida Digital, 1",
                municipio="Luanda",
                provincia="Luanda",
                telefone="222444444",
                email="operacao@fibra.ao",
                representante_legal="Rita Almeida",
                representante_documento="44455566677",
                representante_cargo="Diretora",
            )
            assinante = await assinante_service.cadastrar_assinante(
                operadora_id=operadora.id,
                tipo_plano=TipoPlano.EMPRESARIAL,
                servico_principal=TipoServico.INTERNET_FIXA,
                municipio="Luanda",
                provincia="Luanda",
                citizen_id=uuid4(),
                contrato_numero="CTR-2026-0001",
                valor_mensal=85000.0,
            )
            infraestrutura = await infraestrutura_service.cadastrar_infraestrutura(
                operadora_id=operadora.id,
                tipo=TipoInfraestrutura.FIBRA_OPTICA,
                identificador="FIB-LDA-ORM",
                municipio="Luanda",
                provincia="Luanda",
                data_implantacao=date.today(),
            )
            outorga = await outorga_service.emitir_outorga(
                operadora_id=operadora.id,
                tipo_outorga=TipoOutorga.AUTORIZACAO,
                faixa_inicio_mhz=3400.0,
                faixa_fim_mhz=3600.0,
                data_outorga=date.today(),
            )
            espectro = await espectro_service.registrar_espectro(
                tipo=TipoEspectro.BANDA_LARGA,
                frequencia_inicial_mhz=3400.0,
                frequencia_final_mhz=3600.0,
                servico_principal=TipoServico.INTERNET_MOVEL,
                municipio="Luanda",
                provincia="Luanda",
                outorga_id=outorga.id,
            )
            sla = await sla_service.criar_sla(
                operadora_id=operadora.id,
                nome="SLA Integra Telecom",
                servico=TipoServico.INTERNET_FIXA,
                disponibilidade_min_percentual=99.0,
                latencia_max_ms=60.0,
                jitter_max_ms=20.0,
                perda_pacotes_max_percentual=1.0,
                velocidade_download_min_mbps=100.0,
                velocidade_upload_min_mbps=50.0,
                data_inicio=date.today(),
            )
            medicao = await qualidade_service.registrar_medicao(
                operadora_id=operadora.id,
                assinante_id=assinante.id,
                sla_id=sla.id,
                servico=TipoServico.INTERNET_FIXA,
                data_medicao=date.today(),
                disponibilidade_percentual=99.3,
                latencia_ms=40.0,
                jitter_ms=15.0,
                perda_pacotes_percentual=0.4,
                velocidade_download_mbps=180.0,
                velocidade_upload_mbps=90.0,
            )
            indicador = await indicador_service.gerar_indicador_operadora(
                operadora_id=operadora.id,
                referencia_ano=date.today().year,
                referencia_mes=date.today().month,
            )
