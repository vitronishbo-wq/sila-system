...
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.handlers.anatel_handler import (
    AnatelHandler,
)
...
class FakeResponse:
    ...
    def raise_for_status(self) -> None:
        ...
        raise httpx.HTTPStatusError(
            message=f"status={self.status_code}", request=request, response=response
        )
...
def test_anatel_adapter_envia_reclamacao_com_circuit_breaker() -> None:
    ...
    async def scenario() -> None:
        client = FakeHttpClient(
            responses=[FakeResponse(json_body={"status": "accepted", "id": "ANATEL-1"})]
        )
        adapter = AnatelAdapter(
            base_url="http://anatel.local",
            api_key="secret-token",
            timeout_seconds=5.0,
            client=client,
        )
        response = await adapter.registrar_reclamacao(
            {
                "protocolo": "REC/2026/0001",
                "assinante_id": "9b8d4ab7-cc74-4705-ae93-8a23f8c8d910",
                "prioridade": "alta",
            }
        )
...
def test_anatel_adapter_abre_circuito_apos_falhas() -> None:
    ...
    async def scenario() -> None:
        ...
        adapter = AnatelAdapter(
            base_url="http://anatel.local", client=client, failure_threshold=2, recovery_timeout=60
        )
        payload = {
            "protocolo": "REC/2026/0002",
            "assinante_id": "406a08f6-88b0-43e2-a6cc-1f4122fd2d55",
            "prioridade": "media",
        }
...
def test_anatel_handler_publica_reclamacao_e_qualidade_nao_conforme() -> None:
    ...
    async def scenario() -> None:
        ...
        await handler.on_reclamacao_aberta(
            ReclamacaoTelecomAbertaEvent(
                event_id=uuid4(),
                timestamp=datetime.now(UTC),
                reclamacao_id=uuid4(),
                protocolo="REC/2026/0099",
                assinante_id=uuid4(),
                tipo="cobertura",
                prioridade="alta",
            )
        )
        await handler.on_qualidade_aferida(
            QualidadeServicoAferidaEvent(
                event_id=uuid4(),
                timestamp=datetime.now(UTC),
                assinante_id=uuid4(),
                referencia="2026-03",
                download_mbps=Decimal("8.00"),
                upload_mbps=Decimal("1.50"),
                latencia_ms=Decimal("210.00"),
                conforme=False,
            )
        )
        await handler.on_qualidade_aferida(
            QualidadeServicoAferidaEvent(
                ...
                assinante_id=uuid4(),
                referencia="2026-03",
                download_mbps=Decimal("150.00"),
                upload_mbps=Decimal("50.00"),
                latencia_ms=Decimal("32.00"),
                conforme=True,
            )
        )
