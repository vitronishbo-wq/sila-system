...
class ReclamacaoService:
    ...
    def __init__(self, *, assinante_repo: AssinanteRepositoryPort, reclamacao_repo: ReclamacaoRepositoryPort, request_service: RequestServicePort | None=None, outbox_repo: OutboxRepositoryPort | None=None) -> None:
        self._assinante_repo = assinante_repo
        self._reclamacao_repo = reclamacao_repo
        self._request_service = request_service
        self._outbox_repo = outbox_repo

    async def criar_reclamacao(self, *, assinante_id: UUID, tipo: TipoReclamacaoTelecom, descricao: str, prioridade: str) -> Reclamacao:
        assinante = await self._assinante_repo.get_by_id(assinante_id)
        if assinante is None:
            raise ValueError('Assinante nao encontrado')
        reclamacao = Reclamacao.abrir(assinante_id=assinante_id, tipo=tipo, descricao=descricao, prioridade=prioridade)
        ...
        if self._request_service is not None:
            await self._request_service.create_request(request_type='RECLAMACAO_TELECOM', entity_id=saved.id, citizen_id=assinante.citizen_id, numero_processo=saved.protocolo, metadata={'tipo': saved.tipo.value, 'prioridade': saved.prioridade, 'assinante_id': str(saved.assinante_id)})
