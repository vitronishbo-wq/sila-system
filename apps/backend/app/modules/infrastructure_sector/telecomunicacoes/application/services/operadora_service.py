...
class OperadoraService:
    ...
    async def cadastrar_operadora(
        ...
        *,
        ...
        operadora = Operadora.cadastrar(
            cnpj=cnpj,
            razao_social=razao_social,
            tipo=tipo,
            servicos_autorizados=servicos_autorizados,
            endereco=endereco,
            municipio=municipio,
            provincia=provincia,
            telefone=telefone,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CADASTRO_OPERADORA_TELECOM",
                entity_id=saved.id,
                ...
                metadata={
                    ...
                    "servicos": [item.value for item in saved.servicos_autorizados],
                },
            )
        return saved
    ...
    async def autorizar_operadora(
        self, *, operadora_id: UUID, outorga_id: UUID, data_autorizacao: date, data_validade: date
    ) -> Operadora:
        operadora = await self.buscar_operadora(operadora_id)
        operadora.autorizar(
            outorga_id=outorga_id, data_autorizacao=data_autorizacao, data_validade=data_validade
        )
