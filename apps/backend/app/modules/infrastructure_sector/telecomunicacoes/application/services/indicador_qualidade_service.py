...
class IndicadorQualidadeService:
    def __init__(
        self,
        *,
        indicador_repo: IndicadorQualidadeRepositoryPort,
        qualidade_repo: QualidadeServicoRepositoryPort,
        operadora_repo: OperadoraRepositoryPort,
    ...
    async def gerar_indicador_operadora(
        self,
        *,
        operadora_id: UUID,
        referencia_ano: int,
        ...
        medicoes = await self.qualidade_repo.list_by_operadora_periodo(
            operadora_id=operadora_id, referencia_ano=referencia_ano, referencia_mes=referencia_mes
        )
        if not medicoes:
            raise ValueError("Nao existem medicoes no periodo informado para gerar indicador")
        existente = await self.indicador_repo.get_by_operadora_periodo(
            operadora_id=operadora_id, referencia_ano=referencia_ano, referencia_mes=referencia_mes
        )
        codigo = (
            existente.codigo_indicador
            if existente is not None
            else await self.indicador_repo.next_codigo()
        )
        indicador = self._build_indicador(
            codigo=codigo,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="GERACAO_INDICADOR_QUALIDADE_TELECOM",
                entity_id=saved.id,
                numero_processo=saved.codigo_indicador,
                metadata={
                    "codigo_indicador": saved.codigo_indicador,
                    "operadora_id": str(saved.operadora_id),
                    "referencia": f"{saved.referencia_ano:04d}-{saved.referencia_mes:02d}",
    ...
    async def listar_indicadores(
        self, *, operadora_id: UUID | None = None, status: StatusIndicadorQualidade | None = None
    ) -> list[IndicadorQualidade]:
    ...
    def _build_indicador(
        *,
        ...
        referencia_ano: int,
        referencia_mes: int,
        medicoes: list[QualidadeServico],
        ...
        return IndicadorQualidade.registrar(
            codigo_indicador=codigo,
            operadora_id=operadora_id,
            referencia_ano=referencia_ano,
            referencia_mes=referencia_mes,
            total_medicoes=total,
            disponibilidade_media_percentual=disponibilidade_media,
            latencia_media_ms=latencia_media,
            jitter_medio_ms=jitter_medio,
            perda_pacotes_media_percentual=perda_media,
