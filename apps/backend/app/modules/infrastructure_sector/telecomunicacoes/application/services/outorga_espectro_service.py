...
class OutorgaEspectroService:
    def __init__(
        self,
        *,
        outorga_repo: OutorgaEspectroRepositoryPort,
    ...
    async def emitir_outorga(
        ...
        *,
        ...
        outorga = OutorgaEspectro.emitir(
            numero_outorga=numero,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="EMISSAO_OUTORGA_ESPECTRO_TELECOM",
                entity_id=saved.id,
                numero_processo=saved.numero_outorga,
                metadata={
                    "numero_outorga": saved.numero_outorga,
                    "operadora_id": str(saved.operadora_id),
                    "tipo_outorga": saved.tipo_outorga.value,
                },
            )
        return saved
    ...
    async def listar_outorgas(
        self, *, operadora_id: UUID | None = None, status: StatusOutorga | None = None
    ) -> list[OutorgaEspectro]:
