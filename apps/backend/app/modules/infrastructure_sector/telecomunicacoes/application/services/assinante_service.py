...
class AssinanteService:
    ...
    async def cadastrar_assinante(
        ...
        *,
        ...
        assinante = Assinante.cadastrar(
            codigo_assinante=codigo,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CADASTRO_ASSINANTE_TELECOM",
                entity_id=saved.id,
                citizen_id=saved.citizen_id,
                numero_processo=saved.codigo_assinante,
                metadata={
                    "codigo_assinante": saved.codigo_assinante,
                    "operadora_id": str(saved.operadora_id),
                    "servico_principal": saved.servico_principal.value,
                    "tipo_plano": saved.tipo_plano.value,
                },
            )
        return saved
