...
class SLAService:
    ...
    async def criar_sla(
        ...
        *,
        ...
        sla = SLA.criar(
            ...
            nome=nome,
            servico=servico,
            disponibilidade_min_percentual=disponibilidade_min_percentual,
            latencia_max_ms=latencia_max_ms,
            jitter_max_ms=jitter_max_ms,
            perda_pacotes_max_percentual=perda_pacotes_max_percentual,
            velocidade_download_min_mbps=velocidade_download_min_mbps,
            velocidade_upload_min_mbps=velocidade_upload_min_mbps,
        ...
        if self.request_service is not None:
            await self.request_service.create_request(
                request_type="CADASTRO_SLA_TELECOM",
                entity_id=saved.id,
                numero_processo=saved.codigo_sla,
                metadata={
                    "codigo_sla": saved.codigo_sla,
    ...
    async def listar_slas(
        self, *, operadora_id: UUID | None = None, status: StatusSLA | None = None
    ) -> list[SLA]:
