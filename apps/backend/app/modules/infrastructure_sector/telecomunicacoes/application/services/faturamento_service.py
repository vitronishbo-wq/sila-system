...
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.domain.models.franquia import (
    Franquia,
)
...
class FaturamentoService:
    ...
    async def gerar_fatura_mensal(
        self, assinante_id: UUID, referencia: str, *, consumo_total_gb: Decimal
    ) -> FaturaTelecom:
        ...
        valor_plano = Decimal(
            str(assinante.valor_mensal if assinante.valor_mensal is not None else 0.0)
        )
        valor_excedente = franquia.calcular_valor_excedente(consumo_total_gb)
        fatura = FaturaTelecom.gerar(
            assinante_id=assinante.id,
            referencia=referencia,
            consumo_total_gb=consumo_total_gb,
            franquia_gb=franquia.franquia_gb,
