...
assinante_service_dep = Depends(get_assinante_service)


@router.post("/", response_model=AssinanteResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_assinante(
    data: AssinanteCreate, service: AssinanteService = assinante_service_dep
) -> AssinanteResponse:
    try:
        return await service.cadastrar_assinante(
            ...
            valor_mensal=float(data.valor_mensal) if data.valor_mensal is not None else None,
...
async def obter_assinante(
    assinante_id: UUID, service: AssinanteService = assinante_service_dep
) -> AssinanteResponse:
    try:
...
async def listar_assinantes(
    ...
    service: AssinanteService = assinante_service_dep,
) -> list[AssinanteResponse]:
    return await service.listar_assinantes(
        operadora_id=operadora_id, municipio=municipio, somente_ativos=somente_ativos
    )


@router.patch("/{assinante_id}/status", response_model=AssinanteResponse)
async def atualizar_status_assinante(
    assinante_id: UUID,
    data: AssinanteStatusUpdate,
    service: AssinanteService = assinante_service_dep,
) -> AssinanteResponse:
    try:
