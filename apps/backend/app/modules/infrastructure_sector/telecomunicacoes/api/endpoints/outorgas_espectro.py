...
outorga_espectro_service_dep = Depends(get_outorga_espectro_service)


@router.post("/", response_model=OutorgaEspectroResponse, status_code=status.HTTP_201_CREATED)
async def emitir_outorga(
    data: OutorgaEspectroCreate,
    service: OutorgaEspectroService = outorga_espectro_service_dep,
) -> OutorgaEspectroResponse:
    try:
        return await service.emitir_outorga(
            operadora_id=data.operadora_id,
            tipo_outorga=data.tipo_outorga,
            faixa_inicio_mhz=data.faixa_inicio_mhz,
            faixa_fim_mhz=data.faixa_fim_mhz,
...
async def obter_outorga(
    outorga_id: UUID, service: OutorgaEspectroService = outorga_espectro_service_dep
) -> OutorgaEspectroResponse:
    try:
...
async def listar_outorgas(
    ...
    status_filtro: StatusOutorga | None = None,
    service: OutorgaEspectroService = outorga_espectro_service_dep,
) -> list[OutorgaEspectroResponse]:
...
async def atualizar_status_outorga(
    outorga_id: UUID,
    data: OutorgaEspectroStatusUpdate,
    service: OutorgaEspectroService = outorga_espectro_service_dep,
) -> OutorgaEspectroResponse:
    try:
