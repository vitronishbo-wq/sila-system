...
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.api.deps import (
    get_indicador_qualidade_service,
)
...
router = APIRouter(
    prefix="/indicadores-qualidade", tags=["Telecomunicacoes - Indicadores Qualidade"]
)
indicador_qualidade_service_dep = Depends(get_indicador_qualidade_service)


@router.post(
    "/gerar", response_model=IndicadorQualidadeResponse, status_code=status.HTTP_201_CREATED
)
async def gerar_indicador(
    data: IndicadorQualidadeGerar,
    service: IndicadorQualidadeService = indicador_qualidade_service_dep,
) -> IndicadorQualidadeResponse:
    try:
        return await service.gerar_indicador_operadora(
...
async def listar_indicadores(
    ...
    status_filtro: StatusIndicadorQualidade | None = None,
    service: IndicadorQualidadeService = indicador_qualidade_service_dep,
) -> list[IndicadorQualidadeResponse]:
