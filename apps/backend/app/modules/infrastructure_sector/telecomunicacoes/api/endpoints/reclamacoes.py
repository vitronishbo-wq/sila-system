...
async def criar_reclamacao(
    payload: ReclamacaoCreate, service: ReclamacaoService = reclamacao_service_dep
) -> ReclamacaoResponse:
    try:
        return await service.criar_reclamacao(
...
async def listar_reclamacoes_assinante(
    assinante_id: UUID, service: ReclamacaoService = reclamacao_service_dep
) -> list[ReclamacaoResponse]:
