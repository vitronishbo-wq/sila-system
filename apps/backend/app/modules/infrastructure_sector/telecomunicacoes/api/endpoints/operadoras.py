...
operadora_service_dep = Depends(get_operadora_service)


@router.post("/", response_model=OperadoraResponse, status_code=status.HTTP_201_CREATED)
async def cadastrar_operadora(
    data: OperadoraCreate, service: OperadoraService = operadora_service_dep
) -> OperadoraResponse:
    try:
        return await service.cadastrar_operadora(
            cnpj=data.cnpj,
...
async def obter_operadora(
    operadora_id: UUID, service: OperadoraService = operadora_service_dep
) -> OperadoraResponse:
    try:
...
async def listar_operadoras(
    servico: TipoServico | None = None,
    municipio: str | None = None,
    somente_ativas: bool = False,
    service: OperadoraService = operadora_service_dep,
) -> list[OperadoraResponse]:
    return await service.listar_operadoras(
        servico=servico, municipio=municipio, somente_ativas=somente_ativas
    )


@router.patch("/{operadora_id}/autorizar", response_model=OperadoraResponse)
async def autorizar_operadora(
    operadora_id: UUID,
    data: OperadoraAuthorize,
    service: OperadoraService = operadora_service_dep,
) -> OperadoraResponse:
    try:
        return await service.autorizar_operadora(
