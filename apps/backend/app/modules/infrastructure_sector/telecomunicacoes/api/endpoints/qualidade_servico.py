...
qualidade_servico_service_dep = Depends(get_qualidade_servico_service)


@router.post("/", response_model=QualidadeServicoResponse, status_code=status.HTTP_201_CREATED)
async def registrar_medicao(
    data: QualidadeServicoCreate,
    service: QualidadeServicoService = qualidade_servico_service_dep,
) -> QualidadeServicoResponse:
    try:
        return await service.registrar_medicao(
            operadora_id=data.operadora_id,
            servico=data.servico,
            data_medicao=data.data_medicao,
            disponibilidade_percentual=data.disponibilidade_percentual,
            latencia_ms=data.latencia_ms,
            jitter_ms=data.jitter_ms,
            perda_pacotes_percentual=data.perda_pacotes_percentual,
            velocidade_download_mbps=data.velocidade_download_mbps,
            velocidade_upload_mbps=data.velocidade_upload_mbps,
            assinante_id=data.assinante_id,
            sla_id=data.sla_id,
...
async def obter_medicao(
    medicao_id: UUID, service: QualidadeServicoService = qualidade_servico_service_dep
) -> QualidadeServicoResponse:
    try:
...
async def listar_medicoes(
    ...
    status_filtro: StatusQualidadeServico | None = None,
    service: QualidadeServicoService = qualidade_servico_service_dep,
) -> list[QualidadeServicoResponse]:
    return await service.listar_medicoes(
        operadora_id=operadora_id, assinante_id=assinante_id, status=status_filtro
    )


@router.patch("/{medicao_id}/status", response_model=QualidadeServicoResponse)
async def atualizar_status_medicao(
    medicao_id: UUID,
    data: QualidadeServicoStatusUpdate,
    service: QualidadeServicoService = qualidade_servico_service_dep,
) -> QualidadeServicoResponse:
    try:
