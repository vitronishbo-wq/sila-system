...
async def cadastrar_infraestrutura(
    ...
    try:
        return await service.cadastrar_infraestrutura(
...
async def obter_infraestrutura(
    infraestrutura_id: UUID, service: InfraestruturaService = infraestrutura_service_dep
) -> InfraestruturaResponse:
    try:
...
async def listar_infraestruturas(
    ...
    return await service.listar_infraestruturas(
        operadora_id=operadora_id, municipio=municipio, somente_ativas=somente_ativas
    )


@router.patch("/{infraestrutura_id}/status", response_model=InfraestruturaResponse)
async def atualizar_status_infraestrutura(
    infraestrutura_id: UUID,
    data: InfraestruturaStatusUpdate,
    service: InfraestruturaService = infraestrutura_service_dep,
) -> InfraestruturaResponse:
    try:
        return await service.atualizar_status(
            infraestrutura_id=infraestrutura_id, status=data.status
        )
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
