...
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.events import (
    event_bus,
)
...
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.handlers import (
    AnatelHandler,
    FaturamentoHandler,
    QualidadeHandler,
)
...
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.application.services.reclamacao_service import (
    ReclamacaoService,
)
...
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.infrastructure.adapters.citizen_service_adapter import (
    CitizenServiceAdapter,
)
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.infrastructure.adapters.request_service_adapter import (
    RequestServiceAdapter,
)
from apps.backend.app.modules.infrastructure_sector.telecomunicacoes.infrastructure.persistence import (
...
def _configure_subscriptions() -> None:
    ...
    event_bus.subscribe(
        QualidadeServicoAferidaEvent, qualidade_handler_singleton.on_qualidade_aferida
    )
...
async def get_operadora_service(session: AsyncSession = db_dep) -> OperadoraService:
...
async def get_assinante_service(session: AsyncSession = db_dep) -> AssinanteService:
    operadora_repo = SQLAlchemyOperadoraRepository(session)
    return AssinanteService(
...
async def get_infraestrutura_service(
    session: AsyncSession = db_dep,
) -> InfraestruturaService:
    operadora_repo = SQLAlchemyOperadoraRepository(session)
    return InfraestruturaService(
...
async def get_outorga_espectro_service(
    session: AsyncSession = db_dep,
) -> OutorgaEspectroService:
    operadora_repo = SQLAlchemyOperadoraRepository(session)
    return OutorgaEspectroService(
...
async def get_espectro_service(session: AsyncSession = db_dep) -> EspectroService:
    return EspectroService(
...
async def get_sla_service(session: AsyncSession = db_dep) -> SLAService:
    operadora_repo = SQLAlchemyOperadoraRepository(session)
    return SLAService(
...
async def get_qualidade_servico_service(
    session: AsyncSession = db_dep,
) -> QualidadeServicoService:
    return QualidadeServicoService(
...
async def get_indicador_qualidade_service(
    session: AsyncSession = db_dep,
) -> IndicadorQualidadeService:
    return IndicadorQualidadeService(
...
async def get_faturamento_service(session: AsyncSession = db_dep) -> FaturamentoService:
    return FaturamentoService(
        assinante_repo=SQLAlchemyAssinanteRepository(session),
        fatura_repo=SQLAlchemyFaturaRepository(session),
        outbox_repo=SQLAlchemyOutboxRepository(session),
    )


async def get_reclamacao_service(session: AsyncSession = db_dep) -> ReclamacaoService:
    return ReclamacaoService(
        assinante_repo=SQLAlchemyAssinanteRepository(session),
        reclamacao_repo=SQLAlchemyReclamacaoRepository(session),
        request_service=RequestServiceAdapter(ServiceRequestLifecycleBridge(session)),
        outbox_repo=SQLAlchemyOutboxRepository(session),
    )
