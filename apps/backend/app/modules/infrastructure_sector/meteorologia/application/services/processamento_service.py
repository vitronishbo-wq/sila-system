...
class ProcessamentoService:
    ...
    def __init__(self, *, observacao_repository: ObservacaoRepositoryPort, estacao_repository: EstacaoRepositoryPort, alerta_service: AlertaServicePort | None=None) -> None:
    ...
    async def processar_observacao(self, observacao: ObservacaoMeteorologica) -> dict[str, Any]:
        resultado: dict[str, Any] = {'sucesso': False, 'observacao_id': str(observacao.id), 'alertas_gerados': 0, 'warnings': [], 'erros': [], 'observacao': None}
        ...
        if observacao.estacao_id:
            ...
            if not estacao.is_active:
                resultado['warnings'].append(f'Estacao {estacao.codigo} esta {estacao.status.value}')
        ...
        if alertas and self.alerta_service and observacao.estacao_id:
            for alerta in alertas:
                publicado = await self.alerta_service.publicar_alerta(estacao_id=observacao.estacao_id, alerta=alerta, observacao_id=observacao.id)
                if not publicado:
            ...
            if estacao and estacao.provincia:
                severos = [alerta for alerta in alertas if alerta.get('severidade') in {'HIGH', 'EXTREME'}]
                if severos:
                    ok = await self.alerta_service.notificar_defesa_civil(alertas=severos, provincia=estacao.provincia)
                    if not ok:
                        resultado['warnings'].append('Falha ao notificar Defesa Civil')
        return resultado

    async def processar_lote_observacoes(self, observacoes: list[ObservacaoMeteorologica]) -> dict[str, Any]:
        ...
        for observacao in observacoes:
            ...
            if resultado['sucesso']:
                ...
                falhas += 1
        return {'total': len(observacoes), 'sucessos': sucessos, 'falhas': falhas, 'total_alertas': total_alertas, 'detalhes': resultados}
    ...
    async def listar_observacoes_estacao(self, *, estacao_id: UUID, start_date: datetime | None=None, end_date: datetime | None=None, limit: int=100) -> list[ObservacaoMeteorologica]:
        return await self.observacao_repository.list_by_estacao(estacao_id=estacao_id, start_date=start_date, end_date=end_date, limit=limit)
    ...
    async def listar_observacoes_com_alertas(self, *, start_date: datetime | None=None, end_date: datetime | None=None, limit: int=50) -> list[ObservacaoMeteorologica]:
        return await self.observacao_repository.list_with_alerts(start_date=start_date, end_date=end_date, limit=limit)