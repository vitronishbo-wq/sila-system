...
class EstacaoService:
    ...
    async def listar_estacoes(
        ...
        *,
        ...
        return await self.estacao_repository.list_filtered(
            provincia=provincia, status=status_enum, limit=limit, offset=offset
        )

    async def atualizar_estacao(
        self, estacao_id: UUID, payload: dict[str, Any]
    ) -> EstacaoMeteorologica:
