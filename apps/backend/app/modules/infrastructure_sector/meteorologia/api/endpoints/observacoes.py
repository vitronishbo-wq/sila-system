...
from apps.backend.app.modules.infrastructure_sector.meteorologia.api.deps import (
    get_processamento_service,
)
...
from apps.backend.app.modules.infrastructure_sector.meteorologia.application.services.processamento_service import (
...
processamento_service_dep = Depends(get_processamento_service)
start_date_query = Query(None)
end_date_query = Query(None)
alert_limit_query = Query(50, ge=1, le=200)
estacao_limit_query = Query(100, ge=1, le=1000)


@router.post("/", response_model=ObservacaoResponseSchema, status_code=status.HTTP_201_CREATED)
async def criar_observacao(
    payload: ObservacaoCreateSchema,
    service: ProcessamentoService = processamento_service_dep,
) -> ObservacaoResponseSchema:
    observacao = ObservacaoMeteorologica(
        estacao_id=payload.estacao_id,
        data_observacao=payload.data_observacao,
        temperatura=payload.temperatura,
        humidade=payload.humidade,
        pressao=payload.pressao,
        velocidade_vento=payload.velocidade_vento,
        direcao_vento=payload.direcao_vento,
        precipitacao=payload.precipitacao,
        radiacao_solar=payload.radiacao_solar,
        tipo=payload.tipo,
    ...
    if not resultado["sucesso"]:
        raise HTTPException(
            ...
            detail={"erros": resultado["erros"], "warnings": resultado["warnings"]},
        )
...
async def listar_observacoes_com_alertas(
    start_date: datetime | None = start_date_query,
    end_date: datetime | None = end_date_query,
    limit: int = alert_limit_query,
    service: ProcessamentoService = processamento_service_dep,
) -> list[ObservacaoResponseSchema]:
    observacoes = await service.listar_observacoes_com_alertas(
...
async def listar_observacoes_estacao(
    ...
    start_date: datetime | None = start_date_query,
    end_date: datetime | None = end_date_query,
    limit: int = estacao_limit_query,
    service: ProcessamentoService = processamento_service_dep,
) -> list[ObservacaoResponseSchema]:
    observacoes = await service.listar_observacoes_estacao(
...
async def obter_observacao(
    observacao_id: UUID, service: ProcessamentoService = processamento_service_dep
) -> ObservacaoResponseSchema:
    try:
