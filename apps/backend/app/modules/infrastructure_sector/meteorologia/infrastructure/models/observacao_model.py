...
class ObservacaoMeteorologicaModel(Base):
    ...
    estacao_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("meteorologia_estacoes.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    data_observacao: Mapped[datetime] = mapped_column(
    ...
    __table_args__ = (
        Index("idx_meteorologia_obs_estacao_data", "estacao_id", "data_observacao"),
        Index(
            "idx_meteorologia_obs_alertas_true",
            "created_at",
            postgresql_where=text("has_alerts IS TRUE"),
        ),
    )
