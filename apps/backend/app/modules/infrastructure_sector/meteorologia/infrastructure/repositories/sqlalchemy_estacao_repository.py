...
from apps.backend.app.modules.infrastructure_sector.meteorologia.infrastructure.models.estacao_model import (
    EstacaoMeteorologicaModel,
)
...
class SQLAlchemyEstacaoRepository(EstacaoRepositoryPort):
    ...
    async def get_by_id(self, estacao_id: UUID) -> EstacaoMeteorologica | None:
        stmt = select(EstacaoMeteorologicaModel).where(
            EstacaoMeteorologicaModel.id == estacao_id,
            EstacaoMeteorologicaModel.deleted_at.is_(None),
        )
    ...
    async def get_by_codigo(self, codigo: str) -> EstacaoMeteorologica | None:
        stmt = select(EstacaoMeteorologicaModel).where(
            EstacaoMeteorologicaModel.codigo == codigo.strip().upper(),
            EstacaoMeteorologicaModel.deleted_at.is_(None),
        )
        model = (await self.session.execute(stmt)).scalars().first()
        return self._to_domain(model) if model else None
    ...
    async def list_all(self, limit: int = 100, offset: int = 0) -> list[EstacaoMeteorologica]:
        stmt = (
            select(EstacaoMeteorologicaModel)
            .where(EstacaoMeteorologicaModel.deleted_at.is_(None))
            .order_by(EstacaoMeteorologicaModel.created_at.desc())
            .offset(offset)
            .limit(limit)
        )
    ...
    async def list_active(self, provincia: str | None = None) -> list[EstacaoMeteorologica]:
        stmt = select(EstacaoMeteorologicaModel).where(
            EstacaoMeteorologicaModel.deleted_at.is_(None),
            EstacaoMeteorologicaModel.status == StationStatus.ACTIVE.value,
        )
        if provincia:
    ...
    async def list_filtered(
        ...
        *,
        ...
        stmt = (
            stmt.order_by(EstacaoMeteorologicaModel.created_at.desc()).offset(offset).limit(limit)
        )
        rows = (await self.session.execute(stmt)).scalars().all()
        return [self._to_domain(row) for row in rows]
    ...
    def _to_domain(model: EstacaoMeteorologicaModel) -> EstacaoMeteorologica:
        return EstacaoMeteorologica(
            ...
            status=StationStatus(model.status),
