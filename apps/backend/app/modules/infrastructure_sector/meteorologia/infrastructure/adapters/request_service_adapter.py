...
class RequestServiceAdapter(RequestServicePort):
    ...
    async def criar_request(self, request_data: dict) -> bool:
        ...
        try:
            ...
            return False
        numero_processo = request_data.get('numero_processo') or f'MET-{date.today().year}-{str(entity_id)[:8]}'
        modulo_origem = request_data.get('tipo') or 'METEOROLOGIA'
        await self._bridge.create_education_request(entity_id=entity_id, citizen_id=citizen_id, numero_processo=numero_processo, escola_nome=f'Alertas/{modulo_origem}', ano_letivo=str(date.today().year))
        return True