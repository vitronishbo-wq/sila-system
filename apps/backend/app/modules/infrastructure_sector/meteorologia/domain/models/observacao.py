...
class ObservacaoMeteorologica:
    ...
    def __init__(
        self,
        *,
        observacao_id: UUID | None = None,
        estacao_id: UUID | None = None,
        data_observacao: datetime | None = None,
        temperatura: float | None = None,
        humidade: float | None = None,
        pressao: float | None = None,
        velocidade_vento: float | None = None,
        direcao_vento: float | None = None,
        precipitacao: float | None = None,
        radiacao_solar: float | None = None,
        tipo: ObservationType = ObservationType.SURFACE,
        qualidade_dados: str | None = None,
    ...
    def analisar_severidade(self) -> list[dict[str, Any]]:
        ...
        if self.temperatura is not None and self.temperatura >= self.TEMP_EXTREME_HIGH:
            ...
            self._alertas.append(
                {
                    "tipo": AlertType.HEAT_WAVE.value,
                    "severidade": severity.value,
                    "mensagem": f"Temperatura critica: {self.temperatura}C",
                    "valor": self.temperatura,
                    "limite": self.TEMP_EXTREME_HIGH,
                }
            )
        if self.temperatura is not None and self.temperatura <= self.TEMP_EXTREME_LOW:
            ...
            self._alertas.append(
                {
                    "tipo": AlertType.FROST.value,
                    "severidade": severity.value,
                    "mensagem": f"Temperatura muito baixa: {self.temperatura}C",
                    "valor": self.temperatura,
                    "limite": self.TEMP_EXTREME_LOW,
                }
            )
        if self.humidade is not None and self.humidade <= self.HUMIDITY_CRITICAL_LOW:
            ...
            self._alertas.append(
                {
                    "tipo": AlertType.DROUGHT.value,
                    "severidade": severity.value,
                    "mensagem": f"Humidade critica: {self.humidade}%",
                    "valor": self.humidade,
                    "limite": self.HUMIDITY_CRITICAL_LOW,
                }
            )
        if self.velocidade_vento is not None and self.velocidade_vento >= self.WIND_DANGEROUS_HIGH:
            ...
            self._alertas.append(
                {
                    "tipo": AlertType.STRONG_WIND.value,
                    "severidade": severity.value,
                    "mensagem": f"Vento perigoso: {self.velocidade_vento} km/h",
                    "valor": self.velocidade_vento,
                    "limite": self.WIND_DANGEROUS_HIGH,
                }
            )
        if self.pressao is not None and self.pressao <= self.PRESSURE_CRITICAL_LOW:
            self._alertas.append(
                {
                    "tipo": AlertType.STORM.value,
                    "severidade": AlertSeverity.MODERATE.value,
                    "mensagem": f"Pressao muito baixa: {self.pressao} hPa",
                    "valor": self.pressao,
                    "limite": self.PRESSURE_CRITICAL_LOW,
                }
            )
        if self.precipitacao is not None and self.precipitacao >= self.RAINFALL_HEAVY:
            ...
            self._alertas.append(
                {
                    "tipo": AlertType.HEAVY_RAIN.value,
                    "severidade": severity.value,
                    "mensagem": f"Precipitacao intensa: {self.precipitacao} mm",
                    "valor": self.precipitacao,
                    "limite": self.RAINFALL_HEAVY,
                }
            )
        return list(self._alertas)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "id": str(self.id),
            "estacao_id": str(self.estacao_id) if self.estacao_id else None,
            "data_observacao": self.data_observacao.isoformat() if self.data_observacao else None,
            "leituras": {
