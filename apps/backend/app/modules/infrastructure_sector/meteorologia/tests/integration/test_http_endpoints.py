...
from apps.backend.app.modules.infrastructure_sector.meteorologia.api.router import (
    router as meteorologia_router,
)
...
class FakeEstacaoService:
    ...
    async def atualizar_estacao(self, estacao_id: UUID, payload: dict) -> EstacaoMeteorologica:
        ...
        estacao.updated_at = datetime.now(UTC)
        return estacao
    ...
    async def processar_observacao(self, observacao: ObservacaoMeteorologica) -> dict:
        if observacao.estacao_id not in self._estacao_service._estacoes:
            return {
                ...
                "alertas_gerados": 0,
                "warnings": [],
                "erros": [f"Estacao {observacao.estacao_id} nao encontrada"],
                "observacao": None,
            }
        ...
        return {
            "sucesso": True,
            "observacao_id": str(observacao.id),
            "alertas_gerados": len(observacao.alerts),
            "warnings": [],
            "erros": [],
...
def test_estacoes_obter_atualizar_remover_fluxo(http_client: TestClient) -> None:
    ...
    put_response = http_client.put(
        f"/api/v1/meteorologia/estacoes/{estacao_id}",
        json={"nome": "Estacao Atualizada", "status": "MAINTENANCE"},
    )
...
def test_observacoes_criar_obter_e_listar_alertas(http_client: TestClient) -> None:
    ...
    post_response = http_client.post(
        "/api/v1/meteorologia/observacoes/",
        json={
            "estacao_id": estacao["id"],
            "temperatura": 44.2,
            "humidade": 12.0,
            "velocidade_vento": 90.0,
            "tipo": "SURFACE",
        },
    )
...
def test_observacoes_listar_por_estacao_e_validar_erro_estacao_inexistente(
    ...
    response_a = http_client.post(
        "/api/v1/meteorologia/observacoes/",
        json={"estacao_id": estacao_id, "temperatura": 30.0, "humidade": 60.0, "tipo": "SURFACE"},
    )
    assert response_a.status_code == 201
    response_b = http_client.post(
        "/api/v1/meteorologia/observacoes/",
        json={"estacao_id": estacao_id, "temperatura": 41.0, "humidade": 18.0, "tipo": "SURFACE"},
    )
    assert response_b.status_code == 201
    list_response = http_client.get(
        f"/api/v1/meteorologia/observacoes/estacao/{estacao_id}?limit=10"
    )
    ...
    invalid_response = http_client.post(
        "/api/v1/meteorologia/observacoes/",
        json={"estacao_id": str(uuid4()), "temperatura": 29.0, "humidade": 40.0, "tipo": "SURFACE"},
    )
