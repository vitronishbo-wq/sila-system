...
class OcorrenciaService:
    ...
    def __init__(self, *, ocorrencia_repo: OcorrenciaRepositoryPort, outbox: InMemoryOutbox) -> None:
        self.ocorrencia_repo = ocorrencia_repo
        self.outbox = outbox

    async def registrar_ocorrencia(self, *, tipo: TipoOcorrencia, aeronave_id: UUID, data_ocorrencia: datetime, local: dict, fase_voo: FaseVoo, descricao: str, vitimas: dict[str, int], danos: str, voo_id: UUID | None=None) -> Ocorrencia:
        ocorrencia = Ocorrencia(tipo=tipo, voo_id=voo_id, aeronave_id=aeronave_id, data_ocorrencia=data_ocorrencia, local=local, fase_voo=fase_voo, descricao=descricao, vitimas=vitimas, danos=danos)
        await self.ocorrencia_repo.save(ocorrencia)
        evento = OcorrenciaRegistradaEvent(ocorrencia_id=ocorrencia.id, numero_ocorrencia=ocorrencia.numero_ocorrencia, tipo=ocorrencia.tipo.value, aeronave_id=ocorrencia.aeronave_id, gravidade=ocorrencia.gravidade.value, data_ocorrencia=ocorrencia.data_ocorrencia)
