...
class VooService:
    def __init__(
        self,
        *,
        voo_repo: VooRepositoryPort,
        aeronave_repo: AeronaveRepositoryPort,
        outbox: InMemoryOutbox,
        decea_adapter: DeceaAdapter | None = None,
        meteorologia_adapter: MeteorologiaAdapter | None = None,
    ) -> None:
    ...
    async def programar_voo(
        ...
        *,
        ...
        voo = Voo(
            ...
            aeroporto_origem_id=aeroporto_origem_id,
            aeroporto_destino_id=aeroporto_destino_id,
            data_hora_partida_programada=data_hora_partida,
            data_hora_chegada_programada=data_hora_chegada,
            tipo=tipo,
            natureza=natureza,
            regras=regras,
        ...
        evento = VooProgramadoEvent(
            voo_id=voo.id,
            numero_voo=voo.numero_voo,
            empresa_id=empresa_id,
            aeronave_id=aeronave_id,
            origem=aeroporto_origem_id,
            destino=aeroporto_destino_id,
            partida_programada=data_hora_partida,
        )
        await self.outbox.append(evento)
        await event_bus.publish(evento)
        if self.decea is not None:
            await self.decea.registrar_voo(
                {
                    "numero_voo": voo.numero_voo,
                    "aeronave_id": str(aeronave_id),
                    "origem": str(aeroporto_origem_id),
                    "destino": str(aeroporto_destino_id),
                }
            )
        if self.meteorologia is not None:
            await self.meteorologia.consultar_rota(
                origem=aeroporto_origem_id,
                destino=aeroporto_destino_id,
    ...
    async def registrar_decolagem(self, *, voo_id: UUID, data_hora: datetime) -> Voo:
        ...
        await self.voo_repo.save(voo)
        evento = VooDecoladoEvent(
            voo_id=voo.id, numero_voo=voo.numero_voo, data_hora=data_hora, atraso_minutos=atraso
        )
        await self.outbox.append(evento)
        await event_bus.publish(evento)
        return voo
    ...
    async def registrar_pouso(self, *, voo_id: UUID, data_hora: datetime) -> Voo:
        ...
        evento = VooPousadoEvent(
            voo_id=voo.id, numero_voo=voo.numero_voo, data_hora=data_hora, tempo_voo_horas=tempo_voo
        )
        await self.outbox.append(evento)
        await event_bus.publish(evento)
        return voo
    ...
    async def get_estatisticas_dia(self, *, data: datetime) -> dict:
        ...
        return {
            ...
            "total_voos": len(voos),
            "por_status": {
                status.value: len([item for item in voos if item.status == status])
                for status in StatusVoo
            },
            "passageiros_total": sum(item.passageiros for item in voos),
        }
