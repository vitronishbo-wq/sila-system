...
from apps.backend.app.modules.infrastructure_sector.aviacao_civil.infrastructure.resilience.circuit_breaker import (
    circuit_breaker,
)
...
class DeceaAdapter:
    ...
    async def registrar_voo(self, dados: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self._client.post(
            f"{self.base_url}/voos", json=dados, headers=self._headers()
        )
    ...
    async def consultar_posicao(self, numero_voo: str) -> dict[str, Any] | None:
        ...
        response = await self._client.get(
            f"{self.base_url}/voos/{numero_voo}/posicao", headers=self._headers()
        )
