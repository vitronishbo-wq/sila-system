...
def test_fluxo_basico_api_aviacao() -> None:
    aeronave_payload = {
        "matricula": "D2-XYZ",
        "tipo": "aviao",
        "categoria": "transporte_passageiro",
        "fabricante": "Airbus",
        "modelo": "A320",
        "numero_serie": "A320-SN1",
        "ano_fabricacao": 2019,
        "proprietario_cpf_cnpj": "50010020030",
        "capacidade_passageiros": 180,
        "autonomia_km": 6100,
        "peso_maximo_decolagem_kg": 77000,
    }
    ...
    voo_payload = {
        ...
        "aeroporto_origem_id": str(uuid4()),
        "aeroporto_destino_id": str(uuid4()),
        "data_hora_partida": partida.isoformat(),
        "data_hora_chegada": chegada.isoformat(),
        "tipo": "regular",
        "natureza": "domestico",
        "regras": "ifr",
        "passageiros": 95,
        "tripulantes": [{"funcao": "piloto", "nome": "Cmd B"}],
    }
    ...
    decolagem_resp = client.post(
        f"/v1/aviacao-civil/voos/{voo_id}/decolagem",
        json={"data_hora": (partida + timedelta(minutes=5)).isoformat()},
    )
    assert decolagem_resp.status_code == 200
    assert decolagem_resp.json()["status"] == "em_voo"
    pouso_resp = client.post(
        f"/v1/aviacao-civil/voos/{voo_id}/pouso",
        json={"data_hora": (partida + timedelta(hours=1, minutes=18)).isoformat()},
    )
...
def test_ocorrencia_api() -> None:
    payload = {
        "tipo": "incidente",
        "aeronave_id": str(uuid4()),
        "data_ocorrencia": datetime.utcnow().isoformat(),
        "local": {"latitude": -8.84, "longitude": 13.23},
        "fase_voo": "aproximacao",
        "descricao": "Bird strike sem danos estruturais relevantes",
        "vitimas": {"fatais": 0, "graves": 0, "leves": 0},
        "danos": "LEVE",
    }
