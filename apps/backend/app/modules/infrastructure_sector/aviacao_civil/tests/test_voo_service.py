...
async def test_programar_decolar_pousar_voo() -> None:
    ...
    aeronave = await aeronave_service.registrar_aeronave(matricula='D2-ABC', tipo=TipoAeronave.AVIAO, categoria=CategoriaAeronave.TRANSPORTE_PASSAGEIRO, fabricante='Boeing', modelo='737-800', numero_serie='SN12345', ano_fabricacao=2017, proprietario_cpf_cnpj='12345678901', capacidade_passageiros=160)
    partida = datetime.utcnow() + timedelta(hours=2)
    chegada = partida + timedelta(hours=1, minutes=20)
    voo = await voo_service.programar_voo(numero_voo='DT1001', empresa_id=uuid4(), aeronave_id=aeronave.id, aeroporto_origem_id=uuid4(), aeroporto_destino_id=uuid4(), data_hora_partida=partida, data_hora_chegada=chegada, tipo=TipoVoo.REGULAR, natureza=NaturezaVoo.DOMESTICO, regras=RegrasVoo.IFR, passageiros=120, tripulantes=[{'funcao': 'piloto', 'nome': 'Comandante A'}])
