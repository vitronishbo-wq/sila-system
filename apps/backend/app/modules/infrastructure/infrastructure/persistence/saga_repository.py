...
class SQLAlchemySagaRepository:
    ...
    async def create_if_absent(
        ...
        *,
        ...
        current = await self.get_by_correlation(
            session, saga_type=saga_type, correlation_id=correlation_id, tenant_id=tenant_id
        )
        if current is not None:
            return current
        row = SagaInstanceModel(
            saga_type=saga_type,
        ...
        try:
            ...
            existing = await self.get_by_correlation(
                session, saga_type=saga_type, correlation_id=correlation_id, tenant_id=tenant_id
            )
