...
from apps.backend.app.modules.infrastructure.application.ports.obra_repository_port import (
    ObraRepositoryPort,
)
...
class SQLAlchemyObraRepository(ObraRepositoryPort):
    ...
    async def save(self, item: Obra) -> Obra:
        if self._session:
            existing = await self._session.execute(
                select(ObraModel).where(ObraModel.codigo_obra == item.codigo_obra)
            )
            ...
            if model is None:
                model = ObraModel(
                    ...
                    orgao_responsavel_tipo=item.orgao_responsavel_tipo,
                    ...
                    prazo_original_dias=item.prazo_original_dias,
                    data_cadastro=item.data_cadastro,
                    descricao=item.descricao,
                    gestor_responsavel_id=item.gestor_responsavel_id,
                    fiscal_responsavel_id=item.fiscal_responsavel_id,
                    empreiteira_id=item.empreiteira_id,
                    ...
                    coordenadas_lat=item.coordenadas_lat,
                    coordenadas_long=item.coordenadas_long,
                    imovel_id=item.imovel_id,
                    percentual_executado=item.percentual_executado,
                    prazo_adicionado_dias=item.prazo_adicionado_dias,
                    ...
                    termos_recebimento=item.termos_recebimento,
    ...
    async def get_by_codigo(self, codigo_obra: str) -> Obra | None:
        if self._session:
            result = await self._session.execute(
                select(ObraModel).where(ObraModel.codigo_obra == codigo_obra)
            )
        ...
        if self._session:
            ...
            if municipio:
                statement = statement.where(
                    func.lower(ObraModel.municipio) == municipio.strip().lower()
                )
            if provincia:
                statement = statement.where(
                    func.lower(ObraModel.provincia) == provincia.strip().lower()
                )
    ...
    async def next_codigo(self) -> str:
        if self._session:
            ...
            result = await self._session.execute(
                select(func.count())
                .select_from(ObraModel)
                .where(ObraModel.codigo_obra.like(f"{prefix}%"))
            )
    ...
    def _to_domain(model: ObraModel) -> Obra:
        return Obra(
            id=model.id,
            codigo_obra=model.codigo_obra,
            nome=model.nome,
            tipo=TipoObra(model.tipo),
            natureza=NaturezaObra(model.natureza),
            status=StatusObra(model.status),
            orgao_responsavel_id=model.orgao_responsavel_id,
            orgao_responsavel_tipo=model.orgao_responsavel_tipo,
            valor_orcado=Decimal(model.valor_orcado),
            ...
            prazo_original_dias=model.prazo_original_dias,
            data_cadastro=model.data_cadastro,
            descricao=model.descricao,
            gestor_responsavel_id=model.gestor_responsavel_id,
            fiscal_responsavel_id=model.fiscal_responsavel_id,
            empreiteira_id=model.empreiteira_id,
            contrato_id=model.contrato_id,
            projeto_id=model.projeto_id,
            valor_contratado=Decimal(model.valor_contratado)
            if model.valor_contratado is not None
            else None,
            valor_executado=Decimal(model.valor_executado)
            if model.valor_executado is not None
            else None,
            ...
            coordenadas_lat=Decimal(model.coordenadas_lat)
            if model.coordenadas_lat is not None
            else None,
            coordenadas_long=Decimal(model.coordenadas_long)
            if model.coordenadas_long is not None
            else None,
            imovel_id=model.imovel_id,
            percentual_executado=Decimal(model.percentual_executado),
            prazo_adicionado_dias=model.prazo_adicionado_dias,
            dias_corridos=model.dias_corridos,
            dias_atraso=model.dias_atraso,
            data_atualizacao=model.data_atualizacao,
            observacoes=model.observacoes,
            medicoes=list(model.medicoes or []),
            aditivos=list(model.aditivos or []),
            fiscalizacoes=list(model.fiscalizacoes or []),
            termos_recebimento=list(model.termos_recebimento or []),
