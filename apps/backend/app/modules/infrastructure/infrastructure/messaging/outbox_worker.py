...
from apps.backend.app.core.db import AsyncSessionLocal, engine as write_engine
from apps.backend.app.modules.infrastructure.application.events.registry import EVENT_HANDLERS
from apps.backend.app.modules.infrastructure.application.sagas.execucao_obra_saga import (
    ExecucaoObraSaga,
)
from apps.backend.app.modules.infrastructure.infrastructure.multi_region.global_id import (
    current_region_code,
)
from apps.backend.app.modules.infrastructure.infrastructure.observability.tracing import (
    instrument_sqlalchemy,
    setup_tracing,
    start_span,
)
from apps.backend.app.modules.infrastructure.infrastructure.persistence.outbox_repository import (
    SQLAlchemyOutboxRepository,
)
from apps.backend.app.modules.infrastructure.infrastructure.persistence.saga_repository import (
    SQLAlchemySagaRepository,
)
from apps.backend.app.modules.infrastructure.infrastructure.read_model.dashboard_projection_repository import (
...
EVENTS_PROCESSED_TOTAL = (
    Counter(
        "op_events_processed_total",
        "Total de eventos processados no worker de obras publicas",
...
EVENTS_FAILED_TOTAL = (
    Counter(
        "op_events_failed_total",
        "Total de falhas de processamento de eventos no worker de obras publicas",
        labelnames=("event_type",),
    )
    if Counter
    else None
...
PENDING_QUEUE_GAUGE = (
    Gauge("op_outbox_pending_batch_size", "Tamanho do lote pendente capturado por poll")
    if Gauge
    else None
)
BATCH_DURATION_SECONDS = (
    Histogram(
        "op_outbox_batch_duration_seconds",
        "Duracao do processamento de lote do outbox de obras publicas",
    )
    if Histogram
    else None
)
    ...
    async def process_batch(self) -> int:
        ...
        async with self._session_factory() as session:
            async with session.begin():
                events = await self._outbox_repo.get_pending_for_update(
                    session, limit=self._batch_size, max_failed_attempts=self._max_failed_attempts
                )
                ...
                for event_row in events:
                    ...
                    with start_span(
                        "obras_publicas.outbox.process_event",
                        {
                        ...
                        try:
                            ...
                            await self._run_consumer_step(
                                session=session,
                                event_row=event_row,
                                consumer_name=PROJECTION_CONSUMER,
                                operation=lambda: self._dashboard_projection_repo.project(
                                    session,
                                    event_type=event_row.event_type,
                                    payload=payload,
                                    tenant_id=event_row.tenant_id,
                            ...
                            await self._run_consumer_step(
                                session=session,
                                event_row=event_row,
                                consumer_name=BI_STREAM_CONSUMER,
                                operation=lambda: self._publish_bi_event(event_row),
                            )
