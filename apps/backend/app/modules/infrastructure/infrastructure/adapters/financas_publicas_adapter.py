...
class FinancasPublicasAdapter:
    def __init__(
        ...
        *,
        ...
        self._bulkhead = AsyncBulkhead(
            limit=int(os.environ.get("OP_FINANCAS_BULKHEAD_LIMIT", "25"))
        )
        self._rate_limiter = AsyncRateLimiter(
            rate=int(os.environ.get("OP_FINANCAS_RATE_LIMIT", "600")), per_seconds=60.0
        )

    async def gerar_empenho(
        self, *, obra_id: str, valor: Decimal, idempotency_key: str, correlation_id: str
    ) -> dict[str, Any]:
        payload = {"obra_id": obra_id, "valor": str(valor)}
        return await self._post(
            endpoint="/integracoes/obras/empenhos",
            payload=payload,
            idempotency_key=idempotency_key,
            correlation_id=correlation_id,
        )

    async def registrar_aditivo(
        self, *, obra_id: str, valor_adicional: Decimal, idempotency_key: str, correlation_id: str
    ) -> dict[str, Any]:
        payload = {"obra_id": obra_id, "valor_adicional": str(valor_adicional)}
        return await self._post(
            endpoint="/integracoes/obras/aditivos",
            payload=payload,
            idempotency_key=idempotency_key,
            correlation_id=correlation_id,
        )
    ...
    async def _post(
        self, *, endpoint: str, payload: dict[str, Any], idempotency_key: str, correlation_id: str
    ) -> dict[str, Any]:
        ...
        async def operation() -> dict[str, Any]:
            ...
            async with self._bulkhead:
                with start_span(
                    "obras_publicas.financas_publicas.post",
                    {
                        "endpoint": endpoint,
