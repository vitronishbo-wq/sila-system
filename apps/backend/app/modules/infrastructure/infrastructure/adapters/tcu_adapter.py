    ...
    def __init__(
        ...
        *,
        ...
        self._rate_limiter = AsyncRateLimiter(
            rate=int(os.environ.get("OP_TCU_RATE_LIMIT", "300")), per_seconds=60.0
        )
    ...
    async def registrar_aditivo(
        ...
        async def operation() -> dict[str, Any]:
            ...
            async with self._bulkhead:
                with start_span(
                    "obras_publicas.tcu.post",
                    {"tenant_id": self.tenant_id, "correlation_id": correlation_id},
                ):
                    async with httpx.AsyncClient(timeout=self._timeout_seconds) as client:
                        response = await with_timeout(
                            client.post(
                                f"{self.base_url}/integracoes/obras/aditivos",
                                json=payload,
                                headers=headers,
                            ),
