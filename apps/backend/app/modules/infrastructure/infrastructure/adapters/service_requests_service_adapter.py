...
class ServiceRequestsServiceAdapter(ServiceRequestsServicePort):
    def __init__(
        ...
        self._bulkhead = AsyncBulkhead(
            limit=int(os.environ.get("OP_SERVICE_REQUESTS_BULKHEAD_LIMIT", "25"))
        )
        self._rate_limiter = AsyncRateLimiter(
            rate=int(os.environ.get("OP_SERVICE_REQUESTS_RATE_LIMIT", "600")), per_seconds=60.0
        )
                ...
                with start_span("obras_publicas.service_requests.post"):
                    async with httpx.AsyncClient(timeout=self._timeout_seconds) as client:
                        response = await with_timeout(
                            client.post(f"{self._base_url}/service-requests", json=payload),
