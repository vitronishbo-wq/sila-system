...
class WorkflowServiceAdapter(WorkflowServicePort):
    def __init__(
        self, *, base_url: str | None = None, circuit_breaker: CircuitBreaker | None = None
    ) -> None:
        ...
        self._bulkhead = AsyncBulkhead(
            limit=int(os.environ.get("OP_WORKFLOW_BULKHEAD_LIMIT", "25"))
        )
        self._rate_limiter = AsyncRateLimiter(
            rate=int(os.environ.get("OP_WORKFLOW_RATE_LIMIT", "600")), per_seconds=60.0
        )
                ...
                with start_span("obras_publicas.workflow.iniciar_fluxo"):
                    async with httpx.AsyncClient(timeout=self._timeout_seconds) as client:
                        response = await with_timeout(
                            client.post(f"{self._base_url}/workflow/instancias", json=payload),
            ...
            async with self._bulkhead:
                with start_span("obras_publicas.workflow.registrar_evento"):
                    async with httpx.AsyncClient(timeout=self._timeout_seconds) as client:
                        response = await with_timeout(
                            client.post(
                                f"{self._base_url}/workflow/instancias/{workflow_id}/eventos",
