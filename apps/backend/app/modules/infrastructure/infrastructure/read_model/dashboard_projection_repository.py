...
class DashboardProjectionRepository:
    ...
    async def project(self, session: AsyncSession, *, event_type: str, payload: dict, tenant_id: str, event_id: UUID | None=None) -> None:
    ...
    async def _project_obra_criada(self, session: AsyncSession, *, payload: dict, tenant_id: str) -> None:
        obra_id = str(payload['obra_id'])
        row = await session.get(ObraDashboardReadModel, obra_id)
        if row is None:
            row = ObraDashboardReadModel(obra_id=obra_id, tenant_id=tenant_id, codigo=str(payload.get('codigo_obra') or ''), status='PROJETO', valor_total=Decimal(str(payload.get('valor_orcado') or '0.00')), valor_executado=Decimal('0.00'), percentual_execucao=Decimal('0.00'))
            session.add(row)
            await session.flush()
            return
    ...
    async def _project_medicao_aprovada(self, session: AsyncSession, *, payload: dict, tenant_id: str) -> None:
        obra_id = str(payload['obra_id'])
        row = await session.get(ObraDashboardReadModel, obra_id)
        if row is None:
            row = ObraDashboardReadModel(obra_id=obra_id, tenant_id=tenant_id, codigo=str(payload.get('codigo_obra') or ''), status='EM_EXECUCAO', valor_total=Decimal('0.00'), valor_executado=Decimal('0.00'), percentual_execucao=Decimal('0.00'))
            session.add(row)
        row.valor_executado = Decimal(str(row.valor_executado)) + Decimal(str(payload.get('valor_medido') or '0.00'))
