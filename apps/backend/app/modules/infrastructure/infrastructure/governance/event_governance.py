...
from apps.backend.app.modules.infrastructure.infrastructure.governance.event_catalog_model import (
    EventCatalogModel,
)
from apps.backend.app.modules.infrastructure.infrastructure.governance.schema_validator import (
    validate_payload,
)
...
class EventGovernanceService:
    ...
    async def validate_event(
        ...
        if payload_version != contract.version:
            raise EventGovernanceError(
                f"Versao invalida para {event_name}: payload={payload_version}, catalog={contract.version}"
            )
    ...
    async def _ensure_catalog_entry(
        self, session: AsyncSession, *, contract: EventContract
    ) -> None:
        try:
            stmt = select(EventCatalogModel).where(
                EventCatalogModel.event_name == contract.event_name
            )
        ...
        if int(row.version) != contract.version:
            raise EventGovernanceError(
                f"Versao divergente no catalogo: {contract.event_name} v{row.version} != v{contract.version}"
            )
