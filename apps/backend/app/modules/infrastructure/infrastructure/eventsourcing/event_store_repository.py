...
from apps.backend.app.modules.infrastructure.infrastructure.eventsourcing.event_store_model import (
    EventStoreModel,
)
...
class SQLAlchemyEventStoreRepository:
    ...
    async def append_event(
        ...
        *,
        ...
        if self._session is not None:
            return await self._append_with_session(
                self._session,
    ...
    async def list_events(
        self, session: AsyncSession, *, aggregate_id: str, tenant_id: str | None = None
    ...
    async def list_events_for_aggregate(
        self, *, aggregate_id: str, tenant_id: str | None = None
    ) -> list[EventStoreEntry]:
        if self._session is not None:
            return await self.list_events(
                self._session, aggregate_id=aggregate_id, tenant_id=tenant_id
            )
        async with self._session_factory() as session:
            return await self.list_events(session, aggregate_id=aggregate_id, tenant_id=tenant_id)

    async def get_last_version(
        self, session: AsyncSession, *, aggregate_id: str, tenant_id: str
    ) -> int:
        stmt = select(func.max(EventStoreModel.version)).where(
            EventStoreModel.aggregate_id == aggregate_id, EventStoreModel.tenant_id == tenant_id
        )
        result = await session.execute(stmt)
    ...
    async def _append_with_session(
        ...
        *,
        ...
        current_version = await self.get_last_version(
            session, aggregate_id=aggregate_id, tenant_id=tenant_id
        )
        ...
        if current_version != strict_expected:
            raise EventStoreConcurrencyError(
                f"Concurrency conflict: expected {strict_expected}, found {current_version}"
            )
        row = EventStoreModel(
            aggregate_id=aggregate_id,
            aggregate_type=aggregate_type,
