...
from apps.backend.app.modules.infrastructure.api.schemas.obra_schema import (
    ...
    ObraCreate,
    ObraEntregaInput,
    ObraFiscalizacaoInput,
    ObraInicioExecucaoInput,
    ObraMedicaoDetalhadaInput,
    ObraProgressoInput,
    ObraRehydratedStateResponse,
    ObraResponse,
    ObraSuspensaoInput,
    ObraTermoRecebimentoInput,
    ObraValorInput,
)
...
from apps.backend.app.modules.infrastructure.domain.exceptions import (
    ObraAlreadyExistsError,
    ObraNotFoundError,
)

router = APIRouter(prefix="/obras", tags=["Obras Publicas - Obras"])

x_correlation_id_header = Header(default=None, alias="X-Correlation-ID")
x_tenant_id_header = Header(default=None, alias="X-Tenant-ID")
obra_service_dep = Depends(get_obra_service)


def _resolve_request_context(
    *, x_tenant_id: str | None, x_correlation_id: str | None
) -> tuple[str, str | None]:
...
def _ensure_criacao_adapters(service: ObraService) -> None:
    if not _has_capability(service, "has_urbanismo_habitacao_adapter"):
        raise HTTPException(
            ...
            detail="Adapter de Urbanismo Habitacao indisponivel para criacao da obra.",
        )
    if not _has_capability(service, "has_workflow_adapter"):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Workflow indisponivel para criacao da obra.",
        )
    if not _has_capability(service, "has_service_requests_adapter"):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Service Requests indisponivel para criacao da obra.",
        )
...
def _ensure_execucao_adapters(service: ObraService) -> None:
    if not _has_capability(service, "has_ambiente_adapter"):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Ambiente indisponivel para inicio de execucao da obra.",
        )
    if not _has_capability(service, "has_transportes_adapter"):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Transportes indisponivel para inicio de execucao da obra.",
        )
    if not _has_capability(service, "has_aguas_saneamento_adapter"):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Adapter de Aguas e Saneamento indisponivel para inicio de execucao da obra.",
        )
...
async def criar_obra(
    ...
    x_correlation_id: str | None = x_correlation_id_header,
    service: ObraService = obra_service_dep,
):
    tenant_id, correlation_id = _resolve_request_context(
        x_tenant_id=x_tenant_id, x_correlation_id=x_correlation_id
    )
    ...
    try:
        return await service.criar(
            ...
            orgao_responsavel_tipo=data.orgao_responsavel_tipo,
...
async def iniciar_licitacao_obra(
    codigo_obra: str, service: ObraService = obra_service_dep
):
    try:
...
async def contratar_obra(
    codigo_obra: str, data: ObraContratacaoInput, service: ObraService = obra_service_dep
):
    try:
        return await service.contratar(
            codigo_obra,
            contrato_id=data.contrato_id,
            empreiteira_id=data.empreiteira_id,
            valor=data.valor_contratado,
        )
    except ObraNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except ValueError as exc:
...
async def iniciar_execucao_obra(
    codigo_obra: str,
    data: ObraInicioExecucaoInput,
    x_tenant_id: str | None = x_tenant_id_header,
    x_correlation_id: str | None = x_correlation_id_header,
    service: ObraService = obra_service_dep,
):
...
async def atualizar_progresso_obra(
    codigo_obra: str, data: ObraProgressoInput, service: ObraService = obra_service_dep
):
    try:
...
async def registrar_medicao_obra(
    codigo_obra: str, data: ObraValorInput, service: ObraService = obra_service_dep
):
    try:
...
async def registrar_medicao_detalhada_obra(
    codigo_obra: str,
    data: ObraMedicaoDetalhadaInput,
    x_tenant_id: str | None = x_tenant_id_header,
    x_correlation_id: str | None = x_correlation_id_header,
    service: ObraService = obra_service_dep,
):
    ...
    try:
        return await service.registrar_medicao_detalhada(
            codigo_obra,
            periodo_referencia=data.periodo_referencia,
            valor_medido=data.valor_medido,
            percentual_executado=data.percentual_executado,
...
async def registrar_pagamento_obra(
    codigo_obra: str, data: ObraValorInput, service: ObraService = obra_service_dep
):
    try:
...
async def registrar_aditivo_obra(
    codigo_obra: str,
    data: ObraAditivoInput,
    x_tenant_id: str | None = x_tenant_id_header,
    x_correlation_id: str | None = x_correlation_id_header,
    service: ObraService = obra_service_dep,
):
    ...
    try:
        return await service.registrar_aditivo(
            codigo_obra,
            tipo=data.tipo,
            justificativa=data.justificativa,
            valor_aditivo=data.valor_aditivo,
            prazo_adicional_dias=data.prazo_adicional_dias,
...
async def registrar_fiscalizacao_obra(
    codigo_obra: str, data: ObraFiscalizacaoInput, service: ObraService = obra_service_dep
):
    try:
...
async def suspender_obra(
    codigo_obra: str, data: ObraSuspensaoInput, service: ObraService = obra_service_dep
):
    try:
...
async def registrar_termo_recebimento_obra(
    codigo_obra: str,
    data: ObraTermoRecebimentoInput,
    service: ObraService = obra_service_dep,
):
    try:
        return await service.registrar_termo_recebimento(
            codigo_obra,
            tipo=data.tipo,
            responsavel_id=data.responsavel_id,
            data_termo=data.data_termo,
            observacoes=data.observacoes,
        )
    except ObraNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def retomar_obra(codigo_obra: str, service: ObraService = obra_service_dep):
    try:
...
async def concluir_obra(
    codigo_obra: str,
    data: ObraConclusaoInput,
    x_tenant_id: str | None = x_tenant_id_header,
    x_correlation_id: str | None = x_correlation_id_header,
    service: ObraService = obra_service_dep,
):
...
async def entregar_obra(
    codigo_obra: str, data: ObraEntregaInput, service: ObraService = obra_service_dep
):
    try:
...
async def obter_event_stream_obra(
    codigo_obra: str,
...
async def rehidratar_obra_por_eventos(
    codigo_obra: str,
    tenant_id: str | None = None,
    x_tenant_id: str | None = x_tenant_id_header,
    service: ObraService = obra_service_dep,
...
async def obter_obra(codigo_obra: str, service: ObraService = obra_service_dep):
    try:
...
async def listar_obras(
    status_obra: StatusObra | None = None,
