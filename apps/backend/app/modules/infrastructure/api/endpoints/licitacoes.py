...
from apps.backend.app.modules.infrastructure.domain.exceptions import (
    LicitacaoAlreadyExistsError,
    LicitacaoNotFoundError,
)

router = APIRouter(prefix="/licitacoes", tags=["Obras Publicas - Licitacoes"])
licitacao_service_dep = Depends(get_licitacao_service)


@router.post("/", response_model=LicitacaoResponse, status_code=status.HTTP_201_CREATED)
async def abrir_licitacao(
    data: LicitacaoCreate, service: LicitacaoService = licitacao_service_dep
):
    try:
        return await service.abrir(
            ...
            data_publicacao_edital=data.data_publicacao_edital,
...
async def iniciar_recebimento_licitacao(
    numero_licitacao: str, service: LicitacaoService = licitacao_service_dep
):
    try:
...
async def encerrar_recebimento_licitacao(
    numero_licitacao: str,
    data: LicitacaoAberturaInput,
    service: LicitacaoService = licitacao_service_dep,
):
    try:
        return await service.encerrar_recebimento(
            numero_licitacao, data_abertura=data.data_abertura
        )
    except LicitacaoNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except ValueError as exc:
...
async def iniciar_analise_licitacao(
    numero_licitacao: str, service: LicitacaoService = licitacao_service_dep
):
    try:
...
async def abrir_habilitacao_licitacao(
    numero_licitacao: str, service: LicitacaoService = licitacao_service_dep
):
    try:
...
async def iniciar_recursos_licitacao(
    numero_licitacao: str, service: LicitacaoService = licitacao_service_dep
):
    try:
...
async def adjudicar_licitacao(
    numero_licitacao: str,
    data: LicitacaoAdjudicacaoInput,
    service: LicitacaoService = licitacao_service_dep,
):
    try:
        return await service.adjudicar(
            numero_licitacao, vencedor_id=data.vencedor_id, valor_adjudicado=data.valor_adjudicado
        )
    except LicitacaoNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except ValueError as exc:
...
async def homologar_licitacao(
    numero_licitacao: str,
    data: LicitacaoHomologacaoInput,
    service: LicitacaoService = licitacao_service_dep,
):
    try:
...
async def declarar_deserta_licitacao(
    numero_licitacao: str,
    data: LicitacaoMotivoInput,
    service: LicitacaoService = licitacao_service_dep,
):
    try:
...
async def listar_licitacoes(
    status_licitacao: StatusLicitacao | None = None,
