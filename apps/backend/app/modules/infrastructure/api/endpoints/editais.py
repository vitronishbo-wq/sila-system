...
from apps.backend.app.modules.infrastructure.domain_publicas.api.schemas.edital_schema import (
    EditalCreate,
...
from apps.backend.app.modules.infrastructure.domain_publicas.application.services.edital_service import (
    EditalService,
)
from apps.backend.app.modules.infrastructure.domain_publicas.domain.enums import StatusEdital
from apps.backend.app.modules.infrastructure.domain_publicas.exceptions import (
    EditalAlreadyExistsError,
    EditalNotFoundError,
)
...
async def publicar_edital(data: EditalCreate, service: EditalService = edital_service_dep):
    try:
        return await service.publicar(
            titulo=data.titulo,
            objeto=data.objeto,
...
async def impugnar_edital(
    numero_edital: str,
    data: EditalMotivoInput,
...
async def retificar_edital(
    numero_edital: str,
    data: EditalRetificacaoInput,
    service: EditalService = edital_service_dep,
):
    try:
...
async def encerrar_edital(
    numero_edital: str,
    data: EditalEncerramentoInput,
    service: EditalService = edital_service_dep,
):
    try:
...
async def obter_edital(numero_edital: str, service: EditalService = edital_service_dep):
    try:
...
async def listar_editais(
    status_edital: StatusEdital | None = None,
    licitacao_id: UUID | None = None,
    service: EditalService = edital_service_dep,
):
