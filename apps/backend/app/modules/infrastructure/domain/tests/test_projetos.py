...
async def test_projeto_service_fluxo_sucesso():
    ...
    item = await service.criar(
        nome="Projeto Executivo Escola Central",
        tipo=TipoProjeto.EXECUTIVO,
        orgao_responsavel_id=uuid4(),
        responsavel_tecnico_id=uuid4(),
        valor_estimado=Decimal("850000.00"),
        data_inicio_prevista=date.today(),
        data_fim_prevista=date.today() + timedelta(days=90),
    )
...
def test_endpoint_criar_projeto_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        codigo_projeto="PRJ/2026/000001",
        nome="Projeto Executivo Escola Central",
        tipo=TipoProjeto.EXECUTIVO,
...
def test_endpoint_obter_projeto_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(side_effect=ProjetoNotFoundError("Projeto nao encontrado"))
    )
    app = FastAPI()
    app.include_router(projetos_router, prefix="/obras-publicas")
    app.dependency_overrides[get_projeto_service] = lambda: service
