...
from apps.backend.app.modules.infrastructure.application.events.definitions import (
    AditivoAssinadoEvent,
)
...
async def test_obras_publicas_outbox_repository_roundtrip(db_session_factory):
    async with db_session_factory() as session:
        ...
        assert exists.scalar_one_or_none() is not None, (
            "Tabela fisica ausente: op_outbox_events. Execute migration 20260304_042_obras_publicas_outbox_events."
        )
    repo = SQLAlchemyOutboxRepository(session_factory=db_session_factory)
    event = AditivoAssinadoEvent.build(
        obra_id=uuid4(),
        codigo_obra="OBR/2026/000001",
        aditivo_id=uuid4(),
        tipo="prazo",
        valor_adicional=Decimal("1000.00"),
        prazo_adicional_dias=5,
    )
    event_id = await repo.save(
        tenant_id="tenant-test",
        aggregate_type="Obra",
        aggregate_id=event.obra_id,
        event=event,
