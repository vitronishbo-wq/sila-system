...
async def test_obra_service_fluxo_sucesso():
    ...
    item = await service.criar(
        nome="Requalificacao Avenida Central",
        tipo=TipoObra.PAVIMENTACAO,
        natureza=NaturezaObra.NOVA,
        orgao_responsavel_id=uuid4(),
        orgao_responsavel_tipo="ministerio",
        valor_orcado=Decimal("2500000.00"),
    ...
    item = await service.contratar(
        item.codigo_obra, contrato_id=uuid4(), empreiteira_id=uuid4(), valor=Decimal("2300000.00")
    )
    ...
    item = await service.registrar_medicao_detalhada(
        item.codigo_obra,
        periodo_referencia="2026-03",
        valor_medido=Decimal("300000.00"),
        percentual_executado=Decimal("12.50"),
        fiscal_id=uuid4(),
        documentos=["medicao-marco.pdf"],
    )
    assert len(item.medicoes) == 1
    assert item.percentual_executado == Decimal("12.50")
    item = await service.registrar_aditivo(
        item.codigo_obra,
        tipo="prazo",
        justificativa="Ajuste de cronograma por chuvas",
        prazo_adicional_dias=15,
    )
    assert len(item.aditivos) == 1
    item = await service.registrar_fiscalizacao(
        item.codigo_obra,
        fiscal_id=uuid4(),
        conformidade=True,
        apontamentos="Execucao dentro das especificacoes",
    )
    ...
    item = await service.concluir(
        item.codigo_obra, data_conclusao=date.today() + timedelta(days=170)
    )
    assert item.status == StatusObra.CONCLUIDA
    item = await service.registrar_termo_recebimento(
        item.codigo_obra,
        tipo="definitivo",
        responsavel_id=uuid4(),
        data_termo=date.today() + timedelta(days=175),
    )
...
async def test_obra_service_integra_adapters_em_criacao_e_execucao():
    urbanismo_adapter = SimpleNamespace(
        validar_conformidade_urbanistica=AsyncMock(return_value=True)
    )
    workflow_adapter = SimpleNamespace(
        iniciar_fluxo=AsyncMock(return_value="WF-OBRA-001"),
        registrar_evento=AsyncMock(return_value=None),
    )
    service_requests_adapter = SimpleNamespace(
        abrir_solicitacao=AsyncMock(return_value="SRQ-OBRA-001")
    )
    ...
    service = ObraService(
        obra_repo=SQLAlchemyObraRepository(),
        financas_publicas_adapter=financas_adapter,
        ambiente_adapter=ambiente_adapter,
        urbanismo_habitacao_adapter=urbanismo_adapter,
        transportes_adapter=transportes_adapter,
        aguas_saneamento_adapter=aguas_adapter,
        workflow_adapter=workflow_adapter,
        service_requests_adapter=service_requests_adapter,
    )
    item = await service.criar(
        nome="Requalificacao Avenida Circular",
        tipo=TipoObra.PAVIMENTACAO,
        natureza=NaturezaObra.NOVA,
        orgao_responsavel_id=uuid4(),
        orgao_responsavel_tipo="ministerio",
        valor_orcado=Decimal("1200000.00"),
        data_inicio_prevista=date.today(),
        data_fim_prevista=date.today() + timedelta(days=120),
        endereco="Avenida Circular",
        bairro="Maianga",
        municipio="Luanda",
        provincia="Luanda",
    )
    ...
    assert item.status == StatusObra.LICITACAO
    item = await service.contratar(
        item.codigo_obra, contrato_id=uuid4(), empreiteira_id=uuid4(), valor=Decimal("1000000.00")
    )
    assert item.status == StatusObra.CONTRATADA
    item = await service.iniciar_execucao(item.codigo_obra, data_inicio=date.today())
    assert item.status == StatusObra.EM_EXECUCAO
...
async def test_obra_service_modo_outbox_nao_chama_adapters_externos_diretamente():
    ...
    workflow_adapter = SimpleNamespace(
        iniciar_fluxo=AsyncMock(return_value="WF-OBRA-OUTBOX-001"),
        registrar_evento=AsyncMock(return_value=None),
    )
    service_requests_adapter = SimpleNamespace(
        abrir_solicitacao=AsyncMock(return_value="SRQ-OBRA-OUTBOX-001")
    )
    ambiente_adapter = SimpleNamespace(validar_licenca_ambiental=AsyncMock(return_value=True))
    transportes_adapter = SimpleNamespace(validar_impacto_viario=AsyncMock(return_value=True))
    aguas_adapter = SimpleNamespace(validar_capacidade_rede=AsyncMock(return_value=True))
    ...
    item = await service.criar(
        ...
        data_inicio_prevista=date.today(),
        data_fim_prevista=date.today() + timedelta(days=120),
        endereco="Avenida Outbox",
        bairro="Maianga",
        municipio="Luanda",
        provincia="Luanda",
        tenant_id="tenant-obras",
        correlation_id="corr-obras-001",
    )
    item = await service.iniciar_licitacao(item.codigo_obra)
    item = await service.contratar(
        item.codigo_obra, contrato_id=uuid4(), empreiteira_id=uuid4(), valor=Decimal("1000000.00")
    )
    await service.iniciar_execucao(
        item.codigo_obra,
        data_inicio=date.today(),
        tenant_id="tenant-obras",
        correlation_id="corr-obras-002",
    )
...
def test_endpoint_criar_obra_retorna_201():
    mock_item = SimpleNamespace(
        ...
        data_fim_prevista=date(2026, 8, 28),
        endereco="Avenida Central",
        bairro="Ingombota",
        municipio="Luanda",
        provincia="Luanda",
        ...
        coordenadas_long=None,
        imovel_id=None,
        percentual_executado=Decimal("0.00"),
        prazo_adicionado_dias=0,
        dias_corridos=0,
        dias_atraso=0,
    ...
    response = client.post(
        ...
        json={
            ...
            "orgao_responsavel_tipo": "ministerio",
            "valor_orcado": "2500000.00",
            "data_inicio_prevista": "2026-03-01",
            "data_fim_prevista": "2026-08-28",
            "endereco": "Avenida Central",
            "bairro": "Ingombota",
            "municipio": "Luanda",
            "provincia": "Luanda",
...
def test_endpoint_criar_obra_propaga_tenant_e_correlation():
    ...
    response = client.post(
        "/obras-publicas/obras/",
        headers={"X-Tenant-ID": "tenant-obras", "X-Correlation-ID": "corr-obras-123"},
        json={
            "nome": "Requalificacao Avenida Central",
            "tipo": "pavimentacao",
            "natureza": "nova",
...
def test_endpoint_obter_obra_retorna_404():
    service = SimpleNamespace(
        obter_por_codigo=AsyncMock(side_effect=ObraNotFoundError("Obra nao encontrada"))
    )
    app = FastAPI()
    app.include_router(obras_router, prefix="/obras-publicas")
    app.dependency_overrides[get_obra_service] = lambda: service
...
def test_endpoint_event_stream_e_rehydrate_com_codigo_path():
    service = SimpleNamespace(
        obter_event_stream=AsyncMock(
            return_value=[
                {
                    "id": str(uuid4()),
                    "aggregate_id": "obra-1",
                    "aggregate_type": "Obra",
                    "event_type": "ObraIniciadaEvent",
                    "event_data": {"obra_id": "obra-1"},
                    "version": 1,
                    "tenant_id": "tenant-obras",
                    "region_code": "A",
        ...
        rehidratar_estado_obra=AsyncMock(
            return_value={
                "obra_id": "obra-1",
                "codigo_obra": "OBR/2026/000001",
                "status": "EM_EXECUCAO",
                "valor_total": "1000.00",
                "valor_executado": "250.00",
                "event_count": 1,
            }
        ),
    )
    ...
    stream_response = client.get(
        "/obras-publicas/obras/OBR/2026/000001/event-stream",
        headers={"X-Tenant-ID": "tenant-obras"},
    )
    rehydrate_response = client.get(
        "/obras-publicas/obras/OBR/2026/000001/rehydrate", headers={"X-Tenant-ID": "tenant-obras"}
    )
    ...
    service.rehidratar_estado_obra.assert_awaited_once_with(
        "OBR/2026/000001", tenant_id="tenant-obras"
    )
...
def test_endpoint_iniciar_execucao_sem_adapter_critico_retorna_503():
    ...
    client = TestClient(app)
    response = client.post(
        "/obras-publicas/obras/OBR/2026/000001/iniciar-execucao", json={"data_inicio": "2026-03-20"}
    )
    assert response.status_code == 503
...
def test_endpoint_fluxo_execucao_expandido_com_trilha_auditavel():
    ...
    workflow_adapter = SimpleNamespace(
        iniciar_fluxo=AsyncMock(return_value="WF-OBRA-E2E-001"),
        registrar_evento=AsyncMock(return_value=None),
    )
    service_requests_adapter = SimpleNamespace(
        abrir_solicitacao=AsyncMock(return_value="SRQ-OBRA-E2E-001")
    )
    ambiente_adapter = SimpleNamespace(validar_licenca_ambiental=AsyncMock(return_value=True))
    transportes_adapter = SimpleNamespace(validar_impacto_viario=AsyncMock(return_value=True))
    aguas_adapter = SimpleNamespace(validar_capacidade_rede=AsyncMock(return_value=True))
    ...
    create_response = client.post(
        "/obras-publicas/obras/",
        json={
            "nome": "Terminal Rodoviario Central",
            "tipo": "construcao",
            "natureza": "nova",
            "orgao_responsavel_id": str(uuid4()),
            "orgao_responsavel_tipo": "ministerio",
            "valor_orcado": "5000000.00",
            "data_inicio_prevista": "2026-03-05",
    ...
    contratacao_response = client.post(
        f"/obras-publicas/obras/{codigo}/contratar",
        json={
            "contrato_id": str(uuid4()),
            "empreiteira_id": str(uuid4()),
            "valor_contratado": "4800000.00",
        },
    )
    assert contratacao_response.status_code == 200
    execucao_response = client.post(
        f"/obras-publicas/obras/{codigo}/iniciar-execucao", json={"data_inicio": "2026-03-10"}
    )
    assert execucao_response.status_code == 200
    medicao_response = client.post(
        f"/obras-publicas/obras/{codigo}/medicao-detalhada",
        json={
            "periodo_referencia": "2026-03",
            "valor_medido": "350000.00",
            "percentual_executado": "15.00",
            "fiscal_id": str(uuid4()),
            "documentos": ["medicao-2026-03.pdf"],
            "observacoes": "Primeira medicao validada",
        },
    )
    assert medicao_response.status_code == 200
    aditivo_response = client.post(
        f"/obras-publicas/obras/{codigo}/aditivo",
        json={
            "tipo": "prazo",
            "justificativa": "Interferencia de rede subterranea",
            "prazo_adicional_dias": 20,
        },
    )
    assert aditivo_response.status_code == 200
    fiscalizacao_response = client.post(
        f"/obras-publicas/obras/{codigo}/fiscalizacao",
        json={
            "fiscal_id": str(uuid4()),
            "conformidade": True,
            "apontamentos": "Sem nao conformidades",
        },
    )
    assert fiscalizacao_response.status_code == 200
    conclusao_response = client.post(
        f"/obras-publicas/obras/{codigo}/concluir", json={"data_conclusao": "2026-11-20"}
    )
    assert conclusao_response.status_code == 200
    termo_response = client.post(
        f"/obras-publicas/obras/{codigo}/termo-recebimento",
        json={
            "tipo": "definitivo",
            "responsavel_id": str(uuid4()),
            "data_termo": "2026-11-25",
            "observacoes": "Obra recebida definitivamente",
        },
    )
