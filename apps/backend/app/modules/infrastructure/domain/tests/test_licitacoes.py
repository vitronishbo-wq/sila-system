...
async def test_licitacao_service_fluxo_sucesso():
    ...
    item = await service.abrir(objeto='Contratacao para pavimentacao da via X', tipo=TipoLicitacao.CONCORRENCIA, obra_id=uuid4(), orgao_responsavel_id=uuid4(), valor_estimado=Decimal('1200000.00'), data_publicacao_edital=date.today(), data_entrega_propostas=date.today() + timedelta(days=15))
    ...
    item = await service.encerrar_recebimento(item.numero_licitacao, data_abertura=date.today() + timedelta(days=15))
    ...
    item = await service.adjudicar(item.numero_licitacao, vencedor_id=uuid4(), valor_adjudicado=Decimal('1180000.00'))
...
def test_endpoint_abrir_licitacao_retorna_201():
    mock_item = SimpleNamespace(id=uuid4(), numero_licitacao='LIC/2026/000001', objeto='Contratacao para pavimentacao da via X', tipo=TipoLicitacao.CONCORRENCIA, status=StatusLicitacao.EDITAL_PUBLICADO, obra_id=uuid4(), orgao_responsavel_id=uuid4(), valor_estimado=Decimal('1200000.00'), data_publicacao_edital=date(2026, 3, 1), data_entrega_propostas=date(2026, 3, 16), data_cadastro=date(2026, 3, 1), data_abertura=None, vencedor_id=None, valor_adjudicado=None, data_homologacao=None, data_atualizacao=None, observacoes=None)
    ...
    response = client.post('/obras-publicas/licitacoes/', json={'objeto': 'Contratacao para pavimentacao da via X', 'tipo': 'concorrencia', 'obra_id': str(uuid4()), 'orgao_responsavel_id': str(uuid4()), 'valor_estimado': '1200000.00', 'data_publicacao_edital': '2026-03-01', 'data_entrega_propostas': '2026-03-16'})
    assert response.status_code == 201
    assert response.json()['numero_licitacao'] == 'LIC/2026/000001'

def test_endpoint_obter_licitacao_retorna_404():
    service = SimpleNamespace(obter_por_numero=AsyncMock(side_effect=LicitacaoNotFoundError('Licitacao nao encontrada')))
    app = FastAPI()
    app.include_router(licitacoes_router, prefix='/obras-publicas')
    app.dependency_overrides[get_licitacao_service] = lambda: service
    client = TestClient(app)
