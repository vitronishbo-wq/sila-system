...
from apps.backend.app.modules.infrastructure.domain.enums import (
    NaturezaObra,
    StatusEdital,
    StatusLicitacao,
    StatusObra,
...
async def _assert_table_exists(db_session: AsyncSession, table_name: str) -> None:
    ...
    assert result.scalar_one_or_none() is not None, (
        f"Tabela fisica ausente: {table_name}. Execute migration 20260304_037_obras_publicas_orm_core."
    )
...
async def test_repositories_obras_publicas_roundtrip_orm_real(db_session: AsyncSession) -> None:
    ...
    projeto = await projeto_repo.save(
        ProjetoObra.criar(
            codigo_projeto=await projeto_repo.next_codigo(),
            nome="Projeto Integrado Terminal Norte",
            tipo=TipoProjeto.EXECUTIVO,
            orgao_responsavel_id=uuid4(),
            responsavel_tecnico_id=uuid4(),
            valor_estimado=Decimal("3200000.00"),
    ...
    obra = Obra.criar(
        codigo_obra=await obra_repo.next_codigo(),
        nome="Obra Terminal Norte",
        tipo=TipoObra.CONSTRUCAO,
        natureza=NaturezaObra.NOVA,
        orgao_responsavel_id=uuid4(),
        orgao_responsavel_tipo="ministerio",
        valor_orcado=Decimal("5000000.00"),
        data_inicio_prevista=date.today(),
        data_fim_prevista=date.today() + timedelta(days=300),
        endereco="Av. Integracao",
        bairro="Centro",
        municipio="Luanda",
        provincia="Luanda",
    )
    ...
    obra.registrar_medicao_detalhada(
        MedicaoObra.registrar(
            periodo_referencia="2026-03",
            valor_medido=Decimal("350000.00"),
            percentual_executado=Decimal("14.00"),
            fiscal_id=uuid4(),
            documentos=["medicao-marco.pdf"],
        )
    )
    obra.registrar_aditivo(
        AditivoContratual.registrar(
            tipo=TipoAditivo.PRAZO, justificativa="Ajuste de cronograma", prazo_adicional_dias=20
        )
    )
    obra.registrar_fiscalizacao(
        FiscalizacaoObra.registrar(
            fiscal_id=uuid4(), conformidade=True, apontamentos="Sem nao conformidades"
        )
    )
    obra.concluir(date.today() + timedelta(days=280))
    obra.registrar_termo_recebimento(
        TermoRecebimento.registrar(
            tipo="definitivo", responsavel_id=uuid4(), data_termo=date.today() + timedelta(days=285)
        )
    )
    ...
    licitacao = await licitacao_repo.save(
        Licitacao.abrir(
            numero_licitacao=await licitacao_repo.next_numero(),
            objeto="Contratacao principal da obra",
            tipo=TipoLicitacao.CONCORRENCIA,
            obra_id=obra_salva.id,
            orgao_responsavel_id=obra_salva.orgao_responsavel_id,
            valor_estimado=Decimal("4900000.00"),
            data_publicacao_edital=date.today(),
            data_entrega_propostas=date.today() + timedelta(days=20),
        )
    )
    assert licitacao.status == StatusLicitacao.EDITAL_PUBLICADO
    edital = await edital_repo.save(
        Edital.publicar(
            numero_edital=await edital_repo.next_numero(),
            titulo="Edital Obra Terminal Norte",
            objeto="Execucao da obra terminal norte",
            licitacao_id=licitacao.id,
            data_publicacao=date.today(),
            data_abertura=date.today() + timedelta(days=5),
            data_encerramento=date.today() + timedelta(days=25),
        )
    )
    ...
    assert (
        len(await projeto_repo.list(status=StatusProjeto.ELABORACAO, tipo=TipoProjeto.EXECUTIVO))
        >= 1
    )
    assert (
        len(
            await licitacao_repo.list(
                status=StatusLicitacao.EDITAL_PUBLICADO, obra_id=obra_salva.id
            )
        )
        >= 1
    )
    assert (
        len(await edital_repo.list(status=StatusEdital.PUBLICADO, licitacao_id=licitacao.id)) >= 1
    )
