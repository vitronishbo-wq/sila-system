...
from apps.backend.app.modules.infrastructure.application.services.edital_service import (
    EditalService,
)
...
async def test_edital_service_fluxo_sucesso():
    ...
    item = await service.publicar(
        titulo="Edital de Pavimentacao 2026",
        objeto="Contratacao para pavimentacao de vias urbanas",
        licitacao_id=uuid4(),
    ...
    item = await service.encerrar(
        item.numero_edital, data_encerramento=data_abertura + timedelta(days=10)
    )
...
def test_endpoint_publicar_edital_retorna_201():
    mock_item = SimpleNamespace(
        id=uuid4(),
        numero_edital="EDT/2026/000001",
        titulo="Edital de Pavimentacao 2026",
        objeto="Contratacao para pavimentacao de vias urbanas",
        licitacao_id=uuid4(),
        status=StatusEdital.PUBLICADO,
        data_publicacao=date(2026, 3, 1),
        data_abertura=date(2026, 3, 6),
        data_encerramento=date(2026, 3, 21),
    ...
    response = client.post(
        "/obras-publicas/editais/",
        json={
            "titulo": "Edital de Pavimentacao 2026",
            "objeto": "Contratacao para pavimentacao de vias urbanas",
            "licitacao_id": str(uuid4()),
            "data_publicacao": "2026-03-01",
            "data_abertura": "2026-03-06",
            "data_encerramento": "2026-03-21",
        },
    )
...
def test_endpoint_obter_edital_retorna_404():
    service = SimpleNamespace(
        obter_por_numero=AsyncMock(side_effect=EditalNotFoundError("Edital nao encontrado"))
    )
    app = FastAPI()
    app.include_router(editais_router, prefix="/obras-publicas")
    app.dependency_overrides[get_edital_service] = lambda: service
