...
class _FakeSagaRepo:
    ...
    async def get_by_correlation(
        self, session, *, saga_type: str, correlation_id: str, tenant_id: str | None = None
    ):
...
async def test_execucao_obra_saga_persisted_state_machine():
    ...
    await saga.advance(
        ...
        payload={"obra_id": "obra-1", "codigo_obra": "OBR/1"},
    ...
    await saga.advance(
        session,
        event_type="MedicaoAprovadaEvent",
        payload={"obra_id": "obra-1", "codigo_obra": "OBR/1"},
        correlation_id="corr-1",
        tenant_id="tenant-1",
    )
    ...
    await saga.advance(
        session,
        event_type="AditivoAssinadoEvent",
        payload={"obra_id": "obra-1", "codigo_obra": "OBR/1"},
        correlation_id="corr-1",
        tenant_id="tenant-1",
    )
    assert repo.row.state == ExecucaoObraStates.TCU_NOTIFICADO
    result = await saga.advance(
        session,
        event_type="ObraConcluidaEvent",
        payload={"obra_id": "obra-1", "codigo_obra": "OBR/1"},
        correlation_id="corr-1",
        tenant_id="tenant-1",
    )
