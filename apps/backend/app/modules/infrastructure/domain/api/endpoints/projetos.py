...
async def criar_projeto(data: ProjetoCreate, service: ProjetoService=Depends(get_projeto_service)):
    try:
        return await service.criar(nome=data.nome, tipo=data.tipo, orgao_responsavel_id=data.orgao_responsavel_id, responsavel_tecnico_id=data.responsavel_tecnico_id, valor_estimado=data.valor_estimado, data_inicio_prevista=data.data_inicio_prevista, data_fim_prevista=data.data_fim_prevista, codigo_projeto=data.codigo_projeto, obra_id=data.obra_id, descricao=data.descricao)
...
async def aprovar_projeto(codigo_projeto: str, service: ProjetoService=Depends(get_projeto_service)):
    try:
...
async def iniciar_projeto(codigo_projeto: str, data: ProjetoInicioInput, service: ProjetoService=Depends(get_projeto_service)):
    try:
...
async def concluir_projeto(codigo_projeto: str, data: ProjetoConclusaoInput, service: ProjetoService=Depends(get_projeto_service)):
    try:
...
async def revisar_projeto(codigo_projeto: str, data: ProjetoMotivoInput, service: ProjetoService=Depends(get_projeto_service)):
    try:
...
async def arquivar_projeto(codigo_projeto: str, data: ProjetoMotivoInput, service: ProjetoService=Depends(get_projeto_service)):
    try:
...
async def obter_projeto(codigo_projeto: str, service: ProjetoService=Depends(get_projeto_service)):
    try:
...
async def listar_projetos(status_projeto: StatusProjeto | None=None, tipo: TipoProjeto | None=None, orgao_responsavel_id: UUID | None=None, obra_id: UUID | None=None, service: ProjetoService=Depends(get_projeto_service)):
    return await service.listar(status=status_projeto, tipo=tipo, orgao_responsavel_id=orgao_responsavel_id, obra_id=obra_id)
