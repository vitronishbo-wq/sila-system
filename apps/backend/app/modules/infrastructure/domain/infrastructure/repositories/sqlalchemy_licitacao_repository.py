...
class SQLAlchemyLicitacaoRepository(LicitacaoRepositoryPort):
    ...
    async def save(self, item: Licitacao) -> Licitacao:
        if self._session:
            existing = await self._session.execute(select(LicitacaoModel).where(LicitacaoModel.numero_licitacao == item.numero_licitacao))
            model = existing.scalars().first()
            if model is None:
                model = LicitacaoModel(id=item.id, numero_licitacao=item.numero_licitacao, objeto=item.objeto, tipo=item.tipo.value, status=item.status.value, obra_id=item.obra_id, orgao_responsavel_id=item.orgao_responsavel_id, valor_estimado=item.valor_estimado, data_publicacao_edital=item.data_publicacao_edital, data_entrega_propostas=item.data_entrega_propostas, data_cadastro=item.data_cadastro, data_abertura=item.data_abertura, vencedor_id=item.vencedor_id, valor_adjudicado=item.valor_adjudicado, data_homologacao=item.data_homologacao, data_atualizacao=item.data_atualizacao, observacoes=item.observacoes)
    ...
    async def get_by_numero(self, numero_licitacao: str) -> Licitacao | None:
        if self._session:
            result = await self._session.execute(select(LicitacaoModel).where(LicitacaoModel.numero_licitacao == numero_licitacao))
    ...
    async def list(self, *, status: StatusLicitacao | None=None, tipo: TipoLicitacao | None=None, obra_id: UUID | None=None, orgao_responsavel_id: UUID | None=None) -> list[Licitacao]:
        if self._session:
            ...
            if orgao_responsavel_id:
                statement = statement.where(LicitacaoModel.orgao_responsavel_id == orgao_responsavel_id)
            result = await self._session.execute(statement.order_by(LicitacaoModel.numero_licitacao.asc()))
    ...
    async def next_numero(self) -> str:
        if self._session:
            ...
            prefix = f'LIC/{year}/'
            result = await self._session.execute(select(func.count()).select_from(LicitacaoModel).where(LicitacaoModel.numero_licitacao.like(f'{prefix}%')))
    ...
    def _to_domain(model: LicitacaoModel) -> Licitacao:
        return Licitacao(id=model.id, numero_licitacao=model.numero_licitacao, objeto=model.objeto, tipo=TipoLicitacao(model.tipo), status=StatusLicitacao(model.status), obra_id=model.obra_id, orgao_responsavel_id=model.orgao_responsavel_id, valor_estimado=Decimal(model.valor_estimado), data_publicacao_edital=model.data_publicacao_edital, data_entrega_propostas=model.data_entrega_propostas, data_cadastro=model.data_cadastro, data_abertura=model.data_abertura, vencedor_id=model.vencedor_id, valor_adjudicado=Decimal(model.valor_adjudicado) if model.valor_adjudicado is not None else None, data_homologacao=model.data_homologacao, data_atualizacao=model.data_atualizacao, observacoes=model.observacoes)
