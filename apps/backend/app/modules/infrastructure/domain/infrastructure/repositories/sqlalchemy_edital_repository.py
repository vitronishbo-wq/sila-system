...
class SQLAlchemyEditalRepository(EditalRepositoryPort):
    ...
    async def save(self, item: Edital) -> Edital:
        if self._session:
            existing = await self._session.execute(select(EditalModel).where(EditalModel.numero_edital == item.numero_edital))
            model = existing.scalars().first()
            if model is None:
                model = EditalModel(id=item.id, numero_edital=item.numero_edital, titulo=item.titulo, objeto=item.objeto, licitacao_id=item.licitacao_id, status=item.status.value, data_publicacao=item.data_publicacao, data_abertura=item.data_abertura, data_encerramento=item.data_encerramento, data_cadastro=item.data_cadastro, versao=item.versao, data_atualizacao=item.data_atualizacao, observacoes=item.observacoes)
    ...
    async def get_by_numero(self, numero_edital: str) -> Edital | None:
        if self._session:
            result = await self._session.execute(select(EditalModel).where(EditalModel.numero_edital == numero_edital))
    ...
    async def list(self, *, status: StatusEdital | None=None, licitacao_id: UUID | None=None) -> list[Edital]:
        if self._session:
            ...
            result = await self._session.execute(statement.order_by(EditalModel.numero_edital.asc()))
    ...
    async def next_numero(self) -> str:
        if self._session:
            ...
            prefix = f'EDT/{year}/'
            result = await self._session.execute(select(func.count()).select_from(EditalModel).where(EditalModel.numero_edital.like(f'{prefix}%')))
    ...
    def _to_domain(model: EditalModel) -> Edital:
        return Edital(id=model.id, numero_edital=model.numero_edital, titulo=model.titulo, objeto=model.objeto, licitacao_id=model.licitacao_id, status=StatusEdital(model.status), data_publicacao=model.data_publicacao, data_abertura=model.data_abertura, data_encerramento=model.data_encerramento, data_cadastro=model.data_cadastro, versao=model.versao, data_atualizacao=model.data_atualizacao, observacoes=model.observacoes)
