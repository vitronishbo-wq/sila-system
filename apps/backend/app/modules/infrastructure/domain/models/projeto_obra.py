...
class ProjetoObra:
    ...
    @classmethod
    def criar(cls, *, codigo_projeto: str, nome: str, tipo: TipoProjeto, orgao_responsavel_id: UUID, responsavel_tecnico_id: UUID, valor_estimado: Decimal, data_inicio_prevista: date, data_fim_prevista: date, obra_id: UUID | None=None, descricao: str | None=None) -> 'ProjetoObra':
        ...
        return cls(id=uuid4(), codigo_projeto=codigo_projeto.strip(), nome=nome.strip(), tipo=tipo, status=StatusProjeto.ELABORACAO, orgao_responsavel_id=orgao_responsavel_id, responsavel_tecnico_id=responsavel_tecnico_id, valor_estimado=valor_estimado.quantize(Decimal('0.01')), data_inicio_prevista=data_inicio_prevista, data_fim_prevista=data_fim_prevista, obra_id=obra_id, descricao=descricao.strip() if descricao else None, data_cadastro=date.today(), versao=1)
