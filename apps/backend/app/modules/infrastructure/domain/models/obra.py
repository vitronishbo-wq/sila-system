...
class Obra:
    ...
    @classmethod
    def criar(cls, *, codigo_obra: str, nome: str, tipo: TipoObra, natureza: NaturezaObra, orgao_responsavel_id: UUID, orgao_responsavel_tipo: str, valor_orcado: Decimal, data_inicio_prevista: date, data_fim_prevista: date, endereco: str, bairro: str, municipio: str, provincia: str, descricao: str | None=None) -> 'Obra':
        ...
        if not endereco.strip() or not bairro.strip() or (not municipio.strip()) or (not provincia.strip()):
        ...
        return cls(id=uuid4(), codigo_obra=codigo_obra.strip(), nome=nome.strip(), descricao=descricao.strip() if descricao else None, tipo=tipo, natureza=natureza, status=StatusObra.PROJETO, orgao_responsavel_id=orgao_responsavel_id, orgao_responsavel_tipo=orgao_responsavel_tipo.strip(), valor_orcado=valor_orcado.quantize(Decimal('0.01')), data_inicio_prevista=data_inicio_prevista, data_fim_prevista=data_fim_prevista, endereco=endereco.strip(), bairro=bairro.strip(), municipio=municipio.strip(), provincia=provincia.strip(), prazo_original_dias=prazo_original, data_cadastro=date.today())
    ...
    def registrar_aditivo(self, aditivo: AditivoContratual) -> None:
        ...
        if aditivo.valor_aditivo > Decimal('0'):
            self.valor_orcado = (self.valor_orcado + aditivo.valor_aditivo).quantize(Decimal('0.01'))
            if self.valor_contratado is not None:
                self.valor_contratado = (self.valor_contratado + aditivo.valor_aditivo).quantize(Decimal('0.01'))
        self.data_atualizacao = date.today()
