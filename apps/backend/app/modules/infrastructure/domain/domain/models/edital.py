...
class Edital:
    ...
    @classmethod
    def publicar(cls, *, numero_edital: str, titulo: str, objeto: str, licitacao_id: UUID, data_publicacao: date, data_abertura: date, data_encerramento: date) -> 'Edital':
        ...
        return cls(id=uuid4(), numero_edital=numero_edital.strip(), titulo=titulo.strip(), objeto=objeto.strip(), licitacao_id=licitacao_id, status=StatusEdital.PUBLICADO, data_publicacao=data_publicacao, data_abertura=data_abertura, data_encerramento=data_encerramento, data_cadastro=date.today(), versao=1)
    ...
    def retificar(self, descricao: str) -> None:
        if self.status not in {StatusEdital.PUBLICADO, StatusEdital.IMPUGNADO, StatusEdital.RETIFICADO}:
