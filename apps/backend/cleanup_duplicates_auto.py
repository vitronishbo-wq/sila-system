...
class AutoCleaner:
    def __init__(self, backend_path: str):
        ...
        self.stats = {"dirs_removed": [], "dirs_failed": [], "files_migrated": 0, "errors": []}

    def run_cmd(self, cmd: str) -> tuple[int, str, str]:
        """Execute command silently"""
        try:
            result = subprocess.run(
                cmd, shell=True, capture_output=True, text=True, timeout=30, cwd=self.backend
            )
    ...
    def cleanup(self) -> bool:
        ...
        rc, _, _ = self.run_cmd(
            f"git commit -m 'Pre-cleanup backup [{self.timestamp}]' 2>/dev/null || true"
        )
        ...
        if src.exists():
            try:
                ...
                print("   ✅ Removido application/ (legacy)")
    ...
    def validate(self) -> bool:
        ...
        for pattern in orphan_patterns:
            rc, out, _ = self.run_cmd(
                f"grep -r '{pattern}' . --include='*.py' 2>/dev/null | grep -v '.venv' | wc -l"
            )
        ...
        if orphans_found == 0:
            print("   ✅ Nenhum import órfão encontrado")
        print()
        ...
        test_imports = [
            ...
            "from apps.backend.app.infrastructure import *",
            "from apps.backend.app.application import *",
        ]

        for imp in test_imports:
            rc, out, err = self.run_cmd(
                f"python -c 'import sys; sys.path.insert(0, \"..\"); {imp}' 2>&1"
            )
            status = "✅" if rc == 0 else "❌"
