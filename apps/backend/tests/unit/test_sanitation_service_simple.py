"""
Testes de unidade simplificados para o serviço de saneamento.
...
sys.path.insert(0, str(backend_dir / "modules" / "sanitation" / "services"))
from sanitation_service import SanitationService


class TestSanitationServiceSimple:
    """Testes de unidade simplificados para SanitationService."""
    ...
    async def test_can_schedule_service_business_hours(self, service):
        ...
        # Arrange - Usa uma data que é segunda-feira (weekday=0) às 14h
        service_time = datetime(
            2025, 10, 27, 14, 0, tzinfo=timezone.utc
        )  # Segunda-feira
    ...
    async def test_can_schedule_service_after_hours(self, service):
        ...
        # Arrange - Usa uma data que é segunda-feira às 22h
        service_time = datetime(
            2025, 10, 27, 22, 0, tzinfo=timezone.utc
        )  # Segunda-feira
    ...
    async def test_can_schedule_service_weekend(self, service):
        ...
        # Arrange - Usa uma data que é domingo (weekday=6)
        service_time = datetime(2025, 10, 26, 10, 0, tzinfo=timezone.utc)  # Domingo
