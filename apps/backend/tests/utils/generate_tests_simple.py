...
def generate_tests(module_name: str) -> None:
    ...
    if not test_file.exists():
        test_file.write_text(
            get_test_content(module_name, class_name), encoding="utf-8"
        )
        print(
            f"{Colors.OKGREEN}Criado: {test_file.relative_to(Path.cwd())}{Colors.ENDC}"
        )
    ...
    if not conftest_file.exists():
        conftest_file.write_text(
            get_conftest_content(module_name, class_name), encoding="utf-8"
        )
        print(
            f"{Colors.OKGREEN}Criado: {conftest_file.relative_to(Path.cwd())}{Colors.ENDC}"
        )
...
def main():
    ...
    if not BASE_DIR.exists():
        print(
            f"{Colors.FAIL}Erro: Diretório não encontrado: {BASE_DIR.absolute()}{Colors.ENDC}"
        )
        print(
            "Certifique-se de que o script está sendo executado a partir do diretório raiz do projeto."
        )
        sys.exit(1)
    ...
    print(
        f"/n{Colors.BOLD}Selecione os módulos para gerar testes (ex: 1,2,3 ou 'todos'):{Colors.ENDC}"
    )
    while True:
        try:
            ...
            if selection == "todos":
                ...
                if selected_modules:
                    break
                print(
                    f"{Colors.WARNING}Seleção inválida. Tente novamente.{Colors.ENDC}"
                )
        except (ValueError, IndexError):
            print(
                f"{Colors.WARNING}Entrada inválida. Use números separados por vírgula ou 'todos'.{Colors.ENDC}"
            )
