...
from apps.backend.app.modules.justice.civil_registry.application.services.queries import (
    ProfileQueries,
)

# from apps.backend.app.core.bridges.identity_bridge import CitizenRequest  # TODO: Import CitizenRequest when it exists
...
class TestCommuneQueries:
    ...
    def test_commune_nao_ve_escalados(self, db_session: Session):
        ...
        request_normal = CitizenRequest(
            id=uuid4(), commune_id=commune_id, is_escalated=False, state="EM_ANALISE_COMUNAL"
        )

        # Criar pedido escalado (não deve aparecer)
        request_escalated = CitizenRequest(
            id=uuid4(), commune_id=commune_id, is_escalated=True, state="ESCALADO_MUNICIPIO"
        )
...
class TestProvinceQueries:
    ...
    def test_province_ve_escalados(self, db_session: Session):
        ...
        request_escalated = CitizenRequest(
            id=uuid4(), province_id=province_id, is_escalated=True, state="ESCALADO_PROVINCIA"
        )

        # Normal (não deve aparecer)
        request_normal = CitizenRequest(
            id=uuid4(), province_id=province_id, is_escalated=False, state="EM_ANALISE_COMUNAL"
        )
...
class TestCentralQueries:
    ...
    def test_central_tem_kpis_nao_casos(self, db_session: Session):
        ...
        assert (
            kpis["media_nacional"] is not None or kpis["media_nacional"] is None
        )  # Pode ser None se nenhum resolvido
