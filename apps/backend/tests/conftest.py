...
for module_name in ("apps.backend.app.core.db", "apps.backend.app.core.database", "config.database", "apps.backend.app.config.database"):
    try:
...
def _resolve_test_app():
    ...
    try:
        return importlib.import_module("apps.backend.app.main").app
    except Exception as exc:
        pytest.skip(f"Test app indisponível para fixture HTTP ({type(exc).__name__}: {exc})")
...
def _resolve_auth_dependencies():
    ...
    deps_module = importlib.import_module("apps.backend.app.api.deps")
    user_module = importlib.import_module("apps.backend.app.models.iam_user")
...
def _create_sync_engine_with_shared_mappers():
    ...
    if "postgresql" not in settings.DATABASE_URL:
        raise RuntimeError(f"DATABASE_URL must be PostgreSQL, got: {settings.DATABASE_URL}")
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers",
        "async_ready: marca testes que já foram refatorados para async (migração completa)",
    )
    config.addinivalue_line(
