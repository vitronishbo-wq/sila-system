...
class MockServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if len(path_parts) >= 4 and path_parts[-2] == "citizens":
            ...
            if citizen_id in MOCK_DB["citizens"]:
                ...
                self.wfile.write(
                    json.dumps({"detail": "Cidadão não encontrado"}).encode()
                )
            return
    ...
    def do_POST(self):
        ...
        if self.path == "/api/v1/auth/login":
            ...
            if (
                username in MOCK_DB["users"]
                and MOCK_DB["users"][username]["password"] == password
            ):
                ...
                self._set_headers(200)
                self.wfile.write(
                    json.dumps({"access_token": token, "token_type": "bearer"}).encode()
                )
            else:
                self._set_headers(401)
                self.wfile.write(
                    json.dumps({"detail": "Credenciais inválidas"}).encode()
                )
            return
        ...
        if self.path == "/api/v1/citizenship/citizens/":
            ...
            if (
                "citizenship:full_access" not in user_permissions
                and "citizenship:write" not in user_permissions
            ):
    ...
    def do_DELETE(self):
        ...
        if len(path_parts) >= 4 and path_parts[-2] == "citizens":
            ...
            if citizen_id in MOCK_DB["citizens"]:
                ...
                self._set_headers(200)
                self.wfile.write(
                    json.dumps({"status": "Cidadão excluído com sucesso"}).encode()
                )
            else:
                self._set_headers(404)
                self.wfile.write(
