...
# Fixtures imported from root conftest.py:
# - db_session: AsyncSession with PostgreSQL transactional isolation
# - test_engine: PostgreSQL async engine
# - event_loop: asyncio event loop
# - anyio_backend: asyncio configuration
...
def test_create_and_start_workflow(client, random_ids):
    ...
    # 3) Execute transition to final
    r = client.post(
        f"/api/v1/workflow/instances/{instance_id}/transitions", 
        json={"transition_code": "TO_DONE"}
    )
    assert r.status_code == 200
    inst3 = r.json()
    # final state should set completed_at or status COMPLETED
...
def test_task_lifecycle(client, random_ids):
    ...
    # If tasks present, assign and complete one
    if items:
        task = items[0]
        task_id = task.get("id")
        ...
        # Assign task
        r = client.post(
            f"/api/v1/workflow/tasks/{task_id}/assign", 
            json={"user_id": random_ids["created_by"]}
        )
        assert r.status_code == 200
