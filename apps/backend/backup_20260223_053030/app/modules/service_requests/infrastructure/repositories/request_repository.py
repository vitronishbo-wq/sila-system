...
class RequestRepository(RequestRepositoryPort):
    ...
    async def save(self, request: ServiceRequest) -> ServiceRequest:
        ...
        stmt = select(ServiceRequestModel).where(
            ServiceRequestModel.id == request.id
        )
    ...
    async def get_by_citizen(self, citizen_id: UUID, skip: int = 0, limit: int = 100) -> Tuple[List[ServiceRequest], int]:
        ...
        count_stmt = select(func.count()).select_from(ServiceRequestModel).where(
        ...
        stmt = select(ServiceRequestModel).where(
            ServiceRequestModel.citizen_id == citizen_id
        ).order_by(desc(ServiceRequestModel.created_at)).offset(skip).limit(limit)
        result = await self.db.execute(stmt)
        models = result.scalars().all()
        return [self._to_domain(m) for m in models], total
    
    async def get_by_assignee(self, user_id: UUID, status: Optional[ServiceRequestStatus] = None,
        ...
        stmt = select(ServiceRequestModel).where(where_clause).order_by(
            desc(ServiceRequestModel.priority),
            desc(ServiceRequestModel.created_at)
    ...
    async def search(self, query: str, filters: dict = None,
        ...
        stmt = select(ServiceRequestModel).where(combined_where).order_by(
            desc(ServiceRequestModel.created_at)
        ).offset(skip).limit(limit)
        result = await self.db.execute(stmt)
        models = result.scalars().all()
    ...
    async def get_next_sequence(self, year: int) -> int:
        ...
        pattern = f"SR/{year}/%"
        stmt = select(func.count()).select_from(ServiceRequestModel).where(
            ServiceRequestModel.request_number.like(pattern)
        )
        result = await self.db.execute(stmt)
    ...
    def _to_domain(self, model: ServiceRequestModel) -> Optional[ServiceRequest]:
        ...
        # Handle legacy priority values ("MEDIUM" → "MEDIA", etc)
        ...
        request = ServiceRequest(
            ...
            channel=RequestChannel(model.channel),
