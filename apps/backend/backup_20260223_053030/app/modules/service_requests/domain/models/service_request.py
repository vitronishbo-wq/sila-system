...
class ServiceRequest:
    ...
    # Identidades (FKs para IAM e Citizen)
    citizen_id: UUID = field(default_factory=uuid4)
    # Backwards-compatibility: accept `created_by` keyword used in older code/tests
    ...
    def __post_init__(self):
        # Backwards compatibility: map legacy `created_by` param to
        # `created_by_user_id` when provided.
        if self.created_by is not None:
            self.created_by_user_id = self.created_by
        
        # Type validation: ensure enums are not strings
        if isinstance(self.service_type, str):
            # Handle legacy service type values
            legacy_service_map = {
    ...
    def change_status(self, new_status: ServiceRequestStatus, changed_by: UUID, 
