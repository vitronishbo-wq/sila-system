...
class CitizenProjector:
    ...
    def apply_event(self, event: CitizenEventModel):
        ...
        handler_map = {
            EventType.BIRTH_REGISTRATION: self._handle_birth,
            EventType.ADDRESS_UPDATE: self._handle_address_update,
            EventType.VITAL_STATUS_CHANGE: self._handle_vital_status_change,
            EventType.ID_CARD_ISSUED: self._handle_id_card,
        }
    ...
    def _handle_birth(self, event: CitizenEventModel):
        ...
        result = self.db.execute(
            select(CitizenFUC).where(CitizenFUC.citizen_id == event.citizen_id)
        )
