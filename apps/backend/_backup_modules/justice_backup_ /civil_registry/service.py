...
class CitizenService:
    ...
    async def get_citizen_data(self, citizen_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            citizen_data = {'id': str(citizen.citizen_id), 'full_name': citizen.full_name, 'status': safe_get(citizen, 'vital_status', 'ACTIVE'), 'document_type': 'BI', 'document_number': safe_get(citizen, 'document_number'), 'birth_date': safe_isoformat(safe_get(citizen, 'birth_date')), 'gender': safe_get(citizen, 'gender'), 'nationality': safe_get(citizen, 'nationality'), 'email': safe_get(citizen, 'email'), 'phone': safe_get(citizen, 'phone'), 'nif': safe_get(citizen, 'nif'), 'vital_status': safe_get(citizen, 'vital_status'), 'updated_at': datetime.utcnow().isoformat()}
    ...
    async def validate_citizen(self, citizen_id: str) -> bool:
        ...
        try:
            ...
            if status != CitizenStatus.ACTIVE:
                logger.warning(f'Cidadão {citizen_id} validação falhou: status={status}, esperado={CitizenStatus.ACTIVE}')
                event = CitizenValidationFailed(citizen_id=citizen_id, reason='STATUS_INACTIVE', details={'current_status': status, 'expected_status': CitizenStatus.ACTIVE})
                await self.event_publisher.publish(event)
                from app.core.audit import audit_log
                await audit_log(action='CITIZEN_VALIDATION_FAILED', resource_id=citizen_id, resource_type='CitizenFUC', new_value={'reason': 'STATUS_INACTIVE', 'current_status': status}, db=self.db)
                return False
            event = CitizenValidated(citizen_id=citizen_id, status=status, full_name=citizen_data.get('full_name', ''), document_type=citizen_data.get('document_type', ''))
            await self.event_publisher.publish(event)
            from app.core.audit import audit_log
            await audit_log(action='CITIZEN_VALIDATION_SUCCESS', resource_id=citizen_id, resource_type='CitizenFUC', new_value={'status': status, 'full_name': citizen_data.get('full_name')}, db=self.db)
            ...
            event = CitizenValidationFailed(citizen_id=citizen_id, reason='NOT_FOUND', details={'error': str(e)})
            await self.event_publisher.publish(event)
            from app.core.audit import audit_log
            await audit_log(action='CITIZEN_NOT_FOUND', resource_id=citizen_id, resource_type='CitizenFUC', new_value={'error': str(e)}, db=self.db)
            return False
        except Exception as e:
            logger.exception(f'Erro inesperado ao validar cidadão {citizen_id}: {str(e)}')
            event = CitizenValidationFailed(citizen_id=citizen_id, reason='INTERNAL_ERROR', details={'error': str(e), 'type': type(e).__name__})
            await self.event_publisher.publish(event)
            from app.core.audit import audit_log
            await audit_log(action='CITIZEN_VALIDATION_ERROR', resource_id=citizen_id, resource_type='CitizenFUC', new_value={'error': str(e), 'type': type(e).__name__}, db=self.db)
            return False
