...
class StructuredLogger:
    ...
    def audit_context(self, operation: str, **kwargs):
        ...
        audit_log = {'audit_id': audit_id, 'operation': operation, 'citizen_fuc_id': citizen_fuc_id, 'operator_id': operator_id, 'timestamp': datetime.utcnow().isoformat(), 'extra': kwargs}
        self.logger.info(f'AUDIT_START: {operation}', extra={'audit': audit_log})
        try:
            yield audit_log
            self.logger.info(f'AUDIT_SUCCESS: {operation}', extra={'audit': audit_log})
        except Exception as e:
            audit_log['error'] = str(e)
            audit_log['error_type'] = type(e).__name__
            self.logger.error(f'AUDIT_FAILURE: {operation}', extra={'audit': audit_log}, exc_info=True)
            raise
