...
class Citizen:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'citizen_id': str(self.citizen_id), 'full_name': self.full_name, 'document_number': self.document_number, 'birth_date': self.birth_date.isoformat() if isinstance(self.birth_date, date) else None, 'age': self.age, 'gender': self.gender, 'phone': self.phone, 'email': self.email, 'vital_status': self.vital_status, 'fuc_sync_timestamp': self.fuc_sync_timestamp.isoformat() if isinstance(self.fuc_sync_timestamp, datetime) else None, 'created_at': self.created_at.isoformat() if isinstance(self.created_at, datetime) else None, 'updated_at': self.updated_at.isoformat() if isinstance(self.updated_at, datetime) else None}
    ...
    def from_fuc_projection(cls, data: Dict[str, Any]) -> 'Citizen':
        ...
        citizen = cls(citizen_id=citizen_id, full_name=full_name, document_number=doc, birth_date=birth, gender=gender, phone=phone, email=email, vital_status=vital, fuc_sync_timestamp=datetime.utcnow())
        return citizen