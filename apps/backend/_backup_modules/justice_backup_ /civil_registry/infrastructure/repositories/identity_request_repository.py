...
class IdentityRequestModel(Base):
    ...
    def to_domain(self):
        ...
        req = IdentityRequest(id=self.id, citizen_fuc_id=self.citizen_fuc_id, service_code=self.service_code, status=status_val, created_at=self.created_at, updated_at=self.updated_at)
        if self.payload and isinstance(self.payload, dict):
            req.notes = self.payload.get('notes', [])
            req.bi_id = self.payload.get('bi_id')
        return req

class IdentityRequestRepository(IdentityRequestRepositoryPort):
    ...
    async def save(self, identity_request) -> Any:
        ...
        payload_data = {'notes': getattr(identity_request, 'notes', []), 'bi_id': str(identity_request.bi_id) if getattr(identity_request, 'bi_id', None) else None}
        ...
        if obj:
            ...
            obj = IdentityRequestModel(id=identity_request.id, citizen_fuc_id=identity_request.citizen_fuc_id, service_code=identity_request.service_code, status=status_val, payload=_sanitize_for_json(payload_data), created_at=getattr(identity_request, 'created_at', datetime.utcnow()))
    ...
    async def get_pending(self):
        ...
        stmt = select(IdentityRequestModel).where(IdentityRequestModel.status.in_(['PENDING', 'PENDING_FUC_VALIDATION', 'FUC_VALIDATION']))
        result = await self.session.execute(stmt)
