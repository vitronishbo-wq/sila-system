...
def get_alter_data_service(db: AsyncSession=Depends(get_db), fuc_client: CitizenFUCClient=Depends(get_fuc_client)):
    repository = IdentityRequestRepository(db)
    return AlterDataService(ir_repository=repository, fuc_client=fuc_client)

@router.post('/sincronizar-fuc', status_code=status.HTTP_200_OK)
async def sync_bi_with_fuc(payload: Dict[str, Any], current_user: User=Depends(get_current_user), service: AlterDataService=Depends(get_alter_data_service)):
    """
    ...
    result = await service.execute_data_sync(citizen_fuc_id=citizen_id, operator_id=operator_id, updated_fields=data_to_sync)
...
async def check_fuc_connection(citizen_id: str):
    ...
    return {'status': 'SYNCED', 'last_sync': projection.sync_timestamp.isoformat(), 'is_alive': projection.is_alive}