...
class BIEventHandler:
    ...
    def __init__(self):
        self._handlers: Dict[BIEventType, Callable] = {BIEventType.REQUEST_CREATED: self._handle_request_created, BIEventType.DATA_VERIFIED_FUC: self._handle_fuc_validation, BIEventType.BI_PROJECTION_SYNCED: self._handle_projection_sync, BIEventType.BI_APPROVED: self._handle_approval, BIEventType.BI_PRINTED: self._handle_printing, BIEventType.BI_ISSUED: self._handle_issuance, BIEventType.BI_RENEWED: self._handle_renewal, BIEventType.BI_CANCELLED: self._handle_cancellation, BIEventType.LOST_REPORTED: self._handle_loss_report}
    ...
    async def _handle_fuc_validation(self, event: BIEvent):
        ...
        logger.debug('Workflow: Dados biográficos confirmados pela Soberania FUC.')
