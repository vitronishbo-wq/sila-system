...
from .audit_engine import get_audit_engine, AuditAction
...
class AuditMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        method_map = {'GET': AuditAction.READ, 'POST': AuditAction.CREATE, 
                      'PUT': AuditAction.UPDATE, 'DELETE': AuditAction.DELETE}
        ...
        try:
            ...
            await self.audit_engine.complete(audit_record.audit_id, result_code=response.status_code)
            return response
        except Exception as e:
            await self.audit_engine.fail(audit_record.audit_id, error_code=500, error_message=str(e))
            raise
