...
from ..policies.policy_engine import AccessDecision, PolicyEngine
    ...
    def require_role(self, *required_roles: str) -> Callable:
        ...
        async def dependency(request: Request) -> dict:
            ...
            if not user_roles.intersection(required_set):
                raise HTTPException(
                    status_code=403, detail=f"Requires one of: {', '.join(required_set)}"
                )
        ...
        async def dependency(request: Request) -> dict:
            ...
            if require_all:
                if not roles_set.issubset(user_roles):
                    raise HTTPException(
                        status_code=403, detail=f"Requires all roles: {', '.join(roles)}"
                    )
            else:
                if not user_roles.intersection(roles_set):
                    raise HTTPException(
                        status_code=403, detail=f"Requires one of: {', '.join(roles)}"
                    )
        ...
        async def dependency(request: Request) -> dict:
            ...
            try:
                ...
                if not check_fn(user_dict, request_data):
                    raise HTTPException(
                        status_code=403, detail="Access denied: resource attribute check failed"
                    )
