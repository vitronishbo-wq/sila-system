...
class PolicyEngine:
    ...
    def register_policy(
        ...
        try:
            ...
            raise ValueError(f"Unknown action: {action}. Use PolicyAction enum.") from err
    ...
    def authorize(
        ...
        if cache_key in self._cache:
            return AccessDecision(allowed=self._cache[cache_key], reason="Cached decision")

        # Fast path: superuser always allowed
        if user_roles.intersection(self.SUPERUSER_ROLES):
            decision = AccessDecision(
                allowed=True, reason="Superuser role", policy_matched="SUPERUSER_ELEVATION"
            )
        ...
        if resource not in self.policies:
            decision = AccessDecision(allowed=True, reason="No policy defined (permissive default)")
            self._cache[cache_key] = decision.allowed
            return decision
        ...
        if action.lower() not in self.policies[resource]:
            decision = AccessDecision(
                allowed=True, reason=f"No policy for action '{action}' (permissive default)"
            )
            self._cache[cache_key] = decision.allowed
            return decision
    ...
    def get_policy(self, resource: str, action: str) -> ResourcePolicy | None:
        """Get a specific policy definition"""
        if resource in self.policies:
            return self.policies[resource].get(action.lower())
        return None

    def list_policies(self, resource: str | None = None) -> list[ResourcePolicy]:
        """
