...
from app.infrastructure.event_sourcing.registry import EventRegistry, register_events
from app.modules.justice.domain.events import CitizenCreated
...
if hasattr(reconstructed, 'first_name'):
    ...
    print(f"  - Has first_name? NO (only has: {', '.join([k for k in reconstructed.__dict__.keys() if not k.startswith('_')])})")
