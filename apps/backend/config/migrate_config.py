...
class ConfigMigrator:
    ...
    def analyze_imports(self, file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            old_imports = {
                ...
                "from config.settings import settings": False,
            ...
            return {"file": str(file_path), "error": str(e), "needs_migration": False}
    ...
    def create_migration_report(self, results: dict[str, Any]) -> str:
        ...
        if results["dry_run"]:
            ...
            report.append("Run again with dry_run=False to apply changes")

        # Files that need migration
        files_needing_migration = [
            r for r in results["analysis_results"] if r.get("needs_migration", False)
        ]
        ...
        if results["files_to_migrate"] > 0:
            report.append("  - Review migrated files for any custom logic")
            report.append("  - Update imports to use specific config functions")
            report.append("  - Test migrated modules thoroughly")
        else:
            report.append("  - All files are already using the new configuration system")

        report.append("  - Update documentation to reflect new configuration structure")
        report.append("  - Remove old core.settings.py file after migration is complete")
...
def main():
    ...
    parser.add_argument(
        "--backend-path", default="/opt/sila-system/backend", help="Path to backend directory"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if (
        results["files_to_migrate"] > 0
        and results["successful_migrations"] < results["files_to_migrate"]
    ):
        sys.exit(1)
