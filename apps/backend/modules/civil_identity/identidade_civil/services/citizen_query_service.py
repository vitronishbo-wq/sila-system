...
class CitizenQueryService:
    ...
    async def get_by_fuc_id(self, citizen_fuc_id: str) -> Optional[FucSovereigntyProjection]:
        ...
        """
        async with logger_query.audit_context(
            "CITIZEN_QUERY",
            citizen_fuc_id=citizen_fuc_id,
        ):
            logger_query.info(
                "Consultando soberania",
                citizen_fuc_id=citizen_fuc_id,
            )
    ...
    async def search_citizens(self, criteria: Dict[str, Any]) -> List[FucSovereigntyProjection]:
        ...
        async with logger_query.audit_context(
            "CITIZEN_SEARCH",
            search_criteria=list(criteria.keys()),
        ):
            logger_query.info(
                "Pesquisa biográfica iniciada",
                criteria_keys=list(criteria.keys()),
            )
            
            # Em uma implementação real, o cliente FUC teria um método de busca robusto
    ...
    async def verify_vital_status(self, citizen_fuc_id: str) -> bool:
        ...
        async with logger_query.audit_context(
            ...
            logger_query.info(
                "Verificando estado vital",
                citizen_fuc_id=citizen_fuc_id,
            )
            
            projection = await self.get_by_fuc_id(citizen_fuc_id)
            if not projection:
                logger_query.warning(
                    "Cidadão não encontrado para verificação de estado vital",
                    citizen_fuc_id=citizen_fuc_id,
                )
                return False
            
            # Verificação de status vital (simplificado)
            is_alive = getattr(projection, "vital_status", "alive") == "alive"
            logger_query.info(
                "Estado vital verificado",
                citizen_fuc_id=citizen_fuc_id,
                is_alive=is_alive,
            )
            return is_alive
    ...
    async def get_citizen_documents(self, citizen_fuc_id: str) -> List[Dict[str, Any]]:
        ...
        """
        async with logger_query.audit_context(
            "CITIZEN_DOCUMENTS_QUERY",
            citizen_fuc_id=citizen_fuc_id,
        ):
            logger_query.info(
                "Consultando documentos do cidadão",
                citizen_fuc_id=citizen_fuc_id,
            )
            ...
            # Documentos são consultas do FUC
        ...
        projection = await self.get_by_fuc_id(citizen_fuc_id)
        return projection.is_alive if projection else False
