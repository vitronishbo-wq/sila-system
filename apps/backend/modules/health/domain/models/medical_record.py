...
@dataclass
class MedicalRecord:
    """Registro médico - Aggregate Root"""
    ...
    diagnosis_codes: List[str] = field(default_factory=list)  # CID-10
    ...
    def add_diagnosis(self, diagnosis: str, code: Optional[str] = None):
    ...
    def add_prescription(self, prescription_id: UUID):
        """Adiciona referência de prescrição"""
        if prescription_id not in self.prescription_ids:
            self.prescription_ids.append(prescription_id)
            self.updated_at = datetime.now()
    
    def add_exam_request(self, exam_request_id: UUID):
        """Adiciona referência de pedido de exame"""
        if exam_request_id not in self.exam_request_ids:
            self.exam_request_ids.append(exam_request_id)
            self.updated_at = datetime.now()
    
    def update_vitals(self, vitals: VitalSigns):
