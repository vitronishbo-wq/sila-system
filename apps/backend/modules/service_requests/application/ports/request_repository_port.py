...
class RequestRepositoryPort(ABC):
    ...
    @abstractmethod
    def get_by_id(self, request_id: UUID) -> Optional[ServiceRequest]:
        """Busca pedido por ID"""
        pass
    
    @abstractmethod
    def get_by_number(self, request_number: str) -> Optional[ServiceRequest]:
        """Busca pedido por número"""
        pass
    
    @abstractmethod
    def get_by_citizen(self, citizen_id: UUID, skip: int = 0, limit: int = 100) -> Tuple[List[ServiceRequest], int]:
        """Busca pedidos por cidadão"""
        pass
    
    @abstractmethod
    def get_by_assignee(self, user_id: UUID, status: Optional[ServiceRequestStatus] = None,
                        skip: int = 0, limit: int = 100) -> Tuple[List[ServiceRequest], int]:
        """Busca pedidos atribuídos a um usuário"""
        pass
    
    @abstractmethod
    def get_by_status(self, status: ServiceRequestStatus, skip: int = 0, limit: int = 100) -> Tuple[List[ServiceRequest], int]:
        """Busca pedidos por status"""
        pass
    
    @abstractmethod
    def get_by_service_type(self, service_type: ServiceType, skip: int = 0, limit: int = 100) -> Tuple[List[ServiceRequest], int]:
