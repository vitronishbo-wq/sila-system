...
export const useDocumentUpload = () => {
    ...
    const uploadFile = async (file: File) => {
        ...
        // Backend espera um array de ficheiros em "files" para suportar upload múltiplo
        ...
        try {
            ...
            // O endpoint pode devolver um único ID ou uma lista de IDs (caso multiple)
            const documentId = response?.id ?? response?.document_id ?? (Array.isArray(response?.ids) ? response.ids[0] : undefined);
            ...
            // Inicia SSE para este documento específico (autenticação via query param)
            if (documentId) {
    ...
    // Upload em lote (utiliza o mesmo endpoint /documents/upload)
    ...
    return { uploads, uploadFile, uploadFiles, cancelUpload };
};
