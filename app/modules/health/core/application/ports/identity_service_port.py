"""
SILA 3.0: Unified Identity Service Port (Health Module)
Rule 5: No Coupling - Abstract Port for DID-based Authentication
Status: Production-ready for Dependency Injection

This port is IDENTICAL across Health and Education modules,
...
class IdentityServicePort(ABC):
    ...
    async def verify_citizen_by_did(self, did: str) -> VerificationResult:
        ...
        Example Usage (Health):
            ...
            result = await identity_port.verify_citizen_by_did("did:sila:patient123")
            if result.is_valid:
                patient = result.citizen  # Only safe fields exposed
                appointment = Appointment(citizen_id=patient.citizen_id, ...)
    ...
    async def validate_credential_issued_by(self, did: str, issuer_did: str) -> bool:
        ...
        Args:
            ...
            issuer_did: DID of credential issuer (e.g., Ministry of Health)
        
        Returns:
            True if citizen holds valid credential from issuer
        
        Example Usage (Health):
            is_healthcare_professional = await identity_port.validate_credential_issued_by(
                did="did:sila:doctor123",
                issuer_did="did:sila:ministry_health"
            )
            if is_healthcare_professional:
                # Allow doctor to access patient records
